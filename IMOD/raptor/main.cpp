...
int main(int argc, char* argv[])
{
    ...
    if (!PipGetBoolean("usage", &error)) {
      PipPrintHelp("RAPTOR", 0, 0, 0);
      exit(0);
    }
    char* input_;
    char *path_;
    ...
    */
    ...
    posDot = basename.find_last_of("_preali");
    if (posDot == basename.length() - 1)
      basename = basename.substr(0, posDot - 6);
