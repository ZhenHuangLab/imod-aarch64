...
program xyzproj
  ...
  real*4 fill, dmin2, dmax2, dmean2, dsum, angle, projMiddle, adjustAngles
  ...
  logical*4 commonLine, fullImage, haveAngles, useIntersections
  real*4 sind, cosd
  ...
  !
  if (pipInput) then
    ierr = PipGetLogical('UseRayIntersections', useIntersections)
    ...
    if (commonLine .and. useIntersections) call exitError &
        ('You cannot use intersection areas when input is a tilt series')
    ...
    if (ixLoad1 > 0 .and. PipGetFloat('AdjustFileAngles', adjustAngles) == 0) then
      do i = 1, nview
        tiltAngles(i) = tiltAngles(i) + adjustAngles
      enddo
    endif
...
301 format('XYZPROJ: x',2i5,', y',2i5,', z ',2i5,' about ',a1,t57,a9,2X,a8)
  ...
  do iload = 1, numLoads
    ...
    do iproj = 0, nzOut - 1
      ...
      if (useIntersections) then
        
        call useIntersectionAreas()

      !
      ! for projections at an angle, process multiple slices at once
      else if (sinAng(iproj) .ne. 0.) then
        !
        ...
        do ixOut = 1, nxout
          ...
          if (nraypts > 0) then
            ...
            do iray = 0, nraypts - 1
              ...
              ixy4 = ixy - 1
              ixy6 = ixy + 1
              ixy8 = ixr + min(iyr, nySlice - 1) * nxSlice
              ixy2 = ixr + max(iyr - 2, 0) * nxSlice
              !
...
CONTAINS
  ...
  ! useIntersectionAreas will project using areas of intersection between rays and pixel
  !
  subroutine useIntersectionAreas()
    integer MAX_DIST
    parameter (MAX_DIST = 1000)
    integer*4 indDelRay(MAX_DIST + 1), numRaysHit(MAX_DIST + 1)
    real*4 rayAreas(3 * (MAX_DIST + 1))
    integer*4 iySlice, ixStart, ixEnd, ixLeft, ixRight, ixSlice, lutInd, rayInd, iray
    integer*4 indOut
    real*4 rxOut, rayDist, area, ypart

    ! Get the angle with the usual inversion, then make table of intersection areas
    angle = invertAng * (tiltStart + iproj * tiltInc)
    if (.not. commonLine .and. haveAngles) angle = invertAng * tiltAngles(iproj + 1)
    call makeRayAreaLookupTable(angle, 1, MAX_DIST, 0.01, indDelRay, numRaysHit, &
        rayAreas)
    ! 
    ! Loop on slices in Y and set starting and ending points
    do iySlice = 1, nySlice
      ixStart = 1
      ixEnd = nxSlice
      ypart = sinAng(iproj) * (iySlice - 0.5 - nySlice / 2.)
      if (cosAng(iproj) .ne. 0.) then
        ixLeft = (ypart - nxout / 2.) / cosAng(iproj) + nxSlice / 2. + 0.5
        ixRight = (ypart + nxout / 2.) / cosAng(iproj) + nxSlice / 2. + 0.5
        ixStart = min(ixLeft, ixRight)
        ixEnd = max(ixLeft, ixRight)
      endif
      ixStart = max(1, ixStart)
      ixEnd = min(nxOut, ixEnd)
      !
      ! Loop across each line, get the output coordinate (0 to nxout) of each point
      do ixSlice = ixStart, ixEnd
        rxOut = cosAng(iproj) * (ixSlice - 0.5 - nxSlice / 2.) - ypart + nxOut / 2.
        if (cosAng(iproj) == 0)  &
            rxOut = sign(iySlice - (nySlice + 1.) / 2., sinAng(iproj)) + nxOut / 2.
        if (rxOut >= 0 .and. rxOut <= nxOut) then
          !
          ! If with limits, get fraction and index in table
          ixOut = rxOut + 1.
          rayDist = ixOut - rxOut
          lutInd = rayDist * MAX_DIST + 1
          lutInd = min(MAX_DIST, lutInd)
          !
          ...
          do rayInd = 1, numRaysHit(lutInd)
            iray = ixOut + indDelRay(lutInd) + rayInd - 1
            if (iray > 0 .and. iray <= nxOut) then
              area = rayAreas(3 * (lutInd - 1) + rayInd)
              ixy = ixSlice  + (iySlice - 1) * nxSlice
              indOut = iOutBase + iray - 1
              do ipix = 1, numSlices
                array(indOut) = array(indOut) + area * array(ixy)
                ixy = ixy + istackDel
                indOut = indOut + nxOut
              enddo
            endif
    ...
    do ixOut = 1, nxOut
      rayAdd = scaleAdd * scaleFac + rayFac * (nrayMax(iproj) -  &
          nrayInc(ixOut + irayBase)) * (fill / scaleFac - scaleAdd)
      indOut = iOutBase + ixOut - 1
      do ipix = 1, numSlices
        array(indOut) = rayFac * array(indOut) + rayAdd
        indOut = indOut + nxOut
      enddo
    enddo
    return
  end subroutine useIntersectionAreas
