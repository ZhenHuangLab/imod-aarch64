...
program subimage
  ...
  real*4 cmin, cmax, tmean, sd, lowThresh, highThresh, sdLimit, diffLimit, range
  real*8 totPixels, tsum, tsumSq, sdSum, sdSumSq, sum, sumSq, diffMean, realSum
  integer*4 modeOut, ifZeroMean, ifLowThresh, ifHighThresh, maxUnitOut, ifRangeFrac
  ...
  do isec = 1, numAsec
    ...
    if (ifZeroMean .ne. 0) then
      call iiuSetPosition(1, asec, 0)
      call iiuSetPosition(2, bsec, 0)
      sum = 0.
    ...
    do iChunk = 1, numChunks
      ...
      if (cfile .ne. ' ') call iiuWriteLines(3, array, numLines)
  ...
  if (numAsec > 1) then
    do i = 6, maxUnitOut
      ...
      if (ifRangeFrac > 0 .and. range > 0.)  &
          write(i, 6000) abs(dmin / range), dmax / range, abs(dmean / range), sd / range
    enddo
  endif
  ...
  call iiuWriteHeader(3, title, 1, realMin, realMax, realMean)
...
6000 format(' range frac',f9.6, 3f15.6)
