...
30 if (pipInput) then
    ...
    if (indPipOpt == 23) then
      if (PipGetString('RemoveTitles', string) == 0) then
        call parselist2(string, listdel, ndel, 1000)
        if (ndel == 1 .and. listdel(1) <= 0)  &
            call exitError('Title number to remove must be positive')
        go to 10
      endif
    endif
    if (indPipOpt == 24) then
      if (PipGetInteger('PositionForTitle', j) == 0) then
        if (j <= 0) call exitError('Position for title to add must be positive')
        ndel = 1
        listdel(1) = 1 - j
        ifAddTitle = -2
      endif
    endif
    if (indPipOpt == 25) then
      if (PipGetString('TitleToAdd', string) == 0) then
        read(string, '(20a4)') (title(j, 1), j = 1, 20)
        if (ifAddTitle < -1) go to 10
        ifAddTitle = 1
      endif
    endif
    if (indPipOpt == 26) go to 15
    go to 30
  else
    write(*,102)
