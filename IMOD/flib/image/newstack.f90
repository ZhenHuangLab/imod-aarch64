...
program newstack
  ...
  real*4 optFloatRange, optFloatMin, floatZmargin, floatInterpRangeFac
  ...
  integer*4 ifContrast, ifScaleMM, ifFloatDen, ifMapRange, iHdfCompression
  ...
  real*4 physicalMem, ftExpandFac, dmapLow, dmapHigh
  ...
  options(1) = &
      ...
      'format:FormatOfOutputFile:CH:@compression:HDFCompressionIndex:I:@'// &
      'volumes:VolumesToRead:LI:@3d:Store3DVolumes:I:@chunk:ChunkSizesInXYZ:IT:@'// &
      'mdoc:UseMdocFiles:B:@tilt:TiltAngleFile:FN:@reorder:ReorderByTiltAngle:I:@'// &
      'angle:AngleFileToReorder:FN:@newangle:NewAngleOutputFile:I:@'// &
      'secs:SectionsToRead:LIM:@samesec:SameSectionsToRead:B:@'// &
      ...
      'onexform:OneTransformPerFile:B:@phase:PhaseShiftFFT:B:@'// &
      'rotate:RotateByAngle:F:@expand:ExpandByFactor:F:@shrink:ShrinkByFactor:F:@'// &
      'antialias:AntialiasFilter:I:@bin:BinByFactor:I:@'// &
      'ftreduce:FourierReduceByFactor:F:@ftexpand:FourierExpandByFactor:F:@'// &
      'noise:NoisePadForFFT:B:@distort:DistortionField:FN:@'// &
      'imagebinned:ImagesAreBinned:F:@fields:UseFields:LIM:@'// &
      'subarea:SubareaOffsetsXandY:FAM:@gradient:GradientFile:FN:@'// &
      'origin:AdjustOrigin:B:@linear:LinearInterpolation:B:@'// &
      'nearest:NearestNeighbor:B:@size:SizeToOutputInXandY:IP:@mode:ModeToOutput:I:@'// &
      'bytes:BytesSignedInOutput:I:@strip:StripExtraHeader:B:@'// &
      'float:FloatDensities:I:@meansd:MeanAndStandardDeviation:FP:@'// &
      'contrast:ContrastBlackWhite:IP:@scale:ScaleMinAndMax:FP:@map:MapFromRange:FP:@'// &
  ...
  if (pipInput) then
    ...
    ierr = PipGetInteger('HDFCompressionIndex', iHdfCompression)
  endif
  !
  nxMax = 0
  nyMax = 0
...
132 format('(i',i1,'.',i1,')')
  ...
  if (pipinput) then
    ...
    ierr = PipGetFloat('FourierExpandByFactor', ftExpandFac)
    ierr = PipGetLogical('NoisePadForFFT', noisePad)
    fourierScaling = ftReduceFac > 0. .or. ftExpandFac > 0.
    if ((phaseShift .or. fourierScaling) .and. ifLinear .ne. 0) &
        call exitError('You cannot enter -phase, -ftreduce, or -ftExpand with -linear'// &
        ' or -nearest')
    if (ftReduceFac > 0 .and. ftReduceFac <= 1.) call exitError( &
        'Factor for reducing with FFT must be > 1')
    if (ftExpandFac > 0. .and. ftExpandFac <= 1.) call exitError( &
        'Factor for expanding with FFT must be > 1')
    if (ftReduceFac > 0 .and. ftExpandFac > 0.) call exitError( &
        'You cannot enter both -ftreduce and -ftexpand')
    if (fourierScaling) phaseShift = .false.
  else
  ...
  if (pipinput) then
    ...
    ifMeanSdEntered = 1 - PipGetTwoFloats('MeanAndStandardDeviation', enteredMean, &
        enteredSD)
    ifMapRange = 1 - PipGetTwoFloats('MapFromRange', dmapLow, dmapHigh)
    call PipNumberOfEntries('MultiplyAndAdd', numScaleFacs)
    if (ifMapRange .ne. 0 .and. ifContrast + ifScaleMM == 0) call exitError &
    ...
    if (ifFloat >= 4) then
      ...
      if (ifMapRange .ne. 0) call exitError('You cannot use -map with -float 4')
    else
...
102 format(' Enter 0 for no floating',/,8x, &
  ...
  if (pipinput) then
    ...
    if (phaseShift .or. fourierScaling) then
      if (.not. sizesMatch) call exitError( &
          'All input files must have the same size in x and y with -phase or -ftreduce')
      if (fourierScaling .and. (rotateAngle .ne. 0. .or. &
          expandFactor .ne. 0. .or. idfFile .ne. ' ' .or. magGradFile .ne. ' ' .or. &
          ifWarping .ne. 0 .or. readReduction > 1.))  &
      ...
      if (applyFirst .ne. 0 .and. ftExpandFac < 1.) call exitError( &
          'You cannot use -applyfirst with -phase or -ftreduce')
      !
      ...
      if (fourierScaling) then
        tmpMin = ftReduceFac
        if (ftExpandFac > 0.) tmpMin = 1. / ftExpandFac
        ierr = fourierCropSizes(nxFirst, tmpMin, 0.01, 16, niceFFTlimit(), nxFSpad, &
            nxFCropPad, actualFac)
        if (ierr > 0) call exitError('Reduction or expansion factor must be an '// &
            'integer or integer divided by 2, 3, 4, 5, 6, 8, or 10 to 3 decimal places')
        ierr = fourierCropSizes(nyFirst, tmpMin, 0.01, 16, niceFFTlimit(), nyFSpad, &
            nyFCropPad, actualFac)
        if (iVerbose > 0) print *,'Actual factor',actualFac
        !
        ! When cropping, reduce the shifts here for use when extracting image area; the
        ! fractional part will be boosted back up for the shift in the crop
        if (ftReduceFac > 1. .or. applyfirst .ne. 0) then
          do i = 1, listTotal
    ...
    if (PipGetString('ReplaceSections', listString) == 0) then
      ...
      if (numReplace > 0) then
        ...
        if (numSecLists > 0) then
          if (nlist(1) .ne. numReplace) call exitError( &
              'You must specify the same number of input sections as ones to replace')
        else
          if (nlist(1) .lt. numReplace) call exitError( &
  ...
  if (fourierScaling) expandFactor = 1. / actualFac
  if (expandFactor == 0.) expandFactor = 1.
  !
  ...
  if (readReduction > 1.) then
    ...
    idimInOut = limToAlloc - lenTemp
  else if (fourierScaling .and. nxFSpad > 0) then
  ...
  do iFile = 1, numInFiles
    ...
    do ilist = 1, nlist(iFile)
      ...
      if (numReplace == 0 .and. isecOut == 1) then
        ...
        !
        !  Set HDF compression
        if (iHdfCompression >= 0 .and. iiuFileType(2) == 5)  &
            call iiuSetHDFCompression(2, iHdfCompression)
...
302     format('NEWSTACK: Images copied',a13,a18,t57,a9,2x,a8)
      ...
      if (ifFloat == 2) then
        optFloatRange = optimalOut
        optFloatMin = 0
        floatZmargin = 0.
        if ((ifXform .ne. 0 .or. phaseShift .or. fourierScaling) .and. &
            (newMode == 1 .or. newMode == 6)) then
          optFloatMin = optFloatMin + floatInterpRangeFac * optFloatRange
          optFloatRange = (1. - 2. * floatInterpRangeFac) * optFloatRange
          floatZmargin = floatInterpRangeFac * (zmax - zmin)
        endif
      endif
      !
      ...
      if (numChunks > 1 .and. (phaseShift .or. fourierScaling)) &
          call exitError('Cannot apply Fourier operations - input and output images'// &
      ...
      if (numChunks == 1 .or. (numChunks > 0 .and. numChunks <= maxChunks .and. &
        ...
        do iChunk = 1, numChunks
          ...
          if (processInPlace .and. .not. phaseShift .and. .not. fourierScaling) then
          ...
          if (phaseShift .or. fourierScaling) then
            ...
            if (phaseShift) then
              ...
              if (ftReduceFac > 0.) then
                ...
                call fourierExpandImage(array, nxFSpad, nyFSpad, array(ioutBase), &
                    nxFCropPad, nyFCropPad, dx, dy, &
                    array(1 + (nxFSpad + 2) * nyFSpad))
              endif
            endif
...
CONTAINS
  ...
  subroutine reallocateIfNeeded()
    ...
    if (limEntered .ne. 1) then
      ...
      if (fourierScaling .and. nxFSpad > 0) &
          needTemp = max(needTemp, (int(nxFCropPad, kind = 8) + 2) * nyFCropPad)
      if ((phaseShift .or. fourierScaling) .and. nxFSpad > 0 .and. noisePad) &
    ...
    else if (fourierScaling .and. nxFSpad > 0 .and.  &
    ...
    if (limEntered == 0) then
      ...
      if ((phaseShift .or. fourierScaling) .and. nxFSpad > 0)  &
      ...
      if (nxOut > 0 .and. nyOut > 0) then
        ...
        else if (ftExpandFac > 0.) then
          needDim = needDim + max(int(nxOut, kind = 8) * nyOut, &
              int(nxFCropPad, kind = 8) * nyFCropPad)
        else
  ...
  subroutine findScaleFactors(tmpMinIn, tmpMaxIn)
    ...
    real*4 tmpMean, tmpMinShift, tmpMaxShift, tmpMinIn, tmpMaxIn, useMin, useMax
    real*4 tmpZmin, tmpZmax
    !
    ...
    elseif (ifFloat < 0 .and. numScaleFacs == 0) then
      ...
      useMin = dminIn
      useMax = dmaxIn
      if (ifMapRange .ne. 0) then
        useMin = dmapLow
        useMax = dmapHigh
      endif
      dminOut = (tmpMin - useMin) * (dmaxNew - dminNew) / (useMax - useMin) + dminNew
      dmaxOut = (tmpMax - useMin) * (dmaxNew - dminNew) / (useMax - useMin) + dminNew
    elseif (ifFloat > 0 .and. ifMeanSdEntered == 0) then
      ...
      elseif (ifFloat == 2) then
        ...
        !
        ! If either the min or the max has become MORE extreme due to interpolation than
        ! the margin on the range,
        ...
        ind = ilist + listInd(iFile) - 1
        tmpZmin = (tmpMin - avgSec) / sdSec
        tmpZmax = (tmpMax - avgSec) / sdSec
        if (.not.(iSecRead < 0 .or. iSecRead >= nz .or. newMode == 2) .and.  &
            (tmpZmin < zmin - floatZmargin .or. tmpZmax > zmax + floatZmargin)) then
          boostForMin = 1.
          boostForMax = 1.
          if (avgSec - secMins(ind) > 0.25 * (avgSec - tmpMin)  .and.  &
              tmpZmin < zmin - floatZmargin)  &
              boostForMin = tmpZmin / (zmin - floatZmargin)
          if (secMaxes(ind) - avgSec > 0.25 * (tmpMax - avgSec) .and.  &
              tmpZmax > zmax + floatZmargin)  &
              boostForMax = tmpZmax / (zmax + floatZmargin)
        ...
        dminOut = (zminSec - zmin) * optFloatRange / (zmax - zmin) + optFloatMin
        dmaxOut = (zmaxSec - zmin) * optFloatRange / (zmax - zmin) + optFloatMin
  ...
  ! transferCollections gtes each collection other than ZValue and T and transfers it
  ! from the input autodoc to the output autodoc
  !
  subroutine transferCollections()
    integer*4 numColl, coll, numSect, isect, AdocGetSectionName
    integer*4 AdocGetCollectionName, AdocGetNumCollections, AdocGetNumberOfSections
    character*80 collName
    numColl = AdocGetNumCollections()
    do coll = 1, numColl
      if (AdocGetCollectionName(coll, collName) .ne. 0) call exitError( &
          'Getting collection name for transferring other autodoc sections')
      if (collName .ne. zvalueName .and. collName .ne. 'T') then
        numSect = AdocGetNumberOfSections(collName);
        do isect = 1, numSect
          if (AdocGetSectionName(collName, isect, listString) .ne. 0) &
              call exitError( &
              'Getting section name for transferring other autodoc sections')
          if (AdocTransferSection(collName, isect, indAdocOut,  listString, 0) .ne. 0) &
              call exitError('transferring other autodoc section')
        enddo
      endif
    enddo
  end subroutine transferCollections
