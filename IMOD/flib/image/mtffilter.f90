...
!
  ...
  logical fftInput, filter3d, oneDfilter, rWeight, noisePad, bidirReversed
  ...
  options(1) = &
      ...
      'initial:InitialDose:F:@bidir:BidirectionalNumViews:I:@'// &
      'reversed:ReversedBidirectional:B:@volt:Voltage:I:@'// &
...
20  close(2)
  ...
  if (ifImageDose > 0 .or. idoseFileType > 0) then
    ...
    ierr = PipGetLogical('ReversedBidirectional', bidirReversed)
    if (bidirReversed) invertForBidir = -invertForBidir
  ...
  if (ifImageDose > 0 .or. idoseFileType > 0) titleStr = 'MTFFILTER: Dose weight filtered'
  !
      ...
      do kk = izStart, izEnd
        ...
        if (idoseFileType >= 0) then
          call doseWeightFilter(priorDoses(kk), priorDoses(kk) + secDoses(kk),  &
              ...
              0.71, delta)
          write(*, '(a,i4,a,f9.2)')'View', kk, '  prior dose', priorDoses(kk)
          if (iVerbose > 0) then
