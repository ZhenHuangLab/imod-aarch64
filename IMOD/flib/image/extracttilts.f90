...
program extracttilts
  ...
  if (ifNoImage == 0) then
    ...
    !
    hdfFile = iiuFileType(1) == 5 .or. iiuFileType(1) == 7
    if (ifAttrib > 0 .and. iiuFileType(1) .ne. 5) call exitError( &
    ...
    if (hdfFile .and. ifAddMdoc .ne. 0) then
      ...
      if (indAdoc <= 0) call exitError('Getting autodoc index for HDF or IDOC file')
      if (AdocSetCurrent(indAdoc) .ne. 0) call exitError( &
          'Setting autodoc structure of HDF or IDOC file as current autodoc')
      numSect = nz
      !
      ...
      if (AdocGetImageMetaInfo(montage, numSect, iTypeAdoc) < 0 .or. numSect < nz)  &
          call exitError('This file does not have metadata about each section')
    else
