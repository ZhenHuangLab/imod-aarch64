...
program binvol
  ...
  options(1) = &
      'input:InputFile:FN:@output:OutputFile:FN:@mode:ModeToOutput:I:@'// &
      'binning:BinningFactor:I:@xbinning:XBinningFactor:I:@'// &
      'ybinning:YBinningFactor:I:@zbinning:ZBinningFactor:I:@'// &
      'antialias:AntialiasZFilter:I:@spread:SpreadSlicesInZ:B:@memory:MemoryLimit:I:@'// &
  ...
  if (nz == 1) ibinZ = 1
  !
  ierr = PipGetInteger('ModeToOutput', mode)
  if (mode .ne. 0 .and. mode .ne. 1 .and. mode .ne. 2 .and. mode .ne. 6) call exitError &
      ('The mode of the input or output must be 0, 1, 2, or 6')
  ...
  if (ifiltType > 0 .and. ibinZ > 1) then
...
CONTAINS
  ...
  subroutine sliceWeighting(izIn, weight)
    ...
    real*4 weight
    if (ifiltType <= 0 .or. ibinZ == 1) then
      weight = 1. / ibinZ
      return
    endif
    weight = 0.
