...
program header
  ...
  character*25 extractCom(ntypes) /'extracttilts', 'extractpieces', &
      ...
      'extracttilts -exp'/
  character*16 briefName(ntypes) /'Tilts', 'Piece coords', &
      'Stage positions', 'Mags', 'Intensities', 'Exposure Doses'/
  character*16 computed /'(not computed)'/
  character*14 briefSep /'  Contains:'/
  ...
  real*8 axis8
  integer*4 iiuRetNumVolumes, iiuVolumeOpen, getExtraHeaderValue, tiffGetMaxEERsuperRes
  ...
  options(1) = &
      ...
      'eer:FullSizeOfEERFile:B:@help:usage:B:'
  !
  ! defaults
  !
  doSize = .false.
  ...
  if (pipinput) then
    ...
    ierr = PipGetLogical('FullSizeOfEERFile', doFullEER);
    ...
    if (doFullEER) call tiffSetEERreadProperties(tiffGetMaxEERsuperRes(), 1, 0);
  endif

  silent = doSize .or. doMode .or. doMin .or. doMax .or. doMean .or. doRMS .or. &
      doPixel .or. doOrigin
  if (silent) call iiuAltPrint(0)
  call ialBrief(ifBrief * 2)
...
104         format(10x,'Original/extended header pixel size in nanometers =', g11.4)
        ...
        !
        ! SerialEM type
        if (nbytes_and_flags(numInt, numReal) .and. .not.silent) then
          if (ifBrief == 0) write(*,'(/,a)') 'Extended header from SerialEM contains:'
          numInt = 0
          do j = 1, ntypes
            if (mod(numReal / 2**(j - 1), 2) .ne. 0) then
              if (ifBrief == 0) then
                write(*,103) typeName(j), trim(extractCom(j))
103             format(2x, a, ' - Extract with "',a,'"')
              else
                write(*,'(a,1x,a,$)') trim(briefSep), trim(briefName(j))
                briefSep = ' - '
                numInt = numInt + 1
              endif
            endif
