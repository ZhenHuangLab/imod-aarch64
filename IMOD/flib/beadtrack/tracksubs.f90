...
! are returned with 0.
!
subroutine rescue(boxTmp, nxBox, nyBox, xpeak, ypeak, radMax, relaxCrit, stepSize, wsum, &
    edgeSD)
  use cgPixels
  implicit none
  real*4 boxTmp(*), xpeak, ypeak, radMax, relaxCrit, wsum, edgeSD, stepSize, rdx, rdy
  ...
  do while(rad <= radMax .and. wsum == 0.)
    ...
    wBest = 0.
    rdy = -nyBox / 2.
    do while (rdy <= nyBox / 2.)
      rdx = -nxBox / 2.
      do while (rdx <= nxBox / 2.)
        distSq = rdx**2 + rdy**2
