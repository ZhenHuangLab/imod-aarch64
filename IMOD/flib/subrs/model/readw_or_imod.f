...
c       Calls for model allocation based upon the size of the read-in model and 
c       specifications of fmBoostReadInBy, fmIncReadObjBy, fmIncReadPointsBy, and
c       fmMaxObjLoaded already set by the calling program.
...
c       
      ...
      include 'endian.inc'
      logical read_mod
      integer*4 getimod,int4(4), openImodData, getOpenedImod, imodCountContsPoints
      integer*4 numPtsTot, maxNumPts, numContsTot, maxNumConts 
      integer*4 ierr, istrm, ier, i, ninobj, ipt, ii
      character*(*) filename
      real*4 flt(3)
      integer*2 int2(26)
      integer*1 ptbyte
c       
      ...
      if (ierr .eq. 0) then
        if (imodCountContsPoints(numContsTot, maxNumConts, numPtsTot, maxNumPts)
     &      .ne. 0) return
        if (fmMaxObjLoaded .gt. 0) then
          fmNeedObjects = fmMaxObjLoaded * max(int(maxNumConts * fmBoostReadInBy),
     &        maxNumConts + fmIncReadObjBy)
          fmNeedPoints = fmMaxObjLoaded * max(int(maxNumPts * fmBoostReadInBy),
     &        maxNumPts + fmIncReadPointsBy)
        else
          fmNeedObjects = max(int(numContsTot * fmBoostReadInBy),
     &        numContsTot + fmIncReadObjBy)
          fmNeedPoints = max(int(numPtsTot * fmBoostReadInBy),
     &        numPtsTot + fmIncReadPointsBy)
        endif
        call allocateFortModel()
c         
        if (getimod(ibase_obj,npt_in_obj,p_coord,obj_color,n_point,
     &      n_object,filename) .ne. 0) return
...
c       
     ...
     &    p_coord,obj_color,n_point, n_object)
      ...
      ntot_in_obj=n_point
      do i = max_mod_obj + 1, max_obj_num
        npt_in_obj(i) = 0
      enddo
      n_clabel=0
      ibase_free = 0
