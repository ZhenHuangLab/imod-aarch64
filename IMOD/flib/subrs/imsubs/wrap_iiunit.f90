...
integer*4 function iiuOpenPrint(iunit, name, attribute)
  ...
  integer*4 nxTile, nyTile, nzChunk, nxyz(3), mxyz(3), nxyzst(3)
  ...
  call iiuRetChunkSizes(iunit, nxTile, nyTile, nzChunk)
  if (nxTile > 0 .or. nyTile > 0) then
    if (itype == 5 .and. nxTile == 0) nxTile = nxyz(1)
    if (nyTile == 0) nyTile = nxyz(2)
  endif
...
2100 format(/,1x,a,' image file on unit',i4,' : ',a, '     Size= ',i10,' K')
  if (doPrint .and. iiuRetBrief() == 0) then
    if (itype == 1) then
      if (nyTile > 0) then
        if (nxTile == 0) write(6, '(/,20x,a,i7,a,i7,a)')'This is a TIFF file (in '// &
            'strips of', nxyz(1), ' x',nyTile,').'
        if (nxTile > 0) write(6, '(/,20x,a,i7,a,i7,a)')'This is a TIFF file (in tiles'// &
            'of', nxTile,' x',nyTile,').'
      else
        write(6,2200) 'This is a TIFF file.'
      endif
    else if (itype == 5) then
      if (nxTile > 0) write(6, '(/,20x,a,i7,a,i7,a,i5,a)')'This is an HDF file (in '// &
          'chunks of', nxTile,' x',nyTile,' x', nzChunk,').'
      if (nxTile == 0) write(6,2200) 'This is an HDF file.'
    else if (itype == 6) then
      write(6,2200) 'This is a JPEG file.'
    else if (itype == 7) then
      write(6,2200) 'This is an image series file.'
