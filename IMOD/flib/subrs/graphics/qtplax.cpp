...
static int startPlaxApp()
{
  ...
  // If the DPI is high, the window will normally get scaled by the devPix
  // ratio, this scales the window down by less than the square root since
  // it doesn't need to be so big.  For Windows, scale font up by that same
  // factor, the full sqrt used in dia_qtutils is a bit much
  dpiRatio = diaGetAppDevPixRatioInfo(maxDPR, dprVaries);
#ifdef _WIN32
  sFontScale = pow(dpiRatio, 0.4f);
#endif
  dpiRatio = pow(dpiRatio, 0.4f);
  sPlaxWidth /= dpiRatio;
  sPlaxHeight /= dpiRatio;
...

{
  ...
  int ysize = (int)(iysize * TEXT_SIZE_SCALE * sFontScale);
  int ifbold = 0;
