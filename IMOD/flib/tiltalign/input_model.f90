...
subroutine input_model(imodObj, imodCont, numProjPt,  iwhichOut, xcen, ycen, delta, &
    ...
    xorig, yorig, zorig, imageBinned, pipinput)
!
  use alivar
  use fortmodel
  implicit none
  ...
  integer*4 numProjPt, maxZlist, imageBinned
  ...
  logical exist, readw_or_imod, pipinput
  ...
  if (modelFile == ' ') then
    ...
    if (pipinput) then
      ...
      nxyz(1) = nxyz(1) / imageBinned
      nxyz(2) = nxyz(2) / imageBinned
    else
      write(*,'(1x,a,$)') &
