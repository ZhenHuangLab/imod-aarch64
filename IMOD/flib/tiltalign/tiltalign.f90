...
program tiltalign
  ...
  real*4, allocatable :: allxyz(:,:), fixedxyz(:,:)
  ...
  call input_model(iallRealStr, indAllReal, nprojpt, iwhichOut, xcen, ycen, pixelDelta, &
      ...
      yOrigin, zOrigin, imageBinned, pipinput)
  !
  ...
  allocate(ninReal(maxReal), igroup(maxReal), &
      ...
      allxyz(3, maxReal), allxx(maxProjPt), allyy(maxProjPt), fixedxyz(3, maxReal), &
  ...
  if (pipinput) then
    ...
    if (ifDoRobust > 0 .and. robustByTrack .and. .not. patchTrackModel) then
      print *, &
          'Option to weight whole contours is being ignored for non-patch track model'
      robustByTrack = .false.
    endif
    ...
    ierr = PipGetString('FixedXYZInputFile', fixedXyzFile)
    if (ierr == 0) then
      call dopen(9, fixedXyzFile, 'old', 'f')
      index = 0
22    if (index > nRealPt) call errorExit( &
          'More coordinates in fixed XYZ input file than fiducial points', 0)
      read(9,*, err = 23, end = 24)(fixedxyz(j, index + 1), j = 1, 3)
      index = index + 1
      fixedxyz(1, index) = fixedxyz(1, index) - transposeXYadj
      fixedxyz(2, index) = fixedxyz(2, index) + transposeXYadj
      go to 22
23    call errorExit('Error reading fixed XYZ input file')
24    if (index < nRealPt) call errorExit( &
          'Fewer coordinates in fixed XYZ input file than fiducial points', 0)
      close(9)
    endif
  else
...
CONTAINS
  ...
  subroutine alignAndOutputResults()
    ...
    if (max((maxvar + 3) * maxvar, 3 * nview + 3) > maxh) then
      deallocate(h)
      maxh = max((maxvar + 3) * maxvar, 3 * nview + 3)
      allocate(h(maxh), stat = ierr)
      call memoryError(ierr, 'array for h matrix')
    endif
    !
    ...
    if (ifDoRobust .ne. 0 .and. metroError == metroRobust) then
      ...
      do while (numTotCycles .lt. maxTotCycles .and. .not. robTooFew)
        ...
        nw2 = 0
        nwTot = 0
        errMean = 0.    ! Mean change of weights < 0.5
        errSd = 0.      ! Max change in all weights
        prevMean = 0.   ! Mean difference between new weight and previous (2 ago) weights
        prevMax = 0.    ! Max difference between new weight and previous (2 ago) weights
        allMean = 0.    ! Mean change of all weights
        do jpt = 1, nrealPt
...
85      if (ncompSearch == 0) then
          ...
          if (mapAlfStart > mapAlfEnd) then
            if (ifLocal == 0) write(iunit, '(/,a,f8.2)') &
          ...
          elseif (ifRotFix == - 1 .or. ifRotFix == - 2) then
            if (ifRotFix == - 1) write(iunit, '(/,a,f8.2)') &
                ' Fixed rotation angle is', rot(1)
            if (ifRotFix == - 2) write(iunit, '(/,a,f8.2)') &
...
!!$299     continue
      ...
      if (ifLocal == 0 .and. ifOriginalZ .ne. 0 .and. nint(znew) .ne. 1000) then
        call lsfit2(h(1 + nview), h(1 + 2 * nview), h, nview, zShift, dxmid, offmin)
        znew = znew + zShift
      endif
...
!!$        enddo
        ...
        do i = 1, nrealPt
          j = indAllReal(i)
          if (fixedXyzFile .ne. ' ') then
            xyz(1, i) = fixedxyz(1, j) - xcen
            xyz(2, i) = fixedxyz(2, j) - ycen
            xyz(3, i) = fixedxyz(3, j)
          else
