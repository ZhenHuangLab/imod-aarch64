...
program clipmodel
  ...
  character*320 modelFile, inputModel
  ...
  real*4 cutSum, volume, xmax, xmin, xofs, xx, valRangeMin, valRangeMax
  ...
  if (PipGetInOutFile('InputFile', 1, 'Name of input model file', inputModel) .ne. 0)  &
  ...
  if (pipinput) then
    ...
    ierr = PipGetInteger('AreasInOrOutOfRange', ioptAreas)
    if (ioptAreas .ne. 0 .and. ioptValues .ne. 0)  &
        call exitError('You cannot use -areas with -values')
    if (ioptAreas .ne. 0 .and. ifValRange == 0) call exitError( &
        'You must enter an area range with -range when deleting by area')
  else
  ...
  fmBoostReadInBy = 1.05
  if (ifPoint == 0) fmMaxObjLoaded = 1
  exist = readw_or_imod(inputModel)
  ...
  ierr = getImodHeadO(xyScale, zScale)
  if (ierr .ne. 0) then
    xyScale = 1.
    pixelMicrons = 'pixels'
    if (abs(ioptAreas) > 1) call exitError( &
        'The model has no pixel size so the area range must be specified in pixels')
  endif
  if (abs(ioptAreas) == 2) then
    valRangeMin = valRangeMin * 1.e-6 / xyScale**2
    valRangeMax = valRangeMax * 1.e-6 / xyScale**2
  elseif (abs(ioptAreas) > 2) then
    valRangeMin = valRangeMin / xyScale**2
    valRangeMax = valRangeMax / xyScale**2
  endif
...
105     format(/,' Selected volume is',g15.6,' cubic ',a)
    ...
    do imodObj = 1, numObjTot
      ...
      do iobj = 1, max_mod_obj
        ...
        if (ioptAreas .ne. 0 .and. mod(iflags(imodObj), 4) .eq. 0) then
          gsval = contourArea(iobj, indY, indZ, zScale)
          inside = gsval >= valRangeMin .and. gsval <= valRangeMax
          if (ioptAreas < 0) inside = .not. inside
          if (inside) then
            call addToDeletionList(1, numInObj)
            npt_in_obj(iobj) = 0
            numInObj = 0
...
102 format(/,' Number of non-empty contours reduced from',i8,' to', &
        i9,/, ' Number of points reduced from',i10,' to',i10,/)
    !
    if (cutSum > 0.) then
...
! Remove all empty contours for the givenobject.  4/28/20: modified to assume what is
! true, that one object is loaded, and to call efficient function
subroutine removeEmptyContours(loadedObj)
  use fortmodel
  implicit none
  integer*4 iobjToDelete(max_mod_obj)
  integer*4 numKeep, numDelete, iobj, idel, deleteListOfConts, loadedObj
  numKeep = 0
  numDelete = 0
  do iobj = 1, max_mod_obj
    if (npt_in_obj(iobj) > 0) then
  ...
  enddo
  max_mod_obj = numKeep
  if (numDelete == 0) return
  if (deleteListOfConts(loadedObj, iobjToDelete, numDelete) .ne. 0)  &
      call exitError('Deleting contours from IMOD model')
  return
end subroutine removeEmptyContours

real*4 function contourArea(iobj, indY, indZ, zScale)
  use fortmodel
  implicit none
  real*4 zScale, xx, yy, zz, xnext, ynext, znext, xnorm, ynorm, znorm
  integer*4 iobj, indY, indZ, ipt, jpnt, numInObj, ibase
  contourArea = 0.
  numInObj = npt_in_obj(iobj)
  ibase = ibase_obj(iobj)
  ...
  do ipt = 1, numInObj
    ...
    xx = p_coord(1, jpnt)
    yy = p_coord(indY, jpnt)
    zz = p_coord(indZ, jpnt) * zScale
    jpnt = abs(object(mod(ipt, numInObj) + 1 + ibase))
    xnext = p_coord(1, jpnt)
    ynext = p_coord(indY, jpnt)
    znext = p_coord(indZ, jpnt) * zScale
    xnorm = xnorm + yy * znext - zz * ynext
    ynorm = ynorm + zz * xnext - xx * znext
    znorm = znorm + xx * ynext - yy * xnext
  enddo
  contourArea = sqrt(xnorm**2 + ynorm**2 + znorm**2) * 0.5
  return
