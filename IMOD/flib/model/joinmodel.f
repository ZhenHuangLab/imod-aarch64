...
c       
      integer*4, allocatable :: object2(:)             !pointer to p_coord array
      integer*2, allocatable :: npt_in_obj2(:)        !# of points in object
      integer*4, allocatable :: ibase_obj2(:)         !base index of object in OBJECT
      integer*2, allocatable :: obj_color2(:,:)       !ON/OFF and color indexes
      real*4, allocatable :: p_coord2(:,:)                !model coordinates of points
      integer*1, allocatable :: pt_label2(:)    !symbol number for point
...
c       
      call setExitPrefix('ERROR: JOINMODEL - ')
...
15    exist=readw_or_imod(modelfile)
      ...
      less_pt = max_pt/5
      allocate(object2(len_object), npt_in_obj2(max_obj_num), ibase_obj2(max_obj_num),
     &    obj_color2(2,max_obj_num), p_coord2(3,less_pt), pt_label2(less_pt),
     &    matched(max_obj_num), stat = ierr)
      call memoryError(ierr, 'arrays for second model')
c       
