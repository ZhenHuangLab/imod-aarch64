...
program boxstartend
  ...
  real*4 array(IDIM), brray(IDIM), avgArray(IDIM)
  equivalence (array, avgArray), (brray, avgArray)
  ...
  logical*4 readw_or_imod, getModelObjectList
  !
  logical pipInput
          ...
          do indZ = indZlo, indZhi
            ...
            if (ifAverage .ne. 0) then
              do ix = 1, numPixSq
                ...
                avgArray(jnd) = avgArray(jnd) + brray(ibBase + ix)
              enddo
            endif
          enddo
          !
  ...
  if (ifSeries == 0) then
    if (ifAverage .ne. 0) then
      do indar = 1, nzClip
        do ix = 1, numPixSq
          jnd = ix + (indar - 1) * numPixSq
          avgArray(jnd) = avgArray(jnd) / numClip
        enddo
        jnd = 1 + (indar - 1) * numPixSq
        call iclden(avgArray(jnd), numPixBox, nyPixBox, 1, numPixBox, 1, nyPixBox, tmin, &
            tmax, tmean)
        dmin2 = min(dmin2, tmin)
        dmax2 = max(dmax2, tmax)
        dsum = dsum + tmean
        call iwrsec(2, avgArray(jnd))
        nzOut = nzOut + 1
      enddo
    endif
    dmean2 = dsum / nzOut
