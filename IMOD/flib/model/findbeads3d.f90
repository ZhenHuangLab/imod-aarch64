...
program findbeads3d
  ...
  real*4 origin(3), delta(3), histo(limHisto), curTilt(3)
  ...
  real*4 elongation, radius, distMin, xpeak, ypeak, zpeak, elongCapLimit, elongFailLimit
...
20    continue
    ...
    if (elongation < elongCapLimit) then
      print *,'Elongation factor is', elongation
    elseif (elongation < elongFailLimit) then
      write(*,'(a,f5.2,a,f5.2)')'Elongation factor computed to be', elongation, &
          '; limiting it to', elongCapLimit
      elongation = elongCapLimit
    else
      write(tiltFile, '(a,f6.1,a)')'An angular range of', dyAdjacent - dxAdjacent, &
          ' degrees is too low for finding gold'
      call exitError(trim(tiltFile))
    endif
  endif
  ...
  call writePeakModel(firstFile, indPeak, peakVal, peakPos, &
      numPeaks, blackThresh, nxyz, delta, origin, curTilt, radius)
  !
...
subroutine writePeakModel(firstFile, indPeak, peakVal, peakPos, &
  ...
  real*4 peakVal(*), peakPos(3,*), delta(*), origin(*), curTilt(*), radius, blackThresh
  ...
  ierr = putImageRef(delta, origin, curTilt)
