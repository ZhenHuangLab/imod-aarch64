...
C       David Mastronarde  9/8/88
      ...
      character*320 modelfile,pointfile,oldmodel
      logical moreobj,topend,botend,failed,exist,readw_or_imod
      integer*2 itmp
      integer*4 indsecst(1001),listcolor(9)
      integer*2, allocatable :: xpnt(:),ypnt(:),zpnt(:)
      integer*4, allocatable :: indsort(:),nextpnt(:),modpnt(:)
      logical*1, allocatable :: matched(:)
      logical*1 gap(1001)
      data nsyms/8/
...
c       
c       
c       initialize and zero out model properties
c       
      call setExitPrefix('ERROR: MTMODEL - ')
      call allocateFortModel()
      less_pt = max_pt/5
      allocate(xpnt(less_pt), ypnt(less_pt), zpnt(less_pt), indsort(less_pt),
     &    nextpnt(less_pt), modpnt(max_obj_num), matched(less_pt), stat = ierr)
      call memoryError(ierr, 'arrays for points')
