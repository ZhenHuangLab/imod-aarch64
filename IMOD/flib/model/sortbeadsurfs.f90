...
program sortbeadsurfs
  ...
  logical*4 exist, readw_or_imod, already, majority, invertZ, rescale
  logical*4 oneSurface, checkGroups
  integer*4 ifFlip, local, ierr, i, j, ix, iy, indY, indZ, maxx, maxy, maxz
  real*4 xmax, xmin, ymin, ymax, deltaX, deltaY, xlo, xhi, ylo, yhi, tiltDum
  ...
  real*4, allocatable :: xyz(:,:), xyzFit(:,:), values(:), outlie(:)
  integer*4, allocatable :: igroup(:), igrpSort(:), imodObjOrig(:)
  !
  ...
  exist = readw_or_imod(inFile)
  if (.not.exist) call exitError('Reading model file')
  call scale_model(0)
  ierr = getImodMaxes(maxx, maxy, maxz)
  allocate(xyz(3,max_pt), xyzFit(3,max_pt), values(max_pt), outlie(max_pt), &
      igroup(max_pt), igrpSort(max_pt), imodObjOrig(max_pt), stat = ierr)
  call memoryError(ierr, 'arrays for model data')
  !
