...
module ccdvars
  implicit none
  integer LIMPATCH, LIMPTOUT, LIMDIFF, LIMPATCHOUT
  parameter (LIMDIFF = 512, LIMPATCHOUT = 40000)
  parameter (LIMPATCH = 40000, LIMPTOUT = 25 * LIMPATCHOUT)
  ...
  integer*4 nxyz(3), nx, ny, nz, maxDev
  logical*1, allocatable :: inList(:,:)
  integer*2, allocatable :: adjacent(:,:)
...
program ccderaser
  ...
  use fortmodel
  implicit none
  real*4 title(20), delta(3), origin(3), curTilt(3)
  integer*4 mxyz(3)
  ...
  real*4 zmin, zmax, xmin, xmax, yMin, ymax, diamMerge, curObjThresh, contValue
  ...
  integer*4 indx, indy, indz, ipcx, ipcy, ipcz, lastObjWithThresh
  logical*4 circleCont, allSecCont, contOnSecOrAllSec, skipLowValues, lowTurnedOff
  logical readw_or_imod, typeOnList, nearby
  integer*4 getImodObjSize, getImodSizes, putImageRef
  ...
  options(1) = &
      ...
      'expand:ExpandCircleIterations:I:@merge:MergePatches:B:@'// &
  ...
  nxAllocScan = 0
  nyAllocScan = 0
  maxDev = 300             ! Latest fixed value
  call b3dDate(dat)
  call time(tim)
  call allocateFortModel()
  !
  ...
  if (.not. pipInput .or. ierr == 0) then
    ...
    ierr = PipGetLogical('SkipTurnedOffPoints', skipLowValues)
    !
...
107       format('ERROR: CCDERASER - object',i4,', contour',i6,1x,a)
        ...
        enddo
        maxDev = max(maxDev, 2 * int(2 + xmax - xmin), 2 * int(2 + ymax - ymin))
  ...
  allocate(inList(-maxDev:maxDev, -maxDev:maxDev), &
      adjacent(-maxDev:maxDev,-maxDev:maxDev), stat = ierr)
  call memoryError(ierr, 'arrays for adjacent points')
  !
  ! Load point sizes
  indFree = 1
  indCont = 1
...
101 format('Section',i5,' -',$)
    ...
    do iobj = numObjOrig, 1, -1
      if (npt_in_obj(iobj) > 0) then
        ...
        if (skipLowValues) then
          if (contIsBelowThresh(iobj)) cycle
        endif
        !
...
109 format('CCDERASER: Bad points replaced with interpolated values', t57, a9, 2x, a8)
  ...
  if (modelOut .ne. ' ') then
    ...
    do i = 1, maxObjectsOut
      ...
      call putSymSize(i, 5)
    enddo
    i = putImageRef(delta, origin, curTilt)
...
CONTAINS
  ...
  ! contIsBelowThresh tests whether the contour jjobj has a value below threshold and
  ! low values are turned off.  First get the threshold and whether low is turned off
  ! for the IMOD object if it is not the same as last one, then test the contour
  !
  logical function contIsBelowThresh(jjobj)
    integer*4 jjobj, modObj, modCont
    integer*4 getObjValueThresh, getObjSkipLowValues, getContValue
    contIsBelowThresh = .false.
    call objToCont(jjobj, obj_color, modObj, modCont)
    if (modObj .ne. lastObjWithThresh) then
      if (getObjValueThresh(modObj, curObjThresh) == 0) then
        lowTurnedOff = getObjSkipLowValues(modObj) > 0
      else
        lowTurnedOff = .false.
      endif
      lastObjWithThresh = modObj
    endif
    if (lowTurnedOff) then
      if (getContValue(modObj, modCont, contValue) == 0) then
        contIsBelowThresh = contValue < curObjThresh
      endif
    endif
    return
  end function contIsBelowThresh
