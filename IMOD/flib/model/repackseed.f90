...
program repackseed
  ...
  integer*4, allocatable :: ifidInA(:), mapList(:), iobjToDelete(:)
  integer*4, allocatable :: ixyzCont(:), ixyzPoint(:), ixyzObj(:), imodObj(:), imodCont(:)
  real*4, allocatable :: xOrig(:), yOrig(:)
  ...
  real*4 dum, xImScale, yImScale, zImScale
...
101 format(a)
  ...
  call scale_model(0)
  allocate(ifidInA(max_obj_num), mapList(max_obj_num), iobjToDelete(max_obj_num), &
      ixyzCont(max_obj_num), ixyzPoint(max_obj_num), ixyzObj(max_obj_num), &
      imodObj(max_obj_num), imodCont(max_obj_num), xOrig(max_obj_num), &
      yOrig(max_obj_num), stat = ierr)
  call memoryError(ierr, 'arrays for object data')
  !
