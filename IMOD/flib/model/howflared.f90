...
program howflared
  ...
  logical readw_or_imod, exist, started
  ...
  real*4 zWall(LIMDATA, 2)
  integer*4 izSave(2,LIMDATA), icolOut(LIMDATA), iobjSave(LIMDATA), isurfSave(LIMDATA)
  integer*4 icontSave(LIMDATA), npTimes(LIMDATA)
  integer*4, allocatable :: itimes(:), isurfs(:)
  logical, allocatable :: objIsUsed(:)
  real*4 rmat(3,3), packXYZ(3,LIMDATA), packedRot(3,LIMDATA), straightEnd(3,2,LIMDATA)
  real*4 xCurve(LIMDATA), yCurve(LIMDATA), zCurve(LIMDATA), vec(3), endMean(3)
  ...
  logical*4 useSurf, noPairs, outputZvals
  ...
  !
  ! fallbacks from ../../manpages/autodoc2man -3 2  howflared
  !
  integer numOptions
  ...
  options(1) = &
      ...
      'point:PointOutputFile:FN:@zout:OutputRotatedZ:B:@pixel:PixelSizeDefault:F:@'// &
      'width:WidthDefault:F:@surface:UseSurfaceNumbers:B:@'// &
      'nopairs:NoPairsOrMarkers:B:@model:ModelFile:FNM:@fit:FitTopAndBottom:FPM:@'// &
      'id:Identifier:IM:@help:usage:B:'
  !
  pixSizeDef = 1.
  widthDef = 20.
  idStart = 1
  ...
  ierr = PipGetLogical('OutputRotatedZ', outputZvals)
  ...
  do iModel = 1, numModels
    ...
    if (.not.readw_or_imod(modelFile)) call exitError('Reading model file')
    if (.not. allocated(itimes)) then
      allocate(itimes(max_obj_num), isurfs(max_obj_num), objIsUsed(max_obj_num), &
          stat = ierr)
      call memoryError(ierr, 'arrays for surfaces/times')
    endif
