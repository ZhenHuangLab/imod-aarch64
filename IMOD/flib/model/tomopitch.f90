...
program tomopitch
  ...
  options(1) = &
      'model:ModelFile:FNM:@extra:ExtraThickness:F:@spacing:SpacingInY:F:@'// &
  ...
  do while(ifile <= numFiles)
    ...
    if (.not.useTimes .and. numVertical == 0 .and. numHoriz == 0) then
      ...
      if (.not.readw_or_imod(inFile)) then
      ...
      if (.not. allocated(itimes)) then
        allocate(itimes(max_obj_num), stat = ierr)
        call memoryError(ierr, 'array for object times')
      endif
