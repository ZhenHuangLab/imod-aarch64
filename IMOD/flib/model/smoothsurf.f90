...
program smoothsurf
  ...
  integer*4, allocatable :: izObj(:), isurf(:)
  logical*1, allocatable :: objectOK(:)
  ...
  fmBoostReadInBy = 1.1         ! There shouldn't be new points to accommodate
  if (.not.readw_or_imod(inputFile)) call exitError('Reading model')
  allocate(p_new(2,max_pt), izObj(max_obj_num), isurf(max_obj_num),  &
      objectOK(max_obj_num), stat = ierr)
  call memoryError(ierr, 'additional model arrays')
  !
