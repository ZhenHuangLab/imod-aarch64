...
c       
      ...
      real*4, allocatable :: xyzmt(:,:,:)
      integer*4, allocatable :: itype(:)
      integer*4 newtype(100),nmap(100) ,ioldtype(100,100),ifend(100,100)
c       
      call setExitPrefix('ERROR: ENDMODEL - ')
91    write(*,'(1x,a,$)')'Name of input model file: '
      read(5,'(a)')modelfile
c       
75    exist=readw_or_imod(modelfile)
      if(.not.exist)go to 91
      allocate(itype(max_obj_num), xyzmt(3,2,max_obj_num), stat = ierr)
      call memoryError(ierr, 'arrays for objects')
