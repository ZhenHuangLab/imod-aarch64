...
#define b3daddressablememory B3DADDRESSABLEMEMORY
#else
...
#define b3daddressablememory b3daddressablememory_
#endif
 ...
 * environment variable.  Valid values are OUTPUT_TYPE_MRC (2), OUTPUT_TYPE_TIFF (1),
 * OUTPUT_TYPE_HDF (5), or OUTPUT_TYPE_JPEG (6).  The override will not be set for HDF if
 * there is no HDF support, or JPEG if there is no JPEG support.
 */
void overrideOutputType(int type)
{
#ifdef NO_JPEG_LIB
  if (type == OUTPUT_TYPE_JPEG)
    return;
#endif
#ifdef NO_HDF_LIB
  if (type == OUTPUT_TYPE_HDF)
    return;
#endif
...
/*!
 * Returns the current output file type  OUTPUT_TYPE_MRC (2), OUTPUT_TYPE_TIFF (1),
 * OUTPUT_TYPE_HDF (5) (provided the package was build with HDF), or OUTPUT_TYPE_JPEG (6),
 * provided that the package was built with JPEG support.
 * The type is either the default type (which is MRC), or the value of the environment 
 * variable IMOD_OUTPUT_FORMAT if that is set and is 'MRC', 'TIF', 'TIFF', 'HDF', 'JPG',
 * or 'JPEG', overridden by a value set with @@overrideOutputType@.
 */
...
/*!
 * Like @@balancedGroupLimits@, but the remainder from division by the number of groups
 * is distributed among the last groups instead of the first ones.
 */
void groupLimitsRemainderAtEnd(int numTotal, int numGroups, int groupInd, int *start, 
                               int *end)
{
  int invSt, invEnd;
  balancedGroupLimits(numTotal, numGroups, (numGroups - 1) - groupInd, &invSt, &invEnd);
  *start = (numTotal - 1) - invEnd;
  *end = (numTotal - 1) - invSt;
}
...
/*!
 * Returns the total addressable memory, which is minimum of the physical memory and 4 GB
 * for a 32-bit version.
 */
double b3dAddressableMemory()
{
  double mem = b3dPhysicalMemory();
  if (sizeof(void *) == 4)
    mem = B3DMIN(mem, 4.e9);
  return mem;
}
