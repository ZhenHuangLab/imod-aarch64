...
static int processLoadedNodes(mxml_node_t *xml, int withXMLdecl, char **rootElement);

/*!
 * Reads an XML file from [filename] and returns a copy of the name of its root element in
 * [rootElement], which should be freed with {free}.  Set [withXMLdecl] non-zero to 
 * process a file with the standard opening ?xml (XML declaration) node, or 0 to consider
 * the top node to be the root. The node index list is initialized with a pointer to the 
 * top xml node at index 0 and a pointer to the root element at index 1 (if [withXMLdecl]
 * is non-zero).  The return value is the index for this set of XML data, or -1 for a
 * memory error, -2 for failure to open file, or -3 for failure to load as XML.
 */
int ixmlReadFile(const char *filename, int withXMLdecl, char **rootElement)
{
  ...
  if (!xml)
    return -3;
  fclose(fp);
  return processLoadedNodes(xml, withXMLdecl, rootElement);
}

/*!
 * Like @@ixmlReadFile@, but loads an XML structure from a string in [string] instead of 
 * from a file.  
 */
int ixmlLoadString(const char *string, int withXMLdecl, char **rootElement)
{
  ...
  xml = mxmlLoadString(NULL, string, MXML_OPAQUE_CALLBACK);
  if (!xml)
    return -3;
  return processLoadedNodes(xml, withXMLdecl, rootElement);
}
...
/* 
 * Common operation after loading file or string
 */
static int processLoadedNodes(mxml_node_t *xml, int withXMLdecl, char **rootElement)
{
  int xmlInd;
