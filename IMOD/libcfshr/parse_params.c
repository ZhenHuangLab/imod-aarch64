...
void PipPrintEntries()
{
  ...
  if (sOptTable[sNonOptInd].count) {
    ...
    for (j = 0; j < sOptTable[sNonOptInd].count; j++)
      if (strchr(sOptTable[sNonOptInd].valuePtr[j], ' '))
 ...
 * indicates all values should be returned, and a value of -1 indicates that up to 
 * [arraySize] values should be returned; in either case the number gotten is 
 * returned in [numToGet].  The return value is -1 for errors in parsing, too few values
 * on the line, or not enough space in the array (when [numToGet] is not -1).
 */
                       ...
                       int *numToGet, int arraySize)
{
  ...
  while (strlen(strPtr)) {
    ...
    /* Done if at end of line, or if count is fulfilled, or if buffer is full and
     numToGet < 0 */
    if (!*endPtr || (*numToGet > 0 && numGot >= *numToGet) ||
        (*numToGet < 0 && numGot >= arraySize))
      break;
