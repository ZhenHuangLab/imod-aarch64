...
#define adocgetnumcollections ADOCGETNUMCOLLECTIONS
#define adocgetcollectionname ADOCGETCOLLECTIONNAME
...
#define adocgetcollectionname adocgetcollectionname_
#define adocgetnumcollections adocgetnumcollections_
...
{
  return AdocGetNumCollections();
}

int adocgetcollectionname(int *collInd, char *string, int strSize)
{
  char *strPtr = NULL;
  int err = AdocGetCollectionName(*collInd - 1, &strPtr);
  if (!err && c2fString(strPtr, string, strSize)) {
    PipSetError("In AdocGetCollectionName, string is too long for character variable");
    err = -1;
    free(strPtr);
  }
  return err;
