...
static void deleteSection(AdocSection *sect);
...
int AdocDeleteSection(const char *typeName, int sectInd)
{
  ...
  if (masterInd < 0)
    return -1;
  deleteSection(&coll->sections[sectInd]);
...
/* BEWARE.  As far as callers are concerned, collections are numbered from 0 and do
   no include predata.  Hence return 1 less than the number of collections here, 
   and add 1 when they ask for a collection by index */
 ...
 * allocated and should be freed with {free}.  The index is numbered from 0 (1 in Fortran)
 * and excludes the global data collection.  Returns -1 for errors.
 */
int AdocGetCollectionName(int collInd, char **string)
{
  if (!sCurAdoc || collInd < 0 || collInd >= sCurAdoc->numCollections - 1)
    return -1;
  *string = strdup(sCurAdoc->collections[collInd + 1].name);
