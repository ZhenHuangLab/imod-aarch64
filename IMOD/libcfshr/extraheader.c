...
static void freeValStrings(char ***valStrings, int nz);
                      ...
                      float *val2, int *numVals, int *numFound, int maxVals, int *izPiece)
{
  float val3;
  char **valPtr = NULL;
  const char keys[9][20] = {"TiltAngle", "N", "StagePosition","Magnification","Intensity",
 ...
 * Returns 1 for an error and sets error string with @@b3dutil.html#b3dError@.
 */
int getMetadataByKey(int indAdoc, int iTypeAdoc, int nz, const char *key, int ivalType,
 ...
 * every image, then the routine analyzes the DateTime entries to determine the order of 
 * images.  If those are not available for every image, then the images are assumed to be 
 * in the standard order of a bidirectional tilt series if the value of [bidirNumInvert] 
 * is greater than 1.  Namely, views 1 through [bidirNumInvert] are assumed to be in the 
 * file in inverted order of acquisition, while the rest are assumed to be in order, and 
 * doses are summed for the images in that order.  If [bidirNumInvert] is less than 0
 ...
 * 1 or less than 0, then the routine just assumes the images are all in order and the 
 * return value is -1.
 * ^The routine relies on the Z values of the sections in the file as listed in [izPiece],
                              ...
                              int bidirNumInvert, float *priorDose, float *secDose)
{
  ...
  char **valStrings;
  time_t *fullTimes = NULL;
  int *timeInds = NULL;
  char monthBuf[8];
  int isBidir = (bidirNumInvert > 1 || bidirNumInvert < 0) ? 1 : 0;
  int saveError = b3dGetStoreError();
  struct tm tim;
  ...
  if (!retVal) {
    ...
    if (getMetadataByKey(indAdoc, iTypeAdoc, nz, "DateTime", 0, priorDose, &dummy1,
                         &dummy2, valStrings, &numVals, &numFound, nz, izPiece)) {
      freeValStrings(&valStrings, nz);
      retVal = 1;
      if (!isBidir)
        return 1;
    }
  ...
  if (retVal && isBidir) {
    if (retVal > 0)
      printf("WARNING: %s", b3dGetError());
    printf("%salling back to using entry for bidirectional tilt series\n", retVal > 0 ?
           "F" : "No date-time information; f");
    fflush(stdout);
    retVal = 0;
    b3dError(stdout, "");
 ...
 * summed in inverted order; if [bidirNumInvert] is less than 0, doses
 * [bidirNumInvert] + 1 to the end are summed in order then doses 1 to [bidirNumInvert] 
 * are summed in inverted order; otherwise all image doses are summed in order.
 */
                              ...
                              float *priorDose)
{
  ...
  if (bidirNumInvert > 1) {
    ...
    for (ind = bidirNumInvert + 1; ind < nz; ind++)
      priorDose[ind] = priorDose[ind - 1] + secDose[ind - 1];
  } else if (bidirNumInvert < 0) {
    bidirNumInvert *= -1;
    priorDose[bidirNumInvert] = 0.;
    for (ind = bidirNumInvert + 1; ind < nz; ind++)
      priorDose[ind] = priorDose[ind - 1] + secDose[ind - 1];
    priorDose[bidirNumInvert - 1] = priorDose[nz - 1] + secDose[nz - 1];
    for (ind = bidirNumInvert - 2; ind >= 0; ind--) 
      priorDose[ind] = priorDose[ind + 1] + secDose[ind + 1];
  } else {
    priorDose[0] = 0.;
