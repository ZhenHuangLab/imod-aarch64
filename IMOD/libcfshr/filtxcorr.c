...
#define fourierexpandimage FOURIEREXPANDIMAGE
...
#define fourierexpandimage fourierexpandimage_
...
static float peakHalfWidth(float *array, int ixPeak, int iyPeak, int nx, int ny, int delx,
                           int dely);
static void sortAndRepack(int *indexes, short *ixFrom, short *iyFrom, float *peakFrom,
                          int maxPeaks, int numPeaks, short *ixTo, short *iyTo, 
                          float *peakTo, int ifFirst);
...
/*!
 * Finds multiple cross-correlation peaks, such as from correlating with a template, much
 * more efficiently than @@XcorrPeakFindWidth@.  Finds the coordinates of up to the 
 * [maxPeaks] highest peaks in [array], which is dimensioned to [nxdim] by [ny], and 
 * returns the positions in [xpeak], [ypeak], the peak values in [peak], and the number 
 * of peaks in [numFound].  The offsets
 * [ixOffset] and [iyOffset] can be set to the amount of padding on each side in X and Y 
 * to ignore the borders where the array was padded.  Rather than maintaining peaks in 
 * order while finding them, up to [maxGrow] peaks are collected into arrays allocated
 * for this purpose, then sorted, and the top [maxPeaks] peaks are retained.  The array 
 * is allowed to grow repeatedly.  After the first time, only the newest peaks need to be 
 * sorted, but all the data need to be repacked to integrate new high ones.  Values for
 * [maxGrow] of 1.2 - 1.5 were found to be most efficient in a case where several thousand
 * peaks were being selected out of several hundred thousand.  Returns 1 if [maxGrow] is
 * not more than 1.05 times [maxPeaks] and -1 for memory error.
 */
int findManyXCorrPeaks(float *array, int nxdim, int ny, int ixOffset, int iyOffset,
                       float *xpeak, float *ypeak, float *peak, int maxPeaks, int maxGrow,
                       int *numFound)
{
  float cx, cy, val;
  int ix, iy, xbase, ybase, ind;
  int *indexes = NULL;
  short *ixTemp1 = NULL, *iyTemp1 = NULL, *ixTemp2 = NULL, *iyTemp2 = NULL;
  short *ixTemp, *iyTemp;
  float *peakTemp1 = NULL, *peakTemp2 = NULL, *peakTemp;
  float threshold = -1.e30, minFound = 1.e30;
  int numPeaks = 0, useTemp2 = 1, ifFirst = 1;

  if (maxGrow <= 1.05 * maxPeaks)
    return 1;

  /* Allocate arrays and test them */
  indexes = B3DMALLOC(int, maxGrow);
  ixTemp1 = B3DMALLOC(short, maxGrow);
  ixTemp2 = B3DMALLOC(short, maxGrow);
  iyTemp1 = B3DMALLOC(short, maxGrow);
  iyTemp2 = B3DMALLOC(short, maxGrow);
  peakTemp1 = B3DMALLOC(float, maxGrow);
  peakTemp2 = B3DMALLOC(float, maxGrow);
  if (!ixTemp1 || !ixTemp2 || !iyTemp1 || !iyTemp2 || !peakTemp1 || !peakTemp2 ||
      !indexes) {
    free(indexes);
    free(ixTemp1);
    free(ixTemp2);
  ...
  /* Loop on points */
  for (iy = iyOffset + 1; iy < ny - iyOffset - 1; iy++) {
    ybase = nxdim * iy;
    for (ix = ixOffset + 1; ix < nxdim - ixOffset - 1; ix++) {
      xbase = ybase + ix;
      val = array[xbase];

      /* Ignore value less than the threshold, which will be the lowest known peak */
      if (val < threshold)
        continue;

      /* Test for actual peak value */
      if (val > array[xbase - nxdim] && val >= array[xbase + nxdim] &&
          val > array[xbase - 1] && val >= array[xbase + 1] &&
          val > array[xbase + nxdim - 1] && val >= array[xbase + 1 - nxdim] &&
          val > array[xbase + nxdim + 1] && val >= array[xbase - 1 - nxdim]) {

        /* Found a peak above the current threshold: put it in the arrays */
        ixTemp[numPeaks] = ix;
        iyTemp[numPeaks] = iy;
        peakTemp[numPeaks] = -val;
        indexes[numPeaks] = numPeaks;
        numPeaks++;

        /* Keep track of min initially, and assign it to threshold when max is reached */
        if (numPeaks <= maxPeaks)
          ACCUM_MIN(minFound, val);
        if (numPeaks == maxPeaks)
          threshold = minFound;

        /* At the grow limit, it is time to sort and repack arrays */
        if (numPeaks == maxGrow) {
          sortAndRepack(indexes, ixTemp, iyTemp, peakTemp, maxPeaks, numPeaks, 
                        useTemp2 ? ixTemp2 : ixTemp1, useTemp2 ? iyTemp2 : iyTemp1,
                        useTemp2 ? peakTemp2 : peakTemp1, ifFirst);

          /* Reset the number, switch to other set of arrays, and set new threshold */
          ifFirst = 0;
          numPeaks = maxPeaks;
          ixTemp = useTemp2 ? ixTemp2 : ixTemp1;
          iyTemp = useTemp2 ? iyTemp2 : iyTemp1;
          peakTemp = useTemp2 ? peakTemp2 : peakTemp1;
          useTemp2 = 1 - useTemp2;
          threshold = -peakTemp[maxPeaks - 1];
  ...
  /* Do final sort of new data (or all data, if never got maxPeaks */
  sortAndRepack(indexes, ixTemp, iyTemp, peakTemp, maxPeaks, numPeaks, 
                useTemp2 ? ixTemp2 : ixTemp1, useTemp2 ? iyTemp2 : iyTemp1, 
                useTemp2 ? peakTemp2 : peakTemp1, ifFirst);
  ixTemp = useTemp2 ? ixTemp2 : ixTemp1;
  iyTemp = useTemp2 ? iyTemp2 : iyTemp1;
  peakTemp = useTemp2 ? peakTemp2 : peakTemp1;
  numPeaks = B3DMIN(numPeaks, maxPeaks);
  
  /* Get interpolated position for each and return into arrays */
  for (ind = 0; ind < numPeaks; ind++) {
    peak[ind] = -peakTemp[ind];
    ix = ixTemp[ind];
    iy = iyTemp[ind];
    xbase = ix + iy * nxdim;
    cx = (float)parabolicFitPosition(array[xbase - 1], array[xbase], array[xbase + 1]);
    cy = (float)parabolicFitPosition(array[xbase - nxdim], array[xbase], 
                                     array[xbase + nxdim]);
    xpeak[ind] = ix + cx;
    ypeak[ind] = iy + cy;
  }
...
/* Function to sort new part of peak arrays and repack just the ones being keep into a 
   different set of arrays */

#define COPY_FROM_TO                            \
  ixTo[ind] = ixFrom[from];                     \
  iyTo[ind] = iyFrom[from];                     \
  peakTo[ind] = peakFrom[from];                 \
  indexes[ind] = ind;

static void sortAndRepack(int *indexes, short *ixFrom, short *iyFrom, float *peakFrom,
                          int keepPeaks, int numPeaks, short *ixTo, short *iyTo, 
                          float *peakTo, int ifFirst)
{
  int ind, from, low, high;

  /* First time or not enough peaks, just sort and repack */
  if (ifFirst > 0 || numPeaks <= keepPeaks) {
    rsSortIndexedFloats(peakFrom, indexes, numPeaks);
    for (ind = 0; ind < B3DMIN(keepPeaks, numPeaks); ind++) {
      from = indexes[ind];
      COPY_FROM_TO;
    }
  } else {

    /* Otherwise sort the upper part of the array, setting index offset appropriately */
    rsSetSortIndexOffset(keepPeaks);
    rsSortIndexedFloats(&peakFrom[keepPeaks], &indexes[keepPeaks], numPeaks - keepPeaks);

    /* Merge the two sections by taking the lowest value from each eat each step */
    low = 0;
    high = keepPeaks;
    for (ind = 0; ind < keepPeaks && low < keepPeaks && high < numPeaks; ind++) {
      if (peakFrom[indexes[low]] < peakFrom[indexes[high]])
        from = indexes[low++];
      else
        from = indexes[high++];
      COPY_FROM_TO;
    ...
    low = (low < keepPeaks ? low : high);
    for (; ind < keepPeaks; ind++) {
      from = indexes[low++];
      COPY_FROM_TO;
    }
  }
...
#define MAX_SPCP_THREADS 8
/*!
 * Finds multiple cross-correlation peaks with a minimum spacing between them even
 * more efficiently than @@findManyXCorrPeaks@.  Finds the coordinates of up to the 
 * [maxPeaks] highest peaks separated by [minSpacing] in [array], whose X dimension is 
 * [nxdim], within the * coordinates from [ixMin] to [ixMax] and [iyMin] to [iyMax],
 * inclusive.  Returns the positions in [xpeak], [ypeak], the peak values in [peak], and 
 * the number of peaks in [numPeaks].  If [minStrength] is non-zero, only those peaks 
 * that are greater than that fraction of the highest peak will be returned. The 
 * function divides the image into blocks of
 * [minSpacing] / sqrt(2.) pixels, searches for the highest peak within each block, and
 * stores that peak for each block.  The loop on blocks is paralellized with OpenMP.
 * At the end, data are merged from the different threads, sorted, and weaker peaks too
 * close to other ones eliminated.  Returns -1 for memory error.
 */
int findSpacedXCorrPeaks(float *array, int nxdim, int ixMin, int ixMax, int iyMin,
                         ...
                         float minStrength)
{
  float cx, cy, val, maxVal, spaceCrit, dx, dy;
  int ix, iy, xbase, ybase, ind, ixPeak, iyPeak, indOut, totBlocks, thrd, numFound;
  int blockSize = minSpacing / sqrt(2.);
  int ixBlock, iyBlock, ixStart, ixEnd, iyStart, iyEnd, nBlocksX, nBlocksY, sind, jnd;
  int numThreads;
  short *ixThread[MAX_SPCP_THREADS];
  short *iyThread[MAX_SPCP_THREADS];
  float *peakThread[MAX_SPCP_THREADS];
  int *boxIndThread[MAX_SPCP_THREADS];
  int numPkThread[MAX_SPCP_THREADS];
  short *ixAll = NULL, *iyAll = NULL;
  float *peakAll = NULL;
  int *indexes = NULL;
  int *boxIndAll = NULL;
  
  nBlocksX = ((ixMax - (ixMin + 1)) + blockSize - 1) / blockSize;
  nBlocksY = ((iyMax - (iyMin + 1)) + blockSize - 1) / blockSize;
  totBlocks = nBlocksX * nBlocksY;

  // Tested up to 12 threads, ~80% efficient at 8 on somewhat less than 2K images
  numThreads = B3DMIN(MAX_SPCP_THREADS, (int)(0.005 * sqrt((double)(ixMax - ixMin) * 
                                                           (iyMax - iyMin))));
  numThreads = numOMPthreads(numThreads);
  ind = totBlocks * numThreads;
  ixAll = B3DMALLOC(short, ind);
  iyAll = B3DMALLOC(short, ind);
  peakAll = B3DMALLOC(float, ind);
  indexes = B3DMALLOC(int, ind);
  boxIndAll = B3DMALLOC(int, ind);
  if (!ixAll || !iyAll || !peakAll || !indexes || !boxIndAll) {
    B3DFREE(indexes);
    B3DFREE(ixAll);
    B3DFREE(iyAll);
    B3DFREE(peakAll);
  ...
  for (thrd = 0; thrd < numThreads; thrd++) {
    ixThread[thrd] = ixAll + thrd * totBlocks;
    iyThread[thrd] = iyAll + thrd * totBlocks;
    peakThread[thrd] = peakAll + thrd * totBlocks;
    boxIndThread[thrd] = boxIndAll + thrd * totBlocks;
    numPkThread[thrd] = 0.;
  }
  for (ind = 0; ind < totBlocks * numThreads; ind++)
    boxIndAll[ind] = -1;

#pragma omp parallel for default(none) num_threads(numThreads)          \
  shared(nBlocksY, nBlocksX, iyMin, blockSize, ixMin, ixThread, iyThread, peakThread, \
         nxdim, ixMax, iyMax, array, numPkThread, boxIndThread)         \
  private(iyBlock, ixBlock, iyStart, iyEnd, ixStart, ixEnd, maxVal, ixPeak, iy, ix, \
          ind, ybase, xbase, val, thrd, iyPeak)
  for (iyBlock = 0 ; iyBlock < nBlocksY; iyBlock++) {
    thrd = b3dOMPthreadNum();

    // Block start and end in Y
    iyStart = iyMin + 1 + iyBlock * blockSize;
    iyEnd = B3DMIN(iyStart + blockSize, iyMax);
      
    for (ixBlock = 0 ; ixBlock < nBlocksX; ixBlock++) {
        
      // Block start and end in X
      ixStart = ixMin + 1 + ixBlock * blockSize;
      ixEnd = B3DMIN(ixStart + blockSize, ixMax);
      ...
      if (ixPeak > 0) {
        ind = numPkThread[thrd]++;
        ixThread[thrd][ind] = ixPeak;
        iyThread[thrd][ind] = iyPeak;
        peakThread[thrd][ind] = maxVal;
        boxIndThread[thrd][ixBlock + iyBlock * nBlocksX] = ind;
      }
    }
  }
  ...
  for (thrd = 0; thrd < numThreads; thrd++) { 
    xbase = numFound;
    for (ind = 0; ind < numPkThread[thrd]; ind++) {
      indexes[numFound] = numFound;
      ixAll[numFound] = ixThread[thrd][ind];
      iyAll[numFound] = iyThread[thrd][ind];
      peakAll[numFound++] = -peakThread[thrd][ind];
    }
    for (ind = 0; ind < totBlocks; ind++) {
      if (boxIndThread[thrd][ind] >= 0)
        boxIndAll[ind] = boxIndThread[thrd][ind] + xbase;
    }
  }
  rsSortIndexedFloats(peakAll, indexes, numFound);

  /* Eliminate any lesser peaks too close to stronger one */
  spaceCrit = minSpacing * minSpacing;
  for (sind = 0; sind < numFound - 1; sind++) {
    ind = indexes[sind];
    if (peakAll[ind] > 1.e29)
      continue;

    /* Need to loop up to 2 blocks away except for corner blocks but it costs more to 
       test if in corner */
    ixBlock = (ixAll[ind] - (ixMin + 1)) / blockSize;
    iyBlock = (iyAll[ind] - (iyMin + 1)) / blockSize;
    ixStart = B3DMAX(0, ixBlock - 2);
    ixEnd = B3DMIN(nBlocksX, ixBlock + 3);
    iyStart = B3DMAX(0, iyBlock - 2);
    iyEnd = B3DMIN(nBlocksY, iyBlock + 3);
    for (iy = iyStart; iy < iyEnd; iy++) {
      for (ix = ixStart; ix < ixEnd; ix++) {
        if (ix == ixBlock && iy == iyBlock)
            continue;
        jnd = boxIndAll[ix + iy * nBlocksX];
        if (jnd >= 0 && peakAll[jnd] < 1.e29 && peakAll[jnd] >= peakAll[ind]) {
          dx = ixAll[ind] - ixAll[jnd];
          dy = iyAll[ind] - iyAll[jnd];
          if (dx * dx + dy * dy < spaceCrit)
            peakAll[jnd] = 1.e30;
        }
      }
    }
  ...
  /* Get interpolated position for each and return into arrays */
  indOut = 0;
  for (sind = 0; sind < numFound && indOut < maxPeaks; sind++) {
    ind = indexes[sind];
    if (peakAll[ind] > 1.e29)
      continue;
    if (indOut && minStrength > 0. && -peakAll[ind] < minStrength * peak[0])
      break;
    peak[indOut] = -peakAll[ind];
    ix = ixAll[ind];
    iy = iyAll[ind];
    xbase = ix + iy * nxdim;
    cx = (float)parabolicFitPosition(array[xbase - 1], array[xbase], array[xbase + 1]);
...
void conjugateProduct(float *array, float *brray, int nx, int ny)
{
  int jx, jp1;
  float a, b, c, d;
  int numThreads = B3DMIN(4, B3DNINT(0.025 * sqrt((double)nx * ny)));
  numThreads = numOMPthreads(numThreads);
#pragma omp parallel for default(none) num_threads(numThreads) \
  shared(array, brray, nx, ny) \
  private(jx, jp1, a, b, c, d)
 ...
 * dimensioned to at least [nxrOut] + 2, unless [temp] is NULL, in which case the FFT
...
/*!
 * Pads the Fourier transform in [fftIn] into [fftOut] to achieve an
 * image expansion from real-space image size [nxrIn], [nyrIn] to [nxrOut], [nyrOut], 
 * after shifting the image by [dxIn], [dyIn].  A Fourier shift is required to keep the
 * image centered anyway, so it is efficient to combine this with another shift if one
 * is needed. The X-dimension of [fftIn] is assumed to be [nxrIn] + 2 and that of 
 * [fftOut] is assumed to be [nxrOut] + 2.  [temp] is a temporary array that must be 
 * dimensioned to at least [nxrIn] + 2, unless [temp] is NULL, in which case the FFT
 * is simply padded, @fourierShiftImage is not called and [dxIn] and [dyIn] are ignored.
 */
                        ...
                        int nyrOut, float dxIn, float dyIn, float *temp)
{
  ...
  int nxDimOut = nxrOut + 2;
  float *inp = fftIn;
  float expFac = (float)nxrOut / (float)nxrIn;
  float dxy = (expFac - 1) / (2. * expFac);
  memset(fftOut, 0, nxDimOut * nyrOut * sizeof(float));

  /* Shift by the amount found empirically to be needed */
  if (temp)
    fourierShiftImage(fftIn, nxrIn, nyrIn, dxIn + dxy, dyIn + dxy, temp);
  for (loop = 0; loop < 2; loop++) {
    if (loop) {
      iyStart = nyrOut - (nyrIn / 2);
      iyEnd = nyrOut;
    } else {
      iyStart = 0;
      iyEnd = nyrIn - (nyrIn / 2);
    ...
    for (iy = iyStart; iy < iyEnd; iy++) {
      ybase = iy * nxDimOut;
      for (ix = 0; ix < nxDimIn; ix++)
        fftOut[ybase + ix] = *inp++ * expFac;
    }
  }
}

/*! Fortran wrapper for @@fourierExpandImage@. */
void fourierexpandimage(float *fftIn, int *nxrIn, int *nyrIn, float *fftOut,
                        int *nxrOut, int *nyrOut, float *dxIn, float *dyIn, float *temp)
{
  fourierExpandImage(fftIn, *nxrIn, *nyrIn, fftOut, *nxrOut, *nyrOut, *dxIn, *dyIn, temp);
}
...
/*!
 * Computes padded sizes in one dimension for Fourier reduction or expansion, given the 
 * original size in [size], the reduction factor or inverse of expansion factor in 
 * [factor], fraction to pad in [padFrac], minimum 
 * padding in [minPad], and the nice or necessary limiting factor for FFTs in [niceLimit].
 * The padded size for the original and processed data are returned in [fullPadSize] and
 * [cropPadSize] and the actual factor being applied is returned in [actualFac].
 * The reduction or expansion factor or its multiple by 2, 3, 4, 5, 6, 8, or 10 is 
 * required to be an integer, where the factor must be accurate to the third decimal 
 * place for a multiple to be acceptable.  The return value is 1 for a factor equal to 
 * 1 or 2 if it is not an acceptable fractional value.
 */
...
{
  float useFac = factor, tolerance = 0.001;
  ...
  for (ind = 0; ind < numDenoms; ind++) {
    numerator = B3DNINT(useFac * denoms[ind]);
    *actualFac = (float)numerator / denoms[ind];
    if (fabs(useFac - *actualFac) < tolerance) {
      nDenom = denoms[ind];
      break;
    }
  }
  ...
  if (factor < 1.) {
    *actualFac = 1. / *actualFac;
    B3DSWAP(nDenom, numerator, ind);
  }
