...
#define robustpolyfit ROBUSTPOLYFIT
#define robustpolysmooth ROBUSTPOLYSMOOTH
#else
...
#define robustpolyfit robustpolyfit_
#define robustpolysmooth robustpolysmooth_
#endif
 ...
 * with an error.
 */
int multRegress(float *x, int xSize, int colFast, int numInpCol, int numData, 
                ...
                float *xMean, float *xSD, float *work)
{
  ...
  memcpy(work, sol, numInpCol * numOutCol * sizeof(float));
  for (j = 0; j < numOutCol; j++) {
    if (cons)
...
int multregress(float *x, int *xSize, int *colFast, int *numInpCol, int *numData, 
                ...
                float *xMean, float *xSD, float *work)
{
  return multRegress(x, *xSize, *colFast, *numInpCol, *numData, *numOutCol, *wgtCol - 1, 
                     sol, *solSize, cons, xMean, xSD, work);
}

/*! Fortran wrapper to call @multRegress to fit with no constant term */
int multregressnoc(float *x, int *xSize, int *colFast, int *numInpCol, int *numData, 
 ...
 */
int robustRegress(float *x, int xSize, int colFast, int numInpCol, int numData,
                  ...
                  int maxZeroWgt, float maxChange, float maxOscill)
{
  ...
  int wgtCol = numInpCol + numOutCol;
  ...
  for (iter = 0; iter < maxIter; iter++) {
    ...
    ierr = multRegress(x, xSize, colFast, numInpCol, numData, numOutCol, wgtCol, sol, 
    ...
    for (j = 0; j < numData; j++) {
      if (numOutCol == 1) {
        colres = (cons ? cons[0] : 0.) - XRC(j, numInpCol);
        for (i = 0; i < numInpCol; i++)
        ...
        for (k = 0; k < numOutCol; k++) {
          colres = (cons ? cons[k] : 0.) - XRC(j, (numInpCol + k));
          for (i = 0; i < numInpCol; i++)
...
{
  return robustRegress(x, *xSize, *colFast, *numInpCol, *numData, *numOutCol, sol,
...
int robustregressnoc(float *x, int *xSize, int *colFast, int *numInpCol, int *numData, 
...
/*
 * Uses robust linear regression to fit a polynomial of order
 * [order] to [ndata] points whose (x,y) coordinates are in the arrays [x] and [y].
 * It returns the coefficient of x to the i power in the array [slopes] and a
 * constant term in [intcpt].  The equation fit is:  ^
 * Y = intcpt + slopes\[0\] * X + slopes\[1\] * X**2 + ...  ^
 * [work] is an array for temporary use whose size must be at least 
 * ([order] + 3) * [ndata] + 2 * ([order] + 1) + the maximum of ([order] + 1)^2 and 
 * 2 * [ndata].  [kfactor], [numIter], [maxIter], and [maxZeroWgt] are as described for
 * @@robustRegress@, which is also called with a maximum change of 0.02 and a maximum 
 * for oscillations of 0.05.  The final weights are returned at the start of the work
 * array.  Returns 1 for zero order or the error code from a failure in @@robustRegress@.
 */
int robustPolyFit(float *x, float *y, int ndata, int order, float *slopes, float *intcpt,
                  float *work, float kfactor, int *numIter, int maxIter, int maxZeroWgt)
{
  float maxChange = 0.02f, maxOscill = 0.05f;
  int wdim = order + 1;
  int i, j;
  float *xMean = work + (order + 3) * ndata;
  float *xSD = xMean + wdim;
  float *mwork = xSD + wdim;
  if (!order)
    return 1;
  ...
  i = robustRegress(work, ndata, 0, order, ndata, 1, slopes, order, intcpt, xMean, xSD,
                       mwork, kfactor, numIter, maxIter, maxZeroWgt, maxChange, 
                       maxOscill);
  if (i)
    return i;
  for (i = 0; i < ndata; i++)
    work[i] = work[i + (order + 1) * ndata];
  return 0;
}

/* Fortran wrapper to @robustPolyFit@@. */
int robustpolyfit(float *x, float *y, int *ndata, int *order, float *slopes,
                  ...
                  int *maxZeroWgt)
{
  return robustPolyFit(x, y, *ndata, *order, slopes, intcpt, work, *kfactor, numIter,
                       *maxIter, *maxZeroWgt);
}

/* !
 * Does smoothing of the [data] points in [x] and [yIn] by fitting a polynomial of the 
 * given [order] to [numFit] points around each point, or at least [minFit] points near 
 * the ends.  The fitted value of Y at each point is returned in [yOut]. Robust 
 * polynomial fitting is used if possible, with the arguments [kfactor],
 * [maxIter], and [maxZeroWgt] passed to @@robustPolyFit@.  If that fails, a regular
 * polynomial fit is used.  [work] is an array for temporary use whose size has to be at
 * least [order] + 2 * [numFit] plus the maximum of the sizes needed for @@robustPolyFit@
 * or @@polynomialFit@ with [numFit] values.  Call with [ndata] 0 or negative to have the 
 * needed size returned in the first element of [yOut].  If [weights] is not NULL, it
 * returns the weight for each point on the fit that smoothed it in that array.
 * Returns 1 for too few data points
 * or the error code from a failure in @@polynomialFit@.  
 */
int robustPolySmooth(float *x, float *yIn, int ndata, int order, float *yOut, int numFit,
                     int minFit, float *work, float kfactor, int maxIter, int maxZeroWgt,
                     float *weights)
{
  int numThisFit, ind, thisFit, fitStart, fitEnd, j, err, numIter, ordp1 = order + 1;
  float intcpt, xmean = 0., ymean = 0.;
  float *slopes = work;
  float *xfit = slopes + order;
  float *yfit = xfit + numFit;
  float *mwork = yfit + numFit;
  if (ndata <= 0) {
    yOut[0] = B3DMAX(ordp1 + 2 * numFit + (order + 3) * numFit + 2 * ordp1 + 
                     B3DMAX(ordp1 * ordp1, 2 * numFit),
                     ordp1 * (order + 3 + numFit));
    return 0;
  }
  if (minFit > ndata)
    return 1;
  
  /* Determine range of fitting */
  for (ind = 0; ind < ndata; ind++) {
    fitStart = ind - numFit / 2;
    fitEnd = fitStart + numFit - 1;
    if (fitEnd >= ndata) {
      fitEnd = ndata - 1;
      fitStart = B3DMIN(fitStart, ndata - minFit);
    }
    if (fitStart < 0) {
      fitStart =  0;
      fitEnd = B3DMAX(fitEnd, minFit - 1);
    }

    /* Find the local mean and subtract it loading data, helps control numeric errors */
    thisFit = fitEnd + 1 - fitStart;
    xmean = ymean = 0.;
    for (j = 0; j < thisFit; j++) {
      xfit[j] = x[fitStart + j];
      yfit[j] = yIn[fitStart + j];
      xmean += xfit[j];
      ymean += yfit[j];
    }
    xmean /= thisFit;
    ...
    /* Get the fit */
    err = robustPolyFit(xfit, yfit, thisFit, order, 
                        slopes, &intcpt, mwork, kfactor, &numIter, maxIter, maxZeroWgt);
    if (weights)
      weights[ind] = mwork[ind - fitStart];
    if (err) {
      weights[ind] = 1.;
      err = polynomialFit(xfit, yfit, thisFit, order, slopes, &intcpt, mwork);
    }
    if (err)
      return err;
    yOut[ind] = intcpt + ymean;
    for (j = 0; j < order; j++)
      yOut[ind] += slopes[j] * pow((double)(x[ind] - xmean), j + 1.);
  }
  return 0;
}

/* Fortran wrapper to @robustPolySmooth@@. */
...
{
  return robustPolySmooth(x, yIn, *ndata, *order, yOut, *numFit, *minFit, work,
                          *kfactor, *maxIter, *maxZeroWgt, weights);
}
