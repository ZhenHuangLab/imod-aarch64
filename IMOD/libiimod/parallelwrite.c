...
#define iiuparwrtsecpart IIUPARWRTSECPART
...
#define iiuparwrtsecpart iiuparwrtsecpart_
...
typedef struct {
  ...
  int chunkLines;  /* 1 for chunk writing via iiu, 2 for chunks via iiWriteSection */
  int indX0;       /* The rest of these are arguments for iiuWriteSecPart or for */
  int indX1;       /* similar item in the iiFile information */
  int nxdimPadRight;
  int ixStart;
  int iyStart;
  int iyEnd;
...
static int addSegment(char *buf, int numBytes, int section, int startLine,
                      ImodImageFile *iiFile, MrcHeader *hdata, int iunit, int chunk,
                      int nxdim, int ixStart, int x0, int x1, int iyStart, int iyEnd);
...
int parallelWriteSlice(void *buf, FILE *fout, MrcHeader *hdata, int slice)
{
  ...
  for (ib = 0; ib < 2; ib++) {
    if (sections[ib] >= 0 && slice == sections[ib]) {
      b3dFseek(fpBound, hbound.headerSize + ib * hbound.nx * linesBound * csize *
            dsize, SEEK_SET);
      filename = (char *)buf;
...
/*!
 * Writes data to one chunk of an HDF file Z = [section] from the buffer [buf] to file
 * [iiFile], accessing the header in [hdata], and following the subarea and subarray
 * information in the {llx}, {urx}, {lly}, {ury}, {padLeft, and {padRight} members of
 * [iiFile].  Returns errors from writing the slice with @@iimage.html#iiWriteSection@ 
 * and also returns other non-zero values from opening the boundary file, writing its 
 * header, or writing to the file.
 */
int parWrtToHDFchunk(ImodImageFile *iiFile, MrcHeader *hdata, void *buf, int section)
{
  int dsize, csize, err, numBytes;

  if (sCurInfo < 0 || sInfos[sCurInfo].hdfIndex < 0)
    return 1;
  mrc_getdcsize(hdata->mode, &dsize, &csize);
  numBytes = dsize * csize * (iiFile->padLeft + iiFile->padRight + iiFile->urx + 1 -
                              iiFile->llx) * (iiFile->ury + 1 - iiFile->llx);
  err = addSegment((char *)buf, numBytes, section, 0, iiFile, hdata, 0, 2,
                   iiFile->padRight, iiFile->padLeft, iiFile->llx, iiFile->urx,
                   iiFile->lly, iiFile->ury);
  if (err >= 0)
    return err;

  /* printf("parallelWriteSlice writing %d\n", slice);*/
  err = iiWriteSection(iiFile, buf, section);

  if (parWrtRecloseHDF(iiFile, hdata))
    return 1;
  return err;
}
                        ...
                        int nzIn)
{
  ...
  if (!retval && filename != NULL && strlen(filename) > 0) {
...
/*!
 * Writes data to one or more chunks of an HDF file from a potential subarray of [array]
 * at the current line and section of the file open on unit [iunit]. The X dimension of 
 * [array] is given in [nxdim], the starting and ending index at which to write the lines
 * in X in [indX0] and [indX1], the starting index from which to take data in X in 
 * [ixStart], and the starting and ending indices from which to take data in Y in 
 * [iyStart] and [iyEnd] (inclusive, numbered from 0).
 * Returns errors from writing to the slice with @@unitIO.html#iiuWriteSecPart@ 
 * and also returns other non-zero values from storing the data or reopening the file.
 */
int iiuParWrtSecPart(int iunit, void *array, int nxdim, int ixStart, int indX0, 
                     int indX1, int iyStart, int iyEnd)
{
  int ierr, numBytes, numTrim;
  ImodImageFile *iiFile;

  if (sCurInfo >= 0 && sInfos[sCurInfo].hdfIndex >= 0) {
    iiFile = iiuGetIIFile(iunit);
    numBytes = iiuBufBytesPerPixel(iunit) * nxdim * (iyEnd + 1 - iyStart);
    numTrim = iiuBufBytesPerPixel(iunit) * nxdim * iyStart;
    ierr = addSegment((char *)array + numTrim, numBytes, sIzCur[sCurInfo], 
                      sIyCur[sCurInfo], iiFile, NULL,
                      iunit, 1, nxdim, ixStart, indX0, indX1, 0, iyEnd - iyStart);

    /* It returned 1 for error, 0 for data handled somehow and file reclosed, or -1
       to go on and write.  Don't bother to adjust the current line, the caller will
       need to be calling parWrtPosn */
    if (ierr > 0)
      return 1;
    if (!ierr)
      return 0;
  }
  ierr = iiuWriteSecPart(iunit, array, nxdim, ixStart, indX0, indX1, iyStart, iyEnd);
  if (ierr)
    return ierr;
...
int iiuparwrtsecpart(int *iunit, void *array, int *nxdim, int *ixStart, int *indX0, 
                     int *indX1, int *iyStart, int *iyEnd)
{
  return iiuParWrtSecPart(*iunit, array, *nxdim, *ixStart, *indX0, 
                          *indX1, *iyStart, *iyEnd);
}
                      ...
                      int nxdim, int ixStart, int x0, int x1, int iyStart, int iyEnd)
{
  ...
  seg->chunkLines = chunk;
  seg->indX0 = x0;
  seg->indX1 = x1;
  seg->nxdimPadRight = nxdim;
  seg->ixStart = ixStart;
  seg->iyStart = iyStart;
  seg->iyEnd = iyEnd;
...
static int writeSegments(ImodImageFile *iiFile, MrcHeader *hdata, int iunit)
{
  ...
  for (ind = 0; ind < sNumSegments[sCurInfo]; ind++) {
    ...
    } else if (seg->chunkLines == 1) {

      /* It is to an HDF chunk via iiu */
      iiuSetPosition(iunit, seg->section, seg->startLine);
      err = iiuWriteSecPart(iunit, seg->bufStart, seg->nxdimPadRight, seg->ixStart,
                            seg->indX0, seg->indX1, seg->iyStart, seg->iyEnd);
      if (err)
        return 1;
      
    } else if (seg->chunkLines == 2) {

      /* It is to an HDF chunk via iiWriteSection */
      iiFile->llx = seg->indX0;
      iiFile->urx = seg->indX1;
      iiFile->lly = seg->iyStart;
      iiFile->ury = seg->iyEnd;
      iiFile->padLeft = seg->ixStart;
      iiFile->padRight = seg->nxdimPadRight;
      err = iiWriteSection(iiFile, seg->bufStart, seg->section);
      if (err)
        return 1;
