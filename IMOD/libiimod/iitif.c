...
static void countEERBytesAndElectrons(TIFF *tif, int is7bitEER, int *rawTotalBytes,
                                      int *maxElectrons);
static int decodeEERimage(unsigned char *buf, int *positions, unsigned char *symbols,
                          int is7bitEER, int eer_image_pixels, int pos_limit, 
                          int *numElectrons);
static void convertEERpositions(ImodImageFile *inFile, int *positions, 
                                unsigned char *symbols, int numElectrons,
                                unsigned char *buf, int xmin, int xmax, int ymin,
                                int ymax);
static void cleanupFromEER(unsigned char *tmp, unsigned char *map, int *positions, 
...
static int sMaxEERsuperResolution = 2;
static int sReadEERasSuperRes = -1;
static int sEERflags = 0;
static int sAutogroupEER = -1;

#define NO_WASTED_BITS

#define IS_EER_FILE(comp) \
  (comp == IICOMPRESSION_EER_7BIT || comp == IICOMPRESSION_EER_8BIT)
...
int iiTIFFCheck(ImodImageFile *inFile)
{
  ...
  int fromSEMCCD = 0;
  int byteMax, eerFile, skipEER = (sEERflags & IIFLAG_SKIP_EER_DIRS) ? 1 : 0;
  int numEERimages = 0, numNonEERimages = 0, skipFile, maxElectrons, numAutogroup;
  float frameMean;
  RawImageInfo info;

  if (sReadEERasSuperRes < 0 || sAutogroupEER < 0) {
    sReadEERasSuperRes = sMaxEERsuperResolution;
    sAutogroupEER = 1;
    resvar = getenv("IMOD_READ_EER_SUPER_RES");
    if (resvar) {
      sReadEERasSuperRes = atoi(resvar);
      B3DCLAMP(sReadEERasSuperRes, 0, sMaxEERsuperResolution);
    }
    resvar = getenv("IMOD_READ_EER_Z_SUMMING");
    if (resvar) {
      sAutogroupEER = atoi(resvar);
      sAutogroupEER = B3DMAX(0, sAutogroupEER);
    }
  }
  ...
  inFile->directoryNums = ilistNew(sizeof(int), 4);
  if (!inFile->directoryNums)
    return IIERR_MEMORY_ERR;
  ilistQuantum(inFile->directoryNums, 20);
  ...
  do {
    ...
    compressionIm = IICOMPRESSION_NONE;
    TIFFGetField(tif, TIFFTAG_COMPRESSION, &compressionIm);
    eerFile = IS_EER_FILE(compressionIm) ? 1 : 0;
    if (eerFile)
      numEERimages++;
    else
      numNonEERimages++;

    if (eerFile && !skipEER && (numEERimages == 2 || numEERimages == 20)) {
      countEERBytesAndElectrons(tif, compressionIm == IICOMPRESSION_EER_7BIT ? 1 : 0,
                                &i, &maxElectrons);
      frameMean = (float)maxElectrons / (nxim * nyim);
    }

    /* If this is a bigger image, it is a new standard, so set all the
       properties and reset to one directory */
    skipFile = ((skipEER && eerFile) || (!skipEER && !eerFile && numEERimages > 0)) ?
      1 : 0;
    
    if (((float)nxim * (float)nyim > (float)inFile->nx * (float)inFile->ny ||
         (nxim == inFile->nx && nyim == inFile->ny && eerFile && numEERimages == 1)) && 
        !skipFile) {
      inFile->nx = nxim;
      inFile->ny = nyim;
      ...
      xPixel = xPixelIm;
      yPixel = yPixelIm;
      compression = compressionIm;
      
      if (dirnum && !(eerFile || numNonEERimages > 0))
        inFile->multipleSizes = 1;
      dirnum = 1;
      inFile->directoryNums->size = 0;
      if (ilistAppend(inFile->directoryNums, &fileDirNum)) {
        closeWithError(inFile, "Memory error adding to directory list\n");
        return(IIERR_MEMORY_ERR);
      }
    } else if (nxim == inFile->nx && nyim == inFile->ny && !skipFile) {
      dirnum++;
      if (ilistAppend(inFile->directoryNums, &fileDirNum)) {
        closeWithError(inFile, "Memory error adding to directory list\n");
        return(IIERR_MEMORY_ERR);
    ...
    } else if (dirnum && !skipFile)
      inFile->multipleSizes = 1;
    fileDirNum++;
  } while (TIFFReadDirectory(tif));

  if (skipEER && !numNonEERimages) {
    closeWithError(inFile, 
                   "ERROR: iiTIFFCheck - No non-EER images present in EER file\n");
    return(IIERR_NOT_PRESENT);
  }
  eerFile = IS_EER_FILE(compression);
  ...
  if (mismatch || 
      !((((bits == 4 && !(formatDefined && sampleformat == SAMPLEFORMAT_INT) && 
         ...
         photometric < PHOTOMETRIC_RGB) || eerFile ||
  ...
  byteMax = 255;
  inFile->numFramesInEERfile = 0;
  if (eerFile) {

    /* Set up autogrouping: save the full # of frames, compute the number of groups 
       and adjust the estimate frame mean for summing and super-res */
    inFile->numFramesInEERfile = dirnum;
    if (sAutogroupEER > 1) {
      dirnum = (dirnum + sAutogroupEER - 1) / sAutogroupEER;
      numAutogroup = (inFile->numFramesInEERfile + dirnum - 1) / dirnum;
      frameMean *= numAutogroup;
    }

    /* Set size based on super-res and adjust maximum for that */
    for (i = 0; i < sReadEERasSuperRes; i++) {
      inFile->nx *= 2;
      inFile->ny *= 2;
      inFile->tileSizeY *= 2;
      frameMean /= 4.;
    ...
    inFile->readEERasSuperRes = sReadEERasSuperRes;

    /* Set the max as 3 SDs above mean for Poisson sample where SD = mean */
    byteMax = (int)ceil(4. * frameMean);
    B3DCLAMP(byteMax, 1, 255);
  }
  if (numEERimages > 0)
    sEERflags = 0;
  ...
  if (bits == 8 || eerFile) {
    ...
    inFile->amin  = 0;
    inFile->amean  = eerFile ? frameMean : (byteMax / 2.);
    inFile->amax   = byteMax;
  ...
  inFile->amean = eerFile ? frameMean : ((inFile->amin + inFile->amax) / 2.);
  ...
  if (hasPixel && (inFile->anyTiffPixSize || xPixel / 1.e4 <= pixelLimit)) {
    ...
    inFile->zscale = xPixel * (eerFile ? numAutogroup : 1);
  }
...
{
  ilistDelete(inFile->directoryNums);
  inFile->directoryNums = NULL;
  tiffClose(inFile);
}
...
/* Set the properties for opening EER files and flags for opening or reading
   superRes can be 0 to the maximum allowed value; autogroup specifies the frame summing;
   flags are defined in iimage.h.  Flags are cleared when opening or reading from file. */
void tiffSetEERreadProperties(int superRes, int autogroup, int flags)
{
  sReadEERasSuperRes = superRes;
  sEERflags = flags;
  sAutogroupEER = autogroup;
  B3DCLAMP(sReadEERasSuperRes, 0, sMaxEERsuperResolution);
}

/* Return the maximum super-resolution supported */
int tiffGetMaxEERsuperRes()
{
  return sMaxEERsuperResolution;
}
...
static int setMatchingDirectory(ImodImageFile *inFile, int dirnum)
{
  int *numPtr;
  TIFF *tif = (TIFF *)inFile->header;
  numPtr = (int *)ilistItem(inFile->directoryNums, dirnum);
  if (!numPtr || !TIFFSetDirectory(tif, *numPtr))
    return 1;
  return 0;
}
...
/*
 * Add up the raw bytes in a frame and compute the maximum electrons
 */
static void countEERBytesAndElectrons(TIFF *tif, int is7bitEER, int *rawTotalBytes,
                                      int *maxElectrons)
{
  int si, nstrip = TIFFNumberOfStrips(tif);
  *rawTotalBytes = 0;
  
  /* Add up the strip sizes to get buffer needs */
  for (si = 0; si < nstrip; si++)
    *rawTotalBytes += TIFFRawStripSize(tif, si);
      
  /* maximum number of electrons is 11 or 12 bits each */
  *maxElectrons = 8 * (*rawTotalBytes) / ((is7bitEER ? 7 : 8) + 4);
}
...
static int ReadSection(ImodImageFile *inFile, char *buf, int inSection, int convert)
{
  ...
  int stripsize, maxElectrons, is7bitEER, rawTotalBytes, outBufSize, isEERfile;
  int numElectrons, byteBufSize, chipSize, startFinish, addToSum, autoGroupEER;
  int secStart, secEnd, rawArrSize, elecArrSize;
  int *positions = NULL;
  ...
  /*double wallStart, curWall, wallRead = 0.,wallDecode = 0., wallConvert = 0.;*/
  ...
  TIFF* tif = (TIFF *)inFile->header;
  if (inFile->axis == 2) {
    b3dError(stderr, "ERROR: tiffReadSection - Cannot read Y planes from a TIFF file\n");
    return -1;
  }
  ...
  isEERfile = IS_EER_FILE(inFile->tiffCompression);
  autoGroupEER = (isEERfile && inFile->numFramesInEERfile > inFile->nz) ? 1 : 0;
  if (autoGroupEER && (sEERflags & (IIFLAG_ADD_TO_EER_SUM | IIFLAG_START_END_EER_SUM))) {
    b3dError(stderr, "ERROR: tiffReadSection - Cannot set flags for summing frames "
             "when autogrouping an EER file\n");
    return -1;    
  }
  ...
  if (!autoGroupEER && setMatchingDirectory(inFile, row)) {
  ...
  if (isEERfile) {
    is7bitEER = inFile->tiffCompression == IICOMPRESSION_EER_7BIT ? 1 : 0;
    secStart = secEnd = inSection;
    if (autoGroupEER)
      groupLimitsRemainderAtEnd(inFile->numFramesInEERfile, inFile->nz, inSection,
                                &secStart, &secEnd);
    rawArrSize = 0;
    elecArrSize = 0;

    for (row = secStart; row <= secEnd; row++) {
      /*wallStart = wallTime();*/
      if (autoGroupEER && setMatchingDirectory(inFile, row)) {
        b3dError(stderr, "ERROR: tiffReadSection - Cannot find directory %d\n", row);
        return -1;
      }

      /* Get buffer needs */
      nstrip = TIFFNumberOfStrips(tif);
      countEERBytesAndElectrons(tif, is7bitEER, &rawTotalBytes, &maxElectrons);

      /*printf("sec %d bytes %d  max elec %d\n", row, rawTotalBytes, maxElectrons);*/
      
      /* Allocate arrays; make sure they are big enough */
      if (rawTotalBytes + 10 > rawArrSize) {
        rawArrSize = 1.05 * rawTotalBytes + 10;
        if (tmp)
          _TIFFfree(tmp);
        tmp = (unsigned char *)_TIFFmalloc(rawArrSize);
      }
      
      if (maxElectrons + 10 > elecArrSize) {
        free(positions);
        free(symbols);
        elecArrSize = 1.05 * maxElectrons + 10;
        positions = B3DMALLOC(int, elecArrSize);
        symbols = B3DMALLOC(unsigned char, elecArrSize);
      }
      if (!tmp || !positions || !symbols) {
        cleanupFromEER(tmp, freeMap ? map : NULL, positions, symbols);
        return -1;
      }
      ...
      ystart = 0;
      for (si = 0; si < nstrip; si++) {
        stripsize = TIFFRawStripSize(tif, si);
        nread = TIFFReadRawStrip(tif, si, tmp + ystart, stripsize);
        if (nread < 0) {
          cleanupFromEER(tmp, freeMap ? map : NULL, positions, symbols);
          return IIERR_IO_ERROR;
        }
      ...
      chipSize = xsize * ysize;
      for (si = 0; si < inFile->readEERasSuperRes; si++)
        chipSize /= 4;
      /*curWall = wallTime(); wallRead += curWall - wallStart; wallStart = curWall;*/

      /* Set the flags for this frame */
      if (autoGroupEER) {
        sEERflags = 0;
        if (row > secStart)
          sEERflags = IIFLAG_ADD_TO_EER_SUM;
        if (row == secStart || row == secEnd)
          sEERflags |= IIFLAG_START_END_EER_SUM;
      }
      
      /* Decode it */
      if (decodeEERimage(tmp, positions, symbols, is7bitEER, 
                         chipSize, rawTotalBytes,  &numElectrons)) {
        cleanupFromEER(tmp, freeMap ? map : NULL, positions, symbols);
        return IIERR_IO_ERROR;
      }
      /*curWall = wallTime(); wallDecode += curWall - wallStart; wallStart = curWall;*/
      
      /*printf("Actuals electrons %d  mm %d %d %d %d\n", numElectrons, xmin, xmax,
        ymin, ymax);*/
      outBufSize = moveSize * xDimension * (ymax + 1 - ymin);
      byteBufSize = xout * (ymax + 1 - ymin);
      convertEERpositions(inFile, positions, symbols, numElectrons, 
                          (unsigned char *)buf + (outBufSize - byteBufSize), xmin, xmax,
                          ymin, ymax);
      /*curWall = wallTime(); wallConvert += curWall - wallStart; wallStart = curWall;*/
    }

    /* The data need conversion/copying if convert is set, and not to a byte with no map,
       or if there is any padding */
    startFinish = (sEERflags & IIFLAG_START_END_EER_SUM);
    addToSum = (sEERflags & IIFLAG_ADD_TO_EER_SUM);
    if (((convert && (convert != MRSA_BYTE || map)) || padLeft || padRight) && 
        ((!startFinish && !addToSum) || (startFinish && addToSum))) {
      for (y = 0; y <= ymax - ymin; y++) {
        ofsin = y * xout;
        bdata = (unsigned char *)buf + (outBufSize - byteBufSize) + ofsin;
        ofsout = moveSize * (y * xDimension + padLeft);
        ...
        if (!(convert && (convert != MRSA_BYTE || map))) {
          for (si = 0; si < xout; si++)
            obuf[si] = bdata[si];
        } else {
          ...
          copyLine(bdata, obuf, xout, convert, pixsize, inFile->type, 
                   ...
                   0, map, NULL, NULL, NULL);
        }
      }
    }
    /*printf("Read %.2f  decode %.2f  convert %.2f copy %.2f\n", 1000. * wallRead,
      1000. * wallDecode, 1000. * wallConvert, 1000. * (wallTime() - wallStart));*/
    cleanupFromEER(tmp, freeMap ? map : NULL, positions, symbols);
    return 0;

  } else if (TIFFGetField(tif, TIFFTAG_ROWSPERSTRIP, &rowsperstrip)) {
...
/*
 * Uncompress the run-length encoded EER image
 * This code is based largely on renderEER.cpp from Relion source, with enhancements
 * for no wasted bits in 8-bit storage
 */
static int decodeEERimage(unsigned char *buf, int *positions, unsigned char *symbols,
                          int is7bitEER, int eer_image_pixels, int pos_limit,
                          int *numElectrons)
{
  unsigned int bit_pos = 0, first_byte, bit_offset_in_first_byte, chunk, pos = 0;
  unsigned int n_pix = 0, n_electron = 0, misAligned = 0;
  unsigned char p, s, p1, p2, s1, s2;
  ...
  if (is7bitEER) {
    ...
    for(;;) {
      ...
      /*printf("buf %p  fb %d bofb %d  %d\n", buf, first_byte, bit_offset_in_first_byte,
        eer_image_pixels);*/
      chunk = (*(unsigned int*)(buf + first_byte)) >> bit_offset_in_first_byte;
...
#ifdef NO_WASTED_BITS
      if (misAligned) {
        p1 = (buf[pos] >> 4) | (buf[pos + 1] << 4);
        s1 = (buf[pos + 1] >> 4) ^ 0x0A;
        n_pix += p1;
        if (n_pix >= eer_image_pixels)
          break;
  ...
  *numElectrons = n_electron;
  if (n_pix != eer_image_pixels) {
    b3dError(stderr, "ERROR: TIFFReadSection - Final pixel position in EER image is not "
        "right (%d instead of %d\n.", n_pix, eer_image_pixels);
    return 1;
  }
  return 0;
}
...
static void convertEERpositions(ImodImageFile *inFile, int *positions, 
                                ...
                                int ymax)
{
  int outXsize = (xmax + 1 - xmin);
  int outYsize = (ymax + 1 - ymin);
  int ind, x, y, yOffset, chipXsize = inFile->nx, chipYsize = inFile->ny;
  for (ind = 0; ind < inFile->readEERasSuperRes; ind++) {
    chipXsize /= 2;
    chipYsize /= 2;
  }
  if (!(sEERflags & IIFLAG_ADD_TO_EER_SUM))
    memset(buf, 0, outXsize * outYsize);
  yOffset = (inFile->ny - 1) - ymin;
 

  /* OpenMP is useless on this loop.  The if test actually makes little difference, 
   but using bit shifts helps when the size is standard */
  if (inFile->readEERasSuperRes == 2) {
    if (chipXsize == 4096) {
      for (ind = 0; ind < numElectrons; ind++){
        x = (((positions[ind] & 4095) << 2) | (symbols[ind] & 3)) - xmin; 
        y = yOffset - (((positions[ind] >> 12) << 2) | ((symbols[ind] & 12) >> 2));
        if (x >= 0 && x < outXsize && y >= 0 && y < outYsize)
          buf[x + y * outXsize]++;
      }
      
    } else {
      for (ind = 0; ind < numElectrons; ind++){
        x = (((positions[ind] % chipXsize) << 2) | (symbols[ind] & 3)) - xmin; 
        y = yOffset - (((positions[ind] / chipXsize) << 2) | ((symbols[ind] & 12) >> 2));
        if (x >= 0 && x < outXsize && y >= 0 && y < outYsize)
          buf[x + y * outXsize]++;
      }
    }
    
  } else if (inFile->readEERasSuperRes == 1) {
    if (chipXsize == 4096) {
      for (ind = 0; ind < numElectrons; ind++){
        x = (((positions[ind] & 4095) << 1) | ((symbols[ind] & 2) >> 1)) - xmin; 
        y = yOffset - (((positions[ind] >> 12) << 1) | ((symbols[ind] & 8) >> 3));
        if (x >= 0 && x < outXsize && y >= 0 && y < outYsize)
          buf[x + y * outXsize]++;
      }
    } else {
      for (ind = 0; ind < numElectrons; ind++){
        x = (((positions[ind] % chipXsize) << 1) | ((symbols[ind] & 2) >> 1)) - xmin; 
        y = yOffset - (((positions[ind] / chipXsize) << 1) | ((symbols[ind] & 8) >> 3));
        if (x >= 0 && x < outXsize && y >= 0 && y < outYsize)
          buf[x + y * outXsize]++;
      }
    }
    ...
    if (chipXsize == 4096) {
      for (ind = 0; ind < numElectrons; ind++){
        x = (positions[ind] & 4095) - xmin; 
        y = yOffset - (positions[ind] >> 12);
        if (x >= 0 && x < outXsize && y >= 0 && y < outYsize)
          buf[x + y * outXsize]++;
      }
    } else {
      for (ind = 0; ind < numElectrons; ind++){
        x = (positions[ind] % chipXsize) - xmin; 
        y = yOffset - (positions[ind] / chipXsize);
        if (x >= 0 && x < outXsize && y >= 0 && y < outYsize)
          buf[x + y * outXsize]++;
      }
    }
...
/*
 * Free the arrays and clear the flags from reading EER
 */
static void cleanupFromEER(unsigned char *tmp, unsigned char *map, int *positions, 
                           unsigned char *symbols)
{
 ...
 * resolution < 0 to set a resolution in DPI in the file, or resolution > 0 to set
 * one in dots per cm.
 */
                   ...
                   int quality, int *outRows, int *outNum, int *tileSizeX)
{
  ...
  TIFFSetField(tif, TIFFTAG_RESOLUTIONUNIT, 
               B3DCHOICE (resolution == 0, RESUNIT_NONE, 
                          resolution < 0 ? RESUNIT_INCH : RESUNIT_CENTIMETER));
  if (resolution != 0) {
    TIFFSetField(tif, TIFFTAG_XRESOLUTION, B3DABS((float)resolution));
    TIFFSetField(tif, TIFFTAG_YRESOLUTION, B3DABS((float)resolution));
  }
                               ...
                               int ifFloat)
{
  ...
  if (inFile->xscale != 1.)
    resolution = (getenv("IMOD_TIFF_RESOL_PER_INCH") != NULL ? -2.54e8 : 1.e8) / 
      inFile->xscale;
...
int tiffNumReadThreads(int nx, int ny, int compression, int maxThreads)
{
  ...
  if (compression == IICOMPRESSION_NONE || IS_EER_FILE(compression))
    return 1;
