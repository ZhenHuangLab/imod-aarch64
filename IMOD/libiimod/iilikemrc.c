...
static int checkWinkler(FILE *fp, char *filename, RawImageInfo *info)
{
  ...
  if (b3dFseek(fp, 16, SEEK_SET))
    return IIERR_IO_ERROR;
  if (b3dFread(svals, 2, 4, fp) != 4)
    return IIERR_IO_ERROR;
  if (info->swapBytes)
  ...
  if (svals[3] / 2 != 1)
    return IIERR_NO_SUPPORT;
  if (b3dFseek(fp, 24, SEEK_SET))
    return IIERR_IO_ERROR;

  if (b3dFread(ivals, 4, 1, fp) != 1)
    return IIERR_IO_ERROR;
  if (info->swapBytes)
    mrc_swap_longs(ivals, 1);
  info->headerSize = ivals[0];
  if (b3dFseek(fp, 64, SEEK_SET))
    return IIERR_IO_ERROR;
  if (b3dFread(ivals, 4, svals[3] * 4, fp) != svals[3] * 4)
    return IIERR_IO_ERROR;
  if (info->swapBytes)
...
static int checkPif(FILE *fp, char *filename, RawImageInfo *info)
{
  ...
  if (b3dFseek(fp, 32, SEEK_SET) != 0)
  	return IIERR_IO_ERROR;
  
  if (b3dFread(cvals, 1, 5, fp) != 5)
    return IIERR_IO_ERROR;
  ...
  if (b3dFseek(fp, 64, SEEK_SET) != 0)
    return IIERR_IO_ERROR;
    
  if (b3dFread(ivals, 4, 5, fp) != 5)
    return IIERR_IO_ERROR;
