...
#define iiuwritesecpart IIUWRITESECPART
...
#define iiusethdfcompression IIUSETHDFCOMPRESSION
...
#define iiuwritesecpart iiuwritesecpart_
...
#define iiusethdfcompression iiusethdfcompression_
 ...
 * Fortran wrapper iiuretchunksizes.
 */
...
/*!
 * Sets the ZIP compression level for the file open on unit [iunit] to [compression],
 * overriding the environment variable IMOD_HDF_COMPRESSION.
 * Fortran wrapper iiusethdfcompression.
 */
void iiuSetHDFCompression(int iunit, int compression)
{
  Unit *u = lookupUnit(iunit, "iiuSetHDFCompression", 1, 0);
  u->iiFile->hdfCompression = compression;
}

void iiusethdfcompression(int *iunit, int *compression)
{ iiuSetHDFCompression(*iunit, *compression); };
...
const char *sWritePartMess = NULL;
...
int iiuWriteSection(int iunit, char *array)
{
  ...
  sWritePartMess = mess;
  return iiuWriteSecPart(iunit, array, u->iiFile->nx, 0, 0, u->iiFile->nx - 1, 0,
                         u->iiFile->ny - 1);
}
 ...
 * advanced to the first line of the next section.  If not exiting on error, returns 
 * negative values for internal errors (bad coordinates) and positive values for an
                     ...
                     int iyStart, int iyEnd)
{
  ...
  if (!u)
    return -1;
  sprintf(mess, "\nERROR: iiuWriteSubarray - writing section %d, lines %d to %d to unit"
          " %d\n", u->currentSec - 1, iyStart, iyEnd, iunit);
  sWritePartMess = mess;

  return iiuWriteSecPart(iunit, array, nxdim, ixStart, 0, u->iiFile->nx - 1,
                         iyStart, iyEnd);
}
...
/*!
 * Writes data from a subset of [array] to the current section and line of unit [iunit],
 * where a subset of the output file in X can be written if the file is a chunked HDF file
 * and the starting and ending indexes to write to in [indX0], [indX1] correspond to chunk
 * boundaries.  The X dimension of [array] is given in [nxdim], the starting index from 
 * which to take data in X in [ixStart], and the starting and ending indices from which to
 * take data in Y in [iyStart] and [iyEnd] (inclusive, numbered from 0).  The current 
 * position is advanced to the first line of the next section.  If not exiting on error, 
 * returns negative values for internal errors (bad coordinates) and positive values for 
 * an error from the writing operation.  Fortran wrapper iiuWriteSecPart.
 */
int iiuWriteSecPart(int iunit, char *array, int nxdim, int ixStart, int indX0, 
                      int indX1, int iyStart, int iyEnd)
{
  int err;
  const char *useMess = sWritePartMess;
  Unit *u = lookupUnit(iunit, "iiuWriteSecPart", sExitOnError, 2);
  char *arrStart;
  if (!u)
    return -1;

  sWritePartMess = NULL;
  if (u->iiFile->file != IIFILE_HDF && (indX0 || indX1 != u->iiFile->nx - 1)) {
    b3dError(stdout, "\nERROR: iiuWriteSecPart - Attempting to write to a portion of "
             "a line for a non-HDF file, unit %d\n", iunit);
    EXIT_OR_RETURN(-2);
  }
  ...
  u->iiFile->padRight = nxdim - (indX1 + 1 - indX0) - ixStart;
  if (u->iiFile->padRight < 0) {
    b3dError(stdout, "\nERROR: iiuWriteSecPart - X dimension of data (%d) is not big "
             "enough for specified X indexes in writing to unit %d (xstart %d x0 %d "
             "x1 %d)\n", nxdim, iunit, ixStart, indX0, indX1);
    EXIT_OR_RETURN(-2);
  }
  if (u->iiFile->ury >= u->iiFile->ny) {
    b3dError(stdout, "\nERROR: iiuWriteSecPart - Starting and ending lines (%d to %d) to"
             " write to unit %d go past end of end for section from line %d\n",
             iyStart, iyEnd, iunit, ixStart, u->currentLine);
    EXIT_OR_RETURN(-3);
  }
  arrStart = array + iyStart * nxdim * iiuBufBytesPerPixel(iunit);
  if (u->noConvert)
    err = iiWriteSection(u->iiFile, arrStart, u->currentSec);
  else
    err = iiWriteSectionFloat(u->iiFile, arrStart, u->currentSec);
  if (err) {
    if (useMess)
      b3dError(stdout, "\nERROR: iiuWriteSecPart - writing X %d to %d, Y %d to %d to"
               " section in unit %d\n", iunit);
    EXIT_OR_RETURN(err);
  }
  u->currentSec++;
...
int iiuwritesecpart(int *iunit, char *array, int *nxdim, int *ixStart, int *indX0,
                    int *indX1, int *iyStart, int *iyEnd)
{ return iiuWriteSecPart(*iunit, array, *nxdim, *ixStart, *indX0, *indX1, *iyStart,
                          *iyEnd);}
/*!
 * Writes [numLines] lines to unit [iunit] from [array], starting at the current 
