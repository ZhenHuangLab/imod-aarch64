...
{
  return sliceNewModeEx(s, mode, 1);
}

/*!
 * Like @sliceNewMode except that existing data in the slice are freed only if [freeData]
 * is non-zero.
 */
 ...
 * is called by @sliceNewMode for float mode.  Returns -1 for error.
 */
int sliceFloat(Islice *slice)
{
  return sliceFloatEx(slice, 1);
}
...
/*!
 * Writes the data in [array] of mode [mode] and size [nx] by [ny] into a new MRC file 
 * whose name is given in [filename].  Calls @sliceWriteMRCfile and returns its return
 * value.
 */
int mrcWriteImageToFile(const char *filename, void *array, int mode, int nx, int ny)
{
  Islice slice;
  sliceInit(&slice, nx, ny, mode, array);
