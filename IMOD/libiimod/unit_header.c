...
/*!
 * Prints a brief header for unit [iunit]; if [filePrefix] is not NULL, it prints
 * the filename after that string.
 */
void iiuPrintHeader(int iunit, const char *filePrefix)
{
  ...
  int labInd;
  MrcHeader *hdr = iiuMrcHeader(iunit, "iiuPrintHeader", 1, 0);
  if (filePrefix) {
    iiFile = iiuGetIIFile(iunit);
    printf("%s: %s\n", filePrefix, iiFile->filename);
  }
  mrc_get_scale(hdr, &xscale, &yscale, &zscale);
  printf(" Dimensions: %6d %6d %6d   Pixel size:%10.4g %10.4g %10.4g\n"
         "Mode: %2d               Min, max, mean: %12.5g %12.5g %12.5g\n", 
         hdr->nx, hdr->ny, hdr->nz, xscale, yscale, zscale, hdr->mode,
         hdr->amin, hdr->amax, hdr->amean);
  for (labInd = 0; labInd < (hdr->nlabl > 1 ? 2 : 1); labInd++)
    mrcPrintLabelString(hdr, labInd ? hdr->nlabl - 1 : 0);
}
...
int iiuTransExtendedData(int intoUnit, int iunit)
{
  ...
  if (err)
    ...
    err = iiuAltExtendedData(intoUnit, numBytes, extra);
  free(extra);
  return err;
}
