                              ...
                              int fromFloat)
{
  ...
  int padLeft, padRight, yStart, yEnd, xStart, xEnd, tiled, nxOut;
  ...
  tiled = (inFile->tileSizeX > 0 && inFile->tileSizeX < nx) ? 1 : 0;
  if ((!tiled && xStart) || (tiled && (xStart % inFile->tileSizeX) != 0) || 
      (xEnd != nx - 1 && (!tiled || (tiled && ((xEnd + 1) % inFile->tileSizeX) != 0)))) {
    b3dError(stderr, "ERROR: hdfWriteSectionAny - only full lines%s can be written "
             "(xmin %d, xmax %d nx %d tile %d)\n", tiled ? " or chunks" : "",
             xStart, xEnd, nx, inFile->tileSizeX);
    return 1;
  }
  ...
  xDimension = nxOut + padLeft + padRight;
  ...
  if (needData && fromFloat >= 0) {
    chunkLines = B3DNINT(targetMemory / (nxOut * pixSizeOut));
    B3DCLAMP(chunkLines, 1, yEnd + 1 - yStart);
    tmpData = B3DMALLOC(unsigned char, pixSizeOut * nxOut * chunkLines);
    if (!tmpData) {
  ...
  memCount[rank - 1] = nxOut * fileXscale;
  memOffset[rank - 2] = 0;

  /* Setting a memory offset to the padLeft did not work; just set the write pointer to
     the start of the data to be written in the buffer and keep offset at 0 */
  memOffset[rank - 1] = 0;
  if (needData) {
    memDim[rank - 1] = nxOut * fileXscale;
  } else {
  ...
  fileCount[rank - 1] = nxOut * fileXscale;
  fileOffset[rank - 1] = xStart * fileXscale;
  fileCount[0] = 1;
  fileOffset[0] = cz;
  ...
  while (line <= yEnd) {
    ...
    for (; line <= lineEnd; line++) {
      ...
      if (convert) {
        ...
        /* Convert floats */
        iiConvertLineOfFloats(fbufp, bdata, nxOut, hdata->mode, bytesSigned, 0);
        fbufp += xDimension;
        ...
        if (bytesSigned)
          b3dShiftBytes(bufp, sbdata, nxOut, 1, 1, 1);
        bufp += pixSizeBuf * xDimension;
      }

      /* Advance output data pointer */
      bdata += pixSizeOut * nxOut;
    }
...
static hid_t createGroupAndDataset(ImodImageFile *inFile, int zValue, char *buf)
{
  ...
  int xInd = rank - 1, yInd = rank - 2;
  int fileXscale = getFileXscale(inFile->format);
  int bytesPerChanBuf, numChanBuf, numChunks, tileSizeX, tileSizeY;
  size_t nslots, nbytes, nbytesNew;
  double w0;
  char *compEnvVar;

  /* Determine compression from environment variable if not set  for this file */
  if (inFile->hdfCompression < 0) {
    inFile->hdfCompression = 0;
    compEnvVar = getenv("IMOD_HDF_COMPRESSION");
    if (compEnvVar) {
      inFile->hdfCompression = atoi(compEnvVar);
      B3DCLAMP(inFile->hdfCompression, 0, 9);
    }
  }
  ...
  groupID = H5Gcreate((hid_t)inFile->hdfFileID, buf, H5P_DEFAULT, H5P_DEFAULT, 
                      H5P_DEFAULT);
  maxDims[xInd] = fileDims[xInd] = inFile->nx * fileXscale;
  maxDims[yInd] = fileDims[yInd] = inFile->ny;
  if (rank > 2 || inFile->hdfCompression > 0) {
    cparms = H5Pcreate(H5P_DATASET_CREATE);
    if (H5Pset_deflate(cparms, inFile->hdfCompression) < 0)
      return -1;

    /* Set the chunk sizes for 3D or compressed data set */
    if (rank > 2) {
      maxDims[0] = H5S_UNLIMITED;
      fileDims[0] = inFile->nz;
      chunkDims[0] = inFile->zChunkSize;
      tileSizeX = inFile->tileSizeX;
      tileSizeY = inFile->tileSizeY;
    } else {

      /* For 2D, set them as strips so that full lines will read efficiently 
         square tiles of 256 were atrocious when read with binning in newstack */
      tileSizeX = inFile->nx;
      tileSizeY = B3DMAX(4, 65536 / tileSizeX);
    }
    if (tileSizeY <= 0 || tileSizeY > inFile->ny)
      chunkDims[yInd] = inFile->ny;
    else
      chunkDims[yInd] = tileSizeY;
    if (tileSizeX <= 0 || tileSizeX > inFile->nx)
      chunkDims[xInd] = inFile->nx * fileXscale;
    else
      chunkDims[xInd] = tileSizeX * fileXscale;
    ...
    numChunks = (inFile->nx + chunkDims[xInd] - 1) / chunkDims[xInd];
    nbytesNew = (bytesPerChanBuf * numChunks * chunkDims[xInd] * fileXscale * 
                 chunkDims[yInd] * 11) / 10;
