 ...
 *
 *  Copyright (C) 2014-2020 by the Regents of the University of 
...
static int removeAttributes(hid_t groupID);
static void freeReorderArrays(int *newMap, int *adocSecs, char *tempName, char *newName);
...
int iiHDFopenNew(ImodImageFile *inFile, const char *mode)
{
  ...
  if (inFile->file != IIFILE_UNKNOWN) {
    ...
    if (inFile->file != IIFILE_HDF || inFile->hdfSource != IIHDF_IMOD || 
        (inFile->stackSetList && inFile->nz > 1) || !inFile->writeSection) {
...
/*
 * Renames group names of sections to reorder their Z values in place.
 */
int iiReorderHDFstack(ImodImageFile *inFile, int *sectOrder)
{
  int ind,iz, newZ, dsInd, mapInd, ordInd = 0, newSize = 0, maxLen = 0;
  int *newMap = NULL, *adocSecs = NULL;
  char *tempName = NULL, *newName = NULL;
  hid_t dsetID, fileID, groupID;
  StackSetData *stackp;

  /* Check basic conditions */
  if (!inFile || !inFile->nz || !inFile->zMapSize)
    return 1;
  if (!inFile->stackSetList) {
    b3dError(stderr, "ERROR: iiReorderHDFstack - The HDF file is a volume not a stack\n");
    return 1;
  }
  for (iz = 0; iz < inFile->nz; iz++) {
    ACCUM_MAX(newSize, sectOrder[iz]);
    if (sectOrder[iz] < 0) {
      b3dError(stderr, "ERROR: iiReorderHDFstack - Negative value for new section Z\n");
      return 1;
    }
  }
      
  /* Allocate arrays for new map and list of adoc sections */
  fileID = (hid_t)inFile->hdfFileID;
  newSize += 8;
  newMap = B3DMALLOC(int, newSize);
  adocSecs = B3DMALLOC(int, inFile->nz);
  if (!newMap || !adocSecs) {
    b3dError(stderr, "ERROR: iiReorderHDFstack - Allocating new section map\n");
    freeReorderArrays(newMap, adocSecs, tempName, newName);
    return 1;
  }

  /* Fill the new map of Z values and determine maximum dataset name length */
  for (iz = 0; iz < newSize; iz++)
    newMap[iz] = -1;
  for (mapInd = 0, ordInd = 0; mapInd < inFile->zMapSize; mapInd++) {
    dsInd = inFile->zToDataSetMap[mapInd];
    if (dsInd < 0)
      continue;
    stackp = (StackSetData *)ilistItem(inFile->stackSetList, dsInd);
    ACCUM_MAX(maxLen, strlen(stackp->name));
    newZ = sectOrder[ordInd];
    if (newMap[newZ] >= 0) {
      b3dError(stderr, "ERROR: iiReorderHDFstack - Duplicate new section # %d\n", newZ);
      freeReorderArrays(newMap, adocSecs, tempName, newName);
      return 1;
    }
    newMap[newZ] = dsInd;
  ...
  /* Allocate generous arrays for names */
  maxLen += 64;
  tempName = B3DMALLOC(char, maxLen);
  newName = B3DMALLOC(char, maxLen);
  if (!tempName || !newName) {
    b3dError(stderr, "ERROR: iiReorderHDFstack - Allocating arrays for group names\n");
    freeReorderArrays(newMap, adocSecs, tempName, newName);
    return 1;
  }

  /* Loop through the map skipping non-existent Z's in old map */
  for (mapInd = 0, ordInd = 0; mapInd < inFile->zMapSize; mapInd++) {
    dsInd = inFile->zToDataSetMap[mapInd];
    if (dsInd < 0)
      continue;
    ...
    /* The first rename has to be to a unique temporary name */
    strcpy(tempName, stackp->name);
    tempName[strlen(tempName) - 6] = 0x00;
    sprintf(newName, "/MDF/images/Reordered%d", newZ);
    if (H5Lmove(fileID, tempName, fileID, newName, H5P_DEFAULT, H5P_DEFAULT) < 0) {
      b3dError(stderr, "ERROR: iiReorderHDFstack - Trying to change dataset name "
               "from original to temporary\n", mapInd);
      freeReorderArrays(newMap, adocSecs, tempName, newName);
      return 1;
    }

    /* This time, get adoc section list */
    adocSecs[ordInd] = AdocLookupByNameValue(sZvalue, mapInd);
    ordInd++;
  }

  /* Loop again for rename from temporary to final name */
  for (mapInd = 0, ordInd = 0; mapInd < inFile->zMapSize; mapInd++) {
    dsInd = inFile->zToDataSetMap[mapInd];
    if (dsInd < 0)
      continue;
    ...
    /* This time rename to a final name */
    sprintf(tempName, "/MDF/images/Reordered%d", newZ);
    sprintf(newName, "/MDF/images/%d", newZ);
    if (H5Lmove(fileID, tempName, fileID, newName, H5P_DEFAULT, H5P_DEFAULT) < 0) {
      b3dError(stderr, "ERROR: iiReorderHDFstack - Trying to change dataset name "
               "from temporary to new name\n", mapInd);
      freeReorderArrays(newMap, adocSecs, tempName, newName);
      return 1;
    }
    strcat(newName, "/image");
    free(stackp->name);
    stackp->name = strdup(newName);
    
    /* Now rename the adoc sections, not worrying about temporary name duplication 
       because we have indexes */
    if (adocSecs[ordInd] >= 0) {
      sprintf(newName, "%d", newZ);
      if (AdocChangeSectionName(sZvalue, adocSecs[ordInd], newName)) {
        b3dError(stderr, "ERROR: iiReorderHDFstack - Renaming autodoc section from Z = %d"
                 " to %d\n", mapInd, newZ);
        freeReorderArrays(newMap, adocSecs, tempName, newName);
        return 1;
      }
  ...
  /* Clean up */
  freeReorderArrays(inFile->zToDataSetMap, adocSecs, tempName, newName);
  inFile->zToDataSetMap = newMap;
  inFile->zMapSize = newSize;
  return 0;
}


/* Free arays from the reordering operation on error or at end */
static void freeReorderArrays(int *newMap, int *adocSecs, char *tempName, char *newName)
{
  free(adocSecs);
  free(newMap);
...
/*
 * Function to remove all attributes from a group - in case it is needed
 */
static int removeAttributes(hid_t groupID)
{
  int ind, err = 0, num = H5Aget_num_attrs(groupID);
  for (ind = num - 1; ind >= 0; ind--) {
    if (H5Adelete_by_idx(groupID, ".", H5_INDEX_NAME, H5_ITER_INC, (hsize_t)ind,
                         H5P_DEFAULT) < 0)
      err++;
  }
  return err;
...
static int hdfWriteHeader(ImodImageFile *inFile)
{
  ...
  if (groupID >= 0) {
    ...
    if (addIntegerAttribute(groupID, "imageid_max", &iz, 1))
      error++;
    if (addStringAttribute(groupID, "DISPLAY_ORIGIN", "LL"))
      error++;
  }
...
int hdfWriteGlobalAdoc(ImodImageFile *inFile)
{
  ...
  /* In any case, write any section-only attributes except ZValue and T as groups under
   the images group.  Loop on all the collections */
  ...
  for (coll = 0; coll < numColl && !err; coll++) {
    if (AdocGetCollectionName(coll, &collName)) {
      ...
      /* If a collection is not Zvalue or T, first try to open an existing group for the 
         collection then create it if needed */
      if (strcmp(collName, sZvalue) && strcmp(collName, "T")) {
        numSect = AdocGetNumberOfSections(collName);
        collID = H5Gopen(groupID, collName, H5P_DEFAULT);
        if (collID < 0)
          collID = H5Gcreate(groupID, collName, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
        if (collID < 0) {
          err = 1;
        } else {

          /* Loop on the sections, opening or creating a subgroup and putting the 
             entries in */
          for (sectInd = 0; sectInd < numSect && !err; sectInd++) {
            if (AdocGetSectionName(collName, sectInd, &sectName)) {
              err = 1;
            } else {
              sectID = H5Gopen(collID, sectName, H5P_DEFAULT);
              if (sectID < 0)
                sectID = H5Gcreate(collID, sectName, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
              if (sectID < 0) {
                err = 1;
              } else {
                if (adocToAttributes(sectID, collName, sectInd, NULL))
                  err = 1;
                H5Gclose(sectID);
              }
              free(sectName);
                            ...
                            const char *prefix)
{
  ...
  for (keyInd = 0; keyInd < numKeys; keyInd++) {
    ...
    } else if (!retval && valType >= ADOC_ONE_INT && valType <= ADOC_INT_ARRAY) {
      if (manageMallocBuf((void **)&sIntBuf, &sIntBufSize, numVals, 4)) {
        retval = 1;
      } else {
        numVals = 0;
        if (AdocGetIntegerArray(typeName, sectInd, key, sIntBuf, &numVals, sIntBufSize))
          retval = 1;
        else if (addIntegerAttribute(parentID, prefKey, sIntBuf, numVals))
          retval = 1;
      }

    } else if (!retval && valType >= ADOC_ONE_FLOAT && valType <= ADOC_FLOAT_ARRAY) {
      if (manageMallocBuf((void **)&sFloatBuf, &sFloatBufSize, numVals, 4)) {
        retval = 1;
      } else {
        numVals = 0;
        if (AdocGetFloatArray(typeName, sectInd, key, sFloatBuf, &numVals, sFloatBufSize))
          retval = 1;
        else if (addFloatAttribute(parentID, prefKey, sFloatBuf, numVals))
          retval = 1;
      }
    }
    free(key);
