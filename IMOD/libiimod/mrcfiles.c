...
int mrc_head_label(MrcHeader *hdata, const char *label)
{
  ...
  if (hdata->nlabl >= MRC_NLABELS)
    hdata->nlabl--;
  mrcFillLabelString(label, hdata->labels[hdata->nlabl]);
  hdata->nlabl++;
  return(0);
}

/*!
 * Copies [label] into [outLabel], which must be at least 80 bytes, and adds the standard
 * date-time string, truncating the label to 55 characters if necessary.
 */
...
/*!
 * Prints the label at index [labelInd] in the header [hdata].  Returns 1 for NULL header
 * or index out of range.
 */ 
int mrcPrintLabelString(MrcHeader *hdata, int labelInd)
{
  char label[MRC_LABEL_SIZE + 1];
  int ind, gotNonBlank = 0;
  if (!hdata || labelInd < 0 || labelInd >= hdata->nlabl)
    return 1;
  for (ind = MRC_LABEL_SIZE - 1; ind >= 0; ind--) {
    if (!gotNonBlank && hdata->labels[labelInd][ind] != ' ') {
      gotNonBlank = 1;
      label[ind + 1] = 0x00;
    }
    if (gotNonBlank)
      label[ind] = hdata->labels[labelInd][ind];
  }
    printf("%s\n", label);
    return 0;      
}
...
/*!
 * Writes a single FFT to a file named [filename]; the FFT should be in [fft] in standard
 * memory organization, with real-space dimensions of [nxReal] by [nyReal].  Data will be
 * scaled down by the image size if [ifScale] is non-zero.  Returns 1 for any errors.
 */
int mrcWriteFFT(const char *filename, float *fft, int nxReal, int nyReal, int ifScale)
{
  ...
  FILE *fp;
  double asum = 0;
  float scaleFac = (float)(1./sqrt((double)nxReal * nyReal));
  float ampl;
  int ind;
  float *shiftTemp = B3DMALLOC(float, 2 * nxReal + 4);
  if (!shiftTemp)
    return 1;
  wrapFFTslice(fft, shiftTemp, (nxReal + 2) / 2, nyReal, 0);
  if (ifScale)
    for (ind = 0; ind < (nxReal + 2) * nyReal; ind++)
      fft[ind] *= scaleFac;
  mrc_head_new(&hdr, (nxReal + 2) / 2, nyReal, 1, MRC_MODE_COMPLEX_FLOAT);
  ...
  if (fp) {
    ...
    hdr.amax = -1.e37;
    hdr.amin = 1.e37;
    for (ind = 0; ind < (nxReal + 2) * nyReal; ind += 2) {
      ampl = sqrt(fft[ind] * fft[ind] + fft[ind + 1] * fft[ind + 1]);
      asum += ampl;
      ACCUM_MIN(hdr.amin, ampl);
      ACCUM_MAX(hdr.amax, ampl);
    }
    hdr.amean = asum / (0.5 * (nxReal + 2) * nyReal);
    if (mrc_head_write(fp, &hdr) || mrc_write_slice(fft, fp, &hdr, 0, 'Z'))
      retval = 1;
    fclose(fp);
  }
  if (ifScale)
    for (ind = 0; ind < (nxReal + 2) * nyReal; ind++)
      fft[ind] /= scaleFac;
  wrapFFTslice(fft, shiftTemp, (nxReal + 2) / 2, nyReal, 1);
  free(shiftTemp);
  return retval;
}
