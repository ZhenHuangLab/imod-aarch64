...
#define iilimitedtilesize IILIMITEDTILESIZE
#define tiffseteerreadproperties TIFFSETEERREADPROPERTIES
#define tiffgetmaxeersuperres TIFFGETMAXEERSUPERRES
#else
#define iibesttilesize iibesttilesize_
#define iitestifhdf iitestifhdf_
#define iilimitedtilesize iilimitedtilesize_
#define tiffseteerreadproperties tiffseteerreadproperties_
#define tiffgetmaxeersuperres tiffgetmaxeersuperres_
#endif
...
ImodImageFile *iiNew()
{
  ...
  /* Make sure any allocated item for TIFF files (or any other where iiCopyOpen might
     be used) gets duplicated in iiCopyOpen */
  ofile->readEERasSuperRes = 0;
  ofile->directoryNums = NULL;
  ofile->adocIndex = -1;
  ...
  ofile->numVolumes = 0;
  ofile->hdfCompression = -1;   /* -1 = not set, environment variable can be used */
  return(ofile);
}
...
ImodImageFile *iiCopyOpen(ImodImageFile *inFile)
{
  ...
  if (inFile->directoryNums) {
    copy->directoryNums = ilistDup(inFile->directoryNums);
    if (!copy->directoryNums) {
      iiDelete(copy);
      b3dError(stderr, "ERROR: iiCopyOpen - Memory error copying directory list\n");
      return NULL;
    }
  }
  err = iiReopen(copy);
...
/*!
 * Set up an MRC header with the size, mode, min, max, mean, and scale vales from 
 * [inFile] and sets bytes to be unsigned.  All other values are initialized by
 * @@mrcfiles.html#mrc_head_new@, so if this is used for syncing new information into
 * an established header, other values like {nlabl} must be saved and restored.
 */
int iiSimpleFillMrcHeader(ImodImageFile *inFile, MrcHeader *hdata)
{
  ...
  hdata->xlen = inFile->nx * inFile->xscale;
  hdata->ylen = inFile->ny * inFile->yscale;
  hdata->zlen = inFile->nz * inFile->zscale;
  return 0;
}
...
FILE *iiFOpenNewVolume(ImodImageFile *inFile)
{
  ...
  if (!inFile)
    return NULL;
  if (inFile->file != IIFILE_HDF || (inFile->stackSetList && inFile->nz > 1)) {
                 ...
                 int ny, int nz)
{
  ...
  if (inFile->file == IIFILE_HDF || inFile->file == IIFILE_ADOC) {
...
{
  iiLimitedTileSize(imSize, tileSize, numTiles, multipleOf, 0);
}
 ...
 * and a factor that the tile size must be a multiple of in [multipleOf], returns an
 * optimal tile size that minimizes extra total image size in [tileSize] and the number of
 * tiles in [numTiles].  It picks a tile size less than or equal to [limit] if that is 
 * greater than 0, and less than or equal to the target unless
 * the tile size with one fewer tile gives both less extra total image size and a tile
 * size closer to the target.  Fortran wrapper iilimitedtilesize.
 */
                       ...
                       int limit)
{
  ...
  *numTiles = (imSize + *tileSize - 1) / *tileSize;
  if (*numTiles > 1) {
    ...
    if (*numTiles * *tileSize > numLess * tileLess && (limit <= 0 || tileLess <= limit) &&
...
void iilimitedtilesize(int *imSize, int *tileSize, int *numTiles, int *multipleOf, 
                       int *limit)
{
  iiLimitedTileSize(*imSize, tileSize, numTiles, *multipleOf, *limit);
}
...
{
  tiffSetEERreadProperties(*superRes, *autoGroup, *flags);
}

int tiffgetmaxeersuperres() 
{
  return tiffGetMaxEERsuperRes();
}
...
/*!
 * Changes the order of sections in an HDF stack file open on [inFile] by renaming the 
 * HDF groups containing the section data.  The array [sectOrder] specifies the Z value 
 * that each section should become; there must be the same number of values as the Z size
 * of the file, they must be non-negative, and there must be no duplicates in the list.
 * In principle, this list can contain gaps, as can the original group names for the 
 * sections, but it has been tested only without such gaps.  Returns 1 for a variety of 
 * errors.
 */
