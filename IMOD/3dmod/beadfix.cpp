...
static int    sWarnedDrawMode = 0;
static char   *sSkipList = NULL;
static char   *sFilename = NULL;
static const char *sUnitPixels = "pixels", *sUnitNm = "nm";
...
int imodPlugKeys(ImodView *vw, QKeyEvent *event)
{
  ...
  keypad = event->modifiers() & Qt::KeypadModifier;
  ...
  case Qt::Key_Slash:
    ...
    break;
  case Qt::Key_Insert:
    if (keypad && ivwGetMovieModelMode(vw)
...
void imodPlugExecute(ImodView *inImodView)
{
  ...
  sTopWin = new DockingDialog(imodDialogManager.parent(IMOD_DIALOG), &imodDialogManager,
                              "Bead Fixer", "beadfix.html#TOP", 'F', Qt::Window);
  sWindow  = new BeadFixer(sTopWin, "bead fixer");
  sTopWin->addWidgetToStack(sWindow);
  ...
  if (!firstTime) {
    ...
    diaLimitWindowPos(sWindow->width(), sWindow->height(), sLeft, sTop, sWindow);
    sWindow->move(sLeft, sTop);
  }
  firstTime = 0;
...
int BeadFixer::reread()
{
  ...
  while (fgets(line, MAXLINE, fp) != NULL) {
    ...
    if (strstr(line, " Z ") != NULL && strstr(line, " obj ") != NULL && 
        (strstr(line, "mean resid") != NULL || strstr(line, "resid-") != NULL)) {
      if (strstr(line, "resid-nm") != NULL)
        mUnitPtr = sUnitNm;
      while (fgets(line, MAXLINE, fp) != NULL) {
        if (strlen(line) < 3)
          break;
...
void BeadFixer::reportContRes()
{
  ...
  if (mNumContRes) {
    ...
    wprint("%d contour mean residuals:\n Average %.2f  SD %.2f  Max %.2f %s\n",
           mNumContRes, avg, sd, mMaxContRes, mUnitPtr);
  } else
    ...
    if(floor((double)(pts[ipt].z + 1.5f)) == inview) {
      ...
      if (bell > 0 || ((outputResid || !mMovingAll) && mIteratingMoveAll <= 0)) {
        if (mHasWeights)
          ...
          wprint("%sResid =%6.2f (%5.1f,%5.1f) pix,%5.2f SDs\n", bell > 0 ? "\a" : "",
                 resval, xr, yr, rpt->sd);
      }
...
int BeadFixer::moveToCont(int idir)
{
  ...
  if (mObjForContRes >= 0) {
    wprint("Contour mean residual %.2f %s\n", nearval, mUnitPtr);
...
int BeadFixer::insertPoint(float imx, float imy, bool keypad)
{
  ...
  if (imod->drawmode <= 0 && !sWarnedDrawMode) {
    wprint("\aModel drawing is turned off; it is toggled by T and current point "
           "features are toggled by Shift-T\n");
    sWarnedDrawMode = 1;
  } else if (imod->drawmode > 0)
    sWarnedDrawMode = 0;

  ivwGetLocation(sView, &curx, &cury, &curz);
  ivwGetImageSize(sView, &xsize, &ysize, &zsize);

  // Check for existing point on view in non-seeding and reject if one found
  cont = imodContourGet(imod);
  if (cont && sShowMode != SEED_MODE) {
    for (i = 0; i < cont->psize; i++) {
      if (B3DNINT(cont->pts[i].z) == curz) {
        wprint("\aNo point added; there is already one on this view.  "
               "Delete it first or use third mouse button to move it\n");
        return 1;
      }
    }
  }
  ...
  // With insert key or in seed mode, check for duplicate points
  if ((keypad || sShowMode == SEED_MODE) && curz == newPt.z &&
      fabs((double)(newPt.x - imx)) < 2. && fabs((double)(newPt.y - imy)) < 2.)
    return 1;
  newPt.x = imx;
  newPt.y = imy;
  newPt.z = curz;
...
BeadFixer::BeadFixer(DockingDialog *parent, const char *name)
  : DialogFrame(parent, 0, 1, NULL, NULL, true, 
...
void BeadFixer::fixSize()
{
  ...
  // Qt 5 linux:, adjustSize made it jump out, so just use the size hint
  //mTopWin->adjustSize();
  //imod_info_input();
  QSize hint = mTopWin->sizeHint();
  mTopWin->resize(B3DMIN(width(), hint.width()), 
                  B3DMIN(mTopWin->height(), hint.height()));
}
...
// Change to flag to keep on top (eliminated hack 3/12/19)
...
void BeadFixer::topCloseEvent ( QCloseEvent * e )
{
...
void BeadFixer::topChangeEvent(QEvent *e)
{
  mRoundedStyle = ImodPrefs->getRoundedStyle();
