 ...
 *  language.
 * DNM 4/2/19: Conditional because it was being called when saving the model from Linux,
 * which put back the initial xxxx strings.
 */
 ...
 * Vbox inside each frame: spacing 6, l/r margin 4, t/b margin 2
 * HBox inside Vbox inside frame: spacing 3
 * Spin box Vboxes: spacing 1
 * Other Inner vbox's all spacing 6
 ...
 * The trick to preventing horizontal compression was to get rid of the minimum size of
 * the top widget and give it the regular policy of minimum expanding
 * And the trick to preventing unwanted vertical stretch or compression was to set 
 * the infoWidget to fixed vertical policy and set the two frames from vertical policy
 * preferred to fixed.
 */
...
void InfoControls::setFontDependentWidths()
{
  ...
  int height = B3DNINT(1.45 * fontMetrics().height());
  for (int i = 0; i < 3; i++) {
    mOCPBox[i]->setMinimumWidth(minWidth);
    mOCPBox[i]->setMinimumHeight(height);
    mXYZBox[i]->setMinimumWidth(minWidth);
    mXYZBox[i]->setMinimumHeight(height);
