...
/* function unsuitable for libimod because of undo stuff */
...
void InfoWindow::editSlot(int item)
{
  ...
  case EDIT_MENU_SCALEBAR:
    scaleBarOpen();
    break;
  case EDIT_MENU_SAVE_DOCK:
    imodDialogManager.saveStackStateToSettings();
    break;
  case EDIT_MENU_REOPEN_DOCK:
    imodDialogManager.restoreStackFromSettings();
    break;
...
void InfoWindow::editObjectSlot(int item)
{
  ...
  case EOBJECT_MENU_CLEAN: /* Clean: delete empty contours */
    ...
    if (numDel <= 100) {
      ...
      delList = B3DMALLOC(int, numDel);
      if (!delList) {
        wprint("\aMemory error, nothing deleted!\n");
        return;
      }
      numDel = 0;
      for (co = 0; co < obj->contsize; co++)
        if (!obj->cont[co].psize)
          delList[numDel++] = co;
      imodDeleteListOfConts(imod, delList, numDel);

      free(delList);
      wprint("%d empty contours deleted, no undo possible\n", numDel);
    }
  ...
  case EOBJECT_MENU_SORTDIST:  // Sort all contours by distance in object
    ...
    if (obj->contsize) {
      imodGetIndex(imod, &ob, &co, &curPt);
      if (lastSortdist < 2) {
        qstr.sprintf("Are you sure you want to sort points by distance in all the "
                     "contours of object %d?", ob + 1);
        lastSortdist = dia_ask_forever(LATIN1(qstr));
        if (!lastSortdist)
          break;
      }
      for (coInd = 0; coInd < obj->contsize; coInd++) {
...
void InfoWindow::editSurfaceSlot(int item)
{
  ...
  case ESURFACE_MENU_SORT: /* sort */
    sortContours(App->cvi, imod, 1);
    break;
...
/*
 * Common routine for sorting contours by Z/surface/time
 */
void InfoWindow::sortContours(ImodView *vi, Imod *imod, int ifBySurf)
{
  ...
  if (imod->mousemode != IMOD_MMODEL){
    ...
    vi->undo->clearUnits();
    imodObjectSortBySurf(obj, ifBySurf);
    imod->cindex.contour = -1;
    imod->cindex.point   = -1;
    imodSelectionListClear(vi);
    imod_info_setocp();
