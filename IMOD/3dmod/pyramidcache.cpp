...
{
  int i, ind, tileTot, winx, winy, scale;
  int fullXsize, fullYsize, xsizeIn, ysizeIn, fullXstart, fullYstart, xstart, ystart;
  int xsizeOut, ysizeOut, bufXstart, bufYstart;
  float xoffset, yoffset;
  size_t maxBuf, bufXsize, bufYsize;
  double minZoom, zoom;
  TileCache *cache;
  ImodImageFile *image;
  for (ind = 0; ind < mNumCaches; ind++) {
    ...
    imodTrace('t', "cache %d start tile %d %d fxoffset %d %d tiles %d %d  offsets %.2f "
              "%.2f", ind, cache->startXtile, cache->startYtile, cache->firstXoffset, 
  ...
  // If cache pixels are negative, it was based on a # of sections, so we need to allow
  // for a buffer as well
  if (mVMpixels < 0) {
    mVMpixels = -mVMpixels;

    // Get the minimum zoom
    cache = &mTileCaches[mNumCaches - 1];
    diaMaximumWindowSize(winx, winy);
    fullXsize = cache->fullXsize;
    fullYsize = cache->fullYsize;
    minZoom = B3DMIN((double)winx / fullXsize, (double)winy / fullYsize);
    zoom = minZoom;
    maxBuf = 0;

    // Determine maximum buffer extent needed for any zoom between here and 1
    // Dpulicate the variables and calls of getSectionArea
    while (zoom < 1.02) {
      ind = pickBestCache(zoom, mZoomUpLimit, mZoomDownLimit, scale);
      cache = &mTileCaches[ind];
      xsizeIn = B3DMIN(fullXsize, winx / zoom);
      ysizeIn = B3DMIN(fullYsize, winy / zoom);
      fullXstart = B3DMAX(0, (fullXsize - xsizeIn) / 2);
      fullYstart = B3DMAX(0, (fullYsize - ysizeIn) / 2);
      scaledAreaSize(ind, fullXstart, fullYstart, xsizeIn, ysizeIn, xstart, ystart, 
                     xsizeOut, ysizeOut, xoffset, yoffset, true);
      optimalBufferSize(xstart, xsizeOut, cache->loadXsize, bufXstart, bufXsize);
      optimalBufferSize(ystart, ysizeOut, cache->loadYsize, bufYstart, bufYsize);
      ACCUM_MAX(maxBuf, bufXsize * bufYsize);
      zoom *= 1.02;
    }
    mVMpixels += maxBuf;
  }
...
int PyramidCache::loadRequestedTiles(int asyncLoad)
{
  ...
  while (!mRequests.empty() && !App->closing) {
