...
enum {FILE_MENU_NEW, FILE_MENU_OPEN, FILE_MENU_RELOAD, FILE_MENU_SAVE,
      ...
      EDIT_MENU_GRAIN, EDIT_MENU_ANGLES, EDIT_MENU_SCALEBAR, EDIT_MENU_SAVE_DOCK, 
      EDIT_MENU_REOPEN_DOCK, EDIT_MENU_PREFS,
      ...
      EOBJECT_MENU_SORTDIST, EOBJECT_MENU_RENUMBER, EOBJECT_MENU_COMBINE,
      ESURFACE_MENU_NEW, ESURFACE_MENU_GOTO, ESURFACE_MENU_MOVE, 
      ESURFACE_MENU_DELETE, ESURFACE_MENU_SORT,
...
class InfoWindow : public QMainWindow
{
 ...
 public:
  ...
  void setInitialHeights(bool forScreenChange);
  void resizeToHeight(int newHeight, int trimWidth = 0);
  int getResizedHeight() {return mResizedHeight;};
  void doOrSetupMove(int x, int y);
  QString getAdjustedFilePath(int timeLock);
  bool getStayingOnTop() {return mStayingOnTop;};
  void extendInfoTimer();
...
public:
 ...
 private:
  ...
  void processFile();
  void objectInfo();
  void sortContours(ImodView *vi, Imod *imod, int ifBySurf);
