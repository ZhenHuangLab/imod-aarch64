 ...
 */
MovieSequenceForm::MovieSequenceForm(DockingDialog* parent, Qt::WindowFlags fl)
  : QWidget(parent, fl)
{
  mTopWin = parent;
  setupUi(this);
...
void MovieSequenceForm::topChangeEvent(QEvent *e)
{
...
void MovieSequenceForm::setForFourRows()
{
  QSize winSize = mTopWin->sizeHint();
  QSize tableSize = table->sizeHint();
  int desired = 5.6 * (table->fontMetrics().height() + 3);
  mTopWin->resize(winSize.width(), winSize.height() + (desired - tableSize.height()));
}
...
{
  int width, extra = 8, sum = 0;
  QString str;
  bool rounded = ImodPrefs->getRoundedStyle();
  diaSetButtonWidth(addBeforeButton, rounded, 1.2, "Add Before");
  diaSetButtonWidth(addAfterButton, rounded, 1.2, "Add After");
  diaSetButtonWidth(replaceButton, rounded, 1.2, "Replace");
  diaSetButtonWidth(deleteButton, rounded, 1.2, "Delete");
  width = diaSetButtonWidth(setMovieButton, rounded, 1.2, "Use as Movie");
  width = diaSetButtonWidth(setStartButton, rounded, 1.2, "Set to Start");
  width = diaSetButtonWidth(setEndButton, rounded, 1.2, "Set to End");
  //setStartButton->setFixedWidth(width);
  //setEndButton->setFixedWidth(width);
  width = diaSetButtonWidth(runAllButton, rounded, 1.3, "Run All");
  ...
  sum += width;
  width = table->fontMetrics().width("3 objects w/ extra half-spin") + extra;
  table->setColumnWidth(2, width);
  sum += width;
  table->setMinimumWidth(sum);
}

/*
 * Close window: first ask whether to save; cancel if that fails
 */
void MovieSequenceForm::topCloseEvent( QCloseEvent *e )
{
  int choice;
  if (mModified) {
