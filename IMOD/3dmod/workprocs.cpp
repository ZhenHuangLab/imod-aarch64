...
ImodWorkproc::ImodWorkproc(ImodView *vw)
{
  ...
  mDlgDockTimers[0] = NULL;
  mDlgDockTimers[1] = NULL;
...
void ImodWorkproc::startMenuSwitch()
{
  QTimer::singleShot(1, this, SLOT(switchMenuTimeout()));
}

void ImodWorkproc::switchMenuTimeout()
{
  ivwSwitchMacMenus();
}

/*
 * Functions for timer to defer operations on dock until some time has passed
 */
void ImodWorkproc::startDlgDockTimer(int dlgClass, int timeout)
{
  QTimer **timer =  &mDlgDockTimers[dlgClass];
  QTime **runTime = &mDlgDockRunTimes[dlgClass];

  if (!*timer) {
    *timer = new QTimer();
    *runTime = new QTime();
    
    if (dlgClass == IMOD_DIALOG)
      connect(*timer, SIGNAL(timeout()), this, SLOT(imodDockTimeout()));
    else
      connect(*timer, SIGNAL(timeout()), this, SLOT(imodvDockTimeout()));
    (*timer)->setSingleShot(true);
  } else if (mLastDlgTimeout[dlgClass] - (*runTime)->elapsed() > timeout) {
    return;
  }
  (*timer)->start(timeout);
  (*runTime)->start();
  mLastDlgTimeout[dlgClass] = timeout;
}

void ImodWorkproc::imodDockTimeout()
{
  delete mDlgDockTimers[IMOD_DIALOG];
  mDlgDockTimers[IMOD_DIALOG] = NULL;
  delete mDlgDockRunTimes[IMOD_DIALOG];
  mDlgDockRunTimes[IMOD_DIALOG] = NULL;
  imodDialogManager.stackChangeTimeout();
}

void ImodWorkproc::imodvDockTimeout()
{
  delete mDlgDockTimers[IMODV_DIALOG];
  mDlgDockTimers[IMODV_DIALOG] = NULL;
  delete mDlgDockRunTimes[IMODV_DIALOG];
  mDlgDockRunTimes[IMODV_DIALOG] = NULL;
  imodvDialogManager.stackChangeTimeout();
}
...
/* DNM: New timing scheme, where the MovieProc starts a MovieTimer as soon
   ...
   between displays if possible.  With SGIs, the timing was in 10 msec
