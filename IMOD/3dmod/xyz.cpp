...
static void xyzDraw_cb(ImodView *vi, void *client, int drawflag)
{
  ...
  if ((!vi) || (!xyz) || (!drawflag) || xyz->mGLw->mClosing) 
    return;
  ...
  if (drawflag) {
    if (drawflag & IMOD_DRAW_IMAGE) {
      ...
      b3dFlushImage(xyz->mXyImage);
      b3dFlushImage(xyz->mXzImage);
      b3dFlushImage(xyz->mYzImage);
  ...
  : QMainWindow(parent, f)
{
  ...
  mXyImage = mXzImage = mYzImage = NULL;
  mSumTemp = NULL;
  mFdataXY = NULL;
  mSumTempSize = 0;
  ...
  mXtrans2 = 0;
  mYtrans2 = 0;
  mHq = (!vi->colormapImage && ImodPrefs->startInHQ()) ? 1 : 0;
  mThickness = 1;
  mLastThickness = -1;
  ...
  if (!mVi->rgbStore) {
    ...
    mThickSpin = (QSpinBox *)diaLabeledSpin(0, 1., 100., 1, NULL, this, NULL);
    mToolBar->addWidget(mThickSpin);
    diaSetSpinBox(mThickSpin, 1);
    connect(mThickSpin, SIGNAL(valueChanged(int)), this, SLOT(thicknessChanged(int)));
    mThickSpin->setToolTip("Number of slices to sum");
  }
  ...
  // the new window size
  // 7/19/19: With Qt 5 RHEL7, a show is needed here when opening after startup only when
  // App->devPixVaries, but is needed unconditionally when opening with -xyz
  show();
  imod_info_input();  
  utilInitializeScreenChange(this, mDevicePixelRatio);
  ...
  diaMaximumWindowSize(maxWinx, maxWiny, this);
  XY_PIXEL_TO_DEVICE(m, maxWinx, maxWiny, maxWinx, maxWiny);
  needWinx = (int)(mZoom * (vi->xsize + zscaled)) + ALL_BORDER;
  needWiny = (int)(mZoom * (vi->ysize + zscaled)) + ALL_BORDER;
  ...
  XY_DEVICE_TO_PIXEL(m, needWinx, needWiny, needWinx, needWiny);
  ...
  mGLw->mInitWidth = newWidth;
  mGLw->mInitHeight = newHeight;
  diaLimitWindowSize(needWinx, needWiny, this);

  // Fix the position too then set up the window
  QRect pos = geometry();
  xleft = pos.x();
  ytop = pos.y();
  diaLimitWindowPos(newWidth, newHeight, xleft, ytop, this);
...
void XyzWindow::closeEvent (QCloseEvent * e )
{
  ...
  b3dFreeCIImage(mXyImage);
  b3dFreeCIImage(mXzImage);
  b3dFreeCIImage(mYzImage);
...
void XyzWindow::changeEvent(QEvent *e)
{
  QMainWindow::changeEvent(e);
  ivwCheckAndSetMacMenu(e, this, IMOD_IMAGE);
...
void XyzWindow::GetCIImages()
{
  ...
  mXyImage = (B3dCIImage *)b3dGetNewCIImageSize
    (mXyImage, App->depth, mWinXdim1, mWinYdim1);

  mXzImage = (B3dCIImage *)b3dGetNewCIImageSize
    (mXzImage, App->depth,  mWinXdim1, mWinYdim2);

  mYzImage = (B3dCIImage *)b3dGetNewCIImageSize
    (mYzImage, App->depth,  mWinXdim2, mWinYdim1);

  if (!mXyImage || !mXzImage || !mYzImage)
...
void XyzWindow::B2Drag(int x, int y)
{
  ...
  if (dist < scaleModelRes(mVi->imod->res, mZoom / mDevicePixelRatio))
    return;
...
void XyzWindow::thicknessChanged(int value)
{
  int maxSize = b3dIMax(3, mVi->xsize * mVi->ysize, mVi->zsize * mVi->ysize,
                        mVi->xsize * mVi->zsize);
  int oldVal = mThickness;
  mThickness = value;
  if (!mVi->pyrCache && oldVal == 1 && value > 1) {
    allocateSumTemp(maxSize, mVi->xsize * mVi->ysize);
...
void XyzWindow::allocateSumTemp(int maxSize, int xySize)
{
  int pixSize = ivwGetPixelBytes(mVi->rawImageStore);
  B3DFREE(mSumTemp);
  mSumTemp = B3DMALLOC(int, maxSize);
  B3DFREE(mFdataXY);
  mFdataXY = B3DMALLOC(unsigned char, xySize * pixSize);
  mSumTempSize = maxSize;
  mFdataXYsize = xySize;
  if (!mSumTemp || !mFdataXY) {
    wprint("\aFailed to allocate array for summing slices\n");
    mThickness = 1;
    mSumTempSize = 0;
    mFdataXYsize = 0;
    diaSetSpinBox(mThickSpin, 1);
...
void XyzWindow::DrawImage()
{
  ...
  unsigned char *fdata;
  int cyi, cacheInd = -1;
  int cx, cy, cz, cxz, zinv, izStart, izEnd, sumWidth, sumHeight, useThickness;
  int imdataxsize;
  ...
  int saveXoffset1, saveYoffset1, needMax, zslice,saveWidth1, saveHeight1;
  ...
  bool flipped, needNewXZ, needNewYZ, needNewXY;
  ...
  time = ivwWindowTime(mVi, mTimeLock);
  if (time != mTimeDrawn) {
    b3dFlushImage(mXyImage);
    b3dFlushImage(mYzImage);
    b3dFlushImage(mXzImage);
    mLz = mLx = mLy = -1;
  }
  mTimeDrawn = time;
  ...
  if (mVi->pyrCache) {
    ...
    // get a cache sum and determine if XY image needed
    cacheInd = mVi->pyrCache->getBufCacheInd();
    cacheSum = mVi->pyrCache->loadedCacheSum(cacheInd);
    needNewXY = cz != mLz || mLastXoffset1 != xoffset1 || mLastYoffset1 != yoffset1 || 
      width1 != mLastWidth1 || height1 != mLastHeight1 || mThickness != mLastThickness ||
      (mThickness > 1 && (cacheInd != mLastTileCacheInd || cacheSum != mLastCacheSum));
    ...
    // Make sure temp array is big enough; allocation failure will reset thickness to 1
    if (mThickness > 1) {
      needMax = b3dIMax(3, width1 * height1, height2 * width1, height1 * width2);
      if (needMax > mSumTempSize || width1 * height1 > mFdataXYsize)
        allocateSumTemp(needMax, width1 * height1);
    }
  }
  ...
  if (cacheInd >= 0) {
    saveXoffset1 = xoffset1;
    saveYoffset1 = yoffset1;
    xoffset1 = 0;
    yoffset1 = 0;
    xoffset2 = 0;
    yoffset2 = 0;
  ...
  if (width1 > 0 && height1 > 0) {
    useThickness = thicknessToUse(cz, mVi->zsize, cacheInd >= 0 ? tileZscale : 1, izStart,
                                  izEnd);
    if (useThickness > 1) {
      zslice = cz;
      sumWidth = cacheInd < 0 ? mVi->xsize : width1;
      sumHeight = cacheInd < 0 ? mVi->ysize : height1;
      if ((cacheInd >= 0 && needNewXY) || 
          (cacheInd < 0 && (cz != mLz || cacheSum != mLastCacheSum ||
                            mThickness != mLastThickness))) {
        memset(mSumTemp, 0, 4 * sumWidth * sumHeight);
        for (iz = izStart; iz <= izEnd; iz++) {

          // Get the image from the tile cache or regular data
          if (cacheInd >= 0) {
            zImage = mVi->pyrCache->getSectionArea(iz, saveXoffset1, saveYoffset1, 
                                                   saveWidth1, saveHeight1, mZoom, false,
                                                   nx, ny, pyrXoffset, pyrYoffset, 
                                                   tileScale, status);
            if (!zImage || status < 0)
              return;
          } else {
            zslice = -1 - cz;
            zImage = ivwGetZSectionTime(mVi, iz, time);
          }

          // Add this slice into the sum
          sumPtr = mSumTemp;
          if (mVi->ushortStore) {
            for (y = 0; y < sumHeight; y++) {
              usdata = (unsigned short *)zImage[y];
              for (x = 0; x < sumWidth; x++)
                *sumPtr++ += usdata[x];
            }
          } else {
            for (y = 0; y < sumHeight; y++)
        ...
        // Get average at the end
        averageSumTemp(mFdataXY, sumWidth * sumHeight, 
                       izEnd + 1 - izStart);
      }
      b3dDrawGreyScalePixelsHQ(ivwMakeLinePointers(mVi, mFdataXY, sumWidth, sumHeight,
                                                   mVi->rawImageStore),
                               sumWidth, sumHeight, xoffset1, yoffset1, wx1, wy1,
                               width1, height1, mXyImage, mVi->rampbase, xyZoom, xyZoom,
                               mHq, zslice, App->rgba);

        // Set up fast access after all the planes have been accessed
      if (cacheInd < 0) {
        if (ivwSetupFastAccess(mVi, &imdata, 0, &cacheSum, time))
          return;
        usim = (b3dUInt16 **)imdata;
      ...
      b3dDrawGreyScalePixelsHQ(zImage, nx,ny, xoffset1, yoffset1, wx1, wy1, width1, 
                               height1, mXyImage, mVi->rampbase, xyZoom, xyZoom, mHq, cz,
                               App->rgba);
    }
    mLz = cz;
  }

  // Now that planes are loaded, get a new cache sum and evaluate need for new XZ and YZ
  // images
  if (cacheInd >= 0) {
    cacheSum = mVi->pyrCache->loadedCacheSum(cacheInd);
    needNewYZ = cx != mLx || cacheInd != mLastTileCacheInd || mLastXoffset2 != xoffset2 ||
      mLastYoffset1 != yoffset1 || mThickness != mLastThickness ||
    ...
    needNewXZ = cy != mLy || cacheInd != mLastTileCacheInd || mLastXoffset1 != xoffset1 ||
      mLastYoffset2 != yoffset2 || mThickness != mLastThickness ||
      width1 != mLastWidth1 || height2 != mLastHeight2 || cacheSum != mLastCacheSum;

    // Flush needed planes and make sure the size is adequate
    if (needNewXZ) {
      b3dFlushImage(mXzImage);
  ...
  if (width2 > 0 && height1 > 0) {
    ...
    if ((cacheInd >= 0 && needNewYZ) || 
        (cacheInd < 0 && (cx != mLx || cacheSum != mLastCacheSum ||
                          mThickness != mLastThickness))) {
 
      // Set up summing
      fdata  = mFdataYZ;
      usdata = (b3dUInt16 *)fdata;
      useThickness = thicknessToUse(cx, mVi->xsize, cacheInd >= 0 ? tileScale : 1,
                                    izStart, izEnd);
      if (useThickness > 1) {
        sumWidth = cacheInd < 0 ? mVi->zsize : width2;
        sumHeight = cacheInd < 0 ? mVi->ysize : height1;
        memset(mSumTemp, 0, 4 * sumWidth * sumHeight);
      }

      // Loop on planes, get tile cache data into array or load regular data with all the
      // possibilities
      for (iz = izStart; iz <= izEnd; iz++) {
        if (cacheInd >= 0) {
          mVi->pyrCache->scaledAreaSize(cacheInd, iz, cy, 1, 1, curXscaled, curYscaled, 
                                        x, y, tmpx, tmpy, false);
          ...
          if (flipped && !mVi->fakeImage) {
            cxz = iz + (mVi->zsize - 1) * imdataxsize;
            for (y = 0; y < ny; y++) {
              if (imdata[y]) {
                if (mVi->rgbStore) {
                  for (z = 0; z < nz; z++) {
                    fdata[3 * (z + y * nz)] = imdata[y][3 * (cxz - z * imdataxsize)];
                    fdata[3 * (z + y * nz) + 1] = imdata[y][3 * (cxz - z * imdataxsize) +
                                                            1];
                    fdata[3 * (z + y * nz) + 2] = imdata[y][3 * (cxz - z * imdataxsize) +
                                                            2];
                  }
            ...
            // Handle unflipped data
            for(z = 0; z < nz; z++) {
              if (!mVi->fakeImage && imdata[z]) {
                if (mVi->rgbStore) {
                  for (i = z, y = 0; y < ny; y++, i += nz) {
                    fdata[3 * i] = imdata[z][3 *(iz + (y * imdataxsize))];
                    fdata[3 * i + 1] = imdata[z][3 *(iz + (y * imdataxsize)) + 1];
                    fdata[3 * i + 2] = imdata[z][3 *(iz + (y * imdataxsize)) + 2];
                  }
                } else if (mVi->ushortStore) {
                  for (i = z, y = 0; y < ny; y++, i += nz)
                    usdata[i] = usim[z][iz + (y * imdataxsize)];
                } else {
                  for (i = z, y = 0; y < ny; y++, i += nz)
                    fdata[i] = imdata[z][iz + (y * imdataxsize)];
                }
              } else {
                if (mVi->rgbStore)
                  for (i= z, y = 0; y < ny; y++, i += nz)
        ...
        // Sum and average if needed
        addIntoSumTemp(fdata, sumWidth * sumHeight, useThickness);
      }
      averageSumTemp(mFdataYZ, sumWidth * sumHeight, izEnd + 1 - izStart);
    }
    ...
    b3dDrawGreyScalePixelsHQ(ivwMakeLinePointers(mVi, mFdataYZ, nzFdataYZ, ny,
                             ...
                             mYzImage, mVi->rampbase, zoomZ, xyZoom, mHq, xslice, 
                             App->rgba);
  }
  ...
  if (width1 > 0 && height2 > 0) {
    ...
    if ((cacheInd >= 0 && needNewXZ) || 
        (cacheInd < 0 && (cy != mLy || cacheSum != mLastCacheSum || 
                          mThickness != mLastThickness))) {
      fdata  = mFdataXZ;
      usdata = (b3dUInt16 *)fdata;

      // Set up summing, loop on planes
      useThickness = thicknessToUse(cy, mVi->ysize, cacheInd >= 0 ? tileScale : 1,
                                    izStart, izEnd);
      if (useThickness > 1) {
        sumWidth = cacheInd < 0 ? mVi->xsize : width1;
        sumHeight = cacheInd < 0 ? mVi->zsize : height2;
        memset(mSumTemp, 0, 4 * sumWidth * sumHeight);
      }
      for (iz = izStart; iz <= izEnd; iz++) {
        if (cacheInd >= 0) {
          mVi->pyrCache->scaledAreaSize(cacheInd, cx, iz, 1, 1, curXscaled, curYscaled, 
                                        x, y, tmpx, tmpy, false);
          ...
          for (i = 0, z = 0; z < nz; z++) {
            ...
            // Process flipped data
            if (flipped && !mVi->fakeImage && imdata[iz]) {
              zinv = mVi->zsize - 1 - z; 
              if (mVi->rgbStore) {
                for (x = 0; x < nx; x++, i++) {
                  fdata[3 * i] = imdata[iz][3 * (x + (zinv * imdataxsize))];
                  fdata[3 * i + 1] = imdata[iz][3 * (x + (zinv * imdataxsize)) + 1];
                  fdata[3 * i + 2] = imdata[iz][3 * (x + (zinv * imdataxsize)) + 2];
                }
              } else if (mVi->ushortStore)
                for(x = 0; x < nx; x++, i++)
                  usdata[i] = usim[iz][x + (zinv * imdataxsize)];
              else
                for(x = 0; x < nx; x++, i++)
                  fdata[i] = imdata[iz][x + (zinv * imdataxsize)];
            } else if (!flipped && !mVi->fakeImage && imdata[z]) {
              if (mVi->rgbStore) {
                for(x = 0; x < nx; x++, i++) {
                  fdata[3 * i] = imdata[z][3 * (x + (iz * imdataxsize))];
                  fdata[3 * i + 1] = imdata[z][3 * (x + (iz * imdataxsize)) + 1];
                  fdata[3 * i + 2] = imdata[z][3 * (x + (iz * imdataxsize)) + 2];
                }
              } else if (mVi->ushortStore)
                for (x = 0; x < nx; x++, i++)
                  usdata[i] = usim[z][x + (iz * imdataxsize)];
              else
                for (x = 0; x < nx; x++, i++)
                  fdata[i] = imdata[z][x + (iz * imdataxsize)];
            } else {
        ...
        // Add to sum and average if needed
        addIntoSumTemp(fdata, sumWidth * sumHeight, useThickness);
      }
      averageSumTemp(mFdataXZ, sumWidth * sumHeight, izEnd + 1 - izStart);
    }
    ...
    b3dDrawGreyScalePixelsHQ(ivwMakeLinePointers(mVi, mFdataXZ, nx, nzFdataXZ,
                             ...
                             wx1, wy2, width1, height2, mXzImage,
...
/*
 * Set up thickness to use given possible tile scaling, and start and end coordinates
 */
int XyzWindow::thicknessToUse(int iz, int zsize, int scale, int &izStart, int &izEnd)
{
  int useThickness = (mThickness + scale - 1) / scale;
  izStart = B3DMAX(0, iz - useThickness / 2);
  izEnd = B3DMIN(zsize - 1, iz + (useThickness - useThickness / 2 - 1));
  return useThickness;
}

/*
 * Add data into the temp array
 */
void XyzWindow::addIntoSumTemp(unsigned char *fdata, int numPix, int thickness)
{
  int x;
  b3dUInt16 *usdata = (b3dUInt16 *)fdata;
  if (thickness > 1) {
    if (mVi->ushortStore) {
      for (x = 0; x < numPix; x++)
        mSumTemp[x] += usdata[x];
...
/*
 * Divide the temp array by the number of slices and put into data array
 */
void XyzWindow::averageSumTemp(unsigned char *fdata, int numPix, int numSlices)
{
  int x;
  b3dUInt16 *usdata = (b3dUInt16 *)fdata;
  ...
  if (numSlices > 1) {
    if (mVi->ushortStore) {
      for (x = 0; x < numPix; x++)
        usdata[x] = (unsigned short)((float)mSumTemp[x] / slices + 0.5f);
    } else {
      for (x = 0; x < numPix; x++)
        fdata[x] = (unsigned char)((float)mSumTemp[x] / slices + 0.5f);
    }
  }
}
...
void XyzWindow::DrawCurrentPoint()
{
    ...
    for (pt = 0; pt < 2; pt ++) {
      if (B3DNINT(point->z) == cz) {
        ...
        b3dDrawCircle((int)(mZoom * point->x+bx),
                      (int)(mZoom * point->y+by), modPtSize, true);
      }
      if ((int)point->y == cy) {
        b3dSubareaViewport(mXorigin1, mYorigin2, mWinXdim1, mWinYdim2);
        b3dDrawCircle((int)(mZoom * point->x+bx),
                      (int)(zoomZ * (point->z + 0.5) + by2), modPtSize, true);
      }
      if ((int)point->x == cx) {
        b3dSubareaViewport(mXorigin2, mYorigin1, mWinXdim2, mWinYdim1);
  ...
  if (mVi->imod->mousemode == IMOD_MMODEL &&  pnt) {
    ...
    b3dSubareaViewport(mXorigin1, mYorigin1, mWinXdim1, mWinYdim1);
    b3dDrawCircle((int)(mZoom * pnt->x + bx), (int)(mZoom * pnt->y + by), psize, true);
    b3dColorIndex(App->curpoint);
    b3dSubareaViewport(mXorigin1, mYorigin2, mWinXdim1, mWinYdim2);
...
void XyzWindow::Draw()
{
  int mx, my, imz, whichBox;
  float imx, imy;
  mGLw->updateGL();

  // Update pixel view if mouse is in this window
  if (pixelViewOpen) {
    XY_PIXEL_TO_DEVICE(m, mGLw->mapFromGlobal(QCursor::pos()).x(),
                       mGLw->mapFromGlobal(QCursor::pos()).y(), mx, my);
    if (mx >= 0 && mx < mWinx && my >= 0 && my < mWiny) {
      whichBox = Getxyz(mx, my, imx, imy, imz);
...
void XyzWindow::screenChanged(QScreen *screen)
{
  float newDPR = utilGetNewDevPixRatio(this);
  if (newDPR) {
    utilSetZoomOnScreenChange(0., mLastXsizeChange, mScreenChanged, mScreenResizeTime,
                              mZoom);
    HANDLE_DPR_CHANGE_FOR_MAC(mGLw->fakeResize, mWinx, mWiny);
  }
}
#endif
...
void XyzWindow::keyPressEvent ( QKeyEvent * event )
{
  ...
  XY_PIXEL_TO_DEVICE(m, mGLw->mapFromGlobal(QCursor::pos()).x(),
                     mGLw->mapFromGlobal(QCursor::pos()).y(), ix, iy);
  ...
  : QGLWidget(inFormat, parent)
{
  ...
  mClosing = false;
  mFirstDraw = 3;
  mInitWidth = mInitHeight = 0;
}

// When the timer fires after the first draw, do another draw
void XyzGL::timerEvent(QTimerEvent * e )
{
  if (!imodLoopStarted() || (!mInitWidth && !mInitHeight))
    return;
  if (mFirstDraw < 2)
    killTimer(mTimerID);
#if defined(Q_OS_LINUX) && QT_VERSION >= 0x050000
  if (mFirstDraw > 0) {
    mWin->resize(mInitWidth + mFirstDraw - 1, mInitHeight);
  } else
#endif
  updateGL();
  mFirstDraw = B3DMAX(0, mFirstDraw - 1);
}
...
void XyzGL::paintGL()
{
  ...
  b3dSetCurDevPixRatio(mWin->mDevicePixelRatio);
  z = mWin->mZoom;
  bx = mWin->mXwoffset1;
  by = mWin->mYwoffset1;
  bx2 = mWin->mXwoffset2;
  ...
  mWin->mScaleBarSize = scaleBarDraw(mWin->mWinx, mWin->mWiny, z, 0, this, 
                                     mWin->mDevicePixelRatio);
  mWin->drawTools();
  glFlush();
}
...
void XyzGL::resizeGL( int winx, int winy )
{
  ...
  if (mWin->mWinx > 0 && mWin->mDevicePixelRatio > 0) {
    utilSetZoomOnScreenChange((float)winx / mWin->mWinx, mWin->mLastXsizeChange,
                              mWin->mScreenChanged, mWin->mScreenResizeTime,
                              mWin->mZoom);
  }
...
void XyzGL::mousePressEvent(QMouseEvent * event )
{
  ...
  int mz, ex, ey;
  mMousePressed = true;
  XY_PIXEL_TO_DEVICE(mWin->m, event->x(), event->y(), ex, ey);
  ...
  if (event->button() == ImodPrefs->actualButton(1) && !button2 && !button3) {
    but1downt.start();
    mWin->mWhichbox = mWin->Getxyz(ex, ey, mx, my, mz);
