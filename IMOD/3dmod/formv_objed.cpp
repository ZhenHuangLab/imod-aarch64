 ...
 */
imodvObjedForm::imodvObjedForm(DockingDialog* parent, Qt::WindowFlags fl)
  : QWidget(parent, fl)
{
  mTopWin = parent;
  setupUi(this);
...
void imodvObjedForm::init()
{
  ...
  if (Imodv->standalone) {
    syncToCurObjBox->hide();
  } else {
    syncToCurObjBox->setChecked(Imodv->syncObjedToCurObj != 0);
    connect(syncToCurObjBox, SIGNAL(toggled(bool)), this,SLOT(syncToCurObjToggled(bool)));
  }
...
void imodvObjedForm::topChangeEvent(QEvent *e)
{
  int i;
  int width = 0, height = 0;
  ObjectEditField *oef;
...
void imodvObjedForm::syncToCurObjToggled(bool state)
{
  Imodv->syncObjedToCurObj = state ? 1 : 0;
  if (state)
    imodvObjedSelect(objectSpinBox->value());
}

void imodvObjedForm::typeSelected( int item )
{
  imodvObjedDrawData(item, false);
}

void imodvObjedForm::styleSelected( int item )
{
  imodvObjedStyleData(item, false);
}
...
{
  diaSetWidgetColor(colorFrame, color);
}
...
void imodvObjedForm::topCloseEvent( QCloseEvent * e )
{
  if (meshingBusy()) {
    e->ignore();
    return;
