...
void ImageScaleWindow::applyLimits()
{
  ...
  // If multiple copies are open, apply to them
  if (vi->numReadThreads > 1 && vi->image == vi->fileCopies[0])
    for (k = 1; k < vi->numReadThreads; k++)
      iiSetMM(vi->fileCopies[k], vi->li->smin, vi->li->smax,
              vi->ushortStore ? 65535. : 255.);
