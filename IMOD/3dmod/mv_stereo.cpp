...
}imodvStereoData = {0, 0, NULL, NULL, 0, 0, 0, 0, 0, 0, 0, 0., NULL, NULL};
...
static void stereoSetUp(void)
{
  ...
  int nx, nwidth, ny , nheight, scrnWidth;
  ...
  if (Imodv->stereo == IMODV_STEREO_HW || Imodv->stereo == IMODV_STEREO_TB) {
    ...
    if (Imodv->stereo == IMODV_STEREO_HW) {
      ...
      diaScreenSize(scrnWidth, nheight, Imodv->mainWin);
      diaLimitWindowSize(nwidth, nheight, Imodv->mainWin);
      diaLimitWindowPos(nwidth, nheight, nx, ny, Imodv->mainWin);
    }
