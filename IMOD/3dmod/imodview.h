 ...
 */
bool DLL_EX_IM ivwGrayScaleImageLoaded(ImodView *inImodView);
 ...
 */
float DLL_EX_IM ivwGetTopZapDevPixelRatio(ImodView *inImodView);
...
/*!
 * Returns the image coordinates of the mouse in the top Slicer window in 
 * [imagePt].  The X and Y coordinates may be out of range if the mouse is 
 * outside the graphics area.  The return value is 1 if there is no Slicer window.
 */
int DLL_EX_IM ivwGetTopSlicerMouse(ImodView *inImodView, Ipoint *imagePt);

/*!
 * Returns the window type (e.g., ZAP_WINDOW_TYPE, etc) calling a plugin with a key,
 * mouse or general event
 */
int DLL_EX_IM ivwGetPlugEventSource();
