...
class SlicerFuncs {
 ...
 public:
  ...
  int checkPlugUseMouse(QMouseEvent *event, int ex, int ey, int but1, int but2, int but3);
  ...
  int manageBuffers();
  void screenChanged(float newDPR);
  ...
  void rubberBandImageCoords(float &x0, float &x1, float &y0, float &y1, float &z0,
                             float &z1);
 ...
 private:
  ...
  B3dCIImage   *mImage;
  int          *mSliceBuf;    /* Buffer in which to compose slice */
  unsigned char *mRedTemp;    /* Arrays to save channels for RGB */
  unsigned char *mGreenTemp;
  int    mBufXsize, mBufYsize;  /* Dimensions of data packed into buffer */
  int    mCurBufSize;          /* Number of elements in buffer */
  ...
  bool  mUseLastScale;  /* Flag to use the last scale, for a montage snapshot */
  float mSumMean, mSumSD;  /* Current sum and SD */
  float mScale, mOffset;   /* Scale and offset to apply */
