 ...
 * In the form_xxx.cpp, load interface element and unload it
 *
 * Do not despair, just saving/restoring a program state variable just takes
 * one line in each place, see usewheelforsize, etc, or imodDlgStackStates etc,
 * which takes a private member variable and getSetMember in .h, read of settings which
 * sets the default, and write.
 * 
 * Hint: settings file on Linux is ~/.config/BL3DEMC/3dmod.conf
 * TODO maybe: next time adding to dialog, consider adding maxSlicerBufMB
 */
...
#ifdef Q_OS_MACX
                            "Macintosh (Aqua)", "Fusion", "Macintosh",
#endif
                            ""};
#endif
...
ImodPreferences::ImodPreferences(char *cmdLineStyle)
{
  ...
  prefs->iconifyImodvDlgDflt = true;
  prefs->iconifyImodDlgDflt = true;
  prefs->iconifyImageWinDflt = false;
  prefs->stackImodDlgsDflt = true;
  prefs->stackImodvDlgsDflt = true;
  prefs->raiseImodDlgStackDflt = true;
  prefs->raiseImodvDlgStackDflt = true;
  prefs->keepDlgStackOnTopDflt = true;
  prefs->dlgFrameAdjustmentDflt = isWindows10() > 0 ? 7 : 0;
  prefs->eerSuperResDflt = 1;
  prefs->eerZbinningDflt = 10;
  ...
  prefs->slicerPanKbDflt = 6000;
  prefs->maxSlicerBufMBDflt = 500;
  prefs->speedupSliderDflt = false;
  prefs->loadUshortsDflt = false;
  prefs->loadIntIfMeanSDDflt = true;
  prefs->loadIntIfEstimateDflt = true;
  prefs->preferMeanSDDflt = false;
  prefs->numSDsForScalingDflt = 6;
  prefs->scaleScanTypeDflt = 0;
  prefs->changeMRCstatsDflt = false;  // ???
  prefs->useAliPieceCoordsDflt = 0;
  prefs->isoHighThreshDflt = false;
  prefs->isoBoxLimitDflt = 500;
  prefs->isoBoxInitialDflt = 130;
#ifdef Q_OS_MACX
  prefs->keySetsHWstereoDflt = false;
#else
  prefs->keySetsHWstereoDflt = true;
#endif
  prefs->noVertBufForContDflt = false;
  prefs->noVBOForSphereDflt = false;
  ...
  READBOOL(stackImodDlgs);
  READBOOL(stackImodvDlgs);
  READBOOL(raiseImodDlgStack);
  READBOOL(raiseImodvDlgStack);
  READBOOL(keepDlgStackOnTop);
  READNUM(dlgFrameAdjustment);
  READNUM(eerSuperRes);
  READNUM(eerZbinning);
  ...
  mImodDlgsInStack = settings->value("imodDlgsInStack", ("")).toString();
  mImodDlgStackStates = settings->value("imodDlgStackStates", "").toString();
  mImodvDlgsInStack = settings->value("imodvDlgsInStack", "").toString();
  mImodvDlgStackStates = settings->value("imodvDlgStackStates", "").toString();
  
  READNUM(snapQuality);
  READNUM(snapDPI);
  READBOOL(scaleSnapDPI);
  READNUM(slicerPanKb);
  READNUM(maxSlicerBufMB);
  READBOOL(speedupSlider);
  READBOOL(loadUshorts);
  READBOOL(loadIntIfMeanSD);
  READBOOL(loadIntIfEstimate);
  READBOOL(preferMeanSD);
  READNUM(numSDsForScaling);
  READNUM(scaleScanType);
  READBOOL(changeMRCstats);
  READNUM(useAliPieceCoords);
  READBOOL(isoHighThresh);
  READNUM(isoBoxInitial);
  READNUM(isoBoxLimit);
  READBOOL(keySetsHWstereo);
  READBOOL(noVertBufForCont);
  READBOOL(noVBOForSphere);
  ...
  for (i = 0; i < MAX_GEOMETRIES; i++) {
    ...
    mGeomModView[i].setRect(0, 0, 0, 0);
    str.sprintf("geomImageSizeV2/%d", i);
    if (settings->contains(str)) {
    ...
    str.sprintf("geomModelView/%d", i);
    if (settings->contains(str)) {
      str = settings->value(str).toString();
      sscanf(LATIN1(str), "%d,%d,%d,%d", &left, &top, &width, &height);
      mGeomModView[i].setRect(left, top, width, height);
      readin = true;
    }
  }
  ...
  if (settings->contains("scaleBarParams")) {
    ...
    if (settings->contains("scaleBarParams2")) {
      str = settings->value("scaleBarParams2").toString();
      sscanf(LATIN1(str), "%d,%d,%d", &left, &scaleParm->labelSize,
             &scaleParm->labelYoffset);
      scaleParm->drawLabels = left != 0;
    }
  }
  ...
  Imodv->syncObjedToCurObj = settings->value("syncObjedToCurObj").toInt();
  imodvObjedSetCurFrame(settings->value("mvObjedCurrentFrame").toInt());

  // App->cvi is initialized AFTER reading preferences so we need to save this here
  if (settings->contains("ghostMode"))
    mGhostMode = settings->value("ghostMode").toInt();
  else
    mGhostMode = IMOD_GHOST_2SHADES;
  mGhostDist = B3DMAX(0, settings->value("ghostDist").toInt());
...
#else
      prefs->styleKey = "Fusion";
#endif
      if (!styleOK(prefs->styleKey) && styleOK("Windows"))
...
void ImodPreferences::saveSettings(int modvAlone)
{
  ...
  if (prefs->rememberGeom && !modvAlone) {
    ...
    if (geomInd < 0) {
      for (i = firstEmpty; i > 0; i--) {
        ...
        mGeomModView[i] = mGeomModView[i - 1];
      }
      geomInd = 0;
      ...
      // Or if there is a match and Zap or Model view hasn't been open, preserve entry
      if (mRecordedZapGeom.width() == 0)
        mRecordedZapGeom = mGeomZapWin[geomInd];
      if (mRecordedModViewGeom.width() == 0)
        mRecordedModViewGeom = mGeomModView[geomInd];
    }
    ...
    mGeomModView[geomInd] = mRecordedModViewGeom;
    ...
    for (i = 0; i < MAX_GEOMETRIES; i++) {
      if (mGeomImageXsize[i]) {
        ...
        str.sprintf("geomModelView/%d", i);
        str2.sprintf("%d,%d,%d,%d", mGeomModView[i].left(), 
                     mGeomModView[i].top(),
                     mGeomModView[i].width(), mGeomModView[i].height());
        settings->setValue(str, str2);
}
    }
  }
  ...
  WRITE_IF_CHANGED(stackImodDlgs);
  WRITE_IF_CHANGED(stackImodvDlgs);
  WRITE_IF_CHANGED(raiseImodDlgStack);
  WRITE_IF_CHANGED(raiseImodvDlgStack);
  WRITE_IF_CHANGED(keepDlgStackOnTop);
  WRITE_IF_CHANGED(dlgFrameAdjustment);
  WRITE_IF_CHANGED(eerSuperRes);
  WRITE_IF_CHANGED(eerZbinning);
  ...
  WRITE_IF_CHANGED(maxSlicerBufMB);
  WRITE_IF_CHANGED(speedupSlider);
  WRITE_IF_CHANGED(loadUshorts);
  WRITE_IF_CHANGED(loadIntIfMeanSD);
  WRITE_IF_CHANGED(loadIntIfEstimate);
  WRITE_IF_CHANGED(preferMeanSD);
  WRITE_IF_CHANGED(numSDsForScaling);
  WRITE_IF_CHANGED(scaleScanType);
  WRITE_IF_CHANGED(changeMRCstats);
  WRITE_IF_CHANGED(useAliPieceCoords);
  WRITE_IF_CHANGED(isoHighThresh);
  WRITE_IF_CHANGED(isoBoxLimit);
  WRITE_IF_CHANGED(isoBoxInitial);
  WRITE_IF_CHANGED(keySetsHWstereo);
  WRITE_IF_CHANGED(noVertBufForCont);
  WRITE_IF_CHANGED(noVBOForSphere);
  ...
  str.sprintf("%d,%d,%d,%d,%d,%d", scaleParm->drawLabels ? 1 : 0, scaleParm->labelSize,
              scaleParm->labelYoffset, 0, 0, 0);
  settings->setValue("scaleBarParams2", str);
  ...
  settings->setValue("syncObjedToCurObj", Imodv->syncObjedToCurObj);
  settings->setValue("mvObjedCurrentFrame", imodvObjedGetCurFrame());

  settings->setValue("ghostMode", B3DCHOICE(App->cvi->ghostmode & IMOD_GHOST_SECTION,
                                            App->cvi->ghostmode, App->cvi->ghostlast));
  settings->setValue("ghostDist", App->cvi->ghostdist);
  ...
  settings->setValue("imodDlgsInStack", mImodDlgsInStack);
  settings->setValue("imodDlgStackStates", mImodDlgStackStates);
  settings->setValue("imodvDlgsInStack", mImodvDlgsInStack);
  settings->setValue("imodvDlgStackStates", mImodvDlgStackStates);
...
void ImodPreferences::editPrefs()
{
  ...
  mTabDlg->mStackWidget->setCurrentIndex(mCurrentTab);
  mTabDlg->mListBox->setCurrentRow(mCurrentTab);
  mTabDlg->show();
}
...
void ImodPreferences::donePressed()
{
  ...
  mTabDlg->mLayoutForm->unload();
  mTabDlg->mAppearForm->unload();
  mTabDlg->mSnapForm->unload();
  mTabDlg->mScalingForm->unload();
  ...
  curp->stackImodDlgsChgd |= !equiv(newp->stackImodDlgs, oldp->stackImodDlgs);
  curp->stackImodvDlgsChgd |= !equiv(newp->stackImodvDlgs, oldp->stackImodvDlgs);
  curp->raiseImodDlgStackChgd |= !equiv(newp->raiseImodDlgStack, oldp->raiseImodDlgStack);
  curp->raiseImodvDlgStackChgd |= !equiv(newp->raiseImodvDlgStack, 
                                         oldp->raiseImodvDlgStack);
  curp->keepDlgStackOnTopChgd |= !equiv(newp->keepDlgStackOnTop, oldp->keepDlgStackOnTop);
  curp->dlgFrameAdjustmentChgd |= newp->dlgFrameAdjustment != oldp->dlgFrameAdjustment;
  curp->eerSuperResChgd |= newp->eerSuperRes != oldp->eerSuperRes;
  curp->eerZbinningChgd |= newp->eerZbinning != oldp->eerZbinning;
  ...
  curp->maxSlicerBufMBChgd |= newp->maxSlicerBufMB != oldp->maxSlicerBufMB;
  ...
  curp->loadIntIfMeanSDChgd |= !equiv(newp->loadIntIfMeanSD, oldp->loadIntIfMeanSD);
  curp->loadIntIfEstimateChgd |= !equiv(newp->loadIntIfEstimate, oldp->loadIntIfEstimate);
  curp->preferMeanSDChgd |= !equiv(newp->preferMeanSD, oldp->preferMeanSD);
  curp->numSDsForScalingChgd |= newp->numSDsForScaling != oldp->numSDsForScaling;
  curp->scaleScanTypeChgd |= newp->scaleScanType != oldp->scaleScanType;
  curp->changeMRCstatsChgd |= !equiv(newp->changeMRCstats, oldp->changeMRCstats);
  curp->useAliPieceCoordsChgd |= newp->useAliPieceCoords != oldp->useAliPieceCoords;
  ...
  curp->noVertBufForContChgd |= !equiv(newp->noVertBufForCont, oldp->noVertBufForCont);
  curp->noVBOForSphereChgd |= !equiv(newp->noVBOForSphere, oldp->noVBOForSphere);
...
void ImodPreferences::timerEvent(QTimerEvent *e)
{
  ...
  if (mDialogPrefs.dlgFrameAdjustment != mCurrentPrefs.dlgFrameAdjustment)
    dlgFrameAdjChanged();
  pointSizeChanged();
}
...
void ImodPreferences::defaultPressed()
{
  ...
  bool adjChanged = mDialogPrefs.dlgFrameAdjustment != prefs->dlgFrameAdjustmentDflt;
  findCurrentTab();
  ...
  case 0:  // APPEARANCE
    ...
    prefs->maxSlicerBufMB = prefs->maxSlicerBufMBDflt;
  ...
  case 1:  // SCALING
    ...
    prefs->loadIntIfMeanSD = prefs->loadIntIfMeanSDDflt;
    prefs->loadIntIfEstimate = prefs->loadIntIfEstimateDflt;
    prefs->preferMeanSD = prefs->preferMeanSDDflt;
    prefs->numSDsForScaling = prefs->numSDsForScalingDflt;
    prefs->scaleScanType = prefs->scaleScanTypeDflt;
    prefs->changeMRCstats = prefs->changeMRCstatsDflt;
    prefs->useAliPieceCoords = prefs->useAliPieceCoordsDflt;
    ...
    prefs->eerSuperRes = prefs->eerSuperResDflt;
    prefs->eerZbinning = prefs->eerZbinningDflt;
    mTabDlg->mScalingForm->update();
    break;
  ...
  case 3: // BEHAVIOR
    ...
    prefs->noVertBufForCont = prefs->noVertBufForContDflt;
    prefs->noVBOForSphere = prefs->noVBOForSphereDflt;
  ...
  case 4: // LAYOUT
    ...
    prefs->stackImodDlgs = prefs->stackImodDlgsDflt;
    prefs->raiseImodDlgStack = prefs->raiseImodDlgStackDflt;
    prefs->keepDlgStackOnTop = prefs->keepDlgStackOnTopDflt;
    prefs->stackImodvDlgs = prefs->stackImodvDlgsDflt;
    prefs->raiseImodvDlgStack = prefs->raiseImodvDlgStackDflt;
    prefs->dlgFrameAdjustment = prefs->dlgFrameAdjustmentDflt;
    mTabDlg->mLayoutForm->update();
    if (adjChanged)
      dlgFrameAdjChanged();
    break;
...
{
  mCurrentTab = mTabDlg->mStackWidget->currentIndex();
}
...
// The current adjustment to subtract from docking dialog frames
int ImodPreferences::dlgFrameAdjustment()
{
  if (mTabDlg)
    return mDialogPrefs.dlgFrameAdjustment;
  return mCurrentPrefs.dlgFrameAdjustment;
}

// A call to restack dialogs when frame adjustment changes
void ImodPreferences::dlgFrameAdjChanged()
{
  mChangingFrameAdj = true;
  imodDialogManager.restackDialogs();
  imodvDialogManager.restackDialogs();
  mChangingFrameAdj = false;
}
...
{
  bool retval;
  ImodPrefStruct *prefs = mTabDlg ? &mDialogPrefs : &mCurrentPrefs;
  retval = prefs->styleKey.indexOf(QString("aqua"), 0, Qt::CaseInsensitive) >= 0 
    || prefs->styleKey.indexOf(QString("macintosh"), 0, Qt::CaseInsensitive) >= 0;
  return retval;
}
...
void ImodPreferences::setInfoGeometry()
{
  ...
  // It does not work to try to set geometry with varying DPI monitors
  if (doneOnce || !mCurrentPrefs.rememberGeom || App->devPixVaries) {
    if (!doneOnce)
      App->infoInitialSetup--;
    return;
  }
  doneOnce++;
...
{
  int i = getGeometryIndex();
  if (i >= 0)
    return (mGeomZapWin[i]);
  return QRect(0, 0, 0, 0);
}

// Return the geometry for the modelview window that matches current image
QRect ImodPreferences::getModViewGeometry()
{
  int i = getGeometryIndex();
  if (i >= 0)
    return (mGeomModView[i]);
  return QRect(0, 0, 0, 0);
}

int ImodPreferences::getGeometryIndex()
{
 int i;
  if (mCurrentPrefs.rememberGeom && !App->devPixVaries) {
    for (i = 0; i < MAX_GEOMETRIES; i++)
      if (App->cvi->xsize == mGeomImageXsize[i] &&
          App->cvi->ysize == mGeomImageYsize[i])
        return i;
  ...
  : QDialog(parent)
{
  ...
  // move mouse to the header.  The list box is the good solution
  // WHEN YOU ADD ONE, CHANGE THE CLAMP ON mCurrentTab WHEN READING SETTINGS
  mStackWidget = new QStackedWidget();
  mListBox = new QListWidget(this);
  mAppearForm = new AppearanceForm();
  mStackWidget->addWidget(mAppearForm);
  mListBox->addItem("Look/Limits");
  mScalingForm = new PrefScalingForm();
  mStackWidget->addWidget(mScalingForm);
  mListBox->addItem("Load/Scale");
  mSnapForm = new SnapshotForm();
  mStackWidget->addWidget(mSnapForm);
  mListBox->addItem("Snapshots");
  mBehaveForm = new BehaviorForm();
  mStackWidget->addWidget(mBehaveForm);
  mListBox->addItem("Behavior");
  mLayoutForm = new LayoutForm();
  mStackWidget->addWidget(mLayoutForm);
  mListBox->addItem("Windows");
  mMouseForm = new MouseForm();
  mStackWidget->addWidget(mMouseForm);
  mListBox->addItem("Mouse");
  setWindowTitle("3dmod: Set preferences");
  
  QVBoxLayout *mainLayout = new QVBoxLayout;
  QHBoxLayout *stackLayout = diaHBoxLayout(mainLayout);
  QVBoxLayout *vLayout = diaVBoxLayout(stackLayout);
  vLayout->addWidget(mListBox);
  stackLayout->setContentsMargins(0,0,0,0);
  stackLayout->setSpacing(0);
  mainLayout->setContentsMargins(0,0,0,0);
  vLayout->setContentsMargins(9, 15, 0, 0);
  mListBox->setFocusPolicy(Qt::NoFocus);
  mListBox->setSelectionMode(QAbstractItemView::SingleSelection);
  vLayout->addStretch();
  vLayout->setSpacing(3);

  connect(mListBox, SIGNAL(currentRowChanged(int)), this, SLOT(panelSelected(int)));

  stackLayout->addWidget(mStackWidget);
  QHBoxLayout *butLayout = diaHBoxLayout(mainLayout);
  butLayout->setContentsMargins(9, 9, 9, 9);
  setLayout(mainLayout);

  mDoneBut = new QPushButton("Done");
  butLayout->addWidget(mDoneBut);
  connect(mDoneBut, SIGNAL(clicked()), ImodPrefs, SLOT(donePressed()));
  
  mDefaultsBut = new QPushButton("Defaults for Panel");
  butLayout->addWidget(mDefaultsBut);
  connect(mDefaultsBut, SIGNAL(clicked()), ImodPrefs, SLOT(defaultPressed())); 

  mCancelBut = new QPushButton("Cancel");
  butLayout->addWidget(mCancelBut);
  connect(mCancelBut, SIGNAL(clicked()), ImodPrefs, SLOT(cancelPressed()));
  setFontDependentWidths();
  utilManageListStackSizes(mListBox, mStackWidget);
}

void PrefsDialog::panelSelected(int which)
{
  mStackWidget->setCurrentIndex(which);
}

void PrefsDialog::setFontDependentWidths()
{
  int width;
  width = diaSetButtonWidth(mCancelBut, ImodPrefs->getRoundedStyle(), 1.8, "Cancel");
  mDoneBut->setFixedWidth(width);
  diaSetButtonWidth(mDefaultsBut, ImodPrefs->getRoundedStyle(), 1.2, 
                            "Defaults for Panel");
}
