...
class ZapFuncs
{
 ...
 private:
  ...
  void fillOverlayRGB(unsigned char **lines, int nx, int ny, int chan,
                      unsigned char *image);
  void drawModel();
  void drawMesh(Iobj *obj, int ob);
  void drawContour(int co, int ob);
  void drawCurrentPoint();
  void drawExtraObject();
  int  drawAuto();
  void drawGhost();
  void setGhostColor(float obr, float obg, float obb, int shade);
  void drawTools();
  int  xpos(float x);
  int  ypos(float x);
 ...
 private:
  ...
  int    mLmx,       mLmy;
  Icont *mTessCont;
  int    mTessMaxPoints;        // Size of pts array in tesselator cont
  int    *mNestContMap;         // Map from contour number to # in current Z level
  int    *mContsAtCurZ;         // Contour lists from Z tables routine for mSection
  int    mNumNests;             // arrays to hold nesting analysis for object
  int   *mNestInd;
  Nesting *mNests;
