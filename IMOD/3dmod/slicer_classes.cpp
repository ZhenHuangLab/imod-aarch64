 ...
 *
 *  Copyright (C) 1995-2019 the Regents of the University of 
...
static PopupEntry sPopupTable[] = {
  ...
  {"Toggle FFT mode on or off", Qt::Key_F, 0, 1, 0},
...
void SlicerWindow::buildToolBar2(bool freeBar)
{
  ...
  // A frame for the cube widget; make the cube with the default GL format on second draw
  // of the main window
  mCubeFrame = new QFrame(rightBox);
  topHLay->addWidget(mCubeFrame);
  mCubeFrame->setFixedWidth(85);
  mCubeFrame->setFrameShadow(QFrame::Sunken);
  mCubeFrame->setFrameShape(QFrame::StyledPanel);
...
{
  QObjectList objList;
  SlicerWindow *slicer, *master = NULL;
  int i;
...
void SlicerWindow::screenChanged(QScreen *screen)
{
  mFuncs->screenChanged(utilGetNewDevPixRatio(this));
}
#endif
...
{
  if (utilIgnoreClosing(e, mFuncs->mClosing != 0))
    return;
  mFuncs->closing();
  e->accept();
  delete mFuncs;
...
{
  if (!imodLoopStarted() || (!mInitWidth && !mInitHeight) || mFuncs->mClosing)
    return;
  if (mFirstDraw < 2)
    killTimer(mTimerID);
#if defined(Q_OS_LINUX) && QT_VERSION >= 0x050000
  if (mFirstDraw > 0) {
    mFuncs->mQtWindow->resize(mInitWidth + mFirstDraw - 1, mInitHeight);
  } else
#endif
    updateGL();
  if (mFirstDraw < 2) {

    // Add the cube after the second resize of main window
    QGLFormat glFormat;
    QVBoxLayout *cubeLayout = new QVBoxLayout(mFuncs->mQtWindow->mCubeFrame);
    cubeLayout->setContentsMargins(2, 2, 2, 2);
    mFuncs->mQtWindow->mCube = new SlicerCube(mFuncs, glFormat, 
                                              mFuncs->mQtWindow->mCubeFrame);
    cubeLayout->addWidget(mFuncs->mQtWindow->mCube);
  }
  mFirstDraw--;
}
...
{
  mFuncs->generalEvent(e);  
}

void SlicerGL::enterEvent ( QEvent * e)
{
  mMouseInWindow = 1;
  mFuncs->generalEvent(e);
}
void SlicerGL::leaveEvent ( QEvent * e)
{
  mMouseInWindow = 0;
  mFuncs->generalEvent(e);
}
