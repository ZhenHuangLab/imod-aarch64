...
static float   sCurDevicePixelRatio = 1.;
...
int b3dInitializeGL()
{
  ...
  // If it fails, try for so.1, which is all there is on Ubuntu
  QLibrary *gllib = new QLibrary("libGL.so");
  if (!gllib->isLoaded()) {
    delete gllib;
    gllib = new QLibrary("libGL.so.1");
  }
#endif
...
// Set a device-pixel ratio for calls that will assume one
void b3dSetCurDevPixRatio(float DPR)
{
  sCurDevicePixelRatio = DPR;
}
...
// Set the line width with an integer value, by default scaling for device-to-pixel
// ratio and passing the resulting float.  OpenGL will round if no anti-aliasing
void b3dLineWidth(int width, bool scaleForDev)
{
  GLfloat fwidth = width;
  if (scaleForDev)
    fwidth = sCurDevicePixelRatio * fwidth + 0.001;
  glLineWidth(fwidth);
}

// Alternate call uses the flag in the object to set whether to scale
void b3dLineWidth(int width, Iobj *obj)
{
  b3dLineWidth(width, (obj->flags & IMOD_OBJFLAG_SCALE_WDTH) != 0);
}

// The point size call can always use a call with an object to set scaling
void b3dPointSize(int width, Iobj *obj)
{
  GLfloat fwidth = width;
  if (obj->flags & IMOD_OBJFLAG_SCALE_WDTH)
    fwidth = sCurDevicePixelRatio * fwidth + 0.001;
  glPointSize(fwidth);
}
...
/* All of these except drawCircle and draw[Filled]Rectangle scale sizes from pixel to 
   device coordinates; for drawCircle it is optional, default not to */
...
{
  PIXEL_TO_DEVICE(sCur, size, size);
  glBegin(GL_LINES);
...
void b3dDrawCircle(int x, int y, int radius, bool scaleForDev)
{
  int linewidth = 1;
  GLdouble inrad;
  if (scaleForDev)
    PIXEL_TO_DEVICE(sCur, radius, radius);
                  ...
                  int thickness, bool antiAlias)
{
  ...
  XY_PIXEL_TO_DEVICE(sCur, xtip, ytip, xtip, ytip);
  glGetFloatv(GL_LINE_WIDTH, &curWidth);
  if (!smoothEnabled && antiAlias)
    glEnable(GL_LINE_SMOOTH);
  b3dLineWidth(thickness, true);
  glBegin(GL_LINE_STRIP);
...
/* This is the display routine called from the zap, xyz, and slicer windows, for regular 
                              ...
                              int quality, int slice, int rgba)  
{
  ...
  if (!quality && (xzoom == 1.50) && (yzoom == 1.50)){
    b3dDrawGreyScalePixels15(dataPtrs, imXsize, imYsize, imXoffset, imYoffset,
                             winXstart, winYstart, imWidth, imHeight, image, base, slice,
                             rgba);
    return;
  } 
  ...
  if (!quality){
    ...
    if (!App->wzoom){
      ...
      if (xzoom >= 1.0 && (int)xzoom == xzoom && xzoom == yzoom){
  ...
  if (!b3dImageMatch(image, &sdata, imXoffset, imYoffset, imWidth, imHeight, zoom, yzoom,
    ...
    if (quality > 0 && rgba > 0 && rgba < 4 && 
      ...
      if (!j) {
        //imodPrintStderr("Filter selector index %d  k %d  ky %d\n", i, k, ky);
    ...
    if (!antialiased && quality > 0 && rgba < 4) {
