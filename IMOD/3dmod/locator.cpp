...
int locatorOpen(ImodView *vi)
{
  ...
  if (App->devPixVaries)
    LocWin->show();
  imod_info_input();
  utilInitializeScreenChange(LocWin, LocWin->mDevicePixelRatio);

  XY_PIXEL_TO_DEVICE(LocWin->m, minSize, maxSize, minSize, maxSize);
  ...
  XY_DEVICE_TO_PIXEL(LocWin->m, winx, winy, winx, winy);
  ...
  : QMainWindow(parent, f)
{
  ...
  ArrowButton *mUpArrow = new ArrowButton(Qt::UpArrow, central, "zoomup button");
  topHBox->addWidget(mUpArrow);
  connect(mUpArrow, SIGNAL(clicked()), this, SLOT(zoomUp()));
  mUpArrow->setToolTip("Increase window size by 1.5 (hot key =)");
  mDownArrow = new ArrowButton(Qt::DownArrow, central, "zoom down button");
  topHBox->addWidget(mDownArrow);
  connect(mDownArrow, SIGNAL(clicked()), this, SLOT(zoomDown()));
  mDownArrow->setToolTip("Decrease window size by 1.5 (hot key -)");
  ...
  GLw = new LocatorGL(glFormat, this, central);
...
void LocatorWindow::screenChanged(QScreen *screen)
{
  float newDPR = utilGetNewDevPixRatio(this);
  if (newDPR) {
    HANDLE_DPR_CHANGE_FOR_MAC(GLw->fakeResize, GLw->mWinx, GLw->mWiny);
  }
}
#endif
 ...
 */
LocatorGL::LocatorGL(QGLFormat format, LocatorWindow *topWin, QWidget * parent)
...
void LocatorGL::changeSize(float factor)
{
  ...
  if (LocWin->mDevicePixelRatio > 1.)
    XY_DEVICE_TO_PIXEL(LocWin->m, newx, newy, newx, newy);
...
void LocatorGL::paintGL()
{
  ...
  b3dSetCurSize(mWinx, mWiny);
  b3dSetCurDevPixRatio(mTopWin->mDevicePixelRatio);
