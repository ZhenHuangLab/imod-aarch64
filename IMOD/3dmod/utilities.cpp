...
PopupEntry sDefaultActions[] = {
  ...
  {"Print single-image filename for current Z", Qt::Key_F, 1, 0, 0},
...
int utilNextSecWithCont(ImodView *vi, Iobj *obj, int curz, int dir)
{
  ...
  for (co = 0; co < obj->contsize; co++) {
    ...
    if (!cont->psize)
      continue;
    if (iobjFlagTime(obj) && vi->numTimes > 1 && cont->time != vi->curTime)
      continue;
    ptLimit = iobjScat(obj->flags) ? cont->psize : 1;
    for (pt = 0; pt < ptLimit; pt++) {
      contz = B3DNINT(cont->pts[pt].z);
...
/*
 * Pass an event and closing flag and it will ignore the event if the flag isn't set 
 * On Mac for Qt >= 5.12
 */
bool utilIgnoreClosing(QCloseEvent *e, bool closing)
{
#if defined(Q_OS_MACX) && QT_VERSION >= 0x050C00 && QT_VERSION < 0x050F00
  if (!closing) {
    wprint("\aIn this IMOD version, you must use the Esc key to close this window\n");
    e->ignore();
    return true;
  }
#else
...
/*
 * Manages the paired meshes used to display surface with thickness for the given object
 */
int utilManagePairedMeshes(Iobj *obj, int obNum)
{
  if (obj->meshThickness) {
    obj->mesh = imodMeshMakePairs(obj->mesh, &obj->meshsize, obj->meshThickness, 
                                  vbCleanupVBD);
    if (!obj->mesh) {
      obj->meshsize = 0;
      wprint("\aObj %d mesh lost in memory reallocation - you need to remesh it\n",
             obNum + 1);
      return 1;
    }
  } else {
    imodMeshRemovePairs(obj->mesh, &obj->meshsize, 0, vbCleanupVBD);
  }
  return 0;
}
...
char *imodwGivenName(const char *intro, const char *filein)
{
  ...
  if (!winame)
    return(NULL);
  sprintf(winame, "%s%s%s", intro, *intro ? " " : "", filename);
  return(winame);
}
...
char *imodwfname(const char *intro)
{
  ...
  if (!filename && App->cvi->numTimes > 1) {
    ...
    if (!filename)
      return NULL;
    sprintf(filename, "%s%s%d image files", intro, *intro ? " " : "", App->cvi->numTimes);
    return(filename);
  }
...
/*
 * Sets or clears the flag to stay on top for the given window
 */
void utilSetStayOnTop(QWidget *widget, bool state)
{
  Qt::WindowFlags flags = widget->windowFlags();
  if ((state && (flags & Qt::WindowStaysOnTopHint)) || 
      (!state && !(flags & Qt::WindowStaysOnTopHint)))
    return;
  if (state)
...
/*
 * Handles size adjustments for a list box - widget stack pair
 */
void utilManageListStackSizes(QListWidget *listBox, QStackedWidget *stack)
{
  int maxwidth = 0, width, i, height, contHeight = 0, contWidth = 0;
  imod_info_input();
  for (i = 0; i < listBox->count(); i++) {
    width = listBox->fontMetrics().width(listBox->item(i)->text());
    maxwidth = B3DMAX(maxwidth, width);
    QSize size = stack->widget(i)->sizeHint();
    contWidth = B3DMAX(contWidth, size.width());
    contHeight = B3DMAX(contHeight, size.height());
  }
  listBox->setFixedWidth(maxwidth + 12);
  height = (listBox->fontMetrics().height() + 1.5) *  listBox->count();
  listBox->setFixedHeight(height);
  stack->setMinimumWidth(contWidth);
  stack->setMinimumHeight(contHeight);
}

/*
 * Returns a starting zoom that is adjusted for high DPI scaling
 */
float utilUnitZoomForDeviceScaling(float devPixRatio)
{
  if (!devPixRatio)
    devPixRatio = App->DevicePixelRatio;
  if (devPixRatio >= 1.8)
    return 2.;
  if (devPixRatio >= 1.3)
    return 1.5;
...
/*
 * Does initial operations when watching for DPI changes: gets a device/pixel ratio and
 * connects the screenCVhanges signal to a slot of the same name
 */
void utilInitializeScreenChange(QWidget *widget, float &devPixRatio)
{
#ifdef WATCH_DPI_CHANGE
  QWidget *topWind = widget->window();
  if (topWind) {
    topWind->winId();
    QWindow *winHandle = topWind->windowHandle();
    if (winHandle) {
      devPixRatio = winHandle->devicePixelRatio();
      widget->connect((QWidget *)winHandle, SIGNAL(screenChanged(QScreen *)), widget,
...
/*
 * Analyzes the situation after either a resize or a screen change to determine whether
 * to set the zoom, starting timers when appropriate for each event
 */
bool utilSetZoomOnScreenChange(float newSizeChange, float &lastSizeChange,
                               bool &screenChanged, QTime *screenResizeTime, float &zoom)
{
  float zlog, power, normed;
  bool isScreen =  newSizeChange == 0.;
  bool tinyChange = newSizeChange > 0. && newSizeChange > 0.99 && newSizeChange < 1.01;
  /*imodPrintStderr("newX %.2f screen %d   resizeTime %d  screenTime %d  lastX %f\n",
                  newSizeChange, screenChanged?1:0, screenResizeTime[1].elapsed(),
                  screenResizeTime[0].elapsed(), lastSizeChange); */
  if (!App->devPixVaries)
    return false;

  // Stick with the last size change if the current change is very small and recent
  if (!isScreen && !(tinyChange && screenResizeTime[1].elapsed() < 2000))
    lastSizeChange = newSizeChange;

  // Consider this change if it is a resize after a recent screen change or a
  // first screen change within the time limit since a resize
  if (((screenChanged && !isScreen && screenResizeTime[0].elapsed() < 2000) ||
       (!screenChanged && isScreen && screenResizeTime[1].elapsed() < 2000))) {

    // For screen change, unconditionally start its timer, set the flag
    if (isScreen) {
      screenResizeTime[0].start();
      screenChanged = true;
    }

    // If the last size change was significant, adjust the zoom by the size change,
    // rounding to 2 digits
    if (lastSizeChange < 0.9 || lastSizeChange > 1.1) {
      zoom *= lastSizeChange;
      zlog = log10(zoom);
      power = floor(zlog);
      normed = B3DNINT(10. * pow(10., (double)zlog - power));
      zoom = pow(10., power - 1.) * normed;
      //imodPrintStderr("Set zoom %.2f\n", zoom);

      // Clear the screen change and reset the size change so it isn't re-used by mistake
      screenChanged = false;
      lastSizeChange = 1.;
      return true;
    }
    ...
    // For a non-qualifying change, start the respective timer and record if it is screen
    screenResizeTime[isScreen ? 0 : 1].start();
    screenChanged = isScreen;
  }
  return false;
}

/*
 * Gets a new device/pixel ratio when the screen changes
 */
float utilGetNewDevPixRatio(QWidget *widget)
{
#ifdef WATCH_DPI_CHANGE
  QWindow *winHandle = widget->window()->windowHandle();
  if (winHandle) {
    float newDPR = winHandle->devicePixelRatio();
    if (newDPR > 0)
      return newDPR;
  }
#endif
