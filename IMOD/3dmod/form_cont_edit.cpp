 ...
 */
ContSurfPoint::ContSurfPoint(DockingDialog* parent, Qt::WindowFlags fl)
  : QWidget(parent, fl)
{
  mTopWin = parent;
  setupUi(this);
...
void ContSurfPoint::pointLabelFinished()
{
  iceLabelFinished(1);
}
...
void ContSurfPoint::downGhostToggled( bool state )
{
  ...
  twoShadesBox->setEnabled(upGhostBox->isChecked() && downGhostBox->isChecked());
}
...
void ContSurfPoint::twoShadesToggled( bool state )
{
  iceGhostToggled(state ? 1 : 0,  IMOD_GHOST_2SHADES);
}
...
void ContSurfPoint::allScatteredToggled( bool state )
{
  iceGhostToggled(state ? 1 : 0,  IMOD_GHOST_ALLSCAT);
}
...
void ContSurfPoint::setGhostState( int interval, int ghostmode)
{
  ...
  diaSetChecked(twoShadesBox, ghostmode & IMOD_GHOST_2SHADES);
  diaSetChecked(allScatteredBox, ghostmode & IMOD_GHOST_ALLSCAT);
  twoShadesBox->setEnabled((ghostmode & IMOD_GHOST_PREVSEC) && 
                           (ghostmode & IMOD_GHOST_NEXTSEC));
}
...
{
  QString str;
  surfaceSpinBox->setEnabled(maxVal > 0); 
  if (value < 0) {
...
// Inform of closing
void ContSurfPoint::topCloseEvent( QCloseEvent * e )
{
  iceClosing();
  e->accept();
}
...
void ContSurfPoint::topChangeEvent(QEvent *e)
{
