...
void imodvPaintGL()
{
  ...
  scale = 0.5 * (a->winx > a->winy ? a->winy : a->winx) /
    a->imod->view->rad;
  a->scaleBarSize = scaleBarDraw(a->winx, a->winy, scale, color, a->mainWin->mCurGLw,
                                 a->mainWin->mDevicePixelRatio);

  // Get back to render mode and get the number of hits before leaving the context
  // Needed on Windows for non-NVidia graphics at least
  if (a->doPick)
    a->pickHits = glRenderMode( GL_RENDER);
