...
static bool sFirstZapOpening = true;
...
static int sMinXforRestraining = 1050;
...
int getTopZapMouse(Ipoint *imagePt)
{
  ...
  if (!zap)
    return 1;
  XY_PIXEL_TO_DEVICE(zap->m, (zap->mGfx->mapFromGlobal(QCursor::pos())).x(), 
                     (zap->mGfx->mapFromGlobal(QCursor::pos())).y(), mx, my);
...
{
  QString str;
  QRect oldGeom, infoRect;
  int needWinx, needWiny;
  int maxWinx, maxWinImx;
  int maxWiny, maxWinImy;
  int i, newWidth, newHeight, xleft, ytop, toolHeight, usableTop = 0, usableLeft = 0;
  int infoLeft, infoTop, infoWidth, usableRight, newOutWidth, newOutHeight;
  bool restrainSize;
  double newZoom;
  int extraTopBot = 20;
  ...
  if (!wintype) {
    diaMaximumWindowSize(maxWinx, maxWiny, mQtWindow);
  ...
  mZoom   = utilUnitZoomForDeviceScaling(mDevicePixelRatio);
  ...
  if (!oldGeom.width()) {
    ...
    if (!wintype) {
      ...
      //mQtWindow->show();
      //imod_info_input();
      diaMinimumWindowPos(usableLeft, usableTop, mQtWindow);
      diaLimitWindowPos(100, 100, usableLeft, usableTop, mQtWindow);
      usableRight += usableLeft;
      infoRect = ImodInfoWin->frameGeometry();
      if (maxWinx > sMinXforRestraining && !App->devPixVaries) {
        restrainSize = true;
        maxWinx -= infoRect.width();
      }
      
      XY_PIXEL_TO_DEVICE(m, maxWinx, maxWiny - toolHeight, maxWinImx, maxWinImy);
      for (i = 0; i < 2; i++) {
        mZoom = imodInitialZoom();
        if (!mZoom) {
          mZoom = utilUnitZoomForDeviceScaling(mDevicePixelRatio);
          while ((mZoom * vi->xsize > 1.1 * maxWinImx ||
                  mZoom * vi->ysize > 1.1 * maxWinImy)) {
            newZoom = b3dStepPixelZoom(mZoom, -1);
            if (fabs(newZoom - mZoom) < 0.0001)
              break;
            if (mVi->pyrCache && mVi->pyrCache->zoomRequiresBigLoad(newZoom, maxWinImx,
                                                                    maxWinImy))
              break;
            mZoom = (float)newZoom;
          }
        ...
        needWinx = B3DQT_PIXELS(m, mZoom * vi->xsize);
        if (restrainSize)
          needWinx = B3DMIN(needWinx, maxWinx);
      ...
      needWiny = B3DQT_PIXELS(m, mZoom * vi->ysize) + toolHeight;
      diaLimitWindowSize(needWinx, needWiny, mQtWindow);
    ...
    // Would have to show first to get a good frame geometry but we can add the borders
    // from the info window to get the outside size
    QRect pos = mQtWindow->frameGeometry();
    xleft = pos.x();
    ytop = pos.y();
    QRect inside = ImodInfoWin->geometry();
    newOutWidth = newWidth + infoRect.width() - inside.width();
    newOutHeight = newHeight + infoRect.height() - inside.height();

    diaLimitWindowPos(newWidth, newHeight, xleft, ytop, mQtWindow);
    if (imodDebug('z'))
    ...
    if (!wintype) {
      ...
      // For first zap, move Info to nearest corner with a bit more allowance top/bottom
      if (restrainSize && sFirstZapOpening) {
        if (infoTop - usableTop > (usableTop + maxWiny) - infoRect.bottom())
          infoTop = (usableTop + maxWiny) - (infoRect.height() + extraTopBot);
        else
          infoTop = usableTop + extraTopBot;
        if (infoLeft - usableLeft > usableRight - (infoLeft + infoWidth))
          infoLeft = usableRight - infoWidth;
        else
          infoLeft = usableLeft;
        ImodInfoWin->move(infoLeft, infoTop);
      }

      // Regardless, now move zap to one side or another if it can uncover some info win
      if (!App->devPixVaries && ((infoLeft - usableLeft) + 20 < usableRight - newOutWidth
                                 || (infoLeft - usableLeft) + infoWidth - 20 > 
                                 newOutWidth)) {
        if ((infoLeft - usableLeft) + 20 < usableRight - newOutWidth)
          xleft = B3DMIN(usableRight - newOutWidth, infoLeft + infoWidth);
        else 
          xleft = B3DMAX(usableLeft, infoLeft - newOutWidth);

        // And move it vertically to be adjacent - its original position may be
        // meaningless because it is before window manager
        if (infoTop - usableTop > (usableTop + maxWiny) - infoRect.bottom())
          ytop = B3DMAX(usableTop, infoTop + infoRect.height() - newOutHeight);
        else
          ytop = B3DMIN(usableTop + maxWiny - newOutHeight, infoTop);
    ...
    diaLimitWindowSize(newWidth, newHeight, mQtWindow);
    ...
    if (!wintype) {
      ...
      if (!mZoom) {
        ...
        XY_PIXEL_TO_DEVICE(m, newWidth, newHeight - toolHeight, needWinx, needWiny);
  ...
  mGfx->mInitWidth = newWidth;
  mGfx->mInitHeight = newHeight;
  mGfx->mInitLeft = xleft;
  mGfx->mInitTop = ytop;
...
void ZapFuncs::resize(int winx, int winy)
{
  ...
  if (imodDebug('z')) {
    imodPrintStderr("Size = %d x %d  win %d x %d tool %d :", winx, winy,
                    mQtWindow->width(),mQtWindow->height(),mQtWindow->mToolBar->height());
    if (mGinit)
      imodPrintStderr("Old Size = %d x %d :", mWinx, mWiny);
  }
  if (mWinx > 1 && mDevicePixelRatio > 0) {
    /*imodPrintStderr("Resize Zap pos %d %d\n", mQtWindow->pos().x(), 
      mQtWindow->pos().y()); */
    utilSetZoomOnScreenChange((float)winx / mWinx, mLastXsizeChange, mScreenChanged,
                              mScreenResizeTime, mZoom);
  }
  mWinx = winx;
  mWiny = winy;
...
void ZapFuncs::draw()
{
  ...
  if (sPixelViewOpen && !mNumXpanels) {
    XY_PIXEL_TO_DEVICE(m, (mGfx->mapFromGlobal(QCursor::pos())).x(), 
                       (mGfx->mapFromGlobal(QCursor::pos())).y(), mx, my);
    if (mx >= 0 && mx < mWinx && my >= 0 && my < mWiny) {
      getixy(mx, my, imx, imy, imz);
...
void ZapFuncs::paint()
{
  ...
  b3dSetCurDevPixRatio(mDevicePixelRatio);
  ...
  if (mDrawCurrentOnly > 0) {
    ...
    if (mDrawingLasso)
      ...
      b3dLineWidth(mVi->imod->obj[ob].linewidth2, &mVi->imod->obj[ob]); 
  ...
  drawTools();
  mScaleBarSize = scaleBarDraw(mWinx, mWiny, mZoom, 0, mGfx, mDevicePixelRatio);
...
void ZapFuncs::screenChanged(float newDPR)
{
  if (newDPR) {
    /*imodPrintStderr("screen change Zap pos %d %d\n", 
                    mQtWindow->pos().x(), mQtWindow->pos().y())*/
    utilSetZoomOnScreenChange(0., mLastXsizeChange, mScreenChanged, mScreenResizeTime,
                                mZoom);
    HANDLE_DPR_CHANGE_FOR_MAC(resize, mWinx, mWiny);
  }
}
...
void ZapFuncs::keyInput(QKeyEvent *event)
{
  ...
  if (utilCloseKey(event) && mGfx->mFirstDraw <= 0) {
  ...
  if (imodPlugHandleKey(vi, event, mNumXpanels ? MULTIZ_WINDOW_TYPE : ZAP_WINDOW_TYPE)) 
    return;
  ...
  case Qt::Key_Insert:
    ...
    XY_PIXEL_TO_DEVICE(m, (mGfx->mapFromGlobal(QCursor::pos())).x(), 
                       (mGfx->mapFromGlobal(QCursor::pos())).y(), ix, iy);
  ...
  case Qt::Key_A:
    ...
    if (ctrl) {
      ...
      if (vi->ax)
        autox_next(vi->ax);
      else if (iprocIsOpen() && !iprocBusy() && !vi->loadingImage)
        iprocApply();
      handled = 1;
    } 
    break;
...
void ZapFuncs::generalEvent(QEvent *e)
{
  ...
  if (utilNeedToSetCursor() && e->type() == QEvent::Enter)
    setCursor(mVi->imod->mousemode, true);
  ifdraw = imodPlugHandleEvent(mVi, e, imx, imy, ZAP_WINDOW_TYPE);
...
void ZapFuncs::mousePress(QMouseEvent *event)
{
  ...
  XY_PIXEL_TO_DEVICE(m, event->x(), event->y(), x, y);
  sBut1downt.start();
  sFirstmx = x;
  sFirstmy = y;
  mLmx = x;
...
{
  Iobj *obj;
  int imz, ind, button1, button2, button3, ifdraw = 0, drew = 0, ex, ey;
  float area, length;
  ...
  XY_PIXEL_TO_DEVICE(m, event->x(), event->y(), ex, ey);

  if (imodDebug('m'))
    imodPrintStderr("release at %d %d   buttons %d %d %d\n", ex, 
                    ey, button1, button2, button3);
  if (mShiftRegistered) {
    mShiftRegistered = 0;
    mVi->undo->finishUnit();
  }
                          ...
                          int but2, int but3)
{
  ...
  XY_PIXEL_TO_DEVICE(m, event->x(), event->y(), ex, ey);
  getixy(ex, ey, imx, imy, imz);
  ifdraw = imodPlugHandleMouse(mVi, event, imx, imy, but1, but2, but3, ZAP_WINDOW_TYPE);
  if (ifdraw & 1) {
    if (ifdraw & 2)
      draw();
  } else if (setControl)
  ...
  if (vi->imod->mousemode == IMOD_MMODEL || mDrawingLasso) {
    ...
    if (mDrawingLasso) {
      ...
      if (iobjPlanar(obj->flags) && (!cont->psize || (cont->flags & ICONT_WILD)) &&
          cont->surf && (imodSurfaceIsPlanar(obj, cont->surf, time, X_SLICE_BOX) > 0 ||
                         imodSurfaceIsPlanar(obj, cont->surf, time, Y_SLICE_BOX) > 0))
...
int ZapFuncs::b1Drag(int x, int y)
{
  ...
  if (mStartingBand) {
    ...
    if (dx || dy) {
      ...
      XY_DEVICE_TO_PIXEL(m, x, y, cursx, cursy);
      QCursor::setPos(mGfx->mapToGlobal(QPoint(cursx, cursy)));
      mLmx = x;
      mLmy = y;
    }
...
int ZapFuncs::b2Drag(int x, int y, int controlDown)
{
  ...
  if ( dist > scaleModelRes(vi->imod->res, mZoom / mDevicePixelRatio)){
...
int ZapFuncs::b3Drag(int x, int y, int controlDown, int shiftDown)
{
  ...
  pt.z = lpt->z;
  if (imodel_point_dist(lpt, &pt) > scaleModelRes(vi->imod->res, 
                                                  mZoom / mDevicePixelRatio)){
  ...
  *limits = NULL;
  if (mRubberband) {
    *limits = limarr;
    ...
    limarr[2] = mRbMouseX1 - (mDevicePixelRatio > 1. ? 2 : 1) - mRbMouseX0;
    limarr[3] = mRbMouseY1 - (mDevicePixelRatio > 1. ? 2 : 1) - mRbMouseY0;
  }
}
...
{
  int width, height, neww, newh, usableWidth, newRight, occludesLeft, occludesRight;
  int dx, dy, newdx, newdy, usableTop = 0, usableLeft = 0, frameWidth;
  int newWidth, usableRight;
  float xl, xr, yb, yt;
  XY_PIXEL_TO_DEVICE(m, mQtWindow->width(), mQtWindow->height(), width, height);
  QRect pos = ivwRestorableGeometry(mQtWindow);
  QRect infoRect = ImodInfoWin->frameGeometry();
  diaMinimumWindowPos(usableLeft, usableTop, mQtWindow);
  diaMaximumWindowSize(usableWidth, dx, mQtWindow);
  ...
  XY_DEVICE_TO_PIXEL(m, neww, newh, neww, newh);
  XY_DEVICE_TO_PIXEL(m, width, height, width, height);

  diaLimitWindowSize(neww, newh, mQtWindow);
  newdx = dx + width / 2 - neww / 2;
  newdy = dy + height / 2 - newh / 2;
  diaLimitWindowPos(neww, newh, newdx, newdy, mQtWindow);

  // Restrain size if info is near edge and window is big enough
  frameWidth = infoRect.width() - ImodInfoWin->width();
  if (usableWidth > sMinXforRestraining && 
      (infoRect.left() < usableLeft + 50 || 
       infoRect.right() > usableLeft + usableWidth - 50)) {
    if (infoRect.left() < usableLeft + 50) {
      neww = B3DMIN(neww, (usableLeft + usableWidth - infoRect.right()) - frameWidth);
    } else {
      neww = B3DMIN(neww, (infoRect.left() - usableLeft) - frameWidth);
    }
  }
  
  // In any case, if window can fit beside info and it would occlude it, shift it over
  newWidth = neww + frameWidth;
  usableRight = usableLeft + usableWidth;
  newRight = newdx + newWidth;
  if (newdx < infoRect.right() && newRight >= infoRect.left()) {

    // Get amount by which info would be occluded if on either side
    occludesLeft = newWidth - (infoRect.left() - usableLeft);
    occludesRight = newWidth - (usableRight - infoRect.right());

    // If both sides work, keep it on the same side
    if (occludesLeft < 0 && occludesRight < 0) {
      if ((newdx + newRight) / 2 < (infoRect.left() + infoRect.right()) / 2)
        newdx = infoRect.left() - newWidth;
      else
        newdx = infoRect.right();

      // Otherwise if one side works at all and is better, put it on that side
    } else if (occludesLeft < occludesRight && occludesLeft < infoRect.width()) {
      newdx = B3DMAX(usableLeft, infoRect.left() - newWidth);
    } else if (occludesRight < occludesLeft && occludesRight < infoRect.width()) {
      newdx = B3DMIN(infoRect.right(), usableRight - newWidth);
    }
  }
...
void ZapFuncs::drawGraphics()
{
  ...
  if (vi->pyrCache) {
    ...
    mLastStatus = status;
    // Old comment was "100 was way too often with debug version": now make sure the
    // redraws do not take more than 1/5 of the time away from loading
    if (status > 0)
      mGfx->scheduleRedraw(B3DMAX(200, 4 * mGfx->getLastDrawMsec()));
  } else {
    xStart = mXstart;
...
void ZapFuncs::drawModel()
{
  ...
  Icont *ztCont;
  int *contZ;
  Ipoint *pmin = NULL, *pmax = NULL;
  int *numAtZ;
  int **contAtZ;
  int *zlist;
  int zlSize, numMax, error, zmin, zmax, numConts, zco, obco, obzco;
  int numwarn = -1;
  bool freeZTables;
  ...
  for (ob = 0; ob < vi->imod->objsize + vi->numExtraObj; ob++){
    if (ob < vi->imod->objsize) {
      obj = &vi->imod->obj[ob];
    } else {
      obj = ivwGetAnExtraObject(vi, ob - vi->imod->objsize);
    ...
    if (ob < vi->imod->objsize)
      imodSetObjectColor(ob); 
    b3dLineWidth(sScaleSizes * obj->linewidth2, obj);
    ...
    mContsAtCurZ = NULL;
    if (obj->extra[IOBJ_EX_2D_TRANS]) {
      error = imodContourMakeZTables(&vi->imod->obj[ob], 1, 0, &contZ, &zlist, &numAtZ,
                                    &contAtZ, &zmin, &zmax, &zlSize, &numMax);
      if (!error)
        freeZTables = true;

      // Save the list of contours at this Z: it is the way to get back to object conts
      if (!error && mSection >= zmin && mSection <= zmax && numAtZ[mSection - zmin] > 0) {
        mContsAtCurZ = contAtZ[mSection - zmin];
        numConts = numAtZ[mSection - zmin];
      }

      if (mContsAtCurZ && iobjClose(obj->flags)) {
        // Make arrays that are needed for nest analysis here and map for drawing
        pmin = B3DMALLOC(Ipoint, numConts);
        pmax = B3DMALLOC(Ipoint, numConts);
        scanCont = B3DMALLOC(Icont *, numConts);
        mNestContMap = B3DMALLOC(int, obj->contsize);
        mNestInd = B3DMALLOC(int, numConts);
        if (!pmin || !pmax || !scanCont || !mNestContMap || !mNestInd) {
          error = 1;
        } else {

          // Make map from contour number to # at this Z that will be in the analysis
          // and get bounding boxes
          zco = 0;
          for (co = 0; co < obj->contsize; co++)
            mNestContMap[co] = -1;
          for (zco = 0; zco < numConts; zco++) {
            co = mContsAtCurZ[zco];
            ztCont = &obj->cont[co];
            imodContourGetBBox(ztCont, &pmin[zco], &pmax[zco]);
            mNestContMap[co] = zco;
            mNestInd[zco] = -1;
            scanCont[zco] = NULL;
          }
          // Look for conts that might overlap based on bounding boxes, make scan conts
          // for them, and check for nesting
          for (co = 0; co < numConts - 1 && !error; co++) {
            obco = mContsAtCurZ[co];
            for (zco = co + 1; zco < numConts && !error; zco++) {
              obzco = mContsAtCurZ[zco];
              if (pmax[co].x >= pmin[zco].x && pmax[zco].x >= pmin[co].x &&
                  pmax[co].y >= pmin[zco].y && pmax[zco].y >= pmin[co].y) {
                if (!scanCont[co])
                  scanCont[co] = imodel_contour_scan(&obj->cont[obco]);
                if (!scanCont[zco])
                  scanCont[zco] = imodel_contour_scan(&obj->cont[obzco]);
                if (!scanCont[co] || !scanCont[zco] || 
                    imodContourCheckNesting(co, zco, scanCont, pmin, pmax, &mNests,
                                            mNestInd, &mNumNests, &numwarn)) {
                  error = 1;
                } 
              }
            }
          ...
          // Analyze the nesting
          imodContourNestLevels(mNests, mNestInd, mNumNests);
        }
      }

      // Now set up the tesselator contour if needed
      if (!error && mContsAtCurZ) {
        setupFilledContTesselator();
        for (co = 0; co < numConts; co++)
          maxPts = B3DMAX(maxPts, obj->cont[mContsAtCurZ[co]].psize);
        mTessCont = imodContourNew();
        if (mTessCont) {
          mTessCont->pts = B3DMALLOC(Ipoint, maxPts);
          
          // Cleanup on error, or initialize Z to 0 (important)
          if (!mTessCont->pts) {
            imodContourDelete(mTessCont);
            mTessCont = NULL;
          } else {
            mTessMaxPoints = maxPts;
...
// Draw mesh lines located on the current image
void ZapFuncs::drawMesh(Iobj *obj, int ob)
{
  Imesh *mesh;
  ...
  bool startOfPoly;
  float lastRed, lastGreen, lastBlue, firstRed, firstGreen, firstBlue;
  float firstVisR, firstVisG, firstVisB, secondVisR, secondVisG, secondVisB;
  int firstVisInd, secondVisInd, firstLI, secondLI;

  imodMeshNearestRes(obj->mesh, obj->meshsize, ImodvClosed ? 0 : Imodv->lowres,
                     &resol);
  if (utilManagePairedMeshes(obj, ob))
    return;
  ...
  for (me = 0; me < obj->meshsize; me++) {
    ...
    //imodPrintStderr("first %d %.3f %.3f %.3f\n", nextChange, curProps.red,
    //              curProps.green, curProps.blue);
    lastRed = firstRed = curProps.red;
    lastGreen = firstGreen = curProps.green;
    lastBlue = firstBlue = curProps.blue;
    ...
    for (i  = 0; i < mesh->lsize; i++) {
      ...
      case IMOD_MESH_BGNPOLYNORM2:
        ...
        // If no changes in whole mesh, process it simply
        // Go through each triangle and see if two points are visbale, if so draw line
        if (nextItemIndex < 0) {
          while (mlist[i] != IMOD_MESH_ENDPOLY) {
            firstVisInd = -1;
            secondVisInd = -1;
            for (j = 0; j < 3; j++) {
              if (pointVisable(&vert[mlist[i]])) {
                if (firstVisInd < 0)
                  firstVisInd = mlist[i];
                else
                  secondVisInd = mlist[i];
              }
              i++;
            }
            if (secondVisInd >= 0) {
              glBegin(GL_LINES);
              glVertex2i(xpos(vert[firstVisInd].x), ypos(vert[firstVisInd].y));
              glVertex2i(xpos(vert[secondVisInd].x), ypos(vert[secondVisInd].y));
              glEnd();
            }
          }            
        } else {
          ...
          // Set the color to starting color at beginning of each triangle
          startOfPoly = true;
          while (mlist[i] != IMOD_MESH_ENDPOLY) {
            curProps.red = firstRed;
            curProps.green = firstGreen;
            curProps.blue = firstBlue;
            firstVisInd = -1;
            secondVisInd = -1;
            for (j = 0; j < 3; j++) {

              // Get a change at appointed time or at start of polygon
              if (nextChange == i || startOfPoly) {
                nextChange = ifgHandleMeshChange(obj, mesh->store, &defProps, &curProps,
                                                 &nextItemIndex, i, &stateFlags, 
                                                 &changeFlags, 0);
                //imodPrintStderr("chg %d %d %.3f %.3f %.3f\n", i, nextChange, curProps.red,
                //              curProps.green, curProps.blue);
                startOfPoly = false;
              }

              // When a visible point is found, record its color too
              if (pointVisable(&vert[mlist[i]])) {
                if (firstVisInd < 0) {
                  firstLI = i;
                  firstVisInd = mlist[i];
                  firstVisR = curProps.red;
                  firstVisG = curProps.green;
                  firstVisB = curProps.blue;
                } else {
                  secondLI = i;
                  secondVisInd = mlist[i];
                  secondVisR = curProps.red;
                  secondVisG = curProps.green;
                  secondVisB = curProps.blue;
                }
              }
              i++;
            }

            // When two points found, set color as needed before each one
            if (secondVisInd >= 0) {
              if (lastRed != firstVisR || lastGreen != firstVisG ||
                  lastBlue != firstVisB) {
                //imodPrintStderr("set to for %d %.3f %.3f %.3f\n", firstLI, firstVisR,
                //              firstVisG, firstVisB);

                glColor3f(firstVisR, firstVisG, firstVisB);
                lastRed = firstVisR;
                lastGreen = firstVisG;
                lastBlue = firstVisB;
              }
              glBegin(GL_LINES);
              glVertex2i(xpos(vert[firstVisInd].x), ypos(vert[firstVisInd].y));
              //printf("%d %.1f %.1f  %d %.1f %.1f\n", firstLI, vert[firstVisInd].x,
              //vert[firstVisInd].y, secondLI,vert[secondVisInd].x, vert[secondVisInd].y);
                
              if (lastRed != secondVisR || lastGreen != secondVisG ||
                  lastBlue != secondVisB) {
                //imodPrintStderr("set to for %d %.3f %.3f %.3f\n", secondLI,
                //              secondVisR, secondVisG, secondVisB);
                glColor3f(secondVisR, secondVisG, secondVisB);
                lastRed = secondVisR;
                lastGreen = secondVisG;
                lastBlue = secondVisB;
              }
              glVertex2i(xpos(vert[secondVisInd].x), ypos(vert[secondVisInd].y));
...
void ZapFuncs::drawContour(int co, int ob)
{
  ...
  float delz;
  Iobj  *obj;
  Icont *cont, *useCont, *inCont, *joinCont, *tmpCont;
  Istore *stp;
  DrawProps contProps, ptProps;
  const char *ptLabel = NULL;
  int pt, radius, lastX, lastY, thisX, thisY, st, labelYoffset, xlab, ylab, nest;
  int pt1, pt2, ind, i, j, inco, zco, level;
  float drawsize, zscale, dist, tdist;
  ...
  if (!iobjScat(obj->flags) && ((cont->flags & ICONT_WILD) || lastVisible)) {
    ...
    if (obj->extra[IOBJ_EX_2D_TRANS] && mTessCont && !(cont->flags & ICONT_WILD)) {
      useCont = cont;
      if (iobjClose(obj->flags) && mNestContMap[co] >= 0 && 
          mNestInd[mNestContMap[co]] >= 0) {
        nest = mNestInd[mNestContMap[co]];

        // If there is a nest, skip everything on even levels and suppress outline drawing
        // for them as it becomes part of the fill
        level = mNests[nest].level;
        if (level % 2 == 0) {
          useCont = NULL;
          skipOutline = !selected && !(obj->flags & IMOD_OBJFLAG_POLY_CONT) && 
            !currentCont;
        } else {

          // Loop on the inside ones and join ones at next level if they are truly inside
          // the current useCont
          for (ind = 0; ind < mNests[nest].ninside; ind++) {
            zco = mNests[nest].inside[ind];
            if (mNests[mNestInd[zco]].level == level + 1) {
              inco = mContsAtCurZ[mNests[nest].inside[ind]];
              inCont = &obj->cont[inco];
              if (imodContourInsideCont(inCont, useCont)) {

                // Find nearest points
                dist = 1.e30;
                for (i = 0; i < useCont->psize; i++) {
                  for (j = 0; j < inCont->psize; j++) {
                    tdist = imodPointDistance(&useCont->pts[i], &inCont->pts[j]);
                    if (dist > tdist) {
                      dist = tdist;
                      pt1 = i;
                      pt2 = j;
                ...
                // Join the contours, duplicating first because they can get inverted in
                // the join which makes an unintended change to the model
                tmpCont = imodContourDup(inCont);
                if (!tmpCont)
                  tmpCont = inCont;
                if (useCont == cont)
                  useCont = imodContourDup(cont);
                joinCont = imodContourJoin(useCont, tmpCont, pt1, pt2, 2, 1);
                if (tmpCont != inCont)
                  imodContourDelete(tmpCont);
                if (joinCont) {
                  imodContourDelete(useCont);
                  useCont = joinCont;
                }
              }
            }
          ...
          if (useCont && useCont->psize > mTessMaxPoints) {
            free(mTessCont->pts);
            mTessCont->pts = B3DMALLOC(Ipoint, useCont->psize);
            if (mTessCont->pts) {
              mTessMaxPoints  = useCont->psize;
              for (pt = 0; pt < mTessMaxPoints; pt++)
                mTessCont->pts[pt].z = 0.;
            } else {
              mTessCont->psize = 0;
              imodContourDelete(mTessCont);
      ...
      // If there is a contour after all that, put it in the tess cont and draw
      if (useCont) {
        glEnable(GL_BLEND);
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
        glColor4f(contProps.red, contProps.green, contProps.blue, 
                  1. - obj->extra[IOBJ_EX_2D_TRANS] / 100.);
        for (pt = 0; pt < useCont->psize; pt++) {
          mTessCont->pts[pt].x = xpos(useCont->pts[pt].x);
          mTessCont->pts[pt].y = ypos(useCont->pts[pt].y);
        }
        mTessCont->psize = useCont->psize;
  ...
  // Draw labels if any
  if (cont->label && (obj->flags & IMOD_OBJFLAG_DRAW_LABEL)) {
    PIXEL_TO_DEVICE(m, mQtWindow->mZoomEdit->fontMetrics().height() / 3, labelYoffset);
    if (!mLabelFont) {
      mLabelFont = new QFont(QApplication::font());
      if (obj->extra[IOBJ_EX_LABEL_SIZE])
        mLabelFont->setPointSizeF(sScaleSizes * obj->extra[IOBJ_EX_LABEL_SIZE]);
      else if (sScaleSizes > 1.)
        mLabelFont->setPointSizeF(sScaleSizes * mLabelFont->pointSize());
    }
    ...
    for (pt = 0; pt < cont->psize; pt++){
      ...
      if (!(ptProps.gap && ptProps.valskip) && pointVisable(&(cont->pts[pt]))) {
        ptLabel = imodLabelItemGet(cont->label, pt);
        if (ptLabel) {
          drawsize = B3DNINT(mZoom * imodPointGetSize(obj, cont, pt) / vi->xybin);
          PIXEL_TO_DEVICE(m, B3DMAX(ptProps.symsize, drawsize) + 3, drawsize);
          XY_DEVICE_TO_PIXEL(m, xpos(cont->pts[pt].x) + drawsize, 
                             mWiny - (ypos(cont->pts[pt].y)- labelYoffset), xlab, ylab);
          mGfx->renderText(xlab, ylab, ptLabel, *mLabelFont);
        }
      }
    }
  }
...
void ZapFuncs::drawCurrentPoint()
{
  ...
  if ((vi->imod->mousemode == IMOD_MMOVIE)||(!pnt)){
    ...
    if ((cont) && (cont->psize) && (pnt)){
      ...
      b3dDrawCircle(x, y, sScaleSizes * curSize, true);
    }
  }
...
void ZapFuncs::drawGhost()
{
  ...
  int pt, npt, lastX, lastY, thisX, thisY, deltaZ, lastShade;
  float delz, zscale, drawsize;
  bool scattered, usePointColor;
  int nextChange, stateFlags, changeFlags;
  int handleFlags;
  ...
  for (ob = 0; ob < mod->objsize; ob++) {
    obj = &(mod->obj[ob]);
    scattered = iobjScat(obj->flags);
    if (ob != mod->cindex.object && 
        !((!scattered && (mVi->ghostmode & IMOD_GHOST_ALLOBJ)) || 
          (scattered && (mVi->ghostmode & IMOD_GHOST_ALLSCAT))))
      continue;
    ...
    deltaZ = mVi->ghostdist;
    if (scattered)
      deltaZ = B3DNINT(B3DMAX(1, obj->pdrawsize) * (2 + mVi->ghostdist) / zscale - 1);
    if (mVi->ghostdist) {
      nextz = mSection + deltaZ;
      prevz = mSection - deltaZ;
    } else {
    ...
    for (co = 0; co < obj->contsize; co++) {
      ...
      if (scattered) {
        ...
        for (pt = 0; pt < cont->psize; pt++){
          ...
          if (drawsize > 0 && !(ptProps.gap && ptProps.valskip)) {
            iz = B3DNINT(cont->pts[pt].z);
            delz = (cont->pts[pt].z - mSection) * zscale;
            if ((delz >= drawsize && drawPrev && iz <= nextz) ||
                (delz <= -drawsize && drawNext && iz >= prevz)) {
              shade = -1;
              if ((mVi->ghostmode & IMOD_GHOST_2SHADES) && drawPrev && drawNext && 
                  iz > mSection)
                shade = 1;
              shade *= (mVi->ghostmode & IMOD_GHOST_LIGHTER) ? -1 : 1;
              if (shade != lastShade) {
                setGhostColor(usePointColor ? ptProps.red : contProps.red,
                              usePointColor ? ptProps.green : contProps.green,
                              usePointColor ? ptProps.blue : contProps.blue, shade);
                lastShade = shade;
              }
              b3dDrawCircle(xpos(cont->pts[pt].x),
                            ypos(cont->pts[pt].y),
      ...
      /* By popular demand, display ghosts from lower and upper sections */
      } else if (cont->pts && !(cont->flags & ICONT_WILD)) {
        ...
        if ((iz > mSection && drawPrev && 
          ...
          if ((mVi->ghostmode & IMOD_GHOST_2SHADES) && drawPrev && drawNext &&
            ...
            shade = 1;
          shade *= (mVi->ghostmode & IMOD_GHOST_LIGHTER) ? -1 : 1;
          setGhostColor(contProps.red, contProps.green, contProps.blue, shade);
          ...
          if (nextChange < 0) {
            ...
            for (pt = 0; pt < cont->psize; pt++) {
              ptProps.gap = 0;
              if (pt == nextChange) {
                nextChange = ifgHandleNextChange(obj, cont->store, &contProps, &ptProps,
                                                 &stateFlags, &changeFlags, handleFlags,
                                                 0, sScaleSizes);
                if (changeFlags & CHANGED_COLOR)
                  setGhostColor(ptProps.red, ptProps.green, ptProps.blue, shade);
                if (ptProps.gap && stippleGaps) {
                  b3dStippleNextLine(true);
                  ptProps.gap = 0;
                }
...
void ZapFuncs::setGhostColor(float obr, float obg, float obb, int shade)
{
...
void ZapFuncs::setCursor(int mode, bool setAnyway)
{
  ...
  //imodPrintStderr("zap mode %d sa %d special %d sizeall %d sb %d mb %d\n", mode, setAnyway?1:0, needSpecial?1:0, needSizeAll?1:0, mStartingBand?1:0, sMoveBandLasso?1:0);
