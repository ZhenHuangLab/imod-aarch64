                             ...
                             bool keepSideBar,  const char *prefix, bool prefAbsolute)
{
  ...
  char *qtplug = getenv("QT_PLUGIN_PATH");
  QString plugStr = mImodDir + "/lib/imodplug/";
  if (qtplug)
    plugStr+= QDir::separator() + QString(qtplug);
#ifdef _WIN32
  plugStr = "QT_PLUGIN_PATH=" + plugStr;
  _putenv((const char *)plugStr.toLatin1());
#else
  setenv("QT_PLUGIN_PATH", (const char *)plugStr.toLatin1(), 1);
#endif
...
int ImodAssistant::showPage(const char *page)
{
  ...
  if (!mAssistant) {
    ...
    if (env.contains("QT_SCALE_FACTOR")) {
      env.insert("QT_SCALE_FACTOR", "1.");
      mAssistant->setProcessEnvironment(env);
    }
#endif
...

{
  ...
  // This made 3dmod run from Etomo crash with Qt 5 on Mac when Assistant was missing 
  // libraries; system messages still come out with imodhelp so it is dispensable
#ifndef Q_OS_MACX
  emit error(fullMsg);
#endif
  if (!mTitle.isEmpty())
