...
static int checkMeshDraw(Imesh *mesh, int checkTime, int resol, int thickness);
...
static void drawImageForCurrentModel(ImodvApp *a, int transparent);

static int sCTime = -1;
static float sDepthShift;
static int sCursurf, sCurCont;
static int sThickCont, sThickObj, sObjBeingDrawn, sModBeingDrawn;
static QFont *sLabelFont = NULL;
static int sFontSize = -1;
...
static void drawImageForCurrentModel(ImodvApp *a, int transparent)
{
  bool clipImage = a->imod->view->world & VIEW_WORLD_CLIP_IMAGE;
  if (!a->standalone){
    imodvSetViewbyModel(a, a->imod);
    imodvSetModelTrans(a->imod);
    setStereoProjection(a);
    if (clipImage)
      clip_obj(a->imod, NULL, 1);
    imodvDrawImage(a, transparent);
    if (clipImage)
      clip_obj(a->imod, NULL, 0);
  }
}
...
static void set_curcontsurf(int ob, Imod* imod)
{
  ...
  if (Imodv->current_subset && imod->cindex.object == ob) {
    ...
    sCursurf = imodGetCurMeshSurf(imod);
    if (sCurCont >= 0)
...
void imodvDraw_model(ImodvApp *a, Imod *imod)
{
  int ob = -1;
  Iobj *obj;
  int obstart, obend, drawTrans, loop, nloop, vbSave;
  Imat *mat = Imodv->mat;
  ...
  sCTime = imod->ctime;
  glPushName(ob);
  imodvRotScaleMatrix(Imodv, mat, imod);
    ...
    for (loop = 0; loop < nloop; loop++) {
      for (ob = obstart; ob < obend; ob++) {
        ...
        if (!iobjOff(obj->flags) && 
          ...
          if (!drawTrans && a->drawLabels) {

            // Drawing after the first setObject gave garbled text in Qt 5, 
            // so just undo it, whichs leaves the color set for the labels
            imodvSetObject(imod, obj, DRAW_POINTS, 0);
            imodvSetObject(imod, obj, 0, 0);
            imodvDrawLabels(imod, obj, mat, a->winy, a->mainWin->mDevicePixelRatio,
                            a->mainWin->fontMetrics().height(), a->mainWin->mCurGLw);
          }
          clip_obj(imod, obj, 0);
          //glFinish();
        }
...
/*
 * Set up clipping planes for the given object, or just the global clip planes if obj is
 * NULL
 */
static int clip_obj(Imod *imod, Iobj *obj, int flag)
{
  GLdouble params[4];
  int cpn = 0, clipSet = 1, ip, numSets = 2;
  IclipPlanes *clips = &imod->view->clips;
  GLint maxPlanes;
  if (obj) {
    clips = &obj->clips;
    clipSet = 0;
    numSets = (clips->flags & (1 << 7)) ? 1 : 2;
  }
  ...
  for (clipSet; clipSet < numSets; clipSet++) {
...
static void imodvSetObject(Imod *imod, Iobj *obj, int style, int drawTrans)
{
  ...
  case DRAW_POINTS:
    b3dPointSize(obj->linewidth, obj);
  case DRAW_LINES:
    ...
    b3dLineWidth(obj->linewidth, obj);
...
static int checkMeshDraw(Imesh *mesh, int checkTime, int resol, int thickness)
{
  ...
  if ((Imodv->current_subset == SUBSET_SURF_ONLY || Imodv->current_subset == 
    ...
    return 0;
  if (imeshThickness(mesh->flag) != thickness)
    return 0;
...
static void imodvDraw_object(Iobj *obj, Imod *imod, int drawTrans)
{
  int co, resol, meshThick, flagSave;
  double zscale;
  Imesh *mesh;
  ...
  if (iobjMesh(obj->flags)) {
    ...
    // Deal with a possible change in mesh thickness by making new mesh pairs, or
    // removing all pairs
    meshThick = obj->meshThickness;
    if (utilManagePairedMeshes(obj, sObjBeingDrawn))
      return;
    ...
    if (iobjFill(obj->flags)) {
      ...
      for (co = 0; co < obj->meshsize; co++) {
        mesh = &(obj->mesh[co]);
        if (checkMeshDraw(mesh, checkTime, resol, meshThick)) {
...
/*
 * Draw lines or points in the contours of an object
 */
static void imodvDraw_contours(Iobj *obj, int mode, int drawTrans)
{
  ...
  vbOK = !(handleCoFlgs & HANDLE_VALUE1) && Imodv->vertBufOK > 0 && Imodv->primRestartOK
    && !Imodv->current_subset && !ImodPrefs->noVertBufForCont();
  if (!vbOK)
  ...
  for (co = 0; co < obj->contsize; co++) {
    ...
    if (mode == GL_POINTS) {
      ...
      // Set up to do points
      b3dPointSize(ptProps.linewidth + thickAdd, obj);
      glBegin(GL_POINTS);
      for (; pt < cont->psize; pt++) {
        ...
        if (nextChange == pt) {
          ...
          if (changeFlags & CHANGED_3DWIDTH) {
            glEnd();
            b3dPointSize((GLfloat)ptProps.linewidth + thickAdd, obj);
            glBegin(GL_POINTS);
          }
      ...
      b3dLineWidth(ptProps.linewidth + thickAdd, obj);
...
// According to a post, this is what QGLWidget->renderText does and that people can use 
// to replicate that in QOpenGLWidget.  It is included here because of the need to draw
// with the 2D renderText version to avoid garbled text in Qt 5, and because of the 
// incomprehensible need to swap the model and projection matrices
static void myTransformPoint(GLdouble out[4], const GLdouble m[16], const GLdouble in[4])
{
...
static GLint myProject(GLdouble objx, GLdouble objy, GLdouble objz,
                     ...
                     GLdouble * winx, GLdouble * winy, GLdouble * winz)
{
  ...
  /* The Qt version rightly applies model first then proj, and its output matches that
     of gluProject.  But this gave completely wrong positions since it ends with a big
     shift.  Swapping the order put the text in the right place! */
  myTransformPoint(out, proj, in);
  myTransformPoint(in, model , out);
...
/*
 * Draw any points labels in an object
 */
void imodvDrawLabels(Imod *imod, Iobj *obj, Imat *mat, int winSizeY, float devPixRatio,
                     int fontHeight, QGLWidget *glw)
{
  int co, pt, trans, drawsize;
  Icont *cont;
  int checkTime = (int)iobjTime(obj->flags);
  DrawProps contProps, ptProps;
  int nextChange, stateFlags, changeFlags, changeFlags2, cumInd, remInd, nextRemnant;
  int handleCoFlgs = 0;
  int handlePtFlgs = 0;
  Ipoint offsetPix, offsetMod, sizeOffset;
  const char *ptLabel;
  float lDevicePixelRatio = devPixRatio;
  if (!sCTime)
    checkTime = 0;

  if (!(obj->flags & IMOD_OBJFLAG_DRAW_LABEL))
    return;

#if QT_VERSION > 0x050000
  GLdouble projection[16], modelview[16], winX, winY, winZ;
  GLint viewport[4];
  ...
  // Set up the offset: get transformation from screen pixels to model offset
  // and apply that to needed pixel offset, then apply to vector in X only and normalize
  // that to get the vector to displace away from the center of a spherical point.
  offsetPix.x = 3.;
  offsetPix.y = -fontHeight / 3.;
  offsetPix.z = 0.;
  XY_PIXEL_TO_DEVICE(l, offsetPix.x, offsetPix.y, offsetPix.x, offsetPix.y);
  imodMatTransform(mat, &offsetPix, &offsetMod);
  offsetPix.y = 0.;
  imodMatTransform(mat, &offsetPix, &sizeOffset);
  imodPointNormalize(&sizeOffset);
  ...
  for (co = 0; co < obj->contsize; co++) {
    ...
    // This does not handle anything, just maintains state, so then we have to handle 
    // a color change but skip the trans value.  We also have to set color without trans
    // on the first contour
    nextChange = ifgHandleContChange(obj, co, &contProps, &ptProps, 
                                     &stateFlags, handleCoFlgs, 0);
    if (!co)
      ifgHandleColorTrans(obj, ptProps.red, ptProps.green, ptProps.blue, 0);
    if (contProps.gap)
      continue;
    if (stateFlags & CHANGED_COLOR)
      ifgHandleColorTrans(obj, ptProps.red, ptProps.green, ptProps.blue, 0);

    // Make the label font if it is needed
    if (!sLabelFont || sFontSize != obj->extra[IOBJ_EX_LABEL_SIZE]) {
      sLabelFont = new QFont(QApplication::font());
      if (obj->extra[IOBJ_EX_LABEL_SIZE])
        sLabelFont->setPointSize(obj->extra[IOBJ_EX_LABEL_SIZE]);
      sFontSize = obj->extra[IOBJ_EX_LABEL_SIZE];
    }

    for (pt = 0; pt < cont->psize; pt++) {
      ptProps.gap = 0;

      // Implement a color change before label is drawn
      if (nextChange == pt) {
        nextChange = ifgHandleNextChange(obj, cont->store, &contProps, 
                                         &ptProps, &stateFlags, 
                                         &changeFlags, handlePtFlgs, 0);
      ...
      // This is odd but points are immune to gaps, which occur for segments
      if (!ptProps.gap || !ptProps.valskip) { 
        ptLabel = imodLabelItemGet(cont->label, pt);
        if (ptLabel) {
          drawsize = imodPointGetSize(obj, cont, pt);
#if QT_VERSION > 0x050000
          PIXEL_TO_DEVICE(l, drawsize, drawsize);
          myProject(cont->pts[pt].x + offsetMod.x + drawsize * sizeOffset.x,
                    cont->pts[pt].y + offsetMod.y + drawsize * sizeOffset.y, 
                    cont->pts[pt].z + offsetMod.z + drawsize * sizeOffset.z, 
                    projection, modelview, viewport, &winX, &winY, &winZ);

          // The projection comes back between 0 and 1 in Z for within clipping bounds
          if (winZ >= 0. && winZ <= 1.) {
            XY_DEVICE_TO_PIXEL(l, winX, winSizeY - winY, winX, winY);
            glw->renderText((int)winX, (int)(winY), ptLabel, *sLabelFont);
          }
#else
          glw->renderText
            (cont->pts[pt].x + offsetMod.x + drawsize * sizeOffset.x,
             cont->pts[pt].y + offsetMod.y + drawsize * sizeOffset.y, 
             cont->pts[pt].z + offsetMod.z + drawsize * sizeOffset.z, ptLabel,
             *sLabelFont);
#endif
        }
      }
...
// Delete label font and set to NULL, undefined size
void imodvCleanupLabelFont()
{
  delete sLabelFont;
  sLabelFont = NULL;
  sFontSize = -1;
                              ...
                              int drawTrans)
{
  ...
  vbOK = !(handleFlags & HANDLE_VALUE1) && Imodv->vertBufOK > 0 && Imodv->primRestartOK
    && !Imodv->current_subset && !(style == DRAW_LINES && ImodPrefs->noVertBufForCont())
    && !(style != DRAW_LINES && ImodPrefs->noVBOForSphere());
  ...
  for (co = 0; co < contsize; co++) {
    ...
    if (imodvCheckThickerContour(co))
      b3dLineWidth(obj->linewidth + 2, obj);
        ...
        if (nextChange < i || nextChange > i + 2) {
          ...
          if (style == DRAW_POINTS) {
            ...
            for (j = 0; j < 2; j++) {
              ...
              if (changeFlags & CHANGED_3DWIDTH) {
                glEnd();
                b3dLineWidth(curProps.linewidth, obj);
                glBegin(GL_LINE_STRIP);
