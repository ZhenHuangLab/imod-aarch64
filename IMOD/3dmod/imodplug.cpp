...
static Ilist *sPlugList = NULL;
static int sNumInternal;
static int sEventSource = -1;
...
int imodPlugInit(void)
{
  ...
  sPlugList = ilistNew(sizeof(PlugData), 8);

  /* Add internal modules to list first */
  ipAddInternalModules();
  sNumInternal = ilistSize(sPlugList);
...
static int ipAddInternalModules()
{
  ...
  mi = ilistSize(sPlugList);
  for (i = 0; i < mi; i++){
    lplug = (PlugData *)ilistItem(sPlugList, i);
  ...
  if (!library->load()) {
    if (Imod_debug)
      imodPrintStderr("Warning: %s cannot be loaded as a 3dmod plugin:\n%s\n",
                      LATIN1(plugpath), LATIN1(library->errorString()));
    delete library;
    return(2);
  }
...
void imodPlugOpen(int item)
{
  ...
  PlugData *pd = (PlugData *)ilistItem(sPlugList, item);
  if (!pd) 
    return;
...
{
  PlugData *pd;
  int i, mi = ilistSize(sPlugList);

  for(i = 0; i < mi; i++){
    pd = (PlugData *)ilistItem(sPlugList, i);
    if (!pd)
      continue;
...
{
  int i;
  for (i = sNumInternal; i < ilistSize(sPlugList); i++)
    imodPlugOpen(i);
}
...
void imodPlugMenu(QMenu *parent, QSignalMapper *mapper)
{
  ...
  if (mi){
    ...
    pd = (PlugData *)ilistFirst(sPlugList);
    for(i = 0; i < mi; i++){
 ...
 */
int imodPlugHandleKey(ImodView *vw, QKeyEvent *event, int source)
{
  PlugData *pd;
  int keyhandled, i, mi = ilistSize(sPlugList);
  SpecialKeys fptr;
...
int imodPlugHandleMouse(ImodView *vw, QMouseEvent *event, float imx, float imy,
                        int but1, int but2, int but3, int source)
{
  PlugData *pd;
  int handled, i, needDraw = 0, mi = ilistSize(sPlugList);
  SpecialMouse fptr;
...
int imodPlugHandleEvent(ImodView *vw, QEvent *event, float imx, float imy, int source)
{
  PlugData *pd;
  int handled, i, needDraw = 0, mi = ilistSize(sPlugList);
  SpecialEvent fptr;
...
{
  PlugData *pd;
  int  i, j, numWords, mi = ilistSize(sPlugList);
...
static void *ipGetFunction(PlugData *pd, int which)
{
  if (!pd->library) {
    ...
    switch (which) {
    case IP_INFO:
      return (void *)pd->library->resolve("imodPlugInfo");
    case IP_EXECUTE:
      return (void *)pd->library->resolve("imodPlugExecute");
    case IP_EXECUTETYPE:
      return (void *)pd->library->resolve("imodPlugExecuteType");
    case IP_EXECUTE_MESSAGE:
      return (void *)pd->library->resolve("imodPlugExecuteMessage");
    case IP_KEYS:
      return (void *)pd->library->resolve("imodPlugKeys");
    case IP_MOUSE:
      return (void *)pd->library->resolve("imodPlugMouse");
    case IP_EVENT:
      return (void *)pd->library->resolve("imodPlugEvent");
    case IP_OPEN_FILE_NAME:
      return (void *)pd->library->resolve("imodPlugOpenFileName");
    case IP_OPEN_FILE_NAMES:
      return (void *)pd->library->resolve("imodPlugOpenFileNames");
    case IP_SAVE_FILE_NAME:
      return (void *)pd->library->resolve("imodPlugSaveFileName");
    default:
      break;
    }
  }
