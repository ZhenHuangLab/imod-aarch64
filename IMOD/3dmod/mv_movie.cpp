...
static DockingDialog *sMovieTopWin = NULL;
static DockingDialog *sSeqTopWin = NULL;
...
void mvMovieClosing()
{
  ...
  imodvDialogManager.remove((QWidget *)sMovieTopWin);
  sDia = NULL;
  sAbort = 1;
  if (sSequenceDia)
...
void mvMovieDialog(ImodvApp *a, int state)
{
  ...
  sMovieTopWin = new DockingDialog(imodvDialogManager.parent(IMODV_DIALOG),
                                   &imodvDialogManager, "Model View Movie Controls",
                                   "modelMovie.html#TOP", 'm', Qt::Window);  
  sDia = new imodvMovieForm(sMovieTopWin, 0);
  if (!sDia){
    dia_err("Failed to create 3dmodv movie window!");
    return;
  }
  sMovieTopWin->addWidgetToStack(sDia);

  // Set title bar
  setModvDialogTitle(sMovieTopWin, "MV Movies");
  ...
  adjustGeometryAndShow((QWidget *)sMovieTopWin, IMODV_DIALOG);
  imodvDialogManager.add((QWidget *)sMovieTopWin, IMODV_DIALOG, DOCKING_DIALOG_TYPE);
...
void mvMovieSequenceDialog(ImodvApp *a, int state)
{
  ...
  sSeqTopWin = new DockingDialog(imodvDialogManager.parent(IMODV_DIALOG),
                                 &imodvDialogManager, "Model View Movie Sequences",
                                 "movieSequence.html#TOP", 'Q', Qt::Window);  
  sSequenceDia = new MovieSequenceForm(sSeqTopWin, 0);
  if (!sSequenceDia) {
    dia_err("Failed to create 3dmodv movie sequence window!");
    return;
  }
  sSeqTopWin->addWidgetToStack(sSequenceDia);

  // Set title bar
  setModvDialogTitle(sSeqTopWin, "Sequences");


  // Height is managed in the init function
  adjustGeometryAndShow(sSeqTopWin, IMODV_DIALOG, true);
  imodvDialogManager.add(sSeqTopWin, IMODV_DIALOG, DOCKING_DIALOG_TYPE);
  sSequenceDia->setForFourRows();
...
{
  imodvDialogManager.remove(sSeqTopWin);
  sSequenceDia = NULL;
  sSeqTopWin = NULL;
  if (sDia)
...
void mvMovieSetSegment(MovieSegment &segment)
{
  ...
  for (se = 0; se < 2; se++) {
    ...
    if (segment.imgAxisFlags && !Imodv->standalone) {
...
static int makeMovie(int frames, bool fromSequence)
{
  ...
  float thickStart, thickStep, transpStart, transpStep, frameFrac;
  ...
  for(frame = 1; frame <= frames; frame++){
    ...
    if (frame < frames) {
      frameFrac = frame / B3DMAX(1., frames - 1.);
      ...
      for (pl = 0; pl < B3DMIN(sStartClips.count, sEndClips.count); pl++) {
        vw->clips.point[pl].x = sStartClips.point[pl].x + 
          frameFrac * (sEndClips.point[pl].x - sStartClips.point[pl].x);
        vw->clips.point[pl].y = sStartClips.point[pl].y + 
          frameFrac * (sEndClips.point[pl].y - sStartClips.point[pl].y);
        vw->clips.point[pl].z = sStartClips.point[pl].z + 
          frameFrac * (sEndClips.point[pl].z - sStartClips.point[pl].z);
      }

      for (obNum = 0; obNum < numObj; obNum++) {
        if (sStartObjTrans[obNum] != sEndObjTrans[obNum]) {
          Imodv->imod->obj[obNum].trans = 
            (unsigned char)B3DNINT(sStartObjTrans[obNum] + frameFrac * 
                                   (sEndObjTrans[obNum] - sStartObjTrans[obNum]));
        }
      }
    }
  }
