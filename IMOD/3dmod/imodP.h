...
typedef struct imod_application
{
  ...
  int infoInitialSetup;  // Flag that info window is being set up
  float DevicePixelRatio;    // Scaling between pixels used in Qt and device pixels
  float maxDevPixRatio;    // Maximum ratio for all screens
  int devPixVaries;    // Flag that there is more than one device/pixel ratio
}ImodApp;
...
typedef struct ViewInfo
{
  ...
  int      didModelInitInLoad; // Flag that model was read in during initial load
  int      xybin;             // Binning in X and Y dimensions 
  int      zbin;              // Binning in Z 
  int      eerSuperRes;       // Super-resolution factor in TIFF reader for EER files 
  int      eerZbinning;       // Grouping of EER files in Z by TIFF reader
  ...
  int      maxReadThreads;    // Maximum # of threads for reading
  int      numReadThreads;    // Number of threads for reading current open file
  ImodImageFile *fileCopies[MAX_READ_THREADS]; // File copies used for parallel reading
  ...
  int     intOptEntered;  // Flag the the user entered the -I option   
  ...
  int     scaleScanType;  // Type of scanning to do if needed for scaling
  int     storeScanInMRC; // Flag to store result of MRC scan in header
  int     switchToUshort; // Flag to switch to ushort loading when estimate or use mean/SD
...
//////////////
/* A NOTE ON SCALING FOR HIGH DPI
   Once high-DPI scaling is enable in the QApplication, Qt will scale fonts and layout
   spacing and widget elements to give well-proportioned dialogs with large fonts
   At that point all sizes and coordinates from Qt are in these device-independent pixels
   which are referred to as PIXEL in the macros.  This includes mouse coordinates, even
   in OpenGL windows.  But OpenGL continues to function with
   the native screen pixels, referred to as DEVICE in the macros.  The scaling between 
   them is stored in App->DevicePixelRatio or in a variable ending in DevicePixelRatio 
   for the individual window.  This must be multiplied by to get from Qt 
   positions and sizes to positions and sizes on the screen in OpenGL windows, and to 
   image pixels at zoom 1.  All coordinates continue to be held as the native 
   image/device pixels.  This includes the variables that store "window size" from a
   resize event, which gives full native pixels for the OpenGL windows.  This means that 
   mouse coordinates from an event are immediately converted by multiplying before use 
   in any following operations, and need to be divided before setting the mouse position.
   Setting initial window sizes and zooms involves conversions to relate them to image 
   pixels, typically in both directions.
   Sizes generally need to be scaled in various symbol drawing functions, except circles
   that may be in image pixels, and rectangles that are assumed to be in image pixels too.
   Line thicknesses are scaled by default, with a user option to turn off this scaling
   for object drawing.
*/
   

/* Macros for scaling between QT device-independent pixels and device and image pixels */
#if QT_VERSION >= 0x050600
#define DEVICE_TO_PIXEL(a, dev, pix)  pix = B3DNINT((dev) / a##DevicePixelRatio)
#define XY_DEVICE_TO_PIXEL(a, dx, dy, px, py) {      \
    px = B3DNINT((dx) / a##DevicePixelRatio);        \
    py = B3DNINT((dy) / a##DevicePixelRatio); }
#define PIXEL_TO_DEVICE(a, pix, dev)  dev = B3DNINT((pix) * a##DevicePixelRatio)
#define XY_PIXEL_TO_DEVICE(a, px, py, dx, dy) {     \
    dx = B3DNINT((px) * a##DevicePixelRatio);       \
    dy = B3DNINT((py) * a##DevicePixelRatio); }
#define B3DQT_PIXELS(a, dev) B3DNINT((dev) / a##DevicePixelRatio)
#else
#define DEVICE_TO_PIXEL(a, dev, pix)  pix = (dev)
#define XY_DEVICE_TO_PIXEL(a, dx, dy, px, py)  { px = (dx); py = (dy); }
#define PIXEL_TO_DEVICE(a, pix, dev)  dev = (pix)
#define XY_PIXEL_TO_DEVICE(a, px, py, dx, dy) { dx = (px); dy = (py); }
#define B3DQT_PIXELS(a, dev) (dev) 
#endif
...
#define IMOD_GHOST_2SHADES (1<<5)
#define IMOD_GHOST_ALLSCAT (1<<6)
