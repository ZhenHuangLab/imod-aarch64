...
static const char *cursorList[] = 
  {":/images/cursor.png", ":/images/cursor_mask.png", ":/images/cursor1.5.png",
   ":/images/cursor_mask1.5.png", ":/images/cursor2.png", ":/images/cursor_mask2.png",
...
int imod_display_init(ImodApp *ap, char **argv)
{
  ...
  // Workaround for M1 mac, it was double-scaling the cursor and the hot spot was in
  // in the corner for a proper-sized cursor
  // 9/26/21: problem appears to have been fixed in OS 11.5, but leave solution for a while
#ifdef Q_OS_MACX
  char buffer[512];
  size_t oldlen = 512;
  if (getenv("IMOD_M1_CURSOR_FIX") && !sysctlbyname("kern.version", &buffer, &oldlen, NULL, NULL)) {
    QString str = buffer;
    macM1 = str.indexOf("ARM64") > 0;
  }
#endif
  ...
  if (!macM1 && ap->DevicePixelRatio >= 1.75)
    cursorInd = 2;
  else if (!macM1 && ap->DevicePixelRatio >= 1.25)
    cursorInd = 1;
  else if (macM1 && ap->DevicePixelRatio >= 1.25)
    cursorInd = 3;

  QBitmap bmCursor = QBitmap::fromImage(QImage(cursorList[2 * cursorInd]), Qt::MonoOnly);
  QBitmap bmMask = QBitmap::fromImage(QImage(cursorList[2 * cursorInd +1]), Qt::MonoOnly);

  // Only Windows needs this to prevent the cursor from being scaled like other pixmaps
#if defined(_WIN32) && QT_VERSION >= 0x050000
  bmCursor.setDevicePixelRatio(ap->DevicePixelRatio);
  bmMask.setDevicePixelRatio(ap->DevicePixelRatio);
#endif

  // For some reason, passing -1, -1 did NOT work on regular Mac, so make a separate call
  // This is still one pixel off so there is a shifted cursor for M1
  if (macM1 && ap->DevicePixelRatio >= 1.25)
    ap->modelCursor = new QCursor(bmCursor, bmMask, bmCursor.width() - 1,
                                  bmCursor.height() - 1);
  else
    ap->modelCursor = new QCursor(bmCursor, bmMask);    
