...
void imod_usage(char *name)
{
  ...
  qstr += "   -b nxy,nz  Bin images by nxy in X and Y, by nz in Z (nz default=1).\n";
  qstr += "   -es #  Set amount of super-resolution when reading EER files (1=2x, "
    "2=4x).\n";
  qstr += "   -ez #  Set initial summing of frames (grouping in Z) when reading EER "
    "files.\n";
  qstr += "   -a <file name>  Load tilt angles from file.\n";
  qstr += "   -p <file name>  Load piece list file.\n";
  qstr += "   -A #  Use aligned piece coordinates from .mdoc file (1, or 2 for very "
    "sloppy)\n";
  qstr += "   -P nx,ny  Display images as montage in nx by ny array.\n";
  qstr += "   -o nx,ny  Set X and X overlaps for montage display.\n";
  qstr += "   -f    Load as frames even if image file has piece coordinates.\n";
  ...
  qstr += "   -J #  Type of scanning to find unknown scaling limits (0-2).\n";
  qstr += "   -K    Store results of scanning for limits in MRC file header.\n";
  ...
  qstr += "   -T    Display multiple single-image files as times not sections.\n";
  qstr += "   -G    Display color file in gray-scale.\n";
...
int main( int argc, char *argv[])
{
  ...
  int i      = 0;
  int nx, ny, nz, mode, newNumArg, ifArgAlloc, noMatchInd;
  int minxpiece, numXpieces, xoverlap, minypiece, numYpieces, yoverlap;
  char **newVec, **argvTemp;
  float initAngles[3];
  Ipoint initCenter;
  int setAngles = 0;
  int setCenter = 0;
  ...
  // See imodP.h for a note on how scaling is done with OpenGL
  diaHandleFontAndDPI(false);

  /* Open the Qt application after setting up library path */
  diaSetQtLibraryPath();
  ...
  // Get the information about device pixel ratios and adjust the default font on Windows,
  // which will be overridden by a font in settings
  App->DevicePixelRatio = diaGetAppDevPixRatioInfo(App->maxDevPixRatio, App->devPixVaries);
  diaAdjustFontForDPI(App->DevicePixelRatio, Imod_debug != 0);

  if (Imod_debug) {
    QDesktopWidget *desk =  QApplication::desktop();
    int nums = desk->numScreens();
    
    imodPrintStderr("%d screens, %d primary, virtual %d width %d hei %d\n", nums,
                    desk->primaryScreen(),
                    desk->isVirtualDesktop()?1:0, desk->width(), desk->height());
    for (int scr = 0; scr < nums; scr++) {
      QRect geom = desk->screenGeometry(scr);
      QRect avail = desk->availableGeometry(scr);
      imodPrintStderr("%d: geom %d x %d l %d t %d, avail %d x %d l %d t %d\n",scr,
                      geom.width(), geom.height(), geom.left(), geom.top(),
                      avail.width(), avail.height(), avail.left(), avail.top());
    }
  }
  ...
  pieceKeyType = ImodPrefs->useAliPieceCoords();
  ...
  if (argcHere < 2 || (doStartup && doImodv)) {
    ...
    if (doImodv) {
      ...
      startup->setValues(&vi, argv, firstfile, argcHere, doImodv, plFileNames, 
                         anglefname, pieceKeyType, xyzwinopen, sliceropen,
                         zapOpen, modelViewOpen, fillCache, ImodTrans, 0, frames,
  ...
  for (argScan = 0; argScan <= doStartup; argScan++) { 
    ...
    // Overlay preferences on initialization of vi
    if (ImodPrefs->loadUshorts())
      vi.rawImageStore = MRC_MODE_USHORT;
    vi.scaleScanType = ImodPrefs->scaleScanType();
    vi.storeScanInMRC = ImodPrefs->changeMRCstats() ? 1 : 0;
    vi.ghostlast = ImodPrefs->getGhostMode();
    vi.ghostmode = vi.ghostlast  & ~IMOD_GHOST_SECTION;
    vi.ghostdist = ImodPrefs->getGhostDist();
    vi.eerSuperRes = ImodPrefs->eerSuperRes();
    vi.eerZbinning = ImodPrefs->eerZbinning();
    plistfname = NULL;
    xyzwinopen   = FALSE;
    sliceropen   = FALSE;
    zapOpen      = FALSE;
    ...
    for (i = 1; i < argcHere; i++){
      if (argv[i][0] == '-'){
        ...
        case 'e':
          if (argv[i][2] == 's') {
            sscanf(argv[++i], "%d", &(vi.eerSuperRes));
            B3DCLAMP(vi.eerSuperRes, 0, tiffGetMaxEERsuperRes());
          } else if (argv[i][2] == 'z') {
            sscanf(argv[++i], "%d", &(vi.eerZbinning));
            vi.eerZbinning = B3DMAX(1, vi.eerZbinning);
          } else
            badOption(argv[i]);
          break;

        case 'E':
          pathlen = strlen(argv[++i]) + 2;
          if (windowKeys) {
            ind = strlen(windowKeys);
            pathlen += ind;
            windowKeys = (char *)realloc(windowKeys, pathlen);
          } else {
            windowKeys = (char *)malloc(pathlen);
            ind = 0;
          }
          if (windowKeys)
            strcpy(&windowKeys[ind], argv[i]);
          break;
        ...
        case 'A':
          pieceKeyType = atoi(argv[++i]);
          B3DCLAMP(pieceKeyType, 0, 2);
          break;
        ...
        case 'J':
          sscanf(argv[++i], "%d", &vi.scaleScanType);
          B3DCLAMP(vi.scaleScanType, 0, 2);
          break;
    ...
    // First time through, expand filename arguments on Windows
    if (!argScan && firstfile > 0) {
      newVec = expandArgList((const char **)&argv[firstfile], argcHere - firstfile, 
                             &newNumArg, &ifArgAlloc, &noMatchInd);
      if (noMatchInd >= 0) {
        imodError(NULL, "No file matches the entry %s\n", argv[noMatchInd + firstfile]);
        exit(1);
      }

      // Get new full vector if newVec was allocated
      if (ifArgAlloc && newVec)
        argvTemp = B3DMALLOC(char *, firstfile + newNumArg);
      if (!newVec || (ifArgAlloc && !argvTemp)) {
        imodError(NULL, "Could not allocate memory for expanding filename arguments.\n");
        exit(1);
      }

      // Copy existing arg pointers up to firstfile, then new pointers, replace argv
      if (ifArgAlloc) {
        for (i = 0; i < firstfile; i++)
          argvTemp[i] = argv[i];
        for (i = 0; i < newNumArg; i++)
          argvTemp[i + firstfile] = newVec[i];
        argv = argvTemp;
        argcHere = firstfile + newNumArg;
      }
    }
...
#if QT_VERSION >= 0x050600
  if (Imod_debug) 
    imodPrintStderr("DevicePixelRatio %f  logical DPI %d x %d  physical %d x %d\n", 
  ...
  tiffSetEERreadProperties(vi.eerSuperRes, vi.eerZbinning, 0);
  ...
  // But initialize the false color ramp for value drawing with default or loaded map now
  ifgMapFalseColor(0, &mode, &mode, &mode);
  ...
  // Disable memory mapping because files will be kept open
  tiffSetMapping(0);
  ...
  if (!vi.fakeImage && vi.numTimes <= 1 && !vi.li->plist && !dataFromStdin) {
    ...
    if (!vi.li->plist && !frames && 
        (vi.image->file == IIFILE_MRC || vi.image->file == IIFILE_HDF || 
         vi.image->file == IIFILE_ADOC)) {
      ivwReopen(vi.image);
      if (QFile::exists(QString(vi.image->filename) + ".mdoc")) {
        useMdoc = pieceKeyType + 1;
        iiPlistSetAdocCoordType(pieceKeyType);
      }
  ...
  // 12/11/19: This is needed in linux with QT_SCALE_FACTOR set to prevent big menu font
#ifdef __linux
  if (App->DevicePixelRatio > 1 && getenv("QT_SCALE_FACTOR"))
  ...
  if (fillCache && vi.vmSize && !vi.stripOrTileCache && !vi.imagePyramid)
    imodCacheFill(&vi);
  ...
  if (sliceropen && setZoom)
    setTopSlicerZoom(sInitialZoom, false);
  sInitialZoom = 0.;
...
bool windowKeysHas(char key)
{
  return windowKeys != NULL && strchr(windowKeys, key) != NULL;
}

// Returns an initial zoom if one was entered, and 0 after startup is done
float imodInitialZoom()
{
  return sInitialZoom;
}
