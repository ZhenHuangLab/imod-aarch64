...
void inputGotoSurface(ImodView *vi, int target)
{
  ...
  // Try to set surface in contourless mesh and if that works, update various places
  imodSetCurMeshSurf(vi->imod, target);
  if (imodGetCurMeshSurf(vi->imod) == target && target >= 0) {
    imodContEditSurfShow();
    fineGrainUpdate();
    imodv_draw();
  }
...
{
  Iobj *obj;
  int co, newcont, cursurf, curTime;
  bool hasTimes;
  ...
  /* Selected Object. */
  obj = &(vi->imod->obj[vi->imod->cindex.object]);
  hasTimes = iobjFlagTime(obj) && vi->numTimes > 1;
  ...
  cursurf = obj->cont[vi->imod->cindex.contour].surf;
  newcont = -1;
  curTime = obj->cont[vi->imod->cindex.contour].time;

  for (co = vi->imod->cindex.contour + direction; 
       co >= 0 && co < obj->contsize; co += direction)
    if (obj->cont[co].surf == cursurf && (!hasTimes || obj->cont[co].time == curTime)) {
      newcont = co;
      break;
    }
  if (newcont == -1)
...
void inputQDefaultKeys(QKeyEvent *event, ImodView *vi)
{
  ...
  case 0x3001:      // For when connecting from a Mac to a cluster. IMOD list 3/28/22
  ...
  case Qt::Key_F:
    if (ctrl) {
      mean = B3DNINT(vi->zmouse);
      if (vi->multiFileZ)
        wprint("File at Z = %d: %s\n", mean + 1, vi->imageList[mean].filename);
    } else if (shifted)
  ...
  case Qt::Key_T:
    if (shifted){
      ...
      vi->imod->drawmode = vi->imod->drawmode > 0 ? -1 : 1;
      imodModelEditUpdate();
    }
