...
// This returns the midpoint of the bin at which the given percentile is crossed
float HistWidget::thresholdForPercentile(float percentile)
{
  int i = 0;
  float currPercent = 0;
  ...
  if (i > 0 && currPercent > percentile && hist[i - 1] > 0.)
    return i - (currPercent - percentile) / hist[i - 1];
  return i + 0.5;
}

// This returns the percentile (cumulative histogram sum) for a given threshold value
float HistWidget::percentileAtPoint(float threshold)
{
  int i, intThresh = (int)threshold;
  float currPercent = 0.;
  for (i = 0; i < intThresh && i < 256; i++)
    currPercent += hist[i];
  if (i < 256)
    currPercent += (threshold - intThresh) * hist[i];
  return currPercent;
}
