...
static objectEditForm *Ioew_dialog = NULL;
...
static void setExtraItem(int index, int value);
...
void ioewDrawLabels(int state)
{
  setObjectFlag(state != 0, 0, IMOD_OBJFLAG_DRAW_LABEL);
}

void ioewLabelSize(int value)
{
  if (value < MIN_LABEL_SIZE)
    value = 0;
  setExtraItem(IOBJ_EX_LABEL_SIZE, value);
}
...
void ioewScaleForDPI(int state)
{
  setObjectFlag(state != 0, 0, IMOD_OBJFLAG_SCALE_WDTH);
  imodvObjedNewView();
}
...
{
  setExtraItem(IOBJ_EX_2D_TRANS, value);
}
...
void ioewCopyObj(int value)
{
  ...
  obj->extra[IOBJ_EX_LABEL_SIZE] = fromObj->extra[IOBJ_EX_LABEL_SIZE];
  App->cvi->undo->finishUnit();
  imodDraw(App->cvi, IMOD_DRAW_MOD);
}
...
int imod_object_edit()
{
  ...
  sTopWin = new DockingDialog(imodDialogManager.parent(IMOD_DIALOG), &imodDialogManager,
                              "Edit Object Properties", "objectEdit.html#TOP", 't',
                              Qt::Window);
  Ioew_dialog = new objectEditForm(sTopWin, 0);

  if (!Ioew_dialog){
    dia_err("Object edit failed.");
    return(-1);
  }
  sTopWin->addWidgetToStack(Ioew_dialog);
  sTopWin->setWindowTitle(imodCaption("Edit Obj"));
...
int imod_object_edit_draw(void)
{
  ...
  Ioew_dialog->setDrawLabelsBox((obj->flags & IMOD_OBJFLAG_DRAW_LABEL) != 0);
  Ioew_dialog->setScaleForDpi((obj->flags & IMOD_OBJFLAG_SCALE_WDTH) != 0, 
                              App->maxDevPixRatio > 1.);
  i = obj->extra[IOBJ_EX_LABEL_SIZE];
  if (i < MIN_LABEL_SIZE)
    i = MIN_LABEL_SIZE - 1;
  Ioew_dialog->setLabelSize(i);
...
{
  imod_cmap(Model);
  imodDialogManager.remove(sTopWin);
  Ioew_dialog = NULL;
  sTopWin = NULL;
}
