...
void SlicerFuncs::fillImageArray(int panning, int meanOnly, int rgbChannel)
{
  ...
  float zoffset, sample, edge, fftBaseScale;
  float matt = 0.1;
  ...
  float sigma1, radius2, sigma2, radius1;
  Islice filtSlice;
  ...
  numThreads = numOMPthreads(MAX_THREADS);
  ...
  if (mNoPixelZoom) {
    if (mHq && mZoom < 1.) {
      ...
      // Set up for full-size image to antialias down; but if buffer is too small, 
      // reduce the amount to zoom down and set to fill buffer at larger intervals
      mRemainingZoom = mZoom;
      if ((float)sIsize * sJsize > mCurBufSize) {
        mRemainingZoom /= sqrt(mCurBufSize / ((sIsize + 4.) * (sJsize + 4.)));
        sIsize = (int)(mWinx / mRemainingZoom + 0.9);
        sJsize = (int)(mWiny / mRemainingZoom + 0.9);
      }
    } else {
      sIsize = mWinx; /* calculate each pixel for zoom unless FFT. */
      sJsize = mWiny;
    }
    sXsx *= mRemainingZoom / xzoom;
    sYsx *= mRemainingZoom / xzoom;
    sZsx *= mRemainingZoom * zs / xzoom; 
    sXsy *= mRemainingZoom / yzoom;
    sYsy *= mRemainingZoom / yzoom;
    sZsy *= mRemainingZoom * zs / yzoom;
    xo -= (sIsize / 2) * sXsx;
    yo -= (sIsize / 2) * sYsx;
    zo -= (sIsize / 2) * sZsx;
    xo -= (sJsize / 2) * sXsy;
  ...
  if (sShortcut) {
    ...
    slicerCubicFillin((b3dUInt16 *)sCidata, sIsize, sJsize, sIzoom, sIlimShort,
                      sJlimShort, sMinVal * sKsize, sMaxVal * sKsize, 1);
  }

  // If computing mean only or scaling to mean, get the mean and SD of the
  // slice with some edges cut off
  if (meanOnly || (mScaleToMeanSD && sKsize > 1 && !mUseLastScale) || mFftMode) {
    linePtrs = ivwMakeLinePointers(mVi, (unsigned char *)sCidata, sIsize, 
  ...
  if (!mUseLastScale || mFftMode) {
    ...
    if (k > 1) {
      ...
      if (mScaleToMeanSD && mSumSD > 0.1 && mOneSliceSD > 0.1) {
        mScale = mOneSliceSD / mSumSD;
        mOffset  = mOneSliceMean - mSumMean * mScale;
        mSumMean = mOneSliceMean;
        mSumSD = mOneSliceSD;
      } else if (mFftMode) {
      ...
      /*sclmap = get_short_map(mScale, mOffset, 0, 255, MRC_RAMP_LIN, 0, 0);
        if (!sclmap) {
  ...
  if (mFftMode) {
    ...
    if (slice) {
      ...
      for (i = 0; i < sIsize * sJsize; i++) {
        tval = (int)(sCidata[i] * mScale + mOffset);
      ...
      // Taper the edges, but less for large images; this is slow and never meant for
      // big tapers
      i = B3DMAX(sIsize, sJsize);
      ntaper = (int)B3DMAX(0.01 * i, B3DMIN(84, 0.05 * i));
      if (ntaper)
      ...
      if (sliceByteBinnedFFT(slice, 1, 0, sIsize - 1, 0, sJsize - 1, &i, &j) > 0) {
        ...
        mScale = fftBaseScale;
        mOffset = 0.;
        if (mSumSD > 0.1 && edge < 254.5) {
          mScale = (numPosSD + numNegSD) * mSumSD / (255 - edge);
          mOffset = (mSumMean - numNegSD * mSumSD) - mScale * edge;
        }
        if (imodDebug('s'))
          imodPrintStderr("FFT edge %f  scale %f  offset %f\n", edge, mScale,
                          mOffset);
        fftmap = get_byte_map(mScale, mOffset, 0, sMaxVal, 0);
        usfftmap = (b3dUInt16 *)fftmap;
        if (!mVi->ushortStore) {
          for (i = 0; i < sIsize * sJsize; i++)
            ubidata[i] = fftmap[*bdata++];
        } else {
          for (i = 0; i < sIsize * sJsize; i++)
            usidata[i] = usfftmap[*bdata++];
        }
    ...
    // Repack data into integer or byte array, scaling as needed
    if (k > 1) {
      for (i = 0; i < sIsize * sJsize; i++)
        *usidata++ = (b3dUInt16)(sCidata[i] * mScale + mOffset);
    } else {
      for (i = 0; i < sIsize * sJsize; i++)
        *usidata++ = (b3dUInt16)sCidata[i];
    }
  } else {
    if (k > 1) {
      for (i = 0; i < sIsize * sJsize; i++) {
        tval = (int)(sCidata[i] * mScale + mOffset);
        *ubidata++ = (b3dUByte)B3DMAX(0, B3DMIN(255, tval));
      }
    } else {
      for (i = 0; i < sIsize * sJsize; i++)
        *ubidata++ = (b3dUByte)sCidata[i];
    }
  }

  // Do fourier filter if selected
  if (iprocFilterForSlicer(radius1, sigma1, radius2, sigma2)) {
    sliceInit(&filtSlice, sIsize, sJsize, mVi->ushortStore ? SLICE_MODE_USHORT : 
              SLICE_MODE_BYTE, sCidata);
    sliceMMM(&filtSlice);
    sliceFourierFilter(&filtSlice, sigma1, sigma2, radius1, radius2);
  }
