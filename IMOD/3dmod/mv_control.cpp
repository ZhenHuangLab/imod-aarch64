 ...
 *
 *  Copyright (C) 1995-20019 by the Regents of the University of 
...
int imodv_control(ImodvApp *a, int state)
{
  ...
  sTopWin = new DockingDialog(imodvDialogManager.parent(IMODV_DIALOG), 
                              &imodvDialogManager, "Model Display Controls",
                              "modvControl.html#TOP", 'C', Qt::Window);
  dialog = new imodvControlForm(sTopWin, 0);
  if (!dialog) {
    dia_err("Failed to create 3dmodv controls window!");
    return(-1);
  }
  sTopWin->addWidgetToStack(dialog);
  setModvDialogTitle(sTopWin, "MV Controls");
