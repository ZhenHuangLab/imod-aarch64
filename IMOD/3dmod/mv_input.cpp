...
void imodvKeyPress(QKeyEvent *event)
{
  ...
  obj = &imod->obj[ob];
  if (obj)
    scatteredObj = iobjScat(obj->flags) != 0;
  ...
  case Qt::Key_C: /* print the current clipping plane parameters */
    if (shifted && ctrl) {
      if (scatteredObj)
        inputNextContour(Imodv->vi);
      
    } else if (shifted) {
      imodv_control(a, 1);
    } else if (ctrl) {
      imodvObjedToggleClip(-1, -1);
    } else {
      objedObject();
      if (a->obj){
  ...
  case Qt::Key_X:
    if (shifted && ctrl) {
      if (scatteredObj)
        inputPrevContour(Imodv->vi);
    } else if (shifted) {
      if (!a->standalone && !a->vi->fakeImage) {
        mvImageTogglePlane(IMODV_DRAW_CX);
        mvImageUpdate(a);
      }
    }
    break;
  ...
  case Qt::Key_ParenLeft:
    if (scatteredObj)
      inputPrevPoint(Imodv->vi);
    break;
          
  case Qt::Key_BraceRight:
  case Qt::Key_ParenRight:
    if (scatteredObj)
      inputNextPoint(Imodv->vi);
    break;
  ...
  case Qt::Key_V:
    if (shifted && ctrl && a->vertBufOK >= 0) {
      ...
      imodvObjedDrawClipPlane(Imodv->drawClip == 0);
    break;

  case Qt::Key_1:
    if (ctrl) {
      imodvObjedToggleClip(0, 0);
    } else {
      imodvStepTime(a,-1);
      imodvDraw(a);
    }
  ...
  case Qt::Key_Y:
    if (ctrl) 
      ...
      if (!a->standalone && !a->vi->fakeImage) {
        mvImageTogglePlane(IMODV_DRAW_CY);
        mvImageUpdate(a);
      }
    } else
      imodv_rotate_model(a, 0, -a->deltaRot, 0);
  ...
  case Qt::Key_Z:
    ...
    else if (!a->standalone && !a->vi->fakeImage) {
      mvImageTogglePlane(IMODV_DRAW_CZ);
      mvImageUpdate(a);
    }
    break;
  ...
  case Qt::Key_4:
    if (ctrl)
      imodvObjedToggleClip(11, keysym - (int)Qt::Key_3);
    break;

  case Qt::Key_F1:
    imodvObjedSetDrawTypeAndStyle(11);
    break;
  case Qt::Key_F2:
    imodvObjedSetDrawTypeAndStyle(22);
    break;
  case Qt::Key_F3:
    imodvObjedSetDrawTypeAndStyle(21);
    break;
  case Qt::Key_F4:
    imodvObjedSetDrawTypeAndStyle(-1);
    break;
...
void imodvMousePress(QMouseEvent *event)
{
  ...
  XY_PIXEL_TO_DEVICE(a->mainWin->m, event->x(), event->y(), ex, ey);
  ...
  if (event->button() == ImodPrefs->actualModvButton(1)) {
    ...
    if (event->button() == ImodPrefs->actualModvButton(2) && shift && ctrl)
      imodvSelect(a, ex, ey, false, true, false);

  } else if (event->button() == ImodPrefs->actualModvButton(3)) {
    imodvSelect(a, ex, ey, false, false, (shift && ctrl));
  }
}
...
void imodvMouseRelease(QMouseEvent *event)
{
  ...
  if (((event->button() & ImodPrefs->actualModvButton(2)) && !shift && !ctrl) || 
      (rightWasDown && shift))
    imodv_rotate(a, ex, ey, 1, rightWasDown);
  if (a->drawLight) {
    a->drawLight = 0;
    imodvDraw(a);
  }
...
void imodvRotScaleMatrix(ImodvApp *a, Imat *mat, Imod *imod)
{
  ...
  /* 11/17/18: add the division by the scale.xyz because it was done after all 3 calls
     to this.  */
  spt.x = 1.0f / (scrnscale * imod->view->scale.x);
  spt.y = 1.0f / (scrnscale * imod->view->scale.y);
  spt.z = 1.0f / (scrnscale * imod->view->scale.z * imod->zscale);
                         ...
                         bool insert, bool curObj)
{
  ...
  int tmo, tob, tco, tpt;
  int moNum, obNum, coNum, ptNum, minco, cosave, obsave, minpt;
  Iindex indSave;
  Ipoint pickpt;
  Ipoint *pts;
  Iobj *obj;
  ...
  imodGetIndex(a->imod, &obNum, &coNum, &ptNum);
  obsave = obNum;
  cosave = coNum;
  ...
  for (i = 0; i < hits; i++) {    /* for each hit */
    ...
    if (names > 3 && (ptNum == -1 || zav <= zmin) && (!curObj || tob == obsave)) { 
      zmin = zav;
      moNum = tmo; obNum = tob; coNum = tco; ptNum = tpt;
      if (imodDebug('p'))
      imodPrintStderr (" *");
    }
    if (imodDebug('p')) {
  ...
  if (coNum < 0 || ptNum == -1 || obNum >= a->mod[moNum]->objsize)
    return;

  // 11/29/08: call central select function if changing model
  if (a->curMod != moNum)
    imodvSelectModel(a, moNum);

  if (obNum >= 0)
    obj = &a->imod->obj[obNum];
  else {
    obj = ivwGetAnExtraObject(a->vi, -1 - obNum);
    if (!obj || a->standalone)
      return;
    ...
    if (obj->contsize) {
      if (coNum < obj->contsize) {
        a->vi->xmouse = obj->cont[coNum].pts[ptNum].x;
        a->vi->ymouse = obj->cont[coNum].pts[ptNum].y;
        a->vi->zmouse = obj->cont[coNum].pts[ptNum].z;
        ivwBindMouse(a->vi);
        if (imodDebug('p'))
  ...
  if (coNum >= obj->contsize) {
    coNum -= 1 + obj->contsize;
    
    if (!iobjMesh(obj->flags) || coNum < 0 || coNum >= obj->meshsize || 
        ptNum >= obj->mesh[coNum].vsize)
      return;
    pickpt = obj->mesh[coNum].vert[ptNum];
    if (!obj->contsize || insert) {
      ...
      ivwBindMouse(a->vi);
      if (imodDebug('p'))
        imodPrintStderr ("%smesh, point at %.1f %.1f %.1f  inserting %d\n", 
                         !obj->contsize ? "Contourless " : "", a->vi->xmouse,
                         a->vi->ymouse, a->vi->zmouse, insert?1:0);
      if (insert)
        inputInsertPoint(a->vi);
      else {
        if (a->imod->mousemode == IMOD_MMODEL || !obj->contsize) {

          // for a real object, set the object index and update the info window
          if (obNum >= 0) {
            imodSetIndex(a->imod, obNum, -1, -1);
            imodSetCurMeshSurf(a->imod, obj->mesh[coNum].surf);
            tpt = ilistSize(a->vi->selectionList);
            if (!moving || imodSelectionListQuery(a->vi, obNum, -1) < -1)
    ...
    for (coNum = 0; coNum < obj->contsize; coNum++) {
      pts = obj->cont[coNum].pts;
      for (ptNum = 0; ptNum < obj->cont[coNum].psize; ptNum++)
        if (pts[ptNum].x == pickpt.x && pts[ptNum].y == pickpt.y && 
            pts[ptNum].z == pickpt.z)
          break;
      if (ptNum < obj->cont[coNum].psize)
        break;
    }
    if (coNum < obj->contsize) {
      if (imodDebug('p'))
      ...
      minsq = 1.e30;
      for (tco = 0; tco < obj->contsize; tco++) {
        pts = obj->cont[tco].pts;
        for (tpt = 0; tpt < obj->cont[tco].psize; tpt++) {
          dx = pts[tpt].x - pickpt.x;
          dx *= dx;
          if (dx < minsq) {
            dy = pts[tpt].y - pickpt.y;
            dy *= dy;
            if (dy < minsq) {
              dz = pts[tpt].z - pickpt.z;
              dz *= dz;
              if (dz < minsq) {
                dsqr = dx + dy + dz;
                if (dsqr < minsq) {
    ...
    pickpt = a->imod->obj[obNum].cont[coNum].pts[ptNum];
  ...
  tpt = ctrlDown;
  if (pickedContour != indSave.contour || pickedObject != indSave.object)
    tpt = 0;
  imodSetIndex(a->imod, obNum, coNum, ptNum);
  if (!moving || imodSelectionListQuery(a->vi, obNum, coNum) < -1)
    imodSelectionNewCurPoint(a->vi, a->imod, indSave, tpt);
  if (!a->standalone)
    imod_setxyzmouse();
  else
    imodvDraw(a);
