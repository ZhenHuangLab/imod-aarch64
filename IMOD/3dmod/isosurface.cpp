...
static struct imodvIsosurfaceDataStruct iisData = 
  {0, 0, 0, 1, NO_KERNEL_SIGMA, 100, 0, IIS_CENTER_VOLUME | IIS_VIEW_BOX | 
   IIS_VIEW_USER_MODEL | IIS_VIEW_ISOSURFACE | IIS_VIEW_ISOSURFACE | IIS_LINK_XYZ};
...
void imodvIsosurfaceEditDialog(ImodvApp *a, int state)
{
  ...
  sTopWin = new DockingDialog(imodvDialogManager.parent(IMODV_DIALOG), 
                              &imodvDialogManager, "Isosurface View Controls",
                              "modvIsosurface.html#TOP", 'U', Qt::Window);

  iisData.dia = new ImodvIsosurface(a->vi, a->vi->pyrCache != NULL, sTopWin,
                                    "isosurface view");
  sTopWin->addWidgetToStack(iisData.dia);
  setModvDialogTitle(sTopWin, "Isosurface");
...
bool imodvIsosurfaceUpdate(int drawFlags)
{
  ...
  if (dia && dia->mVolume) { 
    ...
    if ((drawFlags & IMOD_DRAW_IMAGE) || isBoxChanged(dia->mBoxOrigin, dia->mBoxEnds) ||
        (dia->mNewModelOpened && Imodv->imod)) {
      if (dia->mNewModelOpened && Imodv->imod) {
        dia->mNewModelOpened = false;
        if (dia->mParamLoadConfirmed < 2)
          dia->mParamLoadConfirmed = 0;
        dia->getParametersFromStore(dia->mParamLoadConfirmed < 2, true);
        dia->modelToPaintList();
      }
      dia->setBoundingBox();
      if (iisData.flags & IIS_CENTER_VOLUME)
        dia->setViewCenter();
      dia->setBoundingObj();
      int stackIndex = dia->getCurrStackIndex();
      dia->mCurrStackIndex = stackIndex;

      // Set threshold for current stack, but first time, set threshold from data or from
      // previous stack
      if (dia->mStackThresholds[stackIndex] < 0) {
        if (dia->mMedian < 0)
          percentile = ImodPrefs->isoHighThresh() ? 1. - PERCENTILE : PERCENTILE;
        else
          percentile = dia->mHistPanel->percentileAtPoint(dia->mThreshold);
      }
      dia->mThreshold = dia->mStackThresholds[stackIndex];
      dia->mOuterLimit = dia->mStackOuterLims[stackIndex];
      dia->fillAndProcessVols(percentile);
      dia->setIsoObj(true);
      //imodvDraw(Imodv);
      return true;
    ...
    // Keep the painting object list up to date
    dia->modelToPaintList();
...
{
  if (iisData.dia && iisData.dia->mVolume)
    iisData.dia->invertThreshold();
}

void imodvIsosurfaceNewModel()
{
  if (iisData.dia && iisData.dia->mVolume)
    iisData.dia->mNewModelOpened = true;
}
...
static const char *buttonLabels[] = {"Save Object", "Sort && Save", " Fill "};
static const char *buttonTips[] = {"Save isosurfaces and bounding box as model objects",
                                   "Sort isosurfaces into separate surfaces and save as"
                                   " model object",
...
ImodvIsosurface::ImodvIsosurface(ImodView *vi, bool fillBut, DockingDialog *parent, 
                                 const char *name)
  : DialogFrame(parent, fillBut ? 3 : 2, 1, buttonLabels, buttonTips, false, 
                ImodPrefs->getRoundedStyle(), "", "", name, 0)
{
  int tempMin, tempMin2;
  mCtrlPressed = false;
  mVi = vi;
  //reserve a number;
  ...
  mMadeFiltered = false;
  mMedian = -1.;
  mParamLoadConfirmed = 1;
  mNewModelOpened = false;
  ...
  // Have to get parameters from model before setting up the UI?  Not really but why not
  for (int i = 0; i<mVi->numTimes+1; i++) {
    mStackThresholds.push_back(-1.0);
    mStackOuterLims.push_back(-1);
  }
  mOuterLimit = -1;
  mCurrStackIndex = getCurrStackIndex();
  getParametersFromStore(false, false);
  ...
  mSigmaBox = (QDoubleSpinBox *)diaLabeledSpin(1, NO_KERNEL_SIGMA, 3., 0.1,
                                               "Filter sigma:", this, hLayout, NULL, 1.6);
  mSigmaBox->setSpecialValueText("None");
  mSigmaBox->setCorrectionMode(QAbstractSpinBox::CorrectToNearestValue);
  mSigmaBox->setValue(iisData.kernelSigma);
  rightLayout->addLayout(hLayout);
  connect(mSigmaBox, SIGNAL(valueChanged(double)), this,
          SLOT(kernelSigmaChanged(double)));
  mSigmaBox->setToolTip("Set the sigma for gaussian smoothing of the image data");

  // Smoothing mesh
  hLayout = new QHBoxLayout;
  mSmoothBox = (QSpinBox *)diaLabeledSpin(0, 0., (float)MAXIMAL_ITERATION, 1.,
                                        "Smooth mesh:", this, hLayout, NULL, 1.6);
  mSmoothBox->setValue(iisData.itNum);
  rightLayout->addLayout(hLayout);
  connect(mSmoothBox, SIGNAL(valueChanged(int)), this, SLOT(iterNumChanged(int)));
  mSmoothBox->setToolTip("Set the iteration number for smoothing of the mesh vertices");

  // Deletion of small pieces and minimum size
  ...
  if (App->DevicePixelRatio > 1.) {
    tempMin = (int)(tempMin / sqrt(App->DevicePixelRatio));
    tempMin2 = (int)(tempMin2 / sqrt(App->DevicePixelRatio));
  }
  mHistPanel->setMinimumSize(tempMin, tempMin2);
  ...
  // Cap at box faces
  mCapFaces = diaCheckBox("Cap at box faces", this, rightLayout);
  mCapFaces->setChecked(iisData.flags & IIS_CLOSE_FACES);
  connect(mCapFaces, SIGNAL(toggled(bool)), this, SLOT(closeFacesToggled(bool)));
  mCapFaces->setToolTip("Close surfaces that are cut by edge of box");

  // Make multisliders for position and size
  ...
  // Set area from Zap band and Contour
  ...
  mUseRubber = diaPushButton("Rubberband", this, hLayout);
  mUseRubber->setToolTip("Show isosurfaces of area enclosed by the Zap or Slicer "
                         "rubber band");
  ...
  // Painting widgets
  mPaintLayout = new QHBoxLayout;
  leftLayout->addLayout(mPaintLayout);
  mPaintCheck = diaCheckBox("Paint objects:", this, mPaintLayout);
  ...
  mPaintCheck->setToolTip("Color parts of mesh inside spheres in objects in list");
  
  mPaintListEdit = new QLineEdit("", this);
  mPaintLayout->addWidget(mPaintListEdit);
  connect(mPaintListEdit, SIGNAL(editingFinished()), this, SLOT(paintListEditFinished()));
  mPaintListEdit->setToolTip("List (ranges allowed) of scattered point objects to "
                             "color mesh with");
  mPaintListEdit->setEnabled(iisData.flags & IIS_PAINT_OBJECT);
  modelToPaintList();

  connect(this, SIGNAL(actionClicked(int)), this, SLOT(buttonPressed(int)));
  setFontDependentWidths();

  // 1/15/18: simply set the max here so that MAX_THREADS can be higher
  mInitNThreads = numOMPthreads(4);
  ...
  if (!failed)
    fillAndProcessVols(ImodPrefs->isoHighThresh() ? 1. - PERCENTILE : PERCENTILE,
                       false, mStackThresholds[getCurrStackIndex()]);
...
/*
 * Returns the current time index numbered from 0.
 * When there is only a single image file, the time index is 0;
 * when there are multiple times the index elsewhere is numbered from 1, so subtract 1
 */
int ImodvIsosurface::getCurrStackIndex()
{
  if(mVi->numTimes == 0)
    return 0;
  else
 ...
 */
void ImodvIsosurface::fillAndProcessVols(float percentile, bool skipFill, float useThresh)
{
  float threshold;
  float *hist = mHistPanel->getHist();
  int i, value;
  unsigned char **slicePtrs;
  int nxBin = mBinBoxSize[0];
  int nyBin = mBinBoxSize[1];
  int nzBin = mBinBoxSize[2];
  mVolMin = 255;
  mVolMax = 0;
  if (imodDebug('U'))
    isoTime.start();
  ...
  if (!skipFill) {
    fillVolumeArray();
    imodTrace('U', "fill time=%d", isoTime.elapsed());
  }
  if (doDumps)
    dumpVolume("isovol1.st", false);
  ...
  if (imodDebug('U'))
    isoTime.start();
  fillBinVolume();
  imodTrace('U', "bin time=%d", isoTime.elapsed());

  if (iisData.kernelSigma > NO_KERNEL_SIGMA + 0.001) {
    if (imodDebug('U'))
      isoTime.start();
    slicePtrs = makeLinePointers(mBinVolume, nxBin * nyBin, nzBin, 1);
    if (slicePtrs) {
      volByteSmooth(slicePtrs, nxBin, nyBin, nzBin, iisData.kernelSigma, 0);
      free(slicePtrs);
      imodTrace('U', "kernel time=%d", isoTime.elapsed());
    }
  }
  ...
  for (i = 0; i < 256; i++)
    hist[i] = 0.0;
  for (int xi = 0; xi < nxBin * nyBin * nzBin; xi++) {
    value = mBinVolume[xi];
    hist[value] += 1.0;
    ACCUM_MIN(mVolMin, value);
    ACCUM_MAX(mVolMax, value);
  }
  ...
  for (i = 0; i < 256; i++) 
    hist[i] = hist[i] / (nxBin * nyBin * nzBin);
  imodTrace('U', "histogram time=%d", isoTime.elapsed());
  mHistPanel->setHistMinMax();

  // Set indicated threshold or find it from the given percentile
  if (useThresh >= 0)
    threshold = useThresh;
  else
    threshold = mHistPanel->thresholdForPercentile(percentile);

  mHistPanel->update();
  mMedian = mHistPanel->thresholdForPercentile(0.5);

  // Maintain threshold and outer limit values and sliders
  if (percentile >= 0. || useThresh >= 0.) {
    mThreshold = threshold;
    mStackThresholds[mCurrStackIndex] = mThreshold;
  }    
  ...
  setParametersInStore();

  // Finish modifications of the volume
  closeBoxFaces();
  removeOuterPixels();
  if (doDumps)
    dumpVolume("isovol2.st", true);
  doDumps = 0;
...
void ImodvIsosurface::fillBinVolume()
{
  ...
  float value;
  int zbase, ybase, vbase, numThreads;
  unsigned char *binVolume, *volume= mVolume;
  int *binBoxSize = &mBinBoxSize[0];
  int *boxSize = &mBoxSize[0];
  if (binNum == 1) {
    mBinVolume = mVolume;
    return;
  }
  ...
  binVolume= mBinVolume;
  numThreads = pow((double)mBoxSize[0] * mBoxSize[1] * mBoxSize[2], 0.333) / 100.;
  B3DCLAMP(numThreads, 1, 4);
  numThreads = numOMPthreads(numThreads);

#pragma omp parallel for num_threads(numThreads)                \
  shared(volume, binVolume, binBoxSize, boxSize, binNum, denom) \
  private(vbase, value, zbase, ybase)
  for (int zi = 0; zi < binBoxSize[2]; zi++) {
    for (int yi = 0; yi < binBoxSize[1]; yi++) {
      vbase = zi * binBoxSize[0] * binBoxSize[1] + yi * binBoxSize[0];
      for (int xi = 0; xi < binBoxSize[0]; xi++) {

         value = 0.0;
         for (int zii = 0; zii < binNum; zii++) {
           zbase = (zi * binNum + zii) * boxSize[0] * boxSize[1] + xi * binNum;
           for (int yii = 0; yii < binNum; yii++) {
             ybase = zbase + (yi * binNum + yii) * boxSize[0];
             for (int xii = 0; xii < binNum; xii++) {
               value += volume[xii + ybase];
             }
           }
         }
         binVolume[vbase + xi] = value/denom;
      }
    }
  }
}
...
float ImodvIsosurface::fillVolumeArray()
{
  ...
  // Member pointers and values are not allowed in OpenMP
  unsigned char *volume = mVolume;
  int *boxOrigin = &mBoxOrigin[0];
  int *boxSize = &mBoxSize[0];
  int *boxEnds = &mBoxEnds[0];
  int numThreads =B3DMIN(mInitNThreads, 4);
...
#pragma omp parallel for num_threads(numThreads)                             \
  shared (boxOrigin, boxEnds, boxSize, volume, ivwFastGetValue, bmap) \
  private(value, base)
  for (int zi = boxOrigin[2]; zi < boxEnds[2]; zi++) {
    for (int yi = boxOrigin[1]; yi < boxEnds[1]; yi++) {
      base = (zi - boxOrigin[2]) * boxSize[0] * boxSize[1]
        + (yi - boxOrigin[1]) * boxSize[0] - boxOrigin[0];
      for (int xi = boxOrigin[0]; xi < boxEnds[0]; xi++) {
        value = (*ivwFastGetValue)(xi, yi, zi);
        if (bmap)
          value = bmap[value];
        volume[base + xi] = value;
      }
    }
  }
  ...
  private(i, ii)
    for (i = 0; i<mNThreads; i++) {
      for (ii = 0; ii<2*validNVertex[i]; ii++) {
        ...
        finalVertex_xyz[vertStart[i]+ii].x += 0.5;
        finalVertex_xyz[vertStart[i]+ii].y += 0.5;
      }
...
void ImodvIsosurface::kernelSigmaChanged(double value)
{
  iisData.kernelSigma = value;
  float percentile = mHistPanel->percentileAtPoint(mThreshold);
  bool skipFill =  !(iisData.binningNum == 1 && 
                     ((iisData.flags & IIS_CLOSE_FACES) || iisData.kernelSigma > 0. ||
                      (mOuterLimit > mVolMin && mOuterLimit < mVolMax)));
  fillAndProcessVols(percentile, skipFill);
  setIsoObj(false);
  imodvDraw(Imodv);
}
...
void ImodvIsosurface::binningNumChanged(int binningNum)
{
  ...
  setBoundingBox();
  float percentile = mHistPanel->percentileAtPoint(mThreshold);
  bool skipFill =  !(oldBin == 1 && ((iisData.flags & IIS_CLOSE_FACES) || 
                                     iisData.kernelSigma > 0. ||
                                     (mOuterLimit > mVolMin && mOuterLimit < mVolMax)));
  fillAndProcessVols(percentile, skipFill);
  setIsoObj(false);
  setFocus();
...
/*
 * A new list is entered in the text box
 */
void ImodvIsosurface::paintListEditFinished()
{
  int numList, ind, ob;
  int *paintList;
  Iobj *obj;
  ...
  // Get the list and transfer it to the list vector
  QString str = mPaintListEdit->text();
  paintList = parselist(LATIN1(str), &numList);
  mPaintObjList.resize(numList);
  for (ind = 0; paintList && ind < numList; ind++)
    mPaintObjList[ind] = paintList[ind] - 1;
  free(paintList);
  
  // Eliminate invalid or non-scattered objects, then fix the flags of all the 
  // scattered objects in the model to match the list.  Non-scattered objects can retain
  // their flags
  managePaintObject();
  for (ob = 0; ob < Imodv->imod->objsize; ob++) {
    obj = &Imodv->imod->obj[ob];
    if (iobjScat(obj->flags)) {
      ind = numberInList(ob, &mPaintObjList[0], mPaintObjList.size(), 0);
      marked = (obj->extra[IOBJ_EX_FLAGS] & IOBJ_EXFLAG_ISO_PAINT) != 0;
      if ((ind && !marked) || (!ind && marked)) {
        setOrClearFlags(&obj->extra[IOBJ_EX_FLAGS], IOBJ_EXFLAG_ISO_PAINT, ind);
      }
    }
  }

  // Output string again and repaint
  paintListToString(str);
  diaSetEditText(mPaintListEdit, str);
  setFocus();
  fillPaintVol();
  paintMesh();
  imodvDraw(Imodv);
...
/* 
 * Check that every object on the paint list still exists and is still scattered
 */
void ImodvIsosurface::managePaintObject()
{
  int ind, ob, size = mPaintObjList.size();;
  QString str;

  // Remove non-qualifying objects
  for (ind = size - 1; ind >= 0; ind--) {
    ob = mPaintObjList[ind];
    if (ob < 0 || ob >= Imodv->imod->objsize || !iobjScat(Imodv->imod->obj[ob].flags))
      mPaintObjList.erase(mPaintObjList.begin() + ind);
  }

  // If any removed, output the new string
  if (mPaintObjList.size() < size) {
    paintListToString(str);
    mPaintListEdit->setText(str);
  }

  // Also manage the enabling of the field
  mPaintListEdit->setEnabled(iisData.flags & IIS_PAINT_OBJECT);
}

/*
 * Get a paint list from all marked and scattered objects in model and put in edit box
 */
void ImodvIsosurface::modelToPaintList()
{
  Iobj *obj;
  QString str;
  mPaintObjList.clear();
  for (int ob = 0; ob < mVi->imod->objsize; ob++) {
    obj = &mVi->imod->obj[ob];
    if (iobjScat(obj->flags) && (obj->extra[IOBJ_EX_FLAGS] & IOBJ_EXFLAG_ISO_PAINT))
      mPaintObjList.push_back(ob);
  }
  paintListToString(str);
  mPaintListEdit->setText(str);
}
...
/*
 * Convert the paint list to a string for redisplay
 */
void ImodvIsosurface::paintListToString(QString &str)
{
  std::vector<int> temp;
  if (!mPaintObjList.size()) {
    str = "";
    return;
  }
  for (int ind = 0; ind < mPaintObjList.size(); ind++)
    temp.push_back(mPaintObjList[ind] + 1);
  char *listChars = listToString(&temp[0], temp.size());
  if (listChars)
    str = listChars;
  free(listChars);
}
...
/*
 * Get isosurface parameters from the model, confirm changes if that flag is set,
 * and set changes into the dialog if indicated
 */
int ImodvIsosurface::getParametersFromStore(bool confirm, bool setControls)
{
  Istore *stParam, *stThresh;
  int ind, time, outer, first, after;
  bool foundParam = false, differs = false;
  float uintValue, fileValue = fileValueOfThreshold();
  std::vector<float> thresholds;
  ImodImageFile *image;
  
  // Look for a single PARAMS entry without index
  for (ind = ilistSize(Imodv->imod->store) - 1; ind >= 0; ind--) {
    stParam = istoreItem(Imodv->imod->store, ind);
    if (!stParam || (stParam->flags & GEN_STORE_NOINDEX) == 0)
      break;
    if (stParam->type == ISO_STORE_PARAMS) {
      foundParam = true;
      outer = -1;
      if (stParam->flags & ISO_STORE_HAS_OUTER)
        outer = stParam->index.b[ISO_STORE_OUTER_IND];
      break;
    }
  }

  // Look for THRESH entries at every existing time index
  for (time = 0; time < B3DMAX(1, mVi->numTimes); time++) {
    thresholds.push_back(-1.);
    if (mVi->numTimes)
      image = &mVi->imageList[time];
    else
      image = mVi->image;
    first = istoreLookup(Imodv->imod->store, time + (mVi->numTimes ? 1 : 0), &after);
    if (first >= 0) {
      for (ind = first; ind < after; ind++) {
        stThresh = istoreItem(Imodv->imod->store, ind);
        if (stThresh && stThresh->type == ISO_STORE_THRESH) {
          thresholds[time] = thresholdFromFileValue(image, stThresh->value.f);
          if (mStackThresholds[time] >= 0 &&
              fabs(thresholds[time] - mStackThresholds[time]) > 0.01)
            differs = true;
          break;
  ...
  if (foundParam && 
      (outer != mOuterLimit || 
       stParam->index.b[ISO_STORE_BIN_IND] != iisData.binningNum ||
       stParam->index.b[ISO_STORE_SMOOTH_IND] != iisData.itNum ||
       stParam->value.us[ISO_STORE_SIGMA_IND] != B3DNINT(10000. * iisData.kernelSigma)
       || stParam->value.us[ISO_STORE_MINSIZE_IND] != iisData.minTNum ||
       ((stParam->flags & ISO_STORE_DELETE) ?  1 : 0) != 
       ((iisData.flags & IIS_DELETE_PIECES) ? 1 : 0) ||
       ((stParam->flags & ISO_STORE_CAP) ?  1 : 0) != 
       ((iisData.flags & IIS_CLOSE_FACES) ? 1 : 0)))
    differs = true;
  if (!differs)
    return 0;

  // Confirm if anything differs and it is called for (new model)
  if (confirm) {
    mParamLoadConfirmed = dia_ask_forever
      ("The isosurface parameters stored in the model differ from the current parameters"
       "\n\nDo you want to use the values from this model?");
    if (!mParamLoadConfirmed)
      return 0;
  }

  // Set parameters here
  mOuterLimit = outer;
  iisData.binningNum = stParam->index.b[ISO_STORE_BIN_IND];
  iisData.itNum = stParam->index.b[ISO_STORE_SMOOTH_IND];
  iisData.kernelSigma = stParam->value.us[ISO_STORE_SIGMA_IND] / 10000.;
  iisData.minTNum = stParam->value.us[ISO_STORE_MINSIZE_IND];
  setOrClearFlags(&iisData.flags, IIS_DELETE_PIECES, stParam->flags & ISO_STORE_DELETE);
  setOrClearFlags(&iisData.flags, IIS_CLOSE_FACES, stParam->flags & ISO_STORE_CAP);
  for (time = 0; time < B3DMAX(1, mVi->numTimes); time++)
    if (thresholds[time] >= 0.)
      mStackThresholds[time] = thresholds[time];
  
  // Update the UI if called for (new model)
  if (setControls) {
    diaSetSpinBox(mBinningBox, iisData.binningNum);
    diaSetSpinBox(mSmoothBox, iisData.itNum);
    diaSetDoubleSpinBox(mSigmaBox, iisData.kernelSigma);
    diaSetChecked(mDeletePieces, iisData.flags & IIS_DELETE_PIECES);
    diaSetSpinBox(mPiecesBox, iisData.minTNum);
    diaSetChecked(mCapFaces, iisData.flags & IIS_CLOSE_FACES);
  }
  return 1;
}

/*
 * Store all parameters in the model store
 */
void ImodvIsosurface::setParametersInStore()
{
  int ind;
  bool replaced = false;
  Istore store, *stp;
  store.index.i = 0;
  store.type = ISO_STORE_PARAMS;
  store.flags = GEN_STORE_BYTE | (GEN_STORE_SHORT << 2) | GEN_STORE_NOINDEX |
    B3DCHOICE(iisData.flags & IIS_CLOSE_FACES, ISO_STORE_CAP, 0) |
    B3DCHOICE(iisData.flags & IIS_DELETE_PIECES, ISO_STORE_DELETE, 0);
  if (mOuterLimit >= mVolMin && mOuterLimit <= mVolMax) {
    store.flags |= ISO_STORE_HAS_OUTER;
    store.index.b[ISO_STORE_OUTER_IND] = mOuterLimit;
  }
  store.index.b[ISO_STORE_BIN_IND] = iisData.binningNum;
  store.index.b[ISO_STORE_SMOOTH_IND] = iisData.itNum;
  store.value.us[ISO_STORE_SIGMA_IND] = B3DNINT(10000. * iisData.kernelSigma);
  store.value.us[ISO_STORE_MINSIZE_IND] = iisData.minTNum;

  for (ind = ilistSize(Imodv->imod->store) - 1; ind >= 0; ind--) {
    stp = istoreItem(Imodv->imod->store, ind);
    if (!stp || (stp->flags & GEN_STORE_NOINDEX) == 0)
      break;
    if (stp->type == ISO_STORE_PARAMS) {
      *stp = store;
      replaced = true;
      break;
    }
  ...
  if (!replaced)
    istoreInsert(&Imodv->imod->store, &store);
  
  store.type = ISO_STORE_THRESH;
  store.flags = GEN_STORE_INT | (GEN_STORE_FLOAT << 2);
  store.index.i = mVi->curTime;
  store.value.f = fileValueOfThreshold();
  istoreAddOneIndexItem(&Imodv->imod->store, &store);
}

/*
 * Convert the current threshold to its value in the file for storage
 */
float ImodvIsosurface::fileValueOfThreshold()
{
  float rangeValue;
  if (mVi->ushortStore) {
    rangeValue = mThreshold * (mRangeHigh - mRangeLow) / 255. + mRangeLow;
    return rangeValue * (mVi->image->smax - mVi->image->smin) / 65535. +
      mVi->image->smin;
  }
  return mThreshold * (mVi->image->smax - mVi->image->smin) / 255. + mVi->image->smin;
}

/*
 * Retrieve a threshold in loaded, possibly windowed byte values from file value
 */
float ImodvIsosurface::thresholdFromFileValue(ImodImageFile *image, float value)
{
  float thresh, uintValue;
  if (mVi->ushortStore) {
    uintValue = 65535. * (value - image->smin) / (image->smax - image->smin);
    thresh = 255. * (uintValue - mRangeLow) / (mRangeHigh - mRangeLow);
  } else {
    thresh =  255. * (value - image->smin) / (image->smax - image->smin);
  }
  B3DCLAMP(thresh, 0., 255.);
  return thresh;
}
...
{ 
  ...
  if (!dragging || ImodPrefs->hotSliderActive(mCtrlPressed)){ 
    ...
    mCurrStackIndex = getCurrStackIndex();
    mStackThresholds[mCurrStackIndex] = mThreshold;
    mStackOuterLims[mCurrStackIndex] = mOuterLimit;
    imodvDraw(Imodv);
...
// Function for moving the threshold to complementary percentage on the other side
void ImodvIsosurface::invertThreshold()
{
  float oldPct, complement;
  oldPct = mHistPanel->percentileAtPoint(mThreshold);
  complement = mHistPanel->thresholdForPercentile(1. - oldPct);
  mThreshold = complement;
  if (mOuterLimit >= 0) {
    mOuterLimit = -1;
    fillAndProcessVols(-1.);
  } else {
...
{
  int zapIndex, slicerIndex;
  float x0, x1, y0, y1, z0, z1;
  ZapFuncs *zap = getTopZapWindow(true, false, ZAP_WINDOW_TYPE, &zapIndex);
  SlicerFuncs *slicer = getTopSlicer(true, &slicerIndex);
  if (slicerIndex >= 0 && (zapIndex < 0 || slicerIndex < zapIndex)) {
    slicer->rubberBandImageCoords(x0, x1, y0, y1, z0, z1);
  } else if (zap) {
    zapRubberbandCoords(x0, x1, y0, y1);
  } else {
    return;
  }
  if (x1 - x0 >= 8 && y1 - y0 >= 8)
...
void ImodvIsosurface::buttonPressed(int which)
{
  ...
  if (which == 2)
    ...
    if (which == 1) {
      Imesh *newMesh = imeshSortSurfaces(dup->mesh, &newSize, &error);
      if (error) {
        wprint("\aError %d splitting mesh into surfaces\n", error);
      } else {
        wprint("Split mesh into %d surfaces\n", newSize);
        imodMeshDelete(dup->mesh);
        dup->mesh = newMesh;
        dup->meshsize = newSize;
        dup->surfsize = newSize;
      }
    }
...
bool ImodvIsosurface::fillPaintVol()
{
  ...
  for (listInd = 0; (iisData.flags & IIS_PAINT_OBJECT) && listInd < mPaintObjList.size();
       listInd++) {
    obj = &Imodv->imod->obj[mPaintObjList[listInd]];
  ...
  // If there is nothing on the lists or not painting, leave, clear lists, signal if 
  // this is a change
...
{
  diaSetButtonWidth(mUseRubber, ImodPrefs->getRoundedStyle(), 1.2, "Rubberband");
  diaSetButtonWidth(mSizeContours, ImodPrefs->getRoundedStyle(), 1.2, "Contour");
  diaLimitEditStretch(mPaintListEdit, mPaintLayout, 4);
}

void ImodvIsosurface::topChangeEvent(QEvent *e)
{
...
void ImodvIsosurface::topCloseEvent ( QCloseEvent * e )
{
...
void ImodvIsosurface::dumpVolume(const char *filename, bool binned)
{
  unsigned char **data;
  MrcHeader hdata;
  FILE *fout = fopen(filename, "wb");
  int nx = binned ? mBinBoxSize[0] : mBoxSize[0];
  int ny = binned ? mBinBoxSize[1] : mBoxSize[1];
  int nz = binned ? mBinBoxSize[2] : mBoxSize[2];
  if (!fout)
    return;
  data = (unsigned char **)malloc(nz * sizeof(unsigned char *));
  if (!data)
    return;
  for (int iz = 0; iz < nz; iz++)
    data[iz] = binned ? &mBinVolume[iz *nx * ny] : &mVolume[iz * nx * ny];
  mrc_head_new(&hdata, nx, ny, nz, MRC_MODE_BYTE);
  hdata.amin = 0;
  hdata.amax = 255;
  hdata.amean = 128;
