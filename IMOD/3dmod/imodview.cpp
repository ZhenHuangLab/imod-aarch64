...
static float ivwReadBinnedPoint(ImodView *vi, ImodImageFile *image, int cx, int cy,
                                int cz);
static int getValidScale(ImodImageFile *image, float *min, float *max);
...
int ivwReopen(ImodImageFile *inFile)
{
  ...
  if (App->cvi->maxReadThreads > 1 && App->cvi->numReadThreads < 2 && 
      inFile->file == IIFILE_TIFF) {
    App->cvi->fileCopies[0] = inFile;
    App->cvi->numReadThreads = iiOpenCopiesForThreads(App->cvi->fileCopies, 
                                                      App->cvi->maxReadThreads);
  }
  if (!Imod_IFDpath.isEmpty())
...
// Close an image file, and close multiple copies if they belong to it
void ivwClose(ImodView *vi, ImodImageFile *inFile)
{
  if (vi->numReadThreads > 1 && inFile == vi->fileCopies[0]) {
    for (int ind = 1; ind < vi->numReadThreads; ind++)
      iiDelete(vi->fileCopies[ind]);
    vi->numReadThreads = 1;
  }
  iiClose(inFile);
}
...
int ivwReadBinnedSection(ImodView *vi, ImodImageFile *image, char *buf, int section)
{
  ...
  int yOffset, leftYpad = 0, rightYpad = 0, zOffset, leftZpad, rightZpad, convert;
  ...
  if (vi->rgbStore || vi->colormapImage)
    convert = MRSA_NOPROC;
  ...
  im.rawPaletteBytes = vi->colormapImage;
  if (vi->xybin * vi->zbin == 1) {
    if (vi->numReadThreads > 1 && image == vi->fileCopies[0]) {
      tiffParallelRead(vi->fileCopies, vi->numReadThreads, im.llx, im.urx, im.lly, im.ury,
                       pixsize, buf, section, convert);
    } else {
      iiReadSectionAny(&im, buf, section, convert);
    }
  } else {
    ...
    for (iz = 0; iz < vi->zbin; iz++) {
      if (vi->numReadThreads > 1 && image == vi->fileCopies[0]) {
        tiffParallelRead(vi->fileCopies, vi->numReadThreads, im.llx, im.urx, im.lly, 
                         im.ury, pixsize, (char *)unbinbuf, vi->zbin * section + iz, 
                         convert);
      } else {
        iiReadSectionAny(&im, (char *)unbinbuf, vi->zbin * section + iz, convert);
      }
...
void ivwSetTime(ImodView *vi, int time)
{
  ...
  if (vi->curTime > 0 && !vi->fakeImage)
    ivwClose(vi, &vi->imageList[vi->curTime-1]);
...
float ivwGetFileValue(ImodView *vi, int cx, int cy, int cz)
{
  ...
  if (!vi->image || cz < 0 || cz >= vi->zsize)
    return 0.0f;
  if (vi->noReadableImage || (vi->image->tiffCompression == IICOMPRESSION_EER_7BIT ||
                              vi->image->tiffCompression == IICOMPRESSION_EER_8BIT)) {
    if (!vi->rawImageStore)
 ...
 int fskip, int fox, int foy)   /* from buffer initial offsets, skip between lines. */
{
  ...
  tb += tox*psize; 
  fb += fox*psize;
  tb += ((xcpy+tskip) * (size_t)toy) * psize;
  fb += ((xcpy+fskip) * (size_t)foy) * psize;
...
// Returns true if there are gray scale image data
bool ivwGrayScaleImageLoaded(ImodView *inImodView)
{
  return !inImodView->fakeImage && !inImodView->rgbStore;
}
  ...
  while((imodFgetline(vi->fp, line, IFDLINE_SIZE) > 0)) {
    ...
    if (!strncmp("IMAGE", line, 5)) {
      ...
      if (((image->file == IIFILE_RAW && !image->amin && !image->amax) ||
           !getValidScale(image, NULL, NULL)) && smin >= smax)
        iiRawScan(image);
      if (smin < smax) {
        image->smin = smin;
        image->smax = smax;
                      ...
                      bool &anyHavePieceList)
{
  ...
  for (i = firstfile; i <= lastimage; i++) {
    ...
    for (indVol = 0; indVol < numVols; indVol++) {
      ...
      if (((image->file == IIFILE_RAW && !image->amin && !image->amax) || 
           !getValidScale(image, NULL, NULL)) && iiRawScan(image)) {
        str.sprintf("3DMOD Error: Scanning for scaling limits in %s.\n", argv[i]);
        str = QString(b3dGetError()) + str;
        imodError(NULL, LATIN1(str));
        exit(3);
      }
...
static int ivwProcessImageList(ImodView *vi)
{
  ...
  MrcHeader *header;
  FILE *fp;
  int xsize, ysize, zsize, i, midy, midz, tiffCompression = IICOMPRESSION_NONE;
  float naysum, zratio, mratio, smin, smax;
  int rgbs = 0, cmaps = 0, allByte = 1, allCanReadInt = 1;
  bool anyTiled = false, anyTiffs = false;
  ...
  for (i = 0; i < ilist->size; i++) {
    ...
    if (getValidScale(image, NULL, NULL) > 1 && ImodPrefs->loadIntIfMeanSD())
      vi->switchToUshort = 1;
    
    if ((image->type != IITYPE_UBYTE && image->type != IITYPE_BYTE) ||
    ...
    if (image->format == IIFORMAT_RGB && 
        !((image->file == IIFILE_MRC  || image->file == IIFILE_RAW || 
           image->file == IIFILE_HDF || image->file == IIFILE_TIFF || 
           image->file == IIFILE_JPEG || image->file == IIFILE_QIMAGE) && vi->grayRGBs))
      rgbs++;
    if (image->format == IIFORMAT_COLORMAP && !vi->grayRGBs)
      cmaps++;
    ...
    if (image->file == IIFILE_TIFF) {
      ...
      if (image->tileSizeX)
        anyTiled = true;
      if (image->tiffCompression != IICOMPRESSION_NONE)
  ...
  if (vi->switchToUshort && !vi->rawImageStore) {
  ...
  if (vi->rawImageStore == MRC_MODE_USHORT && !allCanReadInt) {
    if (vi->switchToUshort > 1 || !vi->intOptEntered) {
      vi->rawImageStore = 0;
    } else {
  ...
  for (i = 0; i < ilist->size; i++) {
    ...
    if (vi->li->smin == vi->li->smax) {
      getValidScale(image, &smin, &smax);
      iiSetMM(image, smin, smax, vi->ushortStore ? 65535. : 255.);
    } else {
  ...
  if (!vi->li->plist) {
    ...
    if (anyTiffs && !vi->imagePyramid && !(vi->stripOrTileCache && anyTiled))
      vi->maxReadThreads = tiffNumReadThreads(vi->li->xmax + 1 - vi->li->xmin,
                                                vi->li->ymax + 1 - vi->li->ymin,
                                                tiffCompression, MAX_READ_THREADS);
...
/*
 * Returns 1 if the image has is a valid scaling from min/max, 2 if from mean/SD, or
 * 0 if none, and returns the min and max limits in min and max if they are not NULL
 */
static int getValidScale(ImodImageFile *image, float *min, float *max)
{
  // The min and max are valid if they are not crossed and not both zero
  bool minMaxValid = image->amax >= image->amin && (image->amax != 0 || image->amin != 0);
  bool RMSvalid = image->rms > 0.;
  bool meanSDvalid;
  float minLim, maxLim, dummy;

  // The RMS is valid if it is positive or if 0 is supposed to be valid due to flags
  if (image->file == IIFILE_MRC && image->rms == 0.) {
    MrcHeader *hdata = (MrcHeader *)image->header;
    RMSvalid = mrcGetStandardVersion(hdata) > 0 ||
      (hdata->imodStamp == IMOD_MRC_STAMP && (hdata->imodFlags | MRC_FLAGS_BAD_RMS_NEG));
  }

  // The mean is valid if it is bigger than the min of min and max, and not bigger than
  // a valid max, and something is non-zero; mean & SD valid if RMS is valid too
  meanSDvalid = image->amean >= B3DMIN(image->amin, image->amax) &&
    !(image->amean > image->amax + 1.001 * (image->amax - image->amin) &&
      minMaxValid) && (image->amin != 0. || image->amax != 0. || image->amean != 0.) &&
    RMSvalid;
  
  imodTrace('r', "mmvalid %d  meanvalid %d  rmsvalid %d", minMaxValid?1:0, 
            meanSDvalid?1:0, RMSvalid?1:0);
  // Return mean/SD based value if no valid min/max or pref is to use mean/SD
  if (meanSDvalid && (!minMaxValid || ImodPrefs->preferMeanSD())) {
    if (!min || !max)
      return 2;
    *min = image->amean - ImodPrefs->numSDsForScaling() * image->rms;
    *max = image->amean + ImodPrefs->numSDsForScaling() * image->rms;
    
    // Limit the values for integer modes
    if (image->type != IITYPE_FLOAT && 
        !iiDefaultMinMaxMean(image->type == IITYPE_BYTE ? IITYPE_UBYTE : image->type,
                             &minLim, &maxLim, &dummy)) {
      *min = B3DMAX(*min, minLim);
      *max = B3DMIN(*max, maxLim);
    }
    imodTrace('r', "mean %f  rms %f  return mni/max %f %f", image->amean, image->rms,
              *min, *max);
    return 2;
  }
  ...
  if (minMaxValid) {
    ...
    *min = image->amin;
    *max = image->amax;
    imodTrace('r', "return min/max %f  %f", image->amin, image->amax);
    return 1;
  }
  return 0;
}
...
float ivwGetTopZapDevPixelRatio(ImodView *inImodView)
{
  ZapFuncs *zap = getTopZapWindow(false);
  return zap ? zap->mDevicePixelRatio : App->DevicePixelRatio;
}
