...
// Convert the mouse position to image position
int ivwGetTopSlicerMouse(ImodView *inImodView, Ipoint *imagePt)
{
  int ex, ey;
  SlicerFuncs *ss = getTopSlicer();
  if (!ss)
    return 1;
  XY_PIXEL_TO_DEVICE(ss->m, (ss->mGlw->mapFromGlobal(QCursor::pos())).x(), 
                     (ss->mGlw->mapFromGlobal(QCursor::pos())).y(), ex, ey);
  ss->getxyz(ex, ey, imagePt->x, imagePt->y, imagePt->z);
  return 0;
}
...
void setupLinkedSlicers(ImodView *vi)
{
  ...
  if (slicerOpen(vi, 1))
    return;
  diaMaximumWindowSize(deskWidth, deskHeight, ImodInfoWin);
  deskWidth -= 20;
  ...
  for (i = 1; i <= B3DMIN(numInX * numInY, vi->numTimes); i++) {
    ...
    zooms.push_back(slicer->mFuncs->mZoom);
  }

  // Make sure they all have the same zoom; favor the larger zoom if the median is halfway
  // between two values
  rsFastMedianInPlace(&zooms[0], zooms.size(), &medianZoom);
  bestZoom = zooms[0];
  for (i = 1; i < zooms.size(); i++) 
    if (fabs(zooms[i] - medianZoom) < fabs(bestZoom - medianZoom) ||
        (bestZoom < medianZoom && zooms[i] > medianZoom &&
         zooms[i] - medianZoom < medianZoom - bestZoom + 0.01))
      bestZoom = zooms[i];

  for (i = 0; i < zooms.size(); i++) {
    slicer = (SlicerWindow *)objList.at(objList.count() - zooms.size() + i);
    if (fabs(slicer->mFuncs->mZoom - bestZoom) > 0.01) {
      slicer->mFuncs->mZoom = bestZoom;
      slicer->mFuncs->draw();
    }
  };
...
SlicerFuncs::SlicerFuncs(ImodView *vi, int autoLink)
{
  ...
  // App->devPixVaries, but is needed unconditionally when opening with -S
  mQtWindow->show();
  ...
  XY_PIXEL_TO_DEVICE(m, newWidth, newHeight, devWidth, devHeight);
  if (vi->xsize < devWidth && vi->ysize < newWidth) {
    mZoom = (2. * devWidth) / vi->xsize;
    while ((mZoom * vi->xsize > 1.06 * devWidth || 
           mZoom * vi->ysize > 1.06 * devWidth)) {
  ...
  // Windows (dual monitor?) needed to reassert the size
  // 7/19/19: With Qt 5 RHEL7, the second event processing is needed when opening with -S
  imod_info_input();
  mQtWindow->resize(newWidth, newHeight);
  imod_info_input();
  
  setMouseTracking();
#if defined(Q_OS_MACX) && QT_VERSION >= 0x050700
  if (!sSliceAngDia) {
    imod_info_input();
    setAngleToolbarState(mQtWindow, true);
    setAngleToolbarState(mQtWindow, false);
  }
#endif
}
...
void SlicerFuncs::setInitialZoom(float zoom)
{
  mZoom = zoom;
  mQtWindow->setZoomText(mZoom);
  if (mZoom > 1.5 && !mVi->colormapImage) {
    mHq = 1;
...
void SlicerFuncs::keyInput(QKeyEvent *event)
{
  ...
  if (utilCloseKey(event) && mGlw->mFirstDraw <= 0) {
    mClosing = 1;
    imod_info_input();
    mQtWindow->close();
    return;
  ...
  if (imodPlugHandleKey(vi, event, SLICER_WINDOW_TYPE)) 
    return;
  ...
  case Qt::Key_Equal:
  case Qt::Key_Plus:
    if ((keypad && shift) || (!keypad && keysym == Qt::Key_Plus)) {
      mNslice++;
      drawThickControls();
    } else {
  ...
  case Qt::Key_Minus:
    if ((keypad && shift) || keysym == Qt::Key_Underscore) {
      mNslice = B3DMAX(1, mNslice - 1);
      drawThickControls();
    } else {
      mZoom = b3dStepPixelZoom(mZoom, -1);
      mQtWindow->setZoomText(mZoom);
  ...
  case Qt::Key_PageUp:
    ...
    if (!keypad) {
      unit = mClassic ? -1. : +1.;
      multiplier = ctrl ? 0.2 : 1.;
      if (keysym == Qt::Key_Down) 
        vec.y = unit * multiplier;
      if (keysym == Qt::Key_Left)
        vec.x = unit * multiplier;
  ...
  case Qt::Key_F:
    if (shift) {
      mFftMode = 1 - mFftMode;
      mQtWindow->setToggleState(SLICER_TOGGLE_FFT, mFftMode);
    } else
      handled = 0;
    break;
...
void SlicerFuncs::screenChanged(float newDPR)
{
  if (newDPR) {
    if (utilSetZoomOnScreenChange(0., mLastXsizeChange, mScreenChanged, mScreenResizeTime,
                                  mZoom))
      mQtWindow->setZoomText(mZoom);
...
void SlicerFuncs::mousePress(QMouseEvent *event)
{
  ...
  sLastMouseX = sFirstMouseX = ex;
  sLastMouseY = sFirstMouseY = ey;
  ...
  if (!mDrawingArrow)
    ifdraw = checkPlugUseMouse(event, ex, ey, button1, button2, button3);
  if (ifdraw & 1)
    return;

  if (button1) {
    if (mClassic || mStartingBand || mDrawingArrow) {
      attachPoint(ex, ey, ctrl);
    } else {
      sBut1downt.start();
      mFirstDrag = 1;
      if (mRubberband)
  ...
  if (button2 && (ctrl || !shift))
    insertPoint(ex, ey, ctrl);
  
  if (button3 && (ctrl || !shift))
    modifyPoint(ex, ey, ctrl);
}
...
{
  int ex, ey;
  int button1, button2, button3, ifdraw = 0;
  XY_PIXEL_TO_DEVICE(m, event->x(), event->y(), ex, ey);
  ...
  needDraw = mDrewExtraCursor && !mGlw->extraCursorInWindow();
  if (!(button1 && (mDragBand || mDrawingArrow)))
      ifdraw = checkPlugUseMouse(event, ex, ey, button1, button2, button3);
  if (ifdraw & 1) {
  ...
  // For button 1 up, if not classic mode, either end panning or call attach
    if (button1 && !(ifdraw & 1) && (!mClassic || mDrawingArrow || mDragBand)) {
    if (mDragBand || mDrawingArrow) {
      ...
      attachPoint(ex, ey, event->modifiers() & Qt::ControlModifier);
...
void SlicerFuncs::mouseMove(QMouseEvent *event)
{
  ...
  if (!(mRubberband && (mMoveBand || mDragBand || (button1 && mFirstDrag)) ||
        mDrawingArrow) && (sMousePressed || mVi->trackMouseForPlugs)) {
    ifdraw = checkPlugUseMouse(event, ex, ey, button1, button2, button3);
    if (ifdraw & 1)
      return;
  }
  ...
  if (addingPoints) {
    ...
    if (imodPoint3DScaleDistance
        ...
        scaleModelRes(imod->res, mZoom / mDevicePixelRatio)) {
      mIgnoreCurPtChg = 1;
...
// Pass on mouse event to plugins
int SlicerFuncs::checkPlugUseMouse(QMouseEvent *event, int ex, int ey, int but1, int but2,
                                   int but3)
{
  float xm, ym, zm;
  int ifdraw;
  getxyz(ex, ey, xm, ym, zm);
  ifdraw = imodPlugHandleMouse(mVi, event, xm, ym, but1, but2, but3, SLICER_WINDOW_TYPE);
  if (ifdraw & 2)
    draw();
  return ifdraw;
}

// Pass on general event to plugins, handle draw when requested or mouse leaves window
void SlicerFuncs::generalEvent(QEvent *e)
{
  int ix, iy, ifdraw;
  float imx, imy, imz;
  if (App->closing)
  ...
  XY_PIXEL_TO_DEVICE(m, (mGlw->mapFromGlobal(QCursor::pos())).x(), 
                     (mGlw->mapFromGlobal(QCursor::pos())).y(), ix, iy);
  getxyz(ix, iy, imx, imy, imz);

  ifdraw = imodPlugHandleEvent(mVi, e, imx, imy, SLICER_WINDOW_TYPE);
...
void SlicerFuncs::setCursor(int mode, bool setAnyway)
{
  ...
  //imodPrintStderr("slicer mode %d sa %d special %d sizeall %d sb %d mb %d\n", mode, setAnyway?1:0, needSpecial?1:0, needSizeAll?1:0, mStartingBand?1:0, mMoveBand?1:0);
  utilSetCursor(mode, setAnyway, needSpecial, needSizeAll, mDragging, needModel, 
                mMousemode, mLastShape, mGlw);
}

void SlicerFuncs::setMouseTracking()
{
  mGlw->setMouseTracking(mRubberband || sPixelViewOpen || mVi->trackMouseForPlugs);
}
...
/*
 * Return the rubber band coordinates in proper order, lower then higher
 */
void SlicerFuncs::rubberBandImageCoords(float &x0, float &x1, float &y0, float &y1,
                                        float &z0, float &z1)
{ 
  x0 = B3DMIN(mRbImageX0, mRbImageX1);
  x1 = B3DMAX(mRbImageX0, mRbImageX1);
  y0 = B3DMIN(mRbImageY0, mRbImageY1);
  y1 = B3DMAX(mRbImageY0, mRbImageY1);
  z0 = B3DMIN(mRbImageZ0, mRbImageZ1);
  z1 = B3DMAX(mRbImageZ0, mRbImageZ1);
}
/*
  ...
  *limits = NULL;
  if (mRubberband) {
    *limits = limarr;
    ...
    limarr[2] = mRbMouseX1 - (mDevicePixelRatio > 1. ? 2 : 1);
    limarr[3] = mWiny - mRbMouseY0 - (mDevicePixelRatio > 1. ? 3 : 2);
...
void SlicerFuncs::resize(int winx, int winy)
{
  if (mClosing)
    return;
  if (mWinx > 1) {
    if (utilSetZoomOnScreenChange((float)winx / mWinx, mLastXsizeChange, mScreenChanged,
  ...
  mImage   = b3dGetNewCIImageSize(mImage, App->depth, winx, winy);
  manageBuffers();

  ivwControlPriority(mVi, mCtrl);
}

int SlicerFuncs::manageBuffers()
{
  int needSize = mWinx * mWiny;
  if (mHq && mZoom < 1.)
    needSize = (int)((ceil(mWinx / mZoom) + 1.) * (ceil(mWiny / mZoom) + 1.));
  needSize = B3DMIN(needSize, ImodPrefs->maxSlicerBufMB() * 1024 * 256);
  if (needSize <= mCurBufSize && needSize > 0.8 * mCurBufSize)
    return 0;
  mCurBufSize = needSize;
  B3DFREE(mSliceBuf);
  mSliceBuf = B3DMALLOC(int, needSize);

  // Allocate buffers for doing RGB 3 times and saving first two channels
  if (mVi->rgbStore) {
    B3DFREE(mRedTemp);
    B3DFREE(mGreenTemp);
    mRedTemp = B3DMALLOC(unsigned char, needSize);
    mGreenTemp = B3DMALLOC(unsigned char, needSize);
  }
  if (!mImage || !mSliceBuf || (mVi->rgbStore && (!mRedTemp || !mGreenTemp))) {
    B3DFREE(mSliceBuf);
    B3DFREE(mRedTemp);
    B3DFREE(mGreenTemp);
    b3dFreeCIImage(mImage);
...
void SlicerFuncs::draw()
{
  ...
  if (sPixelViewOpen) {
    mx = mGlw->mapFromGlobal(QCursor::pos()).x();
    my = mGlw->mapFromGlobal(QCursor::pos()).y();
    if (mx >= 0 && mx < mWinx && my >= 0 && my < mWiny) {
      zmouse = getxyz(mx, my, imx, imy, imz);
      pvNewMousePosition(mVi, imx, imy, zmouse);
    }
  }
}
...
void SlicerFuncs::paint()
{
  ...
  unsigned char *bdataIn = (unsigned char *)mSliceBuf;
  ...
  if (mImageFilled <= 0) {
    ...
    if (!mousing && !mVi->colormapImage && mNslice > sliceScaleThresh && !mVi->rgbStore) {
      if (!mUseLastScale || mFftMode)
        fillImageArray(0, 1, 0);
    } else if (!mousing)
      mScaleToMeanSD = false;
    fillImageArray(mousing, 0, 0);
    if (mVi->rgbStore) {
      memcpy(mRedTemp, mSliceBuf, mBufXsize * mBufYsize);
      fillImageArray(mousing, 0, 1);
      memcpy(mGreenTemp, mSliceBuf, mBufXsize * mBufYsize);
      fillImageArray(mousing, 0, 2);
      bdataOut = bdataIn + 3 * (mBufXsize * mBufYsize) - 1;
      for (i = mBufXsize * mBufYsize - 1; i >= 0; i--) {
        *bdataOut-- = bdataIn[i];
        *bdataOut-- = mGreenTemp[i];
        *bdataOut-- = mRedTemp[i];
      }
    }
  ...
  drawZoom = mNoPixelZoom ? mRemainingZoom : mZoom;
  b3dDrawGreyScalePixelsHQ(ivwMakeLinePointers(mVi, (unsigned char *)mSliceBuf, mBufXsize,
                                               mBufYsize, mVi->rawImageStore), 
                           mBufXsize, mBufYsize, 0, 0, (int)mXshift, (int)mYshift, 
                           mBufXsize, mBufYsize, mImage, mVi->rampbase, drawZoom, 
                           drawZoom, mRemainingZoom < 1. ? 1 : 0, -1, App->rgba);


  drawModel();
  drawCurrentPoint();

  mScaleBarSize = scaleBarDraw(mWinx, mWiny, mZoom, 0, mGlw, mDevicePixelRatio);
...
void SlicerFuncs::drawCurrentPoint()
{
  ...
  if (mMousemode == IMOD_MMOVIE || !pnt || !cont) {
    ...
    b3dDrawCircle(xim, yim, curSize, true);
  }
  ...
  if (cont && !ivwTimeMismatch(vi, mTimeLock, obj, cont) && 
    ...
    if (fabs((double)delta) < 0.5 * mDepth) {
      ...
      getWindowCoords(pnt->x, pnt->y, pnt->z, xim, yim, zim);
      b3dDrawCircle(xim, yim, modPtSize, true);
    }
...
{
  int loop, numLoop = mVi->numExtraObj > 0 ? 2 : 1;
  float depth = mDepth;
  Ipoint scale;
  setInverseMatrix();
  scale.x = 1. / mZoom;
  scale.y = 1. / mZoom;
  scale.z = 1. / (mZoom * mVi->imod->zscale);
  imodMatScale(mMat, &scale);
  ...
  for (loop = 0; loop < numLoop; loop++) {
    ...
    if (!loop) {
      imodDrawSetupLabelDraw(mMat, mWiny, mDevicePixelRatio, 
                             mQtWindow->fontMetrics().height(), mGlw);
      imodDrawModel(mVi, mVi->imod, 0, getZScaleBefore());
      imodDrawSetupLabelDraw(mMat, mWiny, mDevicePixelRatio, 
                             mQtWindow->fontMetrics().height(), mGlw);
    }

    mDrewExtraCursor = imodDrawExtraObjects(mVi, mVi->imod, getZScaleBefore(), 
                                            loop * (mGlw->extraCursorInWindow()? 2 : 1));
    glPopMatrix();
    depth = 100. * B3DMAX(mWiny, mWiny);
  }
}
