...
static PopupEntry sPopupTable[] = {
...
#endif
  {"Display Controls (with Object Edit open)", -1, 0, 0, 0},
  {"Change to previous object", Qt::Key_O, 1, 0, 0},
  {"Change to next object", Qt::Key_P, 1, 0, 0},
  {"Switch to Contour - Lines",  Qt::Key_F1, 0, 0, 0},
  {"Switch to Mesh - Fill",  Qt::Key_F2, 0, 0, 0},
  {"Switch to Mesh - Lines",  Qt::Key_F3, 0, 0, 0},
  {"Cycle between 3 display types",  Qt::Key_F4, 0, 0, 0},
  {"Mesh current object", Qt::Key_M, 1, 0, 0},
  ...
  {"Open Movie Sequences window",  Qt::Key_N, 0, 1, 0},
  ...
  {"Previous scattered point", Qt::Key_BraceLeft, 1, 0, 0},
  {"Previous scattered point", Qt::Key_ParenLeft, 1, 0, 0},
  {"Next scattered point", Qt::Key_BraceRight, 1, 0, 0},
  {"Next scattered point", Qt::Key_ParenRight, 1, 0, 0},
  {"Previous scattered contour", Qt::Key_X, 1, 1, 0},
  {"Next scattered contour", Qt::Key_C, 1, 1, 0},
  {"Select visible contours in current object", Qt::Key_A, 1, 0, 0},
  {"Delete selected contour(s) picked here", Qt::Key_D, 0, 0, 0},
  {"Apply last fine-grained change to current selection", Qt::Key_F, 1, 0, 0},
  {"Undo last change to model", Qt::Key_Z, 1, 0, 0},
  {"Redo the last undone change to model", Qt::Key_Y, 1, 0, 0},
  {"Clipping Planes (with Object Edit open)", -1, 0, 0, 0},
  {"Toggle current clipping plane on and off", Qt::Key_C, 1, 0, 0},
  {"Toggle drawing of current clipping plane", Qt::Key_V, 1, 0, 0},
#ifdef Q_OS_MACX
  {"Toggle plane 1 of current object      Ctrl+1", Qt::Key_1, 1, 0, -1},
  {"Toggle plane 2 of current object      Ctrl+2", Qt::Key_2, 1, 0, -1},
  {"Toggle global plane 1                     Ctrl+3", Qt::Key_3, 1, 0, -1},
  {"Toggle global plane 2                     Ctrl+4", Qt::Key_4, 1, 0, -1},
#else
  {"Toggle plane 1 of current object", Qt::Key_1, 1, 0, 0},
  {"Toggle plane 2 of current object", Qt::Key_2, 1, 0, 0},
  {"Toggle global plane 1", Qt::Key_3, 1, 0, 0},
  {"Toggle global plane 2", Qt::Key_4, 1, 0, 0},
#endif
  {"", -2, 0, 0, 0},
  {"", -3, 0, 0, 0},
  ...
  : QMainWindow(parent, f)
{
  ...
  ADD_ACTION(edit, "Save Doc&k", VEDIT_MENU_SAVE_DOCK);
  ADD_ACTION(edit, "Reopen &Dock", VEDIT_MENU_REOPEN_DOCK);
  mActions[VEDIT_MENU_IMAGE]->setEnabled(!Imodv->standalone && !Imodv->vi->fakeImage);
  ...
  ADD_ACTION(view, "&Point Labels", VVIEW_MENU_LABELS);
  mActions[VVIEW_MENU_LABELS]->setCheckable(true);
  mActions[VVIEW_MENU_LABELS]->setChecked(a->drawLabels);
...
#if defined(Q_OS_MACX) && QT_VERSION >= 0x040500 && QT_VERSION < 0x050400
  ...
  // Open tool if it closed with window before, but it crashes with the startup needed
  // for different DPI monitors
  if (sRToolWithImodv > 0 && !App->devPixVaries)
    openRotationTool(a);
}
...
void ImodvWindow::screenChanged(QScreen *screen)
{
  float newDPR = utilGetNewDevPixRatio(this);
  if (newDPR) {
    imodvDialogManager.masterChangedScreen(mDevicePixelRatio, newDPR);
    HANDLE_DPR_CHANGE_FOR_MAC(fakeResize, Imodv->winx, Imodv->winy);
  }
}
#endif

void ImodvWindow::fakeResize(int width, int height)
{
  imodvResizeGL(Imodv->mainWin->mCurGLw, width, height);
}
...
void ImodvWindow::resizeEvent(QResizeEvent *e)
{
  imodvDialogManager.masterWinHasChanged(IMODV_DIALOG);
}

void ImodvWindow::moveEvent(QMoveEvent *e)
{
  imodvDialogManager.masterWinHasChanged(IMODV_DIALOG);
}

void ImodvWindow::changeEvent(QEvent *e)
{
  QMainWindow::changeEvent(e);
  if (e->type() == QEvent::ActivationChange && isActiveWindow())
    imodvDialogManager.dockerWasActivated(NULL);
}
...
void ImodvWindow::newResizerSize(int sizeX, int sizeY)
{
  ...
  XY_DEVICE_TO_PIXEL(m, sizeX, sizeY - a->winy, sizeX, sizeY);
  a->mainWin->resize(sizeX, sizeY + a->mainWin->height());
}
...
{
  ImodvWindow *win = Imodv->mainWin;
  if (mFirstDraw >= 3) {
    mTimerID = startTimer(10);
    mFirstDraw--;
  }
  if (!win->mInitWidth && !win->mInitHeight)
    return;

  b3dSetCurDevPixRatio(win->mDevicePixelRatio);
  imodvPaintGL();
}

// Timer for linux Qt 5 redraws, needed when opening with 3dmod -V at least
void ImodvGL::timerEvent(QTimerEvent * e )
{
  ImodvWindow *win = Imodv->mainWin;
  if (!win->mInitWidth && !win->mInitHeight)
    return;
...
#if defined(Q_OS_LINUX) && QT_VERSION >= 0x050000
  if (mFirstDraw > 0) {
    win->resize(win->mInitWidth + mFirstDraw - 1, win->mInitHeight);
  } else
#endif
    updateGL();
