...
void mvImageEditDialog(ImodvApp *a, int state)
{
  ...
  sTopWin = new DockingDialog(imodvDialogManager.parent(IMODV_DIALOG), 
                              &imodvDialogManager, "Image In Model View",
                              "modvImage.html#TOP", 'I', Qt::Window);
  
  sDia = new ImodvImage(sTopWin, a->vi->pyrCache != NULL, "image view");
  sTopWin->addWidgetToStack(sDia);
  setModvDialogTitle(sTopWin, "MV Image");
...
// Viewing image plane is turned on or off
...
{
  mvImageSetPlaneFlag((sFlags & flag) == 0, flag);
}
...
#define FILLRGB(a)  uvind = 4 * (fillXsize * v + u); \
  sTdata[uvind] = sCmap[0][*(a + 2)];                                \
  sTdata[uvind + 1] = sCmap[1][*(a + 1)];                            \
  sTdata[uvind + 2] = sCmap[2][*a];                                  \
  sTdata[uvind + 3] = 255;
              ...
              if (imdata[i]) {
                ...
                if (a->vi->rgbStore) {
                  for (j = zpatch-1; j < jend+1; j++) {
                    v = j - (zpatch-1);
                    rgbPtr = &imdata[i][3 * (xconst - j * xsize)];
                    FILLRGB(rgbPtr);
                  }
                } else if (a->vi->ushortStore) {
                  for (j = zpatch-1; j < jend+1; j++) {
            ...
            for (j = zpatch-1; j < jend+1; j++) {
              ...
              if (imdata[j]) {
                if (a->vi->rgbStore) {
                  for (i = ypatch-1; i < iend+1; i++) {
                    u = i - (ypatch-1);
                    rgbPtr = &imdata[j][3 * (xpatch + (i * xsize))];
                    FILLRGB(rgbPtr);
                  }
                } else if (a->vi->ushortStore) {
                  for (i = ypatch-1; i < iend+1; i++) {
            ...
            for (j = zpatch-1; j < jend+1; j++) {
              ...
              if (flipped && imdata[ypatch]) {
                ...
                if (a->vi->rgbStore) {
                  for (i = xpatch-1; i < iend+1; i++) {
                    u = i - (xpatch-1);
                    rgbPtr = &imdata[ypatch][3 * (i + xconst)];
                    FILLRGB(rgbPtr);
                  }
                } else if (a->vi->ushortStore) {
                  for (i = xpatch-1; i < iend+1; i++) {
                ...
                if (a->vi->rgbStore) {
                  for (i = xpatch-1; i < iend+1; i++) {
                    u = i - (xpatch-1);
                    rgbPtr = &imdata[j][3 * (i + (ypatch * xsize))];
                    FILLRGB(rgbPtr);
                  }
                } else if (a->vi->ushortStore) {
                  for (i = xpatch-1; i < iend+1; i++) {
...
static const char *buttonLabels[] = {"Fill"};
static const char *buttonTips[] = {"Fill cache for current display zoom and position"};
static const char *sliderLabels[] = {"X", "Y", "Z", "X size", "Y size", "Z size",
                               "# of slices", "Transparency", 
                               "Black Level", "White Level"};

ImodvImage::ImodvImage(DockingDialog *parent, bool fillBut, const char *name)
  : DialogFrame(parent, fillBut ? 1 : 0, 1, buttonLabels, buttonTips, true, 
                ImodPrefs->getRoundedStyle(), "", "", name, 0)
{
  ...
  mApplyClipBox = diaCheckBox("Apply global clip planes", this, mLayout);
  mApplyClipBox->setChecked(Imodv->imod->view->world & VIEW_WORLD_CLIP_IMAGE);
  connect(mApplyClipBox, SIGNAL(toggled(bool)), this, SLOT(applyClipToggled(bool)));
  mApplyClipBox->setToolTip("Clip image display with global clipping planes if any "
                            "are turned on");
...
void ImodvImage::applyClipToggled(bool state)
{
  setOrClearFlags(&Imodv->imod->view->world, VIEW_WORLD_CLIP_IMAGE, state ? 1 : 0);
  imodvDraw(Imodv);
}
...
void ImodvImage::topChangeEvent(QEvent *e)
{
...
void ImodvImage::topCloseEvent ( QCloseEvent * e )
{
