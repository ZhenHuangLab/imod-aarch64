  ...
  : QMainWindow(parent, f)
{
  ...
  mDevicePixelRatio = 0.;
  mTargetMoveX = mTargetMoveY = -30000;
  ...
  // Try to avoid "Start dictation" and special character entries
  ...
  ADD_ACTION(edit, "Save Doc&k", EDIT_MENU_SAVE_DOCK);
  ADD_ACTION(edit, "Reopen &Dock", EDIT_MENU_REOPEN_DOCK);
  ...
  ADD_ACTION(eObject, "&Sort by Dist", EOBJECT_MENU_SORTDIST);
  ...
  ADD_ACTION(eSurface, "&Sort", ESURFACE_MENU_SORT);
  ...
  ADD_ACTION_KEY(eImage, "P&rocess...", EIMAGE_MENU_PROCESS, Qt::CTRL + Qt::Key_P);
  ...
  mOldFontHeight = fontMetrics().lineSpacing();
...
void InfoWindow::setInitialHeights(bool forScreenChange)
{
  imod_info_input();
  int editHeight = mStatusEdit->height();
  int newEdit = B3DNINT(INFO_STARTING_LINES * mOldFontHeight);
  ...
  mInfoTimerID = startTimer(forScreenChange ? 250 : 10);
...
void InfoWindow::resizeToHeight(int newHeight, int trimWidth)
{
  int i;
  if (mInfoTimerID) {
    ...
    for (i = 0; i < 5; i++) {
      resize(100, newHeight);
      imod_info_input();
      if (height() == newHeight && i >= trimWidth)
        break;
    }
    if (Imod_debug && i > trimWidth)
...
void InfoWindow::changeEvent(QEvent *e)
{
  if (App->closing)
    return;
  int oldHeight = mStatusEdit->height();
  int nlines = B3DNINT((float)oldHeight / mOldFontHeight);
  QMainWindow::changeEvent(e);
  if (e->type() == QEvent::ActivationChange && isActiveWindow())
    imodDialogManager.dockerWasActivated(NULL);
  ...
  mOldFontHeight = fontMetrics().lineSpacing();
  int editDelta = B3DNINT((float)nlines * mOldFontHeight) - oldHeight;
  mStatusEdit->setMinimumHeight(B3DNINT(INFO_MIN_LINES * mOldFontHeight));
  //ImodInfoWidget->setMinimumHeight(widgetHi);
  imodTrace('i', "Font change resize, new minhi %d nlines %d   oldstat %d del %d", 
            widgetHi, nlines, oldHeight, editDelta);
  resizeToHeight(height() + (widgetHi - ImodInfoWidget->height()) + editDelta, 4);
}
...
void InfoWindow::screenChanged(QScreen *screen)
{
  float newDPR = utilGetNewDevPixRatio(this);
  if (newDPR) {
    imodDialogManager.masterChangedScreen(mDevicePixelRatio, newDPR);
    if (fabs(newDPR - mDevicePixelRatio) > 0.01) {
      mOldFontHeight = fontMetrics().lineSpacing();
      if (mInfoTimerID)
        extendInfoTimer();
      else
        setInitialHeights(true);

    }
    mDevicePixelRatio = newDPR;
  }
}
#endif
...
// Notify the dialog manager of resizes and moves for it to manage docked stack
void InfoWindow::resizeEvent(QResizeEvent *e)
{
  if (!App->infoInitialSetup) {
    //imodPrintStderr("info resize %d %d\n", e->size().width(), e->size().height());
    imodDialogManager.masterWinHasChanged(IMOD_DIALOG);
  }
}

void InfoWindow::moveEvent(QMoveEvent *e)
{
  if (!App->infoInitialSetup) {
    //imodPrintStderr("info move %d %d\n", e->pos().x(), e->pos().y());
    imodDialogManager.masterWinHasChanged(IMOD_DIALOG);
    extendInfoTimer();
  }
}

void InfoWindow::extendInfoTimer()
{
  if (mInfoTimerID) {
    killTimer(mInfoTimerID);
    mInfoTimerID = startTimer(250);
  }
}
...
bool InfoWindow::event(QEvent *e)
{
  ...
  if (e->type() == QEvent::ActivationChange && isActiveWindow())
    ivwModvMenuIsActive(false);
  if (Imod_debug && minimized)
    imodPrintStderr("State change to minimized (isMinimized %d)\n", isMinimized() ? 1 :0);
...
// Change the keep on top flag - eliminated the old hack 3/12/19
void InfoWindow::keepOnTop(bool state)
{
  mStayingOnTop = state;
  utilSetStayOnTop(this, state);
  imodDialogManager.manageStayingOnTop();
}
...
void InfoWindow::timerEvent(QTimerEvent *e)
{
  ...
  if (mAutoTimerID && e->timerId() == mAutoTimerID ) {
    ...
    killTimer(mInfoTimerID);
    mInfoTimerID = 0;
    imodTrace('i', "Info timer fired and killed; targmove %d %d", mTargetMoveX, mTargetMoveY);
    if (mTargetHeight > 0)
      resizeToHeight(mTargetHeight);
    mTargetHeight = 0;
    if (mTargetMoveX >= -29000 && mTargetMoveY >= -29000)
      move(mTargetMoveX, mTargetMoveY);
    mTargetMoveX = mTargetMoveY = -30000;
    App->infoInitialSetup = B3DMAX(0, App->infoInitialSetup - 1);
  } else
    raise();
}
...
void InfoWindow::openSelectedWindows(const char *keys, int modelViewOpen)
{
  ...
  if (strchr(keys, '4') && imageOK)
    imodPlugOpenByName("Drawing Tools");
  if (strchr(keys, '5'))
    imodPlugOpenByName("Interpolator");
  if (strchr(keys, '6'))
    imodPlugOpenByName("Bead Helper");
  if (strchr(keys, 'T') && imageOK)
    imodPlugOpenByName("Line Track");
  if (strchr(keys, '9'))
    imodPlugOpenByName("Bead Fixer2");
  if (strchr(keys, '1'))
...
int imod_info_open()
{
  ...
  if (App->devPixVaries)
    imod_info_input();
  utilInitializeScreenChange(ImodInfoWin, ImodInfoWin->mDevicePixelRatio);
  ImodInfoWin->setInitialHeights(false);
