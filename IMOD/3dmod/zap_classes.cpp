...
void ZapWindow::screenChanged(QScreen *screen)
{
  mZap->screenChanged(utilGetNewDevPixRatio(this));
}
#endif
  ...
  : QGLWidget(inFormat, parent)
{
  ...
  mNumFirstDraws = 3;
#endif
  mFirstDraw = mNumFirstDraws;
  mLastDrawMsec = 0;
  mInitWidth = mInitHeight = 0;
  mScheduledDraw = false;
}

void ZapGL::paintGL()
{
  if (mFirstDraw >= mNumFirstDraws) {
...
{
  int bump = 2 - (mNumFirstDraws - mFirstDraw);
  double startTime = wallTime();
  if (!imodLoopStarted() || (!mInitWidth && !mInitHeight))
    return;
  if (mFirstDraw < 2 && !mScheduledDraw)
    cancelRedraw();
#if (defined(Q_OS_LINUX) || defined(Q_OS_MACX)) && QT_VERSION >= 0x050000

  // It needed more moves than resizes; do the resizes on the early draw
  if (mFirstDraw > 0) {
    if (bump >= 0)
      mZap->mQtWindow->resize(mInitWidth + bump, mInitHeight);
    mZap->mQtWindow->move(mInitLeft, mInitTop);
  } else
#endif
    updateGL();
  mLastDrawMsec = B3DNINT(1000. * (wallTime() - startTime));
