...
static void setObjFlag(int flag, int state, int types = 0, bool registered = false, 
...
static void setMeshDraw_cb(void);
static void mkMeshDraw_cb(int index);
...
static void setBitInMatFlags2(int flag, int state);
...
static int      sCurrentPanelFrame       = 0;
ObjectEditField objectEditFieldData[]    = {
  ...
  {"Mesh draw",  mkMeshDraw_cb,  setMeshDraw_cb,  NULL,       NULL},
...
// Statics and a function for doing different things depending on current state
...
// Set the data type: the combined flagh here prevents drawing and finishing undo
...
/******************************************************************
 * Style option
 * Here the combined flag is passed on to prevent registering the object change again
 */
void imodvObjedStyleData(int option, bool combined)
{
  switch(option){
  case stylePoints:
    setObjFlag(IMOD_OBJFLAG_NOLINE , 1, 0, combined);
    setObjFlag(IMOD_OBJFLAG_FILL, 0, 0, true);
    break;
  case styleLines:
    setObjFlag(IMOD_OBJFLAG_FILL | IMOD_OBJFLAG_NOLINE, 0, 0, combined);
    break;
  case styleFill:
    setObjFlag(IMOD_OBJFLAG_FILL | IMOD_OBJFLAG_NOLINE, 1, 0, combined);
    break;
  case styleFillOutline:
    setObjFlag(IMOD_OBJFLAG_NOLINE, 0, 0, combined);
    setObjFlag(IMOD_OBJFLAG_FILL, 1, 0, true);
    break;
  default:
    break;
  }
...
// Function for hot keys to modify both combo boxes at once
void imodvObjedSetDrawTypeAndStyle(int newData)
{
  int curType, curStyle;
  if (!objed_dialog)
    return;
  if (newData < 0) {
    curType = objed_dialog->dataTypeComboBox->currentIndex();
    curStyle = objed_dialog->drawStyleComboBox->currentIndex();
    if (curType == 2 && curStyle == 2) {
      newData = 21;
    } else if (curType == 1) {
      newData = 22;
    } else {
  ...
  imodvObjedDrawData(newData / 10, true);
  imodvObjedStyleData(newData % 10, true);
  objed_dialog->dataTypeComboBox->setCurrentIndex(newData / 10);
  objed_dialog->drawStyleComboBox->setCurrentIndex(newData % 10);
}
...
void imodvObjedSelect(int which)
{
  ...
  objset(Imodv);
  if (!Imodv->standalone && Imodv->syncObjedToCurObj && which <= Imodv->imod->objsize)
    imodInfoNewOCP(0, which, 0);
  imodvDraw(Imodv);
}

// External call to change to previous or next object
void imodvObjedChangeObject(int dir)
{
  int newObj = Imodv->objNum + dir;
  bool atEnd = newObj < 0 || newObj >= Imodv->imod->objsize;
  bool draw = false;

  // If standalone, modify the model directly
  if (Imodv->standalone) {
    if (dir > 0)
      imodNextObject(Imodv->imod);
    else
      imodPrevObject(Imodv->imod);
      
  } else {

    // Otherwise, if dialog not open or no syncing, go through 3dmod input routine
    if (!objed_dialog || !Imodv->syncObjedToCurObj) {
      if (dir > 0)
        inputNextObject(Imodv->vi);
      else
        inputPrevObject(Imodv->vi);
      draw = !objed_dialog && !atEnd;
    }
  }

  // Then if dialog not open, just adjust the current number; if it is open use select
  if (!objed_dialog && !atEnd) {
    Imodv->objNum = newObj;
    draw = true;
  } else if (!atEnd)
    imodvObjedSelect(newObj + 1);
  if (draw)
    imodvDraw(Imodv);
}
...
{
  sCurrentPanelFrame = item;
  if (objectEditFieldData[sCurrentPanelFrame].setwidget)
    objectEditFieldData[sCurrentPanelFrame].setwidget();
}

// An extra object is being freed, adjust curobj if appropriate
void imodvObjedFreeingExtraObj(Iobj *xobj)
{
  int ob;
  Iobj *obj;
  int numEditable = numEditableObjects(Imodv->curMod);
  if (!objed_dialog)
    return;
  for (ob = Imodv->mod[Imodv->curMod]->objsize; ob < numEditable; ob++) {
    obj = editableObject(Imodv->curMod, ob);
    if (obj == xobj) {
      if (ob < Imodv->objNum)
        Imodv->objNum--;
      break;
...
static void objset(ImodvApp *a)
{
  ...
  // If editable extra objects have just appeared, switch to first one, but exclude
  // the current point object as annoying
  diff = lastNumEditable - a->mod[a->curMod]->objsize;
  if (lastModNum == a->curMod && numEditable > lastNumEditable && (!diff || diff == 1) &&
      !Imodv->syncObjedToCurObj) {
    obj = editableObject(a->curMod, a->mod[a->curMod]->objsize);
    a->objNum = lastNumEditable - diff;
    if (strstr(obj->name, "point extra")) {
      if (diff)
        a->objNum += 1;
      else if (a->mod[a->curMod]->objsize > 0)
        a->objNum -= 1;
    }
  }
  lastModNum = a->curMod;
...
void imodvObjedNewView(void)
{
  ...
  setOnoffButtons();
  if (!ImodvClosed && !Imodv->standalone && Imodv->syncObjedToCurObj) {
...
void imodvObjedSetCurFrame(int value) { sCurrentPanelFrame = value; };
int imodvObjedGetCurFrame() {return sCurrentPanelFrame;};
...
void objed(ImodvApp *a)
{
  ...
  sTopWin = new DockingDialog(imodvDialogManager.parent(IMODV_DIALOG),
                              &imodvDialogManager, "Model View Object Editing",
                              "modvObjectEdit.html#TOP", 'O', Qt::Window);
  objed_dialog = new imodvObjedForm(sTopWin, 0);
  sTopWin->addWidgetToStack(objed_dialog);

  Imodv_objed_all = 0;  // May want to retain this setting
  setOnoffButtons();

  setModvDialogTitle(sTopWin, "MV Edit Obj");

  ctrlPressed = false;
  objed_dialog->setCurrentFrame(sCurrentPanelFrame, Imodv_objed_all);
...
static QCheckBox *wScaleLineWidth;
...
void ImodvObjed::scaleLineWidth(bool state)
{
  setObjFlag(IMOD_OBJFLAG_SCALE_WDTH, state ? 1 : 0);
  finishChangeAndDraw(1, 1);
}
...
static void setLines_cb(void)
{
  ...
  diaSetChecked(wScaleLineWidth, obj->flags & IMOD_OBJFLAG_SCALE_WDTH);
...
static void mkLines_cb(int index)
{
  ...
  wScaleLineWidth = diaCheckBox("Scale for high DPI", oef->control, layout1);
  QObject::connect(wScaleLineWidth, SIGNAL(toggled(bool)), &imodvObjed, 
                   SLOT(scaleLineWidth(bool)));
  wScaleLineWidth->setToolTip("Scale 2D and 3D widths up for a high-DPI monitor "
                              "(by DPI / 96)");
...
{
  setBitInMatFlags2(MATFLAGS2_CONSTANT, state ? 1 : 0);
  finishChangeAndDraw(1, 1);
}
...
{
  setBitInMatFlags2(MATFLAGS2_SKIP_LOW, state ? 1 : 0);
  finishChangeAndDraw(1, 1);
}

void ImodvObjed::meshSkipHiSlot(bool state)
{
  setBitInMatFlags2(MATFLAGS2_SKIP_HIGH, state ? 1 : 0);
  finishChangeAndDraw(1, 1);
}
 ...
 * The clip edit field
 *****************************************************************************/
static QCheckBox *wClipShowPlane;
...
void imodvObjedDrawClipPlane(bool state)
{
  ...
  imodvDraw(Imodv);
  if (objed_dialog)
    diaSetChecked(wClipShowPlane, Imodv->drawClip != 0);
}
...
{
  clipResetPlaneToAxis(which, -1, -1);
}

void ImodvObjed::clipResetPlaneToAxis(int which, int doGlobal, int planeNum)
{
  Ipoint min, max, mid;
  int ip;
  IclipPlanes *clips;
  Iobj *obj = objedObject();
  if (doGlobal < 0)
    doGlobal = Imodv->imod->editGlobalClip ? 1 : 0;
  ...
  ip = planeNum < 0 ? clips->plane : planeNum;
  clips->point[ip] = mid;
...
{
  toggleClipPlane(state, -1, -1);
}

void ImodvObjed::toggleClipPlane(bool state, int doGlobal, int planeNum)
{
  Iobj *obj = objedObject();     
  int ip, ipst, ipnd;
  if (doGlobal < 0)
  ...
  if (!obj && !doGlobal)
    return;
  IclipPlanes *clips = doGlobal ? &Imodv->imod->view->clips : &obj->clips;
  ip = planeNum < 0 ? clips->plane : planeNum;
  if (doGlobal)
    imodvRegisterModelChg();
  else
  ...
  ipst = ipnd = ip;
  if (planeNum < 0 && (Imodv->imod->view->world & WORLD_MOVE_ALL_CLIP)) {
    ipst = 0;
  ...
  for (ip = ipst; ip <= ipnd; ip++) {
    ...
    // When a plane is turned on, increment clip count to include it
      ...
      if (clips->point[ip].x == 0.0 && clips->point[ip].y == 0.0 &&
        ...
        clipResetPlaneToAxis(2, doGlobal, ip);
        return;
      }
    }
  }
...
// External call to toggle the current clipping plane
void imodvObjedToggleClip(int doGlobal, int planeNum)
{
  ...
  IclipPlanes *clips = doGlobal ? &Imodv->imod->view->clips : &obj->clips;
  if (planeNum < 0)
    planeNum = clips->plane;
  state = (clips->flags & (1 << planeNum)) == 0;
  imodvObjed.toggleClipPlane(state, doGlobal, planeNum);
  if (objed_dialog && planeNum == clips->plane && 
      (doGlobal == (Imodv->imod->editGlobalClip ? 1 : 0)))
...
static void mkClip_cb(int index)
{
  ...
  wClipShowPlane = diaCheckBox("Show current plane", oef->control, layout1);
  diaSetChecked(wClipShowPlane, Imodv->drawClip != 0);
  QObject::connect(wClipShowPlane, SIGNAL(toggled(bool)), &imodvObjed,
                   SLOT(clipShowSlot(bool)));
  wClipShowPlane->setToolTip("Draw semi-transparent plane for clipping plane being"
                             " edited (hot key " CTRL_STRING "+V)");
...
/*****************************************************************************
 * The mesh draw edit field
 *****************************************************************************/

static QSpinBox *wMeshThickSpin;
static QCheckBox *wMeshOnImages;


void ImodvObjed::meshThickSlot(int value)
{
  int m, mst, mnd, ob;
  Iobj *obj;
  ...
  for (m = mst; m <= mnd; m++) {
    for (ob = 0; ob < numEditableObjects(m); ob++)
      if (changeModelObject(m, ob)) {
        obj = editableObject(m, ob);
        imodvRegisterObjectChg(ob);
        obj->meshThickness = value;
      }
  }
  finishChangeAndDraw(0, 1);
}

void ImodvObjed::meshOnImageSlot(bool state)
{
  setObjFlag(IOBJ_EXFLAG_MESH_ON_IMG, state ? 1 : 0, OBJTYPE_CLOSED | OBJTYPE_OPEN,
             false, true);
  finishChangeAndDraw(0, 1);
}
...
static void setMeshDraw_cb(void)
{
  ...
  diaSetSpinBox(wMeshThickSpin, obj->meshThickness);
  diaSetChecked(wMeshOnImages, obj->extra[IOBJ_EX_FLAGS] & IOBJ_EXFLAG_MESH_ON_IMG);
  wMeshOnImages->setEnabled(iobjScat(obj->flags) == 0);
}
...
static void mkMeshDraw_cb(int index)
{
  ...
  // Surface thickness spin box
  QHBoxLayout *hLayout = diaHBoxLayout(layout1);
  wMeshThickSpin = (QSpinBox *)diaLabeledSpin(0, 0., IMESH_THICKNESS_BITS, 1., 
                                              "Surface thickness", oef->control, hLayout);
  QObject::connect(wMeshThickSpin, SIGNAL(valueChanged(int)), &imodvObjed,
                   SLOT(meshThickSlot(int)));
  wMeshThickSpin->setToolTip("To display surface with thickness, set separation between "
                             "paired meshes");

  // Do not know why this was needed instead of spacer to keep it from expanding too much
  // Probably it was equalizing it and the label
  wMeshThickSpin->setSizePolicy(QSizePolicy::Ignored, QSizePolicy::Preferred);
  hLayout->setStretchFactor(wMeshThickSpin, 10);

  wMeshOnImages = diaCheckBox("Draw mesh on images", oef->control, layout1);
  QObject::connect(wMeshOnImages, SIGNAL(toggled(bool)), &imodvObjed, 
          SLOT(meshOnImageSlot(bool)));
  wMeshOnImages->setToolTip("Draw mesh lines on Zap or Slicer images"); 
...
static void mkMakeMesh_cb(int index)
{
  ...
  wMakeDiamSpin = (QDoubleSpinBox *)diaLabeledSpin(1, -2., 400., 0.5, "Diam", 
                                                 oef->control, hLayout);
  ...
  if (!Imodv->standalone && (Imodv->vi->xybin * Imodv->vi->zbin > 1)) {
    ...
    wMakeDoButton->setToolTip("Mesh the object with these parameters (hot key " 
                              CTRL_STRING "+M)");
    wMakeAllButton->setToolTip("Mesh all objects with their respective"
                  " meshing parameters");
  }
  wMeshingLabel = diaLabel("", oef->control, layout1);
  finalSpacer(oef->control, layout1);
  fixMakeMesh_cb();
  setMakeMesh_cb();
}
...
void imodvObjedMeshObject()
{
  if (!objed_dialog)
    return;
  if (wMakeDoButton->isEnabled())
    imodvObjed.makeDoitSlot();
}
...
void ImodvObjed::updateMeshing(int ob)
{
  QString str;
  if (ob < 0) {
    wMeshingLabel->setText("");
  } else {
    str.sprintf("Meshing Object %d", ob + 1);
    wMeshingLabel->setText(str);
  }
}
...
// 4/24/16: No idea why extra objects were excluded from standalone, try putting them in
static int numEditableObjects(int model)
{
  Iobj *obj;
  int i, num = Imodv->imod->objsize;
  if (model != Imodv->curMod)
...
static void setObjFlag(int flag, int state, int types, bool registered, bool extraFlag)
{
  int m, mst, mnd, ob;
  b3dUInt32 obflags;
  Iobj *obj;
    ...
    for (ob = 0; ob < numEditableObjects(m); ob++) {
      ...
      if (changeModelObject(m, ob) && 
        ...
        if (!registered)
          imodvRegisterObjectChg(ob);
        if (extraFlag)
          setOrClearFlags(&obj->extra[IOBJ_EX_FLAGS], flag, state);
        else
  ...
  for (m = mst; m <= mnd; m++) {
    for (ob = 0; ob < numEditableObjects(m); ob++)
      if (changeModelObject(m, ob)) {
        ...
        imodvRegisterObjectChg(ob);
        if (state)
          obj->matflags2 |= (unsigned char)flag;
        else
          obj->matflags2 &= (unsigned char)(~flag);
      }
  }
