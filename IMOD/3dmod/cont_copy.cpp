...
int openContourCopyDialog(ImodView *vw)
{
  ...
  sTopWin = new DockingDialog(imodDialogManager.parent(IMOD_DIALOG), &imodDialogManager,
                              "Copy Contours", "contourCopy.html#TOP", 'c', Qt::Window);
  sData.dia = new ContourCopy(sTopWin, 0);

  if (!sData.dia) 
    return(1);
  sTopWin->addWidgetToStack(sData.dia);
...
static const char *buttonLabels[] = {"Apply"};
static const char *buttonTips[] = {"Copy the selected contours (hot key Shift+K)"};

ContourCopy::ContourCopy(DockingDialog *parent, const char *name)
                ...
                ImodPrefs->getRoundedStyle(), " ", "", name, 0)
{
  ...
  parent->setWindowTitle(imodCaption("Copy Cont"));
}
...
#include "info_cb.h"
void ContourCopy::topChangeEvent(QEvent *e)
{
...
void ContourCopy::topCloseEvent ( QCloseEvent * e )
{
