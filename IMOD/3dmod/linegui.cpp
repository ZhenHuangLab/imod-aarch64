...
void imodPlugExecute(ImodView *inImodView)
{
  ...
  sTopWin = new DockingDialog(imodDialogManager.parent(IMOD_DIALOG), &imodDialogManager,
                              "Line Tracker", "lineTracker.html#TOP", 'T', Qt::Window);
  plug->window  = new LineTrack(sTopWin, "line tracker");
...
const char *buttonLabels[] = {"Track", "Copy", "Undo", "Reset"};
const char *buttonTips[] = 
  {"Track from last to current model point - Hot key: Spacebar",
   ...
   "Reset parameters to built-in default values"};

LineTrack::LineTrack(DockingDialog *parent, const char *name)
...
void LineTrack::topCloseEvent ( QCloseEvent * e )
{
  PlugData *plug = &thisPlug;
  double values[MAX_SETTINGS];
...
void LineTrack::topChangeEvent(QEvent *e)
{
