...
#ifdef Q_OS_MACX
#define HANDLE_DPR_CHANGE_FOR_MAC(func, wx, wy)             \
  float scaling = newDPR / mDevicePixelRatio;               \
  mDevicePixelRatio = newDPR;                               \
  if (fabs(scaling - 1.) > 0.05)                            \
      func(B3DNINT(scaling * wx), B3DNINT(scaling * wy));
#else
#define HANDLE_DPR_CHANGE_FOR_MAC(func, wx, wy) mDevicePixelRatio = newDPR;
#endif
...
float utilUnitZoomForDeviceScaling(float devPixRatio = 0.);
