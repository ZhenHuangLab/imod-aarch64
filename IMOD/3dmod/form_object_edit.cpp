 ...
 */
objectEditForm::objectEditForm(DockingDialog* parent, Qt::WindowFlags fl)
  : QWidget(parent, fl)
{
  mTopWin = parent;
  setupUi(this);
  ...
  connect(drawLabelsBox, SIGNAL(toggled(bool)), this, SLOT(toggledDrawLabels(bool)));
  connect(labelSizeSpinBox, SIGNAL(valueChanged(int)), this, SLOT(labelSizeChanged(int)));
  connect(scaleForDpiBox, SIGNAL(toggled(bool)), this, SLOT(toggledScaleForDpi(bool)));
...
void objectEditForm::toggledDrawLabels( bool state )
{
  ioewDrawLabels(state ? 1 : 0);
}

void objectEditForm::labelSizeChanged( int value )
{
  ioewLabelSize(value);
  setFocus();
}
...
void objectEditForm::toggledScaleForDpi( bool state )
{
  ioewScaleForDPI(state ? 1 : 0);
}
...
void objectEditForm::setDrawLabelsBox( bool state )
{
  diaSetChecked(drawLabelsBox, state);
}

void objectEditForm::setLabelSize( int value )
{
  diaSetSpinBox(labelSizeSpinBox, value);
}
...
void objectEditForm::setScaleForDpi(bool state, bool show)
{
  diaSetChecked(scaleForDpiBox, state);
  diaShowWidget(scaleForDpiBox, show);
}
...
void objectEditForm::topCloseEvent( QCloseEvent *e )
{
  ioew_closing();
  e->accept();
}
...
void objectEditForm::topChangeEvent(QEvent *e)
{
