...
void imodvEditMenu(int item)
{
  ...
  case VEDIT_MENU_ISOSURFACE: /*isosurface*/
    ...
    break;
  case VEDIT_MENU_SAVE_DOCK:  /* Save dialog state */
    imodvDialogManager.saveStackStateToSettings();
    break;
  case VEDIT_MENU_REOPEN_DOCK:  /* Restack dialogs */
    imodvDialogManager.restoreStackFromSettings();
    break;
  }
}
...
void imodvViewMenu(int item)
{
  ...
  case VVIEW_MENU_LABELS:
    toggleWorldFlag(a->drawLabels, VIEW_WORLD_LABELS, VVIEW_MENU_LABELS);
    a->mainWin->setCheckableItem(VVIEW_MENU_LABELS, a->drawLabels);
    break;
  ...
  case VVIEW_MENU_CURPNT:
    ...
    if (a->curPointExtraObj <= 0) {
      ...
      imodvObjedFreeingExtraObj(ivwGetAnExtraObject(a->vi, a->curPointExtraObj));
    }
    if (freeXobj) {
...
void imodvMenuLabels(int value)
{
  Imodv->mainWin->setCheckableItem(VVIEW_MENU_LABELS, value);
}
