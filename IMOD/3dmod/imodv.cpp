...
static int sOnceOpened = 0;
static QRect sLastGeom(0, 0, 0, 0);
...
static int getVisuals(ImodvApp *a)
{
  ...
  for (i = 0; OpenGLAttribList[i] != NULL; i++) {
    if (depthDBstAl < 0 && ImodPrefs->keySetsHWstereo()) {
    ...
    if (depthDBst < 0 && ImodPrefs->keySetsHWstereo()) {
    ...
    OpenGLAttribList[i]->alpha = 0;
    if (depthSBst < 0 && ImodPrefs->keySetsHWstereo()) {
      visual = imodFindGLVisual(*OpenGLAttribList[i]);
      if (visual && visual->stereo) {
...
static int openWindow(ImodvApp *a)
{
  int xleft, ytop, newHeight, newWidth, needy;
  QRect geom(0, 0, 0, 0);
  ...
  if (App->devPixVaries)
    a->mainWin->show();
  imod_info_input();
  utilInitializeScreenChange(a->mainWin, a->mainWin->mDevicePixelRatio);
  if (!a->wantWinx)
    XY_PIXEL_TO_DEVICE( a->mainWin->m, DEFAULT_XSIZE, DEFAULT_XSIZE, a->wantWinx,
                        a->wantWiny);
  XY_DEVICE_TO_PIXEL( a->mainWin->m, a->wantWinx, a->wantWiny, newWidth, needy);
  ...
  if (a->fullscreen) {
    ...
    if (sOnceOpened)
      ...
      geom = ImodPrefs->getModViewGeometry();
      

    // If window was open before, set size and position the same
    if (geom.width() > 0) {
      newWidth = geom.width();
      ...
      a->mainWin->show();
      imod_info_input();
      newHeight = needy + a->mainWin->height() - B3DQT_PIXELS(a->mainWin->m, a->winy);
    ...
    diaLimitWindowSize(newWidth, newHeight, a->mainWin);
    diaLimitWindowPos(newWidth, newHeight, xleft, ytop, a->mainWin);
    a->mainWin->mInitWidth = newWidth;
    a->mainWin->mInitHeight = newHeight;
...
int imodv_main(int argc, char **argv)
{
  ...
  for (i = 1; i < argc; i++){
	...
	break;
      ...
      case 's':
	sscanf(argv[++i], "%d%*c%d", &a->wantWinx, &a->wantWiny);
	break;
  ...
  // For some reason this is done after opening the info window and it works here too
#ifdef __linux
...
void imodv_draw()
{
  if (ImodvClosed)
    return;
  imodvDraw(Imodv);
  if (Imodv->syncObjedToCurObj && !Imodv->standalone && Imodv->imod->cindex.object >= 0 &&
      Imodv->objNum != Imodv->imod->cindex.object)
    imodvObjedNewView();
}
...
void imodvQuit()
{
  ...
  if (a->standalone)
    App->exiting = 1;
  sLastGeom = ivwRestorableGeometry(a->mainWin);
  if (!a->standalone)
    ImodPrefs->recordModViewGeometry(sLastGeom);
