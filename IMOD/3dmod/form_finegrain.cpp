 ...
 */
FineGrainForm::FineGrainForm(DockingDialog* parent, Qt::WindowFlags fl)
  : QWidget(parent, fl)
{
  mTopWin = parent;
  setupUi(this);
...
void FineGrainForm::init()
{
  ...
  connect(noCapCheckBox, SIGNAL(toggled(bool)), this, SLOT(noCapToggled(bool)));
  connect(stippleGapsCheckBox, SIGNAL(toggled(bool)), this,
          SLOT(stippleGapsToggled(bool)));
  connect(prevChangeButton, SIGNAL(clicked()), this, SLOT(prevChangeClicked()));
  gapNoCapLayout->insertStretch(1, 100);
...
void FineGrainForm::setFontDependentWidths()
{
  ...
  // This is not essential with the spacers on each side, but just aesthetic
  cwid = diaSetButtonWidth(prevChangeButton, rounded, 1.25, "Previous");
  nextChangeButton->setFixedWidth(cwid);
}

// Update the interface with the current properties and states
void FineGrainForm::update( int ptContSurf, int enabled, DrawProps *props, int stateFlags,
                            bool nextEnabled, bool prevEnabled)
{
  int i;
  QString str;
  int minEnabled[7] = {1, 1, 1, 2, 1, 2, 2};
  diaSetGroup(surfContPtGroup, ptContSurf);
  changeAllCheckBox->setEnabled(ptContSurf == 1);
  nextChangeButton->setEnabled(nextEnabled);
  prevChangeButton->setEnabled(prevEnabled);
  for (i = 0; i < 7; i++) {
    bool changed = stateFlags & mChangeFlags[i];
    mEndButs[i]->setEnabled(!ptContSurf &&changed && enabled >= minEnabled[i]);
    mClearButs[i]->setEnabled(changed && enabled >= minEnabled[i]);
    mDSlabels[i]->setText((changed && enabled >= minEnabled[i]) ? "S" : "D");
  }
  ...
  lastWidth2DBut->setEnabled(mLast2Dwidth >= 0 && enabled > 1);
  lastWidth3DBut->setEnabled(mLast3Dwidth >= 0 && enabled);
  lastSymtypeBut->setEnabled(mLastSymType >= 0 && enabled > 1);
  lastSymsizeBut->setEnabled(mLastSymsize >= 0 && enabled > 1);
  ...
  diaSetWidgetColor(fillColorFrame, QColor(mCurFillRed, mCurFillGreen,
                                           mCurFillBlue));
  setColorBut->setEnabled(enabled > 0);
  setFillColorBut->setEnabled(enabled > 0);
  mTransSlider->getSlider(0)->setEnabled(enabled > 0);
  mTransSlider->setValue(0, props->trans);
  diaSetSpinBox(width2DSpin, props->linewidth2);
  width2DSpin->setEnabled(enabled > 1);
  diaSetSpinBox(width3DSpin, props->linewidth);
  width3DSpin->setEnabled(enabled > 0);
  ...
  noCapCheckBox->setEnabled(ptContSurf == 1);
  diaSetChecked(noCapCheckBox, props->noCap);
  diaSetSpinBox(connectSpin, props->connect);
  connectSpin->setEnabled(ptContSurf < 2 && enabled > 1);
...
{
  ifgGotoNextChange(false);
}

void FineGrainForm::prevChangeClicked()
{
  ifgGotoNextChange(true);
}
...
int FineGrainForm::applyLastChange(int lastChangeType)
{
  if (lastChangeType < 0 || lastChangeType > 6 || !mLastButs[lastChangeType]->isEnabled())
    return 0;
...
void FineGrainForm::noCapToggled( bool state )
{
  ifgNoCapChanged(state);
}
...
void FineGrainForm::topCloseEvent( QCloseEvent *e )
{
  ifgClosing();
  lineColorDone();
  fillColorDone();
...
void FineGrainForm::topChangeEvent(QEvent *e)
{
