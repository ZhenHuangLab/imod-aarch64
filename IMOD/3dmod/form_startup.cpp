...
void StartupForm::init()
{
  ...
  connect(scanGroup, SIGNAL(buttonClicked(int)), this, SLOT(scanTypeClicked(int)));
  diaSetGroup(scanGroup, 0);  
  
  pieceCoordGroup =  new QButtonGroup(this);
  pieceCoordGroup->addButton(unaliPieceButton, 0);
  pieceCoordGroup->addButton(alignedPieceButton, 1);
  pieceCoordGroup->addButton(alignedVSPieceButton, 2);
  connect(pieceCoordGroup, SIGNAL(buttonClicked(int)), this,
          SLOT(pieceCoordTypeClicked(int)));
  diaSetGroup(pieceCoordGroup, 0);
...
void StartupForm::manageForModView()
{
  ...
  superResSpinBox->setEnabled(!mModvMode);
  eerZbinSpinBox->setEnabled(!mModvMode);
  ...
  manageMontage();
  scanningBox->setEnabled(!mModvMode);
  fullScanButton->setEnabled(!mModvMode);
  subsetMinMaxButton->setEnabled(!mModvMode);
  subsetMeanSDButton->setEnabled(!mModvMode);
  changeMRCstatsBox->setEnabled(!mModvMode);
  unaliPieceButton->setEnabled(!mModvMode);
  alignedPieceButton->setEnabled(!mModvMode);
  alignedVSPieceButton->setEnabled(!mModvMode);
...
void StartupForm::scanTypeClicked(int value)
{
  mScanType = value;
}

void StartupForm::pieceCoordTypeClicked(int value)
{
  mPieceCoordType = value;
}
...
char ** StartupForm::getArguments( int & argc )
{
  ...
  if (loadUnscaledBox->isChecked())
    addArg("-m");
  if (changeMRCstatsBox->isChecked())
    addArg("-K");
  ...
  addArg("-es");
  sprintf(numstr, "%d", superResSpinBox->value());
  addArg(numstr);
  addArg("-ez");
  sprintf(numstr, "%d", eerZbinSpinBox->value());
  addArg(numstr);
  if (imagePyramidBox->isChecked())
    addArg("-py");
  addArg("-J");
  sprintf(numstr, "%d", mScanType);
  addArg(numstr);
  addArg("-A");
  sprintf(numstr, "%d", mPieceCoordType);
  addArg(numstr);
...
void StartupForm::setValues(ImodView *vi, char **argv, int firstfile, int argc, 
                            ...
                            int pieceKeyType, int xyzwinopen, int sliceropen,
                            int zapOpen, int modelViewOpen, int fillCache, int ImodTrans,
                            int mirror, int frames, int nframex, int nframey, int overx,
                            int overy, int overEntered)
{
  int vms;
  ...
  mScanType = vi->scaleScanType;
  diaSetGroup(scanGroup, mScanType);
  changeMRCstatsBox->setChecked(vi->storeScanInMRC > 0);
  ...
  superResSpinBox->setValue(vi->eerSuperRes);
  eerZbinSpinBox->setValue(vi->eerZbinning);

  mPieceCoordType = pieceKeyType;
  diaSetGroup(pieceCoordGroup, mPieceCoordType);
