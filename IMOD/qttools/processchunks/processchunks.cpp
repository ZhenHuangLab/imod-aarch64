 ...
 *  Author: Sue Held
 *
 *  Copyright (C) 2010-2020 by the Regents of the University of
 ...
 */
static const int numOptions = 23;
static const char *options[] = {
  ":r:B:", ":s:B:", ":m:B:", ":G:B:", ":O:I:", ":g:B:", ":n:I:", ":w:FN:", ":d:I:",
  ":e:I:", ":C:FT:", ":T:IP:", ":c:FN:", ":L:I:", ":q:I:", ":Q:CH:", ":I:CH:", ":D:CH:",
...
void Processchunks::loadParams(int &argc, char **argv) {
  ...
  PipGetInteger("L", &mJobLimitPerCycle);
...
bool Processchunks::setup() {
  ...
  setupHostRoot();
  setupComFileJobs();
  retval = probeMachines(machineNameList);
...
int Processchunks::startLoop() {
  ...
  mHoldCrit = B3DMAX(2, (mMachineListSize + 1) / 2);
  ...
  while (++i < mMachineListSize && !loopDone) {
    ...
    for (cpuIndex = 0; cpuIndex < numCpus; cpuIndex++) {
      ...
      if (!mMachineList[i].isDropped() && !process->isJobValid() && !dropout && !noChunks
        ...
        if (!foundChunks || numAdded >= mJobLimitPerCycle) {
          noChunks = true;
        }
...
int Processchunks::runGenericQueueCommand(const char *command, int waitMsec) {
  if (!command || command[0] == 0x00 || !mQueue)
    return 0;
...
bool Processchunks::probeMachines(QStringList &machineNameList) {
  int i;
  bool retval;
  ...
  const QString firstName = machineNameList[0];
  retval = ((firstName == "localhost" || firstName == mHostRoot) &&
            machineNameList.size() == 1);
