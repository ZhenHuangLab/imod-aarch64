...
static void usage(char *progname)
{
  ...
  printf("    -C b,w     Contrast black/white values for conversion to "
         "bytes\n");
  printf("    -a mn,sd   Scale to mean and SD for conversion to bytes (0,0 for "
         "default)\n");
...
int main(int argc, char *argv[])
{
  ...
  float sample, imageMean, imageSD;
  ...
  for (iarg = 1; iarg < argc - 1 ; iarg++){
    if (argv[iarg][0] == '-'){
      ...
      case 'a':
        sscanf(argv[++iarg], "%f%*c%f", &scale, &offset);
        autoContrast = 1;
        if (scale)
          autoMean = scale;
        if (offset)
  ...
  if (convert && autoContrast)
    exitError("You cannot enter -C or -S for scaling with -a for auto-contrasting");
  if (autoContrast)
    convert = 1;
  ...
  if (!makeQimage) {
    ...
    if (version > 0 && !oldcode && (doParallel <= 0 || compression == 1) && !autoContrast
  ...
  for (z = zmin; z <= zmax; z++){
    ...
    for (chunk = 0; chunk < numChunks; chunk++) {
      ...
      if (autoContrast) {
        linePtrs = makeLinePointers(slice.data.b, xsize, ysize, psize);
        if (!linePtrs)
          exitError("Allocating line pointers for autocontrasting");
        sample = B3DMIN(xsize * ysize, numSample) / (float)(xsize * ysize);
        i = sampleMeanSD(linePtrs, typeForSampleMean(hdata.mode), xsize, ysize, sample, 
                         0, 0, xsize, ysize, &imageMean, &imageSD);
        if (i)
          exitError("Error %d calling sampleMeanSD for autocontrasting", i);
        scale = autoSD / imageSD;
        offset = autoMean - scale * imageMean;
      }
      ...
      if (makeQimage) {
        ...
        if (resolution) {
          qim->setDotsPerMeterX(resolution / (resolution > 0. ? 0.01 : -0.0254));
          qim->setDotsPerMeterY(resolution / (resolution > 0. ? 0.01 : -0.0254));
        }
        if (outPsize == 1)
          for (i = 0; i < 256; i++)
