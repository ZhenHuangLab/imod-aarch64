                               ...
                               int time)
{
  ...
  if (xmax[0] - xmin[0] < 0.1 * mNxyz[0] || xmax[1] - xmin[1] < 0.1 * mNxyz[0])
    return 1;
                            ...
                            int *startCoord, int *endCoord, Imod *pitchModel) 
{
  ...
  if (mProjLayerStatType > 0) {
    ...
    for (loop = 0; loop < 2; loop++) {
      ...
      if (bestMult == MAX_MULT) {
        exitError("Could not get smooth enough histogram even by combining bins by %d",
                  MAX_MULT);
      }
      if (mDebugOutput)
        PRINT1(bestMult);
      ...
      if (indMax <= 3 || indMax >= numBins - 4) {
        exitError("The peak in a histogram with bins combined by %d is too close to "
                  "the edge of the volume", bestMult);
      }
      if (mDebugOutput > 1) {
      ...
      if (ind < 0) {
        exitError("Could not find distinct rise above baseline in histogram");
      }
      if (mDebugOutput)
