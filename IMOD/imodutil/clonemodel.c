...
int main( int argc, char *argv[])
{
  ...
  while (fgets(line, lineSz, coordFP)) {
    ...
    if (numRead != 7 && numRead != 8)
      exitError("Error parsing summary file (# read is %d, not 7 or 8)", numRead);
    if (numRead == 8) {
      valuesPresent = 1;
      if (!vRangeSpecified) {
  ...
  if (!outFP)
    exitError("Error opening output file: %s", outFile);
  if (imodWrite(outModel, outFP))
    exitError("Error writing output file: %s", outFile);
  fclose(outFP);
