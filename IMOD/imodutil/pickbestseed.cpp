...
void PickSeeds::main(int argc, char *argv[])
{
  ...
  int contForTrackPos, modelSeedZvalues[MAX_MODELS], numSeedZvalues;
  ...
  const char *options[] = {
    ...
    "border:BordersInXandY:IP:", "middle:MiddleZvalue:I:", "zseed:SeedZvalue:IM:",
    ...
    "verbose:VerboseOutput:I:", "help:usage:B:"};
  ...
  PipNumberOfEntries("SeedZvalue", &numSeedZvalues);
  if (numSeedZvalues && numSeedZvalues != numModels)
    exitError("If SeedZvalue is entered, there must be an entry for each tracked model");
  ...
  for (i = 0; i < numModels; i++) {
    ...
    free(filename);
    if (numSeedZvalues)
      PipGetInteger("SeedZvalue", &modelSeedZvalues[i]);
  }
  ...
  for (co = 0; co < maxConts; co++) {
    ...
    // Check for bad tracking from izMiddle: see if one of the contours was from there
    if (numberInList(izMiddle, modelSeedZvalues, numSeedZvalues, 0)) {
      badAtMiddle = true;
      for (i = 0; i < numModels; i++) {
        j = candid.contours[i];
        if (j >= 0 && modelSeedZvalues[mTracks[j].model] == izMiddle)
          badAtMiddle = false;
      }
      
      // If not, find and use the model seed point itself from the track used to set
      // the position
      if (badAtMiddle) {
        cont = &mTrackMods[mTracks[contForTrackPos].model]->obj[0].cont[contForTrackPos];
        imin = -1;
        for (i = 0; i < cont->psize; i++) {
          dz = fabs(cont->pts[i].z - modelSeedZvalues[mTracks[contForTrackPos].model]);
          if (dz < 0.5)
            imin =  i;
        }
        ...
        if (imin >= 0) {
          candid.pos = cont->pts[imin];
        } else {
          candid.pos.x = xBorder - 1000.;
        }
      }
    }
  ...
  if (!mNumAccepted[2])
    exitError("No candidate points were acceptable.%s%s%s%s%s", 
              numOver || numClust ? "  Consider allowing" : "", 
              numClust ? " clustered " : "", numClust && numOver ? "and/or" : "",
              numOver ? " elongated " : "", numOver || numClust ? "points." : "");
