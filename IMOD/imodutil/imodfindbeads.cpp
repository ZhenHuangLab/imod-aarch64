...
void FindBeads::averageBeads(int izStart, int izEnd)
{
  ...
  if (nsum) {
    ...
    if (mAdjustSizes) {
      if (newDiam <= 0.) {
        printf("WARNING: imodfindbeads - Bead size could not be measured and will not "
               "be adjusted\n");
      } else if (B3DMAX(newDiam, mBeadSize) / B3DMIN(newDiam, mBeadSize) > 
                 mMaxAdjustFactor) {
...
float FindBeads::extractDiameter(float *oneBead)
{
  ...
  float *diams, *avgden, xx[210], yy[210];
  float grad, maxGrad = 0.;
  int i, ifirst = (int)(0.2  * mBeadSize), ndat = 0;
  int indMax = -1, nfit, ind, idir, idiff;
  ...
  i = B3DNINT(0.9 * mBeadSize) - ifirst + 10;
  diams = B3DMALLOC(float, i);
  avgden = B3DMALLOC(float, i);
  if (!diams || !avgden)
    exitError("Allocating arrays for finding diameter");
