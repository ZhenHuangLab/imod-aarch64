...
public:
	...
	{
		static const float _CHAR_WIDTH = 80;
		printf("Num rings: %lu\n", ring.size() );
		
		for(int i=0; i<ring.size(); i++)
		{
			int meanBar  = ((ring[i].meanVal - minValue) / valueRange) * _CHAR_WIDTH;
			int stDevBar = ((ring[i].stDev   - minValue) / valueRange) * _CHAR_WIDTH * 2.0f;
			...
			if(showAllVals)
			{
				int minBar  = ((ring[i].minVal - minValue      ) / valueRange) * _CHAR_WIDTH;
				int maxBar  = ((ring[i].maxVal - ring[i].minVal) / valueRange) * _CHAR_WIDTH;
...
public:
	...
	void setOrigSphere( Sphere &originalSphere )
	{
		origSphere = originalSphere;
	}
	
	void setSphere( float centerX, float centerY, float centerZ, float radius, int objIdx )
	{
	...
	void calcHistogramRings( float binSize, float firstBinSize, float zScale,
...
struct Filter
{
	...
	void print()
	{
		switch(filterType)
		{
			...
			case(FT_SCALE):		  printf("Adjust,    min=%f max=%f\n", min, max);		break;
			case(FT_THRESHOLD):	printf("Threshold, min=%f max=%f\n", min, max);		break;
		}
	}
};
...
public:
	...
	void loadSlice( MrcHeader *inHead, int z, vector<Filter> &filt )
	{
		if(printLevel >= 8)
...
int main(int argc, char *argv[])
{
  ...
  if (!imod){
	...
	if(printLevel >= 2)
	{
		printf("AVERAGED HISTOGRAM: (averaged over %lu spheres)\n", allRings.size() );
  ...
  //## DETERMINE MIN AND MAX ALLOWED VALUES FOR "EARLY ELIMINATION":
	...
	if( spheresOut.size() > maxSpheresOut )	{
		...
		printf("      %lu spheres will be reduced to %d\n\n",spheresOut.size(),maxSpheresOut);
  ...
  imodWriteFile(imod);
	...
	fprintf(stdout, "\nPERFORMANCE SUMMARY:\n"  );
	fprintf(stdout, "  # slice loads:       %ld\n", sliceBuffer.totSliceLoads   );
	fprintf(stdout, "  # reloads avoided:   %ld\n", sliceBuffer.totSliceRepeats );
	...
	fprintf(stdout, "  # spheres input:  %lu\n", spheresIn.size()   );
															 ...
															 bool markForDeleteOnly )
{
	...
	printf(" > # (overlapping) spheres deleted: %ld of %d (%4.2f%%) \n",
				 numSpheresFlaggedForDelete, numSpheresOrig, deletedPercent );
	printf(" > # spheres left:                  %ld\n", spheresLeft );
	printf("\n");
