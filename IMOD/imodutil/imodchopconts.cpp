...
void ChopConts::main( int argc, char *argv[])
{
  Iobj *obj;
  Icont *contmp;
  int lenContour, numContour, numObj = 0, lenContourIn;
  int *objList = NULL;
  ...
  if (breakAtColor > 0) {
    if (!PipGetBoolean("AssignSurfaces", &mAssignSurf) || 
        !PipGetInteger("LengthOfPieces", &lenContourIn) ||
  ...
  lenEntered = 1 - PipGetInteger("LengthOfPieces", &lenContourIn);
  if (lenEntered) {
    if (lenContourIn == -1)
      lenContour = B3DMAX(16, mModel->zmax / 5);
    else
      lenContour = lenContourIn;
  }
  if (lenContour <= 0)
  ...
  if (!PipGetInteger("NumberOfPieces", &numContour)) {
    if (lenEntered && lenContourIn > 0)
      exitError("You cannot enter both -number and -length with a value greater than 0");
