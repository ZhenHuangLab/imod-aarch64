...
int main( int argc, char *argv[])
{
  ...
  float zrange = 0;
  float cylSearch = 0.;
  float rCritLo = 0., rCritHi = 0., window = 100.;
  float symZoom = 0.;
  int *objList;
  int *rampData;
  char *listString;
  ...
  if (!testcurve) {
    ...
    /* Criteria */
    PipGetTwoFloats("RadiusCriterion", &rCritLo, &rCritHi);
    if (PipGetFloat("WindowLength", &window)) {
      if (rCritLo > 0. && rCritHi > rCritLo)
    ...
    if (numColors) {
      if ((rCritLo <= 0. || rCritHi <= 0.) && divColors)
        exitError("You must enter the radius criterion option to subdivide colors "
                  "by radius");
      if (palette) {
      ...
      if (numPts > 2) {
        ...
        if (circleThrough3Pts(xx[0], yy[0], xx[numPts / 2], yy[numPts / 2],
          ...
          if (meetsCrit) {
            ...
            if ((rCritLo > 0. && rCritHi > 0. && (rad < rCritLo || rad > rCritHi)) || 
