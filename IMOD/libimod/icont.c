 ...
 * the same for all points when rounded to the nearest integer.  [cont] can be NULL.
 */
...
{
  Icont *cont;
  float xval, yval, weight, minWeight = 0.01;
  int pix = 0;
  int i, j, y;
  int xmin, xmax;
  int bgnpt,endpt;
    ...
    if (!((endpt - bgnpt) % 2)) {
      ...
      /* add all points in each scan. */
       for (j = bgnpt; j < endpt; j++) {
        ...
        xval = (xmin + xmax) * 0.5f;
        yval = cont->pts[j].y;
        weight = B3DMAX(minWeight, xmax - xmin);
        rcp->x += xval * weight;
        rcp->y += yval * weight;
        *rtw += weight;
...
int imodContourZValue(Icont *cont)
{
  if ((!cont) || (!cont->psize))
    return(-1);
  return(B3DNINT(imodContourFloatZValue(cont)));
}
...
/*! Sets the label pointer of [inContour] to [inLabel] after freeing an existing label 
  if any */
void imodContourSetLabel(Icont *inContour, Ilabel *inLabel)
{
  if (!inContour) return;
  if (!inLabel) return;
  imodLabelDelete(inContour->label);
...
/*! Returns 1 if the point at [index] in [inContour] is defined as a gap in the contour 
 * store or if it is at the end of an open-type contour. */
int imodContourPointIsGap(Icont *inContour, int index)
{
  if (inContour->store)
    return istorePointIsGap(inContour->store, index);
  return ((inContour->flags & ICONT_OPEN) && index == inContour->psize - 1) ? 1 : 0;
}
/* END_SECTION */
