...
double imodObjectChecksum(Iobj *obj, int obNum)
{
  ...
  osum += obj->valblack + obj->valwhite + obj->matflags2 + obj->meshThickness;
  ...
  for(co = 0; co < obj->contsize; co++){
    ...
    psum += cont->psize;
    if (co)
      psum += (cont->psize - obj->cont[co - 1].psize) % 13;
...
{
  return imodObjectSortBySurf(obj, 0);
}

/*!
 * Sorts the contours in object [obj] by their z values, first sorting by surface number
 * if [ifBySurf] is nonzero.  If the object
 * contains time-dependent data, then the contours are sorted first by time,
 * then optionally by surface number, then by z.  Returns non-zero for error.
 */
int imodObjectSortBySurf(Iobj *obj, int ifBySurf)
{
  ...
  Istore *stp;
  int i, j, sz, sindex;
  int minz, maxz, minSurf, maxSurf;
  int hasTime = iobjFlagTime(obj);
  double *keys;
  double surfScale, timeScale, minKey;
 ...
 */
  ...
  keys = B3DMALLOC(double, obj->contsize);
  if (!keys)
    return(-1);

  /* Find the min and max Z values and surface numbers */
  minz = minSurf = INT_MAX;
  maxz = maxSurf = -INT_MAX;
  for (i = 0; i < obj->contsize ; i++) {
    if (cont[i].psize) {
      sz = B3DNINT(cont[i].pts[0].z);
      ACCUM_MIN(minz, sz);
      ACCUM_MAX(maxz, sz);
      ACCUM_MIN(minSurf, cont[i].surf);
      ACCUM_MAX(maxSurf, cont[i].surf);
    }
  }

  /* Set up scaling to put surface higherthan Z and time higher than surface */
  surfScale = 2 * (maxz + 2 - minz);
  timeScale = 2 * (maxSurf + 2 - minSurf) * surfScale;

  /* Assign a key to each contour */
  for (i = 0; i < obj->contsize ; i++) {
    if (cont[i].psize)
      sz = B3DNINT(cont[i].pts[0].z) - minz;
    else
      sz = maxz + 1;
    keys[i] = sz;
    if (ifBySurf)
      keys[i] += surfScale * (cont[i].surf - minSurf);
    if (hasTime)
      keys[i] += timeScale * cont[i].time;
  }
...
int   imodObjectGetMaxPoints(Iobj *inObject)
{
  ...
  if (!inObject) 
    return(0);
  for (co = 0; co < inObject->contsize; co++)
    ACCUM_MAX(maxPts, inObject->cont[co].psize);
  return(maxPts);
}
 ...
 * {IobjFlagMesh}, {IobjFlagLine}, {IobjFlagTime}, {IobjFlagExtraInModv},
 * {IobjFlagPntOnSec}, and {IobjFlagPlanar}.  {IobjFlagConnected} is 0 for scattered 
 * point objects.  {IobjFlagPlanar} is non-zero for closed objects and open objects
 * with the flag set to start a new contour on each section.
 */
int   imodObjectGetValue(Iobj *inObject, int inValueType)
{
  ...
  case IobjFlagPlanar:
    return(iobjPlanar(inObject->flags) ? 1 : 0);
  ...
  case IobjFlagExtraInSlicer:
    return(inObject->extra[IOBJ_EX_FLAGS] & IOBJ_EXFLAG_SLICER_ONLY);
  default:
    return(0);
  }
}
 ...
 * extra object in model view window), {IobjFlagPntOnSec}, and {IobjFlagExtraInSlicer}.
 */
void  imodObjectSetValue(Iobj *inObject, int inValueType, int inValue)
{
  ...
  case IobjFlagExtraInSlicer:
    setOrClearFlags(&inObject->extra[IOBJ_EX_FLAGS], IOBJ_EXFLAG_SLICER_ONLY, inValue);
    return;
