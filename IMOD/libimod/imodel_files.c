...
static int imodel_write(Imod *mod, FILE *fout)
{
  ...
  mod->flags |= IMODF_MAT1_IS_BYTES | IMODF_MULTIPLE_CLIP | IMODF_HAS_MESH_THICK;
...
static int imodel_write_object(Iobj *obj, FILE *fout,
                               Ipoint *scale)
{
  int i, error, mh, numReal = 0;
  int id;
  IclipPlanes *clips;
  Ipoint normScale;

  for (mh = 0; mh < obj->meshsize; mh++) 
    if (imeshThickness(obj->mesh[mh].flag) == 0)
      numReal++;
...
static int imodel_write_mesh(Imesh *mesh, FILE *fout, Ipoint *scale)
{
  ...
  if (!mesh || imeshThickness(mesh->flag)) 
    return(0);
  imodPutInt(fout, &id);
...
static int imodel_read_imat(Iobj *obj, FILE *fin, b3dUInt32 flags)
{
  ...
  /* 9/6/20: this byte was not zero in a few models, so zero it out for older models */
  if (!(flags & IMODF_HAS_MESH_THICK))
    obj->meshThickness = 0;
  return 0;
}
...
int imodReadAscii(Imod *imod)
{
  ...
  while ( ((len = imodFgetline(imod->file,line, MAXLINE)) > 0)){
    ...
    if (substr(line, "meshthick"))
      obj->meshThickness = atoi(&(line[9]));
...
int imodWriteAscii(Imod *imod)
{
  ...
  for (ob = 0; ob < imod->objsize; ob++){
    ...
    for (mh = 0; mh < obj->meshsize; mh++) {
      mesh = &(obj->mesh[mh]);
      if (imeshThickness(mesh->flag))
        continue;
      fprintf(imod->file, "mesh %d %d %d\n", 
              numReal, mesh->vsize, mesh->lsize);
      numReal++;
