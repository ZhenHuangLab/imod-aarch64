                         ...
                         int pt)
{
  ...
  *ptProps = *contProps;
  ...
  /* If not ending exactly on the point, cancel gap and connect and value */
  if (lastChange != pt) {
    ptProps->gap = ptProps->connect = ptProps->value1 = 0;
    stateFlags &= ~(CHANGED_GAP | CHANGED_CONNECT | CHANGED_VALUE1);
  }
