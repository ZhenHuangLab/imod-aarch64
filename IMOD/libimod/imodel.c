 ...
 * freeing all point, size, and label data, maintaining general storage 
 * lists, and reducing the size of the contour array.  Sets the current contour and 
 * point index to -1 and returns the size of the current object or -1 if error.
 */
...
/*!
 * Deletes the list of [numConts] contours in [contours] in the current object of the 
 * model [mod], freeing all point, size, and label data, maintaining general storage 
 * lists, and reducing the size of the contour array.  Contours are numbered from 0.
 * Sets the current contour and point index to -1 and returns the size of the current 
 * object or -1 if error.
 */
int imodDeleteListOfConts(Imod *mod, int *contours, int numConts)
{
  Iobj *obj;
  Icont *cont, *newConts;
  int i, ind, outInd, lastDel, curDel;
  int *sorted;
  ...
  for (i = 0; i < numConts; i++)
    if (contours[i] < 0 || contours[i] >= obj->contsize)
      return -1;

  /* Copy and sort */
  sorted = B3DMALLOC(int, numConts);
  if (!sorted)
    return -1;
  memcpy(sorted, contours, numConts * sizeof(int));
  rsSortInts(sorted, numConts);
  
  /* Eliminate duplicates */
  ind = 1;
  for (i = 1; i < numConts; i++)
    if (sorted[i] != sorted[i - 1])
      sorted[ind++] = sorted[i];
  numConts = ind;

  if (obj->contsize > numConts) {
    newConts = imodContoursNew(obj->contsize - numConts);
    if (!newConts) {
      free(sorted);
      return -1;
    }
  ...
  for (ind = 0; ind < numConts; ind++) {
    ...
    for (i = lastDel + 1; i < curDel; i++)
      imodContourCopy(&(obj->cont[i]), &(newConts[outInd++]));
      
    /* If contour has any points, free them. */
    cont = &obj->cont[curDel];
    if ((cont->pts != NULL) && (cont->psize))
      free(cont->pts);

    free (cont->sizes);

    /* need to delete labels if any */
    imodLabelDelete(cont->label);
    ilistDelete(cont->store);
    istoreDeleteContSurf(obj->store, curDel, 0);
    lastDel = curDel;
  }
  
  /* Copy final contours if any */
  for (i = lastDel + 1; i < obj->contsize; i++)
    imodContourCopy(&(obj->cont[i]), &(newConts[outInd++]));
...
#define PRINT_CHECKSUM(str) if (debug) {          \
    fprintf(stderr, str, sum);                    \
    lastSum = sum;                                \
  }
/*!
...
{
  int ob, co, isum, i, anyPts = 0;
  int debug = getenv("IMOD_DEBUG_CHECKSUM") != NULL ? 1 : 0;
  Iview *view;
  Iobjview *obv;
  IclipPlanes *clips;
  SlicerAngles *slanp;
  double sum = 0.;
  double osum, lastSum = 0.;
  ...
  PRINT_CHECKSUM("obj group %f\n");

  for (ob = 0; ob < imod->objsize; ob++){
    sum += imodObjectChecksum(&(imod->obj[ob]), ob);
    for (co = 0; co < imod->obj[ob].contsize && !anyPts; co++)
      if (imod->obj[ob].cont[co].psize)
        anyPts = 1;
  }
  if (anyPts)
    sum += istoreChecksum(imod->store);
  PRINT_CHECKSUM("object %f\n");
  ...
  for (co = 0; co < imod->viewsize; co++) {
    ...
    if (co) {
      for (ob = 0; ob < view->objvsize; ob++) {
        ...
        osum += obv->valblack + obv->valwhite + obv->matflags2 + obv->meshThickness;
        sum += osum;
      }
    }
  }
  PRINT_CHECKSUM("views %f\n");
  
  /* This will catch fractional values - not perfect but probably good */
  isum = sum / 1000000.;
  isum = (int)(1000. * (sum - 1000000. * isum));
  if (debug)
    fprintf(stderr, "checksum = %f %d\n", sum, isum);
  return (isum);
}
...
int imodSetRefImage(Imod *imod, MrcHeader *hdata)
{
  ...
  /* 3/16/22: This needs to be set too for transformations to work */
