...
#define FWRAP_ERROR_NO_PIXEL_SIZE -13
...
#define getopenedimod GETOPENEDIMOD
#define imodcountcontspoints IMODCOUNTCONTSPOINTS
...
#define getopenedimod getopenedimod_
#define imodcountcontspoints imodcountcontspoints_
             ...
             int *npoint, int *nobject, char *fname, int fsize)
{
  ...
  return getopenedimod(ibase, npt, coord, color, npoint, nobject);
}
...
/*!
 * Returns the total number of contours in the model 
 * in [numContsTotal], the maximum in any one object in [maxNumConts], the total
 * points in [numPtsTotal], and the maximum in any one object in [maxNumPts]
 */
                         ...
                         int *maxNumPts)
{
  ...
  *maxNumConts = 0;
  ...
  for (ob = 0; ob < sImod->objsize; ob++) {
    obj = &sImod->obj[ob];
    numInObj = 0;
    ACCUM_MAX(*maxNumConts, obj->contsize);
    *numContsTotal += obj->contsize;
    for (co = 0; co < obj->contsize; co++)
      numInObj += obj->cont[co].psize;
    ACCUM_MAX(*maxNumPts, numInObj);
    *numPtsTotal += numInObj;
  }
  return FWRAP_NOERROR;
}
...
/*!
 * Deletes the list of [numCont] contours in [contours] in object [ob] using 
 * @imodel.html#imodDeleteListOfConts, which will adjust contour numbers in an object 
 * store, if any.  Contours are numbered from 1 as usual.
 */
int deletelistofconts(int *ob, int *contours, int *numCont)
{
  int ind, err = checkAssignObject(ob);
  if (err)
    return err;
  imodSetIndex(sImod, *ob - 1, -1, -1);
  for (ind = 0; ind < *numCont; ind++)
    contours[ind]--;
  err = imodDeleteListOfConts(sImod, contours, *numCont);
  for (ind = 0; ind < *numCont; ind++)
    contours[ind]++;
  if (err < 0)
    return(FWRAP_ERROR_FROM_CALL);
...
/*!
 * Returns 1 if flags are set to display values and skip ones below threshold for object
 * [ob], 0 if not, or a negative number for error.
 */
int getobjskiplowvalues(int *ob)
{
  ...
  if (err)
    return err;
  return ((sObj->flags & IMOD_OBJFLAG_USE_VALUE) && 
          (sObj->matflags2 & MATFLAGS2_SKIP_LOW)) ? 1 : 0;
}
...
/*!
 * Returns pixel size in microns in [um] and Z-scale in [zscale], -1 if there is no model
 * or -13 if there is no pixel size in the model
 */
int getimodheado(float *um, float *zscale)
{
  ...
  if (sImod->units == IMOD_UNIT_PIXEL)
...
/*!
 * Sets the image reference information of the model with the delta, origin,
 * and current tilt angle values from an MRC file header.
 */
int putimageref(float *delta, float *origin, float *tilt)
{
  ...
  iref->crot.y = tilt[1];
  iref->crot.z = tilt[2];
  sImod->flags |= IMODF_OTRANS_ORIGIN | IMODF_TILTOK;
  return FWRAP_NOERROR;
}
