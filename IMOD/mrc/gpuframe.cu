...
__global__ void shiftAndAddToSum(float *sumArr, float *fullArr, float *nonDWsum,
                                 int ixStart, int iyInStart, int iyOutStart, int numXdo,
                                 int numYdo, int nxInDim, int nxOutDim, int tableYoffset, 
                                 ...
                                 float freqDelY)
{
  ...
  if (ix < numXdo && iy < numYdo) {
    ...
    if (fullArr) {
      ...
      sumReal = sumArr[indIn];
      sumImag = sumArr[indIn + 1];
    } else {
...
__global__ void addOddToEvenSum(float *evenArr, float *oddArr, int sumXplus, int sumYpad)
{
  ...
  if (ix < sumXplus && iy < sumYpad) {
    ...
    if (oddArr)
      evenArr[ind] = evenArr[ind] + oddArr[ind];
    else
...
// If pixel is at the edge of a defect padding row or column, average the super-resolution
// pixels (# indicated by doDefects) and use the average
// If pixel is above truncation limit or marked as a defect, pick a pixel at random from
// box of 17x17 pixels excluding central 9, picking only pixels that pass the same test
// If that fails to find a pixel after sampling 1/4, it expands the box by 8 pixels
#define PREPROC_PIXEL(mtyp, tex)                                        \
  ...
  if (defectVal > 1) {                                                  \
    for (loop = 1; loop < doDefects; loop++) {                          \
      inX = ix + loop * avgDelX;                                        \
      inY = iy + loop * avgDelY;                                        \
      neighVal = tex2D(tex, inX, inY);                                  \
      if (doGain)                                                       \
        neighVal *= tex2D(sGainRefTex, inX, inY);                       \
      pixVal += neighVal;                                               \
    }                                                                   \
    pixVal /= doDefects;                                                \
  }                                                                     \
  if ((truncLimit > 0 && pixVal > truncLimit) || (defectVal == 1)) {    \
    for (loop = 0; loop < 10; loop++) {                                 \
      limit = 4 * (loop + 2);                                           \
      lim2 = 2 * limit;                                                 \
      xStart = max(0, ix - limit);                                      \
      xEnd = min(nx - 1, xStart + lim2);                                \
      xStart = max(0, xEnd - lim2);                                     \
      yStart = max(0, iy - limit);                                      \
      yEnd = min(ny - 1, yStart + lim2);                                \
      yStart = max(0, yEnd - lim2);                                     \
      nxBox = xEnd + 1 - xStart;                                        \
      nyBox = yEnd + 1 - yStart;                                        \
      numPix = nxBox * nyBox;                                           \
      for (trial = 0; trial < numPix / 4; trial++) {                    \
        pseudo = (197 * (pseudo + 1)) & 0xFFFFF;                        \
        indInBox = (pseudo >> 4) % numPix;                              \
        inY = yStart + indInBox / nxBox;                                \
        inX = xStart + indInBox % nxBox;                                \
        delX = inX - ix;                                                \
        delY = inY - iy;                                                \
        ...
        if ((truncLimit > 0 && pixVal > truncLimit) ||                  \
            (doDefects > 0 && tex2D(sDefectTex, inX, inY) == 1))        \
          continue;                                                     \
        sum = pixVal;                                                   \
        nsum++;                                                         \
        break;                                                          \
...
// Kernel to pre-process and write to the surface reference
__global__ void preprocessFrame(float *linearArr, int type, int nx, int ny, int doGain, 
                                float truncLimit, int doDefects, int seed)
{
  int ix = blockIdx.x * blockDim.x + threadIdx.x;
  int iy = blockIdx.y * blockDim.y + threadIdx.y;
  float pixVal, neighVal, sum = 0.;
  int nsum = 0, inX, inY, delX, delY, loop, limit, xStart, xEnd, yStart, yEnd, lim2;
  int pseudo, indInBox, nxBox, nyBox, trial, numPix, avgDelX, avgDelY;
  unsigned char defectVal = 0;
  if (ix >= nx || iy >= ny)
    return;
  pseudo = (seed + 197 * (ix + 157) * (iy + 179)) & 0xFFFFF;
  if (doDefects > 0) {
    defectVal = tex2D(sDefectTex, ix, iy);
    if (defectVal > 1) {
      avgDelX = defectVal == MAP_COL_AVG_SUPER ? 1 : 0;
      avgDelY = defectVal == MAP_ROW_AVG_SUPER ? 1 : 0;
    }
  }
  ...
  // For padding row/column, assign average to other pixels
  if (defectVal > 1) {
    for (loop = 1; loop < doDefects; loop++) { 
      inX = ix + loop * avgDelX;
      inY = iy + loop * avgDelY;
#ifdef NO_SURFACES
      linearArr[inX + inY * nx] = pixVal;
#else
      surf2Dwrite(pixVal, sTempSurfRef, inX * sizeof(float), inY);
#endif
    }
    }
...
#if CUDA_VERSION >= 4000
      utilPrint("  tex1d %d\n", device_properties.maxTexture1DLinear);
#endif
    }
...
#if CUDA_VERSION >= 4000
    mMaxTex1D = best_properties.maxTexture1DLinear;
#endif
                               ...
                               unsigned char *defectMap, int camSizeX, int camSizeY)
{
  ...
  if (mCorrectDefects && mAvgRowColSuperFac > 0)
    mCorrectDefects = mAvgRowColSuperFac;
...
void FrameGPU::setUnpaddedSize(int unpadX, int unpadY, int flags, int debug)
{
  ...
  mAvgRowColSuperFac = 0;
  if (flags & GPU_AVG_SUPER_2X)
    mAvgRowColSuperFac = 2;
  if (flags & GPU_AVG_SUPER_4X)
    mAvgRowColSuperFac = 4;
  mDebug = debug % 10;
                           ...
                           int sumYpad, int evenOdd)
{
  ...
  if (!mDeviceSelected)
    return -1;
  mBindTexForSums = 1;
  if (mMaxTex1D > 0 && mMaxTex1D <= (fullXpad + 2) * fullYpad)
    mBindTexForSums = 0;
  ...
  if (sumChanged) {
    ...
    if (mBindTexForSums && bindSumArray(0))
      mBindTexForSums = 0;
  }
                             ...
                             float shiftY, int shiftSource, bool applyDoseFilt)
{
  ...
  int numYdo, numXdo, byteScale, numXsuperBlocks, numYsuperBlocks;
  ...
  // Take care of binding, just turn it off if it fails
  if ((mBindTexForSums || needBound > 1) && bindSumArray(needBound)) {
    if (needBound < 2)
      mBindTexForSums = 0;
    else
      return 1;
  ...
  if ((mBindTexForSums || needBound > 1) && bindFullOrCorrArray
      (fullArr, (fullXpad + 2 ) * fullYpad * sizeof(float))) {
    if (needBound < 2)
      mBindTexForSums = 0;
    else
      return 1;
  ...
  numXsuperBlocks = (2 * (nxFFT + 1 + sumYpad) + MAX_TABLE - 1) / MAX_TABLE;
    ...
    for (ixSuper = 0; ixSuper < numXsuperBlocks; ixSuper++) {
      ...
      if (shiftSource)
        ...
        shiftAndAddToSum<<<gridSize, blockSize>>>
          (sumArr, mBindTexForSums ? NULL : fullArr, mDoUnDWsum ? mNonDWsum : NULL,
           ixStart, iyInStart, iyStart, numXdo,
  ...
  if (mDoEvenOdd) {
    ...
    if (error < 2) {
      ...
      addOddToEvenSum<<<gridSize, blockSize>>>
        (mEvenSum, mBindTexForSums ? NULL : mOddSum, sumXplus, mSumYpad);
