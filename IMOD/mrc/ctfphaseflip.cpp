...
int main(int argc, char *argv[])
{
  ...
  WL = 12.3984 / sqrt(volt * (volt + 1022.0)); //wavelength in Angstroms;
  C1 = MY_PI * WL;
  C2 = C1 * cs * 1.e7 * WL * WL / 2.0;    // All in Angstroms, cs scaled to Angstroms


  int stripDist[2], stripLimit;
  ...
  for (view = startingView; view <= endingView; view++) {
    ...
    if (!angleIsZero) {
      stripPixelNum =
        2 * ((int)(defocusTol / tan(B3DMAX(fabs(currAngle), MIN_ANGLE * MY_PI / 180.)) /
             pixelSize) / 2);
      maxCopyPixels = stripPixelNum / 2;
    } else {
      maxCopyPixels = stripPixelNum = nx;
    }
    if (stripPixelNum > nxPad)
      stripPixelNum = nxPad;
    ...
    B3DCLAMP(minWidth, minStripWidth, nx);
    ...
    stripLimit = (doFullImages || angleIsZero) ? nxPad : nx;
    if (stripPixelNum > stripLimit) {
      stripPixelNum = niceFrame(stripLimit, -2, niceLimit);
      if (stripPixelNum < 0.9 * minWidth)
        stripPixelNum = niceFrame(stripLimit, -2, 19);
    }
      ...
      */
      ...
      if (doFullImages) {
        ...
        if (angleIsZero) {
          ...
          stripBegin = 0;
          stripEnd = B3DMIN(nx - 1, stripPixelNum - 1);
          stripStride = interPixelNum;
          ...
          // Classic region with stripMid in middle of strip that is shifted from last
                   ...
                   nx) {
          stripBegin = B3DMAX(0, (stripInd - numLeftShiftedMid) * interPixelNum);
          stripEnd = B3DMIN(nx - 1, stripBegin + stripPixelNum - 1);
          stripStride = interPixelNum;
          stripMid = (stripBegin + stripEnd) / 2;
          halfStrip = stripPixelNum / 2;
          if (stripEnd == nx - 1) {
          ...
          stripBegin = B3DMAX(0, nx - stripPixelNum);
          stripEnd = nx - 1;
          stripMid = (stripBegin + stripEnd) / 2;
          halfStrip = stripPixelNum / 2;
          doRightShiftedMid = true;
...
{
  double delz, theta;
  double wavelength = 1.23984 / sqrt(voltage * (voltage +  1022.0));
