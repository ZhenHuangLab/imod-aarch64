...
#else
OutputBaseFilename=imod_{#ImodVersion}{#NoLibs}_win64_NoCUDA
#endif
#else
OutputBaseFilename=imod_{#ImodVersion}{#NoLibs}_win32
#endif
...
#else
Source: "imod_{#ImodVersion}{#NoLibs}_win64_NoCUDA.tar.gz"; DestDir: "{app}"; Flags: deleteafterinstall ignoreversion; Afterinstall: installIMOD
#endif
#else
Source: "imod_{#ImodVersion}{#NoLibs}_win32.tar.gz"; DestDir: "{app}"; Flags: deleteafterinstall ignoreversion; Afterinstall: installIMOD
#endif
  ...
  if RegQueryStringValue(rootKey, cygwinKeyName, pathName, path) and (path <> '') then begin
    ...
    if DirExists(path) then begin
      ...
      if DirExists(bin) then begin
        if fileExists(bin + '\python.exe') or fileExists(bin + '\python') or fileExists(bin + '\python2') or fileExists(bin + '\python3') then begin
...
begin
  ...
  outputBaseFilename := 'imod_{#ImodVersion}{#NoLibs}_win32'
#endif
  if Cygwin and (fileExists(PythonDir + '\python') or fileExists(PythonDir + '\python2') or fileExists(PythonDir + '\python3')) then begin
    ...
    if returnCode <> 0 then begin
      ...
      ' If IMOD install fails, use the command "ls /bin/python*" in a Cygwin terminal to see if "/bin/python", "/bin/python2", or "/bin/python3"' +
      ' If so, try running the command "ln -Lf /bin/python /bin/python.exe" or the same command with "python2" or "python3" instead of "python" depending on what is there', mbCriticalError, MB_OK);
    end;
  end;
