...
position in the tilt series, as well as defocus values if the \fB-defocus\fR
...
tooltip = 
manpage = With this entry, the file of projection angles specified by
\fB-angle\fR will also have defocus values for each point.  The entry must be
...
short = full
type = CH
usage = Name of full original reconstruction
tooltip = 
manpage = Name of full original reconstruction.  The default is
"rootname_full.rec" with the old naming style, or "rootname_full_rec.mrc" with
"mrc" naming style, so this entry is needed if that file does not exists or if
...
short = ali
type = CH
usage = Name of aligned stack file
tooltip = 
manpage = Name of aligned stack file.  The default is "rootname.ali" with old
naming style, or "rootname_ali.mrc" with ".mrc" naming style, so this
...
[Field = SkipBackTransform]
short = skip
type = B
usage = Produce output model that fits on aligned rather than raw stack 
tooltip = 
manpage = Skip the back-transformation to the raw stack and output a
model that fits the aligned stack instead.
