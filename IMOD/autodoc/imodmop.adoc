...
dimensions.  If padding is specified with the \fB-pad\fR option, the padding is
added only if it is positive.
If Z limits are entered with the \fB-zminmax\fR option, then they supercede the
Z limits computed from the objects with this option; this is the way to get
extra slices in Z.  This option cannot be used with \fB-xminmax\fR or \fB-yminmax\fR.
...
use the \fB-invert\fR option to produce the mask value outside the painted regions
and 0 inside.  A mask can be tapered with the \fB-pad\fR and \fB-taper\fR
options.  When creating a mask file, either with this option or the
\fB-label\fR option, a byte file will be output as unsigned so that the values
in the file will equal the mask values.  Also, the input slices will not be
read, which means that the input image file can be just a copy of an MRC file header,
such as is created with the IMOD program copyheader.  However, the pixel
spacing and origin information in whatever image file is provided must match those of
the volume on which the model was generated.

[Field = LabelMaskList]
short = label
type = LI
usage = List of mask values to use for each included object (/ for object #'s)
tooltip = 
manpage = A set of object-specific mask values that allows the regions inside
different objects to be labeled by the different values.  There must be as
many numbers in the list as objects included for consideration (i.e., either
the total number of model objects, or the number entered with the
\fB-objects\fR option).  Enter "/" to use the default, which is the object
number for each object.  The mask values must fit into the output file data
type and not be indistinguishable from the fill value in the output file.  This option
cannot be entered with \fB-mask\fR.

[Field = RetainOutsideMask]
short = retain
type = B
usage = Outside of areas painted with mask, copy input values to the output
tooltip =
manpage = Outside of areas painted with mask values, retain the existing image
data by copying values from the input to the output.  This option, which can
be used only when masking, allows mask output to be added to an existing mask
output file.
...
data, of RGB input data unless making a mask, or when using \fB-color\fR or
\fB-project\fR.
...
closed contour objects, all scattered point objects if \fB-2dscat\fR or \fB-3dscat\fR is
entered, and any open contour objects listed with \fB-tube\fR.
...
Options \fB-2dscat\fR and \fB-3dscat\fR are mutually exclusive.
...
Z plane, pixels will be extracted in that Z plane only.  If there is only one
point in the contour, a circle will be painted around the point, which will
...
This entry should produce the same scaling as the \fB-s\fR option to 3dmod(1).
