...
short = retain
type = B
usage = Do not delete the meshes for the objects being smoothed
tooltip = 
manpage = Do not delete the meshes for the objects being smoothed.  These
meshes will not reflect the smoothing and the objects need to be remeshed, so
they are deleted by default unless this option is entered.
