...
short = outroot
type = CH
usage = Root name for output command files (default is root of input file)
tooltip =
manpage = 
...
[Field = UniqueInfoFile]
short = unique
type = B
usage = Attach the first command file number to a boundary info file
tooltip =
manpage = 
