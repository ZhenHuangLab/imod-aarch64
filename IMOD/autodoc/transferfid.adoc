...
series; the default is setnamea.fid (or setnameb.fid with the \fB-b\fR option.)
...
setnameb.seed (or setnamea.fid with the \fB-b\fR option.)

[Field = BoundaryModel]
short = boundary
type = FN
usage = Model file with boundary contours to transform, instead of
transferring a seed
tooltip = 
manpage = Model file with boundary contours around areas to correlate in the A
stack, which will be transformed to fit the B stack.  The program will find
the best matching pair of views in the image stacks, and then transform this
model appropriately instead of trying to transfer seed points.  The fiducial model file
need not exist.  If the file name does not begin with "setnamea" (or "setnameb"
with the \fB-b\fR option), the \fB-o\fR option must be entered with an output file name.
