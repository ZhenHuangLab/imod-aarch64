...
short = stackext
type = CH
usage = Extension of raw stack excluding period (optional)
tooltip = 
manpage = Extension of raw stack excluding the period.  If this is not
specified, the program will assume the extension ".st" unless the \fB-style\fR
option is entered.  With a \fB-style\fR option and no specified stack
extension, it will look for ".st", ".mrc", ".hdf", ".tif", and ".tiff" and
require that only one of those types is present.  With this entry, which could
in principle be arbitrary, it will not care if files with other extensions are
present.
...
tooltip = 
manpage = Angle from the vertical to the tilt axis for B axis.  The default
for a dual-axis set is the angle entered for A.
...
short = reversed
type = B
usage = First acquired part of tilt series is second part of stack
tooltip = 
manpage = The part of the tilt series acquired first is the second part of the
tilt series stack.  Without this option, the program assumes that the tilt
series is in the same order as the file produced by automated acquisition
software, in which the part acquired first is inverted in order and placed in
the beginning of the stack.

[Field = BReversedBidirectional]
short = breversed
type = B
usage = First acquired part of B axis tilt series is second part of stack
tooltip = 
manpage = 
...
"b" on the command file name.  For a single-axis data set, directives
...
two cases the usual descriptive text is put before the extension, and command
files will contain an environment variable setting to make programs generate
files of the corresponding type.
