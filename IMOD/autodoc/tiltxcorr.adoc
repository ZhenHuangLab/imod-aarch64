...
Reduction will be increased above 4 only to keep the reduced image within a
size of about 4K, up to a reduction of 16.  This default behavior might result
in more reduction
than desired, and the reduction might change when the amount of trimming
is changed.  This option allows direct control of the reduction instead and
also allows a reduction up to 32.
...
[Field = SecondPeakBoxSize]
short = second
type = IP
usage = Report on second peak if within box of given length and width
tooltip = 
manpage = Report on the second peak position if it is within a box of the
given length and width before binning, centered on the first peak and with its
width along the tilt axis and its length perpendicular to the tilt axis.
