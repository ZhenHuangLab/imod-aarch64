...
[Field = ChunkSizeForHDF]
short = chunk
type = IT
usage = Chunk sizes in X, Y, and Z to set up direct writing to HDF file
tooltip = 
manpage = Chunk sizes in X, Y, and Z, for an initial run to set up an HDF file
for direct writing of chunks by later runs.

[Field = LockFileForHDF]
short = lock
type = FN
usage = Name of lock file to use for parallel writing to chunked HDF file
tooltip = 
manpage = Name of lock file to use for parallel writing to chunked HDF file.
This option must be supplied both to the initial setup run and to each later
run.  The file must already exist.

[Field = XSaveStartAndEnd]
short = xsave
type = IP
usage = Starting & ending X coordinates in padded volume to save into HDF chunk
tooltip = 
manpage = 

[Field = YSaveStartAndEnd]
short = ysave
type = IP
usage = Starting & ending Y coordinates in padded volume to save into HDF chunk
tooltip = 
manpage = 

[Field = ZSaveStartAndEnd]
short = zsave
type = IP
usage = Starting & ending Z coordinates in padded volume to save into HDF chunk
tooltip = 
manpage = 

[Field = PlaceChunkAtXYZ]
short = place
type = IT
usage = Starting X, Y, Z coordinates in HDF file at which to write chunk
tooltip = 
manpage = 
