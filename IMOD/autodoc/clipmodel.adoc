...
short = bound
type = LI
usage = List of model objects containing volume to include or exclude
tooltip = 
manpage = List of model objects with contours enclosing a volume to include or
exclude.  If coordinate limits are also entered, then the intersection of the
regions inside those limits and the interior of the contours will be used as
the volume to include or exclude.  Objects must be closed contour objects.
There can be more than one contour on each section, and contours do not need
to be drawn on every section included in the volume.  Points are
tested for whether they are inside the boundary contours on the nearest
section with such contours, unless the \fB-coplanar\fR option is given.
Points beyond the minimum and maximum Z values of the boundary
contours are considered outside the volume.
The listed objects will be excluded from the clipping operations; not even
coordinate limits are applied to them.  If multiple operations are specified,
the boundary contours are applied only on the first operation.

[Field = CoplanarBoundary]
short = coplanar
type = B
usage = Boundary contours apply only on the planes where they are drawn
tooltip = 
manpage = Boundary contours apply only on the planes where they are drawn;
thus, to be considered inside the volume, a point must be on the same plane
as, and inside, some boundary contour.
...
short = range
type = FP
usage = Lower and upper values or areas for deleting or retaining points/contours
tooltip = 
manpage = When deleting points or contours with the \fB-values\fR option, use
this option to enter the lower and upper values of the range used to decide whether a 
...
Model View window.  When deleting closed contours by area with the
\fB-areas\fR option, use this option to enter the lower and upper areas of the
range.
...
[Field = AreasInOrOutOfRange]
short = areas
type = I
usage = Delete closed contours with areas inside (1) or outside (-1) of a range
tooltip = 
manpage = Use this option to delete contours in closed contour objects with
areas inside or outside of a range specified with the \fB-range\fR option,
which must be entered as well.  Enter a positive value to delete inside the
range or a negative one to delete outside the range.  Enter -1 or 1 if 
the area limits entered with \-fBrange\fR are in pixels, -2 or +2 if they are
in square nanometers, or -3 or 3 
if they are in square microns.  This option cannot be entered with \fB-values\fR.
