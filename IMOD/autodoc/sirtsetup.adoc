...
argument is used for the command file.  If the root name of this file ends in
"_for_sirt" and the processing is not starting from the zero iteration, that
ending will be stripped from the command files produced. 
...
tooltip = 
manpage = 0 if image files have descriptive extensions like
".preali", or 1 or 2 if descriptive text is before the extension ".mrc" or
".hdf".  This entry is needed only if the style cannot be determined from
...
\fB-TargetChunks\fR option to Splittilt(1).
...
short = rd
type = FP
usage = Radius and falloff for radial filter in all backprojections
tooltip =
manpage = Set the radius and falloff for the high-frequency cutoff of the filter
used in the backprojections.  The default provides a mild filtering with a
radius is 0.4 and Gaussian falloff with a true sigma of 0.035 (which is produced
by a value of 0.05 when the \fB-FalloffIsTrueSigma\fR option is not entered)
...
tooltip =
manpage = Use the falloff value entered with the \fB-RadiusAndSigma\fR option directly
as the sigma of the Gaussian filter instead of using a sigma that is 0.707
times the entered value.  This option was added in IMOD 4.8.59, after the
incorrect scaling of the falloff value was discovered in the Tilt(1) code, so that
