...
short = gridfix
type = B
usage = Set the sample size the same as the image size and preserve pixel size
tooltip = 
manpage = (Interactive FIXGRID)  Set the sample size the same as
the image size and modify the cell size so as to preserve the pixel size.
...
values to switch the header between different definitions of the origin.  This
operation will not change the flag about the origin inversion and will invert
the origin values seen inside IMOD programs as well as those stored in the
header.

[Field = ToggleOrigin]
short = toggleorg
type = B
usage = Invert the sign of the origin in the file by changing a header flag
tooltip = 
manpage = (Interactive INVERTORG)  Invert the signs of the X, Y, and Z origin
values stored in the file header without changing the values seen inside IMOD
programs.  This will be done by setting the flag that determines whether
values are inverted on output to the file or not.  
...
[Field = Change4BitMode]
short = 4bit
type = I
usage = Change mode for 4-bit data to 101 (1) or from 101 (-1) 
tooltip = 
manpage = (Interactive 4BIT)  For a positive entry, change a mode 0 file to
the 4-bit mode 101 and double the size in X that is stored in the header.  The
header listing will look the same if the file was already recognized as
containing 4-bit data packed into bytes.  For a negative entry, change a mode
101 file back to mode 0 and divide the X size by 2 (X size must be even).
...
in the file, unless the \fB-position\fR option is entered with a specific position.
This option is not equivalent to the interactive LAB option.
If there are already 10 labels, the last one will be replaced.  Be sure to
quote the whole string when running from the command line.

[Field = PositionForTitle]
short = position
type = I
usage = Position that new title should be placed in, numbered from 1
tooltip = 
manpage = Position that new title should be placed in, numbered from 1.  Note
that a positive number is entered here, unlike the 0 or negative number
entered with the interactive LAB option.

[Field = RemoveTitles]
short = remove
type = LI
usage = List of existing titles to remove, numbered from 1
tooltip = 
manpage = List of existing titles to remove, numbered from 1; comma-separate
ranges may be entered.  This option can be used together with \fB-title\fR and
\fB-position\f; titles will be removed before one is added with \fB-title\fR.
