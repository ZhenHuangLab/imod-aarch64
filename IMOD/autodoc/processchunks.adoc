...
[Field = m]
type = B
usage = Run multiple, independently named command files
tooltip = 
manpage = Run multiple, independently named command files, whose full names or
names without extension are given as non-option arguments after the machine list.
The command files are not required to be numbered.  The program behavior
changes in several ways: processing will continue even if some chunks fail;
chunks that fail will be retried only once unless a different value is entered
with the \fB-e\fR option; the detection of slow/hung chunks is disabled unless the
\fB-C\fR option is entered.
...
start) this number of times, unless running multiple
named command files with the \fB-m\fR option.  All running jobs will be killed
when quitting.  The default limit is 5 for numbered command files, or 2 when
running with the \fB-m\fR option or when running on one machine.
...
[Field = L]
type = I
usage = Limit to the number of jobs started per one-second cycle (default 20)
tooltip = 
manpage = Set the limit on the number of jobs started sequentially during one
cycle of checking jobs and machines.  The time between cycles is 1 second.
The default is 20.
...
[Field = I]
type = CH
usage = Command to send to the queue for initialization at the start
tooltip = 
manpage = When running on a queue, this option can be used to specify a
command that will be issued to the queue before submitting jobs, such as for
allocating nodes.  It can have embedded spaces.

[Field = D]
type = CH
usage = Command to send to the queue for deinitialization before exiting
tooltip = 
manpage = When running on a queue, this option can be used to specify a
command that will be issued to the queue before exiting.  It can have
embedded spaces.

[Field = W]
type = I
usage = Seconds to wait for queue initialization command to finish (default 30)
tooltip = 
manpage = Seconds to wait for the queue initialization command given with the
\fB-I\fR to finish.  Enter -1 to wait indefinitely, or a positive value up to
2000000.  The default is 30, or a value set with the environment variable
IMOD_QUEUE_INIT_WAIT.  An entry here overrides a value set with that variable.
