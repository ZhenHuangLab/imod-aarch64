...
surfaces, 1 or -1 for fiducials on one surface, or
...
^The default is 2.  When 1 or -1 is entered, the program will determine
whether the tomograms are upside-down from each other (i.e., inverted in
depth) by assuming that the tomograms have the same handedness.

enum.-2.tooltip = Select this option to use ONLY models of corresponding points
to find the transformation between tomograms.
enum.-1.tooltip = Select this option when the fiducials lie on one surface
...
[Field = InvertedInDepth]
short = inverted
type = I
usage = 1 if tomograms are inverted in depth dimension, 0 if they are not
tooltip =
manpage = This option can be used with a \fB-surfaces\fR entry of 1 to prevent
the program from analyzing whether tomograms are inverted by assuming that
they have the same handedness. Enter 1 if tomograms are inverted in the depth
dimension, or 0 if they are not.  Tomograms are inverted if features at high Z
in one tomogram are at low Z in the other; this property is independent of
whether they are mirror images of each other.  This entry should be needed
only if tomograms are in fact mirror images.
