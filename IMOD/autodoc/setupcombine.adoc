...
only if this extension cannot be determined from files in the dataset.

[Field = ChunkedHDFFiles]
short = chunked
type = I
usage = 1 to do volcombine output directly to a chunked HDF file
tooltip = 
manpage = Enter 1 to make the output from multiple Combinefft(1) runs go
directly into a single chunked HDF file.  Tests on local and networked
file systems gave slower results with this approach.
