...
xcorr_pt, autofidseed, transferfid, cryoposition, newst_3dfind, blend_3dfind,
tilt_3dfind_reproject, tilt_3dfind,  findbeads3d, golderaser, or sirtsetup.
If this option is not entered, the first (not second) non-option argument is
taken as the output file.
...
files in the dataset and is needed to produce the particular command file.

[Field = StackExtension]
short = stackext
type = CH
usage = Extension of raw stack excluding period
tooltip = 
manpage = Extension of raw stack excluding the period. This entry is needed
only if this extension cannot be determined from files in the dataset and is
needed to produce the particular command file.
...
short = thickness
type = I
usage = Unbinned thickness of the reconstruction, required for tilt_3dfind and
cryoposition
tooltip = 
manpage =
...
short = find
type = I
usage = Value to set for -find option in cryoposition
tooltip = 
manpage = Value to set for -find option in cryoposition, should generally be 0
or 2.

[Field = UseGPU]
short = gpu
type = I
usage = Value to set for -gpu option in cryoposition
tooltip = 
manpage =
