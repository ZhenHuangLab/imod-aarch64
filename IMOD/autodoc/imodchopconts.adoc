...
-1 is entered and the \fB-number\fR option is not, the length will be the
default for breaking up contours from patch tracking in Etomo, which is the Z
size in the model divided by 5, but no less than 16.  The default if neither
this option
nor \fB-number\fR is entered is to leave contours as they are.  Unless this option
is entered with a -1, it cannot be entered with \fB-number\fR.
...
each new contour.  This option cannot be entered if \fB-length\fR is entered
