...
integer value is output.  Floating point output is the default unless the
environment variable MODEL2POINT_INTEGERS is set to any value.  This entry is
still useful for overriding the environment variable setting.

[Field = IntegerOutput]
short = integer
type = B
usage = Output model coordinates as integer, not floating point
tooltip = 
manpage = Output model coordinates by taking the nearest 
integer instead of outputting floating point numbers, which is the default
behavior.  This option cannot be entered with \fB-float\fR.
...
tooltip = 
manpage = Output coordinates in the model coordinate system, scaled by the
original image pixel size in Angstroms and offset by origin values.  The
default is to output index coordinates corresponding to the pixel coordinates
in the image.
