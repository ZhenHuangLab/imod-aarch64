...
[Field = AlignedPieceCoordFile]
short = aligned
type = FN
usage = File for coordinates of input pieces adjusted by their alignment shifts
tooltip = 
manpage = Output file for aligned coordinates of input pieces, namely the original
coordinates plus the alignment shift for each piece, rounded to the nearest
integer.
