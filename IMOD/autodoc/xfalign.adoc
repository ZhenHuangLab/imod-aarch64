...
tooltip =
manpage = Size of image area to analyze in X and Y in both Xfsimplex(1) and
Tiltxcorr(1).  The default is to analyze the whole image, with Xfsimplex(1)
ignoring the amount around the edges specified by the \fB-matt\fR option.
...
tooltip =
manpage = When analyzing a subarea, this entry specifies the offset
...
tooltip =
manpage = In Xfsimplex(1), omit areas of images near their edges.  Enter a
...
manpage = Factor to reduce images by in Xfsimplex(1).  Image reduction is done
with antialias filtering.  The default is 2.
...
tooltip =
manpage = Filter images before searching in Xfsimplex(1) with the given sigma1, sigma2,
...
tooltip = 
manpage = Apply edge-detecting Sobel filter to each image in Xfsimplex(1).
...
tooltip =
manpage = In Xfsimplex(1), search for the given number of semi-natural parameters
...
The default is 0, in which case Xfsimplex(1) searches
...
(the \fB-prexcorr\fR option must also be entered in this case).
...
tooltip =
manpage = With this option, Xfsimplex(1) will use bilinear rather than nearest neighbor
...
usage = Sigmas and radii for filter used in initial cross-correlation and warping
tooltip =
manpage = Change the filter that is used in cross-correlation by tiltxcorr(1) to
have the given sigma1, sigma2, radius1, radius2.  This filter is applied both
in initial cross-correlations and when finding warp transformations.  The
values are in the frequency units of the images being correlated after image
reduction, if any.  The default is 0.01,0.05,0,0.25.

[Field = XcorrReduction]
short = xcreduce
type = I
usage = Image reduction to apply in initial cross-correlations and warping
tooltip =

manpage = Image reduction to apply in the initial cross-correlations and when
finding warp transformations.  For the initial cross-correlations, the default
in Tiltxcorr(1) is to make the reduced size be no bigger than 1250 pixels up
to a reduction of 4, then to increase reduction up to 16 to keep the reduced
size under 4100.  There is no reduction by default when finding warping.  The
program will specify antialias filtering when the anticipated or specified
reduction is 4 or higher.
...
cross-correlation if the \fB-prexcorr\fR option
...
short = seed
type = FN
usage = Model with points specifying location of warp patches 
tooltip =
manpage = Model with points specifying location of patches to correlate for
warping, instead of using patches on a regular array.  All points in scattered
point objects will be used.  Points can be defined on multiple sections; for
any given section, the points from the nearest section with points will be used.
...
short = wreduce
type = I
usage = Image reduction to apply to warp patches
tooltip =
manpage = Image reduction to apply when using patch correlation to find
warping.  The default in Tiltxcorr(1) is to reduce patches to be no bigger
than 1250 pixels up to a reduction of 4, then to increase reduction up to 16
to keep the reduced size under 4100. The program will specify antialias
filtering when the anticipated or specified reduction is 4 or higher.
...
be present in a file of initial alignments entered with the \fB-initial\fR option.
...
found by Xfsimplex(1) for each pair of images.
...
tooltip =
manpage = Section numbers in the \fB-skip\fR and \fB-break\fR section list entries are
