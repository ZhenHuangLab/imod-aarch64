...
long = MegaVoxels
type = F
usage = Maximum number of megavoxels to allow in each piece (default 80)
tooltip = 
manpage = 
...
more than 100.  If the maximum for Y is set to 1, a value of 0 for X will make
the number of pieces in X be no more than 100.  These constraints will allow 
...
give a maximum total in X and Y of 100)
tooltip = 
...
Y will make the number of pieces in Y be no more than 100.
...
short = hdf
type = B
usage = Set up for direct output to a chunked HDF file
tooltip = 
manpage = Set up for direct output to a chunked HDF file.  The program will
generate equally spaced chunks on each axis with a size that minimizes the
wasted space in the last chunk. 
