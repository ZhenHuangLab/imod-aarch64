...
peak based on differences (the default is %{default} SDs).
...
[Field = SearchIterations]
short = iterations
type = I
default = 3
usage = Number of times to iterate search for peaks
tooltip = Number of times to iterate search for peaks (default 3).
manpage = Number of times to iterate search for peaks.  For a given section,
the iterations will be terminated after an iteration with no changes.
Moreover, the program will keep
track of which scan regions have changes on each iteration and stop redoing
regions that have had no changes. The default is %{default}.
...
tooltip =
manpage = Skip points in the model with contour values below threshold
...
which includes terms in x, y, x**2, y**2 and x*y.  The order can be between -1
and 3, where 0 will simply replace the pixels with the mean of the border
points instead of fitting to them, and -1 will fill the area with noise
matching the standard deviation in surrounding pixels.
