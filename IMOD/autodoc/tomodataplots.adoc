...
11-13: clip stat, 14: tilt series X/Y/Z, 15-19: ctfplotter)
tooltip =
...
^  15-19: 15 for defocus, 16 for astigmatism, 17 for axis of astigmatism, 
18 for phase shift, and 19 for cuton frequency in a defocus file from
Ctfplotter(1).
^  20-22: 20 for raw and smoothed shifts, 21 for weighted mean residual, and
22 for maximum of maximum weighted residuals in an Alignframes(1) log file.
...
short = append
type = CH
usage = String to add to key on right (@time for time, @file for filename)
tooltip =
manpage = String to append to the key on the right.  Enter "@time" for a
timestamp (HH:MM:SS) or "@file" for the input filename.

[Field = SizeOfPlot]
short = size
type = IP
usage = Size of graphics window in X and Y, in pixels
tooltip =
manpage = 

[Field = PositionOfPlot]
short = position
type = IP
usage = Position of top left corner of graphics window in X and Y
tooltip =
manpage = Coordinates of the top left corner of the graphics window in X and Y, in
pixels, where Y is 0 at the top of the screen.
