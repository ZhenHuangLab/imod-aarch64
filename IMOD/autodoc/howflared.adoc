...
unpaired wall, or 2 for a right wall.  If the \fB-nopairs\fR is also entered,
there will be an additional output file with positions of flaring start points
along the axis, as described above.

[Field = OutputRotatedZ]
short = zout
type = B
usage = Output Z values after rotation into a plane along with X and Y
tooltip = 
manpage = Output Z values after rotation into a plane along with X and Y in the
point output file.
