...
short = defhue
type = B
usage = Assign default colors for all groups without a specific hue entry
tooltip =
manpage = Assign default colors for all groups without a specific hue entry.
The default colors are a subset of 12, including black, that are chosen from 
the stock colors for their distinguishability.  Each group without a color
specified by \fB-hue\fR will be assigned one of these default colors, with the
colors used in order and cycled through again if more than 12 are needed.
