...
int new_view(MidasView *vw)
{
  ...
  vw->truezoom = 1.;
  vw->zoomind = 5;
  vw->binToZoomDown = numOMPthreads(16) >= 4; 
  vw->mousemoving = 0;
  vw->useFixed = 0;
...
{
  int k, ix, ixy, iy, ind, ipclo, ipchi, ned, targetSize;
  QString str;
  int maxedges, nsect, nxypc, maxpieces, nalong, ncross;
  float diff, minDiff;
  const char *targetVar;

  if (load_image(vw, fname))
    return(-1);

  // If no binning entered, check for variable for automatic binning to target size
  if (!vw->binning) {
    targetVar = getenv("MIDAS_BIN_TO_TARGET");
    if (targetVar) {
      targetSize = atoi(targetVar);
      for (ind = 1; ind <= 8; ind++) {
        diff = fabs(targetSize - sqrt((float)vw->hin->nx * vw->hin->ny) / ind);
        if (ind == 1 || diff < minDiff) {
          minDiff = diff;
          vw->binning = ind;
        }
      }
  ...
  if (vw->binning > 1) {
    vw->unbinnedBuf = (unsigned char *)malloc((size_t)vw->hin->nx * vw->hin->ny);
    if (!vw->unbinnedBuf)
      midas_error("Could not allocate memory for unbinned buffer", "Exiting", 3);
  }
  for(k = 0; k < vw->cachesize; k++){
    vw->cache[k].sec = sliceCreate(vw->xsize, vw->ysize, 
                                        MRC_MODE_BYTE);
    if (!vw->cache[k].sec)
      midas_error("Could not get memory for slices in cache", "Exiting", 3);
    vw->cache[k].zval = -1;
  ...
  if (vw->refname) {
    ...
    if (!vw->ref)
      midas_error("Could not get memory for reference slice ", "Exiting", 3);
...
int midas_transform(int zval, Islice *slin, Islice *sout, float *trmat, int izwarp)
{
  ...
  unsigned char umean;
  int nxLim, nyLim, timeOutput = 0;
  int ixStart, ixEnd, nxGrid, nyGrid, ixgStart, ixgEnd, iygStart, iygEnd, numThreads;
...
#pragma omp parallel for num_threads(numThreads)                        \
  shared(xdx, xdy, ydx, ydy, VW, xbase, ybase, umean, nxLim, nyLim, xsize, ysize) \
  private(j, x, y, xStart, xEnd, xLeft, xRight, ixEnd, ixStart, buf, i, ix, iy) \
  private(fx, fy, xline, yline, index)
    for (j = 0; j < ysize; j++) {
      xline = xbase + j * ydx;
      yline = ybase + j * ydy;
