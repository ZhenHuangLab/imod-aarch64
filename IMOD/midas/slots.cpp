/*
 *  slots.cpp -- Slot functions for Qt signals, and actions that they cause
 *
...
void MidasSlots::slotFilemenu(int item)
{
  ...
  case FILE_MENU_SAVE: /* save transforms */
    if (save_transforms())
      midas_error("Error saving to file. ", "Transforms not saved", 0);
    VW->midasWindow->setTitleBar();
    break;
...
void MidasSlots::slotUseBinning(bool state)
{
  VW->binToZoomDown = state;
  VW->midasGL->draw();
}
...
#ifdef WATCH_DPI_CHANGE
void MidasSlots::screenChanged(QScreen *screen)
{
  float trueZoom, diff, newDPR, minDiff = 10000.;
  int minInd, ind;
  QWindow *winHandle =  VW->midasWindow->window()->windowHandle();
  if (winHandle) {
    newDPR = winHandle->devicePixelRatio();
    if (newDPR > 0.) {
      for (ind = 0; ind < MAX_ZOOMIND; ind++) {
        trueZoom = zooms[ind] < 0. ?  1. / zooms[ind] : zooms[ind];
        diff = fabs(trueZoom - VW->truezoom * newDPR / VW->devicePixelRatio);
        if (diff < minDiff) {
          minDiff = diff;
          minInd = ind;
        }
      }
      diff = newDPR / VW->devicePixelRatio;
      VW->devicePixelRatio = newDPR;
      slotZoom(minInd - VW->zoomind);
      if (VW->origHeight) {
        if (mTimerID)
          killTimer(mTimerID);
...
#ifdef Q_OS_MACX
      VW->midasGL->fakeResize(B3DNINT(VW->width * diff), B3DNINT(VW->height * diff));
#endif
    }
  }
}
...
void MidasSlots::extendTimerIfActive(int interval)
{
  if (mTimerID) {
    killTimer(mTimerID);
...
void MidasSlots::timerEvent(QTimerEvent *e)
{
  killTimer(mTimerID);
  mTimerID = 0;
  VW->midasWindow->resize(VW->midasWindow->width(), VW->origHeight);
}
