...
#define XY_PIXEL_TO_DEVICE(px, py, dx, dy)        \
    dx = B3DNINT((px) * VW->devicePixelRatio); \
    dy = B3DNINT((py) * VW->devicePixelRatio);
#else
#define XY_PIXEL_TO_DEVICE(px, py, dx, dy) dx = (px); dy = (py);
#endif
...
typedef struct Midas_view
{
  ...
  bool binToZoomDown;  // Flag to zoom down by binning
