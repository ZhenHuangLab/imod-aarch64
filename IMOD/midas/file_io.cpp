  ...
  if (vw->xtype != XTYPE_MONT) {
    ...
    if (warpFile < 0) {
      ...
      if (!err) {
        ...
        // If the original pixel size was 1 and the scaling is implausibly big or not an
        // integer multiple, error out instead of going into infinite loops.
        if (pixelSize == 1.0 && 
            (xscale > 20. || (xscale > 3.0 && fabs(B3DNINT(xscale) - xscale) > 0.02))) {
          midas_error("It looks like the pixel spacing of the image file was set "
                      "after the warping was computed.\n\n", "Edit the transform file and"
                      " change the fifth value on the second line to the new pixel "
                      "spacing in Angstroms of the file that was analyzed.", 0);
          err = -8;
        }
