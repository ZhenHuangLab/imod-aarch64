...
void MidasGL::draw()
{
  // QT 5 gives "QOpenGLContext::swapBuffers() called with non-exposed window" without:
#ifdef Q_OS_LINUX
  static int wait = 1;
  if (wait) {
    wait--;
...
// 12/11/19: Linux Qt 5.12 message about swap buffers cannot be suppressed by canceling
// draw before startup is done
...
void MidasGL::paintGL()
{
  ...
  glLineWidth(VW->devicePixelRatio + 0.001);
			 ...
			 int llx, int lly, int urx, int ury, int *xdrawn, int *ydrawn)
{
  ...
  if (vw->zoom < 0) {
    ...
    needSamp = 1;
    if (VW->binToZoomDown) {
      linePtrs = makeLinePointers(image, xsize, ysize, 4);
      if (!linePtrs) {
        printf("Error making line pointers for antialiasing\n");
      } else {
        i = selectZoomFilter(0, zoom, &j);
        if (i) {
          printf("Error %d selecting filter for antialiasing\n", i);
        } else {
          i = zoomWithFilter(linePtrs, xsize, ysize, xstart, ystart, xdrawsize, ydrawsize,
                             xdrawsize, 0, 
                             -SLICE_MODE_RGB - (VW->vmode == MIDAS_VIEW_SINGLE ? 1 : 0),
                             vw->sdat, NULL, NULL);
          if (i)
            printf("Error %d doing antialias filter\n", i);
          else
            needSamp = 0;
        }
        free(linePtrs);
...
void MidasGL::mouseMoveEvent ( QMouseEvent * e )
{
  ...
  VW->mousemoving = 1;
  XY_PIXEL_TO_DEVICE(e->x(), e->y(), VW->mx, eyt);
  VW->my = VW->height - eyt;
