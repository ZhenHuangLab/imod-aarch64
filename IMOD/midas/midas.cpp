...
int main (int argc, char **argv)
{
  ...
  int sampleSlices = -1;
  float maxDPR;
  int newWidth, newHeight, xleft, ytop, devWidth, devHeight;
  ...
  VW->devicePixelRatio = diaGetAppDevPixRatioInfo(maxDPR, VW->devPixVaries);
  VW->startupDone = false;
  ...
  diaAdjustFontForDPI(vw->devicePixelRatio, Midas_debug != 0);
  ...
  vw->midasWindow->setTitleBar();

  // Show now, if there is more than one screen, process events to get it onto its
  // final screen before adjusting the location and dimensions 
  vw->midasWindow->show();
...
#ifdef WATCH_DPI_CHANGE
  QWidget *topWind = vw->midasWindow->window();
  if (topWind) {
    topWind->winId();
    QWindow *winHandle = topWind->windowHandle();
    if (winHandle) {
      vw->devicePixelRatio = winHandle->devicePixelRatio();
      vw->midasWindow->connect((QWidget *)winHandle, SIGNAL(screenChanged(QScreen *)),
                                vw->midasSlots, SLOT(screenChanged(QScreen *)));
    }
  }
#endif
  
  // set window width from current width plus image width
  // Use the full image size, not scaled to Qt pixels, because the zoom adjustment will 
  // try to get the zoom to be near the device/pixel ratio anyway
  QSize comSize = vw->midasWindow->mCol->sizeHint();
  QSize winSize = vw->midasWindow->sizeHint();
  ...
  diaLimitWindowSize(newWidth, newHeight, vw->midasWindow);

  QRect pos = vw->midasWindow->geometry();
  xleft = pos.x();
  ytop = pos.y();
  diaLimitWindowPos(newWidth, newHeight, xleft, ytop, vw->midasWindow);

  vw->midasWindow->resize(newWidth, newHeight);
  vw->midasWindow->move(xleft, ytop);
  vw->origHeight = newHeight;

  // This should be a good thing, because widgets were all initialized with
  // extreme numbers
  vw->midasWindow->mOuter->setFixedWidth(commandWidth);

  // Zoom down so it fits.
  XY_PIXEL_TO_DEVICE(newWidth - commandWidth, newHeight - menu, devWidth, devHeight);
  if (vw->devicePixelRatio > 1.3)
    vw->midasSlots->slotZoom(1);
  if (vw->devicePixelRatio > 1.8)
    vw->midasSlots->slotZoom(1);
  while ((vw->xsize * vw->truezoom > 1.1 * devWidth ||
          vw->ysize * vw->truezoom > 1.1 * devHeight) && vw->zoomind > 0)
    vw->midasSlots->slotZoom(-1);
  
  // For edges, shift them so the whole edge shows
  XY_PIXEL_TO_DEVICE(newWidth / 2, newHeight / 2, devWidth, devHeight);
  ...
  : QMainWindow(parent, f)
{
  ...
  mOuter = new QWidget(mainbox);
  mainlay->addWidget(mOuter);
  mCol = new QVBoxLayout(mOuter);
  mCol->setSpacing(4);
  mCol->setContentsMargins(3, 3, 3, 3);
  ...
  mainlay->setStretchFactor(mCol, 0);
  mainlay->setStretchFactor(VW->midasGL, 1);

  createSectionControls(mCol);
  makeSeparator(mCol, 2);
  createContrastControls(mCol);
  makeSeparator(mCol, 2);
  createZoomBlock(mCol);
  makeSeparator(mCol, 1);
  createViewToggle(mCol);
  makeSeparator(mCol, 2);
  createParameterDisplay(mCol);
...
void MidasWindow::moveEvent(QMoveEvent *e)
{
  if (VW->devPixVaries)
    VW->midasSlots->extendTimerIfActive(500);
}
...
void MidasWindow::createZoomBlock(QVBoxLayout *parent)
{
  ...
  QCheckBox *check = diaCheckBox("Zoom down by binning", NULL, parent);
  check->setChecked(VW->binToZoomDown);
  QObject::connect(check, SIGNAL(toggled(bool)), VW->midasSlots,
                   SLOT(slotUseBinning(bool)));
  check->setToolTip("When zooming below 1, reduce noise (aliasing) by binning pixels"
                    " instead of sampling");

  check = diaCheckBox("Interpolate", NULL, parent);
...
void MidasWindow::setTitleBar(void)
{
  QString str = "midas";
  if (VW->xname) {
    QFileInfo file(VW->xname);
    str += QString(": ") + file.fileName();
  }
  setWindowTitle(str);
}
