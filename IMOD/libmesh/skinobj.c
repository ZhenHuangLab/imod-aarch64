                              ...
                              double meshDiameter)
{
  ...
  for (co = 0; co < obj->contsize; co++) {
    ...
    for (pt = 0; pt < cont->psize; pt++) {
      ...
      if (!pt && (flags & IMESH_MK_CAP_DOME && !contProps.noCap)) {
        domeCont1 = makeDomeConts(&clst[pt], &cont->pts[pt], &nrot, scale,
                                  tubeDiameter, -1., &numDome1, &domeCap1);
        domeRot1 = nrot;
      }
      if (pt == cont->psize - 1 && (flags & IMESH_MK_CAP_DOME) && !contProps.noCap) {
    ...
    if ((flags & IMESH_MK_CAP_DOME) && numDome1 && !contProps.noCap) {
    ...
    } else if (flags & IMESH_MK_CAP_TUBE && !contProps.noCap) {
    ...
    if ((flags & IMESH_MK_CAP_DOME) && numDome2 && !contProps.noCap) {
...
#pragma omp parallel for default (none) num_threads(numThreads)         \
  ...
  if (doCap) {
    int skipZ;
    int inside, contState, surfState;
    DrawProps contProps, defProps;
    istoreDefaultDrawProps(obj, &defProps);
    for (coNum = 0; coNum < obj->contsize; coNum++) {
      cont = &(obj->cont[coNum]);
      inside = 0;
      istoreContSurfDrawProps(obj->store, &defProps, &contProps, coNum, cont->surf,
                              &contState, &surfState);
      if (contProps.noCap)
        continue;
