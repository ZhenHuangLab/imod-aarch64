...
/*!
 * Returns device/pixel ratio for the application if Qt version 5.6+, as well as the 
 * maximum ratio for all screens in [maxDPR] and sets [devPixVaries] to 1 if the ratio 
 * varies, 0 otherwise.
 */
...
/*!
 * Scales up the size of the application font for Windows by the square root of the
 * device/pixel ratio in [devPixRatio], or scales it down by the device/pixel ratio 
 * for Linux if QT_SCALE_FACTOR is set, prints the original and scaled font sizes if 
 * [debug] is non-zero, and returns the scaling factor applied
 */
