...
inline void setFloatZValue( Icont *cont, float newZValue );
...
int cont_breakContByZValue( Icont *contOrig, vector<IcontPtr> &contSegs, float zValue, bool removeOffSegments );  // NEW
...
//------------------------
//-- Returns true if the contour's open flag is set.
...
//------------------------
//-- Sets the contour's open flag ("ICONT_OPEN") to on or off.
...
//------------------------
//-- Changes the z value of all the points to the given integer value
...
//------------------------
//-- Changes the z value of all the points to the given floating point value

inline void setFloatZValue( Icont *cont, float newZValue )
{
  for (int i=0; i<psize( cont ); i++)
    getPt( cont, i )->z = newZValue;
  imodContourSetFlag( cont, ICONT_WILD, 0 );
  //imodel_contour_check_wild(cont)
}
