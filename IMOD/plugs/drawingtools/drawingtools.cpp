...
int imodPlugKeys(ImodView *vw, QKeyEvent *event)
{
  ...
  int source = ivwGetPlugEventSource();
  if ((!plug.modelInSlicer && source != ZAP_WINDOW_TYPE) ||
      (plug.modelInSlicer && source != SLICER_WINDOW_TYPE))
    return (0);
  ...
  {
    case Qt::Key_Q:
      plug.window->changeSculptCircleRadius( ((shift) ? -5 : -1) * (ctrl ? -1 : 1) );
      plug.window->drawExtraObject(true);
      break;
    case Qt::Key_W:
      if (shift && plug.modelInSlicer) {
        keyhandled = 0;
      } else {
    ...
    case Qt::Key_Space:
      if( plug.drawMode==DM_CURVE && !shift && !ctrl ) {
        ...
        if (cont && psize(cont) > 2) {
          pcxf_transformCurrentContour(MODEL_TO_PLANAR);
          edit_executeCurve( true );
          plug.window->drawExtraObject(true);
        }
      } else
    ...
    case Qt::Key_9:      plug.window->changeModeSelected( 8 );       break;
...
void imodPlugExecute(ImodView *inImodView)
{
  ...
  // 7/1/22: removed apparently erroneous objection
  if (!ivwGrayScaleImageLoaded(inImodView) && 
      ivwGetImageStoreMode(inImodView) != MRC_MODE_RGB) {
    wprint("\aDrawing Tools will not work on blank data\n");
    return;
  }
  ...
  plug.slicerForwardMat = imodMatNew(3);
  plug.slicerInverseMat = imodMatNew(3);
  plug.currContPoints = NULL;
  plug.currContXformed = false;
  plug.maxTempPoints = 0;
  plug.tempPoints = NULL;
...
int imodPlugEvent(ImodView *vw, QEvent *event, float imx, float imy)
{
  ...
  if (event->type() == QEvent::Wheel)
  {
    ...
    if( wheelEvent->modifiers() & Qt::ControlModifier )     // if [ctrl] is down:
    {
      float zoom;
      int error;
      if (plug.modelInSlicer)
        error = ivwGetTopSlicerZoom(plug.view, &zoom);
      else
        error = ivwGetTopZapZoom(plug.view, &zoom);
      if( error != 1 )
      {
                  ...
                  int but1, int but2, int but3)
{
  ...
  if (plug.modelInSlicer && (plug.drawMode==DM_LIVEWIRE || plug.drawMode==DM_WAND))
    return (0);
  ...
  if (plug.modelInSlicer) {
    int noSlicer = ivwGetTopSlicerMouse(plug.view, &plug.mouse);
    if(noSlicer == 1)
      return (0);
    pcxf_updateSlicerTransforms();
    pcxf_transformForSlicer(&plug.mouse, MODEL_TO_PLANAR);
  ...
  plug.window->drawExtraObject(false, true);
  ...
  {
    if (plug.modelInSlicer) {
      pcxf_transformCurrentContour(REVERT_MODEL);
      return (2);
    }
    float zapZoom = 1.0f;
      ...
      if( isContValid(cont) )
      {
        if( !plug.shiftDown )
        {
          if ( plug.but2Down )            // move currently selected contour
          {
            ...
            {
              pcxf_registerCurrentContDataChg(cont);
      ...
      if( plug.but2Pressed && isCurrContValid() )
			{
				...
				if( isMouseInRange )
				{
					...
					{
            pcxf_registerCurrentContDataChg(cont);        // REGISTER UNDO
						cont_copyPts( newCont, cont, true );
            pcxf_transformCurrentContour(PLANAR_TO_MODEL);
						ivwRedraw( plug.view );
					}
					free(newCont);
				}
        ...
        edit_executeCircleEnd();
			...
			else if( isCurrContValid() && but3Used && !plug.modelInSlicer)
			{
        // little correct circle
				float correctCircleRadius = MAX(plug.sculptRadius / 5.0f, 5.0f);
				
        // DNM: This is worthless without the intermediate in a real sculpt push
				if( plug.but3Pressed )
					undoContourDataChgCC( plug.view );      // REGISTER UNDO
				if ( plug.but3Down )
					edit_executeSculptPush( plug.mouse, correctCircleRadius );
  ...
  if (plug.modelInSlicer) {
    pcxf_transformCurrentContour(PLANAR_TO_MODEL);
    imodel_contour_check_wild(imodContourGet(imod));
  }
  ivwDraw( plug.view, IMOD_DRAW_MOD | IMOD_DRAW_NOSYNC );    
  return (1);
}
  ...
  DialogFrame(parent, 0, NULL, NULL, true, "Drawing Tools", "", name)
{
  ...
  connect(typeButtonGroup, SIGNAL(buttonClicked(int)), this, SLOT(changeMode(int)));
	...
	typeTooltip[DM_CURVE]     = "Click points and then have the straight line segments turn"
                               " into a curve when you connect back to the first point";
  ...
  QCheckBox *slicerBox = diaCheckBox("Model in Slicer", this, mLayout);
  diaSetChecked(slicerBox, plug.modelInSlicer);
  connect(slicerBox, SIGNAL(toggled(bool)), this, SLOT(doSlicerToggled(bool)));
  slicerBox->setToolTip("Draw and modify contours in Slicer, not Zap window");
  ...
  moreActionsButton->setToolTip( "Contains several other actions I didn't "
                                 "want to squeeze into this window");
  ...
  moreSettingsButton->setToolTip(
                "Contains several other settings I didn't want to squeeze "
                "into this window");
...
bool DrawingTools::drawExtraObject( bool redraw, bool keepCurrXformed )
{
  Icont *cont = NULL;
  if ( !plug.window )
    return false;
  ...
  imodObjectSetValue(xobj,  IobjFlagExtraInSlicer, (plug.modelInSlicer)?1:0);
  imodObjectSetValue(xobjT, IobjFlagExtraInSlicer, (plug.modelInSlicer)?1:0);
  ...
  imodContourSetFlag(xcont2, ICONT_CURSOR_LIKE | ICONT_MMODEL_ONLY, 1);
  imodContourSetFlag(xcont3, ICONT_CURSOR_LIKE | ICONT_MMODEL_ONLY, 1);
  setOpenFlag(xcont,  0);
  ...
  if (plug.modelInSlicer)
    ivwGetTopSlicerZoom(plug.view, &zapZoom);
  else
    ivwGetTopZapZoom(plug.view, &zapZoom); 
  ...
  if( plug.drawZhint && !plug.modelInSlicer)
  {    
  ...
  // Now get contour for those who need it
  if (plug.drawMode == DM_WARP || plug.drawMode == DM_TRANSFORM || 
      plug.drawMode == DM_CURVE || plug.drawMode == DM_CORRECT ||
      plug.drawMode == DM_MEASURE) {
	...
	case (DM_MEASURE):            // measure distance
    {
      ...
      if( plug.but2Down )
      {
        ...
        if( ABS(yDiff) < ABS(xDiff) ) {
          if (yDiff*xDiff>0)
            textPos.y -= (slicesSpanned ? 2.5 : 2.) * fontSize;
          else
            textPos.y += (slicesSpanned ? 3.5 : 3.) * fontSize;
        } else
          textPos.x += fontSize;
        
        QString str;
        str.sprintf("%.5g %s", unitDist, unitsChs);
        string text = toString( unitDist ) + " " + toString( unitsChs );
        float spacing = 1.35 * fontSize;
        cont_addLabeledPtToObj(xobjT, textPos.x, textPos.y + spacing * 
                               (0.5 + (slicesSpanned ? 0.5 : 0)), z, str.toLatin1());
        str.sprintf(" (%.2f pixels)", dist3D);
        cont_addLabeledPtToObj(xobjT, textPos.x, textPos.y + spacing *
                               (-0.5 + (slicesSpanned ? 0.5 : 0)), z, str.toLatin1());
        
        if( slicesSpanned ) {
          string text = " spanning " + toString( slicesSpanned ) + " slices";
          cont_addLabeledPtToObj(xobjT, textPos.x, textPos.y - spacing, z, text.c_str());
        }
  ...
  if (plug.modelInSlicer) {
    for (int co = 0; co < imodObjectGetMaxContour(xobj); co++)
      pcxf_transformForSlicer(imodObjectGetContour(xobj, co), 1, NULL);
    for (int co = 0; co < imodObjectGetMaxContour(xobjT); co++)
      pcxf_transformForSlicer(imodObjectGetContour(xobjT, co), 1, NULL);
    if (!keepCurrXformed)
      pcxf_transformCurrentContour(REVERT_MODEL);
  }
...
void DrawingTools::doSlicerToggled(bool state)
{
  plug.modelInSlicer = state;
}
...
void DrawingTools::reduceConts()
{
  ...
  QString toolStr = ( plug.reducePtsOpt == RD_TOL ) ?
    ...
    "\nany 3 consecutive points, else the middle point be deleted. "
  ...
  for(int c=contMin; c<=contMax && c<csize(obj); c++)
  {
    ...
    int pointsDeleted;
    pcxf_transformForSlicer(cont, MODEL_TO_PLANAR, NULL);
    ...
    pcxf_transformForSlicer(cont, PLANAR_TO_MODEL, NULL);
...
void DrawingTools::smoothConts()
{
  ...
  string msg =
    ...
    "\n a large 'tensile fraction' can "
    "\n result in poor contours.";
  
  QString toolStr =
    "The 'tensile fraction' value represents the 'curviness' "
...
void DrawingTools::keyboardSettings()
{
  ...
  ds.addCheckBox( "smart point resize mode", 
                  ...
                  "whenever an 'open' or 'scattered' object is selected. \n"
  ...
  ds.addComboBox( "sculpt resize scheme:",
                  ...
                  "\n"
                  "TIP: Adjust 'wheel resistant' to make the circle resize "
...
void DrawingTools::moreSettings()
{
  ...
  ds.addCheckBox( "reduce contour before smoothing", &plug.smoothReduceFirst,
                  ...
                  "points will be eliminated and the smoothing results more \n"
                  "dramatic." );
  ...
  ds.addSpinBox ( "# times to move pts:", 0, 50,
                   ...
                   "The number of iterations used to move points by averaging \n"
                   "the position of consecutive points using the parameters \n"
  ...
  ds.addComboBox( "show Z hint:",
                  ...
                  "Note you can also see a nice hint and quickly scroll by holding \n"
...
void DrawingTools::findContours()
{
  ...
  ds.addLabel   ( "-----\n"
                  "NOTE: Use [Y] and [Shift+Y] to advance through\n"
...
void DrawingTools::modifyRangeContours()
{
  ...
  ds.addCheckBox( "remove all point sizes", &removePtSize,
                  ...
                  "will have all its points deleted and added again" );
...
void DrawingTools::cropRangeContours()
{
  ...
  if (plug.modelInSlicer) {
    wprint("\aCropping does not work when modeling in Slicer\n");
    return;
  }
...
void DrawingTools::movePoint()
{
  ...
  if (plug.modelInSlicer) {
    wprint("\aMove Point does not work in Slicer\n");
    return;
  }
...
{  
  ...
  // Store the cut contour as a planar contour if modeled in current angles
  pcxf_updateSlicerTransforms();
  pcxf_transformCurrentContour(MODEL_TO_PLANAR);
...
{  
  ...
  if (plug.modelInSlicer) {
    if (ivwGetTopSlicerMouse(plug.view, &plug.mouse) == 1) {
      wprint("\aNo slicer is open\n");
      return;
    }
    pcxf_updateSlicerTransforms();
    pcxf_transformForSlicer(&plug.mouse, MODEL_TO_PLANAR);    
  ...
  if (plug.modelInSlicer)
    pcxf_transformForSlicer(contNew, PLANAR_TO_MODEL, NULL);
  imodel_contour_check_wild(contNew);
...
int  DrawingTools::copyCurrContToView(bool allContsOnSlice)
{
  ...
  if (plug.modelInSlicer) {
    ...
    currZ = edit_getZOfTopZap();
  }
  pcxf_transformCurrentContour(MODEL_TO_PLANAR);
  contZ = pcxf_getPlanarZ(cont, NONPLANAR_Z, true);
  if (contZ < NONPLANAR_TEST) {
    wprint("\aThe current contour is not planar in the window\n");
    pcxf_transformCurrentContour(REVERT_MODEL);
    return (0);
  }
  
  if( fabs(contZ - currZ) < 0.5 )
  {
    wprint("\aCannot copy contour to same slice\n");
    pcxf_transformCurrentContour(REVERT_MODEL);
    return 0;
  }
	...
	{
		Iobj *obj = getCurrObj();
    pcxf_makePlaneContourList(imod, contZ);
    int num = plug.planeContInds.size();
    for (int lind = 0; lind < num; lind++) 
    {
      int c = plug.planeContInds[lind];
			Icont *fromCont = getCont( obj, c );
      Icont *contNew = imodContourDup( fromCont );
      setFloatZValue( contNew, currZ );
      imodContourSetFlag(contNew, ICONT_WILD, plug.modelInSlicer ? 1 : 0);
      int newPos = edit_addContourToObj( obj, contNew, true );
      pcxf_addPlaneContourToList(getCont(obj, newPos), newPos);
      imodContourDelete(contNew);
		}
    pcxf_cleanupPlaneContourList(imod);
	}
...
void DrawingTools::customizeToolOrder()
{
	...
	ds.addComboBox( "preset:  ",
								 ...
								 "Lets you choose from several easy presets" );
	ds.setStylePrev("background-color: rgb(150, 150, 150);");			// grey
...
void DrawingTools::topCloseEvent ( QCloseEvent * e )
{
  ...
  imodMatDelete(plug.slicerForwardMat);
  imodMatDelete(plug.slicerInverseMat);
  B3DFREE(plug.currContPoints);
  B3DFREE(plug.tempPoints);
...
//----------------------------------------------------------------------------
//
//          PLANAR CONTOUR/SLICER TRANSFORMATION FUNCTIONS:
//
//----------------------------------------------------------------------------

//------------------------
//-- If modeling in slicer, gets the angles from the top slicer
//-- and computes forward (model to planar) and inverse (planar to model) transforms

void pcxf_updateSlicerTransforms()
{
  Ipoint center, negCen;
  float angles[3];
  int time;
  if (!plug.modelInSlicer)
    return;
  ...
  // The center is not a stable point around which to transform
  // It could be done around the origin, but since I already wrote trans calls,
  // let's do it around the center of the volume
  center.x = plug.xsize / 2.;
  center.x = plug.ysize / 2.;
  center.x = plug.zsize / 2.;
  negCen.x = -center.x;
  negCen.y = -center.y;
  negCen.z = -center.z;
  imodMatId(plug.slicerForwardMat);
  imodMatId(plug.slicerInverseMat);
  imodMatTrans(plug.slicerForwardMat, &negCen);
  imodMatRot(plug.slicerForwardMat, (double)angles[b3dZ], b3dZ);
  imodMatRot(plug.slicerForwardMat, (double)angles[b3dY], b3dY);
  imodMatRot(plug.slicerForwardMat, (double)angles[b3dX], b3dX); 
  imodMatTrans(plug.slicerForwardMat, &center);
  imodMatTrans(plug.slicerInverseMat, &negCen);
  imodMatRot(plug.slicerInverseMat, -(double)angles[b3dX], b3dX); 
  imodMatRot(plug.slicerInverseMat, -(double)angles[b3dY], b3dY);
  imodMatRot(plug.slicerInverseMat, -(double)angles[b3dZ], b3dZ);
  imodMatTrans(plug.slicerInverseMat, &center);
}

//------------------------
//-- Transforms a single point in place if modeling in slicer

void pcxf_transformForSlicer(Ipoint *pnt, int inverse)
{
  Ipoint outPnt;
  if (!plug.modelInSlicer)
    return;
  imodMatTransform3D(inverse ? plug.slicerInverseMat : plug.slicerForwardMat, pnt,
                     &outPnt);
  *pnt = outPnt;
}

//------------------------
//-- Transforms a contour in the indicated direction, placing its points either into
//-- outPoints if non-NULL, or back into the contour

void pcxf_transformForSlicer(Icont *cont, int inverse, Ipoint *outPoints)
{
  Ipoint outPnt;
  Ipoint *pts;
  int ind, num;
  Imat *mat = inverse ? plug.slicerInverseMat : plug.slicerForwardMat;
  if (!plug.modelInSlicer || !cont)
    return;
  num = psize(cont);
  pts = imodContourGetPoints(cont);
  if (outPoints) {
    for (ind = 0; ind < num; ind++)
      imodMatTransform3D(mat, &pts[ind], &outPoints[ind]);

  } else {
    for (ind = 0; ind < num; ind++) {
      imodMatTransform3D(mat, &pts[ind], &outPnt);
      pts[ind] = outPnt;
    }
  }
}
...
//------------------------
//-- Transforms the current contour or restores original values depending on 
//-- inverseOrRestore: 0 for forward, 1 for inverse, 2 to restore original values

void pcxf_transformCurrentContour(int inverseOrRestore)
{
  Ipoint *temp;
  bool swap = false;
  if (!plug.modelInSlicer || (inverseOrRestore == MODEL_TO_PLANAR && 
                              plug.currContXformed) ||
      (inverseOrRestore != MODEL_TO_PLANAR && !plug.currContXformed))
    return;
  Imod *imod = ivwGetModel(plug.view);
  Icont *cont = imodContourGet(imod);
  if (!cont)
    return;
  if (inverseOrRestore == MODEL_TO_PLANAR) {
    plug.numCurrContPts = psize(cont);
    plug.currContPoints = B3DMALLOC(Ipoint, plug.numCurrContPts);
    if (!plug.currContPoints)
      return;
    pcxf_transformForSlicer(cont, MODEL_TO_PLANAR, plug.currContPoints);
    swap = true;
  } else if (inverseOrRestore == PLANAR_TO_MODEL) {
    pcxf_transformForSlicer(cont, PLANAR_TO_MODEL, NULL);
  } else if (plug.currContPoints) {
    swap = true;
  }
  if (swap) {
    temp = imodContourGetPoints(cont);
    imodContourSetPointData(cont, plug.currContPoints, plug.numCurrContPts);
    plug.currContPoints = temp;
  }
  if (inverseOrRestore != MODEL_TO_PLANAR) {
    B3DFREE(plug.currContPoints);
  }
  plug.currContXformed = inverseOrRestore == MODEL_TO_PLANAR;
}

//------------------------
//-- Gets a Z value only for any planar contour if matchZ is NONPLANAR_Z, or only for 
//-- one that is planar and matches matchZ for values > NONPLANAR_TEST.
//--  In the former case the min and max Z must be with PLANAR_TOL (1); in the
//-- latter case every Z must be within PLANAR_TOL / 2 of matchZ.
//-- Returns the planar Z or NONPLANAR_Z if it fails the test.

float pcxf_getPlanarZ(Icont *cont, float matchZ, bool alreadyXformed)
{
  const float PLANAR_TOL = 1.0;
  int ind, num = psize(cont);
  Ipoint *pts = imodContourGetPoints(cont);
  float zmin = 1.e30, zmax = -1.e30;
  if (!num)
    return NONPLANAR_Z;
  if (plug.modelInSlicer && !alreadyXformed) {

    // Get temp array if not big enough
    if (num > plug.maxTempPoints) {
      free(plug.tempPoints);
      plug.maxTempPoints = num + 10;
      plug.tempPoints = B3DMALLOC(Ipoint, plug.maxTempPoints);
      if (!plug.tempPoints) {
        plug.maxTempPoints = 0;
        return NONPLANAR_Z;
      }
    ...
    // Transform into temp array and test after every point
    for (ind = 0; ind < num; ind++) {
      imodMatTransform3D(plug.slicerForwardMat, &pts[ind], &plug.tempPoints[ind]);
      ACCUM_MIN(zmin, plug.tempPoints[ind].z);
      ACCUM_MAX(zmax, plug.tempPoints[ind].z);
      if (zmax - zmin > PLANAR_TOL ||
          (matchZ > NONPLANAR_TEST && fabs(plug.tempPoints[ind].z -matchZ) > 
           PLANAR_TOL / 2.))
        return NONPLANAR_Z;
    }
  } else {

    // Or if already transformed or modeling in Zap, just test existing points
    for (ind = 0; ind < num; ind++) {
      ACCUM_MIN(zmin, pts[ind].z);
      ACCUM_MAX(zmax, pts[ind].z);
      if (zmax - zmin > PLANAR_TOL || (matchZ > NONPLANAR_TEST &&
                                       fabs(pts[ind].z - matchZ) > PLANAR_TOL / 2.))
        return NONPLANAR_Z;
    }
  }
  return (zmax + zmin) / 2.;
...
//------------------------
//-- Makes list of planar contours at the given Z, excluding the current contour
//-- and transforms all contours on the list when modeling in slicer

void pcxf_makePlaneContourList(Imod *imod, float curZ)
{
  int ind, ob, co, pt;
  Icont *cont;
  Ipoint *pts;
  Iobj *obj = imodObjectGet(imod);
  if (plug.planeContInds.size() && fabs(plug.planeContLastZValue - curZ) < 0.01)
    return;
  imodGetIndex(imod, &ob, &co, &pt);
  
  // Loop on all contours testing if planar when (possibly) transformed
  for (ind = 0; ind < csize(obj); ind++) {
    if (ind != co) {
      cont = getCont(obj, ind);
      if (pcxf_getPlanarZ(cont, curZ, false) > NONPLANAR_TEST) {

        // If found one, save its index etc; if modeling in slicer, save the existing 
        // points and adopt the temp array as the new point array
        plug.planeContInds.push_back(ind);
        pts = NULL;
        if (plug.modelInSlicer) {
          pts = imodContourGetPoints(cont);
          imodContourSetPointData(cont, plug.tempPoints, psize(cont));
          plug.tempPoints = NULL;
          plug.maxTempPoints = 0;
        }
        plug.planeContPoints.push_back(pts);
        plug.planeContXformed.push_back(plug.modelInSlicer);
      }
    }
  }
  plug.planeContLastZValue = curZ;
}

//------------------------
//-- Adds a newly made planar contour the list, storing back-transformed points so it
//-- is ready to restore

void pcxf_addPlaneContourToList(Icont *cont, int index)
{
  Ipoint *backPoints;
  int num = psize(cont);
  plug.planeContInds.push_back(index);
  backPoints = NULL;
  if (plug.modelInSlicer) {
    if (num) {
      backPoints = B3DMALLOC(Ipoint, num);
      if (backPoints)
        pcxf_transformForSlicer(cont, PLANAR_TO_MODEL, backPoints);
    }
  }
  plug.planeContPoints.push_back(backPoints);
  plug.planeContXformed.push_back(plug.modelInSlicer && num);
}

//------------------------
//-- Removes the planar contour at the given index from the list and adjusts contour
//-- numbers for higher numbered contours

void pcxf_removedPlaneContour(int listInd)
{
  if (listInd < 0 || listInd >= plug.planeContInds.size())
    return;
  int contInd = plug.planeContInds[listInd];
  plug.planeContInds.erase(plug.planeContInds.begin() + listInd);
  free(plug.planeContPoints[listInd]);
  for (int ind = 0; ind < plug.planeContInds.size(); ind++)
    if (plug.planeContInds[ind] > contInd)
      plug.planeContInds[ind]--;
}

//------------------------
//-- Computes back-transformed for a changed contour so it is ready to restore

void pcxf_changedPlaneContour(int listInd, Icont *cont)
{
  if (listInd < 0 || listInd >= plug.planeContInds.size() || !plug.modelInSlicer)
    return;
  int num = psize(cont);
  free(plug.planeContPoints[listInd]);
  if (!num)
    return;
  plug.planeContPoints[listInd] = B3DMALLOC(Ipoint, num);
  if (plug.planeContPoints[listInd])
    pcxf_transformForSlicer(cont, PLANAR_TO_MODEL, plug.planeContPoints[listInd]);
}

//------------------------
//-- Swaps current and original point pointers so that an undo accesses original data
//-- listInd <0 means do it for the current contour

void pcxf_swapPointsToRegisterUndo(int listInd, Icont *cont)
{
  if (!plug.modelInSlicer || !cont || listInd >= (int)plug.planeContInds.size() || 
      (listInd >= 0 && !plug.planeContPoints[listInd]) ||
      (listInd < 0 && !plug.currContXformed))
    return;
  Ipoint *pts = listInd < 0 ? plug.currContPoints : plug.planeContPoints[listInd];
  Ipoint *temp = imodContourGetPoints(cont);
  if (!temp || !pts)
    return;
  imodContourSetPointData(cont, pts, psize(cont));
  if (listInd < 0)
    plug.currContPoints = temp;
  else
    plug.planeContPoints[listInd] = temp;
}

//------------------------
//-- Registers the undo for a change in the data of the current contour

void pcxf_registerCurrentContDataChg(Icont *cont)
{
  pcxf_swapPointsToRegisterUndo(-1, cont);
  undoContourDataChgCC(plug.view);
  pcxf_swapPointsToRegisterUndo(-1, cont);
}

//------------------------
//-- Restores the plane contours still on the list and frees the list

void pcxf_cleanupPlaneContourList(Imod *imod)
{
  Iobj *obj = imodObjectGet(imod);
  for (int ind = 0; ind < plug.planeContInds.size(); ind++) {
    Icont *cont = getCont(obj, plug.planeContInds[ind]);
    int num = psize(cont);
    if (plug.planeContPoints[ind] && plug.planeContXformed[ind] && num > 0) {
      Ipoint *pts = imodContourGetPoints(cont);
      imodContourSetPointData(cont, plug.planeContPoints[ind], num);
      free(pts);
    } else
      free(plug.planeContPoints[ind]);
  }
  plug.planeContInds.clear();
  plug.planeContPoints.clear();
  plug.planeContXformed.clear();
}
...
bool edit_selectNearPtInCurrObj( Ipoint *centerPt, float distTol, float distTolCurrCont,
                                 bool countInsideCurrCont )
{
  ...
  int c, objIdx, contIdx, ptIdx, indStart = 0;
  imodGetIndex(imod, &objIdx, &contIdx, &ptIdx);

  //## IF THERE IS A POINT SUFFICIENTLY CLOSE IN THE CURRENT CONTOUR: SELECT IT
  
  
  if( isContValid(currCont) && pcxf_getPlanarZ(currCont, centerPt->z, true) > 
      NONPLANAR_TEST && !isEmpty(currCont) )
  {
    ...
    // Include the current contour in the loop below if there was minimal tolerance
    if (distTolCurrCont <= 1.)
      indStart = -1;
  }
  ...
  int closestPtIdx = -1;
  pcxf_makePlaneContourList(imod, centerPt->z);
  for(int ind = indStart; ind < (int)plug.planeContInds.size(); ind++) //for each contour:
  {
    if (ind < 0)
      c = contIdx;
    else
      c = plug.planeContInds[ind];
    Icont *cont = getCont(obj, c);
    for( int p=0; p<psize(cont); p++ )
      {
...
bool edit_copiedContIfDiffSlice( bool selectNewCont )
{
  ...
  if(! isContValid(cont) )
    return false;
  float contZ = pcxf_getPlanarZ(cont, NONPLANAR_Z, true);
  if (contZ < NONPLANAR_TEST )
    return false;
  if (fabs(contZ - plug.mouse.z) > 0.5)
  {
    Icont *contNew = imodContourDup(cont);
    Ipoint shift;
    shift.x = 0;
    shift.y = 0;
    shift.z = plug.mouse.z - contZ;
...
//## (IF NO SUITABLE CONTOUR WAS FOUND) ADD THE SCULPT CIRCLE AS A NEW CONTOUR
  ...
  else
  {
    ...
    plug.currContXformed = plug.modelInSlicer;
    cont = imodContourGet(imod);
  }

  // For join, make list of contours inside and outside current one before starting
  if (makeInsideLists) {
    pcxf_makePlaneContourList(imod, plug.mouse.z);
    //int zSlice = imodContourZValue( cont );
    int objIdx, contIdx, ptIdx, i, lind;
    imodGetIndex(imod, &objIdx, &contIdx, &ptIdx);
    plug.outsideCurCont.clear();
    plug.insideCurCont.clear();
    for (lind = 0; lind < plug.planeContInds.size(); lind++) {
      i = plug.planeContInds[lind];
      Icont *contCompare = getCont(obj, i);
      if (i == contIdx || isEmpty(contCompare))
        continue;
...
void edit_executeSculpt()
{
  ...
  else										// push
  {
    ...
    if( numIntermediates > 10 ) {
      wprint("\aYou are moving the mouse too fast!\n");
      return;
    }
...
void  edit_executeSculptPush( Ipoint center, float radius )
{
  ...
  if( isContValid(cont) && pcxf_getPlanarZ(cont, center.z, true) > NONPLANAR_TEST)
  {
    const float POINT_SHIFTED = NONPLANAR_Z;
    ...
    {
      bool thisPtShifted = ( getPt(cont,i)->z   < NONPLANAR_TEST);
      bool nextPtShifted = ( getPt(cont,i+1)->z < NONPLANAR_TEST);
...
{   
  ...
  if( isStartPtInsideCont && isEndPtInsideCont )
  {
    ...
    {
      wprint("\aYou must span between the inside of two separate contours\n");
      return;
    }
		
    pcxf_transformForSlicer(startCont, MODEL_TO_PLANAR, NULL);
    vector<IcontPtr> cont1Segs;
    int n1Segs = cont_breakContourByContour( cont1Segs, startCont, contR, radius*0.5f );
    pcxf_transformForSlicer(startCont, PLANAR_TO_MODEL, NULL);
    ...
    else
    {
      ...
      undoContourPropChg( plug.view, objIdx, startContIdx );
      imodContourClear( startCont );
      
      undoContourPropChgCC( plug.view );                  // REGISTER UNDO
      setInterpolated( endCont, false );
      pcxf_registerCurrentContDataChg( endCont );      // REGISTER UNDO
  ...
  else if( !isStartPtInsideCont && !isEndPtInsideCont )
  {
    ...
    int nSegs = cont_breakContourByContour( contSegs, getCont(obj,startContIdx), contR, 
                                            10.0f );
    ...
    {
      wprint("\aRectangle must only intersect two segments of contour\n");
    }
...
void edit_executeWarpEnd()
{
  ...
  if (plug.reducePts)
  {
    pcxf_registerCurrentContDataChg( imodContourGet(imod) );      // REGISTER UNDO
    edit_reduceCurrContour();
  }
...
void edit_executeCurve( bool forceEndContour )
{
  ...
  if (!psize(cont)) {
    undoContourDataChgCC( plug.view );
    imodPointAppendXYZ( cont, plug.mouse.x, plug.mouse.y, plug.mouse.z );
    plug.currContXformed = plug.modelInSlicer;
    pt = &plug.mouse;
  }
  ...
  if( pcxf_getPlanarZ(cont, plug.mouse.z, true) < NONPLANAR_TEST && 
      isContClosed(obj,cont) )
  {
    wprint("\aWrong slice\n");
    return;
...
int edit_eraseContsInCircle( Ipoint center, float radius )
{
  ...
  for (int o=0; o<osize(imod); o++ )
  {
    ...
    pcxf_makePlaneContourList(imod, plug.mouse.z);
    for (int lind = 0; lind < plug.planeContInds.size(); lind++) 
      {
        int c = plug.planeContInds[lind];
        Icont *cont = getCont(obj, c);
        ...
        for(int p=0; p<psize(cont); p++)    // for each point in that contour:
        {
          ...
          {
            //imodSetIndex( imod, objIdx, c, p );
            pcxf_swapPointsToRegisterUndo(lind, cont);
            undoContourRemoval( plug.view, o, c );        // REGISTER UNDO
            pcxf_swapPointsToRegisterUndo(lind, cont);
            imodObjectRemoveContour( obj, c );        // delete the contour
            pcxf_removedPlaneContour(lind);
            lind--;
            numRemoved++;
...
int edit_erasePointsInCircle( Ipoint center, float radius )
{
  ...
  for (int o=0; o<osize(imod); o++ )
  {
    ...
    if ( imodObjectGetValue(obj, IobjFlagPlanar) )
    {
      pcxf_makePlaneContourList(imod, plug.mouse.z);
      for (int lind = 0; lind < plug.planeContInds.size(); lind++) 
      {
        ...
        if( minDistToPt <= radius )
          {
            ...
            // Undo here was flaky, maybe fixed 3/16/21 by not flushing
            pcxf_swapPointsToRegisterUndo(lind, cont);
            undoContourDataChg(plug.view, o, c);
            undoContourPropChg(plug.view, o, c);
            pcxf_swapPointsToRegisterUndo(lind, cont);
            numPtsRemoved += cont_removePointsInCircle( cont, &center, radius, false );
            pcxf_changedPlaneContour(lind, cont);
          }
      }
      pcxf_cleanupPlaneContourList(imod);
    }
    else if (!plug.modelInSlicer)
    {
      for(int c=0; c<csize(obj); c++)    // for each contour:
      {
        Icont *cont = getCont(obj, c);
...
bool edit_breakPointsInCircle( Ipoint center, float radius )
{
  ...
  for (int o=0; o<osize(imod); o++ )
  {
    ...
    pcxf_makePlaneContourList(imod, plug.mouse.z);
    int listSize = plug.planeContInds.size();
    for (int lind = 0; lind < listSize; lind++) 
    {
      int c = plug.planeContInds[lind];
      Icont *cont = getCont(obj, c);
      ...
      if( minDistToPt <= radius )
      {
        imodSetIndex( imod, o, c, 1);
          ...
          {
            // Put first segment points into original contour
            pcxf_swapPointsToRegisterUndo(lind, cont);
            undoContourDataChg(plug.view, o, c);                  // REGISTER UNDO
            pcxf_swapPointsToRegisterUndo(lind, cont);
            ...
            // Add new contours for the rest
            for( int i=1; i<contSegments.size(); i++ ) {
              imodContourSetFlag(contSegments[i].cont, ICONT_WILD,
                                 plug.modelInSlicer ? 1 : 0);
              int newPos = edit_addContourToObj( obj, contSegments[i].cont, true );
              pcxf_addPlaneContourToList(getCont(obj, newPos), newPos);
            }
          
            undoContourPropChg(plug.view, o, c);                    // REGISTER UNDO
          setOpenFlag( cont, 1 );
...
void edit_breakCurrContIntoSimpleContsAndDeleteSmallest ()
{
  ...
  if( isContValid(cont) )
  {
    ...
    // Start with a copy: this will become part of the contour list and either be
    // the one returned on that list, or be deleted if it is broken up
    Icont *curCopy = imodContourDup(cont);  
    vector<IcontPtr> conts;
    cont_breakIntoSimple( conts, curCopy );    // breaks contour into simple contours.
    ...
    //  If there are none left, clear out the current contour
    if (!conts.size()) {
      imodContourClear(cont);
    } else {

      // Otherwise transfer the points from the first one to the current contour
      cont_copyPts(conts[0].cont, cont, true);

    //## ADD NEW CONTOURS TO GRID IF MORE THAN ONE
    
      for( int i=1; i<(int)conts.size(); i++ )
      {
        imodContourSetFlag(conts[i].cont, ICONT_WILD, plug.modelInSlicer ? 1 : 0);
        int newPos = edit_addContourToObj( obj, conts[i].cont, true );
        pcxf_addPlaneContourToList(getCont(obj, newPos), newPos);
      }
    }
...
void edit_joinCurrContWithAnyTouching()
{
  ...
  for (int ind = 0; ind < plug.planeContInds.size(); ind++)
  {
    int i = plug.planeContInds[ind];
    Icont *contCompare = getCont(obj, i);
    
    if( isEmpty(contCompare) )
      continue;
...
{ 
  ...
  Ipoint closestPt;
  int closestPtIdx;
  cont_findClosestPtInContToGivenPt( givenPt, cont, &closestDist, &closestPt,
                                     &closestPtIdx  );
...
void edit_executeLivewireClick()
{
  ...
  ivwGetTopZapZoom(plug.view, &zapZoom);
	...
	bool startNew = (pt==NULL) || ((int)pt->z != mZ) || numLivewirePts == 0;
  ...
  float distFirstLWPt = 0.;
	QPoint qptEnd = QPoint( mX, mY );
  float zapDPR = ivwGetTopZapDevPixelRatio(plug.view);
    ...
    // DNM: Scale to screen pixels
		if( distFirstLWPt <= zapDPR * LW_SNAP_DIST / zapZoom || 
        zapDPR * distLastLWPt <= LW_SNAP_DIST / zapZoom )
			finishContour = true;
	}
    ...
    if (!(plug.lwUseWrap && finishContour && distLastLWPt < 2. * zapDPR))
															 ...
															 QPoint qptEnd, int z, bool addUndo, bool reverse )
{
	...
	if( ptsAdded==0)
	{
    setPt( &newPt, (float)qptEnd.x(), (float)qptEnd.y(), z );
		if( append )
		{
			if(addUndo)
				undoPointAdditionCC( plug.view, psize(cont) );			// REGISTER UNDO
...
bool edit_startLivewireFromPt( int x, int y, int z, bool useLivewireF )
{
	...
	if( x < 0 ) x = 0;
  else if( x >= plug.xsize ) x = plug.xsize-1;
	if( y < 0 ) y = 0;
  else if( y >= plug.ysize ) y = plug.ysize-1;
	if( z < 0 ) z = 0;
  else if( z >= plug.zsize ) z = plug.zsize-1;
...
bool edit_executeLivewireSelectPt()
{
  ...
  // DNM: This was commented out. Sure hope this is the right thing to return
	return true;
}
