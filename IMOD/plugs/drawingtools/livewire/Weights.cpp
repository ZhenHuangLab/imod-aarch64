...
Weights::Weights() : Threaded("Weights"),
	_filter_overflow(CalcFilterOverflow(this->_settings)),
	_width_raw(0), _height_raw(0), _stride(0), _data_raw(NULL),
	_width(0), _height(0), _scale(WINDOW_SIZE(this->_settings.PixelReduction)), _data(NULL),
	_status(NULL) { }
...
inline void Weights::UpdatedScaleOrSize()
{
	if (this->_width_raw || this->_height_raw)
	{
		...
		this->_data   = (byte**)memset(realloc(this->_data, wh_s*sizeof(byte*)), 0, wh_s*sizeof(byte*));
...
void Weights::CalcBlock(uint x_s, uint y_s, uint I)
{
	...
	const bool has_l = (bool)x_s, has_t = (bool)y_s, has_r = x_s != this->_width_status-1, has_b = y_s != this->_height_status-1;
	const bool short_right = !has_r && ((w & (BLOCK_SIZE - 1)) != 0);
	const bool short_bottom = !has_b && ((h & (BLOCK_SIZE - 1)) != 0);
