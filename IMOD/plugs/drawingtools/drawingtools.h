...
enum xftypes  {MODEL_TO_PLANAR = 0, PLANAR_TO_MODEL, REVERT_MODEL};
...
const int PIX_OFF         = -1;
const float NONPLANAR_Z   = -1.e31f;
const float NONPLANAR_TEST = -1.e30f;
...
public:
  ...
  bool drawExtraObject( bool redraw, bool keepCurrXformed = false );
	bool drawExtraObjectLivewire( bool redraw );
	bool drawExtraObjectWand( bool redraw );
	
  void keepOnTop( bool state );
  void doSlicerToggled(bool state);
...
struct DrawingToolsData   // contains all local plugin data
{
  ...
  bool   modelInSlicer;         // Flag to model in top slicer window instead of zap
  ...
  //## SLICER-RELATED
  Imat *slicerForwardMat;     // For converting image coords to coords in slicer plane
  Imat *slicerInverseMat;     // For converting coords in slicer plane to image coords
  Ipoint *currContPoints;     // Original current contour points
  int numCurrContPts;         // Number of them
  bool currContXformed;       // Flag that contour is already transformed
  Ipoint *tempPoints;         // Points for transforming while determining Z
  int maxTempPoints;          // Current allocation of tempPoints
  vector<int> planeContInds;         // Coplanar contours one same plane as current one
  vector<bool> planeContXformed;     // Flag for whether transformed
  vector<Ipoint *> planeContPoints;  // Original points if transformed
  float planeContLastZValue;         // Last Z value for which planar contours listed
};
...
//-------------------------------
//## PLANAR CONTOUR/TRANSFORMATION FUNCTIONS
