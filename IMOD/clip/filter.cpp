...
int clip_bandpass_filter(MrcHeader *hin, MrcHeader *hout, ClipOptions *opt)
{
  ...
  if ((hin->mode != MRC_MODE_COMPLEX_FLOAT) && (hin->mode != MRC_MODE_COMPLEX_SHORT)) {
    if ((!clip_nicesize(opt->ix)) ||
        (!clip_nicesize(opt->iy)) || opt->ix % 2){
      if (usingFFTW())
        printf("ERROR: clip - filter input size in X (%d) must be even: use Mtffilter.\n",
               opt->ix);
      else
        printf("ERROR: clip - filter input size (%d, %d) is odd and/or has factors"
               " greater than 19: use Mtffilter.\n", opt->ix, opt->iy);
      return(-1);
    }
  }
