...
int set_mrc_coords(ClipOptions *opt)
{
  ...
  /* DNM 9/21/01: subtract rather than add tx, ty, tz, because of weird
     ...
     4/15/12: Argh!  It needs to multiply by the scale;  9/17/15, no longer. */
...
{
  int z = 0;
  int limits[3] = {0, 0, 0}, tileSizes[3], numTiles[3];

  /* create a new file */
  if (!opt->add2file){
    mrc_head_new(hout, opt->ox, opt->oy, opt->oz, opt->mode);
    if (setChunkOutput(opt, hout, limits, numTiles, tileSizes))
      return -1;
...
/*
 * Sets up chunk output in an HDF file if there were chunk option entries
 */
int setChunkOutput(ClipOptions *opt, MrcHeader *hout, int *limits, int *numTiles,
                   int *tileSizes)
{
  float chunkInXY, chunkLimit = 3.e9;
  int ind, dsize, csize, maxZ, outSizes[3];
  ImodImageFile *iiFile = iiLookupFileFromFP(hout->fp);
  tileSizes[0] = opt->chunkX;
  tileSizes[1] = opt->chunkY;
  tileSizes[2] = opt->chunkZ;
  outSizes[0] = hout->nx;
  outSizes[1] = hout->ny;
  outSizes[2] = hout->nz;
  for (ind = 0; ind < 3; ind++) {
    if (tileSizes[ind] == IP_DEFAULT || tileSizes[ind] >= outSizes[ind]) {
      numTiles[ind] = 1;
      tileSizes[ind] = ind == 2 ? 1 : 0;
    } else {
      iiLimitedTileSize(outSizes[ind], &tileSizes[ind], &numTiles[ind], 2, limits[ind]);
    }
  }
  if (numTiles[0] * numTiles[1] * numTiles[2] == 1)
    return 0;
  if (!iiFile) {
    fprintf(stderr, "iiLookupFileFromFP cannot find iiFile from fp\n");
    return -1;
  }
  mrc_getdcsize(hout->mode, &dsize, &csize);
  chunkInXY = (float)(tileSizes[0] ? tileSizes[0] : outSizes[0]) * 
    (tileSizes[1] ? tileSizes[1] : outSizes[1]) * csize * dsize;
  maxZ = (int)(chunkLimit / chunkInXY);
  if (maxZ < 1) {
    fprintf(stderr, "Output tile or image size in X and Y too large to fit in one chunk;"
            " enter smaller chunk sizes\n");
    return -1;
  }
  if (maxZ == 1) {
    tileSizes[2] = 1;
    numTiles[2] = hout->nz;
  } else if (tileSizes[2] > maxZ || (opt->chunkZ == IP_DEFAULT && outSizes[2] > maxZ)) {
    tileSizes[2] = maxZ;
    iiLimitedTileSize(outSizes[ind], &tileSizes[2], &numTiles[2], 2, maxZ);
  } else if (opt->chunkZ == IP_DEFAULT) {
    tileSizes[2] = outSizes[2];
  }
  if (iiSetChunkSizes(iiFile, tileSizes[0], tileSizes[1], tileSizes[2]))
    return -1;
  printf("Chunk size set to %d %d %d\n", tileSizes[0], tileSizes[1], tileSizes[2]);
  return 0;
}
