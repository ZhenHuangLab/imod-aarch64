...
/**
 * <p>Description: Builds a list of directives to use in the directive editor.</p>
 * 
 * <p>Copyright: Copyright 2013 - 2017 by the Regents of the University of Colorado</p>
 * 
...
public final class DirectiveEditorBuilder {
  ...
  private final List<DirectiveDescrSection> sectionArray =
    new ArrayList<DirectiveDescrSection>();
  ...
  public StringBuffer build(final AxisType sourceAxisType, StringBuffer errmsg) {
    ...
    DirectiveDescrFile.Iterator descrIterator =
      DirectiveDescrFile.INSTANCE.getIterator(manager, AXID_ID);
    if (descrIterator != null) {
      ...
      while (descrIterator.hasNext()) {
        DirectiveDescrElement element = descrIterator.nextElement();
          ...
          if (directive.isValid()) {
            ...
            if (!matchesType
              && (type != DirectiveFileType.BATCH
                && (directive.isTemplate() || !directive.isBatch()))
              || (type == DirectiveFileType.BATCH
                && (!directive.isTemplate() || directive.isBatch()))) {
              matchesType = true;
            }
    ...
    DirectiveMap.Iterator iterator =
      directiveMap.keySet(DirectiveType.COM_PARAM).iterator();
    AxisID firstAxisID;
    ...
    while (iterator.hasNext()) {
      ...
      // save default values
      commandMapDefaults =
        getCommandMap(directiveName, comFileDefaults, commandMapDefaults, errmsg, true);
  ...
  private boolean updateFromLocalDirectiveFile(final DirectiveFileType type,
    ...
    if (type != null && (directiveFile = type.getLocalFile(manager, AXID_ID)) != null
      ...
      try {
        ReadOnlyStatementList statementList =
          AutodocFactory.getInstance(manager, directiveFile, false);
        if (statementList != null) {
          ...
          while ((statement = statementList.nextStatement(location)) != null) {
            if (statement.getType() == Statement.Type.NAME_VALUE_PAIR) {
              ...
              directive = directiveMap.getDirective(directiveName.getKey());
              if (directive == null) {
                ...
                if (directive.isValid()
                  && directive.getType() == DirectiveType.COM_PARAM) {
              ...
              // Set the value from the file. This value will be overridden by subsequent
              // templates, and then from the coms or from the non-generic code in the
              // manager.
              DirectiveValueType valueType = directive.getValueType();
              String value = statement.getRightSide();
        ...
        e.printStackTrace();
        errmsg.append("Unable to load " + directiveFile.getName() + ".  " + e.getMessage()
          + "  ");
      }
  ...
  private void setDefaultDirectiveValue(final Map<String, String> commandMap,
    ...
    if (commandMap.containsKey(fromDirectiveName.getParameterName())) {
      Directive toDirective = directiveMap.getDirective(toDirectiveName.getKey());
  ...
  private void setDirectiveValue(final Map<String, String> commandMap,
    ...
    if (commandMap.containsKey(directiveName.getParameterName())) {
      Directive directive = directiveMap.getDirective(directiveName.getKey());
  ...
  /**
   * If the type is USER, will attempt to create the user directory if necessary.
   * @return
   */
  public File getDefaultSaveLocation() {
    ...
    if (type == DirectiveFileType.USER) {
      ...
      if (!dir.exists()) {
        System.err.println("Creating user template directory:" + dir.getAbsolutePath());
        dir.mkdirs();
      }
    }
    if (dir != null) {
