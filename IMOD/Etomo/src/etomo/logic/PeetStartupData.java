...
public final class PeetStartupData {
  ...
  public void setDirectory(final String input) {
    ...
    directory =
      FilePath.buildAbsoluteFile(EtomoDirector.INSTANCE.getOriginalUserDir(), input);
  }
  ...
  public String validate() {
    ...
    if (paramFiles != null && paramFiles.length > 0 && (paramFiles.length > 1
      || !DatasetFiles.getPeetRootName(paramFiles[0].getName()).equals(baseName))) {
      return "The directory " + directory.getAbsolutePath() + " can contain only one "
        + DataFileType.PEET.extension + " file.";
    }
    if (isCopyFrom() && copyFrom.getParentFile().equals(directory) && !copyFrom.getName()
      .endsWith(Extension.EXTENSION_DIVIDER + Extension.PRM.toString())) {
