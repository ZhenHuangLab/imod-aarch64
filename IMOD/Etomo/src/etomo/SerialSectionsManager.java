...
public final class SerialSectionsManager extends BaseManager {
  ...
  SerialSectionsManager(final String paramFileName) {
    super();
    metaData = new SerialSectionsMetaData(this, getLogProperties(),
  ...
  public boolean setParamFile(final SerialSectionsStartupData startupData) {
    ...
    if (!metaData.isValid()) {
      uiHarness.openMessageDialog(this,
        "Invalid data, unable to proceed.  Please exit and restart Etomo", "Fatal Error");
      return false;
    }
    imodManager.setMetaData(metaData);
    mainPanel.setStatusBarText(paramFile, metaData, logWindow);
  ...
  private void openSerialSectionsDialog(final SerialSectionsStartupData startupData) {
    if (!loadedParamFile && startupData == null) {
      UIHarness.INSTANCE.openMessageDialog(this,
        "Failed to load the parameter file, unable to continue.", "Failed", AxisID.ONLY);
      valid = false;
      return;
    }
    if (dialog == null) {
    ...
    autoAlignmentController =
      new AutoAlignmentController(this, dialog, imodManager, metaData.getStack());
    ...
    String actionMessage =
      Utilities.prepareDialogActionMessage(DialogType.SERIAL_SECTIONS, AxisID.ONLY, null);
    if (actionMessage != null) {
      System.err.println(actionMessage);
    }
  }
  ...
  boolean startNextProcess(final UIComponent uiComponent, final AxisID axisID,
    ...
    if (process.equals(Task.ADD_OUTPUT_FORMAT)) {
      addOutputFormat(processSeries, axisID);
      return true;
    }
    return false;
  }
  ...
  public void completeStartup(final UIComponent uiComponent, final AxisID axisID) {
    ProcessSeries processSeries =
      new ProcessSeries(this, axisID, DialogType.SERIAL_SECTIONS_STARTUP);
    processSeries.setNextProcess(Task.CHANGE_DIRECTORY);
    processSeries.addProcess(Task.CREATE_COMSCRIPTS);
    processSeries.addProcess(Task.ADD_OUTPUT_FORMAT);
  ...
  public void align(final AxisID axisID, final Deferred3dmodButton deferred3dmodButton,
    ...
    ProcessSeries processSeries =
      new ProcessSeries(this, axisID, DialogType.SERIAL_SECTIONS_STARTUP);
    processSeries.addProcess(Task.XFTOXG);
  ...
  private void extractpieces(final UIComponent uiComponent, final AxisID axisID,
    ...
    if (!pieceListFile.exists()) {
      ...
      String threadName;
      try {
        param.setMdocMetadataFileEnabled(metaData.isextractPlMdocMetadataFile());
        threadName = processMgr.extractpieces(param, axisID, processSeries);
      }
      catch (AxisBusyException e) {
        e.printStackTrace();
        uiHarness.openMessageDialog(this, uiComponent,
          "Can not execute " + ExtractpiecesParam.COMMAND_NAME + "\n" + e.getMessage(),
  ...
  private void createComscripts(final UIComponent uiComponent, final AxisID axisID,
    ...
    if (getViewType() == ViewType.MONTAGE) {
      ...
      if (!FileType.PREBLEND_COMSCRIPT.getFile(this, axisID).exists()) {
        try {
          Utilities.copyFile(FileType.SLOPPY_BLEND_COMSCRIPT, FileType.PREBLEND_COMSCRIPT,
            this, axisID);
        }
        catch (IOException e) {
          e.printStackTrace();
      ...
      BlendmontParam blendmontParam =
        comScriptMgr.getBlendmontParamFromPreblend(axisID, getName());
      startupData.getPreblendParameters(blendmontParam, this);
      comScriptMgr.savePreblend(blendmontParam, axisID);
      // blend
      if (!FileType.BLEND_COMSCRIPT.getFile(this, axisID).exists()) {
        BaseProcessManager
          .touch(FileType.BLEND_COMSCRIPT.getFile(this, axisID).getAbsolutePath(), this);
      }
      comScriptMgr.loadBlend(axisID);
      blendmontParam = comScriptMgr.getBlendmontParamFromBlend(axisID, getName());
      startupData.getBlendParameters(blendmontParam, this);
      ...
      if (!FileType.NEWST_COMSCRIPT.getFile(this, axisID).exists()) {
        BaseProcessManager
          .touch(FileType.NEWST_COMSCRIPT.getFile(this, axisID).getAbsolutePath(), this);
      }
  ...
  /**
   * Add setenv IMOD_OUTPUT_FORMAT to preblend, blend, or newst comscripts where it is
   * missing.  Comscripts must exist.  If the setenv command is already there, do not
   * change it because it reflects the settings when the dataset was created.
   * @param axisID
   */
  private void addOutputFormat(final ProcessSeries processSeries, final AxisID axisID) {
    SetEnvParam param;
    if (getViewType() == ViewType.MONTAGE) {
      // preblend
      comScriptMgr.loadPreblend(axisID);
      param = comScriptMgr.getSetEnvParamFromPreblend(axisID, ImodOutputFormat.ENV_VAR);
      if (param == null) {
        // Keep the existing setting if it was set. If not then add it.
        param = new SetEnvParam(ImodOutputFormat.ENV_VAR);
        param.setValue(metaData.getImageOutputFormat().toString());
        comScriptMgr.savePreblend(param, axisID, ImodOutputFormat.ENV_VAR);
      }
      // blend
      comScriptMgr.loadBlend(axisID);
      param = comScriptMgr.getSetEnvParamFromBlend(axisID, ImodOutputFormat.ENV_VAR);
      if (param == null) {
        // Keep the existing setting if it was set. If not then add it.
        param = new SetEnvParam(ImodOutputFormat.ENV_VAR);
        param.setValue(metaData.getImageOutputFormat().toString());
        comScriptMgr.saveBlend(param, axisID, ImodOutputFormat.ENV_VAR);
      }
    }
    else {
      // newst
      comScriptMgr.loadNewst(axisID);
      param = comScriptMgr.getSetEnvParamFromNewst(axisID, ImodOutputFormat.ENV_VAR);
      if (param == null) {
        // Keep the existing setting if it was set. If not then add it.
        param = new SetEnvParam(ImodOutputFormat.ENV_VAR);
        param.setValue(metaData.getImageOutputFormat().toString());
        comScriptMgr.saveNewst(param, axisID, ImodOutputFormat.ENV_VAR);
      }
    }
    if (processSeries != null) {
      processSeries.startNextProcess(axisID);
  ...
  private ConstNewstParam updateNewstCom(final AxisID axisID,
    ...
    try {
      ...
      param
        .setTransformFile(FileType.GLOBAL_TRANSFORMATION_LIST.getFileName(this, axisID));
  ...
  public void msgPreblendSucceeded() {
    if (dialog == null) {
      return;
    }
    ...
    dialog.setPreblendReadInXcorrs(true);
  }
  ...
  private BlendmontParam updatePreblendComscript(final AxisID axisID,
    ...
    param.setBlendmontState(state.getInvalidEdgeFunctions(),
      !state.equalsPreblendRobustFitting(dialog.isPreblendRobustFitting(),
        dialog.getPreblendRobustFitting()));
  ...
  private void copyDistortionFieldFile(final ConstProcessSeries processSeries,
    ...
    if (distortionField != null) {
      ...
      if (stack != null) {
        try {
          Utilities.copyFile(distortionField,
            new File(stack.getParentFile(), distortionField.getName()));
        }
        catch (IOException e) {
    ...
    if (loadedParamFile && paramFile != null && metaData.isValid()) {
      ...
      if (getViewType() == ViewType.MONTAGE) {
        ...
        BlendmontParam param =
          comScriptMgr.getBlendmontParamFromPreblend(AXIS_ID, getName());
  ...
  public static final class Task implements TaskInterface {
    ...
    private static final Task ADD_OUTPUT_FORMAT = new Task();
