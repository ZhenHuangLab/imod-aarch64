...
public class EtomoDirector {
  public static final boolean DEFAULT_TO_STANDARD_IMAGE_FILE_NAMES = true;
  ...
  public static final boolean NEW_MESSAGE_PARSER = true;
  ...
  private static final boolean DEBUG = INSTANCE.getArguments().isDebug();
  ...
  private UITester uiTester = null;
  ...
  public static void main(String[] args) {
    System.err.println("Running Etomo...");
    System.err.println("Etomo Version:  " + ImodVersion.CURRENT_VERSION + " "
      + VersionControl.TIME_STAMP + "\n");
    EtomoDirector.INSTANCE.arguments.parse(args);
    if (!EtomoDirector.INSTANCE.arguments.isGrabIt()) {
      ...
      ProcessMessages.grabIt();
    }
  }

  public void setMaintainEtomo(final UITester uiTester) {
    this.uiTester = uiTester;
  }

  public void stopMaintainEtomo() {
    if (uiTester != null) {
      uiTester.setMaintainEtomo(false);
    }
  }

  public boolean isUnitTest() {
    return arguments.isTest() && arguments.isHeadless() && arguments.isSelfTest();
  }
  ...
  private void initialize() {
    ...
    if (Utilities.isWindowsOS() && EnvironmentVariable.INSTANCE.exists(null,
  ...
  private void setupImodCalibDir() {
    ...
    if (imodCalibDirectoryName == null) {
      ...
      if (!imodCalibDirectoryName.equals("")) {
        if (!isUnitTest() || DEBUG) {
          System.err
            .println(EnvironmentVariable.CALIB_DIR + " (env): " + imodCalibDirectoryName);
        }
      }
      else if (!isUnitTest() || DEBUG) {
        System.err
          .println("WARNING:\nThe environment variable " + EnvironmentVariable.CALIB_DIR
      ...
      if (!isUnitTest() || DEBUG) {
        System.err
          .println(EnvironmentVariable.CALIB_DIR + " (-D): " + imodCalibDirectoryName);
      }
    }
  ...
  private void initProgram() {
    ...
    if (!isUnitTest() || DEBUG) {
      System.err
        .println("GraphicsEnvironment.isHeadless()=" + GraphicsEnvironment.isHeadless());
      // print versions
    ...
    String sJavaMemoryLimit = EnvironmentVariable.INSTANCE.getValue(null, originalUserDir,
  ...
  public boolean setCurrentManager(UniqueKey key) {
    return setCurrentManager(key, false);
  }
   ...
   * @param newWindow
   */
  public synchronized boolean setCurrentManager(UniqueKey key, boolean newWindow) {
    if (key == null) {
      return false;
    }
   ...
   */
  public BaseManager openTool(ToolType toolType) {
  ...
  public ManagerKey openFrontPage(final boolean makeCurrent, final AxisID axisID,
    final ImageFilenameStyle imageFilenameStyle) {
    closeDefaultWindow(axisID);
    FrontPageManager manager = new FrontPageManager(imageFilenameStyle);
    return setManager(manager, makeCurrent);
  }
  ...
  private ManagerKey openSerialSections(File etomoSerialSectionsFile, boolean makeCurrent,
    AxisID axisID) {
  ...
  public UniqueKey openTomogram(boolean makeCurrent, AxisID axisID) {
    closeDefaultWindow(axisID);
    ManagerKey managerKey = openTomogram(MetaData.getNewFileTitle(), makeCurrent, axisID);
    if (managerKey == null) {
      return null;
    }
    return managerKey.getKey();
  }
  ...
  public void openManager(final File dataFile, final boolean makeCurrent,
    ...
    UIHarness.INSTANCE.openMessageDialog(getCurrentManager(),
      "Unknown file type " + dataFile.getName() + ".", "Unknown File Type", axisID);
    throw new IllegalStateException("unknown dataFile");
  }

  public UniqueKey openTomogram(final File etomoDataFile, final boolean makeCurrent,
    final AxisID axisID, final UIComponent uiComponent) {
    if (etomoDataFile == null) {
      ...
      if (etomoDataFile.exists()) {
        ...
        UIHarness.INSTANCE.openMessageDialog(null, uiComponent,
          "Dataset file " + etomoDataFile.getAbsolutePath() + " does not exist.",
          "Open Dataset Failed", (AxisID) null);
      }
    }
    return null;
  }

  public boolean isOpen(final UniqueKey managerUniqueKey) {
    if (managerList == null) {
      return false;
    }
    return managerList.contains(managerUniqueKey);
  }

  public void closeManagers(final List<UniqueKey> managerUniqueKeyList) {
    if (managerUniqueKeyList == null) {
      return;
    }
    int size = managerUniqueKeyList.size();
    for (int i = 0; i < size; i++) {
      if (setCurrentManager(managerUniqueKeyList.get(i))) {
        closeCurrentManager(null, false);
      }
    }
  }
  ...
  private String getPropertyUserDir() {
    return currentManagerKey == null ? originalUserDir
      : getCurrentManager().getPropertyUserDir();
  }
