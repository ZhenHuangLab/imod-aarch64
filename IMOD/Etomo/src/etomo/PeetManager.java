...
public final class PeetManager extends BaseManager {
  ...
  private final PeetScreenState screenState =
    new PeetScreenState(AXIS_ID, AxisType.SINGLE_AXIS);
  ...
  private PeetManager(final String paramFileName) {
    super();
    metaData = new PeetMetaData(this, getLogProperties(),
  ...
  public void copyDataset(final PeetStartupData startupData) {
    ...
    try {
      ...
      PeetMetaData destMetaData = new PeetMetaData(this, getLogProperties(), false);
      destParameterStore.load(destMetaData);
      PeetScreenState screenState =
        new PeetScreenState(AxisID.ONLY, AxisType.SINGLE_AXIS);
      ...
      uiHarness.openMessageDialog(this,
        "Unable to load " + destPeetFile.getName() + ".  " + e.getMessage(),
        "Unable to Continue");
      return;
    }
    ...
    // Create the new file
    File destMatlabFile =
      new File(destDir, fnOutput + DatasetFiles.MATLAB_PARAM_FILE_EXT);
    ...
    if (!sourceMatlabFile.exists()) {
      uiHarness.openMessageDialog(this, "Unable to copy .prm file. "
        + sourceMatlabFile.getAbsolutePath() + " does not exist.", "Entry Error");
      return;
    }
    try {
  ...
  public void imodAvgVol(final Run3dmodMenuOptions menuOptions) {
    ...
    List avgVolList = new AveragedFileNames().getList(this, AxisID.ONLY,
      "Must press either " + PeetDialog.RUN_LABEL + " or " + PeetDialog.AVERAGE_ALL_LABEL
        + ".",
      "Process Not Run");
    try {
  ...
  public void peetParser(ProcessSeries processSeries, final DialogType dialogType,
    ...
    if (matlabParam == null) {
      uiHarness.openMessageDialog(this,
        "Must set " + PeetDialog.DIRECTORY_LABEL + " and " + PeetDialog.FN_OUTPUT_LABEL,
        "Entry Error");
      processSeries.endSeries();
      return;
    }
  ...
  boolean startNextProcess(final UIComponent uiComponent, final AxisID axisID,
    ...
    if (process.equals(ProcessName.PROCESSCHUNKS.toString())) {
      processchunks(processSeries, process.getOutputImageFileKey(),
  ...
  private void openPeetDialog(final PeetStartupData startupData) {
    ...
    String actionMessage =
      Utilities.prepareDialogActionMessage(DialogType.PEET, AxisID.ONLY, null);
  ...
  private void openPeetStartupDialog() {
    if (peetStartupDialog == null) {
      String actionMessage =
        Utilities.prepareDialogActionMessage(DialogType.PEET_STARTUP, AxisID.ONLY, null);
  ...
  private void setPeetDialogParameters(final File importDir,
    ...
    if (paramFile != null && metaData.isValid()) {
      ...
      peetDialog.checkIncorrectPaths();
      peetDialog.updateMode(loadedParamFile);
    }
  }
  ...
  private boolean savePeetDialog(final boolean forRun, final boolean doValidation) {
    ...
    metaData.setBrowsingDirectory(getBrowsingDir());
  ...
  private void processchunks(final ProcessSeries processSeries,
    final FileKey outputImageFileKey, final ProcessingMethod processingMethod,
    ...
    try {
      ProcesschunksParam param = new ProcesschunksParam(this, AxisID.ONLY,
        peetDialog.getFnOutput(true), outputImageFileKey);
