...
public final class MatlabParam {
  ...
  private static final int WRAP_MIN_LENGTH = 72;
  private static final int WRAP_LENGTH = 80;
  private static final String SQUIGGLY_BRACKET = ParsedList.OPEN_SYMBOL.toString();
  private static final String QUOTE = ParsedQuotedString.DELIMITER_SYMBOL.toString();
  private static final String DIVIDER = ParsedList.DIVIDER_SYMBOL.toString();
  private static final String STRING_DIVIDER =
    QUOTE + ParsedList.DIVIDER_SYMBOL.toString();

  private final ParsedNumber particlePerCpu =
    ParsedNumber.getMatlabInstance(PARTICLE_PER_CPU_KEY);
  private final ParsedArray szVol = ParsedArray.getMatlabInstance(SZ_VOL_KEY);
  private final ParsedQuotedString fnOutput =
    ParsedQuotedString.getInstance(FN_OUTPUT_KEY);
  private final ParsedNumber refFlagAllTom =
    ParsedNumber.getMatlabInstance(REF_FLAG_ALL_TOM_KEY);
  private final ParsedNumber edgeShift = ParsedNumber.getMatlabInstance(EDGE_SHIFT_KEY);
  private final ParsedArray lstThresholds =
    ParsedArray.getMatlabInstance(LST_THRESHOLDS_KEY);
  private final ParsedNumber lstFlagAllTom =
    ParsedNumber.getMatlabInstance(LST_FLAG_ALL_TOM_KEY);
  private final ParsedQuotedString alignedBaseName =
    ParsedQuotedString.getInstance(ALIGNED_BASE_NAME_KEY);
  ...
  private final ParsedQuotedString referenceFile =
    ParsedQuotedString.getInstance(REFERENCE_KEY);
  private final ParsedArray reference = ParsedArray.getMatlabInstance(REFERENCE_KEY);
  private final ParsedNumber yaxisObjectNum =
    ParsedNumber.getMatlabInstance(YAXIS_OBJECT_NUM_KEY);
  private final ParsedNumber yaxisContourNum =
    ParsedNumber.getMatlabInstance(YAXIS_CONTOUR_NUM_KEY);
  private final ParsedNumber flgWedgeWeight =
    ParsedNumber.getMatlabInstance(FLG_WEDGE_WEIGHT_KEY);
  private final ParsedQuotedString sampleSphere =
    ParsedQuotedString.getInstance(SampleSphere.KEY);
  private final ParsedNumber sampleInterval =
    ParsedNumber.getMatlabInstance(EtomoNumber.Type.DOUBLE, SAMPLE_INTERVAL_KEY);
  private final ParsedQuotedString maskType =
    ParsedQuotedString.getInstance(MASK_TYPE_KEY);
  private final ParsedArray maskModelPts =
    ParsedArray.getMatlabInstance(EtomoNumber.Type.DOUBLE, MASK_MODEL_PTS_KEY);
  private final ParsedNumber insideMaskRadius =
    ParsedNumber.getMatlabInstance(EtomoNumber.Type.DOUBLE, INSIDE_MASK_RADIUS_KEY);
  private final ParsedNumber outsideMaskRadius =
    ParsedNumber.getMatlabInstance(EtomoNumber.Type.DOUBLE, OUTSIDE_MASK_RADIUS_KEY);
  private final ParsedNumber nWeightGroup =
    ParsedNumber.getMatlabInstance(N_WEIGHT_GROUP_KEY);
  private final ParsedNumber flgRemoveDuplicates =
    ParsedNumber.getMatlabInstance(FLG_REMOVE_DUPLICATES_KEY);
  private final ParsedNumber flgAlignAverages =
    ParsedNumber.getMatlabInstance(FLG_ALIGN_AVERAGES_KEY);
  private final ParsedNumber flgFairReference =
    ParsedNumber.getMatlabInstance(FLG_FAIR_REFERENCE_KEY);
  private final ParsedNumber flgAbsValue =
    ParsedNumber.getMatlabInstance(FLG_ABS_VALUE_KEY);
  private final ParsedNumber flgStrictSearchLimits =
    ParsedNumber.getMatlabInstance(FLG_STRICT_SEARCH_LIMITS_KEY);
  private final ParsedNumber flgRandomize =
    ParsedNumber.getMatlabInstance(FLG_RANDOMIZE_KEY);
  private final ParsedNumber cylinderHeight =
    ParsedNumber.getMatlabInstance(EtomoNumber.Type.DOUBLE, CYLINDER_HEIGHT_KEY);
  private final ParsedNumber maskBlurStdDev =
    ParsedNumber.getMatlabInstance(EtomoNumber.Type.DOUBLE, MASK_BLUR_STD_DEV_KEY);
  private final ParsedNumber flgVolNamesAreTemplates =
    ParsedNumber.getMatlabInstance(FLG_VOL_NAMES_ARE_TEMPLATES_KEY);
  /**
   * @deprecated
   * backwards compatibility for selectClassID, which used to be a number
   */
  private final ParsedNumber bcSelectClassID =
    ParsedNumber.getMatlabInstance(SELECT_CLASS_ID_KEY);
  private final ParsedArray selectClassID =
    ParsedArray.getMatlabInstance(SELECT_CLASS_ID_KEY);
  private final ParsedNumber flgNoReferenceRefinement =
    ParsedNumber.getMatlabInstance(FLG_NO_REFERENCE_REFINEMENT_KEY);
  ...
  public synchronized boolean read(BaseManager manager, final List<String> errorList,
    ...
    try {
      ...
      UIHarness.INSTANCE.openMessageDialog(manager,
        "Unable to load " + file.getAbsolutePath() + ".  IOException:  " + e.getMessage(),
        "File Error");
      return false;
    }
  ...
  public synchronized void write(BaseManager manager) {
    ...
    try {
      ...
      autodoc.wrapAttributeValues(QUOTE, SQUIGGLY_BRACKET + QUOTE, STRING_DIVIDER,
        DIVIDER, WRAP_MIN_LENGTH, WRAP_LENGTH);
      autodoc.write();
      ...
      UIHarness.INSTANCE.openMessageDialog(manager,
        "Unable to load " + file.getName() + ".  IOException:  " + e.getMessage(),
        "File Error");
    }
    catch (LogFile.LockException e) {
  ...
  public String getReferenceVolumeString() {
    return reference.getRawString(VOLUME_INDEX);
  }
  ...
  private void parseData(final ReadOnlyAutodoc autodoc, final List<String> errorList,
    ...
    if (!addError(refFlagAllTom, errorList)) {
      checkValue(refFlagAllTom, new int[] { 0, 1 }, component, REF_FLAG_ALL_TOM_KEY, null,
        1);
    }
    // edgeShift
    ...
    if (!addError(lstFlagAllTom, errorList)) {
      checkValue(lstFlagAllTom, new int[] { 0, 1 }, component, LST_FLAG_ALL_TOM_KEY, null,
        1);
    }
    // alignedBaseName
      ...
      if (!ok) {
        UIHarness.INSTANCE.openProblemValueMessageDialog(manager, component, "Unknown",
          ...
          replacementValueIndex != -1 && replacementValueIndex < expectedValues.length
            ? String.valueOf(expectedValues[replacementValueIndex]) : null,
          null);
      }
    }
  }
   ...
   */
  void checkValue(final ParsedNumber number, final int min, final int max, final int step,
    final UIComponent component, final String paramName, final String fieldLabel,
    ...
    if (!ok) {
      UIHarness.INSTANCE.openProblemValueMessageDialog(manager, component, "Out of range",
  ...
  private void setNameValuePairValues(BaseManager manager, final Map valueMap,
    ...
    setNameValuePairValue(manager, autodoc, SZ_VOL_KEY, (String) valueMap.get(SZ_VOL_KEY),
      commentMap);
    if (isTiltRangeEmpty()) {
  ...
  private void setNameValuePairValue(BaseManager manager, final WritableAutodoc autodoc,
    final String name, String value, final Map commentMap) {
    if (value == null) {
      return;
    }
  ...
  public static final class InitMotlCode implements EnumeratedType {
    ...
    public static final InitMotlCode X_AND_Z_AXIS =
      new InitMotlCode(2, SharedStrings.INIT_MOTL_X_AND_Z_AXIS_LABEL);
    public static final InitMotlCode RANDOM_ROTATIONS =
      new InitMotlCode(3, SharedStrings.INIT_MOTL_RANDOM_ROTATIONS);
    public static final InitMotlCode RANDOM_AXIAL_ROTATIONS =
      new InitMotlCode(4, SharedStrings.INIT_MOTL_RANDOM_AXIAL_ROTATIONS);
    ...
    private static InitMotlCode getInstance(final ReadOnlyAttribute attribute,
      ...
      UIHarness.INSTANCE.openProblemValueMessageDialog(null, component, "Unknown", KEY,
        null, SharedStrings.INIT_MOTL_LABEL, value, DEFAULT.value.toString(),
        DEFAULT.label);
      return DEFAULT;
    }
  ...
  public static final class YAxisType implements EnumeratedType {
    public static final YAxisType Y_AXIS =
      new YAxisType(new EtomoNumber().set(0), SharedStrings.YAXIS_TYPE_Y_AXIS_LABEL);
    public static final YAxisType PARTICLE_MODEL = new YAxisType(new EtomoNumber().set(1),
      SharedStrings.YAXIS_TYPE_PARTICLE_MODEL_LABEL);
    public static final YAxisType CONTOUR =
      new YAxisType(new EtomoNumber().set(2), SharedStrings.YAXIS_TYPE_CONTOUR_LABEL);
    public static final YAxisType CSV_FILES =
      new YAxisType(new EtomoNumber().set(3), SharedStrings.CSV_FILES_LABEL);
  ...
  public static final class Volume {
    ...
    private final ParsedArray tiltRange =
      ParsedArray.getMatlabInstance(EtomoNumber.Type.DOUBLE, TILT_RANGE_KEY);
    /**
     * @deprecated - only for validation
     */
    private final ParsedArray relativeOrient =
      ParsedArray.getInstance(EtomoNumber.Type.DOUBLE, RELATIVE_ORIENT_KEY,null);
    private final ParsedQuotedString fnVolume =
      ParsedQuotedString.getInstance(FN_VOLUME_KEY);
    private final ParsedQuotedString fnModParticle =
      ParsedQuotedString.getInstance(FN_MOD_PARTICLE_KEY);
    private final ParsedQuotedString initMotl =
      ParsedQuotedString.getInstance(InitMotlCode.KEY);
    private final ParsedQuotedString tiltRangeMultiAxes =
      ParsedQuotedString.getInstance(TILT_RANGE_KEY);
    ...
    private Volume(final BaseManager manager, final AxisID axisID) {
      this.manager = manager;
      this.axisID = axisID;
      relativeOrient.setBackwardCompatibleNullKey();
    }
  ...
  private static final class SearchAngleArea {
    private final ParsedArrayDescriptor descriptor =
      ParsedArrayDescriptor.getInstance(EtomoNumber.Type.DOUBLE, null);
    ...
    private void checkStart(final ParsedElement start, final ParsedElement end,
      ...
      if (bdStart.multiply(new BigDecimal(-1)).compareTo(bdEnd) != 0) {
        UIHarness.INSTANCE.openProblemValueMessageDialog(null, component, "Incorrect",
          key, "start", label, bdStart.toString(),
          bdEnd.multiply(new BigDecimal(-1)).toString(), "-end");
      }
    }
  ...
  public static final class Iteration {
    ...
    private final ParsedArray searchRadius =
      ParsedArray.getMatlabInstance(SEARCH_RADIUS_KEY);
    private final ParsedArray lowCutoff =
      ParsedArray.getMatlabInstance(EtomoNumber.Type.DOUBLE, LOW_CUTOFF_KEY);
    private final ParsedArray hiCutoff =
      ParsedArray.getMatlabInstance(EtomoNumber.Type.DOUBLE, HI_CUTOFF_KEY);
    private final ParsedNumber refThreshold =
      ParsedNumber.getMatlabInstance(EtomoNumber.Type.DOUBLE, REF_THRESHOLD_KEY);
    private final ParsedNumber duplicateShiftTolerance =
      ParsedNumber.getMatlabInstance(DUPLICATE_SHIFT_TOLERANCE_KEY);
    private final ParsedNumber duplicateAngularTolerance =
      ParsedNumber.getMatlabInstance(DUPLICATE_ANGULAR_TOLERANCE_KEY);
    ...
    private void
      setDuplicateAngularTolerance(final ParsedElement duplicateAngularTolerance) {
