...
public final class ParameterStoreTest extends TestCase {
  private static final File testDir =
    new File(StorageTests.TEST_ROOT_DIR, "ParameterStore");
  private static final File testFile = new File(testDir, "test-file");
  private static final File backupFile =
    new File(testFile.getAbsolutePath() + DatasetFiles.BACKUP_CHAR);
  ...
  public void testSave() throws LogFile.LockException, IOException {
    ...
    assertFalse(
      "(AutoStore == false) should prevent save() from writing the file.\ntestData="
        + testData + ",reloadData=" + reloadData,
      testData.equals(reloadData));
    ...
    assertFalse(
      "(AutoStore == false) should prevent save() from writing the file.\ntestMoreData="
        + testMoreData + ",reloadMoreData=" + reloadMoreData,
      testMoreData.equals(reloadMoreData));
  ...
  private final class Data implements Storable {
    ...
    public void store(final Properties props, String prepend) {
      prepend = this.prepend == null ? "" : this.prepend + '.';
      if (i == I_DEFAULT) {
        props.remove(prepend + I_KEY);
      }
      else {
        props.setProperty(prepend + I_KEY, String.valueOf(i));
      }
      if (Double.isNaN(d)) {
        props.remove(prepend + D_KEY);
      }
      else {
        props.setProperty(prepend + D_KEY, String.valueOf(d));
      }
      if (string == null) {
        props.remove(prepend + STRING_KEY);
      }
      else {
        props.setProperty(prepend + STRING_KEY, string);
      }
      number.store(props, this.prepend);
    }

    public void load(final Properties props, String prepend) {
      prepend = this.prepend == null ? "" : this.prepend + '.';
      i = Integer.parseInt(props.getProperty(prepend + I_KEY, String.valueOf(I_DEFAULT)));
      d =
        Double.parseDouble(props.getProperty(prepend + D_KEY, String.valueOf(D_DEFAULT)));
      string = props.getProperty(prepend + STRING_KEY);
      number.load(props, this.prepend);
    }
