...
import etomo.type.SubstitutionString;
...
public final class Node {
  ...
  static final String NCORES_KEY = "ncores";
  ...
  private String gpuncores = "";
  private String gpuspeed = "";
  private String gputype = "";
  private String[] gpuDeviceArray = null;
  // Queue attribute
  private String command = "";
  private SubstitutionString initialize = null;
  private String deinitialize = "";
  private EtomoNumber coresPerNode = null;
  ...
  public void getParameters(final ProcesschunksParam param) {
    if (queue) {
      param.setQueueCommand(command);
      param.setInitialize(initialize);
      param.setDeinitialize(deinitialize);
      if (coresPerNode != null) {
        param.setCoresPerNode(coresPerNode.getInt());
      }
    }
  }
  ...
  void load(final ReadOnlySection section) {
    ...
    ReadOnlyAttribute gpuAttribute = section.getAttribute(CpuAdoc.GPU_KEY);
    if (gpuAttribute != null) {
      attribute = gpuAttribute.getAttribute(MEMORY_KEY);
      if (attribute != null) {
        gpumemory = attribute.getValue();
      }
      attribute = gpuAttribute.getAttribute(NCORES_KEY);
      if (attribute != null) {
        gpuncores = attribute.getValue();
      }
      attribute = gpuAttribute.getAttribute(SPEED_KEY);
      if (attribute != null) {
        gpuspeed = attribute.getValue();
      }
      attribute = gpuAttribute.getAttribute(TYPE_KEY);
      if (attribute != null) {
        gputype = attribute.getValue();
      }
    }
    ...
    if (queue) {
      ...
      attribute = section.getAttribute("coresPerNode");
      if (attribute != null) {
        coresPerNode = new EtomoNumber();
        coresPerNode.setDisplayValue(1);
        coresPerNode.set(attribute.getValue());
      }
      attribute = section.getAttribute("initialize");
      if (attribute != null) {
        initialize = new SubstitutionString(attribute.getValue(), "nodes");
      }
      attribute = section.getAttribute("deinitialize");
      if (attribute != null) {
        deinitialize = attribute.getValue();
      }
    }
  }
  ...
  public boolean isGpuType() {
    return gputype != null && !gputype.matches("\\s*");
  }

  public boolean isGpuSpeed() {
    return gpuspeed != null && !gpuspeed.matches("\\s*");
  }

  public boolean isGpuMemory() {
    return gpumemory != null && !gpumemory.matches("\\s*");
  }

  public boolean isGpuNcores() {
    return gpuncores != null && !gpuncores.matches("\\s*");
  }
