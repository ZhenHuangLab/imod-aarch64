...
final class NameValuePair extends Statement {
  ...
  void wrapValue(final String noWrapPrefix, final String wrapPrefix,
    final String divider, final String defaultDivider, final int minLength,
    final int wrapLength) {
    if (value == null) {
      return;
    }
    String valueString = value.getMultiLineValues();
    if (valueString == null || valueString.startsWith(noWrapPrefix)) {
      return;
    }
    String matchedDivider;
    if (valueString.startsWith(wrapPrefix)) {
      matchedDivider = divider;
    }
    else {
      matchedDivider = defaultDivider;
    }
    if (Utilities.canWrap(valueString, matchedDivider, minLength, wrapLength, 0)) {
      value = new Token();
      value.set(Token.Type.ANYTHING,
        Utilities.wrap(valueString, matchedDivider, minLength, wrapLength, 0));
    }
  }
