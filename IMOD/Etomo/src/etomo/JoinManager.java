...
import etomo.comscript.JoinComscriptManager;
...
public final class JoinManager extends BaseManager {
  ...
  private JoinComscriptManager comScriptMgr;
  ...
  JoinManager(String paramFileName, AxisID axisID) {
    ...
    comScriptMgr = new JoinComscriptManager(this);
  }
  ...
  public void imodOpen(final ProcessSeries processSeries, String imodKey) {
    try {
  ...
  public SlicerAngles imodGetSlicerAngles(String imodKey, int imodIndex) {
    ...
    try {
      if (imodIndex == -1) {
        uiHarness.openMessageDialog(this,
          "The is no open " + imodKey + " 3dmod for the highlighted row.", "No 3dmod",
          AxisID.ONLY);
      }
  ...
  public void postProcess(String commandName, ProcessDetails processDetails) {
    try {
      if (commandName.equals(ProcessName.MAKEJOINCOM.toString())) {
        if (processDetails != null) {
          if (processDetails.getBooleanValue(MakejoincomParam.Fields.ROTATE)) {
            ...
            param.setTotalRows(
              processDetails.getIntValue(MakejoincomParam.Fields.TOTAL_ROWS));
            param.setRotationAnglesList(
              processDetails.getHashtable(MakejoincomParam.Fields.ROTATION_ANGLES_LIST));
          }
        }
      ...
      e.printStackTrace();
      uiHarness.openMessageDialog(this,
        "Unable to read " + DatasetFiles.XFJOINTOMO_LOG + ".\n" + e.getMessage(),
        "Read Error");
    }
    catch (IOException e) {
      e.printStackTrace();
  ...
  public void startjoin(final ProcessSeries processSeries) {
    ...
    try {
      ...
      uiHarness.openMessageDialog(this, "Can't run startjoin.com\n" + except.getMessage(),
        "SystemProcessException", AxisID.ONLY);
      return;
    }
  ...
  public void xfjointomo(final ProcessSeries processSeries) {
    ...
    try {
      ...
      uiHarness.openMessageDialog(this,
        "Can't run " + ProcessName.XFJOINTOMO.toString() + "\n" + except.getMessage(),
        "SystemProcessException", AxisID.ONLY);
      return;
    }
  ...
  private void remapmodel(final ProcessSeries processSeries) {
    ...
    try {
      ...
      uiHarness.openMessageDialog(this,
        "Can't run " + RemapmodelParam.COMMAND_NAME + "\n" + except.getMessage(),
        "SystemProcessException", AxisID.ONLY);
      return;
    }
  ...
  private void xfmodel(XfmodelParam param, ProcessSeries processSeries,
    ...
    try {
      ...
      uiHarness.openMessageDialog(this,
        "Can't run " + XfmodelParam.COMMAND_NAME + "\n" + except.getMessage(),
        "SystemProcessException", AxisID.ONLY);
      return;
    }
  ...
  private void xftoxg(ProcessSeries processSeries, final DialogType dialogType) {
    ...
    try {
      ...
      uiHarness.openMessageDialog(this,
        "Can't run " + XftoxgParam.COMMAND_NAME + "\n" + except.getMessage(),
        "SystemProcessException", AxisID.ONLY);
      return;
    }
  ...
  boolean startNextProcess(final UIComponent uiComponent, final AxisID axisID,
    ...
    if (process.equals(ImodManager.TRANSFORMED_MODEL_KEY)) {
      imodOpen(processSeries, ImodManager.TRANSFORMED_MODEL_KEY);
      return true;
    }
    if (process.equals(ProcessName.JOIN_WARP_2_MODEL.toString())) {
      refineJoin(processSeries);
      return true;
    }
    return false;
  }
  ...
  private Joinwarp2modelParam updateJoinwarp2modelParam(final boolean doValidation) {
    comScriptMgr.loadJoinWarp2Model(AxisID.ONLY);
    Joinwarp2modelParam param = comScriptMgr.getJoinwarp2modelParam(AxisID.ONLY);
    String refineModelFilename = DatasetFiles.getRefineModelFileName(this);
    param.setRefineModelFilename(refineModelFilename);
    String modeledJoinFilename = FileType.MODELED_JOIN.getFileName(this, AxisID.ONLY);
    param.setModeledJoinFilename(modeledJoinFilename);
    String inputWarpFileName =
      FileType.LOCAL_TRANSFORMATION_LIST.getFileName(this, AxisID.ONLY);
    param.setInputWarpFilename(inputWarpFileName);
    String appliedTransformFilename = FileType.WARP_XG.getFileName(this, AxisID.ONLY);
    param.setAppliedTransformFilename(appliedTransformFilename);

    if (!joinDialog.getParameters(param, doValidation)) {
      return null;
    }
    comScriptMgr.saveJoinwarp2model(param, AxisID.ONLY);
    return param;
  }
  ...
  public void startRefine() {
    ...
    ProcessSeries processSeries = new ProcessSeries(this, AxisID.ONLY, DialogType.JOIN);
    boolean useTrial = joinDialog.isRefineWithTrial();
    ...
    String buttonName = joinDialog.getButtonName(useTrial);// ask join dialog
    String joinFileName = joinFileType.getFileName(this, joinDialog.getAxisID());
    // make sure the file to be moved exists
    if (!DatasetFiles.getJoinFile(useTrial, this).exists()) {
      UIHarness.INSTANCE.openMessageDialog(this,
        joinFileName + " does not exist.  Press " + buttonName + " to create it.",
        "Failed File Move");
      processSeries.endSeries();
      return;
    }
    ...
    if (!state.isJoinVersionGe(useTrial, JoinState.MIN_REFINE_VERSION)) {
      joinDialog.setRefineDataHighlight(true);
      convertVersion = UIHarness.INSTANCE.openYesNoDialog(this,
        "IMPORTANT!!  Information about the " + joinFileName
          ...
          + buttonName + " to recreate the file.",
        AxisID.ONLY);
      joinDialog.setRefineDataHighlight(false);
      if (!convertVersion) {
        processSeries.endSeries();
        return;
      }
    ...
    if (FileType.MODELED_JOIN.getFile(this, AxisID.ONLY).exists()
      && DatasetFiles.getRefineModelFile(this).exists()) {
    ...
    if (!closeImod(imodKey, joinDialog.getAxisID(), joinFileName + " file in 3dmod",
      "This file will be moved to " + FileType.MODELED_JOIN.getFileName(this,
        AxisID.ONLY)/*DatasetFiles.getModeledJoinFileName(manager)*/,
      false)) {
    ...
    try {
      ...
      backupImageFile(FileType.MODELED_JOIN, joinDialog.getAxisID());
      System.out.println("rename " + joinFileType + "," + FileType.MODELED_JOIN);
      Utilities.renameFile(joinFileType.getFile(this, joinDialog.getAxisID()),
        FileType.MODELED_JOIN.getFile(this, joinDialog.getAxisID()));
    ...
    if (convertVersion) {
      // ask join dialog
      if (!joinDialog.getSectionTableMetaData()) {
        processSeries.endSeries();
        return;
      }
      state.setJoinVersion1_0(useTrial, getJoinMetaData());
    }
    File modelFile = new File(propertyUserDir, DatasetFiles.getRefineModelFileName(this));
    // Joinwarp2model
    processSeries.setNextProcess(ProcessName.JOIN_WARP_2_MODEL.toString(), null);
    if (!modelFile.exists() && FileType.WARP_XG.exists(this, AxisID.ONLY)) {
      mainPanel.startProgressBar("Start Refine", joinDialog.getAxisID(),
        ProcessName.JOIN_WARP_2_MODEL);
      BaseProcessManager.touch(
        FileType.JOIN_WARP_2_MODEL_COMSCRIPT.getFile(this, AxisID.ONLY).getAbsolutePath(),
        this);
      Joinwarp2modelParam param = updateJoinwarp2modelParam(true);
      if (param == null) {
        mainPanel.stopProgressBar(joinDialog.getAxisID(), ProcessEndState.FAILED);
        processSeries.endSeries();
        return;
      }
      ...
      String threadName;
      try {
        threadName = processMgr.joinwarp2model(param, processSeries);
      }
      catch (AxisBusyException e) {
        e.printStackTrace();
        uiHarness.openMessageDialog(this,
          "Can not execute " + Joinwarp2modelParam.COMMAND_NAME + "\n" + e.getMessage(),
  ...
  private void refineJoin(final ProcessSeries processSeries) {
    // Refine join
    mainPanel.startProgressBar("Refine Join", joinDialog.getAxisID());
    boolean useTrial = joinDialog.isRefineWithTrial();
    joinDialog.setRefiningJoin(true);
    state.setRefineTrial(useTrial);
    joinDialog.updateDisplay();
    mainPanel.stopProgressBar(joinDialog.getAxisID(), ProcessEndState.DONE);
    if (processSeries != null) {
      processSeries.startNextProcess(AxisID.ONLY);
    }
  }
