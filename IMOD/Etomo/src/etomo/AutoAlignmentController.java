...
public final class AutoAlignmentController {
  ...
  public void xfalignRefine(ProcessSeries processSeries, final boolean joinInterface,
    ...
    manager.getMainPanel().startProgressBar("Refine xfalign", axisID,
      ProcessName.XFALIGN);
  }

  public void revertXfFileToMidas() {
    File midasOutputFile =
      FileType.MIDAS_LOCAL_TRANSFORMATION_LIST.getFile(manager, axisID);
  ...
  public void copyXfFile(File xfOutputFile) {
    ...
    if (xfOutputFile != null && xfOutputFile.exists()) {
      try {
        ...
        e.printStackTrace();
        String[] message =
          { "Unable to copy " + xfOutputFile.getAbsolutePath() + " to " + xfFile.getName()
            + ".", "Copy " + xfOutputFile.getName() + " to " + xfFile.getName() + "." };
        UIHarness.INSTANCE.openMessageDialog(manager, message, "Cannot Copy File",
          axisID);
      }
    }
  }
  ...
  private boolean updateMetaData(final boolean doValidation) {
    ...
    if (!manager.getBaseMetaData().isValid()) {
      UIHarness.INSTANCE.openMessageDialog(manager,
        manager.getBaseMetaData().getInvalidReason(), "Invalid Data", axisID);
      return false;
    }
  ...
  boolean copyMostRecentXfFile(final String commandDescription) {
    ...
    if (newestXfFileType != FileType.LOCAL_TRANSFORMATION_LIST) {
      try {
        ...
        e.printStackTrace();
        String[] message = {
          "Unable to copy " + newestXfFileType.getFile(manager, axisID).getAbsolutePath()
            + " to " + FileType.LOCAL_TRANSFORMATION_LIST.getFileName(manager, axisID)
            + ".",
