...
public final class SirtsetupParam implements CommandParam, CommandDetails {
  ...
  private final ScriptParameter numberOfProcessors =
    new ScriptParameter("NumberOfProcessors");
  private final EtomoBoolean2 startFromZero = new EtomoBoolean2(START_FROM_ZERO_KEY);
  private final ScriptParameter resumeFromIteration =
    new ScriptParameter(RESUME_FROM_ITERATION_KEY);
  private final StringParameter leaveIterations =
    new StringParameter(LEAVE_ITERATIONS_KEY);
  private final FortranInputString radiusAndSigma =
    new FortranInputString(RADIUS_AND_SIGMA_KEY, 2);
  private final FortranInputString subareaSize =
    new FortranInputString(SUBAREA_SIZE_KEY, 2);
  private final ScriptParameter yOffsetOfSubarea =
    new ScriptParameter(Y_OFFSET_OF_SUBAREA_KEY);
  private final FortranInputString scaleToInteger =
    new FortranInputString(SCALE_TO_INTEGER_KEY, 2);
  private final EtomoBoolean2 cleanUpPastStart =
    new EtomoBoolean2(CLEAN_UP_PAST_START_KEY);
  private final ScriptParameter flatFilterFraction =
    new ScriptParameter(EtomoNumber.Type.DOUBLE, FLAT_FILTER_FRACTION_KEY);
  private final EtomoBoolean2 skipVertSliceOutput =
    new EtomoBoolean2(SKIP_VERT_SLICE_OUTPUT_KEY);
  ...
  public void setResume(final boolean resume) {
    if (!resume) {
      ...
      commandFile.set(FileType.TILT_FOR_SIRT_COMSCRIPT.getFileName(manager, axisID));
    }
  }
