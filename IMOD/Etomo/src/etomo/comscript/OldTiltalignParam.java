...
* <p> </p>
*/
 class OldTiltalignParam {
  public static final String rcsid = "$Id$";

   String modelFile;
   String imageFile;
   FortranInputString imageParameters;
   String imodFiducialPosFile;
   String asciiFiducialPosFile;
   String tiltAngleSolutionFile;
   String transformSolutionFile;

   int solutionType;
   int includeExcludeType;
   StringList includeExcludeList;
  //  what is a better name for this parameter
  //  projected image rotation?
   double initialImageRotation;
   int rotationAngleSolutionType;

   int nSeparateViewGroups;
   StringList separateViewGroups;

   TiltAngleSpec tiltAngleSpec;
   double tiltAngleOffset;

   TiltalignSolution tiltAngleSolution;
   TiltalignSolution magnificationSolution;
   TiltalignSolution compressionSolution;

   int distortionSolutionType;
   TiltalignSolution xstretchSolution;
   TiltalignSolution skewSolution;

   double residualThreshold;
   int nSurfaceAnalysis;
   FortranInputString minimizationParams;

   double tiltAxisZShift;
   double tiltAxisXShift;

  // Local alignment parameters
   boolean localAlignments;
   String localTransformFile;
   FortranInputString nLocalPatches;
   FortranInputString minLocalPatchSize;
   FortranInputString minLocalFiducials;
   boolean fixLocalFiducialCoodinates;
   FortranInputString localOutputSelection;

   TiltalignSolution localRotationSolution;
   TiltalignSolution localTiltSolution;
   TiltalignSolution localMagnificationSolution;

   int localDistortionSolutionType;
   TiltalignSolution localXstretchSolution;
   TiltalignSolution localSkewSolution;
   ...
   * and parameters.
   */
   void parseComScriptCommand(ComScriptCommand scriptCommand)
   ...
   */
   void updateComScriptCommand(ComScriptCommand scriptCommand)
  ...
  //
   ...
   void setImageParameters(String newImageParameters)
      throws FortranInputSyntaxException {
    imageParameters.validateAndSet(newImageParameters);
  }

   void setImodFiducialPosFile(String filename) {
    imodFiducialPosFile = filename;
  }

   void setAsciiFiducialPosFile(String filename) {
    asciiFiducialPosFile = filename;
  }

   void setTiltAngleSolutionFile(String filename) {
    tiltAngleSolutionFile = filename;
  }

   void setTransformSolutionFile(String filename) {
    transformSolutionFile = filename;
  }

  //TODO validation
   void setSolutionType(int type) {
    solutionType = type;
  }

   void setIncludeExcludeType(int code) {
    includeExcludeType = code;
  }

  //TODO validation
   void setIncludeExcludeList(String zList) {
    includeExcludeList.parseString(zList);
  }

   void setInitialImageRoation(double angle) {
    initialImageRotation = angle;
  }

   void setRotationAngleSolutionType(int type) {
    rotationAngleSolutionType = type;
  }

   void setSeparateViewGroups(String newList) {
    separateViewGroups.parseString(newList);
    nSeparateViewGroups = separateViewGroups.getNElements();
  }

   void setTiltAngleOffset(String newTiltAngleOffset) {
    tiltAngleOffset = Double.parseDouble(newTiltAngleOffset);
  }

   void setTiltAngleSolutionType(int type) {
    tiltAngleSolution.type = type;
  }

   void setTiltAngleSolutionGroupSize(int size) {
    tiltAngleSolution.params.set(0, size);
  }

   void setTiltAngleSolutionGroupSize(String size) {
    tiltAngleSolution.params.set(0, Integer.parseInt(size));
  }

   void setTiltAngleSolutionAdditionalGroups(String list) {
   ...
   void setMagnificationReferenceView(String newReferenceView)
      throws FortranInputSyntaxException {
    magnificationSolution.referenceView.validateAndSet(newReferenceView);
  }

   void setMagnificationType(int type) {
    magnificationSolution.type = type;
  }

   void setMagnificationSolutionGroupSize(int size) {
    magnificationSolution.params.set(0, size);
  }

   void setMagnificationSolutionGroupSize(String size) {
    magnificationSolution.params.set(0, Integer.parseInt(size));
  }

   void setMagnificationSolutionAdditionalGroups(String list) {
   ...
   void setCompressionReferenceView(String newReferenceView)
      throws FortranInputSyntaxException {
   ...
   void setCompressionSolutionGroupSize(int size) {
    compressionSolution.params.set(0, size);
  }

   void setCompressionSolutionGroupSize(String size) {
    compressionSolution.params.set(0, Integer.parseInt(size));
  }

   void setCompressionSolutionAdditionalGroups(String list) {
   ...
   void setDistortionSolutionType(int type) {
    distortionSolutionType = type;
  }

   void setXstretchType(int type) {
    xstretchSolution.type = type;
  }

   void setXstretchSolutionGroupSize(int size) {
    xstretchSolution.params.set(0, size);
  }

   void setXstretchSolutionGroupSize(String size) {
    xstretchSolution.params.set(0, Integer.parseInt(size));
  }

   void setXstretchSolutionAdditionalGroups(String list) {
   ...
   void setSkewType(int type) {
    skewSolution.type = type;
  }

   void setSkewSolutionGroupSize(int size) {
    skewSolution.params.set(0, size);
  }

   void setSkewSolutionGroupSize(String size) {
    skewSolution.params.set(0, Integer.parseInt(size));
  }

   void setSkewSolutionAdditionalGroups(String list) {
   ...
   void setNSurfaceAnalysis(String newNSurfaceAnalysis) {
    nSurfaceAnalysis = Integer.parseInt(newNSurfaceAnalysis);
  }

   void setNSurfaceAnalysis(int n) {
    nSurfaceAnalysis = n;
  }

   void setMinimizationParams(String params) throws FortranInputSyntaxException {
    minimizationParams.validateAndSet(params);
  }

   void setTiltAxisZShift(double shift) {
    tiltAxisZShift = shift;
  }

   void setTiltAxisZShift(String shift) {
    tiltAxisZShift = Double.parseDouble(shift);
  }

   void setTiltAxisXShift(double shift) {
    tiltAxisXShift = shift;
  }

   void setTiltAxisXShift(String shift) {
    tiltAxisXShift = Double.parseDouble(shift);
  }

   void setLocalAlignments(boolean state) {
    localAlignments = state;
  }

   void setLocalTransformFile(String filename) {
    localTransformFile = filename;
  }

   void setMetroFactor(String factor) {
    minimizationParams.set(0, Double.parseDouble(factor));
  }

   void setCycleLimit(String limit) {
    minimizationParams.set(1, Integer.parseInt(limit));
  }

   void setNLocalPatches(String params) throws FortranInputSyntaxException {
    nLocalPatches.validateAndSet(params);
  }

   void setMinLocalPatchSize(String params) throws FortranInputSyntaxException {
    minLocalPatchSize.validateAndSet(params);
  }

   void setMinLocalFiducials(String params) throws FortranInputSyntaxException {
    minLocalFiducials.validateAndSet(params);
  }

   void setFixLocalFiducialCoodinates(boolean state) {
    fixLocalFiducialCoodinates = state;
  }

   void setLocalOutputSelection(String params) throws FortranInputSyntaxException {
    localOutputSelection.validateAndSet(params);
  }

   void setLocalRotationSolutionType(int type) {
    localRotationSolution.type = type;
  }

   void setLocalRotationSolutionGroupSize(int size) {
    localRotationSolution.params.set(0, size);
  }

   void setLocalRotationSolutionGroupSize(String size) {
    localRotationSolution.params.set(0, Integer.parseInt(size));
  }

   void setLocalRotationSolutionAdditionalGroups(String list) {
   ...
   void setLocalTiltSolutionType(int type) {
    localTiltSolution.type = type;
  }

   void setLocalTiltSolutionGroupSize(int size) {
    localTiltSolution.params.set(0, size);
  }

   void setLocalTiltSolutionGroupSize(String size) {
    localTiltSolution.params.set(0, Integer.parseInt(size));
  }

   void setLocalTiltSolutionAdditionalGroups(String list) {
   ...
   void setLocalMagnificationReferenceView(String newReferenceView)
      throws FortranInputSyntaxException {
    localMagnificationSolution.referenceView.validateAndSet(newReferenceView);
  }

   void setLocalMagnificationType(int type) {
    localMagnificationSolution.type = type;
  }

   void setLocalMagnificationSolutionGroupSize(int size) {
    localMagnificationSolution.params.set(0, size);
  }

   void setLocalMagnificationSolutionGroupSize(String size) {
    localMagnificationSolution.params.set(0, Integer.parseInt(size));
  }

   void setLocalMagnificationSolutionAdditionalGroups(String list) {
   ...
   void setLocalDistortionSolutionType(int type) {
    localDistortionSolutionType = type;
  }

   void setLocalXstretchType(int type) {
    localXstretchSolution.type = type;
  }

   void setLocalXstretchSolutionGroupSize(int size) {
    localXstretchSolution.params.set(0, size);
  }

   void setLocalXstretchSolutionGroupSize(String size) {
    localXstretchSolution.params.set(0, Integer.parseInt(size));
  }

   void setLocalXstretchSolutionAdditionalGroups(String list) {
   ...
   void setLocalSkewType(int type) {
    localSkewSolution.type = type;
  }

   void setLocalSkewSolutionGroupSize(int size) {
    localSkewSolution.params.set(0, size);
  }

   void setLocalSkewSolutionGroupSize(String size) {
    localSkewSolution.params.set(0, Integer.parseInt(size));
  }

   void setLocalSkewSolutionAdditionalGroups(String list) {
  ...
  private int getNextNonBlankArgIndex(ComScriptInputArg[] inputArgs, int inputLine) {
   ...
   String getImageParameters() {
    return imageParameters.toString();
  }

   String getIMODFiducialPosFile() {
    return imodFiducialPosFile;
  }

   String getAsciiFiducialPosFile() {
    return asciiFiducialPosFile;
  }

   String getTiltAngleSolutionFile() {
    return tiltAngleSolutionFile;
  }

   String getTransformSolutionFile() {
   ...
   int getIncludeExcludeType() {
    return includeExcludeType;
  }

   StringList getIncludeExcludeList() {
    return includeExcludeList;
  }

   double getInitialImageRotation() {
    return initialImageRotation;
  }

   int getRotationAngleSolutionType() {
    return rotationAngleSolutionType;
  }

   int getNSeparateViewGroups() {
    return nSeparateViewGroups;
  }

   StringList getSeparateViewGroups() {
   ...
   double getTiltAngleOffset() {
    return tiltAngleOffset;
  }

   TiltalignSolution getTiltAngleSolution() {
    return tiltAngleSolution;
  }

   String getTiltAngleSolutionParams() {
    return tiltAngleSolution.params.toString();
  }

   int getTiltAngleSolutionGroupSize() {
    return tiltAngleSolution.params.getInt(0);
  }

   String getTiltAngleSolutionAdditionalGroups() {
    return tiltAngleSolution.additionalGroups.toString();
  }

   String getMagnificationSolutionReferenceView() {
    return magnificationSolution.referenceView.toString();
  }

   TiltalignSolution getMagnificationSolution() {
    return magnificationSolution;
  }

   String getMagnificationSolutionParams() {
    return magnificationSolution.params.toString();
  }

   int getMagnificationSolutionGroupSize() {
    return magnificationSolution.params.getInt(0);
  }

   String getMagnificationSolutionAdditionalGroups() {
    return magnificationSolution.additionalGroups.toString();
  }

   String getCompressionSolutionReferenceView() {
    return compressionSolution.referenceView.toString();
  }

   int getCompressionSolutionType() {
    return compressionSolution.type;
  }

   String getCompressionSolutionParams() {
    return compressionSolution.params.toString();
  }

   int getCompressionSolutionGroupSize() {
    return compressionSolution.params.getInt(0);
  }

   String getCompressionSolutionAdditionalGroups() {
    return compressionSolution.additionalGroups.toString();
  }

   int getDistortionSolutionType() {
    return distortionSolutionType;
  }

   TiltalignSolution getXstretchSolution() {
    return xstretchSolution;
  }

   String getXstretchSolutionParams() {
    return xstretchSolution.params.toString();
  }

   int getXstretchSolutionGroupSize() {
    return xstretchSolution.params.getInt(0);
  }

   String getXstretchSolutionAdditionalGroups() {
    return xstretchSolution.additionalGroups.toString();
  }

   TiltalignSolution getSkewSolution() {
    return skewSolution;
  }

   String getSkewSolutionParams() {
    return skewSolution.params.toString();
  }

   int getSkewSolutionGroupSize() {
    return skewSolution.params.getInt(0);
  }

   String getSkewSolutionAdditionalGroups() {
   ...
   int getNSurfaceAnalysis() {
    return nSurfaceAnalysis;
  }

   String getMinimizationParams() {
    return minimizationParams.toString();
  }

   double getMetroFactor() {
   ...
   double getTiltAxisZShift() {
    return tiltAxisZShift;
  }

   double getTiltAxisXShift() {
    return tiltAxisXShift;
  }

   boolean getLocalAlignments() {
    return localAlignments;
  }

   String getLocalTransformFile() {
    return localTransformFile;
  }

   FortranInputString getNLocalPatches() {
    return nLocalPatches;
  }

   FortranInputString getMinLocalPatchSize() {
    return minLocalPatchSize;
  }

   FortranInputString getMinLocalFiducials() {
    return minLocalFiducials;
  }

   boolean getFixLocalFiducialCoodinates() {
    return fixLocalFiducialCoodinates;
  }

   FortranInputString getLocalOutputSelection() {
    return localOutputSelection;
  }

   TiltalignSolution getLocalRotationSolution() {
    return localRotationSolution;
  }

   int getLocalRotationSolutionType() {
    return localRotationSolution.type;
  }

   int getLocalRotationSolutionGroupSize() {
    return localRotationSolution.params.getInt(0);
  }

   FortranInputString getLocalRotationSolutionParams() {
    return localRotationSolution.params;
  }

   StringList getLocalRotationAdditionalGroups() {
    return localRotationSolution.additionalGroups;
  }

   TiltalignSolution getLocalTiltSolution() {
    return localTiltSolution;
  }

   int getLocalTiltSolutionGroupSize() {
    return localTiltSolution.params.getInt(0);
  }

   String getLocalTiltSolutionParams() {
    return localTiltSolution.params.toString();
  }

   String getLocalTiltAdditionalGroups() {
    return localTiltSolution.additionalGroups.toString();
  }

   String getLocalMagnificationSolutionReferenceView() {
    return localMagnificationSolution.referenceView.toString();
  }

   TiltalignSolution getLocalMagnificationSolution() {
    return localMagnificationSolution;
  }

   String getLocalMagnificationSolutionParams() {
    return localMagnificationSolution.params.toString();
  }

   int getLocalMagnificationSolutionGroupSize() {
    return localMagnificationSolution.params.getInt(0);
  }

   String getLocalMagnificationSolutionAdditionalGroups() {
    return localMagnificationSolution.additionalGroups.toString();
  }

   String getLocalMagnificationAdditionalGroups() {
    return localMagnificationSolution.additionalGroups.toString();
  }

   int getLocalDistortionSolutionType() {
    return localDistortionSolutionType;
  }

   TiltalignSolution getLocalXstretchSolution() {
    return localXstretchSolution;
  }

   String getLocalXstretchSolutionParams() {
    return localXstretchSolution.params.toString();
  }

   int getLocalXstretchSolutionGroupSize() {
    return localXstretchSolution.params.getInt(0);
  }

   String getLocalXstretchSolutionAdditionalGroups() {
    return localXstretchSolution.additionalGroups.toString();
  }

   TiltalignSolution getLocalSkewSolution() {
    return localSkewSolution;
  }

   String getLocalSkewSolutionParams() {
    return localSkewSolution.params.toString();
  }

   int getLocalSkewSolutionGroupSize() {
    return localSkewSolution.params.getInt(0);
  }

   String getLocalSkewSolutionAdditionalGroups() {
