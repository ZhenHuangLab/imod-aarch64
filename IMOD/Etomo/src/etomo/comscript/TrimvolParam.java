...
public class TrimvolParam implements CommandDetails {
  ...
  private boolean convertToBytes = false;
  ...
  /**
   * @return
   * @deprecated (since="2/13/2019", forRemoval=true)
   */
  ...
  public List<String> genOptions() {
    ...
    if (convertToBytes) {
      if (fixedScaling) {
        ...
        options.add("-sz");
        options
          .add(String.valueOf(sectionScaleMin) + "," + String.valueOf(sectionScaleMax));
        if (!scaleXYParam.getXMin().isNull() && !scaleXYParam.getXMax().isNull()) {
          options.add("-sx");
          options.add(
            scaleXYParam.getXMin().toString() + "," + scaleXYParam.getXMax().toString());
        }
        if (!scaleXYParam.getYMin().isNull() && !scaleXYParam.getYMax().isNull()) {
          options.add("-sy");
          options.add(
            scaleXYParam.getYMin().toString() + "," + scaleXYParam.getYMax().toString());
        }
      }
    }
   ...
   * @throws IOException
   */
  public void setDefaultRange(TrimvolInputFileState inputFileState,
   ...
   * @return
   */
  public static String getInputFileName(final BaseManager manager, AxisType axisType,
    String datasetName) {
    if (axisType == AxisType.SINGLE_AXIS) {
      return FileType.TILT_OUTPUT.getFileName(manager, datasetName, axisType,
        AxisID.ONLY);
    }
    return FileType.COMBINED_VOLUME.getFileName(manager, datasetName, axisType,
      AxisID.ONLY);
  }
  ...
  public void setInputFileName(AxisType axisType, String datasetName) {
    inputFile = getInputFileName(manager, axisType, datasetName);
  }
  ...
  public static String getOutputFileName(final BaseManager manager, final AxisID axisID) {
    return FileType.TRIM_VOL_OUTPUT.getFileName(manager, axisID);
   ...
   * @param trim
   * @return
   * @deprecated (since="2/13/2019", forRemoval=true) Not used and most likely not kept up to date
   */
