...
public class ComScriptCommand {
   ...
   */
  ComScriptCommand(boolean caseInsensitive, boolean separateWithASpace) {
   ...
   * Copy constructor.
   */
  ComScriptCommand(ComScriptCommand src) {
  ...
  void appendHeaderComments(String[] moreComments) {
   ...
   *               be added.
   */
  void appendCommandLineArgs(String[] append) {
   ...
   *                 argument list.
   */
  void appendInputArgument(ComScriptInputArg inputArg) {
   ...
   * representation.
   */
  ComScriptInputArg[] getInputArguments() {
   ...
   */
  void setInputArgument(int index, ComScriptInputArg inputArg) {
   ...
   *                  arguments
   */
  void setInputArguments(ComScriptInputArg[] inputArgs) {
   ...
   * @return
   */
  String[] getValues(String keyword) {
   ...
   * @param values
   */
  void setValues(String keyword, String[] values) {
   ...
   * @param valuesArray
   */
  void setValuesInterleaved(String[] keywordArray, ArrayList[] valuesArray) {
    ...
    for (int i = 0; i < maxValuesLen; i++) {
      for (int j = 0; j < maxArrayLen; j++) {
        ...
        if (i < valuesLen) {
          inputArg = new ComScriptInputArg();
          stdinArgs.add(inputArg);
          inputArg.setArgument(
            keyword + divider + (valuesArray[j] != null ? valuesArray[j].get(i) : ""));
        }
      }
    }
  }
   ...
   * @param value
   */
  void addKey(String keyword, String value) {
   ...
   * @param keyword
   */
  void deleteKeyAll(String keyword) {
    Iterator itStdinArgs = stdinArgs.iterator();
    while (itStdinArgs.hasNext()) {
      ComScriptInputArg inputArg = (ComScriptInputArg) itStdinArgs.next();
  ...
  /**
   * Returns true if option is a keyword in this command or in the command line.  Checks
   * the command line if option is not one of the keywords.
   * @param option
   * @return
   */
  boolean containsOption(final String option) {
    if (keywordValuePairs) {
      // Check the keywords.
      try {
        if (hasKeyword(option)) {
          return true;
        }
      }
    ...
    for (int i = 0; i < commandLineArgs.length; i++) {
      if (commandLineArgs[i] != null && commandLineArgs.equals(option)) {
        return true;
      }
    }
    return false;
