...
public class CCDEraserParam extends ConstCCDEraserParam implements Command, CommandParam {
  public static final String COMMAND_NAME = ProcessName.CCDERASER.toString();

  private static final int COMMAND_SIZE = 1;
  private final StringParameter lineObjects = new StringParameter(LINE_OBJECTS_KEY);
  private final StringParameter boundaryObjects =
    new StringParameter(BOUNDARY_OBJECTS_KEY);
  private final StringParameter allSectionObjects =
    new StringParameter(ALL_SECTION_OBJECTS_KEY);
  ...
  public void parseComScriptCommand(ComScriptCommand scriptCommand)
    ...
    if (scriptCommand.isKeywordValuePairs()) {
      ...
      findPeaks = true;
      String junk = etomo.util.Utilities.removeExtension(inputFile);
      String datasetName = junk;
      String ext = etomo.util.Utilities.getExtension(inputFile);
      outputFile =
        datasetName + "_fixed" + (ext != null ? Extension.EXTENSION_DIVIDER + ext : "");
  ...
  public void updateComScriptCommand(ComScriptCommand scriptCommand)
    ...
    if (manager.getConstMetaData().getViewType() == ViewType.MONTAGE
      ...
      scriptCommand.setValue("PieceListFile",
        manager.getBaseMetaData().getDatasetName() + axisID.getExtension() + ".pl");
      scriptCommand.setValue("OverlapsForModel",
        Utilities.MONTAGE_SEPARATION + "," + Utilities.MONTAGE_SEPARATION);
    }
  }
   ...
   */
  void convertOuterRadius() {
    // if annulusWidth already set then return
    if (!annulusWidth.equals("") || outerRadius.equals("") || maximumRadius.equals("")) {
      return;
    }
    annulusWidth =
      String.valueOf(Double.parseDouble(outerRadius) - Double.parseDouble(maximumRadius));
  }
  ...
  public static final class Mode implements CommandMode {
    ...
    // no output file in X_RAYS_TRIAL
