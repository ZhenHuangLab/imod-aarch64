...
public final class SetupCombine {
  ...
  private static String[] patchSizes = null;
  ...
  public static String[] getInfoOnPatchSizes() {
    synchronized (EtomoDirector.INSTANCE) {
      ...
      patchSizes = BaseProcessManager.getCommandOutput(new String[] { "python", "-u",
        ApplicationManager.getIMODBinPath() + COMMAND, "-info" }, null, null);
      return patchSizes;
    }
  }
  ...
  private void genOptions() throws SystemProcessException {
    ...
    if (metaData.isOrigScopeTemplate()) {
      command.add("-change");
      command.add(metaData.getOrigScopeTemplate());
    }
    if (metaData.isOrigSystemTemplate()) {
      command.add("-change");
      command.add(metaData.getOrigSystemTemplate());
    }
    if (metaData.isOrigUserTemplate()) {
      command.add("-change");
      command.add(metaData.getOrigUserTemplate());
    }
