...
public final class TiltParam implements ConstTiltParam, CommandParam {
  ...
  public static final String SUPERSAMPLEFACTOR_KEY = "SuperSampleFactor";
  public static final String EXPANDLINESINPUT_KEY = "ExpandInputLines";
  ...
  private final ScriptParameter logOffset =
    new ScriptParameter(EtomoNumber.Type.DOUBLE, LOG_KEY);
  ...
  private EtomoNumber radialBandwidth = new EtomoNumber(EtomoNumber.Type.DOUBLE);
  private EtomoNumber radialFalloff = new EtomoNumber(EtomoNumber.Type.DOUBLE);
  ...
  private final ScriptParameter xAxisTilt =
    new ScriptParameter(EtomoNumber.Type.DOUBLE, "XAXISTILT");
  ...
  private final ScriptParameter hammingLikeFilter =
    new ScriptParameter(EtomoNumber.Type.DOUBLE, "HammingLikeFilter");
  private final ScriptParameter fakeSIRTiterations =
    new ScriptParameter("FakeSIRTiterations");
  private final ScriptParameter exactFilterSize = new ScriptParameter("ExactFilterSize");
  private final EtomoBoolean2 falloffIsTrueSigma =
    new EtomoBoolean2("FalloffIsTrueSigma");

  private final ScriptParameter superSampleFactor =
    new ScriptParameter(EtomoNumber.Type.INTEGER, SUPERSAMPLEFACTOR_KEY);
  private final EtomoBoolean2 expandInputLines = new EtomoBoolean2(EXPANDLINESINPUT_KEY);
  ...
  public FileKey getOutputImageFileKey() {
    if (commandMode == Mode.SAMPLE) {
      return FileKey.POSITIONING_SAMPLE;
    }
    if (commandMode == Mode.TILT) {
      return FileType.TILT_OUTPUT;
    }
  ...
  public boolean isFalloffIsTrueSigma() {
    return falloffIsTrueSigma.is();
  }
  ...
  public String getRadialBandwidth() {
    return radialBandwidth.toString();
  }

  public boolean hasRadialWeightingFunction() {
    return !radialBandwidth.isNull();
  }
  ...
  public String getXAxisTiltString() {
   ...
   * @return
   */
  public String getRadialFalloff() {
    return radialFalloff.toString();
  }
  ...
  public boolean isSuperSampleFactorSet() {
    return !superSampleFactor.isNull();
  }

  public String getSuperSampleFactor() {
    return superSampleFactor.toString();
  }

  public boolean isExpandInputLinesSet() {
    return expandInputLines.is();
  }
  ...
  public void parseComScriptCommand(final ComScriptCommand scriptCommand)
    ...
    if (!scriptCommand.isKeywordValuePairs()) {
      ...
      if (!tempFullImage.isEmpty()) {
        String[] params = tempFullImage.toString().split("\\s*,\\s*|\\s+", 2);
      ...
      if (!tempOffset.isEmpty()) {
        String[] params = tempOffset.toString().split("\\s*,\\s*|\\s+", 2);
      ...
      if (!tempRadial.isEmpty()) {
        String[] params = tempRadial.toString().split("\\s*,\\s*|\\s+", 2);
        radialBandwidth.set(params[0]);
        radialFalloff.set(params[1]);
      }
      tempScale.parse(scriptCommand);
      if (!tempScale.isEmpty()) {
        String[] params = tempScale.toString().split("\\s*,\\s*|\\s+", 2);
      ...
      if (!tempShift.isEmpty()) {
        String[] params = tempShift.toString().split("\\s*,\\s*|\\s+", 2);
      ...
      if (!tempSlice.isEmpty()) {
        String[] params = tempSlice.toString().split("\\s*,\\s*|\\s+", 3);
      ...
      if (!tempSubsetStart.isEmpty()) {
        String[] params = tempSubsetStart.toString().split("\\s*,\\s*|\\s+", 2);
      ...
      hammingLikeFilter.parse(scriptCommand);
      fakeSIRTiterations.parse(scriptCommand);
      exactFilterSize.parse(scriptCommand);
      falloffIsTrueSigma.parse(scriptCommand);
      superSampleFactor.parse(scriptCommand);
      expandInputLines.parse(scriptCommand);
      loadedFromFile = true;
    }
  }
  ...
  public void updateComScriptCommand(final ComScriptCommand scriptCommand)
    ...
    if (!radialBandwidth.isNull()) {
      tempRadial.set(radialBandwidth.toString() + " " + radialFalloff.toString());
    }
    else {
      tempRadial.reset();
    }
    ...
    if (idxXSubsetStart > Integer.MIN_VALUE) {
      tempSubsetStart
        .set(String.valueOf(idxXSubsetStart) + " " + String.valueOf(idxYSubsetStart));
    }
    else {
    ...
    hammingLikeFilter.updateComScript(scriptCommand);
    fakeSIRTiterations.updateComScript(scriptCommand);
    exactFilterSize.updateComScript(scriptCommand);
    superSampleFactor.updateComScript(scriptCommand);
    expandInputLines.updateComScript(scriptCommand);
  }
  ...
  public void setMontageSubsetStart()
    ...
    resetSubsetStart();
    Goodframe goodframe =
      etomo.comscript.Utilities.getGoodframeFromMontageSize(axisID, manager);
    if (goodframe != null) {
      MRCHeader header =
        MRCHeader.getInstanceFromFileName(manager, axisID, inputFile.toString());
      try {
        ...
        int goodframeX;
        int goodframeY;
        if (etomo.comscript.Utilities.is90DegreeImageRotation(
          manager.getConstMetaData().getImageRotation(axisID).getDouble())) {
          // transpose x and y
        ...
        // which is unbinned.
        idxXSubsetStart =
          ((goodframeX - header.getNColumns() * setImageBinned().getInt()) / 2);
        idxYSubsetStart =
          ((goodframeY - header.getNRows() * setImageBinned().getInt()) / 2);
      }
      catch (IOException e) {
        // ok if tilt is being updated before .ali exists
      }
  ...
  public boolean setSubsetStart() {
    resetSubsetStart();
    MRCHeader stackHeader = MRCHeader.getInstance(manager, axisID, FileType.RAW_STACK);
    try {
      ...
      idxXSubsetStart =
        ((stackX - aliHeader.getNColumns() * setImageBinned().getInt()) / 2);
  ...
  public void setHammingLikeFilter(final String input) {
    hammingLikeFilter.set(input);
  }

  public void resetHammingLikeFilter() {
    hammingLikeFilter.reset();
  }

  public String getHammingLikeFilter() {
    return hammingLikeFilter.toString();
  }

  public boolean isHammingLikeFilter() {
    return !hammingLikeFilter.isNull();
  }
  ...
  public boolean isFakeSIRTiterations() {
    return !fakeSIRTiterations.isNull();
  }

  public void setExactFilterSize(final String input) {
    exactFilterSize.set(input);
  }

  public void resetExactFilterSize() {
    exactFilterSize.reset();
  }

  public String getExactFilterSize() {
    return exactFilterSize.toString();
  }

  public boolean isExactFilterSize() {
    return !exactFilterSize.isNull();
  }
  ...
  public void setRadialBandwidth(final String value) {
    radialBandwidth.set(value);
  }

  /**
   * @param string
   */
  public void setRadialFalloff(final String value) {
    radialFalloff.set(value);
  }

  public void resetRadialFilter() {
    radialBandwidth.reset();
    radialFalloff.reset();
  }
  ...
  @Deprecated // 7/28/2018
  public void setWidth(final int i) {
    width.set(i);
  }

  public void setWidth(final String input) {
    width.set(input);
  }
  ...
  public void setSuperSampleFactor(final int input) {
    superSampleFactor.set(input);
  }

  public void resetSuperSampleFactor() {
    superSampleFactor.reset();
  }

  public void setExpandInputLines(final boolean input) {
    expandInputLines.set(input);
  }
  ...
  public boolean upgradeOldVersion(final int correctionBinning,
    ...
    StringBuffer buffer =
      new StringBuffer("\nUpgraded tilt" + axisID.getExtension() + ".com:\n");
    if (correctionBinning > 1) {
