...
public class AutofidseedParam implements CommandParam, Command {
  ...
  public static final String LOWER_TARGET_FOR_CLUSTERED_KEY = "LowerTargetForClustered";

  private final StringParameter trackCommandFile =
    new StringParameter("TrackCommandFile");
  private final ScriptParameter minGuessNumBeads =
    new ScriptParameter(MIN_GUESS_NUM_BEADS_KEY);
  private final ScriptParameter minSpacing =
    new ScriptParameter(EtomoNumber.Type.DOUBLE, MIN_SPACING_KEY);
  private final ScriptParameter peakStorageFraction =
    new ScriptParameter(EtomoNumber.Type.DOUBLE, PEAK_STORAGE_FRACTION_KEY);
  private final StringParameter boundaryModel = new StringParameter(BOUNDARY_MODEL_KEY);
  private final EtomoBoolean2 excludeInsideAreas =
    new EtomoBoolean2(EXCLUDE_INSIDE_AREAS_KEY);
  private final FortranInputString bordersInXandY =
    new FortranInputString(BORDERS_IN_X_AND_Y_KEY, 2);
  private final EtomoBoolean2 twoSurfaces = new EtomoBoolean2(TWO_SURFACES_KEY);
  private final EtomoBoolean2 appendToSeedModel =
    new EtomoBoolean2(APPEND_TO_SEED_MODEL_KEY);
  private final ScriptParameter targetNumberOfBeads =
    new ScriptParameter(TARGET_NUMBER_OF_BEADS_KEY);
  private final ScriptParameter targetDensityOfBeads =
    new ScriptParameter(TARGET_DENSITY_OF_BEADS_KEY);
  private final ScriptParameter maxMajorToMinorRatio =
    new ScriptParameter(EtomoNumber.Type.DOUBLE, MAX_MAJOR_TO_MINOR_RATIO_KEY);
  private final ScriptParameter clusteredPointsAllowed =
    new ScriptParameter(CLUSTERED_POINTS_ALLOWED_KEY);
  ...
  private final ScriptParameter elongatedPointsAllowed =
    new ScriptParameter(ELONGATED_POINTS_ALLOWED_KEY);
  private final EtomoBoolean2 adjustSizes = new EtomoBoolean2(ADJUST_SIZES_KEY);
  private final ScriptParameter lowerTargetForClustered =
    new ScriptParameter(EtomoNumber.Type.DOUBLE, LOWER_TARGET_FOR_CLUSTERED_KEY);
  ...
  public void parseComScriptCommand(final ComScriptCommand scriptCommand)
    ...
    lowerTargetForClustered.parse(scriptCommand);
  }
  ...
  public void updateComScriptCommand(final ComScriptCommand scriptCommand)
    ...
    lowerTargetForClustered.updateComScript(scriptCommand);
  }
  ...
  public boolean isLowerTargetForClustered() {
    return lowerTargetForClustered.is();
  }

  public String getLowerTargetForClustered() {
    return lowerTargetForClustered.toString();
  }
  ...
  public void resetLowerTargetForClustered() {
    lowerTargetForClustered.reset();
  }

  public void setLowerTargetForClustered(final String input) {
    lowerTargetForClustered.set(input);
  }
