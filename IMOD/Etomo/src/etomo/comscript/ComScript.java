...
public class ComScript {
  ...
  public ComScriptCommand getScriptCommand(String cmdName, int commandIndex,
    ...
    throw (new BadComScriptException(
      "Did not find command: " + cmdName + " at index " + commandIndex));
  }
   ...
   * @return
   */
  int createCommand(String cmdName, boolean caseInsensitive, boolean separateWithASpace) {
    ComScriptCommand currentScriptCommand =
      new ComScriptCommand(caseInsensitive, separateWithASpace);
    scriptCommands.add(currentScriptCommand);
   ...
   * @return
   */
  void createCommand(String cmdName, int commandIndex, boolean caseInsensitive,
   ...
   */
  public int getScriptCommandIndex(String cmdName, boolean caseInsensitive,
    boolean separateWithASpace, final String requiredOption) {
    return getScriptCommandIndex(cmdName, false, caseInsensitive, separateWithASpace,
      requiredOption);
  }
   ...
   * @param requiredOption - option name which must exist in command for it to match
   * @return
   */
  public int getScriptCommandIndex(String cmdName, boolean addNew,
    boolean caseInsensitive, boolean separateWithASpace, final String requiredOption) {
    if (!commandLoaded) {
      createCommand(cmdName, caseInsensitive, separateWithASpace);
    }
    for (int i = 0; i < scriptCommands.size(); i++) {
      ComScriptCommand command = (ComScriptCommand) scriptCommands.get(i);
      if (command.getCommand().equals(cmdName)
        && (requiredOption == null || command.containsOption(requiredOption))) {
        return i;
      }
    }
    if (addNew) {
  ...
  /*
   * Adds a new command at index, shifting the existing commands to make room for it.
   */
  public void addScriptComand(int index, ComScriptCommand newScriptCommand) {
    // make a defensive copy of the scriptCommand object
    scriptCommands.add(index, new ComScriptCommand(newScriptCommand));
  }
