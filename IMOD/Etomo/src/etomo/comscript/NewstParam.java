...
public final class NewstParam implements ConstNewstParam, CommandParam {
  ...
  private final FortranInputString sizeToOutputInXandY =
    new FortranInputString("SizeToOutputInXandY", 2);
  ...
  private final FortranInputString offsetsInXandY =
    new FortranInputString(OFFSETS_IN_X_AND_Y_KEY, 2);
  ...
  private final StringParameter useTransformLines =
    new StringParameter("UseTransformLines");
  private final ScriptParameter rotateByAngle =
    new ScriptParameter(EtomoNumber.Type.DOUBLE, "RotateByAngle");
  private final ScriptParameter expandByFactor =
    new ScriptParameter(EtomoNumber.Type.DOUBLE, "ExpandByFactor");
  private final ScriptParameter binByFactor = new ScriptParameter("BinByFactor");
  private final EtomoBoolean2 linearInterpolation =
    new EtomoBoolean2("LinearInterpolation");
  private final ScriptParameter floatDensities = new ScriptParameter("FloatDensities");
  private final FortranInputString contrastBlackWhite =
    new FortranInputString("ContrastBlackWhite", 2);
  private final FortranInputString scaleMinAndMax =
    new FortranInputString("ScaleMinAndMax", 2);
  ...
  private final ScriptParameter fillValue =
    new ScriptParameter(EtomoNumber.Type.DOUBLE, "FillValue");
  ...
  private final EtomoNumber cnmaxtemp =
    new EtomoNumber(EtomoNumber.Type.DOUBLE, "-cnmaxtemp");
  ...
  public static NewstParam getColorInstance(final BaseManager manager,
    final AxisID axisID) {
    ...
    if (scriptCommand.isKeywordValuePairs()) {
      ...
      for (int i = 0; i < cmdLineArgs.length; i++) {
        ...
        if (cmdLineArgs[i].startsWith("-")) {
          ...
          else if (cmdLineArgs[i].toLowerCase().startsWith(DATA_MODE_OPTION)) {
            i++;
          ...
          else if (cmdLineArgs[i].startsWith("-xf")
            || cmdLineArgs[i].startsWith("-Tra")) {
            i++;
          ...
          else if (cmdLineArgs[i].toLowerCase().startsWith(FLOAT_DENSITIES_OPTION)) {
            i++;
  ...
  public void updateComScriptCommand(final ComScriptCommand scriptCommand)
    ...
    if (!useColorNewst) {
      ...
      for (Iterator<FortranInputString> i = offsetsInXandYExtraEntries.iterator();
        i.hasNext();) {
  ...
  public boolean setSizeToOutputInXandY(String userSize, final int binning,
    ...
    if ((sizeToOutputInXandY.isDefault() || sizeToOutputInXandY.isEmpty())
      ...
      MRCHeader header = MRCHeader.getInstance(manager, axisID, FileType.RAW_STACK);
      header.read(manager);
