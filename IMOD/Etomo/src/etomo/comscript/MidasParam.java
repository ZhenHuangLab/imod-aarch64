...
public final class MidasParam implements Command {
  ...
  private ScriptParameter imageRotation =
    new ScriptParameter(EtomoNumber.Type.DOUBLE, "-a");
  ...
  public MidasParam.Mode getMode() {
    return mode;
  }
  ...
  private ArrayList genOptions() {
    ...
    // Midas must not fork.
    options.add("-D");
    if (mode == Mode.RAW_STACK) {
      if (!imageRotation.isNull()) {
        options.add("-a");
        options.add(String.valueOf(-1 * imageRotation.getDouble()));
      }
      options.add("-t");
      options.add(FileType.RAW_TILT_ANGLES.getFileName(manager, axisID));
      options.add(inputFileName);
      options.add(FileType.PRE_TRANSFORMATION_LIST.getFileName(manager, axisID));
    }
    if (mode == Mode.SAMPLE) {
      ...
      if (sectionTableRowData != null) {
        ...
        for (int i = 0; i < sectionDataSize; i++) {
          ConstSectionTableRowData data =
            (SectionTableRowData) sectionTableRowData.get(i);
  ...
  public void setImageRotation(final double input) {
    imageRotation.set(input);
  }
  ...
  public static final class Mode {
    ...
    public static final Mode RAW_STACK = new Mode();
