...
public final class AnisotropicDiffusionParam implements CommandDetails {
  ...
  private final ParsedArray kValueList =
    ParsedArray.getInstance(EtomoNumber.Type.DOUBLE, null, "K value");
  ...
  public String setKValueList(final String input) {
    ...
    if (debugLevel == DebugLevel.HIGH) {
      System.out
        .println("AnisotropicDiffusionParam.setKValueList:kValueList=" + kValueList);
    }
  ...
  public void createFilterFullFile() throws LogFile.LockException, IOException {
    ...
    LogFile filterFullFile =
      LogFile.getInstance(new File(subdir, getFilterFullFileName()));
    filterFullFile.create();
    LogFile.WriterId writerId = filterFullFile.openWriter();
    filterFullFile
      .write(
        COMMAND_CHAR + PROCESS_NAME + " " + K_VALUE_TAG + " " + kValue + " "
  ...
  public void createTestFiles() throws LogFile.LockException, IOException {
    ...
    for (int i = 0; i < kValueList.size(); i++) {
      ...
      LogFile testFile =
        LogFile.getInstance(new File(subdir, TestNADFileFilter.FILE_NAME_BODY
          + addLeadingZeros(index.toString(), 3) + TestNADFileFilter.FILE_NAME_EXT));
      testFile.create();
      ...
      testFile.write(COMMAND_CHAR + PROCESS_NAME + " " + K_VALUE_TAG + " "
        ...
        + " " + inputFileName + " " + getTestFileName(k, iteration.toString()), writerId);
  ...
  public FileKey getOutputImageFileKey() {
    if (mode == Mode.VARYING_K) {
      return FileKey.NAD_TEST_VARYING_K;
    }
    if (mode == Mode.VARYING_ITERATIONS) {
      return FileKey.NAD_TEST_VARYING_ITERATIONS;
    }
    if (mode == Mode.FULL) {
      return FileType.ANISOTROPIC_DIFFUSION_OUTPUT;
    }
  ...
  public IteratorElementList
    getIteratorElementList(etomo.comscript.FieldInterface fieldInterface) {
    if (fieldInterface == Field.ITERATION_LIST) {
