...
public final class CopyTomoComs {
  ...
  private final EtomoNumber sphericalAberration =
  ...
  private boolean genOptions() {
    ...
    if (directiveFileCollection != null) {
      ...
      Iterator<Entry<String, String>> iterator =
        directiveFileCollection.getCopyArgEntrySet().iterator();
      if (iterator != null) {
    ...
    if (!directiveDrivenAutomation) {
      ...
      if (metaData.getTiltAngleSpecA().getType() == TiltAngleType.RANGE) {
        overrideParameter(commandMap, DirectiveDef.FIRST_INC.getName(AxisID.FIRST),
      ...
      // not been deleted by checkTiltAngleFiles()
        ...
        overrideParameter(commandMap, DirectiveDef.USE_RAW_TLT.getName(AxisID.FIRST),
          "1");
      }
      ...
      if (metaData.getAxisType() == AxisType.DUAL_AXIS) {
        ...
        // exists.
        String key = DirectiveDef.BROTATION.getName(AxisID.SECOND);
        if (!commandMap.containsKey(key)) {
        ...
        if (metaData.getTiltAngleSpecB().getType() == TiltAngleType.RANGE) {
          overrideParameter(commandMap, DirectiveDef.BFIRST_INC.getName(AxisID.SECOND),
        ...
        // Take tilt angle from a .rawtlt file - B
          ...
          overrideParameter(commandMap, DirectiveDef.BUSE_RAW_TLT.getName(AxisID.SECOND),
            "1");
        }
        // Extract the tilt angle data from the stack - B
        else if (metaData.getTiltAngleSpecB().getType() == TiltAngleType.EXTRACT) {
          overrideParameter(commandMap, DirectiveDef.BEXTRACT.getName(AxisID.SECOND),
            "1");
        }
        // List of views to exclude from processing - B
        excludeProjections = metaData.getExcludeProjectionsB();
        if (!excludeProjections.equals("")) {
          overrideParameter(commandMap, DirectiveDef.BSKIP.getName(AxisID.SECOND),
            excludeProjections);
        }
        if (metaData.isTwodir(AxisID.SECOND)) {
          overrideParameter(commandMap, DirectiveDef.BTWODIR.getName(AxisID.SECOND),
            metaData.getTwodir(AxisID.SECOND));
        }
        if (montage && gradient && metaData.getAdjustedFocusB().is()) {
          overrideParameter(commandMap, DirectiveDef.BFOCUS.getName(AxisID.SECOND), "1");
        }
      }
    }
    else if (directiveFileCollection != null) {
      // For directive-driven automation include command line items that are associated
      // with copyarg directives.
      Set<Entry<String, String>> commandLineSet =
        directiveFileCollection.getCopyArgCommandLineSet();
      if (commandLineSet != null) {
        Iterator<Entry<String, String>> iterator = commandLineSet.iterator();
        if (iterator != null) {
          while (iterator.hasNext()) {
            Entry<String, String> entry = iterator.next();
            commandMap.put(entry.getKey(), entry.getValue());
    ...
    if (entrySet != null) {
      ...
      while (iterator.hasNext()) {
        ...
        if (!stackExtSet && DirectiveDef.STACK_EXT.getName().startsWith(key)) {
          stackExtSet = true;
        }
      }
    }
    ...
    if (!stackExtSet) {
      ...
      while (iterator.hasNext()) {
        ...
        if (DirectiveDef.STACK_EXT.getName().startsWith(pair)) {
          stackExtSet = true;
          break;
        }
      }
    ...
    if (!stackExtSet) {
      command.add("StackExtension " + metaData.getRawImageStackExtension());
    }
    command.add("NamingStyle " + metaData.getImageFilenameStyle().toString());
    File file = null;
    if (directiveFileCollection != null) {
      DirectiveFile directiveFile =
        directiveFileCollection.getDirectiveFile(DirectiveFileType.BATCH_DEFAULTS);
      if (directiveFile != null) {
        file = directiveFile.getFile();
        if (file != null) {
          command.add(CHANGE_PARAMETERS_FILE_TAG + " " + file.getAbsolutePath());
        }
      }
  ...
  public int run() {
    ...
    String[] stdInput = copytomocoms.getStdInput();
    if (stdInput != null && debug) {
