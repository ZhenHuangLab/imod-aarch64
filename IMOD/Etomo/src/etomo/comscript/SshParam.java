...
final class SshParam {
  ...
  final List<String> getCommand(BaseManager manager, boolean useTimeoutIfPossible,
    String computer) {
  ...
  synchronized boolean isTimeoutAvailable(BaseManager manager) {
    ...
    SystemProgram systemProgram = new SystemProgram(manager,
      System.getProperty("user.dir"), new String[] { "ssh", "-V" }, AxisID.ONLY);
    systemProgram.run();
    // Find and parse the OpenSSH version.
    System.err.println("For isTimeoutAvailable, ssh -V response:");
    systemProgram.printStdError();
    systemProgram.printStdOutput();
    ...
    if (stderr != null && stderr.length > 0) {
      ...
      for (i = 0; i < stderr.length; i++) {
        if (stderr[i].toLowerCase().indexOf(appString) != -1) {
          found = true;
          break;
        }
      }
      if (found) {
        ...
        if (versionInfoArray != null && versionInfoArray.length > 0) {
          i = 0;
          while (versionInfoArray.length < i + 1
            && versionInfoArray[i++].toLowerCase().indexOf(appString) == -1) {}
          if (i < versionInfoArray.length) {
            EtomoVersion openSshVersion =
              EtomoVersion.getDefaultInstance(versionInfoArray[i]);
