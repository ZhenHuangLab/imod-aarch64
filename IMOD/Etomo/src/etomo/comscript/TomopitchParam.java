...
public class TomopitchParam extends ConstTomopitchParam implements CommandParam {
  public TomopitchParam(final ApplicationManager manager, final AxisID axisID) {
  ...
  public void parseComScriptCommand(ComScriptCommand scriptCommand)
    ...
    if (scriptCommand.isKeywordValuePairs()) {
      ...
      if (scriptCommand.hasKeyword(extraThickness.getName())) {
        extraThickness.set(scriptCommand.getValue(extraThickness.getName()));
      }
    ...
    int inputLine = 0;
    extraThickness.set(inputArgs[inputLine++].getArgument());
  ...
  public void updateComScriptCommand(ComScriptCommand scriptCommand)
    ...
    extraThickness.updateComScript(scriptCommand);
  ...
  public void setExtraThickness(String extraThickness) {
    this.extraThickness.set(extraThickness);
  }
  ...
  public void setScaleFactor(final boolean dialogWholeTomogram) {
    PosSampleType posSampleType = null;
    if (manager != null) {
      TomogramState state = manager.getState();
      if (state != null) {
        posSampleType = PosSampleType.getInstance(state.getPosSampleType(axisID));
      }
    }
    // Use the dialog whole tomogram as a backup. The state contains what was last run.
    FileType imageFile = FileType.TILT_OUTPUT;
    if (posSampleType == PosSampleType.SAMPLES
      || (posSampleType == null && !dialogWholeTomogram)) {
      imageFile = FileType.TOP_SAMPLE;
    }
    scaleFactor = etomo.util.Utilities.getStackBinning(manager, axisID, imageFile);
  }
