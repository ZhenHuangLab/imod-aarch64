 ...
 * <p>Description: </p>
 * 
 * <p>Copyright: Copyright 2005 - 2020 by the Regents of the University of Colorado</p>
 * 
...
public final class BlendmontParam implements CommandParam, CommandDetails {
  ...
  private final StringParameter rootNameForEdges =
    new StringParameter("RootNameForEdges");
  private final ScriptParameter imagesAreBinned =
    new ScriptParameter(EtomoNumber.Type.DOUBLE, "ImagesAreBinned");
  private final EtomoBoolean2 sloppyMontage = new EtomoBoolean2("SloppyMontage");
  private final EtomoBoolean2 verySloppyMontage =
    new EtomoBoolean2(VERY_SLOPPY_MONTAGE_KEY);
  private final ScriptParameter robustFitCriterion =
    new ScriptParameter(EtomoNumber.Type.DOUBLE, ROBUST_FIT_CRITERION_KEY);
  private final ScriptParameter fillValue =
    new ScriptParameter(EtomoNumber.Type.DOUBLE, FILL_VALUE_KEY);
  ...
  private final FortranInputString startingAndEndingX =
    new FortranInputString(STARTING_AND_ENDING_X_KEY, 2);
  private final FortranInputString startingAndEndingY =
    new FortranInputString(STARTING_AND_ENDING_Y_KEY, 2);
  private final EtomoNumber imageRotation = new EtomoNumber(EtomoNumber.Type.DOUBLE);
  private final FortranInputString unalignedStartingXandY =
    new FortranInputString("UnalignedStartingXandY", 2);
  ...
  public BlendmontParam(final BaseManager manager, final String datasetName,
    ...
    interpolationOrder =
      new ScriptParameter(EtomoNumber.Type.INTEGER, "InterpolationOrder");
  ...
  public boolean convertToStartingAndEndingXandY(String sizeToOutputInXandY,
    ...
    if (validate && description != null) {
      if (!fisSizeToOutputInXandY.isNull(0) && fisSizeToOutputInXandY.isNull(1)) {
        UIHarness.INSTANCE.openMessageDialog(manager,
          "Two values are required for " + description + ".", "Entry Error", axisID);
        retval = false;
      }
    }
    if (fisSizeToOutputInXandY.isDefault() || fisSizeToOutputInXandY.isEmpty()) {
    ...
    Montagesize montagesize =
      Montagesize.getInstance(manager, axisID, FileType.RAW_STACK);
  ...
  public void setImageOutputFile(final FileType fileType, final String rootName,
    ...
    imageOutputFile = fileType.deriveFileName(rootName, axisType, axisID,
      metaData.getImageFilenameStyle(), metaData.getRawImageStackExtension());
  ...
  public boolean setBlendmontState(final ConstEtomoNumber invalidEdgeFunctions) {
    return setBlendmontState(invalidEdgeFunctions, false);
  }
  ...
  public boolean setBlendmontState(final ConstEtomoNumber invalidEdgeFunctions,
    ...
    if (mode == Mode.UNDISTORT) {
      ...
      if (!imageOutputFileFor3dFind) {
        ...
        if (mode == Mode.SERIAL_SECTION_BLEND) {
          sloppyMontage.set(true);
          imageOutputFileType = FileType.PREBLEND_OUTPUT_MRC;
        }
      }
    }
    File ecdFile = new File(manager.getPropertyUserDir(),
      datasetName + axisID.getExtension() + ".ecd");
    File xefFile = new File(manager.getPropertyUserDir(),
      datasetName + axisID.getExtension() + ".xef");
    File yefFile = new File(manager.getPropertyUserDir(),
      datasetName + axisID.getExtension() + ".yef");
    // ReadInXcorrs is a checkbox in serial sections. It should have been already set. Do
    // not change it here.
    if (mode != Mode.SERIAL_SECTION_PREBLEND) {
      ...
      readInXcorrs.set(
        mode == Mode.PREBLEND || mode == Mode.BLEND || mode == Mode.SERIAL_SECTION_BLEND
    ...
    // Robust fitting makes new edge functions, so don't turn this on if robust fitting
    // has changed.
    // If readInXcorrs is off, the ecdfile will change - so edge functions must be
    // updated.
    oldEdgeFunctions.set(readInXcorrs.is() && !robustFittingChanged
      && (mode == Mode.BLEND || mode == Mode.SERIAL_SECTION_BLEND
        || mode == Mode.WHOLE_TOMOGRAM_SAMPLE || mode == Mode.SERIAL_SECTION_PREBLEND
        || (invalidEdgeFunctions.getInt() != EtomoState.TRUE_VALUE && xefFile.exists()
          && yefFile.exists() && ecdFile.lastModified() <= xefFile.lastModified()
    ...
    if (mode == Mode.XCORR) {
      ...
      // stack.
      File stackFile = FileType.RAW_STACK.getFile(manager, axisID);
      File blendFile = new File(manager.getPropertyUserDir(),
        FileType.XCORR_BLEND_OUTPUT.getFileName(manager, axisID)
  ...
  public double getDoubleValue(final FieldInterface field) {
    if (field == Field.ROBUST_FITTING) {
      return robustFitCriterion.getDouble();
    }
    throw new IllegalArgumentException("field=" + field);
  }
  ...
  public static File getDistortionCorrectedFile(final BaseManager manager,
    final String workingDir, AxisID axisID) {
    return new File(workingDir,
      FileType.DISTORTION_CORRECTED_STACK.getFileName(manager, axisID)
  ...
  public boolean isReadInXcorrs() {
    return readInXcorrs.is();
  }

  public void setReadInXcorrs(final boolean input) {
    readInXcorrs.set(input);
  }
  ...
  public void resetRobustFitCriterion() {
    robustFitCriterion.reset();
  }
  ...
  public static final class Field implements FieldInterface {
    ...
    public static final Field ROBUST_FITTING = new Field();
  }
  ...
  public final static class Mode implements CommandMode {
    ...
    public static final Mode SERIAL_SECTION_PREBLEND =
      new Mode("SerialSections_Preblend");
