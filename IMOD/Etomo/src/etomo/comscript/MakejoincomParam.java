...
public final class MakejoincomParam implements CommandDetails {
  ...
  private ArrayList genOptions() {
    ...
    if (sectionData != null) {
      ...
      for (int i = 0; i < totalRows; i++) {
        ...
        if (screen.isRotated()) {
          ...
          String rotFileName =
            DatasetTool.substituteExtension(section.getName(), Extension.ROT);
    ...
    options.add("-tmpext");
    options.add(Extension.ROT.toString());
    ...
    options.add("-NamingStyle ");
    options.add(metaData.getImageFilenameStyle().toString());
    return options;
  }
