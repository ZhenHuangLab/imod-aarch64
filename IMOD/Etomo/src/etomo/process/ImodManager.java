 ...
 * <p> </p>
 **/
public final class ImodManager extends BaseImodManager {
  ...
  public static final String PATCH_VECTOR_CCC_MODEL_KEY =
    new String("patch vector ccc model");
  ...
  public static final String VARYING_ITERATION_TEST_KEY =
    new String("VaryingIterationTest");
  public static final String ANISOTROPIC_DIFFUSION_VOLUME_KEY =
    new String("AnisotropicDiffusionVolume");
  ...
  public static final String FINE_ALIGNED_3D_FIND_KEY =
    new String("fine aligned for findbeads3d");
  public static final String FULL_VOLUME_3D_FIND_KEY =
    new String("full volume for findbeads3d");
  public static final String SMOOTHING_ASSESSMENT_KEY =
    new String("Smoothing assessment flattenwarp output");
  public static final String FLATTEN_INPUT_KEY = new String("Flatten input file");
  public static final String FLATTEN_TOOL_OUTPUT_KEY =
    new String("Flatten tool output file");
  ...
  public static final String BATCH_RUN_TOMO_REC_KEY = new String("BatchRunTomo Tomogram");
  public static final String BATCH_RUN_TOMO_TRIMMED_VOLUME_KEY =
    new String("BatchRunTomo Trimmed Volume");
  public static final String MULTIFILT_KEY =
    new String("processchunks tilt_mulfil output");
  ...
  private static final String batchRunTomoRecKey = BATCH_RUN_TOMO_REC_KEY;
  private static final String batchRunTomoTrimmedVolumeKey =
    BATCH_RUN_TOMO_TRIMMED_VOLUME_KEY;
  private static final String multifiltKey = MULTIFILT_KEY;

  public ImodManager(final BaseManager manager) {
    super(manager);
  }
   ...
   */
  public void setPreviewMetaData(final BaseMetaData metaData) {
    if (metaDataSet) {
      return;
    }
  ...
  public void setMetaData(final ConstMetaData metaData) {
  ...
  public void setMetaData(final JoinMetaData metaData) {
    metaDataSet = true;
    setAxisType(metaData.getAxisType());
  ...
  public void setMetaData(final SerialSectionsMetaData metaData) {
    metaDataSet = true;
    setAxisType(metaData.getAxisType());
    datasetName = metaData.getName();
    if (datasetName.equals("")) {
  ...
  public void setMetaData(final ConstPeetMetaData metaData) {
    metaDataSet = true;
    setAxisType(metaData.getAxisType());
    datasetName = metaData.getName();
    if (datasetName.equals("")) {
  ...
  public void setMetaData(final ParallelMetaData metaData) {
    metaDataSet = true;
    setAxisType(metaData.getAxisType());
    datasetName = metaData.getName();
    if (datasetName.equals("")) {
  ...
  protected ImodState newImodState(final String key, final String fileExtension,
    ...
    if (key.equals(COMBINED_TOMOGRAM_KEY) && equalsAxisType(AxisType.DUAL_AXIS)) {
      return newCombinedTomogram();
    }
    if (key.equals(PATCH_VECTOR_MODEL_KEY) && equalsAxisType(AxisType.DUAL_AXIS)) {
      return newPatchVectorModel();
    }
    if (key.equals(MATCH_CHECK_KEY) && equalsAxisType(AxisType.DUAL_AXIS)) {
    ...
    if (key.equals(PATCH_VECTOR_CCC_MODEL_KEY) && equalsAxisType(AxisType.DUAL_AXIS)) {
    ...
    if (key.equals(BATCH_RUN_TOMO_STACK_KEY)) {
      return newBatchRunTomoStack(file);
    }
    if (key.equals(BATCH_RUN_TOMO_REC_KEY)) {
      return newBatchRunTomoRec(file);
    }
    if (key.equals(BATCH_RUN_TOMO_TRIMMED_VOLUME_KEY)) {
      return newBatchRunTomoTrimmedVolume(file);
    }
    if (key.equals(MULTIFILT_KEY) && axisID != null) {
      return newMultiFilt(fileList, axisID);
    }
    System.out.println("key=" + key);
    throw new IllegalArgumentException(key + " cannot be created in "
      + getAxisTypeString().toString() + " with axisID=" + axisID.getExtension());
  }

  private void createPrivateKeys() {
    if (equalsAxisType(AxisType.SINGLE_AXIS)) {
  ...
  protected String getPrivateKey(final String publicKey) {
  ...
  private ImodState newRawStack(final AxisID axisID, final File file) {
    ...
    if (file == null) {
      imodState =
        new ImodState(manager, FileType.RAW_STACK.getFile(manager, axisID), axisID);
  ...
  private ImodState newErasedStack(final AxisID axisID) {
    ImodState imodState = new ImodState(manager, FileType.FIXED_XRAYS_STACK, axisID);
    return imodState;
  }

  private ImodState newCoarseAligned(final AxisID axisID) {
    ImodState imodState;
    imodState = new ImodState(manager, FileType.PREALIGNED_STACK, axisID);
    return imodState;
  }

  private ImodState newFineAligned(final AxisID axisID) {
    ImodState imodState = new ImodState(manager, axisID, FileType.ALIGNED_STACK);
    return imodState;
  }

  private ImodState newSample(final AxisID axisID) {
    ImodState imodState = new ImodState(manager, axisID, FileType.TOP_SAMPLE,
      FileType.MIDDLE_SAMPLE, FileType.BOTTOM_SAMPLE, "tomopitch", ".mod");
    imodState.setInitialMode(ImodState.MODEL_MODE);
    return imodState;
  }

  private ImodState newFullVolume(final AxisID axisID) {
    ImodState imodState;
    imodState = new ImodState(manager,
      FileType.TILT_OUTPUT/*was: datasetName + "_full.rec"*/, axisID);
  ...
  private ImodState newCombinedTomogram() {
    ImodState imodState =
      new ImodState(manager, FileType.COMBINED_VOLUME /*was: "sum.rec"*/, AxisID.ONLY);
    imodState.setAllowMenuBinningInZ(true);
    imodState.setInitialSwapYZ(true);
    return imodState;
  }
  ...
  /**
   * No need to add file name style because no new files are being created.
   * @deprecated files are no longer created by IMOD
   * @return
   */
  ...
  private ImodState newFiducialModel(final AxisID axisID) {
    ImodState imodState = new ImodState(manager, ImodState.MODV, axisID);
    imodState.setNoMenuOptions(true);
    return imodState;
  }

  private ImodState newSortedModels(final AxisID axisID) {
    ImodState imodState = new ImodState(manager, ImodState.MODV, axisID);
  ...
  private ImodState newTrimmedVolume() {
    ImodState imodState = new ImodState(manager,
      FileType.TRIM_VOL_OUTPUT /*was: datasetName + ".rec"*/, AxisID.ONLY);
    imodState.setAllowMenuBinningInZ(true);
    return imodState;
  }

  private ImodState newTrialTomogram(final AxisID axisID, final String datasetName) {
  ...
  private ImodState newMtfFilter(final AxisID axisID) {
    ImodState imodState = // new ImodState(manager, axisID, datasetName, "_filt.ali");
      new ImodState(manager, axisID, FileType.MTF_FILTERED_STACK);
    return imodState;
  }

  private ImodState newCtfCorrection(final AxisID axisID) {
    ImodState imodState = new ImodState(manager, axisID, FileType.CTF_CORRECTED_STACK);
    return imodState;
  }

  private ImodState newErasedFiducials(final AxisID axisID) {
    ImodState imodState = new ImodState(manager, axisID, FileType.ERASED_BEADS_STACK);
    return imodState;
  }

  private ImodState newFlatVolume(final AxisID axisID) {
    ImodState imodState =
      new ImodState(manager, ImageFileType.FLATTEN_OUTPUT.getFile(manager), axisID);
    return imodState;
  }

  private ImodState newFlattenToolOutput(final AxisID axisID) {
    ImodState imodState = new ImodState(manager, axisID,
      FileType.FLATTEN_TOOL_OUTPUT.getFileName(manager, axisID));
    return imodState;
  }

  private ImodState newPreblend(final AxisID axisID) {
    ImodState imodState = new ImodState(manager, axisID,
      FileType.PREBLEND_OUTPUT_MRC.getFileName(manager, axisID));
    return imodState;
  }

  private ImodState newAlignedStack(final AxisID axisID) {
    ImodState imodState = new ImodState(manager, axisID,
  ...
  private ImodState newPreview(final AxisID axisID, final String fileExtension) {
    ...
    ImodState imodState =
      new ImodState(manager, datasetName + axisExtension + fileExtension, axisID);
    imodState.setLoadAsIntegers();
    imodState.setSuppressSaveQuery();
    return imodState;
  }

  private ImodState newTomogram(final File file, final AxisID axisID) {
  ...
  private ImodState newJoinSamples() {
    ...
    ImodState imodState = new ImodState(manager,
      FileType.JOIN_SAMPLE /*was: datasetName + ".sample"*/, AxisID.ONLY);
    return imodState;
  }
  ...
  private ImodState newJoinSampleAverages() {
    ...
    ImodState imodState = new ImodState(manager,
      FileType.JOIN_SAMPLE_AVERAGES/*was: datasetName + ".sampavg"*/, AxisID.ONLY);
    return imodState;
  }

  private ImodState newJoin() {
    ImodState imodState =
      new ImodState(manager, FileType.JOIN /*was: datasetName + ".join"*/, AxisID.ONLY);
    return imodState;
  }

  private ImodState newRotTomogram(final File file) {
    ImodState imodState = new ImodState(manager, file, AxisID.ONLY);
    return imodState;
  }

  private ImodState newAvgVol(final String[] fileNameArray) {
  ...
  private ImodState newRef(final String[] fileNameArray) {
    ImodState imodState = new ImodState(manager, fileNameArray, AxisID.ONLY);
    return imodState;
  }

  private ImodState newVolume(final File file) {
    ImodState imodState = new ImodState(manager, file, AxisID.ONLY);
    return imodState;
  }

  private ImodState newTestVolume(final File file) {
    ImodState imodState = new ImodState(manager, file, AxisID.ONLY);
    return imodState;
  }

  private ImodState newVaryingKTest(final String[] fileNameArray,
  ...
  private ImodState newMultiFilt(final File[] fileList, final AxisID axisID) {
    ImodState imodState = new ImodState(manager, fileList, axisID);
    imodState.setInitialSwapYZ(true);
    return imodState;
  }

  private ImodState newVaryingIterationTest(final String[] fileNameArray,
    final String subdirName) {
    ImodState imodState = new ImodState(manager, fileNameArray, AxisID.ONLY, subdirName);
    return imodState;
  }

  private ImodState newAnisotropicDiffusionVolume(final File file) {
    ImodState imodState = new ImodState(manager, file, AxisID.ONLY);
    return imodState;
  }
  ...
  private ImodState newModeledJoin() {
    ImodState imodState = new ImodState(manager,
      FileType.MODELED_JOIN/*datasetName + "_modeled.join"*/, AxisID.ONLY);
    imodState.setInitialMode(ImodState.MODEL_MODE);
  ...
  private ImodState newTrialJoin() {
    ImodState imodState = new ImodState(manager,
      FileType.TRIAL_JOIN /*datasetName + "_trial.join"*/, AxisID.ONLY);
    return imodState;
  }

  private ImodState newSqueezedVolume() {
    ImodState imodState = new ImodState(manager,
      FileType.SQUEEZE_VOL_OUTPUT/*was:datasetName + ".sqz"*/, AxisID.ONLY);
    imodState.setAllowMenuBinningInZ(true);
    return imodState;
  }

  private ImodState newFineAligned3dFind(final AxisID axisID) {
  ...
  private ImodState newFullVolume3dFind(final AxisID axisID) {
    ImodState imodState = new ImodState(manager, axisID,
  ...
  private ImodState newSmoothingAssessment(final AxisID axisID) {
    ImodState imodState = new ImodState(manager, ImodState.MODV, axisID);
    imodState.setNoMenuOptions(true);
  ...
  private ImodState newFlattenInput(final File file) {
    return new ImodState(manager, file, AxisID.ONLY);
  }

  private ImodState newBatchRunTomoStack(final File file) {
    ImodState imodState = new ImodState(manager, file, AxisID.ONLY);
    imodState.setLoadAsIntegers();
    return imodState;
  }

  private ImodState newBatchRunTomoRec(final File file) {
    ImodState imodState = new ImodState(manager, file, AxisID.ONLY);
    imodState.setLoadAsIntegers();
    imodState.setSwapYZ(true);
    return imodState;
  }

  private ImodState newBatchRunTomoTrimmedVolume(final File file) {
    ImodState imodState = new ImodState(manager, file, AxisID.ONLY);
    imodState.setLoadAsIntegers();
    return imodState;
  }
