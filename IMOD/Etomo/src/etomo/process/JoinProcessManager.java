...
public final class JoinProcessManager extends BaseProcessManager {
  ...
  public String remapmodel(RemapmodelParam param, final ProcessSeries processSeries)
    ...
    BackgroundProcess backgroundProcess =
      startBackgroundProcess(param, AxisID.ONLY, ProcessName.REMAPMODEL, processSeries);
    return backgroundProcess.getName();
  }
  ...
  public String xftoxg(XftoxgParam param, final ProcessSeries processSeries)
    throws AxisBusyException {
    BackgroundProcess backgroundProcess =
      startBackgroundProcess(param, AxisID.ONLY, ProcessName.XFTOXG, processSeries);
    return backgroundProcess.getName();
  }
  ...
  public String xfjointomo(XfjointomoParam xfjointomoParam,
    ...
    BackgroundProcess backgroundProcess =
      startBackgroundProcess(xfjointomoParam.getCommandArray(), AxisID.ONLY, null,
  ...
  public String rotx(ClipParam clipyzParam, final ProcessSeries processSeries)
    throws AxisBusyException {
    BackgroundProcess backgroundProcess =
      startBackgroundProcess(clipyzParam, AxisID.ONLY, ProcessName.CLIP, processSeries);
    return backgroundProcess.getName();
  }
  ...
  public String startjoin(StartJoinParam param, final ProcessSeries processSeries)
    ...
    ComScriptProcess comScriptProcess =
      startComScript(param, null, AxisID.ONLY, processSeries);
  ...
  protected void postProcess(ComScriptProcess process) {
    ...
    if (commandName.equals(startjoinComscriptName)) {
      ...
      if (processDetails.getBooleanValue(StartJoinParam.Fields.ROTATE)) {
        ...
        state.setRotationAnglesList(
          processDetails.getHashtable(StartJoinParam.Fields.ROTATION_ANGLES_LIST));
      }
    }
  }
    ...
    if (commandName.equals(ClipParam.PROCESS_NAME.toString())) {
      ...
      if (mode == FinishjoinParam.Mode.MAX_SIZE) {
        ...
        if (processDetails != null) {
          ...
          state.setJoinAlignmentRefSection(trial,
            processDetails.getEtomoNumber(FinishjoinParam.Fields.ALIGNMENT_REF_SECTION));
          ...
          if (mode == FinishjoinParam.Mode.TRIAL) {
            state.setJoinTrialBinning(
              processDetails.getEtomoNumber(FinishjoinParam.Fields.BINNING));
            state.setJoinTrialUseEveryNSlices(
              processDetails.getEtomoNumber(FinishjoinParam.Fields.USE_EVERY_N_SLICES));
          }
        }
        ...
        state.setRefineStartList(
          processDetails.getIntKeyList(FinishjoinParam.Fields.REFINE_START_LIST));
        state.setRefineEndList(
          processDetails.getIntKeyList(FinishjoinParam.Fields.REFINE_END_LIST));
        manager.updateJoinDialogDisplay();
      }
      else if (mode == FinishjoinParam.Mode.TRIAL_REJOIN) {
        state.setRefineTrialUseEveryNSlices(
          processDetails.getEtomoNumber(FinishjoinParam.Fields.USE_EVERY_N_SLICES));
      }
    }
      ...
      state
        .setXfModelOutputFile(processDetails.getString(XfmodelParam.Fields.OUTPUT_FILE));
    }
  }
   ...
   * Run joinwarp2model
   */
  public String joinwarp2model(final Joinwarp2modelParam param,
    final ProcessSeries processSeries) throws AxisBusyException {
    ComScriptProcess comScriptProcess = startComScript(
      FileType.JOIN_WARP_2_MODEL_COMSCRIPT.getFileName(manager, AxisID.ONLY), AxisID.ONLY,
      processSeries, FileType.JOIN_WARP_2_MODEL_COMSCRIPT);
    return comScriptProcess.getName();
  }
}