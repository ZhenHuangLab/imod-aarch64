...
class BackgroundProcess extends Thread implements SystemProcessInterface {
  ...
  BackgroundProcess(BaseManager manager, List<String> commandArrayList,
    ...
    if (processSeries != null) {
      ...
      processData.setLastProcess(processSeries,
        processName == null ? false : processName.resumable);
    }
  ...
  BackgroundProcess(BaseManager manager, Command command,
    BaseProcessManager processManager, AxisID axisID,
    ProcessResultDisplay processResultDisplay, ProcessName processName,
    final ProcessSeries processSeries, final boolean allowMultiLineLog) {
    this.manager = manager;
    this.axisID = axisID;
    this.command = command;
    this.commandArray = command.getCommandArray();
  ...
  public final void
    setProcessResultDisplay(final ProcessResultDisplay processResultDisplay) {
    this.processResultDisplay = processResultDisplay;
  }
  ...
  private String getAbbreviatedCommandLine() {
    String commandLine = getCommandLine();
    ProcessName processName = processData.getProcessName();
    if (commandLine == null || processName == null) {
      return commandLine;
    }
    int index = commandLine.indexOf(processName.toString());
    if (index == -1) {
      return commandLine;
    }
  ...
  boolean newProgram() {
    if (commandArray != null) {
      program = new SystemProgram(manager, manager.getPropertyUserDir(), commandArray,
        axisID, allowMultiLineLog);
    }
    else if (command != null) {
      program = new SystemProgram(manager, manager.getPropertyUserDir(),
        command.getCommandArray(), axisID, allowMultiLineLog);
    }
    else if (commandArrayList != null) {
      program = new SystemProgram(manager, manager.getPropertyUserDir(), commandArrayList,
        axisID, allowMultiLineLog);
    }
    else {
      processDone(1);
  ...
  final void processDone(final int exitValue) {
    ...
    if (exitValue == 0) {
      ...
      if (popupChunkWarnings && !errorFound && monitorMessages != null) {
        ...
        // putting one out.
        int size = monitorMessages.size(ProcessMessages.MessageType.CHUNK_WARNING);
        if (size > 0) {
          ...
          warningMessage.add(ProcessMessages.MessageType.WARNING,
            "<html><U>Chunk warnings:</U>");
          for (int i = 0; i < size; i++) {
            warningMessage.add(ProcessMessages.MessageType.WARNING,
              monitorMessages.get(ProcessMessages.MessageType.CHUNK_WARNING, i));
          }
      ...
      errorMessage.add(ProcessMessages.MessageType.ERROR,
        "<html>Command failed: " + getAbbreviatedCommandLine());
      ...
      if (monitorMessages != null) {
        int size = monitorMessages.size(ProcessMessages.MessageType.CHUNK_ERROR);
        if (size > 0) {
          errorMessage.add(ProcessMessages.MessageType.ERROR);
          errorMessage.add(ProcessMessages.MessageType.ERROR,
          ...
          for (int i = 0; i < size; i++) {
            errorMessage.add(ProcessMessages.MessageType.ERROR,
              monitorMessages.get(ProcessMessages.MessageType.CHUNK_ERROR, i));
          }
        }
        ...
        if (!monitorMessages.isEmpty(ProcessMessages.MessageType.ERROR)) {
          errorMessage.add(ProcessMessages.MessageType.ERROR);
          errorMessage.add(ProcessMessages.MessageType.ERROR,
            "<html><U>Monitor error messages:</U>", monitorMessages);
        }
      }*/
      ...
      errorMessage.addProcessOutput("Program output:", stdOutput);
