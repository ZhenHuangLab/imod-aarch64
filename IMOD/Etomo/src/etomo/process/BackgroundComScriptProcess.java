 ...
 * <p> </p>
 */
class BackgroundComScriptProcess extends ComScriptProcess {
   ...
   */
  BackgroundComScriptProcess(final BaseManager manager, final String comScript,
  ...
  void closeOutputImageFile() {
    ...
    manager.closeStaleFile(comscriptState.getOutputImageFileKey(), axisID);
    manager.closeStaleFile(comscriptState.getOutputImageFileKey2(), axisID);
    manager.closeStaleFile(comscriptState.getOutputImageFileKey3(), axisID);
  }
  ...
  boolean isComScriptBusy() {
    ...
    for (int i = 1; i < stdout.length; i++) {
      ...
      if (stdout[i].substring(nameIndex).trim().equals(comscriptLog.getAbsolutePath())) {
        System.err.println("\nisComScriptBusy:");
        System.err.println("lsof:stdout[" + i + "]:" + stdout[i]);
        System.err.println(
  ...
  boolean renameFiles() {
    ...
    while (index <= endCommand) {
      try {
        renameFiles(comscriptState.getWatchedFile(index), getWorkingDirectory(),
