...
public final class SerialSectionsProcessManager extends BaseProcessManager {
  ...
  public String blend(final BlendmontParam blendmontParam, final AxisID axisID,
    ...
    BlendmontProcessMonitor blendmontProcessMonitor =
      new BlendmontProcessMonitor(manager, axisID, blendmontParam.getMode());
    // Start the com script in the background
  ...
  public String xftoxg(final XftoxgParam param, final AxisID axisID,
    ...
    BackgroundProcess backgroundProcess =
      startBackgroundProcess(param, axisID, ProcessName.XFTOXG, processSeries);
    return backgroundProcess.getName();
  }
  ...
  public String newst(final ConstNewstParam newstParam, final AxisID axisID,
    ...
    NewstProcessMonitor newstProcessMonitor =
      new NewstProcessMonitor(manager, axisID, ProcessName.NEWST, newstParam);
    // Start the com script in the background
    ComScriptProcess comScriptProcess =
      startComScript(newstParam, newstProcessMonitor, axisID, null, processSeries);
    return comScriptProcess.getName();
  }
  ...
  private void setInvalidEdgeFunctions(Command command, boolean succeeded) {
    if (manager.getViewType() == ViewType.MONTAGE
      ...
      && (command.getCommandMode() == BlendmontParam.Mode.SERIAL_SECTION_PREBLEND
        || command.getCommandMode() == BlendmontParam.Mode.SERIAL_SECTION_BLEND)) {
  ...
  protected void postProcess(ComScriptProcess script) {
    try {
      ...
      if (processName == ProcessName.PREBLEND) {
        ...
        state.setPreblendRobustFitting(
          processDetails.getDoubleValue(BlendmontParam.Field.ROBUST_FITTING));
        manager.msgPreblendSucceeded();
      }
