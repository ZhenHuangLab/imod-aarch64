...
public class SystemProgram implements Runnable {
  ...
  SystemProgram(final BaseManager manager, final String propertyUserDir,
    final List<String> command, final AxisID axisID, final boolean allowMultiLineLog) {
    this.manager = manager;
    this.propertyUserDir = propertyUserDir;
    this.axisID = axisID;
    processMessages = ProcessMessages.getMultiLineInstance(manager, allowMultiLineLog);
    commandArray = new String[command.size()];
    for (int i = 0; i < command.size(); i++) {
      commandArray[i] = command.get(i);
    }
  ...
  public static SystemProgram getMultiLineInstance(final BaseManager manager,
    final String propertyUserDir, final String[] cmdArray, final AxisID axisID) {
    return new SystemProgram(manager, propertyUserDir, cmdArray, axisID, true, false);
  }

  SystemProgram(final BaseManager manager, final String propertyUserDir,
    final String[] cmdArray, final AxisID axisID, final boolean allowMultiLineLog) {
    this.manager = manager;
    this.propertyUserDir = propertyUserDir;
    this.axisID = axisID;
    processMessages = ProcessMessages.getInstance(manager, allowMultiLineLog);
    commandArray = cmdArray;
  }

  public SystemProgram(final BaseManager manager, final String propertyUserDir,
    final String[] cmdArray, final AxisID axisID, final boolean multilineWarning,
    final boolean multilineInfo, final boolean logInfoMessages) {
    this.manager = manager;
    this.propertyUserDir = propertyUserDir;
    this.axisID = axisID;
   ...
   * @param cmdArray
   * @param axisID
   * @param mutliLineProcessMessages
   */
  public SystemProgram(final BaseManager manager, final String propertyUserDir,
    final String[] cmdArray, final AxisID axisID, final boolean mutliLineProcessMessages,
    final boolean allowMultiLineLog) {
    this.manager = manager;
    this.propertyUserDir = propertyUserDir;
    this.axisID = axisID;
    if (mutliLineProcessMessages) {
      processMessages = ProcessMessages.getMultiLineInstance(manager, allowMultiLineLog);
    }
    else {
      processMessages = ProcessMessages.getInstance(manager, allowMultiLineLog);
  ...
  public void run() {
    ...
    if (debug.isOn()) {
      ...
      else if (debug.isExtra()) {
        maxDebugPrint = 10;
      }
    }

    if (debug.isOn() && commandArray != null && commandArray.length > 0
      && (debug.isVerbose() || (!commandArray[0].equals("env")
        && !commandArray[0].equals("ssh") && !commandArray[0].equals("ps")))) {
      System.err.println();
    ...
    if (debug.isOn()) {
      if (workingDirectory != null) {
        System.err.println(
          "SystemProgram: working directory: " + workingDirectory.getAbsoluteFile());
      }
    }
    ...
    try {
      ...
      if (stdInput != null && stdInput.length > 0) {
        if (debug.isOn()) {
          System.err.println("SystemProgram stdin: " + stdInput.length + " line(s)");
        }
        for (int i = 0; i < Math.min(stdInput.length, maxDebugPrint); i++) {
          System.err.println(stdInput[i]);
        }
        if (maxDebugPrint > 0 && stdInput.length > maxDebugPrint) {
          System.err.println("...");
        }
      }
      ...
      String msg = null;
      if (exitValue == 0) {
        if ((msg = Utilities.getCommandActionMessage(commandAction)) != null) {
          System.err.println(msg);
        }
      }
      else {
        if (debug.isOn()) {
          System.err.println("SystemProgram exit value: " + exitValue);
        }
      }
      ...
      if (stdout != null && size > 0 && debug.isVerbose()) {
        System.err.println("SystemProgram stdout: " + size + " line(s)");
        for (int i = 0; i < Math.min(size, maxDebugPrint); i++) {
          System.err.println(stdout.get(i));
        }
        if (maxDebugPrint > 0 && size > maxDebugPrint) {
          System.err.println("...");
        }
      }

      size = stderr.size();
      if (stderr != null && size > 0) {
        if (debug.isOn()) {
          System.err.println("SystemProgram stderr: " + size + " line(s)");
        }
        for (int i = 0; i < Math.min(size, maxDebugPrint); i++) {
  ...
  public String getStdErrorString() {
    ...
    if (stdErrorArray == null || stdErrorArray.length == 0) {
      return null;
    }
    StringBuilder builder = new StringBuilder();
    for (int i = 0; i < stdErrorArray.length; i++) {
      builder.append(stdErrorArray[i] + "\n");
    }
