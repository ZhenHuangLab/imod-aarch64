 ...
 * <p>Description: </p>
 * 
 * Copyright: Copyright 2002 - 2020 by the Regents of the University of Colorado</p>
 * 
...
public abstract class LogFileProcessMonitor implements ProcessMonitor {
  ...
  abstract void getCurrentSection()
  ...
  public final void run() {
    ...
    try {
      if (standardLogFileName) {
        ...
        logFile =
          LogFile.getInstance(manager.getPropertyUserDir(), axisID, logFileBasename);
      }
      else {
   ...
   */
  private final void waitForLogFile()
    throws InterruptedException, LogFile.LockException, FileNotFoundException {
  ...
  */
  private void calcRemainingTime() {
    ...
    if (doneSections < currentSection - 1) {
      ...
      averageSectionTime =
        ((doneSections * averageSectionTime) + waitTime) / (currentSection - 1);
    ...
    // recalculate updatedAverageSectionTime.
      ...
      updatedAverageSectionTime =
        ((doneSections * averageSectionTime) + waitTime) / (doneSections + 1);
    }
  ...
  void updateProgressBar() {
    ...
    if (etc >= 0) {
      ...
      message =
        String.valueOf(percentDone) + "%   ETC: " + Utilities.millisToMinAndSecs(etc);
    }
    else {
