 ...
 * @version $Revision$
 */
class BlendmontProcessMonitor extends LogFileProcessMonitor {
  ...
  BlendmontProcessMonitor(BaseManager manager, AxisID id, Mode mode) {
    super(manager, id);
    this.mode = mode;
  ...
  void getCurrentSection()
    ...
    if (currentSection >= nSections
      && ((mode != BlendmontParam.Mode.BLEND && mode != BlendmontParam.Mode.BLEND_3DFIND
        && mode != BlendmontParam.Mode.WHOLE_TOMOGRAM_SAMPLE) || lastLineFound)) {
      ending = true;
    }
  }
  ...
  void findNSections() throws InterruptedException, NumberFormatException,
    ...
    montagesize = Montagesize.getInstance(manager, axisID, FileType.RAW_STACK);
