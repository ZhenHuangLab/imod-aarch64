 ...
 * @version $Revision$
 */
final class TiltxcorrProcessWatcher extends LogFileProcessMonitor {
   ...
   * @param id
   */
  TiltxcorrProcessWatcher(final BaseManager manager, final AxisID id,
    ProcessName processName, final boolean runTiltxcorr, final boolean breakContours) {
    super(manager, id);
    logFileBasename = processName.getText();
    if (runTiltxcorr) {
   ...
   *        tiltxcorr output.
   */
  TiltxcorrProcessWatcher(final ApplicationManager appMgr, final AxisID id,
