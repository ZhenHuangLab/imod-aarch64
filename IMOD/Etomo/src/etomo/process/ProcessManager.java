...
public class ProcessManager extends BaseProcessManager {
  ...
  private ProcessMessages setupComScripts(CopyTomoComs copyTomoComs, AxisID axisID,
    ...
    if (exitValue != 0) {
      String stdErrorString = copyTomoComs.getStdErrorString();
      if (stdErrorString != null && !stdErrorString.isEmpty()) {
        UIHarness.INSTANCE.openMessageDialog(appManager, stdErrorString,
          "Copytomocoms Error", axisID);
      }
      return null;
    }
  ...
  public boolean batchruntomo(final AxisID axisID, final BatchruntomoParam param) {
    ...
    boolean retval = true;
    String titlePrefix = "Batchruntomo";
    // process messages
    if (param.getCommandMode() == BatchruntomoParam.Mode.RENAME) {
      titlePrefix = "Raw Image Stack";
      String[] output = param.getStdOutput();
      if (output != null) {
        for (int i = 0; i < output.length; i++) {
          if (output[i].indexOf(ProcessOutputStrings.BRT_RENAMED_MSG_ID) != -1) {
            String file = BatchRunTomoProcessMonitor.getFileFromOutput(output[i],
              ProcessOutputStrings.BRT_RENAMED_MSG_ID, null);
            if (file != null) {
              // The raw image stack has been renamed
              appManager.setRawImageStackExtension(file);
              break;
            }
          }
        }
    ...
    else if (param.getCommandMode() == BatchruntomoParam.Mode.VALIDATION) {
      titlePrefix = "Template Validation";
      ProcessMessages messages = param.getProcessMessages();
      retval = messages.isEmpty(ProcessMessages.MessageType.ERROR);
      if (!retval) {
    ...
    if (exitValue != 0) {
      retval = false;
      String message = param.getStdErrorString();
      if (message == null) {
        message = param.getStdOutputString();
      }
      UIHarness.INSTANCE.openMessageDialog(appManager, message, titlePrefix + " Error",
        axisID);
    }
    return retval;
  }
  ...
  public boolean makecomfile(final AxisID axisID, final MakecomfileParam param) {
    ...
    if (exitValue != 0) {
      String msg = param.getStdErrorString();
      if (msg == null || msg.isEmpty()) {
        msg = param.getStdOutputString();
      }
      UIHarness.INSTANCE.openMessageDialog(appManager, msg, "Makecomfile Error", axisID);
      return false;
    }
    return err;
  }
  ...
  public String eraser(AxisID axisID, ProcessResultDisplay processResultDisplay,
    final ProcessSeries processSeries, Command ccdEraserParam) throws AxisBusyException {
  ...
  public String clipStats(ClipParam param, AxisID axisID,
    final ProcessSeries processSeries) throws AxisBusyException {
    BackgroundProcess backgroundProcess =
      startBackgroundProcess(param, axisID, ProcessName.CLIP, processSeries);
    return backgroundProcess.getName();
  }

  public String findSection(final FindSectionParam param, final AxisID axisID,
    final ProcessSeries processSeries) throws AxisBusyException {
    BackgroundProcess backgroundProcess =
      startBackgroundProcess(param, axisID, ProcessName.FIND_SECTION, processSeries);
    return backgroundProcess.getName();
  }
   ...
   * @throws SystemProcessException
   */
  public void generatePreXG(AxisID axisID)
   ...
   * @param axisID
   */
  public void generateNonFidXF(AxisID axisID)
    throws SystemProcessException, LogFile.LockException {
   ...
   * @param axisID
   */
  public void setupNonFiducialAlign(AxisID axisID)
   ...
   * @deprecated 3/7/2020, Bug# 2315 Don't use SystemProgram w/midas
   */
  public void midasRawStack(AxisID axisID, double imageRotation) {
    // Start the system program thread
    startSystemProgramThread(
      getMidasRawStackCommandLine(getDatasetName(), axisID, imageRotation), axisID,
      appManager);
  }

  /**
   * @deprecated test only, Bug# 2315
   * @param axisID
   */
  public String[] getMidasRawStackCommandLine(final String datasetName, AxisID axisID,
    double imageRotation) {
    // Construct the command line strings
    String[] commandArray = new String[3];

    String stack = FileType.RAW_STACK.getFileName(manager, axisID);
    String xform = datasetName + axisID.getExtension() + ".prexf";
    // Midas must not fork.
    return new String[] { ApplicationManager.getIMODBinPath() + "midas", "-D", "-a",
      String.valueOf(-1 * imageRotation), "-t",
      DatasetFiles.getRawTiltName(appManager, axisID), stack, xform };
  }

  /**
   * Run midas on the specified raw montage stack
   * 
   * @param axisID
   *          the AxisID to run midas on.
   * @deprecated 3/7/2020, Bug# 2315 Don't use SystemProgram w/midas
  ...
  public void midasBlendStack(AxisID axisID, double imageRotation) {
    // Start the system program thread
    startSystemProgramThread(
      getMidasBlendStackCommandLine(getDatasetName(), axisID, imageRotation), axisID,
      appManager);
  }
   ...
   * @param axisID
   */
  public String[] getMidasBlendStackCommandLine(final String datasetName, AxisID axisID,
    double imageRotation) {
    // Construct the command line strings
    String[] commandArray = new String[3];
    ...
    String stack = FileType.XCORR_BLEND_OUTPUT.getFileName(manager, axisID);
    String xform = datasetName + axisID.getExtension() + ".prexf";
    // Midas must not fork.
    return new String[] { ApplicationManager.getIMODBinPath() + "midas", "-D", "-a",
      String.valueOf(-1 * imageRotation), "-t",
  ...
  public void midasFixEdges(AxisID axisID) {
    // Construct the command line strings and Start the system program thread
    startSystemProgramThread(getMidasFixEdgesCommandLine(getDatasetName(), axisID),
      axisID, appManager);
  }
   ...
   * @param axisID
   */
  public String[] getMidasFixEdgesCommandLine(final String datasetName, AxisID axisID) {
    // Construct the command line strings
    String[] commandArray = new String[3];
    FileType fileType;
    ...
    // + "-b 0 -q ";
    String stack = fileType.getFileName(manager, axisID);
    String xform = datasetName + axisID.getExtension() + ".ecd";
    // Midas must not fork.
    return new String[] { ApplicationManager.getIMODBinPath() + "midas", "-D", "-p ",
      datasetName + axisID.getExtension() + ".pl", "-b", "0", "-q", stack, xform };
  }
  ...
  public String createSample(AxisID axisID, ProcessResultDisplay processResultDisplay,
    final ProcessSeries processSeries, ConstTiltParam param) throws AxisBusyException {
    //
  ...
  public String newst(ConstNewstParam newstParam, AxisID axisID,
    ProcessResultDisplay processResultDisplay, final ProcessSeries processSeries,
    ProcessName processName) throws AxisBusyException {
    // Start the com script in the background
  ...
  public String cryoPosition(CryoPositionParam param, AxisID axisID,
    ProcessResultDisplay processResultDisplay, final ProcessSeries processSeries)
    throws AxisBusyException {
    // Start the com script in the background
    ComScriptProcess comScriptProcess =
  ...
  public String multifiltSetup(final AxisID axisID,
    final ProcessResultDisplay processResultDisplay, final ProcessSeries processSeries,
    final MultifiltSetupParam param, final ProcessingMethod processingMethod)
    throws AxisBusyException {
    // Start the com script in the background
    ComScriptProcess comScriptProcess = startComScript(param, null, axisID,
      processResultDisplay, processSeries, processingMethod);
   ...
   */
  public String tilt(final AxisID axisID, final ProcessResultDisplay processResultDisplay,
   ...
   * for this command
   */
  public void modelToPatch(AxisID axisID)
    throws SystemProcessException, LogFile.LockException {
    LogFile patchOut =
   ...
   * @return String
   */
  private String solvematch(final ProcessSeries processSeries) throws AxisBusyException {
   ...
   * @return String
   */
  private String matchvol1(final ProcessSeries processSeries) throws AxisBusyException {
   ...
   * @return String
   */
  public String patchcorr(final ProcessSeries processSeries) throws AxisBusyException {
   ...
   * @return String
   */
  public String matchorwarp(final ProcessSeries processSeries) throws AxisBusyException {
   ...
   * @return String
   */
  public String volcombine(final ProcessSeries processSeries) throws AxisBusyException {
  ...
  public String excludeViews(final Command param, final AxisID axisID,
    final ProcessSeries processSeries) throws AxisBusyException {
    BackgroundProcess backgroundProcess = startBackgroundProcess(param, axisID, null,
      ProcessName.EXCLUDE_VIEWS, processSeries, true);
    return backgroundProcess.getName();
  }
  ...
  public String archiveOrig(ArchiveorigParam param, final ProcessSeries processSeries)
    throws AxisBusyException {
    BackgroundProcess backgroundProcess = startBackgroundProcess(param, AxisID.ONLY, true,
      ProcessName.ARCHIVEORIG, processSeries);
    return backgroundProcess.getName();
  }

  public String restrictalign(final RestrictalignParam param, final AxisID axisID,
    final ProcessSeries processSeries) throws AxisBusyException {
    BackgroundProcess backgroundProcess = startBackgroundProcess(param, axisID, false,
      ProcessName.RESTRICTALIGN, processSeries);
    return backgroundProcess.getName();
  }
  ...
  public String flattenWarp(FlattenWarpParam param,
    ProcessResultDisplay processResultDisplay, final ProcessSeries processSeries,
    AxisID axisID) throws AxisBusyException {
  ...
  public String runraptor(RunraptorParam param, ProcessResultDisplay processResultDisplay,
    final ProcessSeries processSeries, AxisID axisID) throws AxisBusyException {
    BackgroundProcess backgroundProcess = startBackgroundProcess(param.getCommandArray(),
      axisID, processResultDisplay, param.getProcessName(), processSeries);
    return backgroundProcess.getName();
  }
  ...
  protected void postProcess(ComScriptProcess script) {
    try {
      ...
      else if (processName == ProcessName.CRYO_POSITION) {
        appManager.renameCryoPositionOutput(axisID);
        appManager.tomogramPositioningPostProcess(script.getAxisID(),
          command.getSubcommandDetails(), PosSampleType.WHOLE_CRYO);
      }
      else {
  ...
  private void setInvalidEdgeFunctions(Command command, boolean succeeded) {
    if (appManager.getConstMetaData().getViewType() == ViewType.MONTAGE
      ...
      && (command.getCommandMode() == BlendmontParam.Mode.XCORR
        || command.getCommandMode() == BlendmontParam.Mode.PREBLEND)) {
  ...
  void postProcess(BackgroundProcess process) {
    try {
      ...
      if (process.getCommandLine().equals(transferfidCommandLine)) {
      ...
      else if (process.getProcessName() == ProcessName.EXCLUDE_VIEWS) {
        appManager.msgExcludeViewsSucceeded(process.getAxisID());
      }
      else {
        ...
        if (commandName.equals(TrimvolParam.commandName)) {
          ...
          state
            .setTrimvolFlipped(processDetails.getBooleanValue(TrimvolParam.Fields.SWAP_YZ)
          ...
          MRCHeader mrcHeader = MRCHeader.getInstance(appManager.getPropertyUserDir(),
            TrimvolParam.getInputFileName(appManager,
              appManager.getBaseMetaData().getAxisType(),
              appManager.getBaseMetaData().getName()),
            AxisID.ONLY);
          try {
  ...
  void errorProcess(BackgroundProcess process) {
    try {
      if (process.getCommandLine().equals(transferfidCommandLine)) {
        writeLogFile(process, process.getAxisID(), DatasetFiles.TRANSFER_FID_LOG);
        showLogFile(
          new File(appManager.getPropertyUserDir(), DatasetFiles.TRANSFER_FID_LOG));
      }
    }
    catch (Exception e) {
      e.printStackTrace();
