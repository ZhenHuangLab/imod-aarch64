...
public final class ImodState {
  // constants
  // mode
  static final int MODEL_MODE = -1;
  static final int MOVIE_MODE = -2;
  // modelView
  static final int MODEL_VIEW = -3;
  // useModv
  static final int MODV = -4;
  ...
  public ImodState(BaseManager manager, FileType fileType, AxisID axisID) {
    this.manager = manager;
    this.axisID = axisID;
    process = new ImodProcess(manager,
      fileType.getFile(manager, axisID).getAbsolutePath(), axisID);
    reset();
  }
  ...
  void setFile(final File file) {
    process.setDatasetName(file.getAbsolutePath());
  }
  ...
  public ImodState(final BaseManager manager, final File[] fileList,
    final AxisID axisID) {
    this.manager = manager;
    this.axisID = axisID;
  ...
  ImodState(final BaseManager manager, final AxisID axisID, final FileType fileType) {
    this.manager = manager;
    this.axisID = axisID;
    ...
    process = new ImodProcess(manager, fileType.getFileName(manager, axisID), axisID);
    reset();
  }
   ...
   */
  ImodState(final BaseManager manager, final AxisID tempAxisID, final FileType fileType1,
    final FileType fileType2, final FileType fileType3, final String modelName,
    ...
    String[] datasetNameArray = { fileType1.getFileName(manager, axisID),
      fileType2.getFileName(manager, axisID), fileType3.getFileName(manager, axisID) };
    String datasetName =
      datasetNameArray[0] + " " + datasetNameArray[1] + " " + datasetNameArray[2];
  ...
  public String toString() {
    return "[process=" + process.toString() + "]";
  }
  ...
  void open(Run3dmodMenuOptions menuOptions) throws SystemProcessException, IOException {
      ...
      if (openBeadFixer) {
        ...
        if (setAutoCenter) {
          if (manager instanceof ApplicationManager) {
            process.setBeadfixerDiameter(
              ((ApplicationManager) manager).getBeadfixerDiameter(axisID));
          }
    ...
    if ((modelView || useModv) && interpolation == null && usingMode && mode != MODEL_MODE
  ...
  void open(String modelName, boolean modelMode, Run3dmodMenuOptions menuOptions)
    throws SystemProcessException, IOException {
    setModelName(modelName);
   ...
   * @param usingMode
   */
  void setUsingMode(boolean usingMode) {
   ...
   */
  void setOpenContours(boolean openContours) {
    this.openContours = openContours;
  }

  void setStartNewContoursAtNewZ(boolean startNewContoursAtNewZ) {
   ...
   */
  void setPreserveContrast(boolean preserveContrast) {
   ...
   * @param openBeadFixer
   */
  void setOpenBeadFixer(boolean openBeadFixer) {
  ...
  boolean isDefaultOpenWithModel() {
    return defaultOpenWithModel;
  }

  boolean isDefaultPreserveContrast() {
  ...
  void reopenLog() throws SystemProcessException, IOException {
    process.reopenLog();
  }

  void openModel(final String model, final boolean modelMode)
    throws SystemProcessException, IOException {
  ...
  void setOpenModelView() throws IOException, SystemProcessException {
   ...
   */
  void setWarnedStaleFile(boolean warnedStaleFile) {
   ...
   */
  boolean equalsInitialConfiguration(ImodState imodState) {
   ...
   */
  boolean equalsCurrentConfiguration(ImodState imodState) {
    if (modelView == imodState.isModelView() && useModv == imodState.isUseModv()
