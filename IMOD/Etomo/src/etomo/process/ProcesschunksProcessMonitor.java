...
/**
 * <p>Description: Monitor for processchucks.  Follows processchunks.out.</p>
 * 
 ...
 * @version $Id$
 */
class ProcesschunksProcessMonitor
  implements OutfileProcessMonitor, ParallelProcessMonitor {
  ...
  static ProcesschunksProcessMonitor getReconnectInstance(final BaseManager manager,
    final AxisID axisID, final ProcessData processData, final boolean multiLineMessages) {
  ...
  boolean updateState() throws LogFile.LockException, FileNotFoundException, IOException {
    ...
    while ((line = processOutput.readLine(processOutputReaderId)) != null) {
      ...
      // "No match" has been removed. I may not be producing fatal errors.
        ...
        || line.indexOf("Variable syntax") != -1 || line.indexOf("Badly placed (") != -1
        || line.indexOf("Badly formed number") != -1)
        && (process == null || process.getProcessData() == null
          || !process.getProcessData().isRunning())) {
  ...
  void updateProgressBar() {
    ...
    manager.getMainPanel().setProgressBarValue(chunksFinished.getInt(), getStatusString(),
      axisID);
    if (messageReporter != null) {
  ...
  private final synchronized void createProcessOutput() throws LogFile.LockException {
    if (processOutput == null) {
      processOutput =
        LogFile.getInstance(manager.getPropertyUserDir(), DatasetFiles.getOutFileName(
          manager, subdirName, ProcessName.PROCESSCHUNKS.toString(), axisID));
