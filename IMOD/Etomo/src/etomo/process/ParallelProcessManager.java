...
public final class ParallelProcessManager extends BaseProcessManager {
  ...
  void postProcess(final BackgroundProcess process) {
    ...
    if (commandDetails.getCommandName()
      .equals(ProcessName.ANISOTROPIC_DIFFUSION.toString())) {
      ParallelState state = manager.getState();
      state.setTestKValue(
        commandDetails.getDoubleValue(AnisotropicDiffusionParam.Field.K_VALUE));
  ...
  void postProcess(final DetachedProcess process) {
    ...
    if (command.getCommandName().equals(ProcessName.PROCESSCHUNKS.toString())) {
      ...
      if (subcommandDetails != null
        && subcommandDetails.getCommandName()
          .equals(ProcessName.ANISOTROPIC_DIFFUSION.toString())
        && subcommandDetails
          .getCommandMode() == AnisotropicDiffusionParam.Mode.VARYING_K) {
        ParallelState state = manager.getState();
        state.setTestKValueList(
          subcommandDetails.getString(AnisotropicDiffusionParam.Field.K_VALUE_LIST));
        state.setTestIteration(
          subcommandDetails.getIntValue(AnisotropicDiffusionParam.Field.ITERATION));
      }
    }
  }
}
