 ...
 * <p> $$ </p>
 */
class CombineProcessMonitor implements DetachedProcessMonitor {
   ...
   * @param axisID
   */
  CombineProcessMonitor(ApplicationManager manager, AxisID axisID,
  ...
  private void
    setNextProcessResultDisplay(ProcessResultDisplay nextProcessResultDisplay) {
  ...
  public void run() {
    ...
    try {
      ...
      endMonitor(ProcessEndState.FAILED);
      e.printStackTrace();
      UIHarness.INSTANCE.openMessageDialog(manager, e.getMessage(), "Etomo Error",
        axisID);
    }
    catch (InterruptedException e) {
      endMonitor(ProcessEndState.DONE);
  ...
  private void waitForLogFile()
    throws LogFile.LockException, InterruptedException, FileNotFoundException {
    if (logFile == null) {
    ...
    case CONSTRUCTED_STATE:
      ...
      if (combineComscriptState == null) {
        throw new NullPointerException(
          stateString + "CombineComscriptState should not be null");
      }
      if (!processRunning) {
    ...
    case WAITED_FOR_LOG_STATE:
      ...
      if (logFile.exists() && sleepCount != 0) {
        throw new IllegalStateException(
          stateString + "The sleepCount should be reset when the log file is found.  "
