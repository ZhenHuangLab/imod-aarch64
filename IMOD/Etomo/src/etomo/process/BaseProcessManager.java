...
public abstract class BaseProcessManager {
  ...
  protected BaseProcessManager(final BaseManager manager) {
  ...
  final String paramString() {
    return "axisProcessData:" + axisProcessData.toString()
  ...
  public final String processchunks(final AxisID axisID, final ProcesschunksParam param,
    ...
    final boolean multiLineMessages) throws AxisBusyException {
  ...
  public final void createNewFile(final String absolutePath) {
    ...
    try {
      ...
      uiHarness.openMessageDialog(manager,
        "Cannot create  " + file.getAbsolutePath() + ".\n" + e.getMessage(),
        "File Error");
    }
  }
  ...
  /**
   * run b3dtomosetexts command on dir
   * @param file
   */
  public static final TomosetextsOutput tomosetexts(final BaseManager manager,
    final AxisID axisID, final File dir) {
    if (!dir.exists()) {
      return null;
    }
    if (!dir.canRead()) {
    ...
    SystemProgram systemProgram =
      new SystemProgram(
        manager, manager.getPropertyUserDir(), new String[] { "python",
          BaseManager.getIMODBinPath() + "b3dtomosetexts", dir.getAbsolutePath() },
        axisID);
    systemProgram.run();
    String[] stdout = systemProgram.getStdOutput();
    if (stdout != null) {
      return new TomosetextsOutput(stdout);
    }
    else {
      return null;
    }
  ...
  final ComScriptProcess startComScript(final String command,
    ...
    return startComScript(
      new ComScriptProcess(manager, command, this, axisID, null, processMonitor,
        processResultDisplay, commandDetails, processSeries),
  ...
  final ComScriptProcess startComScript(final String commandString,
    ...
    final ProcessSeries processSeries) throws AxisBusyException {
    return startComScript(
      new ComScriptProcess(manager, commandString, this, axisID, null, processMonitor,
        processResultDisplay, command, processSeries),
  ...
  final ComScriptProcess startComScript(final String command,
    final ProcessMonitor processMonitor, final AxisID axisID,
    final ProcessResultDisplay processResultDisplay, final ProcessSeries processSeries,
    final boolean resumable) throws AxisBusyException {
  ...
  final ComScriptProcess startComScript(final String command, final AxisID axisID,
    final ProcessSeries processSeries, final FileType fileType) throws AxisBusyException {
    return startComScript(new ComScriptProcess(manager, command, this, axisID, null, null,
      null, processSeries, false, fileType), command, null, axisID);
  }
  ...
  final ComScriptProcess startComScript(final String commandString,
    ...
    final ProcessSeries processSeries, final FileType fileType) throws AxisBusyException {
    return startComScript(
      new ComScriptProcess(manager, commandString, command, this, axisID, null,
        processMonitor, processResultDisplay, processSeries, fileType),
      commandString, processMonitor, axisID);
  }
  ...
  final void startNonBlockingComScript(final String command, final AxisID axisID,
    ...
    startNonBlockingComScript(
      new ComScriptProcess(manager, command, this, axisID, processResultDisplay), command,
      axisID);
  }
  ...
  final ComScriptProcess startComScript(final String command,
    final ProcessMonitor processMonitor, final AxisID axisID,
    final ProcessSeries processSeries, final boolean resumable) throws AxisBusyException {
    return startComScript(new ComScriptProcess(manager, command, this, axisID, null,
   ...
   * @throws SystemProcessException
   */
  protected final ComScriptProcess startComScript(final CommandDetails commandDetails,
    final ProcessMonitor processMonitor, final AxisID axisID,
    final ProcessResultDisplay processResultDisplay, final ProcessSeries processSeries,
    final ProcessingMethod processingMethod) throws AxisBusyException {
    return startComScript(
      new ComScriptProcess(manager, commandDetails, this, axisID, null, processMonitor,
        processResultDisplay, processSeries, processingMethod),
      commandDetails.getCommandLine(), processMonitor, axisID);
  }

  protected final ComScriptProcess startComScript(final Command command,
    final ProcessMonitor processMonitor, final AxisID axisID,
    final ProcessResultDisplay processResultDisplay, final ProcessSeries processSeries)
    throws AxisBusyException {
    return startComScript(
      new ComScriptProcess(manager, command, this, axisID, null, processMonitor,
        processResultDisplay, processSeries, null),
  ...
  final ComScriptProcess startComScript(final String command,
    ...
    return startComScript(new ComScriptProcess(manager, command, this, axisID,
      watchedFileName, processMonitor, processSeries, resumable), command, processMonitor,
      axisID);
  }
  ...
  final ComScriptProcess startComScript(final ComScriptProcess comScriptProcess,
    ...
    if ((DEBUG || EtomoDirector.INSTANCE.getArguments().isTest())
  ...
  public final boolean inUse(final AxisID axisID,
    ...
    try {
      ...
      if (popupError) {
        uiHarness.openMessageDialog(manager,
          "A process is already executing" + (isDualAxis() ? " in the current axis" : ""),
  ...
  final void isAxisBusy(final AxisID axisID,
    final ProcessResultDisplay processResultDisplay) throws AxisBusyException {
    ...
    if (savedProcessData.isRunning()) {
      ...
      message.append(
        "." + "\n\nReferences:" + "\nProcessName=" + savedProcessData.getProcessName()
          + "\n" + manager.getParamFile() + "\nPID = " + savedProcessData.getPid());
      throw new AxisBusyException(message.toString());
    ...
    if (axisID == AxisID.SECOND) {
      if (axisProcessData.isBlockAxis(AxisID.SECOND)) {
        throw new AxisBusyException("Process attempting to restart - axis B is blocked.");
      }
    }
    else if (axisProcessData.isBlockAxis(AxisID.FIRST)) {
      throw new AxisBusyException("Process attempting to restart - axis A is blocked.");
    }
  }
  ...
  private void kill(final String processID, final AxisID axisID) {
    ...
    killShell.run();
    Utilities
      .debugPrint("imodkillgroup " + processID + " at " + killShell.getRunTimestamp());
  }
  ...
  private void logProcessOutput(final String commandAction, final String[] stdOutput,
    ...
    if (!EtomoDirector.INSTANCE.isUnitTest() || DEBUG) {
      if (printStdOutput) {
        ...
        for (int i = stdOutput.length > linesToLog ? stdOutput.length - linesToLog : 0;
          i < stdOutput.length; i++) {
          System.err.println(stdOutput[i]);
        }
      }
      if (printStdError) {
        System.err.println("Standard error:");
        for (int i = stdError.length > linesToLog ? stdError.length - linesToLog : 0;
          i < stdError.length; i++) {
  ...
  public final void msgComScriptDone(final OutfileComScriptProcess process,
    ...
    if (failed) {
      ...
      if (processMessages != null
        ...
        uiHarness.openErrorMessageDialog(manager, processMessages, "Comscript Terminated",
          process.getAxisID());
      }
    ...
    if (exitValue != 0) {
      ...
      if (stdError != null && stdError.length > 0
        ...
        combinedMessages.add(ProcessMessages.MessageType.ERROR,
          "<html>Terminated: " + script.getComScriptName());
      }
      else {
        ...
        combinedMessages.add(ProcessMessages.MessageType.ERROR,
          "\n<html><U>Log file errors:</U>", messages);
        combinedMessages.add(ProcessMessages.MessageType.ERROR,
          "\n<html><U>Standard error output:</U>", stdError);
      }
    ...
    if (exitValue != 0) {
      ...
      if (stdError != null && stdError.length > 0
        ...
        combinedMessages.add(ProcessMessages.MessageType.ERROR,
          "\n<html><U>Standard error output:</U>", stdError);
        combinedMessages.add(ProcessMessages.MessageType.ERROR,
          ProcessMessages.MessageType.CHUNK_ERROR, "<html><U>Chunk errors:</U>",
          messages);
      }
      if (script.getProcessEndState() != ProcessEndState.KILLED
        && script.getProcessEndState() != ProcessEndState.PAUSED) {
    ...
    if (exitValue != 0) {
      ...
      if (script.getProcessEndState() != ProcessEndState.KILLED
        ...
        if (!messages.isEmpty(ProcessMessages.MessageType.ERROR)) {
          ...
          uiHarness.openMessageDialog(manager, "Unable to reconnect",
            "Reconnect Terminated", script.getAxisID());
        }
        if (!EtomoDirector.INSTANCE.isUnitTest() || DEBUG) {
          System.err.print("Reconnect Terminated (" + Utilities.getDateTimeStamp()
  ...
  final BackgroundProcess startBackgroundProcess(final Command command,
    ...
    BackgroundProcess backgroundProcess = new BackgroundProcess(manager, command, this,
      axisID, processResultDisplay, processName, processSeries);
    return startBackgroundProcess(backgroundProcess, command.getCommandArray().toString(),
      axisID, null);
  }
  ...
  final BackgroundProcess startBackgroundProcess(final Command command,
    final AxisID axisID, final ProcessResultDisplay processResultDisplay,
    final ProcessName processName, final ProcessSeries processSeries,
    final boolean allowMultiLineLog) throws AxisBusyException {
    BackgroundProcess backgroundProcess = new BackgroundProcess(manager, command, this,
      axisID, processResultDisplay, processName, processSeries, allowMultiLineLog);
    return startBackgroundProcess(backgroundProcess, command.getCommandArray().toString(),
      axisID, null);
  }
  ...
  final BackgroundProcess startBackgroundProcess(final String[] commandArray,
    ...
    final ProcessResultDisplay processResultDisplay, final ProcessSeries processSeries,
    final ProcessName processName) throws AxisBusyException {
    BackgroundProcess backgroundProcess = new BackgroundProcess(manager, commandArray,
  ...
  final BackgroundProcess startBackgroundProcess(final String[] commandArray,
    final AxisID axisID, final ProcessName processName, final ProcessSeries processSeries)
    throws AxisBusyException {
    BackgroundProcess backgroundProcess = new BackgroundProcess(manager, commandArray,
    ...
    return startBackgroundProcess(backgroundProcess, commandLine.toString(), axisID,
      null);
  }
  ...
  final BackgroundProcess startDetachedProcess(
    ...
    throws AxisBusyException {
    DetachedProcess detachedProcess = new DetachedProcess(manager, detachedCommandDetails,
      this, axisID, monitor, processResultDisplay, processName, processSeries,
  ...
  final BackgroundProcess startDetachedProcess(
    final DetachedCommandDetails detachedCommandDetails, final AxisID axisID,
    final OutfileProcessMonitor monitor, final ProcessResultDisplay processResultDisplay,
    final ProcessName processName, final String subdirName, final String shortCommandName,
  ...
  final BackgroundProcess startBackgroundProcess(final CommandDetails commandDetails,
    final AxisID axisID, final ProcessName processName, final ProcessSeries processSeries,
    final boolean popupChunkWarnings) throws AxisBusyException {
    BackgroundProcess backgroundProcess = new BackgroundProcess(manager, commandDetails,
  ...
  private BackgroundProcess startBackgroundProcess(
    ...
    final AxisID axisID, final Monitor processMonitor) throws AxisBusyException {
    ...
    if (processMonitor != null) {
      ...
      Thread processMonitorThread = new Thread(
        new etomo.process.ThreadGroup("startBackgroundProcess"), processMonitor);
      processMonitorThread.start();
      axisProcessData.mapAxisProcessMonitor(processMonitorThread, processMonitor, axisID);
    }
  ...
  public static String[] getCommandOutput(final String[] commandLine, final AxisID axisID,
    final BaseManager manager) {
    SystemProgram command = new SystemProgram(manager,
      manager != null ? manager.getPropertyUserDir() : null, commandLine, axisID);
    command.run();
