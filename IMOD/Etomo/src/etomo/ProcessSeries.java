...
public final class ProcessSeries implements ConstProcessSeries {
  ...
  private final AxisID busyAxisID;
  ...
  public String toString() {
    return ((Object) super.toString()) + ":[forceNextProcess:" + forceNextProcess
      + ",nextProcess:" + nextProcess + ",processList:" + processList + ",lastProcess:"
      + lastProcess + ",failProcess:" + failProcess + ",pauseProcess:" + pauseProcess
      + "]";
  }
  ...
  /**
   * BusyAxisID is different from the axisID of the processes.  Archiveorig, for instance,
   * runs on axis A and archives both axes.
   * @param manager
   * @param busyAxisID - the axisID where the processes are running.
   * @param dialogType
   */
  public ProcessSeries(final BaseManager manager, final AxisID busyAxisID,
    DialogType dialogType) {
    busyStatusMediator = manager.getBusyStatusMediator();
    busyStatusMediator.msgProcessSeriesConstructed(busyAxisID);
    this.manager = manager;
    uiComponent = null;
    this.dialogType = dialogType;
    this.processDisplay = null;
    this.busyAxisID = busyAxisID;
  }
   ...
   */
  public ProcessSeries(final BaseManager manager, final AxisID busyAxisID,
    final UIComponent uiComponent, final DialogType dialogType) {
    busyStatusMediator = manager.getBusyStatusMediator();
    busyStatusMediator.msgProcessSeriesConstructed(busyAxisID);
    this.manager = manager;
   ...
   * @param busyAxisID - the axisID where the processes are running.
   * @param dialogType
   * @param processDisplay
   */
  public ProcessSeries(final BaseManager manager, final AxisID busyAxisID,
    DialogType dialogType, ProcessDisplay processDisplay) {
    busyStatusMediator = manager.getBusyStatusMediator();
    busyStatusMediator.msgProcessSeriesConstructed(busyAxisID);
    this.manager = manager;
    uiComponent = null;
  ...
  public boolean startNextProcess(final AxisID axisID,
    ...
    if (nextProcess != null) {
      ...
      clearProcesses();
      busyStatusMediator.msgProcessSeriesDone(busyAxisID);
      return false;
    }
    ...
    if (process.target == null) {
      ...
      process.target.startNextProcess(uiComponent, axisID, process, processResultDisplay,
        this, dialogType, processDisplay);
    }
    return true;
  }
  ...
  /**
   * This is a bandage function.  It tells BusyStatusMediator that the series is done
   * without changing ProcessSeries.  It is being used on existing code that doesn't call
   * startFailProcess for failures.  For new functions it is standard to call
   * startFailProcess when there is a failure that ends the series - even if no failure
   * function exists.
   */
  ...
  /*
   * Works like setNextProcess but does not overwrite an existing nextProcess.  If
   * nextProcess is already set, the old nextProcess is moved to the beginning of
   * processList.
   */
  public void prependNextProcess(final TaskInterface task) {
    prependProcess(nextProcess);
    nextProcess =
      new Process(null, null, null, null, null, task, false, null, null, null);
  }
  ...
  public void setNextProcess(final NextProcessTarget target, final TaskInterface task) {
    nextProcess =
      new Process(null, null, null, null, null, task, false, null, target, null);
  }

  public void setNextProcess(final TaskInterface task, final Command command) {
    nextProcess =
      new Process(null, null, null, null, null, task, false, command, null, null);
  }
  ...
  public void addProcess(final TaskInterface task, final boolean forceNextProcess) {
    appendProcess(new Process(null, null, null, null, null, task, forceNextProcess, null,
      null, null));
  }

  public void addProcess(final TaskInterface task, final Command command,
    final AxisType axisType) {
    appendProcess(
      new Process(null, null, null, null, null, task, false, command, null, axisType));
  }

  private void prependProcess(final Process process) {
    if (process == null) {
      return;
    }
    // Place process at the beginning of ProcessList
    Process oldProcessList = processList;
    processList = process;
    processList.next = oldProcessList;
  }

  private void appendProcess(final Process process) {
    if (process == null) {
      return;
    }
    // Go to the end of processList.
    Process endOfList = null;
    Process processPointer = processList;
    while (processPointer != null) {
      endOfList = processPointer;
      processPointer = processPointer.next;
    }
    // Append process to processList.
    if (endOfList != null) {
  ...
  public void setNextProcess(final String process, final ProcessName subprocessName,
    ...
    nextProcess = new Process(process, subprocessName, outputImageFileType, null,
      processingMethod, null, false, null, null, null);
  }
   ...
   */
  public void setNextProcess(final String process, final ProcessName subprocessName,
    final FileKey outputImageFileKey, final FileKey outputImageFileKey2,
    final ProcessingMethod processingMethod) {
    nextProcess = new Process(process, subprocessName, outputImageFileKey,
      outputImageFileKey2, processingMethod, null, false, null, null, null);
  }
  ...
  public void setLastProcess(final String process) {
    lastProcess =
      new Process(process, null, null, null, null, null, false, null, null, null);
  }

  public void setLastProcess(final NextProcessTarget target, final String process) {
    lastProcess =
      new Process(process, null, null, null, null, null, false, null, target, null);
  }
  ...
  public void setLastProcess(final NextProcessTarget target, final TaskInterface task) {
    lastProcess =
      new Process(null, null, null, null, null, task, false, null, target, null);
  }
  ...
  public static final class Process {
    ...
    private final FileKey outputImageFileKey;
    private final FileKey outputImageFileKey2;
    private final ProcessingMethod processingMethod;
    final TaskInterface task;
    private final boolean forceNextProcess;
    private final Command command;
    private final NextProcessTarget target;
    ...
    public String toString() {
      return ((Object) super.toString()) + ":[forceNextProcess:" + forceNextProcess
        + ",process:" + process + ",task:" + task + ",target:" + target + "]";
    }
    ...
    private Process(final String process, final ProcessName subprocessName,
      ...
      final boolean forceNextProcess, final Command command,
      final NextProcessTarget target, final AxisType axisType) {
      this.process = process;
      this.subprocessName = subprocessName;
      this.outputImageFileKey = outputImageFileKey;
      this.outputImageFileKey2 = outputImageFileKey2;
    ...
    public FileType getOutputImageFileType() {
      if (outputImageFileKey instanceof FileType) {
        return (FileType) outputImageFileKey;
      }
      return null;
    }

    public FileKey getOutputImageFileKey() {
      return outputImageFileKey;
    }
