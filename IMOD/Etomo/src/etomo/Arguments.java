 ...
 * <p>Description: </p>
 * 
 * <p>Copyright: Copyright 2006 - 2020 by the Regents of the University of Colorado</p>
 * 
...
public final class Arguments {
  ...
  private static final String[] HELP_TAGS =
  ...
  private static final String DIR_TAG_ = "-dir";
  /**
   * @deprecated 4/8/2019 replaced by --rawimagestack
   */
  private static final String DATASET_TAG = "-dataset";
  private static final String RAW_IMAGE_STACK_TAG = "-rawimagestack";
  ...
  private static final String GRAB_IT_TAG = "-grabit";
  public static final String IMAGE_FILENAME_STYLE_TAG = "-namingstyle";
  // Moveb moves subframe out of the way of main frame.
  public static final String MOVE_B_TAG = "-moveb";
  ...
  private boolean rawImageStack = false;
  private String sRawImageStack = null;
  ...
  private ImageFilenameStyle imageFilenameStyle = null;
  private GrabItParameter grabItParameter = null;
        ...
        if (sectionLocation != null) {
          ...
          while ((section = autodoc.nextSection(sectionLocation)) != null) {
            ...
            if (sectionType.equals(EtomoAutodoc.HEADER_SECTION_NAME)) {
              if ((attribute =
                section.getAttribute(EtomoAutodoc.USAGE_ATTRIBUTE_NAME)) != null
              ...
              if ((attribute =
                section.getAttribute(EtomoAutodoc.SHORT_ATTRIBUTE_NAME)) != null
                && (attributeValue = attribute.getValue()) != null) {
              ...
              if ((attribute =
                section.getAttribute(EtomoAutodoc.FORMAT_ATTRIBUTE_NAME)) != null
                && (attributeValue = attribute.getValue()) != null) {
  ...
  public GrabItParameter getGrabItParameter() {
    return grabItParameter;
  }
  ...
  void parse(String[] args) {
    ...
    while (i < args.length) {
      ...
      if (!args[i].startsWith("-")) {
        ...
        else if (args[i].equals(TEST_TAG) || args[i].equals("-" + TEST_TAG)) {
          test = true;
        }
        else if (args[i].equals(HEADLESS_TAG) || args[i].equals("-" + HEADLESS_TAG)) {
          headless = true;
        }
        else if (args[i].equals(SELFTEST_TAG) || args[i].equals("-" + SELFTEST_TAG)) {
          selfTest = true;
        }
        else if (args[i].equals(NAMES_TAG) || args[i].equals("-" + NAMES_TAG)) {
          printNames = true;
        }
        else if (args[i].equals(MEMORY_TAG) || args[i].equals("-" + MEMORY_TAG)) {
          displayMemory = true;
        ...
        else if (args[i].equals(TIMESTAMP_TAG) || args[i].equals("-" + TIMESTAMP_TAG)) {
          Utilities.setTimestamp(true);
        }
        else if (args[i].equals(NEWSTUFF_TAG) || args[i].equals("-" + NEWSTUFF_TAG)) {
          newstuff = true;
        }
        else if (args[i].equals(DATASET_TAG) || args[i].equals(RAW_IMAGE_STACK_TAG)
          || args[i].equals("-" + RAW_IMAGE_STACK_TAG)) {
          rawImageStack = true;
          reconAutomation = true;
          if (i < args.length - 1) {
            sRawImageStack = args[i + 1];
            i++;
          }
        }
        else if (args[i].equals(DIR_TAG_) || args[i].equals("-" + DIR_TAG_)) {
          dir = true;
          reconAutomation = true;
          if (i < args.length - 1) {
        ...
        else if (args[i].equals(AXIS_TAG) || args[i].equals("-" + AXIS_TAG)) {
          axis = true;
          reconAutomation = true;
          if (i < args.length - 1) {
        ...
        else if (args[i].equals(GRAB_IT_TAG) || args[i].equals("-" + GRAB_IT_TAG)) {
          grabIt = true;
          if (i < args.length - 1 && !args[i + 1].startsWith("--")) {
            grabItParameter = GrabItParameter.getInstance(args[i + 1]);
            i++;
          }
        }
        else if (args[i].equals(MOVE_B_TAG) || args[i].equals("-" + MOVE_B_TAG)) {
          moveB = true;
          i++;
        }
        else if (args[i].equals(IMAGE_FILENAME_STYLE_TAG)
          || args[i].equals("-" + IMAGE_FILENAME_STYLE_TAG)) {
          if (i < args.length - 1 && !args[i + 1].startsWith("--")) {
            imageFilenameStyle = ImageFilenameStyle.getInstance(args[i + 1], true);
            i++;
          }
        }
        else if (args[i].equals(FRAME_TAG) || args[i].equals("-" + FRAME_TAG)) {
          frame = true;
          reconAutomation = true;
          if (i < args.length - 1) {
        ...
        else if (args[i].equals(FIDUCIAL_TAG) || args[i].equals("-" + FIDUCIAL_TAG)) {
          fiducial = true;
          reconAutomation = true;
          if (i < args.length - 1) {
        ...
        else if (args[i].equals(SCAN_TAG) || args[i].equals("-" + SCAN_TAG)) {
          reconAutomation = true;
          scan = true;
        }
        else if (args[i].equals(CREATE_TAG) || args[i].equals("-" + CREATE_TAG)) {
          reconAutomation = true;
          create = true;
        }
        else if (args[i].equals(EXIT_TAG) || args[i].equals("-" + EXIT_TAG)) {
          exit = true;
        }
        else if (args[i].equals(FG_TAG) || args[i].equals("-" + FG_TAG)) {
          reconAutomation = true;
        }
        else if (args[i].equals(LISTEN_TAG) || args[i].equals("-" + LISTEN_TAG)) {
          listen = true;
        }
        else if (args[i].equals(AUTO_CLOSE_3DMOD_TAG)
          || args[i].equals("-" + AUTO_CLOSE_3DMOD_TAG)) {
          autoClose3dmod = true;
        }
        else if (args[i].equals(IGNORE_LOC_TAG) || args[i].equals("-" + IGNORE_LOC_TAG)) {
          ignoreLoc = true;
        }
        else if (args[i].equals(IGNORE_SETTINGS_TAG)
          || args[i].equals("-" + IGNORE_SETTINGS_TAG)) {
          ignoreSettings = true;
        }
        else if (args[i].equals(ACTIONS_TAG) || args[i].equals("-" + ACTIONS_TAG)) {
          actions = true;
        }
        else if (args[i].equals(DIRECTIVE_TAG) || args[i].equals("-" + DIRECTIVE_TAG)) {
          directive = true;
          reconAutomation = true;
          headless = true;
          create = true;
        ...
        else if (args[i].equals(CPUS_TAG) || args[i].equals("-" + CPUS_TAG)) {
          cpus = true;
        ...
        else if (args[i].equals(GPUS_TAG) || args[i].equals("-" + GPUS_TAG)) {
          gpus = true;
        ...
        else if (args[i].equals(FROM_BRT_TAG) || args[i].equals("-" + FROM_BRT_TAG)) {
          fromBRT = true;
        }
        else if (args[i].equals(USER_TEMPLATE_LOC_TAG)
          || args[i].equals("-" + USER_TEMPLATE_LOC_TAG)) {
          // The quotes have already been stripped.
          userTemplateLoc = new File(args[i + 1]);
          i++;
        }
        else if (args[i].equals(PLUGIN_TAG) || args[i].equals("-" + PLUGIN_TAG)) {
          plugin = true;
        }
        else {
          errorMessageList.add("WARNING:  unknown argument, " + args[i]
            + ", ignored. Please run 'etomo --help' for further assistance.");
        }
      }
      i++;
    }
  ...
  private boolean checkNumericAndSingleDash(String input) {
    if (isNumericInteger(input) || !input.startsWith("-")) {
      return true;
    }
    return false;
  }

  private boolean isNumericInteger(String input) {
    try {
  ...
  boolean validate(final UIComponent component) {
    for (Iterator<String> i = paramFileNameList.iterator(); i.hasNext();) {
      ...
      if (!file.exists()) {
        errorMessageList.add("File parameter, " + file.getAbsolutePath()
          + ", does not exist." + ",string:" + string);
      }
      else {
        if (!file.canRead()) {
          errorMessageList
            .add("File parameter, " + file.getAbsolutePath() + ", is not readable.");
        }
        if (file.isDirectory()) {
          errorMessageList
            .add("File parameter, " + file.getAbsolutePath() + ", is a directory.");
        }
        if (!file.canWrite()) {
          errorMessageList
            .add("File parameter, " + file.getAbsolutePath() + ", is not writable.");
        }
    ...
    if (directive) {
      ...
      if (axis || rawImageStack || dir || fiducial || frame) {
        warningMessageList.add("The parameters: " + AXIS_TAG + ", " + RAW_IMAGE_STACK_TAG
          + ", " + DIR_TAG_ + ", " + FIDUCIAL_TAG + ", and " + FRAME_TAG
          + " are ignored when " + DIRECTIVE_TAG + " is used.");
      }
    }
    if (axis && atAxis == null) {
      errorMessageList.add("Missing or invalid " + AXIS_TAG + " parameter value.");
    }
    if (rawImageStack && sRawImageStack == null) {
      errorMessageList.add("Missing " + RAW_IMAGE_STACK_TAG + " parameter value.");
    }
    if (dir) {
      if (fDir == null) {
        errorMessageList.add("Missing " + DIR_TAG_ + " parameter value.");
      }
      else if (!fDir.exists()) {
        errorMessageList.add(
          DIR_TAG_ + " parameter value, " + fDir.getAbsolutePath() + ", does not exist.");
      }
      else {
        if (!fDir.canRead()) {
          errorMessageList.add(DIR_TAG_ + " parameter value, " + fDir.getAbsolutePath()
            + ", is not readable.");
        }
        if (!fDir.isDirectory()) {
          errorMessageList.add(DIR_TAG_ + " parameter value, " + fDir.getAbsolutePath()
    ...
    if (fiducial) {
      if (enFiducial.isNull()) {
        ...
        if (errorMessage.equals("")) {
          ...
          errorMessageList
            .add(FIDUCIAL_TAG + " parameter value is an invalid number: " + errorMessage);
        }
      }
      else if (enFiducial.isNegative()) {
        errorMessageList.add(
          FIDUCIAL_TAG + " parameter value, " + enFiducial + ", cannot be negative.");
      }
    }
  ...
  public List<String> getParamFileNameList() {
  ...
  public static final class DebugLevel {
    ...
    private static final DebugLevel LIMITED = new DebugLevel(-1);// No debug except for
                                                                 // limited level
                                                                 // debugging
    public static final DebugLevel STANDARD = new DebugLevel(1);// same as debug without
                                                                // an argument
  ...
  public static final class GrabItParameter {
    public static final GrabItParameter BATCHRUNTOMO = new GrabItParameter("b");
    public static final GrabItParameter COPY_TOMO_COMS = new GrabItParameter("c");
    public static final GrabItParameter PARALLEL_PROCESSING = new GrabItParameter("p");

    private final String parameter;

    private GrabItParameter(final String parameter) {
      this.parameter = parameter;
    }

    private static GrabItParameter getInstance(final String string) {
      if (string == null) {
        return null;
      }
      if (string.equals(BATCHRUNTOMO.parameter)) {
        return BATCHRUNTOMO;
      }
      if (string.equals(COPY_TOMO_COMS.parameter)) {
        return COPY_TOMO_COMS;
      }
      if (string.equals(PARALLEL_PROCESSING.parameter)) {
        return PARALLEL_PROCESSING;
      }
      System.out.println("ERROR: unknown grabit parameter");
      return null;
    }
  }
}
