...
public final class RemotePath {
  ...
  private final String getRemotePath(int ruleIndex, String localPath)
    ...
    if (mountNameIndex != -1) {
      if (mountName == null) {
        throw new InvalidMountRuleException("Unable to use remote mount rule, \""
          + remoteMountRule + "\" because %mountname has no value.  "
    ...
    // create remote path
    return remoteMountRule + localPath
  ...
  private synchronized final void loadMountRules(BaseManager manager, AxisID axisID) {
    ...
    catch (IOException e) {if (!EtomoDirector.INSTANCE.isUnitTest() || DEBUG) {
      e.printStackTrace();}
      return;
    }
    catch (LogFile.LockException e) {if (!EtomoDirector.INSTANCE.isUnitTest() || DEBUG) {
      e.printStackTrace();}
      return;
    }
    if (autodoc == null) {
    ...
    if ((localSection =
      loadMountRules(autodoc, CpuAdoc.COMPUTER_SECTION_TYPE, hostName, true)) == null) {
  ...
  private final boolean isValidRule(ReadOnlyAttribute localRule,
    ...
    if (sectionName != null) {
      errorTitle
        .append(", section " + sectionHeader + sectionName + AutodocTokenizer.CLOSE_CHAR);
    }
    errorTitle.append(" - ");
    // validate local rule
    ...
    if (remoteValue != null && remoteValue.indexOf(MOUNT_NAME_TAG) != -1
      ...
      if (!EtomoDirector.INSTANCE.isUnitTest() || DEBUG) {
        System.err.println(errorTitle + "remote mount rule " + mountRuleNumber
          + ".  Cannot use " + MOUNT_NAME_TAG
  ...
  private final boolean isValidRule(ReadOnlyAttribute rule, int mountRuleNumber,
    ...
    if (rule == null) {
      if (!EtomoDirector.INSTANCE.isUnitTest() || DEBUG) {
        System.err.println(
          errorTitle + ruleType + " mount rule " + mountRuleNumber + " is missing.");
      }
      return false;
    }
    ...
    if (value == null) {
      if (!EtomoDirector.INSTANCE.isUnitTest() || DEBUG) {
        System.err.println(
          errorTitle + ruleType + " mount rule " + mountRuleNumber + " cannot be blank.");
      }
      return false;
    }
