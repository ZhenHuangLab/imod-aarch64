...
public class MRCHeader {
  ...
  private static String COMMENT_DIVIDER = "=";
  //
  ...
  // comment data
  private final Vector<CommentData> commentDataVector = new Vector<CommentData>();
  // When adding a new comment data tag, make sure there is no conflict. Use order of
  // construction to resolve conflicts.
  private final CommentData binning =
    new CommentData(commentDataVector, "binning", EtomoNumber.Type.DOUBLE, 1);
  private final CommentData imageRotation = new CommentData(commentDataVector,
    "Tilt axis angle", "Tilt axis rotation angle", EtomoNumber.Type.DOUBLE);
  private final CommentData twodir =
    new CommentData(commentDataVector, "bidir", EtomoNumber.Type.DOUBLE, true);
  private final CommentData feiPixelSize = new CommentData(commentDataVector,
    "Pixel size in nanometers", EtomoNumber.Type.DOUBLE);
  //
  ...
  public static MRCHeader getInstance(final String filePath, final AxisID axisID) {
    if (filePath == null) {
      return null;
    }
    ...
    MRCHeader mrcHeader = (MRCHeader) instances.get(key);
    if (mrcHeader == null) {
      return createInstance(keyFile.getParent(), key, keyFile, axisID);
    }
    return mrcHeader;
  }
  ...
  public synchronized boolean read(BaseManager manager)
    ...
    if (!feiPixelSize.isNull()
      && (pixelEquals(1.0) || pixelEquals(2.0) || pixelEquals(4.0))) {
      // This was function parseFEIPixelSize:
      if (parsePixelSpacing(manager, xPixelSize, feiPixelSize.toString(),
        !pixelsParsed)) {
    ...
    if (!pixelSpacing.isValid() || dPixelSpacing == -1 || dPixelSpacing == 0) {
      if (popupErrorMessage) {
        UIHarness.INSTANCE.openMessageDialog(
          manager, "Invalid pixel spacing:  " + sPixelSpacing
            + ".  Fix the mrc header in " + filename + " with alterheader.",
          "Header Error", axisID);
      }
      return false;
    }
  ...
  /**
   * Return the binning found in the header
   */
  ...
  /**
   * Parse all recognized comment data.
   * @param line
   */
  private void parseCommentData(final String line) {
    // Comment data varies in different headers. Comment format is variable and may change
    // in the future. Some data has more then one identifying tag. Not all comment data
    // needs to be saved.
    //
    // Assumptions:
    // - An equals sign is always used between the name and the value.
    // - The value is always present.
    // - A comma and/or whitespace is used between the value and the next name/value pair.
    ...
    if (line.indexOf(COMMENT_DIVIDER) == -1) {
      return;
    }
    String[] splitArray = line.trim().split("\\s*" + COMMENT_DIVIDER + "\\s*");
    ...
    int numTags = commentDataVector.size();
    for (int i = 0; i < splitArray.length - 1; i++) {
      for (int commentDataIndex = 0; commentDataIndex < numTags; commentDataIndex++) {
        ...
        CommentData commentData = commentDataVector.get(commentDataIndex);
        // Look for comment data. Value is in the next element of splitArray.
        if (!commentData.find(splitArray[i]) || splitArray[i + 1] == null) {
          continue;
        }
        String[] valueSplit = splitArray[i + 1].split("\\s*,\\s*|\\s+");
        if (valueSplit == null || valueSplit.length == 0) {
          continue;
        }
        // The value is at the beginning of the next element.
        commentData.set(valueSplit[0]);
      }
    }
  }
  ...
  String paramString() {
    return ",\nfilename=" + filename + ",nColumns=" + nColumns + ",nRows=" + nRows
      ...
      + ",\naxisID=" + axisID + ",twodir =" + twodir + ",feiPixelSize=" + feiPixelSize;
  }
   ...
   * @author sueh
   */
  private final class CommentData {
    private final String tag;// required
    private final String altTag;// optional
    private final EtomoNumber value;
    private final boolean nameEndsWithTag;

    /**
     * Creates an integer comment data instance.
     * @param commentData - optional - instance is added to this vector
     * @param tag
     */
    private CommentData(final Vector<CommentData> commentDataVector, final String tag) {
      this.tag = tag;
      altTag = null;
      value = new EtomoNumber();
      nameEndsWithTag = false;
      if (commentDataVector != null) {
        commentDataVector.add(this);
      }
    }
     ...
     */
    private CommentData(final Vector<CommentData> commentDataVector, final String tag,
      final EtomoNumber.Type type, final Integer defaultValue) {
      this.tag = tag;
      altTag = null;
      value = new EtomoNumber(type);
      if (defaultValue != null) {
        value.setDisplayValue(defaultValue);
      }
      nameEndsWithTag = false;
      if (commentDataVector != null) {
        commentDataVector.add(this);
     ...
     */
    private CommentData(final Vector<CommentData> commentDataVector, final String tag,
      final EtomoNumber.Type type, final boolean nameEndsWithTag) {
      this.tag = tag;
      altTag = null;
      value = new EtomoNumber(type);
      this.nameEndsWithTag = nameEndsWithTag;
      if (commentDataVector != null) {
        commentDataVector.add(this);
      }
    }
     ...
     */
    private CommentData(final Vector<CommentData> commentDataVector, final String tag,
      final String altTag, final EtomoNumber.Type type) {
      this.tag = tag;
      this.altTag = altTag;
      nameEndsWithTag = false;
      value = new EtomoNumber(type);
    ...
    /**
     * @param string
     * @return true if tag or altTag is in string, avoiding the image file name
     */
    private boolean find(final String string) {
      if (string == null || string.indexOf("RO image file") != -1) {
        return false;
      }
      if (!nameEndsWithTag) {
        ...
        if (altTag != null && string.indexOf(altTag) != -1) {
          return true;
        }
      }
      ...
      if (altTag != null && string.endsWith(altTag)) {
        return true;
      }
      return false;
    }
    ...
    private ConstEtomoNumber get() {
      return value;
    }

    /**
     * Returns the string version of nValue.
     */
    public String toString() {
