...
public final class TestUtilites {
  ...
  private void initUnitTestData() {
    ...
    synchronized (this) {
      ...
      // May be a repository
      if (!unitTestData.exists() || !unitTestData.isDirectory()) {
        unitTestData = new File(new File(
          EnvironmentVariable.INSTANCE.getValue(null, null, "IMOD_DIR", AxisID.ONLY),
          "Etomo"), "unitTestData");
      }
    }
  }
  ...
  public File copyTestFile(String testRootAbsolutePath, String testDirName,
    ...
    TestCase.assertTrue(
      "unitTestDataFile does not exist:" + unitTestDataFile.getAbsolutePath(),
      unitTestDataFile.exists());
    ...
    if (unitTestDataFile.exists()) {
      ...
      if (testDirFile.exists() && !testDirFile.delete()) {
        throw new SystemProcessException(
          "Cannot delete testDirFile: " + testDirFile.getAbsolutePath());
      }
      ...
      if (!EtomoDirector.INSTANCE.isUnitTest() || DEBUG) {
        for (int i = 0; i < copyCommand.length; i++) {
          System.err.print(copyCommand[i] + " ");
        }
        System.err.println();
      }
      BaseManager manager = EtomoDirector.INSTANCE.getCurrentManagerForTest();
      SystemProgram copy = new SystemProgram(manager,
        manager == null ? null : manager.getPropertyUserDir(), copyCommand, AxisID.ONLY);
      copy.run();
      return testDirFile;
    }
    return null;
