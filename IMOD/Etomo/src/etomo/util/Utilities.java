...
public class Utilities {
  ...
  public static final String NOTHING_TO_DO_STATUS = " nothing to do";
  private static final boolean DEBUG = EtomoDirector.INSTANCE.getArguments().isDebug();

  private Utilities() {}

  public static String createPropertyKey(final String prepend, final String key) {
  ...
  /**
   * Returns a suffix that starts with the last instance of tag.  If tag is not found
   * returns null.  Trims whitespace
   * @param string
   * @param tag
   * @return
   */
  public static String getSuffix(final String string, final String tag) {
    return stripString(true, false, string, true, true, tag, true, true);
  }

  /**
   * Returns string with left side ending in tag removed.  The first instance of tag is
   * used.
   * @param string
   * @param tag
   * @return
   */
  public static String removeLeftSide(final String string, final String tag) {
    return stripString(false, false, string, true, false, tag, false, false);
  }

  /**
   * Returns string with right side starting with tag removed.  The last instance of tag
   * is used.
   * @param string
   * @param tag
   * @return
   */
  public static String removeRightSide(final String string, final String tag) {
    return stripString(false, false, string, false, false, tag, true, false);
  }

  /**
   * Returns file name with the extension removed.  The "." is removed.  Trims off
   * leading and following spaces from the input string.
   * @param filePath
   * @return
   */
  public static String removeExtension(final String filePath) {
    return stripString(false, false, filePath, false, false, Extension.EXTENSION_DIVIDER,
      true, true);
  }

  /**
   * Returns the extension of the file path.  The "." is not included.  Trims off
   * leading and following spaces from the input string and removes the path.  Returns
   * null if no extension is found.
   * @param filePath
   * @return
   */
  public static String getExtension(final String filePath) {
    String extension = stripString(true, true, filePath, true, false,
      Extension.EXTENSION_DIVIDER, true, true);
  ...
  /**
   * Returns a string with the left or right side stripped off.
   * @param returnNullIfFail - returns null if tag is not found
   * @param stripFilePath ignores and strips the file path
   * @param string
   * @param removeLeftSide - remove a string on the left side ending with removeTag.  If false removes a string on the right side starting with removeTag.
   * @param keepTag - keep the found tag in the result
   * @param tag - remove a string starting or ending with tag
   * @param lastIndexOf - use the last instance of tag rather then the first
   * @param trim - remove whitespace from the start and end of the string before stripping
   * @return
   */
  public static String stripString(final boolean returnNullIfFail,
    final boolean stripFilePath, String string, final boolean removeLeftSide,
    final boolean keepTag, final String tag, final boolean lastIndexOf,
    final boolean trim) {
    if (string == null || string.isEmpty() || tag == null || tag.isEmpty()) {
      if (returnNullIfFail) {
        return null;
      }
      return string;
  ...
  /**
   * Return the number of elements in a comma-divided array.  Goal is to match python's
   * element count.  Variable length array do not allow double commas.  This treats
   * multiple commas as a single commas.  Also removes preceeding commas before returning
   * size as these are ignored.
   * @param text
   * @return
   */
  public static int getNumberElements(String text) {
  ...
  public static void printComponents(StringBuilder prepend, final Container container) {
    if (prepend == null) {
    ...
    if (container instanceof AbstractButton
      && (!EtomoDirector.INSTANCE.isUnitTest() || DEBUG)) {
      System.err.println(prepend + ((AbstractButton) container).getText() + ",selected:"
        + ((AbstractButton) container).isSelected());
    }
    if (container instanceof JComboBox) {
      Object item = ((JComboBox) container).getSelectedItem();
      if (!EtomoDirector.INSTANCE.isUnitTest() || DEBUG) {
        System.err
          .println(prepend.toString() + ((JComboBox) container).getSelectedItem());
      }
      if (item instanceof Container) {
        printComponents(prepend, (Container) item);
      }
      Object[] objects = ((JComboBox) container).getSelectedObjects();
      if (objects != null) {
        for (int i = 0; i < objects.length; i++) {
          if (objects[i] instanceof Container) {
            printComponents(prepend, (Container) objects[i]);
          }
        }
      }
    }
    if (!EtomoDirector.INSTANCE.isUnitTest() || DEBUG) {
      if (container instanceof JFileChooser) {
        System.err.println(prepend + ((JFileChooser) container).getDialogTitle());
      }
      if (container instanceof JInternalFrame) {
        System.err.println(prepend + ((JInternalFrame) container).getTitle());
      }
      if (container instanceof JLabel) {
        System.err.println(prepend + ((JLabel) container).getText());
      }
      if (container instanceof JProgressBar) {
        System.err.println(prepend + ((JProgressBar) container).getString());
      }
      if (container instanceof JSpinner) {
        System.err.println(prepend.toString() + ((JSpinner) container).getValue());
      }
      if (container instanceof JTextComponent) {
        System.err.println(prepend + ((JTextComponent) container).getText());
      }
    }
    Component[] list = container.getComponents();
    if (list != null) {
      for (int i = 0; i < list.length; i++) {
        if (list[i] instanceof Container) {
          printComponents(prepend, (Container) list[i]);
        }
      }
    }
  }
  ...
  /**
   * Returns false if wrap would return the original string.  It is not required to call
   * this.
   * @param string
   * @param divider
   * @param minLength
   ...
   * @return
   */
  public static boolean canWrap(final String string, final String divider,
    final int minLength, final int wrapLength, final int maxLength) {
    if (string == null || string.isEmpty() || string.length() < maxLength
      || string.indexOf("\n") != -1
      || (maxLength <= 0 && ((divider == null || divider.isEmpty() || wrapLength < 0)))) {
      return false;
    }
    return true;
  }
  ...
  /**
   * Wraps a string using \n.  Does not go through the string character by character.
   * Tries to wrap as close to wrapLength as possible at the divider.  If this fails, goes
   * shorter or longer (prefers shorter) to wrap at the divider.  Wraps exactly at
   * maxLength if no divider is found up to dividerLength, and no divider is found before
   * maxLength.  The divider string is left at the end of each line, and each line is
   * trimmed.  Priorities: 1. as close to wrap length as possible, 2. longer then ideal
   * (less then max length), 3. max length or to the end of the line.  At least one length
   * parameter and a divider are required for wrapping to a divider.  Minimum wrap length
   * is divider length.  A string which can't be wrapped will be returned as is.
   * @param string - required - string to wrap - non-empty and not containing "\n" characters
   * @param divider - required with wrapLength - string on which to break - any non-empty string
   * @param minLength - don't bother to wrap strings or partial strings that are shorter then this - 0 to ignore
   * @param wrapLength - required with divider - highest ideal length to wrap to - 0 to ignore
   * @param maxLength - causes divider string to be ignored when no divider exists at an index less then maxLength. - 0 to ignore
   * @return
   */
  public static String wrap(final String string, final String divider, int minLength,
    int wrapLength, final int maxLength) {
    if (!canWrap(string, divider, minLength, wrapLength, maxLength)) {
      return string;
    }
    int dividerLength = 0;
    if (divider != null) {
    ...
    if (wrapLength > 0 && wrapLength < dividerLength) {
      wrapLength = dividerLength;
    }
    ...
    if (wrapLength <= 0 || divider == null || divider.isEmpty()
      || string.indexOf(divider) == -1) {
      if (maxLength > 0) {
        useMaxLength = true;
      }
      else {
    ...
    while (startIndex + minLength < length) {
      if (!useMaxLength) {
        eolIndex = string.lastIndexOf(divider, startIndex + wrapLength - dividerLength);
        if (eolIndex >= startIndex) {
          // Found the largest index up to the wrap lengthndex.
          eolIndex += dividerLength;
        }
        else {
          // Look for a longer wrap.
          eolIndex = string.indexOf(divider, startIndex + wrapLength - dividerLength);
          if (eolIndex >= 0 && (maxLength <= 0 || eolIndex < startIndex + maxLength)) {
            // Found a longer wrap.
            eolIndex += dividerLength;
          }
          else if (maxLength > 0) {
      ...
      if (eolIndex < 0) {
        ...
        if (startIndex == 0) {
          ...
          line = string.substring(startIndex).trim();
        }
      }
      else {
        line = string.substring(startIndex, eolIndex).trim();
      }
      // Add line to lines
    ...
    // Add the last line.
    String line = string.substring(startIndex).trim();
    if (line != null && !line.equals("")) {
      lines.append("\n" + line);
    }
    return lines.toString();
  ...
  public static String roundToMaxDecimalPlaces(final Number number,
    final ConstEtomoNumber maxDecimalPlaces) {
    if (number == null) {
      return null;
    }
    ...
    EtomoNumber enDouble = new EtomoNumber(EtomoNumber.Type.DOUBLE);
    enDouble.set(number);
    return roundToMaxDecimalPlaces(enDouble, maxDecimalPlaces);
  }

  public static String roundToMaxDecimalPlaces(final String number,
    final ConstEtomoNumber maxDecimalPlaces) {
    if (number == null) {
      return null;
    }
  ...
  /**
   * @param etomoNumber
   * @param maxDecimalPlaces - max digits allowed to the right of decimal (may not be negative)
   * @return rounded etomoNumber, or etomoNumber.toString() if unable to round
   */
  public static String roundToMaxDecimalPlaces(final ConstEtomoNumber etomoNumber,
    final ConstEtomoNumber maxDecimalPlaces) {
    char decimal = '.';
    if (etomoNumber == null) {
      return null;
    }
    String sNumber = etomoNumber.toString();
    if (maxDecimalPlaces == null || maxDecimalPlaces.isNull()
      || maxDecimalPlaces.isNegative() || etomoNumber.isNull()
      || etomoNumber.getType() != EtomoNumber.Type.DOUBLE) {
      return sNumber;
    }
    if (sNumber == null) {
      return null;
    ...
    int decimalIndex = -1;
    if ((decimalIndex = sNumber.indexOf(decimal)) == -1) {
      // No decimal places
      return sNumber;
    }
    int iMaxDecimalPlaces = maxDecimalPlaces.getInt();
    if (decimalIndex + iMaxDecimalPlaces + 1 > sNumber.length()) {
    ...
    if (iMaxDecimalPlaces > 0) {
      String sNum = (decimalIndex > 0 ? sNumber.substring(0, decimalIndex) : "")
        + sNumber.substring(decimalIndex + 1, decimalIndex + 1 + iMaxDecimalPlaces)
        + decimal
        + sNumber.substring(decimalIndex + 1 + iMaxDecimalPlaces, sNumber.length());
      EtomoNumber movedDecimalPlace = new EtomoNumber(EtomoNumber.Type.DOUBLE);
      movedDecimalPlace.set(sNum);
      // If this doesn't work send a warning and return the original number
      if (!movedDecimalPlace.isValid() || movedDecimalPlace.isNull()) {
        System.err
          .println("Warning:  unable to round " + sNumber + " to " + iMaxDecimalPlaces
            + "(" + sNum + ").  " + movedDecimalPlace.getInvalidReason());
        return sNumber;
      }
      // Round the number with the moved decimal place and put the decimal back in its
      // original location.
      EtomoNumber rounded = new EtomoNumber(EtomoNumber.Type.LONG);
      rounded.set(Math.round(movedDecimalPlace.getDouble()));
      return Double
        .toString(rounded.getLong() / (double) Math.pow(10, iMaxDecimalPlaces));
    }
    else {
      // Remove all decimal places
      EtomoNumber rounded = new EtomoNumber(EtomoNumber.Type.LONG);
      rounded.set(Math.round(etomoNumber.getDouble()));
  ...
  public static File getFile(BaseManager manager, boolean mustExist, AxisID axisID,
    ...
    if (!file.exists() && mustExist) {
      UIHarness.INSTANCE.openMessageDialog(manager,
        "The " + fileDescription + " file: " + file.getAbsolutePath() + " doesn't exist.",
        "Missing File", axisID);
      return null;
    }
    return file;
  ...
  public static File getFile(BaseManager manager, boolean mustExist, AxisID axisID,
    final FileType fileType, String fileDescription) {
    File file = fileType.getFile(manager, axisID);
    if (!file.exists() && mustExist) {
      UIHarness.INSTANCE.openMessageDialog(manager,
        "The " + fileDescription + " file: " + file.getAbsolutePath() + " doesn't exist.",
        "Missing File", axisID);
  ...
  public static String prepareRenameActionMessage(final File from, final File to) {
    return prepareRenameActionMessage(from, to, false);
  }

  public static String prepareRenameActionMessage(final File from, final File to,
    final boolean alwaysPrintAction) {
    if (!EtomoDirector.INSTANCE.getArguments().isActions() && !alwaysPrintAction) {
      return null;
    }
  ...
  public static String getCommandAction(final String[] commandArray,
    ...
    if ((commandArray.length == 1 && (command.equals("env") || command.equals("hostname")
  ...
  public static boolean renameFileSafely(final BaseManager manager, final AxisID axisID,
    ...
    if (destination.exists()) {
      UIHarness.INSTANCE.openMessageDialog(manager, "Unable to rename " + source.getName()
        + " to " + destination.getName() + " because the target file already exists.",
    ...
    if (source.exists()) {
      ...
      if (!source.renameTo(destination)) {
        if (!EtomoDirector.INSTANCE.isUnitTest() || DEBUG) {
          ...
          System.err
            .println("Unable to rename file to: " + destination.getAbsolutePath());
        }
      ...
      else if (actionMessage != null && (!EtomoDirector.INSTANCE.isUnitTest() || DEBUG)) {
   ...
   */
  public static void renameFile(final File source, final File destination,
    final boolean alwaysPrintAction) throws IOException {
    if (!source.exists()) {
      return;
    }
    ...
    if (source.exists()) {
      ...
      String actionMessage =
        prepareRenameActionMessage(source, destination, alwaysPrintAction);
      if (!source.renameTo(destination)) {
        if (!EtomoDirector.INSTANCE.isUnitTest() || DEBUG) {
          if (source.exists()) {
            System.err.println(source.getAbsolutePath() + " still exists");
  ...
  public static void copyFile(final File source, final File destination)
    ...
    destStream.close();
    if (actionMessage != null && (!EtomoDirector.INSTANCE.isUnitTest() || DEBUG)) {
      System.err.println(actionMessage);
    }
  }
  ...
  public static String quoteLabel(final String label) {
    ...
    char quote = '\'';
    return quote + stripLabel(label.trim()) + quote;
  }
  ...
  public static String getDateTimeStampRootName() {
    ...
    return new SimpleDateFormat("MMMdd-HHmmss", usLocale).format(new Date());
  }
  ...
  public static final ConstEtomoNumber
    convertNanometersToMicrons(final ConstEtomoNumber nm) {
  ...
  public static final String convertLabelToName(String label) {
    ...
    while (token != null && !token.is(Token.Type.EOF) && !token.is(Token.Type.EOL)) {
      if (token.equals(Token.Type.SYMBOL, '(')) {
        ...
        if (token.equals(Token.Type.SYMBOL, NAME_SEPARATOR) && !firstToken) {
          buffer.append(' ');
        }
