...
public final class EnvironmentVariable {
  ...
  private final HashSet<String> variableNotFoundList = new HashSet<String>();
  ...
  public String getValue(BaseManager manager, String propertyUserDir, String varName,
    ...
    if (Utilities.isWindowsOS()) {
      ...
      readEnvVar = new SystemProgram(manager, propertyUserDir,
        new String[] { "cmd.exe", "/C", "echo", var }, axisID);
      try {
        readEnvVar.run();
      }
        ...
        System.err.println(
          "Unable to run cmd command to find " + varName + " environment variable");
    ...
    // Non windows environment
      ...
      readEnvVar =
        new SystemProgram(manager, propertyUserDir, new String[] { "env" }, axisID);
      try {
        readEnvVar.run();
      }
      catch (Exception excep) {
        ...
        System.err.println(excep.getMessage());
        System.err.println(
          "Unable to run env command to find " + varName + " environment variable");
  ...
  public boolean exists(BaseManager manager, String propertyUserDir, String varName,
    ...
    synchronized (variableNotFoundList) {
      if (variableNotFoundList.contains(varName)) {
        return false;
      }
    }
    // There is not a real good way to access the system environment variables
    ...
    if (Utilities.isWindowsOS()) {
      ...
      try {
        ...
        System.err.println(
          "Unable to run cmd command to find " + varName + " environment variable");
        variableNotFoundList.add(varName);
        return false;
      }
      String[] stderr = readEnvVar.getStdError();
      if (stderr != null && stderr.length > 0) {
