...
public class MRCHeaderTest extends TestCase {
  private static final String FLOAT_BINNING_MRC_NAME = "binning0.5Header.st";
  private static final String TEST_DIR = "MRCHeader";
  private static final String FEI_MRC_NAME = "feiHeader.st";
  private static final String NEWER_BINNED_MRC_NAME = "newerHeaderBinned.st";
  private static final String BIDIR_SERIES_NAME = "bidirSeries.st";
  ...
  private final File testDir = new File(UtilTests.TEST_ROOT_DIR, TEST_DIR);
  ...
  public MRCHeaderTest(String arg0) {
    ...
    badFilename = MRCHeader.getInstance(testDirPath,
      testDirectory1 + "/non_existant_image_file", AxisID.ONLY);
    mrcHeader =
      MRCHeader.getInstance(testDirPath, testDirectory1 + "/headerTest.st", AxisID.ONLY);
    mrcWithSpaces =
      MRCHeader.getInstance(testDirPath, testDirectory2 + "/headerTest.st", AxisID.ONLY);
  }
  ...
  public void testRead()
    throws IOException, InvalidParameterException, SystemProcessException {
    // TEMP
    ...
    assertEquals("image rotation should be `Tilt axis rotation angle` in comment", 77.1,
      mrcHeader.getImageRotation().getDouble());
    // float binning
    TestUtilites.INSTANCE.copyTestFile(UtilTests.TEST_ROOT_DIR.getAbsolutePath(),
      TEST_DIR, FLOAT_BINNING_MRC_NAME);
    MRCHeader header =
      MRCHeader.getInstance(testDirPath, FLOAT_BINNING_MRC_NAME, AxisID.ONLY);
    assertTrue(header.read(manager));
    assertEquals("image rotation should be `Tilt axis angle` in comment", -12.5,
      header.getImageRotation().getDouble());
    assertEquals("binning is now a double - returns as a string", "0.5",
      header.getBinning());
    assertTrue("make sure twodir isn't being set from the first line in the header",
      header.getTwodir().isNull());
    // fei pixel size
    TestUtilites.INSTANCE.copyTestFile(UtilTests.TEST_ROOT_DIR.getAbsolutePath(),
      TEST_DIR, FEI_MRC_NAME);
    header = MRCHeader.getInstance(testDirPath, FEI_MRC_NAME, AxisID.ONLY);
    assertTrue(header.read(manager));
    assertEquals("image rotation should be `Tilt axis rotation angle` in comment", -24.9,
      header.getImageRotation().getDouble());
    assertEquals("default value scan pixel (1,2,4) means use the fei pixel size * 10",
      10.16, header.getXPixelSize().getDouble());
    assertTrue("make sure twodir isn't being set from the first line in the header",
      header.getTwodir().isNull());
    // newer binned
    TestUtilites.INSTANCE.copyTestFile(UtilTests.TEST_ROOT_DIR.getAbsolutePath(),
      TEST_DIR, NEWER_BINNED_MRC_NAME);
    header = MRCHeader.getInstance(testDirPath, NEWER_BINNED_MRC_NAME, AxisID.ONLY);
    assertTrue(header.read(manager));
    assertEquals("image rotation is `Tilt axis angle` in comment", -4.8,
      header.getImageRotation().getDouble());
    assertEquals("binning is a double", "2.0", header.getBinning());
    assertTrue("make sure twodir isn't being set from the first line in the header",
      header.getTwodir().isNull());
    // bidirSeries
    TestUtilites.INSTANCE.copyTestFile(UtilTests.TEST_ROOT_DIR.getAbsolutePath(),
      TEST_DIR, BIDIR_SERIES_NAME);
    header = MRCHeader.getInstance(testDirPath, BIDIR_SERIES_NAME, AxisID.ONLY);
    assertTrue(header.read(manager));
    assertEquals("image rotation is `Tilt axis angle` in comment", -4.8,
      header.getImageRotation().getDouble());
    assertEquals("binning is a double", "2.0", header.getBinning());
    assertTrue("twodir is set", header.getTwodir().toString().equals("0.0"));
  }
