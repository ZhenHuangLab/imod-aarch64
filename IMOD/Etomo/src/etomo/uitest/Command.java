 ...
 * 
 * <p>Copyright: Copyright 2008 - 2019</p>
 *
 * <p>Organization:
 ...
 * <p> </p>
 */
final class Command extends Assert implements UITestCommandInfo {
  public static final String rcsid =
    "$Id$";

  private static final String SEPARATOR_OPERATOR = "|";
  private static final char IGNORE_VARIABLE_OPERATOR = '"';
  private static final char COMMAND_ELEMENT_DIVIDER = '.';
  ...
  Command(final AxisID testAxisID, final Field field) {
    this.testAxisID = testAxisID;
    this.subject = new Subject(testAxisID);
    if (field != null) {
      this.field = field;
    }
    else {
      this.field = new Field(testAxisID);
    }
  }

  /**
   * return the command for "bn.buttonName="
   * @param testAxisID
   * @param buttonName
   * @return
   */
  static Command getButtonFieldInstance(AxisID testAxisID, final String buttonName) {
    String string = UITestFieldType.BUTTON.toString() + COMMAND_ELEMENT_DIVIDER
      + buttonName + AutodocTokenizer.DEFAULT_DELIMITER;
    Field field = Field.getButtonInstance(testAxisID, buttonName, string);
    Command instance = new Command(testAxisID, field);
  ...
  public String formatCommandInfo(final AxisID axisID) {
    return " (" + "line:" + getLineNum() + " "
      + (axisID != null ? axisID.getExtension() + ": " : "") + toString() + ")";
  }
  ...
  private void assertValid() {
    ...
    if (modifierType != null) {
      assertFalse(
        "modifierType can't exist without a subject or a field (" + string + ")",
  ...
  public void set(ReadOnlyStatement statement, VariableList variableList) {
    ...
    assertNull(
      "unknown attributes at the end of the command - " + leftSide + " (" + string + ")",
      leftSide);
  ...
  private void setValue(String rightSide, VariableList variableList) {
    ...
    if (valueArray != null) {
      ...
      for (int i = 0; i < valueArray.length; i++) {
        ...
        buffer
          .append(valueArray[i] + (i < valueArray.length - 1 ? SEPARATOR_OPERATOR : ""));
      }
      if (buffer.length() > 0) {
