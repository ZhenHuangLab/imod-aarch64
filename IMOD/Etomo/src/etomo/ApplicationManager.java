...
import etomo.comscript.SetEnvParam;
...
import etomo.storage.BatchRunTomoLog;
...
import etomo.storage.ProcessSignal;
...
import etomo.ui.swing.MultifiltSetupDisplay;
...
public final class ApplicationManager extends BaseManager
  implements ContinuousListenerTarget {
   ...
   */
  public ApplicationManager(final String paramFileName, AxisID axisID) {
    super();
    metaData = new MetaData(this, getLogProperties(),
      paramFileName == null || paramFileName.isEmpty());
    createState();
    ...
    state.moveBatchruntomoSettings();
    initializeAdvanced();
  ...
  public ProcessResultDisplayFactoryInterface
    getProcessResultDisplayFactoryInterface(AxisID axisID) {
    ...
    if (axisID == AxisID.SECOND) {
      if (processResultDisplayFactoryB == null) {
        processResultDisplayFactoryB = ProcessResultDisplayFactory
          .getInstance(getBaseScreenState(axisID), axisID, metaData.getAxisType());
      }
  ...
  public boolean isSetupChanged() {
    ...
    String rawImageStack = setupDialogExpert.getRawImageStack();
    return rawImageStack != null && !rawImageStack.matches("\\s*");
  }
  ...
  public void openSetupDialog() {
    ...
    if (setupDialogExpert == null) {
      ...
      setupDialogExpert = setupReconUIHarness
        .getSetupDialogExpert(mainPanel.mapAxisProgressPanel(AxisID.ONLY));
  ...
  BatchruntomoParam updateBatchruntomoForRenameInputImageFiles(final AxisID axisID) {
    if (EtomoDirector.INSTANCE.getArguments().isFromBRT()) {
      // Batchruntomo will take care of this.
      return null;
    }
    BatchruntomoParam param =
      BatchruntomoParam.getRenameInputImageFilesInstance(this, axisID);
    if (setupReconUIHarness.getParametersForRenameInputImageFiles(param)) {
      return param;
    }
    return null;
  }
   ...
   * @returns the input image file
   */
  public void doneSetupDialog(boolean removeExcludedViewsA, boolean removeExcludedViewsB,
    final String datasetDir, final boolean dualAxis,
    final AxisProgressPanel progressPanel) {
    AxisType axisType = dualAxis ? AxisType.DUAL_AXIS : AxisType.SINGLE_AXIS;
    if (progressPanel != null) {
      progressPanel.correctAxisID(axisType);
    ...
    // set Failure process
    ProcessSeries processSeries = new ProcessSeries(this, axisID, DialogType.SETUP_RECON);
    processSeries.setFailProcess(Task.SETUP_RECON_FAILED);
    ...
    boolean doValidation = exitState == DialogExitState.EXECUTE;
    ...
    if (metaData.isValid()) {
      Extension rawStackExtension =
        setupReconUIHarness.getRawStackExtension(doValidation);
      if (rawStackExtension == null) {
        processSeries.startFailProcess(axisID);
        return;
      }
      if (setupReconUIHarness.checkForSharedDirectory(rawStackExtension)) {
        uiHarness.openMessageDialog(this,
          "This directory (" + propertyUserDir
            + ") is already being used by an .edf file.  Either open the "
            + "existing .edf file or create a new directory for the new "
            + "reconstruction.",
          "WARNING:  CANNOT PROCEED", AxisID.ONLY);
        processSeries.startFailProcess(axisID);
        return;
      }
      metaData.setRawImageStackExtension(rawStackExtension);
      metaData.setOrigRawImageStackExtension(rawStackExtension);
    ...
    if (exitState == DialogExitState.EXECUTE) {
      // Set processes
      processSeries.addProcess(Task.VALIDATE_DIRECTIVE_FILES);
      processSeries.addProcess(Task.SET_RAW_STACK_EXTENSION);
      processSeries.addProcess(Task.SET_ORIG_RAW_STACK_EXTENSION);
      if (!setupReconUIHarness.isDirectiveDrivenAutomation()) {
        processSeries.addProcess(Task.RENAME_INPUT_IMAGE_FILES);
      }
      // remove excluded views Axis A
      String viewsToExcludeA = setupReconUIHarness.getViewsToSkip(axisID, doValidation);
      removeExcludedViewsA = removeExcludedViewsA && viewsToExcludeA != null
        && !viewsToExcludeA.matches("\\s*");
      if (removeExcludedViewsA) {
        if (dualAxis) {
          axisID = AxisID.FIRST;
        }
        processMgr.unblockAxis(axisID);
        processSeries.addProcess(Task.EXCLUDE_VIEWS_A,
          updateExcludeViewsParam(axisID, datasetDir, dualAxis, viewsToExcludeA, true),
          axisType);
      }
      // remove excluded views Axis B
      axisID = AxisID.SECOND;
      String viewsToExcludeB = setupReconUIHarness.getViewsToSkip(axisID, doValidation);
      removeExcludedViewsB = removeExcludedViewsB && dualAxis && viewsToExcludeB != null
        && !viewsToExcludeB.matches("\\s*");
      if (removeExcludedViewsB) {
        processMgr.unblockAxis(axisID);
        processSeries.addProcess(Task.EXCLUDE_VIEWS_B,
          updateExcludeViewsParam(axisID, datasetDir, dualAxis, viewsToExcludeB, true),
          axisType);
      }
      // copytomocoms
      processSeries.setLastProcess(Task.COPY_TOMO_COMS);
    }
    processSeries.startNextProcess(AxisID.ONLY);
  }

  /**
   * Runs validation on either the automation directive file or the three templates
   * @param processSeries required, should contain a fail process
   */
  private void validateDirectiveFiles(final ProcessSeries processSeries,
    final AxisID axisID) {
    // Automation
    if (setupReconUIHarness.isDirectiveDrivenAutomation()
      ...
      BatchruntomoParam param = updateBatchruntomo(true, setupReconUIHarness
        .getDirectiveFileCollection().getDirectiveFile(DirectiveFileType.BATCH), axisID);
      if (param != null && !processMgr.batchruntomo(axisID, param)) {
        processSeries.startFailProcess(axisID);
        return;
      }
    }
      ...
      BatchruntomoParam param = updateBatchruntomo(false, setupReconUIHarness
        .getDirectiveFileCollection().getDirectiveFile(DirectiveFileType.SCOPE), axisID);
      if (param != null && !processMgr.batchruntomo(axisID, param)) {
        processSeries.startFailProcess(axisID);
        return;
      }
      param = updateBatchruntomo(false, setupReconUIHarness.getDirectiveFileCollection()
        .getDirectiveFile(DirectiveFileType.SYSTEM), axisID);
      if (param != null && !processMgr.batchruntomo(axisID, param)) {
        processSeries.startFailProcess(axisID);
        return;
      }
      param = updateBatchruntomo(false, setupReconUIHarness.getDirectiveFileCollection()
        .getDirectiveFile(DirectiveFileType.USER), axisID);
      if (param != null && !processMgr.batchruntomo(axisID, param)) {
        processSeries.startFailProcess(axisID);
        return;
      }
  ...
  private void renameInputImageFile(final ProcessSeries processSeries,
    final AxisID axisID) {
    if (!EtomoDirector.INSTANCE.getArguments().isFromBRT()) {
      BatchruntomoParam param = updateBatchruntomoForRenameInputImageFiles(axisID);
      if (param != null && processMgr.batchruntomo(axisID, param)) {
        processSeries.startNextProcess(axisID);
      }
      else {
  ...
  private void setRawImageStackExtension(final ProcessSeries processSeries,
    final AxisID axisID) {
    setRawImageStackExtension(setupReconUIHarness.getRawImageStack());
    processSeries.startNextProcess(axisID);
  }

  private void setOrigRawImageStackExtension(final ProcessSeries processSeries,
    final AxisID axisID) {
    String rawImageStackFileName = setupReconUIHarness.getRawImageStack();
    if (rawImageStackFileName == null) {
      Thread.dumpStack();
    ...
    Extension extension = Extension.getInstance(rawImageStackFileName);
    if (extension == null || !extension.isInputImageFile()) {
      System.err.println(
        "Error:  invalid extension for raw image stack: " + rawImageStackFileName);
      if (processSeries != null) {
        processSeries.endSeries();
      }
      return;
    }
    metaData.setOrigRawImageStackExtension(extension);
    processSeries.startNextProcess(axisID);
  }

  /**
   * Needs to be usable during setup - before the param file is marked as loaded.
   * @param rawImageStackFileName
   */
  public void setRawImageStackExtension(final String rawImageStackFileName) {
    if (rawImageStackFileName == null) {
      return;
    }
    Extension extension = Extension.getInstance(rawImageStackFileName);
    ...
    metaData.setRawImageStackExtension(extension);
  }

  private void excludeViews(AxisID axisID, ProcessSeries processSeries,
    final String datasetDir, final AxisType axisType, final String viewsToExclude,
    Command param) {
    // Make sure that the post processing panel is open
    if (setupReconUIHarness == null) {
      uiHarness.openMessageDialog(this, "Setup Reconstruction dialog not open",
        "Program logic error", AxisID.ONLY);
      processSeries.startFailProcess(axisID);
      return;
    }
    if (axisID != AxisID.SECOND) {
      axisID = axisType == AxisType.DUAL_AXIS ? AxisID.FIRST : AxisID.ONLY;
    }
    // force previews to exit
    try {
      imodManager.quitAll(ImodManager.PREVIEW_KEY, axisID);
      releaseFile();
    }
    ...
    if (setupDialogExpert != null) {
      setupDialogExpert.showProgressPanel();
    }
    if (param == null) {
    ...
    if (setupDialogExpert != null
      ...
      if (!FileType.RAW_TILT_ANGLES.exists(this, axisID)) {
        uiHarness.openMessageDialog(this,
          "Missing raw tilt file: " + FileType.RAW_TILT_ANGLES.getFileName(this, axisID),
          "Unable to execute command", AxisID.ONLY);
        processSeries.startFailProcess(axisID);
        return;
      }
    }
    String threadName;
    try {
      threadName = processMgr.excludeViews(param, axisID, processSeries);
    }
    catch (AxisBusyException e) {
      e.printStackTrace();
      String[] message = new String[2];
      message[0] = "Can not execute excludeviews command";
    ...
    if (mainPanel != null) {
      mainPanel.setProcessCanBeKilled(false, axisID);
      mainPanel.startProgressBar(
        "Removing excluded views" + ((axisID == AxisID.FIRST || axisID == AxisID.SECOND)
          ? " for " + axisID.toString() + " axis" : ""),
        axisID, ProcessName.EXCLUDE_VIEWS);
    }
  }

  private Command updateExcludeViewsParam(final AxisID axisID, final String datasetDir,
    final boolean dualAxis, final String viewsToExclude, final boolean doValidation) {
    if (setupReconUIHarness == null) {
      return null;
    }
    // Get the metadata excludeviews param.
    ExcludeViewsParam param = new ExcludeViewsParam(axisID, datasetDir);
    if (!setupReconUIHarness.getParameters(param, axisID, dualAxis, doValidation)) {
      return null;
    }
    param.setViewsToExclude(viewsToExclude);
    return param;
  }

  private void msgSetupReconFailed(final ProcessSeries processSeries) {
    if (setupDialogExpert != null) {
      setupDialogExpert.msgSetupReconFailed();
    }
    if (processSeries != null) {
      processSeries.endSeries();
    }
  ...
  public void endAutomation(final boolean success) {
    if (EtomoDirector.INSTANCE.getArguments().isDirective()
      && EtomoDirector.INSTANCE.getArguments().isExit()) {
      uiHarness.exit(AxisID.ONLY, success ? 0 : 1);
    }
  }
  ...
  public void kill(final AxisID axisID) {
    super.kill(axisID);
    if (setupDialogExpert != null) {
      setupDialogExpert.updateDisplay(true);
    }
  }

  private boolean copyTomoComs(final ProcessSeries processSeries) {
    setupReconUIHarness.getFields(metaData, true);
    // copytomocoms
    ...
    if (messages == null) {
      ...
      if (processSeries != null) {
        processSeries.startFailProcess(AxisID.ONLY);
      }
      mainPanel.stopProgressBar(null);
      return false;
    }
    // Send a specific INFO: message to the project log
    if (!messages.isEmpty(ProcessMessages.MessageType.INFO)) {
      ArrayList<String> infoMessages = messages.match(ProcessMessages.MessageType.INFO,
        new String[] { "Setting logarithm offset", "Pixel spacing",
  ...
  private void completeSetupState() {
  ...
  private void closeSetupDialog() {
    if (setupDialogExpert != null) {
      setupDialogExpert.setDisplayed(false);
    }
  ...
  private void copyDirectiveFiles() {
    ...
    try {
      ...
      if (directiveFile != null) {
        ...
        if (file != null) {
          uiHarness.openMessageDialog(this,
            "Unable to copy " + file.getAbsolutePath() + " to dataset.",
            "Unable to Copy File");
        }
        else {
          System.err
  ...
  public void openPreProcDialog(AxisID axisID) {
    ...
    preProcDialog.setCCDEraserParams(
      comScriptMgr.getCCDEraserParam(axisID, CCDEraserParam.Mode.X_RAYS));
  ...
  public void findXrays(AxisID axisID, final ProcessResultDisplay processResultDisplay,
    ...
    if (processSeries == null) {
      processSeries = new ProcessSeries(this, axisID, dialogType);
    }
  ...
  public void imodErasedStack(AxisID axisID, Run3dmodMenuOptions run3dmodMenuOptions) {
    if (Utilities.getFile(this, true, axisID, FileType.FIXED_XRAYS_STACK,
      "erased stack") == null) {
      return;
    }
    try {
   ...
   * currentAxisID - stack to archive
   * 
   * @param axisID
   */
  private void archiveOriginalStack(AxisID currentAxisID, ProcessSeries processSeries,
    final DialogType dialogType) {
    if (processSeries == null) {
      processSeries = new ProcessSeries(this, AxisID.FIRST, dialogType);
    }
    ...
    File originalStack = Utilities.getFile(this, false, stackAxisID,
      FileType.ORIGINAL_RAW_STACK, "original stack");
  ...
  public void deleteOriginalStack(Command archiveorigParam, String[] output) {
    ...
    File originalStack =
      Utilities.getFile(this, false, axisID, FileType.ORIGINAL_RAW_STACK, "");
  ...
  public String getArchiveInfo(AxisID axisID) {
    File stack = Utilities.getFile(this, false, axisID, FileType.RAW_STACK, "");
    File originalStack =
      Utilities.getFile(this, false, axisID, FileType.ORIGINAL_RAW_STACK, "");
  ...
  public void updateArchiveDisplay() {
    ...
    if (metaData.getAxisType() == AxisType.SINGLE_AXIS) {
      cleanUpDialog
        .updateArchiveDisplay(DatasetFiles.getOriginalStack(this, AxisID.ONLY).exists());
    }
    else {
      cleanUpDialog
        .updateArchiveDisplay(DatasetFiles.getOriginalStack(this, AxisID.FIRST).exists()
  ...
  public void replaceRawStack(AxisID axisID, ProcessResultDisplay processResultDisplay,
    ...
    if (!Utilities.isValidStack(fixedXraysFile, this, axisID)) {
      uiHarness.openMessageDialog(this,
        fixedXraysFile.getName() + " is not a valid MRC file.", "Entry Error", axisID);
      sendMsgProcessFailedToStart(processResultDisplay);
      return;
    }
    processTrack.setState(ProcessState.INPROGRESS, axisID, dialogType);
  ...
  private boolean renameXrayStack(AxisID axisID) {
    File xrayStack = new File(propertyUserDir,
      metaData.getDatasetName() + axisID.getExtension() + "_xray.st.gz");
    if (!xrayStack.exists()) {
      return true;
    }
    ...
    if (xrayStackArchives != null) {
      ...
      for (int i = 0; i < xrayStackArchives.length; i++) {
        newFileNumber
          .set(xrayStackArchives[i].substring(xrayStackArchives[i].lastIndexOf('.') + 1));
  ...
  public void imodPreview(final String fileExtension, final AxisID axisID,
    ...
    File previewWorkingDir = previewMetaData.getValidDatasetDirectory(
      setupDialogExpert.getWorkingDirectory().getAbsolutePath());
  ...
  public void saveCoarseAlignDialog(CoarseAlignDialog coarseAlignDialog, AxisID axisID) {
    setAdvanced(coarseAlignDialog.getDialogType(), axisID,
      coarseAlignDialog.isAdvanced());
  ...
  public void tiltxcorr(final AxisID axisID, ProcessResultDisplay processResultDisplay,
    ...
    ProcessName processName, final boolean runTiltxcorr, final boolean breakContours) {
    sendMsgProcessStarting(processResultDisplay);
    if (processSeries == null) {
      processSeries = new ProcessSeries(this, axisID, dialogType);
    }
    ...
    try {
      ...
      if (blendmontParam == null) {
        ...
        threadName = processMgr.tiltxcorr(tiltxcorrParam, processName, axisID,
   ...
   * @param metaData
   */
  private void processBatchRunTomoLog() {
    BatchRunTomoLog log = new BatchRunTomoLog();
    BatchRunTomoLog.Iterator iterator = log.seek(this, metaData);
    if (iterator == null) {
      return;
    }
    while (iterator.hasNext()) {
      ProcessSignal processSignal = iterator.next();
      if (processSignal == null) {
        continue;
      }
      ProcessState processState = processSignal.getProcessState();
      DialogType dialogType = processSignal.getDialogType();
      AxisID axisID = processSignal.getAxisID();
      if (axisID == AxisID.SECOND) {
  ...
  public void preCrossCorrelate(AxisID axisID, ProcessResultDisplay processResultDisplay,
    ...
    if (processSeries == null) {
      processSeries = new ProcessSeries(this, axisID, dialogType, display);
    }
    sendMsgProcessStarting(processResultDisplay);
    if (metaData.getViewType() == ViewType.MONTAGE
    ...
    tiltxcorr(axisID, processResultDisplay, null, null, processSeries, dialogType,
      display, true, ProcessName.XCORR/*was: FileType.CROSS_CORRELATION_COMSCRIPT*/, true,
      false);
  }
  ...
  private boolean processBStack() {
    ...
    File bStack = DatasetFiles.getStack(this, propertyUserDir, metaData, axisID);
    if (!bStack.exists()) {
      return false;
    }
  ...
  private void extractmagrad(AxisID axisID, ProcessResultDisplay processResultDisplay,
    ...
    if (magGradientFileName == null
      || magGradientFileName.matches("\\s*+") && processSeries != null) {
      processSeries.startNextProcess(axisID, processResultDisplay);
      return;
    }
  ...
  public void clipStats(AxisID axisID, FileType inputFileType,
    ...
    catch (AxisBusyException except) {
      except.printStackTrace();
  ...
  public void coarseAlign(AxisID axisID, final ProcessResultDisplay processResultDisplay,
    ...
    if (metaData.getViewType() == ViewType.MONTAGE) {
      ...
      try {
        if ((prenewstParam =
          updatePrenewstCom(newstackDisplay, axisID, true, true)) == null) {
          sendMsgProcessFailedToStart(processResultDisplay);
          processSeries.endSeries();
          return;
        }
  ...
  public void imodSortedModels(final AxisID axisID, final Run3dmodMenuOptions menuOptions,
    final List<String> modelNameList) {
    try {
  ...
  public void midasRawStack(AxisID axisID, ProcessResultDisplay processResultDisplay) {
    ...
    MidasParam param = new MidasParam(this, axisID, MidasParam.Mode.RAW_STACK);
    getParameters(param, axisID);
    try {
      ...
      UIHarness.INSTANCE.openMessageDialog(this, "Unable open midas on the raw stack.  ",
  ...
  public void midasFixEdges(AxisID axisID, ProcessResultDisplay processResultDisplay) {
    sendMsgProcessStarting(processResultDisplay);
    MidasParam param = new MidasParam(this, axisID, MidasParam.Mode.FIX_EDGES);
    getParameters(param, axisID);
    try {
      processMgr.midas(param);
      processTrack.setCoarseAlignmentState(ProcessState.INPROGRESS, axisID);
      ...
      UIHarness.INSTANCE.openMessageDialog(this,
        "Unable open midas on " + param.getInputFileName() + ".  ",
        "Unable to Run Process", axisID);
      sendMsgProcessFailedToStart(processResultDisplay);
      return;
    }
  ...
  public boolean getParameters(final MidasParam param, final AxisID axisID) {
    if (param == null) {
      return false;
    }
    MidasParam.Mode mode = param.getMode();
    if (mode == MidasParam.Mode.FIX_EDGES) {
      FileType fileType;
      if (metaData.isDistortionCorrection()) {
        fileType = FileType.DISTORTION_CORRECTED_STACK;
      }
      else {
        fileType = FileType.RAW_STACK;
      }
      param.setInputFileName(fileType.getFileName(this, axisID));
      param.setBinning(0);
    }
    else if (mode == MidasParam.Mode.RAW_STACK) {
      if (metaData.getViewType() == ViewType.MONTAGE) {
        param.setInputFileName(FileType.XCORR_BLEND_OUTPUT.getFileName(this, axisID));
      }
      else {
        param.setInputFileName(FileType.RAW_STACK.getFileName(this, axisID));
      }
      param.setImageRotation(metaData.getImageRotation(axisID).getDouble());
    }
    return true;
  }
  ...
  public void openFiducialModelDialog(AxisID axisID) {
    ...
    fiducialModelDialog.setBeadtrackParams(comScriptMgr.getBeadtrackParam(axisID), false);
    ...
    fiducialModelDialog.setParameters(autofidseedParam, false);
  ...
  private boolean okToFiducialModelTrack(AxisID axisID) {
    ...
    if (fiducialLastModified > seedLastModified && !fiducialLastTracked.isNull()
      ...
      return uiHarness.openYesNoWarningDialog(this,
   ...
   * track com script
   */
  public void fiducialModelTrack(AxisID axisID, ProcessResultDisplay processResultDisplay,
    final ProcessSeries processSeries, DialogType dialogType, BeadTrackDisplay display) {
    sendMsgProcessStarting(processResultDisplay);
  ...
  public boolean makeFiducialModelSeedModel(AxisID axisID) {
    ...
    if (!fiducialModel.exists()) {
      uiHarness.openMessageDialog(this,
        "Fiducial model does not exist.  " + BeadtrackPanel.USE_MODEL_LABEL + " failed.",
    ...
    try {
      if (imodManager.isOpen(ImodManager.COARSE_ALIGNED_KEY, axisID)) {
        if (seedModel.getName()
          .equals(imodManager.getModelName(ImodManager.COARSE_ALIGNED_KEY, axisID))) {
  ...
  public int getBeadfixerDiameter(AxisID axisID) {
    return (int) Math.round(metaData.getFiducialDiameter() / metaData.getPixelSize()
      / UIExpertUtilities.INSTANCE.getStackBinning(this, axisID,
        FileType.PREALIGNED_STACK));
  }

  public double calcBinnedBeadDiameterPixels(final AxisID axisID, final FileType fileType,
    ...
    return (double) (Math.round(metaData.getFiducialDiameter() / metaData.getPixelSize()
      / Utilities.getStackBinning(this, axisID, fileType) * adj)) / adj;
  }

  public double calcUnbinnedBeadDiameterPixels() {
    return (double) (Math
      .round(metaData.getFiducialDiameter() / metaData.getPixelSize() * 100)) / 100.0;
  }
  ...
  public void saveDialogs() {
    ...
    getUIExpert(DialogType.TOMOGRAM_POSITIONING, firstAxisID)
      .saveDialog(DialogExitState.SAVE);
    getUIExpert(DialogType.TOMOGRAM_POSITIONING, AxisID.SECOND)
      .saveDialog(DialogExitState.SAVE);
    getUIExpert(DialogType.FINAL_ALIGNED_STACK, firstAxisID)
      .saveDialog(DialogExitState.SAVE);
    getUIExpert(DialogType.FINAL_ALIGNED_STACK, AxisID.SECOND)
      .saveDialog(DialogExitState.SAVE);
    getUIExpert(DialogType.TOMOGRAM_GENERATION, firstAxisID)
      .saveDialog(DialogExitState.SAVE);
    getUIExpert(DialogType.TOMOGRAM_GENERATION, AxisID.SECOND)
      .saveDialog(DialogExitState.SAVE);
  ...
  private void updateDirective(final DirectiveMap map, final DirectiveDef directiveDef,
    ...
    Directive directive = map.getDirective(directiveDef);
    if (directive != null) {
  ...
  private void updateDirective(final DirectiveMap map, DirectiveDef directiveDef,
    final AxisID pairAxisID, final StringBuffer errmsg, final boolean value) {
    Directive directive = map.getDirectiveFromPair(directiveDef, pairAxisID);
    if (directive != null) {
      directive.setValue(value);
    }
    else {
  ...
  private void updateDirective(final DirectiveMap map, final DirectiveDef directiveDef,
    final AxisID pairAxisID, final StringBuffer errmsg, final ConstEtomoNumber value) {
    Directive directive = map.getDirectiveFromPair(directiveDef, pairAxisID);
    if (directive != null) {
      directive.setValue(value);
    }
  ...
  private void updateDirective(final DirectiveMap map, final DirectiveDef directiveDef,
    final AxisID pairAxisID, final StringBuffer errmsg, final double[] value) {
    Directive directive = map.getDirectiveFromPair(directiveDef, pairAxisID);
    if (directive != null) {
      directive.setValue(value);
    }
  ...
  private void updateDirective(final DirectiveMap map, final DirectiveDef directiveDef,
    final AxisID pairAxisID, final StringBuffer errmsg, final String value) {
    Directive directive = map.getDirectiveFromPair(directiveDef, pairAxisID);
    if (directive != null) {
      directive.setValue(value);
    }
  ...
  public void updateDirectiveMap(final DirectiveMap directiveMap,
    ...
    if (dualAxis) {
      ...
      if (tiltAngleSpec.getType() == TiltAngleType.RANGE) {
        updateDirective(directiveMap, DirectiveDef.BFIRST_INC, curAxisID, errmsg,
          new double[] { tiltAngleSpec.getRangeMin(), tiltAngleSpec.getRangeStep() });
      }
      else if (tiltAngleSpec.getType() == TiltAngleType.FILE) {
        updateDirective(directiveMap, DirectiveDef.BUSE_RAW_TLT, curAxisID, errmsg, true);
      }
      else if (tiltAngleSpec.getType() == TiltAngleType.EXTRACT) {
        updateDirective(directiveMap, DirectiveDef.BEXTRACT, curAxisID, errmsg, true,
          true);
      }
    }
    curAxisID = firstAxisID;
    ...
    if (metaData.isTwodir(curAxisID)) {
    ...
    updateDirective(directiveMap, DirectiveDef.BINNING, errmsg, metaData.getBinning(),
      "1");
    updateDirective(directiveMap, DirectiveDef.GRADIENT, errmsg,
      metaData.getMagGradientFile());
    updateDirective(directiveMap, DirectiveDef.FOCUS, curAxisID, errmsg,
      metaData.getAdjustedFocusA().is());
    if (dualAxis) {
      curAxisID = AxisID.SECOND;
      updateDirective(directiveMap, DirectiveDef.FOCUS, curAxisID, errmsg,
        metaData.getAdjustedFocusB().is());
    }
    curAxisID = firstAxisID;
    ...
    // Preprocessing
    updateDirective(directiveMap, DirectiveDef.REMOVE_XRAYS, errmsg,
    ...
    updateDirective(directiveMap, DirectiveDef.SEEDING_METHOD, errmsg,
      ...
      SeedingMethod.MANUAL.getValue().toString());
    // Beadtracking
    ...
    updateDirective(directiveMap, DirectiveDef.CORRECT_CTF, errmsg,
      getScreenState(curAxisID).getButtonState(getProcessResultDisplayFactory(curAxisID)
        .getCtfCorrection().getButtonStateKey()));
    updateDirective(directiveMap, DirectiveDef.ERASE_GOLD, errmsg,
      getScreenState(curAxisID).getButtonState(getProcessResultDisplayFactory(curAxisID)
        .getCcdEraserBeads().getButtonStateKey()));
    updateDirective(directiveMap, DirectiveDef.FILTER_STACK, errmsg,
    ...
    updateDirective(directiveMap, DirectiveDef.DO_BACKPROJ_ALSO, errmsg,
      getScreenState(curAxisID).getButtonState(getProcessResultDisplayFactory(curAxisID)
        .getTilt(DialogType.TOMOGRAM_GENERATION).getButtonStateKey()));
    // Postprocessing
    // Trimvol module
   ...
   * 
   */
  public void saveAlignmentEstimationDialog(AlignmentEstimationDialog fineAlignmentDialog,
    AxisID axisID) {
  ...
  public void closeImods(String key, AxisID axisID, String description) {
    if (EtomoDirector.INSTANCE.getArguments().isTest()) {
      System.err.println(
        "closeImods:key:" + key + ",axisID:" + axisID + ",description:" + description);
    }
  ...
  public void restrictalign(AxisID axisID, ProcessSeries processSeries) {
    ...
    if (processSeries == null) {
      processSeries = new ProcessSeries(this, axisID, DialogType.FINE_ALIGNMENT);
  ...
  private void reloadAlignCom(final AxisID axisID, final ProcessSeries processSeries) {
  ...
  public void imodViewModel(AxisID axisID, FileType modelFileType) {
    try {
      ...
      uiHarness.openMessageDialog(this, except.getMessage(),
        "Can't open 3dmod on " + modelFileType.getFileName(this, axisID), axisID);
    }
    catch (IOException e) {
      e.printStackTrace();
      uiHarness.openMessageDialog(this, e.getMessage(), "IO Exception", axisID);
  ...
  public void imodCtfCorrection(AxisID axisID, Run3dmodMenuOptions menuOptions) {
    try {
      ...
      uiHarness.openMessageDialog(this, except.getMessage(),
        "Can't open 3dmod on " + ProcessName.CTF_CORRECTION.toString() + " results",
        axisID);
    }
    catch (IOException e) {
      e.printStackTrace();
  ...
  void processSeriesSucceeded(AxisID axisID, ProcessName processName) {
  ...
  /**
   * The exclude views value must be removed because, if excludeviews was rerun with the
   * values from the previous run, more views will be removed.
   * @param axisID
   */
  public void msgExcludeViewsSucceeded(final AxisID axisID) {
    if (setupReconUIHarness != null) {
      setupReconUIHarness.msgExcludeViewsSucceeded(axisID, true, false);
    }
    metaData.resetExcludeProjections(axisID);
  }
  ...
  public void transferfid(final AxisID destAxisID,
    ...
    final Run3dmodMenuOptions run3dmodMenuOptions, final DialogType dialogType) {
    if (processSeries == null) {
      processSeries = new ProcessSeries(this, destAxisID, dialogType);
    }
    sendMsgProcessStarting(processResultDisplay);
    // Set a reference to the correct object
    FiducialModelDialog fiducialModelDialog;
    ...
    if (destAxisID == AxisID.SECOND) {
      fiducialModelDialog = fiducialModelDialogB;
      fromAxis = AxisID.FIRST;
    }
    else {
      fiducialModelDialog = fiducialModelDialogA;
      fromAxis = AxisID.SECOND;
    }
    if (destAxisID != AxisID.ONLY && !Utilities.fileExists(this, "fid.xyz",
    ...
    if (comScriptMgr.loadAutofidseed(fromAxis, false)) {
      AutofidseedParam fromAutofidseedParam = comScriptMgr.getAutofidseedParam(fromAxis);
      fiducialModelDialog.setParameters(fromAutofidseedParam, true);
    }
    comScriptMgr.loadTrack(fromAxis);
    BeadtrackParam fromTrackParam = comScriptMgr.getBeadtrackParam(fromAxis);
    fiducialModelDialog.setBeadtrackParams(fromTrackParam, true);
    //
      ...
      if (axisID == AxisID.SECOND) {
        if (tomogramPositioningExpertB == null) {
          tomogramPositioningExpertB = new TomogramPositioningExpert(this, mainPanel,
            processTrack, axisID, metaData.getAxisType());
        }
  ...
  public ProcessResult findSection(AxisID axisID,
    final ProcessResultDisplay processResultDisplay, ProcessSeries processSeries,
    final FindSectionParam param) {
    String threadName;
    try {
      threadName = processMgr.findSection(param, axisID, processSeries);
    }
    catch (AxisBusyException except) {
      except.printStackTrace();
      uiHarness.openMessageDialog(this, "Can't run findsection\n" + except.getMessage(),
        "SystemProcessException", axisID);
      return ProcessResult.FAILED_TO_START;
    }
    setThreadName(threadName, axisID);
    mainPanel.startProgressBar("Find Section", axisID);
    return null;
  }

  public ProcessResult cryoPosition(final AxisID axisID,
    final ProcessResultDisplay processResultDisplay, final ProcessSeries processSeries,
    final CryoPositionParam param) {
    sendMsgProcessStarting(processResultDisplay);
    String threadName;
    try {
      threadName =
        processMgr.cryoPosition(param, axisID, processResultDisplay, processSeries);
    }
    catch (AxisBusyException e) {
      e.printStackTrace();
      String[] message = new String[2];
      message[0] = "Can not execute cryoposition";
      message[1] = e.getMessage();
      uiHarness.openMessageDialog(this, message, "Unable to execute com script", axisID);
      return ProcessResult.FAILED_TO_START;
    }
    mainPanel.startProgressBar("Running cryoposition", axisID, ProcessName.CRYO_POSITION);
    setThreadName(threadName, axisID);
    return null;
  }

  public void renameCryoPositionOutput(final AxisID axisID) {
    File cryoPositionOutput = FileType.CRYO_POSITION_OUTPUT.getFile(this, axisID);
    File tiltOutput = FileType.TILT_OUTPUT.getFile(this, axisID);
    try {
      Utilities.renameFile(cryoPositionOutput, tiltOutput);
    }
    catch (IOException e) {
      e.printStackTrace();
      uiHarness.openMessageDialog(this, "Unable to rename " + cryoPositionOutput.getName()
        + " to " + tiltOutput.getName()
        + ".  In order to use the automated positioning functionality going forward, please perform this rename.",
        "Rename Failed", axisID);
    }
  }
   ...
   * 
   */
  public ProcessResult tomopitch(AxisID axisID, ProcessResultDisplay processResultDisplay,
    final ProcessSeries processSeries) {
    sendMsgProcessStarting(processResultDisplay);
    String threadName;
    try {
  ...
  public void tomogramPositioningPostProcess(AxisID axisID, ProcessDetails processDetails,
    ...
    ((TomogramPositioningExpert) getUIExpert(DialogType.TOMOGRAM_POSITIONING, axisID))
      .postProcess(processDetails, state, posSampleType);
  }
  ...
  public void makeRawtltFile(AxisID axisID)
    throws IOException, InvalidParameterException {
    File rawtlt = new File(propertyUserDir,
    ...
    try {
      ...
      MRCHeader rawStackHeader = MRCHeader.getInstance(this, axisID, FileType.RAW_STACK);
  ...
  public BlendmontParam updateBlend3dFindCom(BlendmontDisplay display, AxisID axisID,
    ...
    mrcTaperParam
      .setInputFile(FileType.NEWST_OR_BLEND_3D_FIND_OUTPUT.getFileName(this, axisID));
  ...
  public void blend3dFind(ProcessResultDisplay processResultDisplay,
    ...
    if (!UIExpertUtilities.INSTANCE.updateFiducialessParams(this,
      state.getStackImageRotation(axisID).toString(),
      state.isNewstFiducialessAlignment(axisID), axisID)) {
      sendMsgProcessFailedToStart(processResultDisplay);
      processSeries.endSeries();
      return;
    }
  ...
  public void setProcessState(final ProcessState processState, final AxisID axisID,
    final DialogType dialogType) {
  ...
  public void sendMsg(ProcessResult displayState,
  ...
  public ProcessResult sampleTilt(AxisID axisID,
    ...
    String threadName;
    try {
      threadName = processMgr.tilt(axisID, processResultDisplay, processSeries, tiltParam,
        null, null);
    }
    catch (AxisBusyException e) {
      e.printStackTrace();
  ...
  private void splittilt(ProcessResultDisplay processResultDisplay,
    ...
    ProcessResult processResult =
      splittilt(axisID, processResultDisplay, processSeries, splittiltParam, dialogType);
    if (processResult == null) {
      processSeries.setNextProcess(ProcessName.PROCESSCHUNKS.toString(), ProcessName.TILT,
        FileType.TILT_OUTPUT, tiltProcessingMethod);
    }
    sendMsg(processResult, processResultDisplay);
  }
  ...
  private void reprojectModel(ProcessResultDisplay processResultDisplay,
    ...
    if (!tilt3dFindReprojectCom.exists()) {
      UIHarness.INSTANCE.openMessageDialog(this,
        tilt3dFindReprojectCom.getName() + " does not exist.  Run "
          + FinalAlignedStackDialog.getTilt3dFindButtonLabel() + " before running "
  ...
  public ConstTiltParam updateTiltCom(TiltDisplay display, AxisID axisID,
    ...
    try {
      ...
      String outputFileName = FileType.TILT_OUTPUT.getFileName(this, axisID);
  ...
  public MultifiltSetupParam updateMultifiltSetupCom(final MultifiltSetupDisplay display,
    final AxisID axisID, final boolean doValidation) {
    // Set a reference to the correct object
    if (display == null) {
      UIHarness.INSTANCE.openMessageDialog(this,
        "Can not update " + FileType.MULTIFILT_SETUP_COMSCRIPT.getFileName(this, axisID)
          + " without an active tomogram generation dialog",
        "Program logic error", axisID);
      return null;
    }
    MultifiltSetupParam param = null;
    param = comScriptMgr.getMultifiltSetupParam(axisID);
    if (!display.getParameters(param, doValidation)) {
      return null;
    }
    comScriptMgr.saveMultifiltSetup(param, axisID);
    // Set the IMOD_OUTPUT_FORMAT environment variable in new image file name style
    // datasets only.
    if (!metaData.isOldImageFilenameStyle()) {
      comScriptMgr.loadPreblend(axisID);
      SetEnvParam setEnvParam =
        comScriptMgr.getSetEnvParamFromMultifiltSetup(axisID, ImodOutputFormat.ENV_VAR);
      if (setEnvParam == null) {
        // Keep the existing setting if it was set. If not then add it.
        setEnvParam = new SetEnvParam(ImodOutputFormat.ENV_VAR);
        setEnvParam.setValue(metaData.getImageOutputFormat().toString());
        comScriptMgr.saveMultifiltSetup(setEnvParam, axisID, ImodOutputFormat.ENV_VAR);
      }
    }
    return param;
  }
  ...
  public void openTomogramCombinationDialog() {
    ...
    if (tomogramCombinationDialog == null) {
      ...
      if (!combineParams.isPatchBoundarySet()
        && !metaData.isNewBatchruntomoCombineSettings()
        && !state.isNewBatchruntomoCombineSettings()) {
        System.err.println("No NewBatchruntomoCombineSettings");
        init = true;
        ...
        if (matchMode == null || matchMode == MatchMode.B_TO_A) {
          recFileName = FileType.TILT_OUTPUT_DUAL.getFileName(this, AxisID.FIRST);
          // recFileName = metaData.getDatasetName() + "a.rec";
        }
        else {
          recFileName = FileType.TILT_OUTPUT_DUAL.getFileName(this, AxisID.SECOND);
      ...
      if (metaData.isNewBatchruntomoCombineSettings()
        || state.isNewBatchruntomoCombineSettings()) {
        ...
        metaData.resetBatchruntomoCombineSettings();
        state.resetBatchruntomoCombineSettings();
  ...
  public void imodClusteredElongatedModel(final AxisID axisID,
    ...
    try {
      ...
      uiHarness.openMessageDialog(this, except.getMessage(),
        "Can't open 3dmod on " + fileType.getFileName(this, axisID) + " for " + model,
        axisID);
    }
    catch (AxisTypeException except) {
      except.printStackTrace();
   ...
   */
  public void
    saveTomogramCombinationDialog(TomogramCombinationDialog tomogramCombinationDialog) {
  ...
  private boolean updateCombineParams(final boolean doValidation, final boolean silent) {
    ...
    try {
      ...
      try {
        ...
        uiHarness.openMessageDialog(this, except.getMessage(), "IO Error: " + recFileName,
          AxisID.ONLY);
        // Delete the dialog
        tomogramCombinationDialog = null;
        return false;
  ...
  public boolean updateSolvematchCom(final boolean doValidation) {
    ...
    try {
      ...
      errorMessage[1] = except.getMessage();
      uiHarness.openMessageDialog(this, errorMessage, "Solvematch Parameter Syntax Error",
        AxisID.ONLY);
      return false;
    }
    return true;
  ...
  private void loadVolcombine() {
    ...
    tomogramCombinationDialog
      .enableReductionFactor(setParam != null && setParam.isValid());
    ...
    tomogramCombinationDialog
      .enableLowFromBothRadius(setParam != null && setParam.isValid());
  }
  ...
  private boolean updateVolcombineCom(final boolean doValidation) {
    ...
    try {
      ...
      errorMessage[1] = except.getMessage();
      uiHarness.openMessageDialog(this, errorMessage, "Volcombine Parameter Syntax Error",
        AxisID.ONLY);
      return false;
    }
    return true;
  ...
  private CombineComscriptState getCombineComscript() {
    ...
    if (combineComscriptState == null) {
      try {
        ComScriptUtil.useTemplate(this, CombineComscriptState.COMSCRIPT_NAME,
   ...
   * @return boolean
   */
  private CombineComscriptState
    updateCombineComscriptState(CombineProcessType startCommand) {
    if (tomogramCombinationDialog == null) {
      uiHarness.openMessageDialog(this,
    ...
    if (startCommand == CombineProcessType.VOLCOMBINE
      ...
      if (tomogramCombinationDialog.getRunProcessingMethod().isLocal()) {
        ...
        combineComscriptState.setEndCommand(CombineProcessType
          .getInstance(CombineProcessType.VOLCOMBINE_INDEX - 1, false).getIndex(),
          comScriptMgr);
      }
    }
    else {
  ...
  private boolean updateMatchorwarpCom(boolean trialMode, final boolean doValidation) {
    ...
    try {
      ...
      if (!tomogramCombinationDialog.getMatchorwarpParams(matchorwarpParam,
        doValidation)) {
        return false;
      }
  ...
  private boolean checkCPUsIfParallelProcessSet(final boolean parallelProcess,
    final boolean noVolCombine) {
    try {
      if (parallelProcess && !noVolCombine
        && mainPanel.getCPUsSelectedInt(AxisID.ONLY, false) <= 0) {
        uiHarness.openMessageDialog(this, "No CPU's selected",
          "Parallel Processing Failed", AxisID.ONLY);
        mainPanel.stopProgressBar(AxisID.ONLY, ProcessEndState.FAILED);
        return false;
      }
    }
  ...
  public void combine(ProcessResultDisplay processResultDisplay,
    ...
    final boolean initialVolumeMatching, final boolean parallelProcess,
    final boolean noVolCombine) {
    sendMsgProcessStarting(processResultDisplay);
    if (processSeries == null) {
      processSeries = new ProcessSeries(this, AxisID.FIRST, dialogType);
    }
    if (!checkCPUsIfParallelProcessSet(parallelProcess, noVolCombine)) {
      return;
    }
  ...
  public void matchvol1Combine(ProcessResultDisplay processResultDisplay,
    ProcessSeries processSeries, Deferred3dmodButton deferred3dmodButton,
    Run3dmodMenuOptions run3dmodMenuOptions, final DialogType dialogType,
    final ProcessingMethod volcombineProcessingMethod, final boolean parallelProcess,
    final boolean noVolCombine) {
    if (processSeries == null) {
      processSeries = new ProcessSeries(this, AxisID.ONLY, dialogType);
    }
    if (!checkCPUsIfParallelProcessSet(parallelProcess, noVolCombine)) {
      return;
    }
  ...
  public void openPostProcessingDialog() {
    ...
    boolean createDialog = postProcessingDialog == null;
    TrimvolInputFileState trimvolInputFileState = null;
    if (createDialog) {
      ...
      try {
        trimvolInputFileState = TrimvolInputFileState.getPostProcessingInstance(this,
          AxisID.ONLY,
          TrimvolParam.getInputFileName(this, metaData.getAxisType(), metaData.getName()),
          state);
      }
      catch (InvalidParameterException except) {
        String[] detailedMessage = new String[4];
        ...
        detailedMessage[3] = except.getMessage();
        uiHarness.openMessageDialog(this,
          detailedMessage + "\n" + "Invalid parameter: " + TrimvolParam
            .getInputFileName(this, metaData.getAxisType(), metaData.getName()),
          "Invalid Parameter", AxisID.ONLY);
        // Delete the dialog
        ...
        uiHarness.openMessageDialog(this, except.getMessage(), "IO Error: " + TrimvolParam
          .getInputFileName(this, metaData.getAxisType(), metaData.getName()),
          AxisID.ONLY);
        // Delete the dialog
    ...
    if (createDialog && postProcessingDialog != null) {
      ...
      if (!dialogExists || restoreToDefaults) {
        ...
        postProcessingDialog.initParameters(trimvolParam);
      }
    ...
    if (comScriptMgr.isWarpVolParamInFlatten(AxisID.ONLY)) {
      postProcessingDialog
        .setParameters(comScriptMgr.getWarpVolParamFromFlatten(AxisID.ONLY));
    }
    ...
    // dialog is set up.
    metaData
      .movePostTrimvolBatchruntomoSettings(postProcessingDialog.getTrimvolDisplay());
  ...
  public void imodCombinedTomogram(Run3dmodMenuOptions menuOptions) {
    try {
      imodManager.open(ImodManager.COMBINED_TOMOGRAM_KEY, AxisID.ONLY, menuOptions);
    }
    catch (SystemProcessException except) {
      except.printStackTrace();
      uiHarness.openMessageDialog(this, except.getMessage(),
  ...
  public void imodTrimmedVolume(Run3dmodMenuOptions menuOptions, AxisID axisID) {
    ...
    try {
      ...
      imodManager.setStartNewContoursAtNewZ(ImodManager.TRIMMED_VOLUME_KEY, axisID,
        false);
  ...
  public void imodMakeSurfaceModel(Run3dmodMenuOptions menuOptions, AxisID axisID,
    ...
    try {
      ...
      uiHarness.openMessageDialog(this,
        except.getMessage() + "\nCan't open 3dmod on the " + key, "Cannot Open 3dmod",
        axisID);
    }
    catch (AxisTypeException except) {
      except.printStackTrace();
   ...
   * @param axisID
   */
  public void useRunraptorResult(ProcessResultDisplay processResultDisplay, AxisID axisID,
    DialogType dialogType) {
    sendMsgProcessStarting(processResultDisplay);
    if (isAxisBusy(axisID, processResultDisplay)) {
      return;
    ...
    try {
      ...
      e.printStackTrace();
      uiHarness.openMessageDialog(this, "Unable to rename " + raptorDatasetFile.getName(),
        "File Error");
      sendMsgProcessFailedToStart(processResultDisplay);
      return;
    }
  ...
  private TrimvolParam updateTrimvolParam() {
    ...
    param.setOutputFileName(FileType.TRIM_VOL_OUTPUT.getFileName(this,
      AxisID.FIRST)/*was: metaData.getDatasetName() + ".rec"*/);
    if (metaData.getAxisType() == AxisType.SINGLE_AXIS
  ...
  boolean startNextProcess(final UIComponent uiComponent, AxisID axisID,
    ...
    if (uiExpert != null && uiExpert.startNextProcess(process, processResultDisplay,
      processSeries, dialogType, display)) {
      return true;
    }
    ...
    if (process.equals(ProcessName.XCORR.toString())) {
      tiltxcorr(axisID, processResultDisplay, null, null, processSeries, dialogType,
        ...
        ProcessName.XCORR/*was: FileType.CROSS_CORRELATION_COMSCRIPT*/, true, false);
      return true;
    }
    ...
    if (process.equals(Task.RELOAD_ALIGN_COM)) {
      reloadAlignCom(axisID, processSeries);
      return true;
    }
    if (process.equals(Task.COPY_TOMO_COMS)) {
      copyTomoComs(processSeries);
      return true;
    }
    if (process.equals(Task.SETUP_RECON_FAILED)) {
      msgSetupReconFailed(processSeries);
      return true;
    }
    if (process.equals(Task.EXCLUDE_VIEWS_A)) {
      excludeViews(AxisID.FIRST, processSeries, null, process.getAxisType(), null,
        process.getCommand());
      return true;
    }
    if (process.equals(Task.EXCLUDE_VIEWS_B)) {
      excludeViews(AxisID.SECOND, processSeries, null, process.getAxisType(), null,
        process.getCommand());
      return true;
    }
    if (process.equals(Task.VALIDATE_DIRECTIVE_FILES)) {
      validateDirectiveFiles(processSeries, axisID);
      return true;
    }
    if (process.equals(Task.SET_RAW_STACK_EXTENSION)) {
      setRawImageStackExtension(processSeries, axisID);
      return true;
    }
    if (process.equals(Task.SET_ORIG_RAW_STACK_EXTENSION)) {
      setOrigRawImageStackExtension(processSeries, axisID);
      return true;
    }
    if (process.equals(Task.RENAME_INPUT_IMAGE_FILES)) {
      renameInputImageFile(processSeries, axisID);
      return true;
    }
    return false;
  }
  ...
  private void updateDialog(FiducialModelDialog dialog, AxisID axisID) {
    ...
    boolean prealisExist = FileType.PREALIGNED_STACK.exists(this, AxisID.FIRST)
      && FileType.PREALIGNED_STACK.exists(this, AxisID.SECOND);
  ...
  void checkUpdateFiducialModel(AxisID axisID, ProcessResultDisplay processResultDisplay,
    ...
    MRCHeader prealiHeader =
      MRCHeader.getInstance(this, axisID, FileType.PREALIGNED_STACK);
    MRCHeader rawstackHeader = MRCHeader.getInstance(this, axisID, FileType.RAW_STACK);
    try {
      fidXyz.read();
    ...
    if (!fidXyzPixelSizeSet) {
      if (Math.round(
        prealiHeader.getXPixelSpacing() / rawstackHeader.getXPixelSpacing()) == 1) {
    ...
    if (!fidXyzPixelSizeSet || fidXyz.getPixelSize() != prealiHeader.getXPixelSpacing()) {
      ...
      String[] message =
        new String[] { "The prealigned image stack binning has changed.  You " + "must:",
  ...
  public SirtsetupParam updateSirtSetupCom(final AxisID axisID,
    final SirtsetupDisplay display, final boolean doValidation) {
    ...
    SirtsetupParam param = comScriptMgr.getSirtsetupParam(axisID);
    if (!display.getParameters(param, doValidation)) {
      return null;
    }
  ...
  public void useTrackAdjustedComfile(final AxisID axisID,
    final ProcessResultDisplay processResultDisplay) {
    mainPanel.startProgressBar("Using the adjusted " + ProcessName.TRACK + " comfile",
      axisID);
    if (!FileType.TRACK_ADJUSTED_COMSCRIPT.exists(this, axisID)) {
      // Nothing to do.
      mainPanel.stopProgressBar(axisID, ProcessEndState.CANCELLED,
        FileType.TRACK_ADJUSTED_COMSCRIPT.getFileName(this, axisID)
          + " is not available");
      return;
    }
    sendMsgProcessStarting(processResultDisplay);
    if (FileType.TRACK_COMSCRIPT.exists(this, axisID)) {
      try {
        Utilities.renameFile(FileType.TRACK_COMSCRIPT.getFile(this, axisID),
          FileType.TRACK_ORIG_COMSCRIPT.getFile(this, axisID), true);
      }
      catch (IOException e) {
        uiHarness.openMessageDialog(this,
          "Unable to back up " + FileType.TRACK_COMSCRIPT.getFileName(this, axisID)
            + " to " + FileType.TRACK_ORIG_COMSCRIPT.getFileName(this, axisID) + ".",
          "Unable to Backup Comscript", axisID);
        sendMsg(ProcessResult.FAILED_TO_START, processResultDisplay);
        mainPanel.stopProgressBar(axisID, ProcessEndState.FAILED);
        return;
      }
    ...
    try {
      Utilities.renameFile(FileType.TRACK_ADJUSTED_COMSCRIPT.getFile(this, axisID),
        FileType.TRACK_COMSCRIPT.getFile(this, axisID), true);
    }
    catch (IOException e) {
      uiHarness.openMessageDialog(this,
        "Unable to copy " + FileType.TRACK_ADJUSTED_COMSCRIPT.getFileName(this, axisID)
          + " to " + FileType.TRACK_COMSCRIPT.getFileName(this, axisID) + ".",
        "Unable to Copy Comscript", axisID);
      sendMsg(ProcessResult.FAILED_TO_START, processResultDisplay);
      mainPanel.stopProgressBar(axisID, ProcessEndState.FAILED);
      return;
    }
  ...
  public void multifiltSetup(final AxisID axisID,
    final ProcessResultDisplay processResultDisplay, ProcessSeries processSeries,
    final DialogType dialogType, final ProcessingMethod processingMethod,
    final MultifiltSetupDisplay display) {
    sendMsgProcessStarting(processResultDisplay);
    if (updateTiltCom(
      ((TomogramGenerationExpert) getUIExpert(dialogType, axisID)).getTiltDisplay(),
      axisID, true) == null) {
    ...
    MultifiltSetupParam param = updateMultifiltSetupCom(display, axisID, true);
    if (param == null) {
      sendMsg(ProcessResult.FAILED_TO_START, processResultDisplay);
      processSeries.endSeries();
      return;
    ...
    String threadName;
    try {
      threadName = processMgr.multifiltSetup(axisID, processResultDisplay, processSeries,
        param, processingMethod);
    }
    catch (AxisBusyException e) {
      e.printStackTrace();
      String[] message = new String[2];
      message[0] = "Can not execute " + ProcessName.MULTIFILT_SETUP;
      message[1] = e.getMessage();
      uiHarness.openMessageDialog(this, message, "Unable to execute command", axisID);
      sendMsg(ProcessResult.FAILED_TO_START, processResultDisplay);
      return;
    ...
    processSeries.setNextProcess(ProcessName.PROCESSCHUNKS.toString(),
      ProcessName.TILT_MULTIFILT, processingMethod);
    setThreadName(threadName, axisID);
    mainPanel.startProgressBar("Running " + ProcessName.MULTIFILT_SETUP, axisID,
  ...
  public void resume(final AxisID axisID, final ProcesschunksParam param,
    ...
    if (rootName.indexOf("sirt") != -1) {
      if (processSeries == null) {
        processSeries = new ProcessSeries(this, axisID, DialogType.TOMOGRAM_GENERATION);
        processSeries.setLastProcess(TomogramGenerationExpert.SIRT_DONE);
      }
    }
   ...
   * @return true if succeeded
   */
  public boolean useImageFile(final ProcessResultDisplay processResultDisplay,
    ...
    if (outputFileType.getFile(this, axisID).exists() && useFile.exists()) {
      if (!Utilities.isValidStack(useFile, this, axisID)) {
        uiHarness.openMessageDialog(this, useFile.getName() + " is not a valid MRC file.",
          "Entry Error", axisID);
        mainPanel.stopProgressBar(axisID);
        sendMsgProcessFailedToStart(processResultDisplay);
        return false;
    ...
    try {
      ...
      UIHarness.INSTANCE.openMessageDialog(this, except.getMessage(), "File Rename Error",
        axisID);
      mainPanel.stopProgressBar(axisID);
      sendMsg(ProcessResult.FAILED, processResultDisplay);
      return false;
  ...
  public void splitcombine(ProcessSeries processSeries,
    ...
    final boolean parallelProcess, final boolean noVolCombine) {
    // Check for CPU's selected if parallel processing is enabled
    if (!checkCPUsIfParallelProcessSet(parallelProcess, noVolCombine)) {
      return;
    }
    splitcombine(processSeries, deferred3dmodButton, run3dmodMenuOptions, dialogType,
      volcombineProcessingMethod);
  }

  private void splitcombine(ProcessSeries processSeries,
    Deferred3dmodButton deferred3dmodButton, Run3dmodMenuOptions run3dmodMenuOptions,
    final DialogType dialogType, final ProcessingMethod volcombineProcessingMethod) {
    if (processSeries == null) {
      processSeries = new ProcessSeries(this, AxisID.FIRST, dialogType);
  ...
  private void processchunksVolcombine(ProcessResultDisplay processResultDisplay,
    ...
    processchunks(AxisID.ONLY, DialogType.TOMOGRAM_COMBINATION, tomogramCombinationDialog,
      processResultDisplay, processSeries, ProcessName.VOLCOMBINE,
      FileType.COMBINED_VOLUME, processingMethod);
  }
  ...
  public static final class Task implements TaskInterface {
    private static final Task RELOAD_ALIGN_COM = new Task("RELOAD_ALIGN_COM");
    private static final Task COPY_TOMO_COMS = new Task("COPY_TOMO_COMS");
    private static final Task EXCLUDE_VIEWS_A = new Task("EXCLUDE_VIEWS_A");
    private static final Task EXCLUDE_VIEWS_B = new Task("EXCLUDE_VIEWS_B");
    private static final Task SETUP_RECON_FAILED = new Task("SETUP_RECON_FAILED");
    private static final Task VALIDATE_DIRECTIVE_FILES =
      new Task("VALIDATE_DIRECTIVE_FILES");
    private static final Task SET_RAW_STACK_EXTENSION =
      new Task("SET_RAW_STACK_EXTENSION");
    private static final Task SET_ORIG_RAW_STACK_EXTENSION =
      new Task("SET_ORIG_RAW_STACK_EXTENSION");
    private static final Task RENAME_INPUT_IMAGE_FILES =
      new Task("RENAME_INPUT_IMAGE_FILES");

    private final boolean droppable;
    private final String descr;

    private Task(final String descr) {
      this(descr, false);
    }

    private Task(final String descr, final boolean droppable) {
