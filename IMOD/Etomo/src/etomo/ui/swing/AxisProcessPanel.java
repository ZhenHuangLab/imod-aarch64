...
public abstract class AxisProcessPanel implements ContextMenu {
  ...
  private final boolean altParallelLoc;
  /**
  ...
  private final AxisProgressPanel axisProgressPanel;
  ...
  AxisProcessPanel(AxisID axisID, BaseManager manager, boolean popupChunkWarnings,
    ...
    final boolean altParallelLoc, final AxisProgressPanel axisProgressPanel) {
    this.axisID = axisID;
    this.manager = manager;
    this.popupChunkWarnings = popupChunkWarnings;
    this.runnableParallel = runnableParallel;
    this.interfaceType = interfaceType;
    this.altParallelLoc = altParallelLoc;
    this.axisProgressPanel = axisProgressPanel;
    // Create the status panel
    outerStatusPanel.setLayout(new BoxLayout(outerStatusPanel, BoxLayout.Y_AXIS));
    outerStatusPanel.add(axisProgressPanel.getComponent());
  ...
  void setBackground(final Color color) {
    ...
    axisProgressPanel.setBackground(color);
  }
  ...
  int getCPUsSelectedInt(final boolean doValidation)
    ...
    if (parallelPanel != null) {
      return parallelPanel.getCPUsSelectedInt(doValidation);
    }
  ...
  private void showParallelPanel(final boolean show, final boolean force) {
    ...
    if (!show) {
      ...
      if (parallelPanel == null) {
        parallelPanel = ParallelPanel.getInstance(manager, axisID,
          manager.getBaseScreenState(axisID).getParallelHeaderState(), this,
          popupChunkWarnings, runnableParallel, interfaceType);
  ...
  final JPanel getParallelStatusPanel() {
    return parallelStatusPanel;
  }

  final void done() {
    if (parallelPanel != null) {
      parallelPanel
        .getHeaderState(manager.getBaseScreenState(axisID).getParallelHeaderState());
    }
  }
   ...
   * @param enablePause
   */
  final void setPauseEnabled(final boolean enablePause) {
    if (parallelPanel != null) {
