...
final class EtomoMenu {
  ...
  private final JMenuItem menuTomosnapshot =
    new MenuItem("Run Tomosnapshot", KeyEvent.VK_T);
  private final JMenuItem[] menuMRUList = new MenuItem[nMRUFileMax];
  private final JMenuItem menuExportBatch =
    new MenuItem("Export Batch Directive File", KeyEvent.VK_B);
  ...
  private final JMenuItem menuSerialSections =
    new MenuItem(SERIAL_SECTIONS_LABEL, KeyEvent.VK_R);
  private final JMenuItem menuNewAnisotropicDiffusion =
    new MenuItem(NAD_LABEL, KeyEvent.VK_D);
  private final JMenuItem menuNewBatchRunTomo =
    new MenuItem(BATCH_RUN_TOMO_LABEL, KeyEvent.VK_H);
  private final JMenuItem menuNewGenericParallel =
    new MenuItem(GENERIC_LABEL, KeyEvent.VK_G);

  private final JMenuItem menuSaveScope =
    new MenuItem("Save Scope Template", KeyEvent.VK_C);
  private final JMenuItem menuSaveSystem =
    new MenuItem("Save System Template", KeyEvent.VK_Y);
  ...
  private final JMenuItem menuFlattenVolume =
    new MenuItem(FLATTEN_VOLUME_LABEL, KeyEvent.VK_F);
  private final JMenuItem menuGpuTiltTest =
    new MenuItem(GPU_TILT_TEST_LABEL, KeyEvent.VK_U);

  private final JMenu menuView = new Menu("View");
  private final JMenuItem menuLogWindow =
    new MenuItem("Show/Hide Log Window", KeyEvent.VK_L);
  private final JMenuItem menuAxisA = new MenuItem("Axis A", KeyEvent.VK_A);
  private final JMenuItem menuAxisB =
    new MenuItem(TomogramProcessPanel.AXIS_B_LABEL, KeyEvent.VK_B);
  ...
  private final JCheckBoxMenuItem menu3dmodStartupWindow =
    new CheckBoxMenuItem("Open 3dmod with Startup Window");
  private final JCheckBoxMenuItem menu3dmodBinBy2 =
    new CheckBoxMenuItem("Open 3dmod Binned by 2");
  ...
  private final JMenuItem menu3dmodGuide =
    new MenuItem("3dmod Users Guide", KeyEvent.VK_3);
  private final JMenuItem menuEtomoGuide =
    new MenuItem("Etomo Users Guide", KeyEvent.VK_E);
  ...
  private final JMenuItem menuBatchGuide =
    new MenuItem("Batch Interface Guide", KeyEvent.VK_B);
  ...
  private final boolean peetAvailable =
    EnvironmentVariable.INSTANCE.exists(null, EtomoDirector.INSTANCE.getOriginalUserDir(),
      EnvironmentVariable.PARTICLE_DIR, AxisID.ONLY);
  ...
  void menuToolsAction(AxisID axisID, ActionEvent event) {
    if (equalsFlattenVolume(event)) {
      EtomoDirector.INSTANCE.openTool(ToolType.FLATTEN_VOLUME);
    }
    else if (equalsGpuTiltTest(event)) {
      EtomoDirector.INSTANCE.openTool(ToolType.GPU_TILT_TEST);
    }
  }

  boolean menuFileAction(ActionEvent event) {
   ...
   * @param event
   */
  void menuHelpAction(final BaseManager manager, final AxisID axisID, final JFrame frame,
    ...
    if (equalsPeetHelp(event)) {
      BaseProcessManager.startSystemProgramThread(new String[] {
        new File(new File(new File(EnvironmentVariable.INSTANCE.getValue(null, null,
          EnvironmentVariable.PARTICLE_DIR, null)), "bin"), "PEETHelp")
            .getAbsolutePath() },
        axisID, manager);
    }
  ...
  public boolean equals(final JMenuItem menuItem, final ActionEvent event) {
