...
final class BoundaryTable implements Viewable {
  ...
  BoundaryTable(final JoinManager manager, final JoinDialog joinDialog) {
    ...
    parent = joinDialog;
    focusableParents = new JComponent[] { joinDialog.getModelTabJComponent(),
      joinDialog.getRejoinTabJComponent() };
    // construct panels
  ...
  private void setToolTipText() {
    ...
    text =
      "Describes how the final start and end values will change when the join is recreated, "
    ...
    header1Error.setToolTipText(
      "Deviations between transformed points extrapolated from above and below the corresponding boundary.");
  ...
  private static final class RowList {
    ...
    private void add(final int size, final ConstJoinMetaData metaData,
      final JoinScreenState screenState, final JPanel panel, final GridBagLayout layout,
      final GridBagConstraints constraints, final Viewport viewport) {
