...
public final class ContextPopup {
  ...
  private final JMenuItem serialSectionsGuideItem =
    new MenuItem("Serial Section Guide ...");
   ...
   */
  ContextPopup(final Component component, final MouseEvent mouseEvent,
  ...
  public ContextPopup(final Component component, final MouseEvent mouseEvent,
    ...
    this.mouseEvent = mouseEvent;
    validate(manPageLabel, manPage);
    if (logFileLabel != null && logFileLabel.length != logFile.length) {
      ...
      public void actionPerformed(final ActionEvent actionEvent) {
        ...
        for (int i = 0; i < logFileItem.length; i++) {
          if (actionEvent.getActionCommand() == logFileItem[i].getText()) {
            ...
            logFileWindow.setVisible(logFileWindow.setFile(
              manager.getPropertyUserDir() + File.separator + getLogFileName()[i]));
          }
        }
  ...
  */
  ContextPopup(final Component component, final MouseEvent mouseEvent,
    ...
    actionListener = new ActionListener() {
      ...
      public void actionPerformed(final ActionEvent actionEvent) {
        ...
        for (int i = 0; i < getManPageItem().length; i++) {
          if (actionEvent.getActionCommand() == getManPageItem()[i].getText()) {
            ImodqtassistProcess.INSTANCE.open(applicationManager,
              "man/" + getManPageName()[i], axisID);
          }
        }
  ...
  private void globalItemAction(final ActionEvent actionEvent, String guideLocation,
    ...
    if (actionEvent.getActionCommand() == it3dmodGuide.getText()) {
      ...
      ImodqtassistProcess.INSTANCE.open(manager, "3dmodguide.html" + Constants.TOP_ANCHOR,
        axisID);
    }
    ...
    if (actionEvent.getActionCommand() == etomoGuideItem.getText()) {
      ...
      ImodqtassistProcess.INSTANCE.open(manager, "UsingEtomo.html" + Constants.TOP_ANCHOR,
        axisID);
    }
    ...
    if (actionEvent.getActionCommand() == batchGuideItem.getText()) {
      if (guide != null && guide.equals(BATCHRUNTOMO_GUIDE)) {
        ...
        ImodqtassistProcess.INSTANCE.open(manager,
          BATCHRUNTOMO_GUIDE + Constants.TOP_ANCHOR, axisID);
      }
    }
    if (actionEvent.getActionCommand() == peetGuideItem.getText()) {
      ImodqtassistProcess.INSTANCE.open(manager, "PEETmanual.html" + Constants.TOP_ANCHOR,
        axisID);
    }
