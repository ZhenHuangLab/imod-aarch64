...
final class CleanupPanel {
  ...
  private CleanupPanel(final ApplicationManager appMgr) {
    ...
    ImageFilenameStyle imageFilenameStyle = metaData.getImageFilenameStyle();
    IntermediateFileFilter intermediateFileFilter =
      IntermediateFileFilter.getInstance(appMgr, datasetName);
    File trimmedTomogram =
      new File(applicationManager.getPropertyUserDir(), FileType.TRIM_VOL_OUTPUT
        .getFileName(applicationManager, AxisID.ONLY) /*was: datasetName + ".rec"*/);
    if (trimmedTomogram.exists()) {
      intermediateFileFilter.setAcceptPretrimmedTomograms();
    }
    fileFilterCollection.addFileFilter(intermediateFileFilter);
    if (appMgr.getMetaData().getAxisType() == AxisType.DUAL_AXIS) {
      fileFilterCollection.addFileFilter(SirtOutputFileFilter.getInstance(appMgr,
        imageFilenameStyle, AxisID.FIRST, true, true, true));
      fileFilterCollection.addFileFilter(SirtOutputFileFilter.getInstance(appMgr,
        imageFilenameStyle, AxisID.SECOND, true, true, true));
    }
    else {
      fileFilterCollection.addFileFilter(SirtOutputFileFilter.getInstance(appMgr,
        imageFilenameStyle, AxisID.ONLY, true, true, true));
    }
  ...
  private void deleteSelected() {
    ...
    if (!deletedAll) {
      StringBuffer message =
        new StringBuffer("Unable to delete file(s).  " + "Check file permissions.");
