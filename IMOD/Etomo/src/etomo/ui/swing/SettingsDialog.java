...
public final class SettingsDialog extends JDialog {
  ...
  private final LabeledTextField ltfFontSize =
    new LabeledTextField(FieldType.INTEGER, "Size: ");
  private final LabeledTextField ltfTooltipsInitialDelay =
    new LabeledTextField(FieldType.FLOATING_POINT, "Tooltips initial delay: ");
  private final LabeledTextField ltfTooltipsDismissDelay =
    new LabeledTextField(FieldType.FLOATING_POINT, "Tooltips dismiss delay: ");
  ...
  private final CheckBox cbParallelProcessing =
    new CheckBox("Enable " + ParallelPanel.FIELD_LABEL);
  private final CheckBox cbGpuProcessing = new CheckBox("Enable graphics processing");
  private final LabeledTextField ltfCpus =
    new LabeledTextField(FieldType.INTEGER, "# of true CPU cores: ");
  private final CheckBox cbSingleAxis =
    new CheckBox(SetupDialog.AXIS_TYPE_LABEL + ":  " + SetupDialog.SINGLE_AXIS_LABEL);
  private final CheckBox cbMontage =
    new CheckBox(SetupDialog.FRAME_TYPE_LABEL + ":  " + SetupDialog.MONTAGE_LABEL);
  private final CheckBox cbNoParallelProcessing =
    new CheckBox("Start with " + ParallelPanel.FIELD_LABEL + " off");
  private final CheckBox cbGpuProcessingDefault =
    new CheckBox("Start with graphics card processing on");
  private final CheckBox cbTiltAnglesRawtltFile =
    new CheckBox("Angle Source:  " + TiltAnglePanel.EXISTING_RAWTILT_FILE);
  private final CheckBox cbSwapYAndZ = new CheckBox(
    TrimvolPanel.REORIENTATION_GROUP_LABEL + "  " + TrimvolPanel.SWAP_YZ_LABEL);
  private final LabeledTextField ltfParallelTableSize =
    new LabeledTextField(FieldType.INTEGER, "Parallel table size: ");
  private final LabeledTextField ltfJoinTableSize =
    new LabeledTextField(FieldType.INTEGER, "Join tables size: ");
  private final LabeledTextField ltfPeetTableSize =
    new LabeledTextField(FieldType.INTEGER, "PEET table size: ");
  private final LabeledTextField ltfBatchTableSize =
    new LabeledTextField(FieldType.INTEGER, "Batchruntomo table size: ");
  private final CheckBox cbSetFEIPixelSize =
    new CheckBox("Set pixel size in files from FEI");
  private final FileTextField2 ftfUserTemplateDir =
    FileTextField2.getInstance(null, "User templates directory: ");
  private final SettingsDialogListener listener = new SettingsDialogListener(this);
  private final LabeledTextField ltfSmtpServer =
    new LabeledTextField(FieldType.STRING, "Outgoing mail server: ");
  private final CheckBox cbRemoveExcludedViews =
    new CheckBox("Default to removing excluded views");
  ...
  private void buildDialog() {
    ...
    ftfUserTemplateDir.setUseTextAsFileChooserDir(true);
    ftfUserTemplateDir.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
    ...
    // Scroll bar
    JScrollPane scrollPane = new JScrollPane(pnlMain.getJPanel());
    setScrollBarIncrements(scrollPane.getVerticalScrollBar());
    setScrollBarIncrements(scrollPane.getHorizontalScrollBar());
    scrollPane.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
    scrollPane.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED);
    ((JPanel) getContentPane()).add(scrollPane);
    ...
    pnlEnhancedProcessing.setBorder(new EtchedBorder("Enhanced Processing").getBorder());
    pnlSettings.add(pnlEnhancedProcessing);
    JPanel pnlCheckBoxParallelProcessing = new JPanel();
    pnlCheckBoxParallelProcessing
      .setLayout(new BoxLayout(pnlCheckBoxParallelProcessing, BoxLayout.X_AXIS));
    ...
    panelDefaults.add(cbRemoveExcludedViews.getComponent());
    ...
    pnlTableSize.add(ltfBatchTableSize.getContainer());
  ...
  private void loadData(final BaseManager manager, final String propertyUserDir) {
    ...
    cbParallelProcessing.setEnabled(
      !Network.isParallelProcessingSetExternally(manager, AxisID.ONLY, propertyUserDir));
  ...
  private void updateDisplay() {
    ltfCpus
      .setEnabled(cbParallelProcessing.isEnabled() && cbParallelProcessing.isSelected());
  }
  ...
  public void setParameters(final UserConfiguration userConfig) {
    ...
    ltfBatchTableSize.setText(userConfig.getBatchTableSize());
  ...
  public void getParameters(final UserConfiguration userConfig) {
    ...
    userConfig.setRemoveExcludedViews(cbRemoveExcludedViews.isSelected());
    ...
    userConfig.setBatchTableSize(ltfBatchTableSize.getText());
  ...
  public boolean isAppearanceSettingChanged(final UserConfiguration userConfig) {
    ...
    if (userConfig.getNativeLookAndFeel() != cbNativeLAF.isSelected()
      ...
      || userConfig.isRemoveExcludedViews() != cbRemoveExcludedViews.isSelected()
      ...
      || !userConfig.getFontFamily()
        .equals(fontFamilies.getName(listFontFamily.getSelectedIndex()))
      ...
      || !userConfig.getBatchTableSize().toString().equals(ltfBatchTableSize.getText())
  ...
  void setTooltips() {
    cbSetFEIPixelSize.setToolTipText(TooltipFormatter.INSTANCE.format(
      "During tomogram setup, transfer pixel size from extended header to " + "pixel."));
  }
