...
final class TiltAnglePanelExpert {
  ...
  TiltAnglePanelExpert(final ApplicationManager manager, final AxisID axisID) {
    this.manager = manager;
    this.axisID = axisID;
    panel = new TiltAnglePanel(manager, this, axisID);
  }
  ...
  TiltAngleType getTiltAngleType() {
    if (panel.isExtractSelected()) {
      return TiltAngleType.EXTRACT;
    }
    if (panel.isSpecifySelected()) {
      return TiltAngleType.RANGE;
    }
    if (panel.isFileSelected()) {
      return TiltAngleType.FILE;
    }
    return null;
  }
  ...
  boolean getFields(final TiltAngleSpec tiltAngleSpec, final boolean doValidation) {
    try {
      tiltAngleSpec.setType(getTiltAngleType());
