...
public final class PeetDialog implements ContextMenu, AbstractParallelDialog,
  Run3dmodButtonContainer, FileContainer, ReferenceParent, MissingWedgeCompensationParent,
  IterationParent, MaskingParent, YAxisTypeParent, SphericalSamplingForThetaAndPsiParent,
  ProcessInterface, UIComponent, SwingComponent {
  ...
  private final LabeledTextField ltfDirectory =
    new LabeledTextField(FieldType.STRING, DIRECTORY_LABEL + ": ");
  private final LabeledTextField ltfFnOutput =
    new LabeledTextField(FieldType.STRING, FN_OUTPUT_LABEL + ": ");
  ...
  private final CheckBox cbFlgStrictSearchLimits =
    new CheckBox(SharedStrings.FLG_STRICT_SEARCH_LIMITS_LABEL);
  private final CheckBox cbFlgNoReferenceRefinement =
    new CheckBox(SharedStrings.FLG_NO_REFERENCE_REFINEMENT_LABEL);
  private final LabeledTextField ltfLowCutoff =
    new LabeledTextField(FieldType.FLOATING_POINT, "Low frequency cutoff:", 5);
  private final LabeledTextField ltfLowCutoffSigma =
    new LabeledTextField(FieldType.FLOATING_POINT, "Sigma: ");
  private final CheckBox cbRefFlagAllTom = new CheckBox("For new references");
  private final LabeledTextField ltfLstThresholdsStart =
    new LabeledTextField(FieldType.INTEGER, LST_THRESHOLD_START_TITLE + ": ");
  private final LabeledTextField ltfLstThresholdsIncrement =
    new LabeledTextField(FieldType.INTEGER, LST_THRESHOLD_INCREMENT_TITLE + ": ");
  private final LabeledTextField ltfLstThresholdsEnd =
    new LabeledTextField(FieldType.INTEGER, LST_THRESHOLD_END_TITLE + ": ");
  ...
  private final SingleLineButton btnRun = new SingleLineButton(RUN_LABEL);
  ...
  private final RadioButton rbInitMotlZero =
    new RadioButton(MatlabParam.InitMotlCode.ZERO, bgInitMotl);
  private final RadioButton rbInitAlignParticleYAxes =
    new RadioButton(MatlabParam.InitMotlCode.X_AND_Z_AXIS, bgInitMotl);
  private final RadioButton rbInitMotlRandomRotations =
    new RadioButton(MatlabParam.InitMotlCode.RANDOM_ROTATIONS, bgInitMotl);
  private final RadioButton rbInitMotlRandomAxialRotations =
    new RadioButton(MatlabParam.InitMotlCode.RANDOM_AXIAL_ROTATIONS, bgInitMotl);
  private final RadioButton rbInitMotlFiles =
    new RadioButton(SharedStrings.CSV_FILES_LABEL, bgInitMotl);
  ...
  private final Run3dmodSingleLineButton btnAvgVol =
    Run3dmodSingleLineButton.get3dmodInstance("Open averages in 3dmod", this);
  ...
  private final Run3dmodSingleLineButton btnRef =
    Run3dmodSingleLineButton.get3dmodInstance("Open references in 3dmod", this);
  private final SingleLineButton btnAverageAll = new SingleLineButton(AVERAGE_ALL_LABEL);
  private final CheckBox cbflgAlignAverages =
    new CheckBox(SharedStrings.FLG_ALIGN_AVERAGES_LABEL);
  private final CheckBox cbFlgAbsValue = new CheckBox(SharedStrings.FLG_ABS_VALUE_LABEL);
  private final LabeledTextField ltfSelectClassID =
    new LabeledTextField(FieldType.INTEGER, "Average only members of classes: ");
  ...
  */
   ...
   * @param paramFileSet
   */
  public void updateMode(final boolean paramFileSet) {
   ...
   */
  public boolean fixIncorrectPath(FileTextFieldInterface fileTextField,
    boolean choosePath) {
    File newFile = null;
  ...
  JFileChooser getFileChooserInstance() {
    return new FileChooser(manager, lastLocation, manager);
  }
  ...
  public void setParameters(final MatlabParam matlabParam, File importDir) {
    ...
    if (lsParticlePerCPU.isInRange(number)) {
      ...
      UIHarness.INSTANCE.openProblemValueMessageDialog(manager, this, "Unknown",
        MatlabParam.PARTICLE_PER_CPU_KEY, null, PARTICLE_PER_CPU_LABEL, number.toString(),
        String.valueOf(MatlabParam.PARTICLE_PER_CPU_DEFAULT), null);
  ...
  public boolean getParameters(final MatlabParam matlabParam, final boolean forRun,
    ...
    try {
      ...
      matlabParam
        .setLstThresholdsIncrement(ltfLstThresholdsIncrement.getText(doValidation));
      matlabParam.setLstThresholdsEnd(ltfLstThresholdsEnd.getText(doValidation));
      matlabParam
        .setLstThresholdsAdditional(ltfLstThresholdsAdditional.getText(doValidation));
      ...
      if (!sphericalSamplingForThetaAndPsiPanel.getParameters(matlabParam,
        doValidation)) {
        return false;
      }
  ...
  void msgVolumeTableSizeChanged(final boolean init) {
    updateDisplay(init);
  }
  ...
  private void setTooltipText() {
    ...
    pnlInitMotl.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, MatlabParam.InitMotlCode.KEY, false));
    ...
    cbflgAlignAverages.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, MatlabParam.FLG_ALIGN_AVERAGES_KEY, false));
    ...
    cbFlgRandomize.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, MatlabParam.FLG_RANDOMIZE_KEY, false));
    ...
    cbAlignedBaseName
      .setToolTipText("Save individual aligned particles to " + "files aligned*.mrc.");
    ...
    ltfLowCutoffSigma.setToolTipText("An optional parameter which defines the transition "
      + "width of the low frequency filter.");
    lsDebugLevel.setToolTipText(
  ...
  private void createSetupPanel() {
    ...
    pnlReferenceAndMissingWedgeCompensation.setLayout(
      new BoxLayout(pnlReferenceAndMissingWedgeCompensation, BoxLayout.X_AXIS));
    pnlReferenceAndMissingWedgeCompensation.add(referencePanel.getComponent());
    pnlReferenceAndMissingWedgeCompensation
      .add(missingWedgeCompensationPanel.getComponent());
    // init MOTL and Y axis type
    pnlInitMotlAndYAxisType
      .setLayout(new BoxLayout(pnlInitMotlAndYAxisType, BoxLayout.X_AXIS));
  ...
  private void createRunPanel() {
    ...
    pnlEqualNumber.setBorder(
      new EtchedBorder("Use Equal Numbers of Particles from All Tomograms").getBorder());
  ...
  void msgFlgVolNamesAreTemplates(final boolean init, final boolean on) {
    referencePanel.msgFlgVolNamesAreTemplates(init, on);
  }
  ...
  private boolean validateRun() {
    ...
    if (ltfDirectory.isEmpty()) {
      ...
      UIHarness.INSTANCE.openMessageDialog(manager,
        "Please set the " + PeetDialog.DIRECTORY_LABEL + " field.", "Entry Error");
      return false;
    }
    ...
    if (ltfFnOutput.isEmpty()) {
      gotoSetupTab();
      UIHarness.INSTANCE.openMessageDialog(manager,
        "Please set the " + PeetDialog.FN_OUTPUT_LABEL + " field.", "Entry Error");
      return false;
    }
    ...
    if (startIsEmpty && incrementIsEmpty) {
      ...
      if (additionalIsEmpty) {
        UIHarness.INSTANCE.openMessageDialog(manager,
          LST_THRESHOLDS_LABEL + " is required.", "Entry Error");
        return false;
      }
      // check empty list descriptor
      if (!endIsEmpty) {
        UIHarness.INSTANCE.openMessageDialog(manager,
          "In " + LST_THRESHOLDS_LABEL + ", invalid list description.", "Entry Error");
        return false;
      }
    }
   ...
   */
  public void updateDisplay(final boolean init) {
    // tilt range
    boolean volumeRows = volumeTable.size() > 0;
    referencePanel.updateDisplay(init);
    ...
    iterationTable
      .updateDisplay(!sphericalSamplingForThetaAndPsiPanel.isSampleSphereNoneSelected());
    // volume table
