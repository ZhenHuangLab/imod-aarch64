...
import etomo.comscript.Joinwarp2modelParam;
...
public final class JoinDialog
  ...
  private final LabeledTextField ltfRootName =
    new LabeledTextField(FieldType.STRING, "Root name for output file: ");;
  ...
  private LabeledTextField ltfMidasLimit =
    new LabeledTextField(FieldType.INTEGER, "Squeeze samples to ");
  ...
  private final Run3dmodButton btnMakeRefiningModel =
    Run3dmodButton.get3dmodInstance("Make Refining Model", this);
  private final MultiLineButton btnXfjointomo =
    new MultiLineButton("Find Transformations");

  private final BoundaryTable boundaryTable;
  private boolean refiningJoin = false;
  private SpacedPanel pnlTransformations = null;
  private final TransformChooserPanel tcModel =
    TransformChooserPanel.getJoinModelInstance();
  private final LabeledTextField ltfBoundariesToAnalyze =
    new LabeledTextField(FieldType.INTEGER_LIST, "Boundaries to analyze: ");
  private final LabeledTextField ltfObjectsToInclude =
    new LabeledTextField(FieldType.INTEGER_LIST, "Objects to include: ");
  private final LabeledTextField ltfGapStart =
    new LabeledTextField(FieldType.FLOATING_POINT, "Start: ");
  private final LabeledTextField ltfGapEnd =
    new LabeledTextField(FieldType.FLOATING_POINT, "End: ");
  private final LabeledTextField ltfGapInc =
    new LabeledTextField(FieldType.FLOATING_POINT, "Increment: ");
  private final LabeledTextField ltfPointsToFitMin =
    new LabeledTextField(FieldType.INTEGER, "Min: ");
  private final LabeledTextField ltfPointsToFitMax =
  ...
  private final BinnedXY3dmodButton b3bOpenRejoinWithModel =
    new BinnedXY3dmodButton("Open Rejoin with Transformed Model", this);
  private final Run3dmodButton btnTransformModel =
    Run3dmodButton.getDeferred3dmodInstance("Transform Model", this);
  private final JoinState state;
  private final MultiLineButton btnTransformAndViewModel =
    new MultiLineButton("Transform & View Model");
  private final Run3dmodButton btnOpenSampleAverages =
    Run3dmodButton.get3dmodInstance("Open Sample Averages in 3dmod", this);
  private final Run3dmodButton btnMakeSamples =
    Run3dmodButton.getDeferred3dmodInstance("Make Samples", this, "averages in 3dmod");
  ...
  private final BinnedXY3dmodButton b3bOpenTrialIn3dmod =
    new BinnedXY3dmodButton("Open Trial in 3dmod", this);
  private final Run3dmodButton btnTrialJoin =
    Run3dmodButton.getDeferred3dmodInstance(TRIAL_JOIN_TEXT, this);
  private final FileTextField ftfModelFile = new FileTextField("Model file: ");
  private final CheckBox cbGap = new CheckBox("Try gaps: ");
  private final BinnedXY3dmodButton b3bOpenIn3dmod =
    new BinnedXY3dmodButton(OPEN_IN_3DMOD, this);
  private final Run3dmodButton btnFinishJoin =
    Run3dmodButton.getDeferred3dmodInstance(FINISH_JOIN_TEXT, this);
  private final BinnedXY3dmodButton b3bOpenRejoin =
    new BinnedXY3dmodButton("Open Rejoin in 3dmod", this);
  private final Run3dmodButton btnRejoin =
    Run3dmodButton.getDeferred3dmodInstance(REJOIN_TEXT, this);
  private final BinnedXY3dmodButton b3bOpenTrialRejoin =
    new BinnedXY3dmodButton("Open Trial Rejoin in 3dmod", this);
  private final Run3dmodButton btnTrialRejoin =
    Run3dmodButton.getDeferred3dmodInstance(TRIAL_REJOIN_TEXT, this);
  ...
  private void addListeners() {
    ...
    cbLocalFits.addActionListener(joinActionListener);
    cbsAlignmentRefSection.addActionListener(joinActionListener);
  }
  ...
  public void updateDisplay() {
    ...
    btnRefineJoin.setEnabled(
      DatasetFiles.getJoinFile(false, manager).exists() || trialJoinRefinable);
    // If local fitting and a reference section are both selected, the local fitting has
    // no effect. Reference section has priority.
    cbLocalFits.setEnabled(!cbsAlignmentRefSection.isSelected());
    cbsAlignmentRefSection
      .setEnabled(!cbLocalFits.isSelected() || !cbLocalFits.isEnabled());
  }
  ...
  private void setSizeAndShift(Vector coordinates) throws NullRequiredNumberException {
    ...
    while (index < size) {
      if (ImodProcess.RUBBERBAND_RESULTS_STRING
  ...
  private void createModelPanel() {
    ...
    pnlTransformAndViewModel
      .setLayout(new BoxLayout(pnlTransformAndViewModel, BoxLayout.X_AXIS));
  ...
  private void createRejoinPanel() {
    ...
    pnlTransformModelButtons
      .setLayout(new BoxLayout(pnlTransformModelButtons, BoxLayout.X_AXIS));
  ...
  private void createFinishJoinPanel() {
    ...
    // seventh component
    b3bOpenIn3dmod.setSpinnerToolTipText(
  ...
  private void createTrialJoinPanel() {
    ...
    // fourth component
    b3bOpenTrialIn3dmod.setSpinnerToolTipText(
  ...
  void setNumSections(int numSections, final boolean init) {
    ...
    spinDensityRefSection.setModel(spinnerValue.getInt(), 1,
      numSections < 1 ? 1 : numSections, 1);
    // alignment (join)
  ...
  public boolean getParameters(Joinwarp2modelParam param, final boolean doValidation) {
    try {
      param.setBinningOfJoin(spinTrialBinning.getValue().toString());
      int offsetInX = Integer.valueOf(ltfShiftInX.getText(doValidation)) * (-1);
      int offsetInY = Integer.valueOf(ltfShiftInY.getText(doValidation)) * (-1);
      String offsetInXandY = String.valueOf(offsetInX) + ", " + String.valueOf(offsetInY);
      param.setOffsetInXandY(offsetInXandY);
      param.setChunkSizes(pnlSectionTable.getChunkSizes());
      return true;
    }
    catch (FieldValidationFailedException e) {
      return false;
  ...
  public void setXfjointomoResult()
  ...
  void setScreenState(JoinScreenState screenState) {
    if (cbRefineWithTrial.isEnabled()) {
      cbRefineWithTrial.setSelected(screenState.getRefineWithTrial().is());
    }
  }

  public boolean isRefineWithTrial() {
    return cbRefineWithTrial.isSelected();
  }
  ...
  public boolean equals(ConstJoinMetaData metaData) {
    ...
    if (!metaData.getAlignmentRefSection()
      .equals((Number) cbsAlignmentRefSection.getValue())) {
      return false;
    }
    ...
    if (!metaData.getRejoinUseEveryNSlices()
      .equals((Number) spinRejoinUseEveryNSlices.getValue())) {
      return false;
    }
   ...
   * @return
   */
  boolean equalsSample(ConstJoinMetaData metaData) {
    if (!ltfRootName.equals(metaData.getDatasetName())) {
      return false;
    }
    if (!metaData.getDensityRefSection()
  ...
  public void setRefineDataHighlight(boolean highlight) {
  ...
  public boolean getSectionTableMetaData() {
  ...
  public String getButtonName(boolean useTrial) {
    return useTrial ? TRIAL_JOIN_TEXT : FINISH_JOIN_TEXT;
  }

  private void setRefiningJoin() {
    refiningJoin = FileType.MODELED_JOIN.getFile(manager, AxisID.ONLY).exists();
    // refiningJoin = DatasetFiles.getModeledJoinFile(manager).exists();
  }

  public void setRefiningJoin(boolean input) {
    refiningJoin = input;
  }

  void workingDirAction() {
  ...
  private void setToolTipText() {
    ...
    btnOpenSample.setToolTipText("Press to the sample file in 3dmod.");
    cbsAlignmentRefSection.setCheckBoxToolTipText(
    ...
    cbsAlignmentRefSection.setSpinnerToolTipText(
    ...
    try {
      autodoc = AutodocFactory.getInstance(manager, AutodocFactory.XFJOINTOMO,
        AxisID.ONLY, false);
      ltfBoundariesToAnalyze.setToolTipText(
        EtomoAutodoc.getTooltip(autodoc, XfjointomoParam.BOUNDARIES_TO_ANALYZE_KEY));
      ltfObjectsToInclude.setToolTipText(
        EtomoAutodoc.getTooltip(autodoc, XfjointomoParam.OBJECTS_TO_INCLUDE));
    }
    ...
    cbGap.setToolTipText(
      "Check to allow the the final start and end values to change when the join is recreated.  "
  ...
  class TabChangeListener implements ChangeListener {
    ...
    TabChangeListener(JoinDialog dialog) {
      adaptee = dialog;
    }
