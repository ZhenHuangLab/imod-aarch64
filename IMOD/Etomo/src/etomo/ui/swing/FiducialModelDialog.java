 ...
 *
 * <p>Copyright: Copyright 2002 - 2023 by the Regents of the University of Colorado</p>
 * 
 ...
 * <p> </p>
 */
public final class FiducialModelDialog extends ProcessDialog
  ...
  private final RadioButton rbMethodSeed =
    new RadioButton("Make seed and track", TrackingMethod.SEED, bgMethod);
  ...
  private final RadioButton rbMethodRaptor =
    new RadioButton("Run RAPTOR and fix", TrackingMethod.RAPTOR, bgMethod);
  ...
  private final RadioButton rbSeedModelAuto = new RadioButton(AUTOFIDSEED_NEW_MODEL_TITLE,
    SeedModelEnumeratedType.AUTO, bgSeedModel);
  private final RadioButton rbSeedModelTransfer =
    new RadioButton("Transfer seed model from the other axis",
      SeedModelEnumeratedType.TRANSFER, bgSeedModel);
  ...
  private final Run3dmodButton btnBoundaryModel =
    Run3dmodButton.get3dmodInstance("Create/Edit Boundary Model", this);
  private final CheckBox cbExcludeInsideAreas =
    new CheckBox("Exclude inside boundary contours");
  private final LabeledTextField ltfBordersInXandY =
    new LabeledTextField(FieldType.INTEGER_PAIR, "Borders in X & Y: ");
  private final LabeledTextField ltfMinGuessNumBeads =
    new LabeledTextField(FieldType.INTEGER, "Estimated number of beads in sample: ");
  private final LabeledTextField ltfMinSpacing =
    new LabeledTextField(FieldType.FLOATING_POINT, "Minimum spacing: ");
  private final LabeledTextField ltfPeakStorageFraction =
    new LabeledTextField(FieldType.FLOATING_POINT, "Fraction of peaks to store: ");
  private final ButtonGroup bgTarget = new ButtonGroup();
  private final RadioTextField rtfTargetNumberOfBeads =
    RadioTextField.getInstance(FieldType.INTEGER, "Total number:", bgTarget);
  private final RadioTextField rtfTargetDensityOfBeads = RadioTextField
    .getInstance(FieldType.FLOATING_POINT, "Density (per megapixel):", bgTarget);
  ...
  private final LabeledTextField ltfIgnoreSurfaceData =
    new LabeledTextField(FieldType.INTEGER_LIST, "Ignore sorting in tracked models: ");
  private final LabeledTextField ltfDropTracks =
    new LabeledTextField(FieldType.INTEGER_LIST, "Drop tracked models: ");
  private final LabeledTextField ltfMaxMajorToMinorRatio =
    new LabeledTextField(FieldType.FLOATING_POINT, "Maximum ratio between surfaces: ");
  private final CheckBox cbClusteredPointsAllowedClustered =
    new CheckBox("Allow clustered beads");
  private final CheckBoxSpinner cbsElongatedPointsAllowed =
    CheckBoxSpinner.getInstance("Allow elongated beads of severity: ", 1, 1, 3);
  private final LabeledTextField ltfLowerTargetForClustered =
    new LabeledTextField(FieldType.FLOATING_POINT, "Lower target number for allowing ");
  private final Run3dmodButton btn3dmodAutofidseed =
    Run3dmodButton.get3dmodInstance("Open Seed Model", this);
  private final Run3dmodButton btn3dmodInitialBeadFinding =
    Run3dmodButton.get3dmodInstance("Open Initial Bead Model", this);
  private final Run3dmodButton btn3dmodBeadSelectionAndSorting =
    Run3dmodButton.get3dmodInstance("Open Sorted 3D Models", this);
  private final Run3dmodButton btn3dmodClusteredElongatedModel =
    Run3dmodButton.get3dmodInstance("Open Clustered / Elongated Model", this);
  private final MultiLineButton btnCleanup =
    new MultiLineButton("Clean Up Temporary Files");
  ...
  private final MultiLineButton btnUseAdjustedTrackCom;
  ...
  private FiducialModelDialog(final ApplicationManager appMgr, final AxisID axisID,
    ...
    btnUseAdjustedTrackCom = (MultiLineButton) factory.getUseAdjustedTrackCom();
    btnUseAdjustedTrackCom.setEnabled(false);
  ...
  private void createPanel() {
    ...
    JPanel pnlAllowClusteredElongated = new JPanel();
    JPanel pnlAllowClustered = new JPanel();
    JPanel pnlAllowElongated = new JPanel();
    JPanel pnlAllowOnlyIfNumber = new JPanel();
    ...
    for (i = 0; i < SeedAndTrackTab.NUM_TABS; i++) {
      ...
      tpSeedAndTrack.addTab(SeedAndTrackTab.getInstance(i).title,
        pnlSeedAndTrackArray[i]);
      pnlSeedAndTrackBodyArray[i] = new JPanel();
      pnlSeedAndTrackBodyArray[i]
        .setLayout(new BoxLayout(pnlSeedAndTrackBodyArray[i], BoxLayout.Y_AXIS));
    }
    ...
    pnlInitialBeadFinding
      .setBorder(new EtchedBorder("Initial Bead Finding Parameters").getBorder());
    ...
    pnl3dmodInitialBeadFinding
      .setLayout(new BoxLayout(pnl3dmodInitialBeadFinding, BoxLayout.X_AXIS));
    ...
    pnlBeadSearchingAndSorting
      .setLayout(new BoxLayout(pnlBeadSearchingAndSorting, BoxLayout.Y_AXIS));
    pnlBeadSearchingAndSorting
      .setBorder(new EtchedBorder("Selection and Sorting Parameters").getBorder());
    ...
    pnlBeadSearchingAndSorting.add(pnlAllowClusteredElongated);
    ...
    pnlAllowClusteredElongated
      .setLayout(new BoxLayout(pnlAllowClusteredElongated, BoxLayout.Y_AXIS));
    pnlAllowClusteredElongated
      .setBorder(new EtchedBorder("Clustered/Elongated Beads").getBorder());
    pnlAllowClusteredElongated.add(pnlAllowClustered);
    pnlAllowClusteredElongated.add(pnlAllowElongated);
    pnlAllowClusteredElongated.add(pnlAllowOnlyIfNumber);
    // AllowClustered
    pnlAllowClustered.setLayout(new BoxLayout(pnlAllowClustered, BoxLayout.X_AXIS));
    pnlAllowClustered.add(Box.createRigidArea(FixedDim.x3_y0));
    pnlAllowClustered.add(cbClusteredPointsAllowedClustered.getComponent());
    pnlAllowClustered.add(Box.createHorizontalGlue());
    // AlowElongated
    pnlAllowElongated.setLayout(new BoxLayout(pnlAllowElongated, BoxLayout.X_AXIS));
    pnlAllowElongated.add(cbsElongatedPointsAllowed.getContainer());
    pnlAllowElongated.add(Box.createHorizontalGlue());
    // AllowOnlyIfNumber<
    pnlAllowOnlyIfNumber.setLayout(new BoxLayout(pnlAllowOnlyIfNumber, BoxLayout.X_AXIS));
    pnlAllowOnlyIfNumber.add(Box.createRigidArea(FixedDim.x3_y0));
    pnlAllowOnlyIfNumber.add(ltfLowerTargetForClustered.getComponent());
    pnlAllowOnlyIfNumber.add(Box.createHorizontalGlue());
    // 3dmodBeadSortingAndSearching
    pnl3dmodBeadSortingAndSearching
      .setLayout(new BoxLayout(pnl3dmodBeadSortingAndSearching, BoxLayout.X_AXIS));
    ...
    pnl3dmodClusteredElongatedModel
      .setLayout(new BoxLayout(pnl3dmodClusteredElongatedModel, BoxLayout.X_AXIS));
    ...
    pnlButtons.add(Box.createHorizontalGlue());
    pnlButtons.add(btnUseAdjustedTrackCom.getComponent());
    pnlButtons.add(Box.createHorizontalGlue());
  ...
  private void changeRunRaptorTab() {
    ...
    // Change the tab body panel
    if ((curRunRaptorTab == null && newIndex != -1)
      || !curRunRaptorTab.equals(newIndex)) {
      // If the tab has changed:
      // Remove the body from previous tab so that the size of the tabbed pane won't
      // reflect it.
  ...
  private void addListeners() {
    ...
    btnUseAdjustedTrackCom.addActionListener(actionListener);
    ...
    rtfTargetNumberOfBeads.addActionListener(actionListener);
    rtfTargetDensityOfBeads.addActionListener(actionListener);
    cbClusteredPointsAllowedClustered.addActionListener(actionListener);
    cbsElongatedPointsAllowed.addActionListener(actionListener);
  }
  ...
  public void updateDisplay() {
    ...
    ltfLowerTargetForClustered.setEnabled((cbClusteredPointsAllowedClustered.isSelected()
      || cbsElongatedPointsAllowed.isSelected())
      && !rtfTargetDensityOfBeads.isSelected());
  }
  ...
  private void updateAdvanced(final boolean advanced) {
    ...
    ltfLowerTargetForClustered.setVisible(advanced);
  ...
  public void updateEnabled() {
    ...
    btn3dmodInitialBeadFinding
      .setEnabled(AutofidseedInitFileFilter.exists(applicationManager, axisID));
    boolean modelsExist =
    ...
    // comfile.
    try {
      btnUseAdjustedTrackCom
        .setEnabled(FileType.TRACK_ADJUSTED_COMSCRIPT.exists(applicationManager, axisID)
          && AutofidseedLog.getInstance(applicationManager.getPropertyUserDir(), axisID)
            .isTrackingAdjusted());
    }
    catch (IOException e) {
      e.printStackTrace();
      ...
      // available.
      btnUseAdjustedTrackCom.setEnabled(true);
    }
  ...
  public void setBeadtrackParams(final BeadtrackParam beadtrackParams,
    final boolean forTransferFid) {
    if (!forTransferFid && raptorPanel != null) {
      raptorPanel.setBeadtrackParams(beadtrackParams);
    }
    pnlBeadtrack.setParameters(beadtrackParams, forTransferFid);
  }
  ...
  public void getParameters(final MetaData metaData) {
    ...
    metaData.setTrackMethod(axisID,
    ...
    metaData.setTrackLowerTargetForClustered(axisID,
      ltfLowerTargetForClustered.getText());
  ...
  public void setParameters(final ConstMetaData metaData) {
    ...
    cbsElongatedPointsAllowed
      .setSelected(metaData.isTrackClusteredPointsAllowedElongated(axisID));
    cbsElongatedPointsAllowed
      .setValue(metaData.getTrackClusteredPointsAllowedElongatedValue(axisID));
    ...
    ltfLowerTargetForClustered.setText(metaData.getTrackLowerTargetForClustered(axisID));
  ...
  public void setParameters(final AutofidseedParam param, final boolean forTransferFid) {
    ...
    if (param.isLowerTargetForClustered()) {
      ltfLowerTargetForClustered.setValue(param.getLowerTargetForClustered());
    }
    if (!forTransferFid) {
  ...
  public boolean getParameters(final AutofidseedParam param, final boolean doValidation)
    ...
    try {
      ...
      param.setExcludeInsideAreas(
        cbExcludeInsideAreas.isEnabled() && cbExcludeInsideAreas.isSelected());
      ...
      if (ltfLowerTargetForClustered.isEnabled()) {
        param
          .setLowerTargetForClustered(ltfLowerTargetForClustered.getText(doValidation));
      }
      else {
        param.resetLowerTargetForClustered();
      }
  ...
  public void popUpContextMenu(final MouseEvent mouseEvent) {
    ...
    if (autofidseed) {
      ...
      manPage = new String[] { "autofidseed.html", "imodfindbeads.html", "beadtrack.html",
    ...
    new ContextPopup(rootPanel, mouseEvent, anchor, ContextPopup.TOMO_GUIDE, manPagelabel,
  ...
  private void setToolTipText() {
    btnUseAdjustedTrackCom.setToolTipText(
      "For tracking this seed, use the com file with an adjusted bead size or "
        + "information on large shifts between views.");
    rbMethodSeed.setToolTipText(
    ...
    rbSeedModelManual.setToolTipText(
    ...
    btn3dmodInitialBeadFinding.setToolTipText(
    ...
    try {
      autodoc = AutodocFactory.getInstance(applicationManager, AutodocFactory.AUTOFIDSEED,
        axisID, false);
    }
    ...
    cbExcludeInsideAreas.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, AutofidseedParam.EXCLUDE_INSIDE_AREAS_KEY));
    cbAdjustSizes.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, AutofidseedParam.ADJUST_SIZES_KEY));
    ltfBordersInXandY.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, AutofidseedParam.BORDERS_IN_X_AND_Y_KEY));
    ltfMinGuessNumBeads.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, AutofidseedParam.MIN_GUESS_NUM_BEADS_KEY));
    ltfMinSpacing
      .setToolTipText(EtomoAutodoc.getTooltip(autodoc, AutofidseedParam.MIN_SPACING_KEY));
    ltfPeakStorageFraction.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, AutofidseedParam.PEAK_STORAGE_FRACTION_KEY));
    rtfTargetNumberOfBeads.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, AutofidseedParam.TARGET_NUMBER_OF_BEADS_KEY));
    rtfTargetDensityOfBeads.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, AutofidseedParam.TARGET_DENSITY_OF_BEADS_KEY));
    cbTwoSurfaces.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, AutofidseedParam.TWO_SURFACES_KEY));
    cbAppendToSeedModel.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, AutofidseedParam.APPEND_TO_SEED_MODEL_KEY));
    ltfIgnoreSurfaceData.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, AutofidseedParam.IGNORE_SURFACE_DATA_KEY));
    ltfDropTracks
      .setToolTipText(EtomoAutodoc.getTooltip(autodoc, AutofidseedParam.DROP_TRACKS_KEY));
    ltfMaxMajorToMinorRatio.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, AutofidseedParam.MAX_MAJOR_TO_MINOR_RATIO_KEY));
    cbClusteredPointsAllowedClustered.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, AutofidseedParam.CLUSTERED_POINTS_ALLOWED_KEY));
    cbsElongatedPointsAllowed.setCheckBoxToolTipText(
      EtomoAutodoc.getTooltip(autodoc, AutofidseedParam.ELONGATED_POINTS_ALLOWED_KEY));
    ltfLowerTargetForClustered.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, AutofidseedParam.LOWER_TARGET_FOR_CLUSTERED_KEY));
  }
  ...
  public void action(final String command, final Deferred3dmodButton deferred3dmodButton,
    ...
    else if (command.equals(btnUseAdjustedTrackCom.getActionCommand())) {
      applicationManager.useTrackAdjustedComfile(axisID, btnUseAdjustedTrackCom);
      updateEnabled();
    }
    updateDisplay();
  }
  ...
  void done() {
    ...
    btnUseAdjustedTrackCom.removeActionListener(actionListener);
    pnlBeadtrack.done();
    tiltxcorrPanel.done();
    setDisplayed(false);
  }
  ...
  private static final class SeedAndTrackTabChangeListener implements ChangeListener {
    ...
    SeedAndTrackTabChangeListener(final FiducialModelDialog dialog) {
      this.dialog = dialog;
    }
  ...
  private static final class RunRaptorTabChangeListener implements ChangeListener {
    ...
    RunRaptorTabChangeListener(final FiducialModelDialog dialog) {
      this.dialog = dialog;
    }
  ...
  public static final class SeedModelEnumeratedType implements EnumeratedType {
    private static final SeedModelEnumeratedType MANUAL =
      new SeedModelEnumeratedType(true, 0, "Manual");
    private static final SeedModelEnumeratedType AUTO =
      new SeedModelEnumeratedType(false, 1, "Auto");
    public static final SeedModelEnumeratedType TRANSFER =
      new SeedModelEnumeratedType(false, 2, "Transfer");
