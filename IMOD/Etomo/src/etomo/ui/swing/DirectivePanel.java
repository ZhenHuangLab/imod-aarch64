...
* 
* <p>Copyright: Copyright 2013 - 2016 by the Regents of the University of Colorado</p>
*
...
final class DirectivePanel {
  ...
  private final CheckBox cbInclude;
  ...
  private DirectivePanel(final BaseManager manager, final Directive directive,
    ...
    if (booleanValueType || directive.isChoiceList()) {
      if (booleanValueType) {
        cbValue = ComboBox.getUnlabeledInstance(title);
        cbValue.addItem("Yes");
        cbValue.addItem("No");
        valueList = null;
      }
      else {
        cbValue = ComboBox.getUnlabeledEmptyChoiceInstance(title);
        DirectiveDescrChoiceList choiceList = directive.getChoiceList();
      ...
      tfValue = new TextField(fieldType, title, null);
      cbValue = null;
      valueList = null;
      if (valueType == DirectiveValueType.FILE) {
        sbFileValue = new SimpleButton(new ImageIcon(ClassLoader.getSystemResource(
  ...
  private void createPanel(final Directive directive) {
    ...
    if (cbValue != null) {
      ...
      pnlRoot.add(tfValue.getComponent());
      if (sbFileValue != null) {
  ...
  void setStateInDirective() {
    ...
    if (cbValue != null) {
      ...
      directive.setValue(tfValue.getText());
    }
  }
  ...
  void init() {
    if (tfValue != null) {
      tfValue.setColumns(directive.getValueType().getColumns());
    }
  ...
  */
  ...
  private void fileValueAction() {
    ...
    if (!tfValue.isEmpty()) {
      chooser.setCurrentDirectory(new File(tfValue.getText()));
    }
    ...
    if (chooser.showOpenDialog(pnlRoot) == JFileChooser.APPROVE_OPTION) {
      tfValue.setText(chooser.getSelectedFile());
    }
   ...
   */
   boolean msgControlChanged(final boolean includeChange,
  ...
  private void copyValue(final DirectivePanel input) {
    if (input == null) {
      ...
      if (cbValue != null) {
        ...
        tfValue.setText(input.tfValue.getText());
      }
    }
  }
  ...
  private void enableValue() {
   ...
   public boolean equals(final DirectivePanel input) {
    if (input == null) {
      return false;
    }
  ...
  private boolean equalsValue(final DirectivePanel input) {
    ...
    else if (!FieldValidator.equals(fieldType, tfValue.getText(),
      input.tfValue.getText())) {
      return false;
    }
    return true;
  }
  ...
  private void initValue(final Directive directive) {
   ...
   boolean isDifferentFromCheckpoint(final boolean checkInclude) {
    ...
    return tfValue.isDifferentFromCheckpoint(true);
  }
