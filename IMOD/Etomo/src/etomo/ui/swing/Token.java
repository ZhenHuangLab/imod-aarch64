...
public final class Token {
  ...
  public void write(LogFile file, LogFile.WriterId writerId)
    ...
    // Avoid using recursion here because token list may be large enough to cause a stack
    // overflow
    Token pointer = next;
    while (pointer != null) {
      if (pointer.type == Type.EOL) {
        file.newLine(writerId);
      }
      else {
        file.write(pointer.value, writerId);
      }
  ...
  public static final class Type {
    ...
    public static final Type SEPARATOR =
      new Type(AutodocTokenizer.SEPARATOR_CHAR.toString());
    public static final Type OPEN = new Type(AutodocTokenizer.OPEN_CHAR.toString());
    public static final Type CLOSE = new Type(AutodocTokenizer.CLOSE_CHAR.toString());
    ...
    public static final Type SUBOPEN = new Type(
      AutodocTokenizer.OPEN_CHAR.toString() + AutodocTokenizer.OPEN_CHAR.toString());
    public static final Type SUBCLOSE = new Type(
      AutodocTokenizer.CLOSE_CHAR.toString() + AutodocTokenizer.CLOSE_CHAR.toString());
    ...
    private Type(final String descr) {
      this.descr = descr;
    }
