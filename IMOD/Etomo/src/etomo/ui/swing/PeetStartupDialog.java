...
public final class PeetStartupDialog implements UIComponent, SwingComponent {
  ...
  private final LabeledTextField ltfBaseName =
    new LabeledTextField(FieldType.STRING, "Base name: ");
  ...
  private PeetStartupDialog(final PeetManager manager, final AxisID axisID) {
    ...
    ftfDirectory = FileTextField2.getPeetInstance(manager, axisID, "Directory: ");
  ...
  private boolean validate() {
    ...
    if (ftfDirectory.isEmpty()) {
      ...
      if (!file.exists()) {
        errorMessage =
          ftfDirectory.getQuotedLabel() + " must contain a directory which exists.";
      }
        ...
        errorMessage =
          ftfDirectory.getQuotedLabel() + " must contain a readable directory.";
      }
      else if (!file.canWrite()) {
        errorMessage =
          ftfDirectory.getQuotedLabel() + " must contain a writable directory.";
      }
      // CopyFrom
        ...
        if (ftfCopyFrom.isEmpty()) {
          ...
          if (!file.exists()) {
            ...
            errorMessage =
              ftfCopyFrom.getQuotedLabel() + " must contain a readable file.";
          }
          // BaseName
  ...
  private void setTooltips() {
    ...
    String tooltip =
      "Check and fill in an .epe or .prm file to create a new PEET project "
  ...
  private static final class PeetStartupResultListener implements ResultListener {
    ...
    public void processResult(final Object resultOrigin, final boolean init) {
