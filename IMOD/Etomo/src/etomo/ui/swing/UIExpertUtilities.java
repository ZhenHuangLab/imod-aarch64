...
public final class UIExpertUtilities {
  ...
  public int getStackBinning(BaseManager manager, AxisID axisID, FileType stackFileType) {
    return getStackBinning(manager, axisID,
      MRCHeader.getInstance(manager.getPropertyUserDir(),
        stackFileType.getFileName(manager, axisID), axisID),
      false);
  }
   ...
   * @return
   */
  public boolean updateFiducialessParams(ApplicationManager manager, String imageRotation,
   ...
   * @param axisID
   */
  private void updateRotationXF(BaseManager manager, String propertyUserDir, String angle,
    AxisID axisID) {
    // Open the appropriate rotation file
    String fnRotationXF =
      propertyUserDir + File.separator + "rotation" + axisID.getExtension() + ".xf";
    ...
    try {
      ...
      out.write(String.valueOf(Math.cos(rads)) + "   " + String.valueOf(Math.sin(-rads))
        + "   " + String.valueOf(Math.sin(rads)) + "   " + String.valueOf(Math.cos(rads))
        + "   0   0");
      out.newLine();
      // Close the file
      out.close();
  ...
  public void upgradeOldAlignCom(ApplicationManager manager, AxisID axisID,
    ...
    int currentBinning = getStackBinning(manager, axisID, FileType.PREALIGNED_STACK);
  ...
  public void upgradeOldTiltCom(ApplicationManager manager, AxisID axisID,
    ...
    int currentBinning = getStackBinning(manager, axisID, FileType.ALIGNED_STACK);
   ...
   * @return
   */
  private int getBackwardCompatibleAlignBinning(ApplicationManager manager,
    AxisID axisID) {
    MRCHeader rawstackHeader = MRCHeader.getInstance(manager, axisID, FileType.RAW_STACK);
    try {
    ...
    int binning =
      (int) Math.round(stackHeader.getXPixelSpacing() / rawstackXPixelSpacing);
    if (binning < 1) {
      return 1;
    }
    return binning;
  ...
  public FidXyz getFidXyz(ApplicationManager manager, AxisID axisID) {
    return new FidXyz(manager.getPropertyUserDir(),
      manager.getMetaData().getDatasetName() + axisID.getExtension() + "fid.xyz");
  }
