...
import etomo.type.MirrorInX;
import etomo.type.ReconScreenState;
import etomo.type.Run3dmodMenuOptions;
import etomo.ui.FieldType;
import etomo.ui.FieldValidationFailedException;
import etomo.ui.swing.RadioButton.RadioButtonModel;
...
final class TransferfidPanel implements Expandable, Run3dmodButtonContainer {
  ...
  private final LabeledTextField ltfCenterViewA =
    new LabeledTextField(FieldType.INTEGER, "Center view A: ");
  private final LabeledTextField ltfCenterViewB =
    new LabeledTextField(FieldType.INTEGER, "Center view B: ");
  private final LabeledTextField ltfNumberViews =
    new LabeledTextField(FieldType.INTEGER, "Number of views in the search: ");
  ...
  private final TransferfidPanelActionListener actionListener =
    new TransferfidPanelActionListener(this);
  private final Run3dmodButton btn3dmodSeed =
    Run3dmodButton.get3dmodInstance("Open Seed Model", this);
  private final ButtonGroup bgMirrorInX = new ButtonGroup();
  private final RadioButton rbMirrorInXAssessBoth =
    new RadioButton(MirrorInX.ASSESS_BOTH, bgMirrorInX);
  private final RadioButton rbMirrorInXAlways =
    new RadioButton(MirrorInX.ALWAYS, bgMirrorInX);
  private final RadioButton rbMirrorInXNever =
    new RadioButton(MirrorInX.NEVER, bgMirrorInX);
  private final     JPanel pnlOuterMirrorXaxis = new JPanel();
  ...
  private TransferfidPanel(ApplicationManager manager, AxisID axisID,
    ...
    this.axisID = axisID;
    // panels
    JPanel pnlMirrorXaxis = new JPanel();
    ...
    panelTransferfidBody.add(Box.createRigidArea(FixedDim.x0_y1));
    panelTransferfidBody.add(opnlSearchDirection);
    panelTransferfidBody.add(Box.createRigidArea(FixedDim.x0_y1));
    panelTransferfidBody.add(pnlOuterMirrorXaxis);
    panelTransferfidBody.add(Box.createRigidArea(FixedDim.x0_y5));
    // OuterMirrorInX
    pnlOuterMirrorXaxis.setLayout(new BoxLayout(pnlOuterMirrorXaxis, BoxLayout.X_AXIS));
    pnlOuterMirrorXaxis.add(Box.createHorizontalGlue());
    pnlOuterMirrorXaxis.add(pnlMirrorXaxis);
    pnlOuterMirrorXaxis.add(Box.createHorizontalGlue());
    // MirrorInX
    pnlMirrorXaxis.setLayout(new BoxLayout(pnlMirrorXaxis, BoxLayout.Y_AXIS));
    pnlMirrorXaxis.setBorder(new EtchedBorder("Mirroring around X axis").getBorder());
    pnlMirrorXaxis.add(rbMirrorInXAssessBoth.getComponent());
    pnlMirrorXaxis.add(rbMirrorInXAlways.getComponent());
    pnlMirrorXaxis.add(rbMirrorInXNever.getComponent());
    //
  ...
  static TransferfidPanel getInstance(ApplicationManager manager, AxisID axisID,
    DialogType dialogType, GlobalExpandButton globalAdvancedButton) {
    TransferfidPanel instance =
      new TransferfidPanel(manager, axisID, dialogType, globalAdvancedButton);
  ...
  void setParameters() {
    ...
    MirrorInX mirrorInX = params.getMirrorXaxis();
    if (mirrorInX == MirrorInX.ASSESS_BOTH) {
      rbMirrorInXAssessBoth.setSelected(true);
    }
    if (mirrorInX == MirrorInX.ALWAYS) {
      rbMirrorInXAlways.setSelected(true);
    }
    if (mirrorInX == MirrorInX.NEVER) {
      rbMirrorInXNever.setSelected(true);
    }
  }
  ...
  boolean getParameters(TransferfidParam params, final boolean doValidation) {
    try {
      ...
      params.setMirrorXaxis(
        ((RadioButtonModel) bgMirrorInX.getSelection()).getEnumeratedType().getValue());
      if (axisID == AxisID.SECOND) {
  ...
  void updateAdvanced(boolean isAdvanced) {
    ...
    pnlOuterMirrorXaxis.setVisible(isAdvanced);
  ...
  void setEnabled(boolean isEnabled) {
    ...
    rbMirrorInXAssessBoth.setEnabled(isEnabled);
    rbMirrorInXAlways.setEnabled(isEnabled);
    rbMirrorInXNever.setEnabled(isEnabled);
  }
