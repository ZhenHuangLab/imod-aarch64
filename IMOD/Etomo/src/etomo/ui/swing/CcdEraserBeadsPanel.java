...
final class CcdEraserBeadsPanel implements Run3dmodButtonContainer, CcdEraserDisplay {
  ...
  private final LabeledTextField ltfFiducialDiameter =
    new LabeledTextField(FieldType.FLOATING_POINT, "Diameter to erase (pixels): ");
  private final CheckBoxSpinner cbspExpandCircleIterations =
    CheckBoxSpinner.getInstance("Iterations to grow circular areas:", 2, 1, 5);
  private final EtomoButtonGroup bgPolynomialOrder = new EtomoButtonGroup();
  ...
  private final RadioButton rbPolynomialOrderFillNoise = new RadioButton(
    "Fill pixels with noise values", PolynomialOrder.FILL_WITH_NOISE, bgPolynomialOrder);
  ...
  private final Run3dmodButton btn3dmodCcdEraser =
    Run3dmodButton.get3dmodInstance("View Erased Stack", this);
  ...
  void initialize() {
    if (ltfFiducialDiameter.isEmpty()) {
      ltfFiducialDiameter
        .setText(manager.calcBinnedBeadDiameterPixels(axisID, FileType.ALIGNED_STACK, 1));
    }
  }
  ...
  private void createPanel() {
    ...
    polynomialOrderPanel.setBorder(new EtchedBorder("Pixel Filling Method").getBorder());
    polynomialOrderPanel.add(rbPolynomialOrderUseMean.getComponent());
    polynomialOrderPanel.add(rbPolynomialOrderFillNoise.getComponent());
  ...
  void setPolynomialOrder(final EnumeratedType enumeratedType) {
    ...
    else if (rbPolynomialOrderFillNoise.getEnumeratedType() == enumeratedType) {
      rbPolynomialOrderFillNoise.setSelected(true);
    }
  ...
  void setParameters(final ConstMetaData metaData) {
    if (!metaData.isFinalStackFiducialDiameterNull(axisID)) {
      ...
      ltfFiducialDiameter.setText(
        (Math.round(metaData.getFiducialDiameter() / metaData.getPixelSize() * 10.0)
          / 10.0));
    }
    setPolynomialOrder(
      PolynomialOrder.getInstance(metaData.getFinalStackPolynomialOrder(axisID)));
    cbspExpandCircleIterations
      .setSelected(metaData.isUseFinalStackExpandCircleIterations(axisID));
    if (metaData.isFinalStackExpandCircleIterationsSet(axisID)) {
      cbspExpandCircleIterations
        .setValue(metaData.getFinalStackExpandCircleIterations(axisID));
    }
  }
  ...
  public void setParameters(final ConstCCDEraserParam param) {
    if (param.isBetterRadiusSet()) {
      EtomoNumber fiducialDiameter = new EtomoNumber(EtomoNumber.Type.DOUBLE);
      fiducialDiameter.set(param.getBetterRadius() * 2.0);
      ltfFiducialDiameter.setText(fiducialDiameter);
    }
    cbspExpandCircleIterations.setSelected(param.isExpandCircleIterationsSet());
    if (cbspExpandCircleIterations.isSelected()) {
      cbspExpandCircleIterations.setValue(param.getExpandCircleIterations());
    }
    bgPolynomialOrder
      .setSelected(PolynomialOrder.getInstance(param.getPolynomialOrder()));
  }
  ...
  final void setParameters(final ReconScreenState screenState) {
    btnCcdEraser
      .setButtonState(screenState.getButtonState(btnCcdEraser.getButtonStateKey()));
    btnUseCcdEraser
      .setButtonState(screenState.getButtonState(btnUseCcdEraser.getButtonStateKey()));
  }
  ...
  private void setToolTipText() {
    ...
    cbspExpandCircleIterations.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, CCDEraserParam.EXPAND_CIRCLE_ITERATIONS_KEY));
    rbPolynomialOrderUseMean
      .setToolTipText("Fill the erased pixels with the mean of surrounding " + "pixels.");
    rbPolynomialOrderFillNoise.setToolTipText(
    ...
    rbPolynomialOrderFitAPlane
      .setToolTipText("Fill the erased pixels with a gradient based on plane "
        + "fit to surrounding pixels.");
    btnCcdEraser.setToolTipText(
    ...
    btnUseCcdEraser.setToolTipText(
  ...
  private static final class PolynomialOrder implements EnumeratedType {
    ...
    private static final PolynomialOrder FILL_WITH_NOISE = new PolynomialOrder(-1);
    ...
    private static PolynomialOrder getInstance(final int value) {
      ...
      if (FILL_WITH_NOISE.value.equals(value)) {
        return FILL_WITH_NOISE;
      }
      return DEFAULT;
    }

    private static PolynomialOrder getInstance(final String value) {
      if (USE_MEAN.value.equals(value)) {
        return USE_MEAN;
      }
      if (FIT_A_PLANE.value.equals(value)) {
