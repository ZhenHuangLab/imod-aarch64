...
/**
 * <p>Description: Rows for the Peet volume table.</p>
 * 
...
final class VolumeRow implements Highlightable {
  ...
  static VolumeRow getInstance(final BaseManager manager, final int index,
    ...
    VolumeRow instance =
      new VolumeRow(manager, index, table, panel, layout, constraints, volumeFileFilter);
  ...
  static VolumeRow getInstance(final BaseManager manager, final File fnVolume,
    ...
    VolumeRow instance = new VolumeRow(manager, fnVolume, fnModParticle,
      tiltRangeMultiAxesFile, index, table, panel, layout, constraints, volumeFileFilter);
    instance.addActionTargets();
    instance.setTooltips();
    return instance;
  }
  ...
  private VolumeRow(final BaseManager manager, final int index, final VolumeTable table,
    ...
    fbFnVolume = FileButtonCell.getInstance(manager);
    fbFnVolume.setBrowsingDirectory(manager);
    ...
    fbFnModParticle = FileButtonCell.getInstance(manager);
    fbFnModParticle.setBrowsingDirectory(manager);
    fbFnModParticle.setFileFilter(new ModelFileFilter());
    initMotlFile = FieldCell.getExpandableInstance(rootDir);
    fbInitMotlFile = FileButtonCell.getInstance(manager);
    fbInitMotlFile.setBrowsingDirectory(manager);
    ...
    fbTiltRangeMultiAxes = FileButtonCell.getInstance(manager);
    fbTiltRangeMultiAxes.setBrowsingDirectory(manager);
  ...
  private VolumeRow(final BaseManager manager, final String fnVolumeFile,
    final String fnModParticleFile, final String tiltRangeMultiAxesFile, final int index,
    final VolumeTable table, final JPanel panel, final GridBagLayout layout,
    final GridBagConstraints constraints, final VolumeFileFilter volumeFileFilter) {
    this.manager = manager;
    this.index = index;
  ...
  void setNames() {
    ...
    fbInitMotlFile.setLabel(
      VolumeTable.INIT_MOTL_FILE_HEADER1 + " " + VolumeTable.INIT_MOTL_FILE_HEADER2);
  ...
  void getParameters(final PeetMetaData metaData) {
    ...
    metaData.setTiltRangeMultiAxesFile(tiltRangeMultiAxes.getExpandedValue(), index);
  }
  ...
  boolean isIncorrectPaths() {
    if (!fnVolume.isEmpty() && !FilePath
      .buildAbsoluteFile(manager.getPropertyUserDir(), fnVolume.getExpandedValue())
      .exists()) {
      return true;
    }
    if (!fnModParticle.isEmpty() && !FilePath
      .buildAbsoluteFile(manager.getPropertyUserDir(), fnModParticle.getExpandedValue())
      .exists()) {
      return true;
    }
    if (!initMotlFile.isEmpty() && !FilePath
      .buildAbsoluteFile(manager.getPropertyUserDir(), initMotlFile.getExpandedValue())
      .exists()) {
      return true;
    }
    if (!tiltRangeMultiAxes.isEmpty()
  ...
  private boolean fixIncorrectPath(FieldCell fieldCell, boolean choosePath,
    ...
    while (newFile == null || !newFile.exists()) {
      ...
      if (table.isCorrectPathNull() || choosePath
        ...
        fileChooser.setSelectedFile(FilePath
          .buildAbsoluteFile(manager.getPropertyUserDir(), fieldCell.getExpandedValue()));
        fileChooser
  ...
  void getParameters(final MatlabParam matlabParamFile,
    final boolean isTiltRangeMultiAxes) {
    ...
    if (!isTiltRangeMultiAxes) {
      ...
      volume.setTiltRangeMultiAxes(tiltRangeMultiAxes.getExpandedValue());
    }
  }
   ...
   */
  String validateRun(final boolean tiltRangeRequired,
    final boolean isTiltRangeMultiAxes) {
    if (fnModParticle.isEmpty()) {
