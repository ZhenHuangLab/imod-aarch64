...
/**
 * <p>Description: Handles the B axis when both axes are displayed.  Manager independent.
 * </p>
 * 
 * 
 ...
 * @version $Id$
 */
final class SubFrame extends EtomoFrame implements BusyStatusListener {
  static final String NAME = "sub-frame";

  private final JPanel pnlStatus = new JPanel();
  private final JPanel pnlBusyStatus = new JPanel();
  private final JLabel lBusyStatusB = MainPanel.createBusyStatusLabel();
  private final JLabel lGuiBusyStatusB = MainPanel.createGuiBusyStatusLabel();
  ...
  SubFrame(MainFrame mainFrame) {
    ...
    // Status
    pnlStatus.setLayout(MainPanel.createStatusBorderLayout());
  ...
  void initialize(String title, BaseManager currentManager, String[] mRUList) {
    ...
    if (this.currentManager != null) {
      this.currentManager.removeBusyStatusListener(this);
    }
    this.currentManager = currentManager;
    currentManager.addBusyStatusListener(this);
    setTitle(title);
    ...
    rootPanel
      .setName(UITestFieldType.PANEL.toString() + AutodocTokenizer.SEPARATOR_CHAR + name);
    if (EtomoDirector.INSTANCE.getArguments().isPrintNames()) {
      System.out
        .println(UITestFieldType.PANEL.toString() + AutodocTokenizer.SEPARATOR_CHAR + name
    ...
    menu.setMenu3dmodStartupWindow(mainFrame.menu.isMenu3dmodStartupWindow());
    menu.setMenu3dmodBinBy2(mainFrame.menu.isMenu3dmodBinBy2());
    setVisible(true);
  }
  ...
  public void msgBusyStatusChanged(final AxisID axisID, final boolean busyStatus) {
    SwingUtilities.invokeLater(new SetBusyStatus(axisID, false, busyStatus));
  }

  public void msgGuiBusyStatusChanged(final AxisID axisID, final boolean busyStatus) {
    SwingUtilities.invokeLater(new SetBusyStatus(axisID, true, busyStatus));
  }
  ...
  private void setAxis() {
    ...
    rootPanel.add(pnlStatus, BorderLayout.SOUTH);
    validate();
  }
