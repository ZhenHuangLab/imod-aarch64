...
public final class SetupDialogExpert {
  ...
  public void doAutomation() {
    ...
    String dataset = arguments.getRawImageStack();
    if (dataset != null) {
      ...
      if (dir != null && dataset.endsWith(FileType.RAW_STACK.getExtension())) {
        dialog.setRawImageStack(new File(dir, dataset).getAbsolutePath());
      }
      else {
        dialog.setRawImageStack(arguments.getRawImageStack());
      }
    }
    // check radio buttons
  ...
  public void showProgressPanel() {
    dialog.showProgressPanel();
  }

  public void updateDisplay(final boolean processDone) {
    dialog.updateDisplay(false, processDone);
  }

  public void msgExcludeViewsSucceeded(final AxisID axisID, final boolean processRunning,
    final boolean processDone) {
    dialog.msgExcludeViewsSucceeded(axisID, processRunning, processDone);
  }

  public void msgSetupReconFailed() {
    dialog.msgSetupReconFailed();
  }
  ...
  public boolean checkForSharedDirectory(final Extension rawImageStackExtension) {
    ...
    String rawImageStack = getRawImageStack();
    if (axisType == AxisType.DUAL_AXIS) {
      firstStack = new File(propertyUserDir, datasetName + AxisID.FIRST.getExtension()
        + Extension.EXTENSION_DIVIDER + rawImageStackExtension);
      ...
      firstStack = new File(propertyUserDir,
        datasetName + AxisID.ONLY.getExtension() + Extension.EXTENSION_DIVIDER
          + Extension.EXTENSION_DIVIDER + rawImageStackExtension);
      firstStackName = firstStack.getName();
    }
    ...
    for (int i = 0; i < edfFiles.length; i++) {
      MetaData savedMetaData = new MetaData(manager, manager.getLogProperties(), true);
      try {
      ...
      String savedRawStackExtension =
        savedMetaData.getRawImageStackExtension().toString();
      if (savedRawStackExtension == null) {
        // This dataset appears to date from before the naming style upgrade so use the
        // old standard dataset extension.
        savedRawStackExtension = Extension.ST.toString();
      }
      ...
      if (savedAxisType == AxisType.DUAL_AXIS) {
        savedFirstStack =
          new File(propertyUserDir, savedDatasetName + AxisID.FIRST.getExtension()
            + Extension.EXTENSION_DIVIDER + savedRawStackExtension);
  ...
  public String getRawImageStack() {
    return dialog.getRawImageStack();
  }
  ...
  public File getWorkingDirectory() {
    String datasetText = dialog.getRawImageStack();
  ...
  public void initializeFields(ConstMetaData metaData, UserConfiguration userConfig) {
    if (!metaData.getDatasetName().equals("")) {
      ...
      dialog.setRawImageStack(canonicalPath);
    }
    // Parallel processing is optional in tomogram reconstruction, so only use it
    // if the user set it up.
    dialog.setParallelProcess(
      UserEnv.isParallelProcessing(manager, AxisID.ONLY, getPropertyUserDir()));
    dialog.setParallelProcessEnabled(
      UserEnv.isParallelProcessingEnabled(manager, AxisID.ONLY, getPropertyUserDir()));
    dialog.setGpuProcessingEnabled(
      UserEnv.isGpuProcessingEnabled(manager, AxisID.ONLY, getPropertyUserDir()));
    dialog.setGpuProcessing(
      UserEnv.isGpuProcessing(manager, AxisID.ONLY, getPropertyUserDir()));
  ...
  public String getViewsToSkip(final AxisID axisID, final boolean doValidation) {
    return dialog.getViewsToSkip(axisID, doValidation);
  }

  public TiltAngleType getTiltAngleType(final AxisID axisID) {
    if (axisID == AxisID.SECOND) {
      return tiltAnglePanelExpertB.getTiltAngleType();
    }
    return tiltAnglePanelExpertA.getTiltAngleType();
  }
  ...
  void action(final String actionCommand) {
    if (dialog.equalsSingleAxisActionCommand(actionCommand)) {
      setTiltAnglePanelEnabled(AxisID.SECOND, false);
      dialog.updateDisplay(false, false);
    }
  ...
  private void setTooltips() {
    dialog.setRawImageStackTooltip(
    ...
    dialog.setFiducialDiameterTooltip(
