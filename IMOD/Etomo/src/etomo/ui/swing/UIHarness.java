...
public final class UIHarness implements UIComponent {
  ...
  private static final boolean MOVE_B = EtomoDirector.INSTANCE.getArguments().isMoveB();
  ...
  public void openProblemValueMessageDialog(final BaseManager manager,
    ...
    openWarningMessageDialog(manager, uiComponent,
      problem + " '" + paramName + "' parameter " + (paramDescr != null ? paramDescr : "")
        + " value '" + problemValue + "'."
  ...
  public synchronized void openInfoMessageDialog(BaseManager manager, String message,
    String title, AxisID axisID) {
    openInfoMessageDialog(manager, null, message, title, axisID);
  }

  public synchronized void openInfoMessageDialog(final BaseManager manager,
    final UIComponent uiComponent, final String message, final String title,
    final AxisID axisID) {
    if (isHead() && !EtomoDirector.INSTANCE.isTestFailed()) {
  ...
  /**
   * open one dialog and display all messages in process messages.
   *
   * @param message
   * @param title
   */
  public synchronized void openMessageDialog(BaseManager manager, ProcessMessages message,
    String title, AxisID axisID) {
    if (isHead() && !EtomoDirector.INSTANCE.isTestFailed()) {
      getMessageFrame(manager, null).displayMessage(manager, message, title, axisID);
    }
  ...
  public synchronized EtomoBoolean2 openYesNoCancelDialog(BaseManager manager,
    ...
    if (isHead() && !EtomoDirector.INSTANCE.isTestFailed()) {
      int dialogRetValue = getMessageFrame(manager, null)
  ...
  public synchronized boolean openYesNoDialogWithDefaultNo(BaseManager manager,
    ...
    if (isHead() && !EtomoDirector.INSTANCE.isTestFailed()) {
      return getMessageFrame(manager, null).openYesNoDialogWithDefaultNo(manager, message,
        title, axisID);
    }
    log(message, axisID);
    if (EtomoDirector.INSTANCE.isTestFailed()) {
  ...
  public synchronized boolean openDeleteDialog(BaseManager manager, String[] message,
    AxisID axisID) {
    if (isHead() && !EtomoDirector.INSTANCE.isTestFailed()) {
      return getMessageFrame(manager, null).displayDeleteMessage(manager, message,
        axisID);
    }
    log(message, axisID);
    if (EtomoDirector.INSTANCE.isTestFailed()) {
  ...
  public void pack(BaseManager manager) {
    if (isHead()) {
      BusyStatusMediator mediator = null;
      if (manager != null) {
        mediator = manager.getBusyStatusMediator();
        mediator.msgPacking(null, true);
      }
      if (manager != null) {
        manager.pack();
      }
      ...
      if (mediator != null) {
        mediator.msgPacking(null, false);
      }
    }
  }
  ...
  public void pack(AxisID axisID, BaseManager manager) {
    if (isHead()) {
      ...
      if (manager != null) {
        mediator = manager.getBusyStatusMediator();
        mediator.msgPacking(axisID, true);
      }
      manager.pack();
      AbstractFrame abstractFrame = getFrame(manager);
      ...
      // }
      if (mediator != null) {
        mediator.msgPacking(axisID, false);
      }
    }
  }
  ...
  public void exit(final AxisID axisID, final int exitValue) {
    ...
    if (EtomoDirector.INSTANCE.exitProgram(axisID)) {
      ...
      EtomoDirector.INSTANCE.stopMaintainEtomo();
  ...
  private void logMessage(ProcessMessages processMessages, String title, AxisID axisID) {
    logHeader(title, axisID);
    ProcessMessages.ListTypeIterator listTypeIterator =
      processMessages.listTypeIterator();
    while (listTypeIterator.hasNext()) {
      processMessages.print(listTypeIterator.next());
    }
    if (!EtomoDirector.INSTANCE.isUnitTest() || DEBUG) {
      System.err.println();
      System.err.flush();
