...
public final class ParallelPanel implements Expandable, Storable {
  ...
  private final SingleLineButton btnSaveDefaults =
    new SingleLineButton("Save As Defaults");
  private final SpacedPanel bodyPanel = SpacedPanel.getInstance();
  private final EtomoPanel rootPanel = new EtomoPanel();
  private final SingleLineButton btnRestartLoad = new SingleLineButton("Restart Load");
  ...
  private final SingleLineButton btnResume;
  private final SingleLineButton btnPause;
  ...
  static ParallelPanel getInstance(final BaseManager manager, final AxisID axisID,
    ...
    ParallelPanel instance = new ParallelPanel(manager, axisID, state, parent,
      popupChunkWarnings, runnable, interfaceType);
  ...
  private ParallelPanel(final BaseManager manager, final AxisID axisID,
    ...
    if (runnable) {
      btnResume = new SingleLineButton(RESUME_LABEL);
      btnPause = new SingleLineButton("Pause");
    }
    else {
      btnResume = null;
      btnPause = null;
    ...
    header.setSaveMoreLessState(false);
    cpuTable = new CpuTable(manager, this, axisID, runnable, header.getMoreLessButton(),
      interfaceType);
    ...
    queueTable = new QueueTable(manager, this, axisID, runnable,
      header.getMoreLessButton(), interfaceType);
    queueTable.createTable();
    queueTable.setVisible(false);
    gpuTable = new GpuTable(manager, this, axisID, runnable, header.getMoreLessButton(),
      interfaceType);
  ...
  LoadDisplay getSecondaryLoadDisplay() {
  ...
  int getCPUsSelectedInt(final boolean doValidation)
    throws FieldValidationFailedException {
    try {
      return Integer.parseInt(getCPUsSelected(doValidation));
    }
  ...
  ProcessingMethod getSecondaryProcessingMethod() {
  ...
  public boolean getResumeParameters(final ProcesschunksParam param,
    ...
    try {
      ...
      if (cpusSelected.equals(0) && doValidation) {
  ...
  public String getNumberOfProcessors(final boolean doValidation) {
    try {
      ...
      if (!numMachines.isValid() && doValidation) {
        if (numMachines.equals(0)) {
          UIHarness.INSTANCE.openMessageDialog(manager, getNoCpusSelectedErrorMessage(),
            "Unable to run", axisID);
          return null;
        }
        else {
  ...
  public boolean getParameters(final BatchruntomoParam param) {
    ...
    currentTable
      .getParameters(mediator.getRunMethodForParallelPanel(getProcessingMethod()), param);
    if (secondaryTable != null) {
