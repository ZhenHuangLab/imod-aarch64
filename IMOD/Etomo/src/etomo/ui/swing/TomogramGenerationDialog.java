...
public class TomogramGenerationDialog extends ProcessDialog
  implements ContextMenu, TomogramGenerationMethod {
  ...
  private final RadioButton rbBackProjection =
    new RadioButton("Back Projection", bgMethod);
  private final RadioButton rbMultifilt = new RadioButton("Filter Trials", bgMethod);
  ...
  private final MultifiltPanel multifiltPanel;
  private final SirtPanel sirtPanel;
  private final Component tiltPanelRoot;
  private final RadioButton rbMethodPlugin;
  private final PluginPanel methodPluginPanel;

  private TomogramGenerationDialog(final ApplicationManager appMgr,
    final TomogramGenerationExpert expert, final AxisID axisID,
    final TomoGenMethodPlugin methodPlugin) {
    ...
    if (methodPlugin == null || !methodPlugin.hasCustomTiltPanel()) {
      ...
      tiltPanel = methodPlugin.getTiltPanel(this, btnAdvanced);
    }
    multifiltPanel = MultifiltPanel.getInstance(appMgr, axisID, this, dialogType);
    sirtPanel = SirtPanel.getInstance(appMgr, axisID, dialogType, btnAdvanced, this);
    if (methodPlugin != null) {
      methodPluginPanel = methodPlugin.getPanel(this, btnAdvanced);
    }
    else {
      methodPluginPanel = null;
    }
    if (methodPluginPanel != null) {
      rbMethodPlugin = new RadioButton(methodPluginPanel.getButtonTitle(), bgMethod);
    }
    else {
      rbMethodPlugin = null;
    }
  ...
  static TomogramGenerationDialog getInstance(final ApplicationManager appMgr,
    final TomogramGenerationExpert expert, final AxisID axisID,
    final TomoGenMethodPlugin methodPlugin) {
    TomogramGenerationDialog instance =
      new TomogramGenerationDialog(appMgr, expert, axisID, methodPlugin);
  ...
  private void createPanel() {
    ...
    JPanel pnlFilterTrials = new JPanel();
    // init
    ...
    rootPanel.add(multifiltPanel.getComponent());
    rootPanel.add(sirtPanel.getRoot());
    if (methodPluginPanel != null) {
      rootPanel.add(methodPluginPanel.getComponent());
    }
    // method panel
    ...
    pnlMethod.add(Box.createHorizontalGlue());
    pnlMethod.add(rbMultifilt.getComponent());
    pnlMethod.add(Box.createHorizontalGlue());
    pnlMethod.add(rbSirt.getComponent());
    pnlMethod.add(Box.createHorizontalGlue());
    if (rbMethodPlugin != null) {
      pnlMethod.add(rbMethodPlugin.getComponent());
      pnlMethod.add(Box.createHorizontalGlue());
    }
    // buttons
  ...
  private void addListeners() {
    ...
    rbMultifilt.addActionListener(listener);
    rbSirt.addActionListener(listener);
    if (rbMethodPlugin != null) {
      rbMethodPlugin.addActionListener(listener);
    }
  }

  public GlobalExpandButton getAdvancedButton() {
    return btnAdvanced;
  }
  ...
  void sirtCheckpoint(final ConstTiltParam tiltForSirtParam, final TomogramState state) {
    sirtPanel.checkpoint(state);
    if (methodPluginPanel != null) {
      methodPluginPanel.updateDisplay();
    }
  }

  /**
   * This function is called when tilt.com may be updated - and prevents the update if it returns
   * false.  Tilt.com may be updated when either tilt, multifiltsetup, or sirtsetup is
   * run.  A resume for sirt is unusual because the com files are recreated if the user
   * chooses to resume from an iteration.  In this case nothing else can change so this
   * function returns false and tilt.com isn't updated at all.
   * 
   * If the user has switched to Back Projection or Multifilt this function returns true
   * and allow tilt.com to be modified.
   * @return
   */
  ...
  void setParameters(ConstMetaData metaData) {
    ...
    else if (metaData.isGenFilterTrials(axisID)) {
      rbMultifilt.setSelected(true);
    }
    else {
      rbSirt.setSelected(true);
    }
    tiltPanel.setParameters(metaData);
    multifiltPanel.setParameters(metaData);
    sirtPanel.setParameters(metaData);
    methodChanged();
    tiltPanel.setFilterTypeActionListener(multifiltPanel);
  }
  ...
  void setParameters(MultifiltSetupParam param) {
    multifiltPanel.setParameters(param);
  }
  ...
  final void setParameters(ReconScreenState screenState) {
    ...
    multifiltPanel.setParameters(screenState);
    sirtPanel.setParameters(screenState);
    if (methodPluginPanel != null) {
      methodPluginPanel.setParameters(screenState);
    }
  }
  ...
  private void updateAdvanced() {
    ...
    sirtPanel.updateAdvanced(advanced);
    if (methodPluginPanel != null) {
      methodPluginPanel.updateAdvanced(advanced);
    }
  ...
  public void popUpContextMenu(MouseEvent mouseEvent) {
    ...
    boolean filterTrials = rbMultifilt.isSelected();
    int numManpages;
    int numLogs;
    if (!sirtInUse) {
      numManpages = 2;
    ...
    if (!sirtInUse) {
      ...
      if (filterTrials) {
        manPageLabel[i] = "Multifiltsetup";
        manPage[i] = "multifiltsetup.html";
        logFileLabel[i] = "Multifiltsetup";
        logFile[i++] = "multifiltsetup" + axisID.getExtension() + ".log";
      }
    }
    else {
  ...
  MultifiltSetupDisplay getMultifiltSetupDisplay() {
    return multifiltPanel;
  }
  ...
  public boolean isMethodPlugin() {
    if (rbMethodPlugin != null) {
      return rbMethodPlugin.isSelected();
    }
    return false;
  }
  ...
  private void methodChanged() {
    ...
    multifiltPanel.msgMethodChanged(tiltPanel);
    sirtPanel.msgMethodChanged(tiltPanel);
    if (methodPluginPanel != null) {
      methodPluginPanel.msgVisibilityChanged(isMethodPlugin());
    }
    UIHarness.INSTANCE.pack(axisID, applicationManager);
  ...
  private void action(final ActionEvent event) {
    ...
    if (actionCommand.equals(rbBackProjection.getActionCommand())
      || actionCommand.equals(rbMultifilt.getActionCommand())
      || actionCommand.equals(rbSirt.getActionCommand()) || (rbMethodPlugin != null
        && actionCommand.equals(rbMethodPlugin.getActionCommand()))) {
      methodChanged();
    }
  }
