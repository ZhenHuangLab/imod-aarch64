...
import etomo.storage.SimpleDefocusFile;
...
public final class FinalAlignedStackExpert extends ReconUIExpert {
  ...
  public void openDialog() {
      ...
      try {
        ...
        UIHarness.INSTANCE.openMessageDialog(manager,
          "Unable to copy to blend.com to blend_3d_find.com.  Will not be "
            + "able to use findbeads3d when erasing gold.",
          "Etomo Error");
      }
    }
    else {
    ...
    try {
      if (!tilt3dFindCom.exists()) {
        ...
        Utilities.copyFile(
          new File(manager.getPropertyUserDir(), ProcessName.TILT.getComscript(axisID)),
          tilt3dFindCom);
      }
    ...
    if (comScriptMgr.loadGoldEraser(axisID, false)) {
      dialog.setParameters(
        comScriptMgr.getCCDEraserParamFromGoldEraser(axisID, CCDEraserParam.Mode.BEADS));
    }
    ...
    boolean newCtfCorrection = false;
    if (!comScriptMgr.loadCtfCorrection(axisID, false)) {
      newCtfCorrection = true;
    ...
    setParameters(ctfPhaseFlipParam, newCtfCorrection);
    ...
    // updateDialog()
    updateFilter(FileType.ALIGNED_STACK.exists(manager, axisID));
    ...
    if (dialog.isCtfPhaseFlipXAxisTiltEmpty()) {
      comScriptMgr.loadTilt(axisID);
      ConstTiltParam tiltParam = comScriptMgr.getTiltParam(axisID);
      dialog.setTiltComParameters(tiltParam);
    }
  ...
  public boolean startNextProcess(ProcessSeries.Process process,
    ...
    if (process.equals(ProcessName.PROCESSCHUNKS.toString())) {
      processchunks(manager, dialog, processResultDisplay, processSeries,
        process.getSubprocessName().toString() + axisID.getExtension(),
        process.getOutputImageFileKey(), process.getProcessingMethod(), false);
      return true;
    }
    ...
    if (exitState != DialogExitState.CANCEL && !manager.isExiting()
      ...
      if (state.isUseCtfCorrectionWarning(axisID)) {
        ...
        UIHarness.INSTANCE.openMessageDialog(null,
          "To use the CTF correction go back to Final Aligned Stack and press " + "the \""
            + FinalAlignedStackDialog.USE_CTF_CORRECTION_LABEL + "\" button in the "
            + FinalAlignedStackDialog.CTF_TAB_LABEL + " tab.",
      ...
      if (state.isUseErasedStackWarning(axisID)) {
        ...
        UIHarness.INSTANCE.openMessageDialog(null,
          "To use the stack with the erased beads go back to Final Aligned "
            + "Stack and press the \"" + FinalAlignedStackDialog.getUseErasedStackLabel()
            + "\" button in the " + FinalAlignedStackDialog.getErasedStackTabLabel()
            + " tab.",
          "Entry Warning", axisID);
        // Only warn once.
      ...
      if (state.isUseFilteredStackWarning(axisID)) {
        ...
        UIHarness.INSTANCE.openMessageDialog(null,
          "To use the MTF filtered stack go back to Final Aligned "
            + "Stack and press the \"" + FinalAlignedStackDialog.USE_FILTERED_STACK_LABEL
            + "\" button in the " + FinalAlignedStackDialog.MTF_FILTER_TAB_LABEL
            + " tab.",
          "Entry Warning", axisID);
        // Only warn once.
  ...
  private ConstMTFFilterParam updateMTFFilterCom(final boolean doValidation) {
    ...
    try {
      ...
      String inputFileName = FileType.ALIGNED_STACK.getFileName(manager, axisID);
      // was metaData.getDatasetName() + AxisID.ONLY.getExtension() + "_filt.ali";
      String outputFileName = FileType.MTF_FILTERED_STACK.getFileName(manager, axisID);
  ...
  boolean createSimpleDefocusFile(final boolean doValidation) {
    try {
      String expectedDefocusInNanometers =
        Utilities.convertMicronsToNanometers(dialog.getExpectedDefocus(doValidation));
      String phaseShiftInDegrees = dialog.getPhaseShiftInDegrees(doValidation);
      if (SimpleDefocusFile.isUpToDate(manager, axisID, expectedDefocusInNanometers,
        phaseShiftInDegrees)) {
        return true;
      }
      if (!SimpleDefocusFile.writeFile(manager, axisID, expectedDefocusInNanometers,
        phaseShiftInDegrees)) {
  ...
  private ConstSplitCorrectionParam
    updateSplitCorrectionParam(final boolean doValidation) {
    if (dialog == null) {
      return null;
    }
  ...
  boolean getParameters(final SplitCorrectionParam param, final boolean doValidation) {
    try {
      ...
      MRCHeader header = MRCHeader.getInstance(manager.getPropertyUserDir(),
        FileType.ALIGNED_STACK.getFileName(manager, axisID), axisID);
      try {
  ...
  void useCtfCorrection(ProcessResultDisplay processResultDisplay) {
    if (manager.useFileAsFullAlignedStack(processResultDisplay,
      FileType.CTF_CORRECTED_STACK, FinalAlignedStackDialog.CTF_CORRECTION_LABEL, axisID,
      dialogType)) {
      state.setUseCtfCorrectionWarning(axisID, false);
    }
  }
  ...
  void useMtfFilter(ProcessResultDisplay processResultDisplay) {
    ...
    if (FileType.ALIGNED_STACK.getFile(manager, axisID).exists()
      ...
      if (!Utilities.isValidStack(mtfFilteredStack, manager, axisID)) {
        UIHarness.INSTANCE.openMessageDialog(manager,
          mtfFilteredStack.getName() + " is not a valid MRC file.", "Entry Error",
          axisID);
        stopProgressBar(axisID);
        sendMsg(ProcessResult.FAILED_TO_START, processResultDisplay);
        return;
    ...
    try {
      manager.renameImageFile(FileType.MTF_FILTERED_STACK, FileType.ALIGNED_STACK,
        axisID);
    }
    catch (IOException except) {
  ...
  private boolean getParameters(final MTFFilterParam mtfFilterParam,
    final boolean doValidation) throws FortranInputSyntaxException {
      mtfFilterParam.setPixelSize(metaData.getPixelSize()
         * Utilities.getStackBinning(manager, axisID, FileType.ALIGNED_STACK));

    if (dialog != null) {
      return dialog.getParameters(mtfFilterParam, doValidation);
    }
    return false;
  }
  ...
  private void setParameters(ConstCtfPlotterParam param) {
    ...
    dialog.setExpectedDefocus(
      Utilities.convertNanometersToMicrons(param.getExpectedDefocus()));
    dialog.setPhaseShiftInDegrees(param.getPhaseShiftInDegrees());
  ...
  private boolean getParameters(CtfPlotterParam param, final boolean doValidation) {
    ...
    try {
      ...
      param.setExpectedDefocus(
        Utilities.convertMicronsToNanometers(dialog.getExpectedDefocus(doValidation)));
      param.setPhaseShiftInDegrees(dialog.getPhaseShiftInDegrees(doValidation));
  ...
  private void setParameters(ConstCtfPhaseFlipParam param, final boolean initialize) {
    if (dialog == null) {
      return;
    }
    ...
    dialog.setUseExpectedDefocus(
      param.getDefocusFile().endsWith(DatasetFiles.SIMPLE_DEFOCUS_EXT));
    dialog.setInterpolationWidth(param.getInterpolationWidth());
    dialog.setCtfPhaseFlipXAxisTilt(param.getXAxisTilt(), false);
    dialog.setScaleByCtfPower(param.getScaleByCtfPower());
    dialog.setMinimumZeroSpacing(param.getMinimumZeroSpacing());
    dialog.setDefocusTol(param.getDefocusTol());
    dialog.setParameters(param, initialize);
  ...
  private boolean getParameters(CtfPhaseFlipParam param, final boolean doValidation) {
    ...
    try {
      ...
      param.setXAxisTilt(dialog.getCtfPhaseFlipXAxisTilt(doValidation));
      param.setScaleByCtfPower(dialog.getScaleByCtfPower(doValidation));
      param.setMinimumZeroSpacing(dialog.getMinimumZeroSpacing(doValidation));
      param.setDefocusTol(dialog.getDefocusTol(doValidation));
      param.setOutputFileName(FileType.CTF_CORRECTED_STACK.getFileName(manager, axisID));
  ...
  private boolean setParameters(final ConstMTFFilterParam mtfFilterParam) {
    if (dialog != null) {
      dialog.setParameters(mtfFilterParam);
      enableUseFilter();
      return true;
    }
    return false;
