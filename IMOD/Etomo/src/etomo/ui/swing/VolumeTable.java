 ...
 * <p> </p>
 */
final class VolumeTable extends HighlightableTable
  implements Expandable, Run3dmodButtonContainer, Viewable {
  ...
  private static final String UNIQUE_KEY = "Volume";

  private final RowList rowList = new RowList();
  private final JPanel rootPanel = new JPanel();
  private final CheckBox cbFlgVolNamesAreTemplates =
    new CheckBox(SharedStrings.FLG_VOL_NAMES_ARE_TEMPLATES_LABEL);
  private final SingleLineButton btnReadTiltFile = new SingleLineButton("Read tilt file");
  private final Run3dmodSingleLineButton r3bVolume;
  ...
  private final HeaderCell header1TiltRangeMultiAxes =
    new HeaderCell(TILT_RANGE_MULTI_AXES_HEADER1_LABEL);
  ...
  private final HeaderCell header2TiltRangeStart =
    new HeaderCell("Min", UIParameters.getInstance().getNumericWidth());
  private final HeaderCell header2TiltRangeEnd =
    new HeaderCell("Max", UIParameters.getInstance().getNumericWidth());
  private final HeaderCell header2TiltRangeMultiAxes =
    new HeaderCell(TILT_RANGE_MULTI_AXES_HEADER2_LABEL);
  ...
  private final SingleLineButton btnInsertRow =
    SingleLineButton.getHtmlInstance("Insert");
  private final SingleLineButton btnDeleteRow =
    SingleLineButton.getHtmlInstance("Delete");
  ...
  private final Viewport viewport = new Viewport(this,
    EtomoDirector.INSTANCE.getUserConfiguration().getPeetTableSize().getInt(),
    UNIQUE_KEY);

  private final VolumeFileFilter volumeFileFilter;
  ...
  private VolumeTable(final PeetManager manager, final PeetDialog parent) {
    ...
    this.parent = parent;
    // construction
    volumeFileFilter = VolumeFileFilter.getInstance(manager);
    focusableParents = new JComponent[] { parent.getSetupJComponent() };
    ...
    r3bVolume = Run3dmodSingleLineButton.get3dmodInstance("Open in 3dmod", this);
    createPanel();
    updateDisplay();
    setToolTipText();
  }
  ...
  void highlightDownActionPerformed() {
    ...
    boolean adjustViewport = viewport.inViewport(rowList.getHighlightedRowIndex());
  ...
  private void refreshHorizontalPadding() {
    ...
    if (maxRowTextSize <= 50) {
      ...
      horizontalRigidArea1 = Box.createRigidArea(new Dimension(
        Math.max((int) Math.round(181 - (maxRowTextSize - 50) * 6.3), 8), 0));
    }
  ...
  public void action(final String command, final Deferred3dmodButton deferred3dmodButton,
    ...
    if (command.equals(btnInsertRow.getActionCommand())) {
      ...
      parent.msgFlgVolNamesAreTemplates(false, cbFlgVolNamesAreTemplates.isSelected());
    }
  }
  ...
  private void setToolTipText() {
    ...
    r3bVolume
      .setToolTipText("Open the volume and model for the highlighted row in " + "3dmod.");
   ...
   */
  private void copyRow(final boolean init) {
    addRow(rowList.getHighlightedRow());
    viewport.adjustViewport(rowList.size() - 1);
    rowList.remove();
    rowList.display(viewport);
    updateDisplay();
    parent.msgVolumeTableSizeChanged(init);
    UIHarness.INSTANCE.pack(manager);
  }
   ...
   */
  private void insertRow(final boolean init) {
    if (!manager.setParamFile()) {
      UIHarness.INSTANCE.openMessageDialog(manager,
        "Please set the " + PeetDialog.DIRECTORY_LABEL + " and "
          + PeetDialog.FN_OUTPUT_LABEL + " fields before adding rows.",
        "Entry Error");
      return;
    }
    addRow();
  ...
  private void updateDisplay() {
    ...
    btnMoveDown.setEnabled(
      enable && highlighted && rowList.getHighlightedRowIndex() < rowList.size() - 1);
  ...
  private static final class RowList {
    ...
    private synchronized VolumeRow add(final BaseManager manager, final VolumeTable table,
      final JPanel panel, final GridBagLayout layout,
    ...
    private int highlightDown() {
      int index = getHighlightedRowIndex();
      if (index < 0) {
        return -1;
      }
      index++;
