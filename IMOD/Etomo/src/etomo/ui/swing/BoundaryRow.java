...
final class BoundaryRow {
  ...
  private static final String INVERTED_TOOLTIP =
    "This value comes from an inverted section.";
  private static final String EMPTY_SLICE_WARNING =
    "Empty slices will be added to the section.";
  ...
  BoundaryRow(int key, ConstJoinMetaData metaData, JoinScreenState screenState,
    ...
    // origEnd
    SectionTableRowData data =
      (SectionTableRowData) metaData.getSectionTableData().get(key - 1);
  ...
  void setXfjointomoResult(BaseManager manager)
  ...
  private static final class Gap {
    ...
    private static final class GapBoundary {
      ...
      int move(int adjusted) {
        ...
        if ((change > 0 && !leftSide && !inverted) || (change > 0 && leftSide && inverted)
