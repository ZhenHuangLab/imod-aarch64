...
* <p> $Log$ </p>
*/
final class AutoAlignmentPanel implements Run3dmodButtonContainer {
  ...
  private final LabeledTextField ltfSigmaLowFrequency =
    new LabeledTextField(FieldType.FLOATING_POINT, "Sigma for low-frequency filter: ");
  private final LabeledTextField ltfCutoffHighFrequency =
    new LabeledTextField(FieldType.FLOATING_POINT, "Cutoff for high-frequency filter: ");
  private final LabeledTextField ltfSigmaHighFrequency =
    new LabeledTextField(FieldType.FLOATING_POINT, "Sigma for high-frequency filter: ");

  private final SpacedPanel pnlButtons = SpacedPanel.getInstance();
  private final MultiLineButton btnInitialAutoAlignment =
    new MultiLineButton("Initial Auto Alignment");
  private final MultiLineButton btnMidas = new MultiLineButton("Midas");
  private final MultiLineButton btnRefineAutoAlignment =
    new MultiLineButton("Refine with Auto Alignment");
  private final MultiLineButton btnRevertToMidas =
    new MultiLineButton("Revert Auto Alignment to Midas");
  private final MultiLineButton btnRevertToEmpty =
    new MultiLineButton("Revert to No Transforms");
  private final LabeledSpinner spReduceByBinning =
    LabeledSpinner.getDefaultedInstance("Binning in search: ", 2, 1, 50, 1, 1);
  private final LabeledTextField ltfSkipSectionsFrom1 =
    new LabeledTextField(FieldType.INTEGER_LIST, "Sections to skip: ");
  private final CheckBox cbPreCrossCorrelation =
    new CheckBox("Find initial shifts with cross-correlation");
  private final LabeledTextField ltfEdgeToIgnore =
    new LabeledTextField(FieldType.FLOATING_POINT, "Fraction to ignore on edges: ");
  private final Spinner spMidasBinning =
    Spinner.getLabeledInstance("Binning in Midas: ", 1, 1, 8);
  ...
  private AutoAlignmentPanel(final BaseManager manager, final boolean joinInterface) {
    ...
    if (joinInterface) {
      ...
      ltfWarpPatchSizeX =
        new LabeledTextField(FieldType.INTEGER, "Correlation patch size in X: ");
      ltfWarpPatchSizeY = new LabeledTextField(FieldType.INTEGER, " Y: ");
      cbBoundaryModel = new CheckBox("Use boundary model:");
      btnBoundaryModel =
        Run3dmodButton.get3dmodInstance("Create/View Boundary Model", this);
      ltfShiftLimitsForWarpX =
        new LabeledTextField(FieldType.INTEGER, "Limits to shifts in X: ");
  ...
  private void createPanel(final boolean joinConfiguration) {
    ...
    if (pnlWarping != null) {
      ...
      pnlShiftLimitsForWarp
        .setLayout(new BoxLayout(pnlShiftLimitsForWarp, BoxLayout.X_AXIS));
    ...
    pnlPreCrossCorrelation
      .setLayout(new BoxLayout(pnlPreCrossCorrelation, BoxLayout.X_AXIS));
   ...
   * @param input
   */
  void setController(final AutoAlignmentController input) {
    controller = input;
    addListeners();
  }
  ...
  boolean getParameters(final AutoAlignmentMetaData metaData,
  ...
  boolean getParameters(final XfalignParam param, final boolean doValidation) {
    try {
  ...
  void getParameters(final MidasParam param) {
   ...
   * @return
   */
  public boolean equals(final AutoAlignmentMetaData metaData) {
    ...
    if (cbFindWarping != null) {
      ...
      if (cbSobelFilter != null
        && cbSobelFilter.isSelected() != metaData.isSobelFilter()) {
        return false;
      }
    }
    return true;
   ...
   */
  void msgProcessChange(final boolean processEnded) {
  ...
  private void setTooltips() {
    ...
    try {
      autodoc =
        AutodocFactory.getInstance(manager, AutodocFactory.XFALIGN, AxisID.ONLY, false);
    }
    ...
    if (autodoc != null) {
      cbPreCrossCorrelation
        .setToolTipText("Use cross-correlation to find initial translations; needed if "
          ...
          + "alignment");
      ltfEdgeToIgnore.setToolTipText(
        EtomoAutodoc.getTooltip(autodoc, XfalignParam.EDGE_TO_IGNORE_KEY));
      spReduceByBinning.setToolTipText(
        EtomoAutodoc.getTooltip(autodoc, XfalignParam.REDUCE_BY_BINNING_KEY));
      ltfSkipSectionsFrom1.setToolTipText(EtomoAutodoc.getTooltip(autodoc,
        XfalignParam.SKIP_SECTIONS_KEY) + "  Also sets the "
        + XfalignParam.SECTIONS_NUMBERED_FROM_ONE_KEY + " option:  "
        + EtomoAutodoc.getTooltip(autodoc, XfalignParam.SECTIONS_NUMBERED_FROM_ONE_KEY));
    }
  }
