 ...
 * dialog.</p>
 * 
 * <p>Copyright: Copyright 2006 - 2019 by the Regents of the University of Colorado</p>
 * 
 ...
 * <p> </p>
 */
public final class AnisotropicDiffusionDialog
  implements ContextMenu, AbstractParallelDialog, Run3dmodButtonContainer,
  FilterFullVolumeParent, ProcessInterface {
  public static final String CLEANUP_LABEL = FilterFullVolumePanel.CLEANUP_LABEL;
  public static final String FILTER_FULL_VOLUME_LABEL =
    FilterFullVolumePanel.FILTER_FULL_VOLUME_LABEL;
  public static final int MEMORY_PER_CHUNK_DEFAULT =
    FilterFullVolumePanel.MEMORY_PER_CHUNK_DEFAULT;
  public static final String MEMORY_PER_CHUNK_LABEL =
    FilterFullVolumePanel.MEMORY_PER_CHUNK_LABEL;
  ...
  /**
   * Replaced with testVolumeName.
   * @deprecated 7/13/2020
   */
  public static final String TEST_VOLUME_NAME = "test.input";

  private final SpacedPanel rootPanel = SpacedPanel.getInstance();
  private final Run3dmodButton btnViewFullVolume =
    Run3dmodButton.get3dmodInstance("View Full Volume", this);
  private final FileTextField ftfVolume =
    FileTextField.getPartialPathInstance("Pick a volume");
  private final MultiLineButton btnExtractTestVolume =
    new MultiLineButton("Extract Test Volume");
  private final Run3dmodButton btnViewTestVolume =
    Run3dmodButton.get3dmodInstance("View Test Volume", this);
  private final CheckBox cbLoadWithFlipping = new CheckBox("Load with flipping");
  private final LabeledTextField ltfTestKValueList =
    new LabeledTextField(FieldType.FLOATING_POINT_ARRAY, K_VALUE_LIST_LABEL);
  private final Spinner spTestIteration =
    Spinner.getLabeledInstance(ITERATION_LABEL, 10, 1, 200);
  private final Run3dmodButton btnRunVaryingK =
    Run3dmodButton.getDeferred3dmodInstance("Run with Different K Values", this);
  private final Run3dmodButton btnViewVaryingK =
    Run3dmodButton.get3dmodInstance("View Different K Values Test Results", this);
  private final LabeledTextField ltfTestKValue =
    new LabeledTextField(FieldType.FLOATING_POINT, K_VALUE_LABEL);
  private final LabeledTextField ltfTestIterationList =
    new LabeledTextField(FieldType.INTEGER_LIST, ITERATION_LIST_LABEL);
  private final Run3dmodButton btnRunVaryingIteration =
    Run3dmodButton.getDeferred3dmodInstance("Run with Different Iterations", this);
  private final Run3dmodButton btnViewVaryingIteration =
    Run3dmodButton.get3dmodInstance("View Different Iteration Test Results", this);
  ...
  private void setToolTipText() {
    ...
    btnViewVaryingK
      .setToolTipText("View the volumes computed with different K values in " + "3dmod.");
    ltfTestKValue.setToolTipText(
    ...
    btnViewVaryingIteration.setToolTipText(
  ...
  public void popUpContextMenu(MouseEvent mouseEvent) {
    String[] manPagelabel =
      { "Anisotropic Diffusion", "3dmod", "Processchunks", "Chunksetup" };
  ...
  private AnisotropicDiffusionDialog(final ParallelManager manager) {
    ...
    testVolumeName = FileType.NAD_TEST_INPUT.getFileName(manager, AxisID.ONLY);
    // root
    ...
    // init
    ltfTestKValueList.setRequired(true);
    ltfTestIterationList.setRequired(true);
    ltfTestKValue.setRequired(true);
    // first column
    ...
    pnlTestVolumeRubberband = RubberbandPanel.getInstance(manager, ImodManager.VOLUME_KEY,
      "Test Volume Range:", "Get Test Volume Range from 3dmod",
    ...
    pnlVaryingIteration.setBorder(
      new EtchedBorder("Find an Iteration Number for Test Volume").getBorder());
  ...
  public boolean getParameters(TrimvolParam param, final boolean doValidation) {
    ...
    param.setOutputFileName(new File(subdirName, testVolumeName).getPath());
    return true;
  }
  ...
  public boolean getParametersForVaryingK(AnisotropicDiffusionParam param,
    ...
    try {
      ...
      // incorrect result.
      if (!(new File(new File(manager.getPropertyUserDir(), subdirName), testVolumeName)
        .exists())) {
      ...
      param.setInputFileName(testVolumeName);
      return true;
    }
  ...
  public boolean getParameters(AnisotropicDiffusionParam param,
    final boolean doValidation) {
    param.setSubdirName(subdirName);
  ...
  public void getParameters(ChunksetupParam param) {
    ...
    param.setOutputFile(
      FileType.ANISOTROPIC_DIFFUSION_OUTPUT.getFileName(manager, AxisID.ONLY));
  }
  ...
  public void action(final String command, Deferred3dmodButton deferred3dmodButton,
    ...
    if (command.equals(btnExtractTestVolume.getActionCommand())) {
      ...
      manager.imod(ImodManager.TEST_VOLUME_KEY, new File(subdirName, testVolumeName),
      ...
      manager.imodVaryingKValue(ImodManager.VARYING_K_TEST_KEY, run3dmodMenuOptions,
        subdirName, testVolumeName, cbLoadWithFlipping.isSelected());
    }
      ...
      manager.imodVaryingIteration(ImodManager.VARYING_ITERATION_TEST_KEY,
        run3dmodMenuOptions, subdirName, testVolumeName, cbLoadWithFlipping.isSelected());
    }
  }
  ...
  private void openVolume() {
    ...
    chooser.setFileFilter(TomogramFileFilter.getAllImageFilenameStyleInstance(manager));
