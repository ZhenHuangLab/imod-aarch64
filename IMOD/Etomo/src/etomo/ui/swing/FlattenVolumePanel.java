...
final class FlattenVolumePanel implements Run3dmodButtonContainer, WarpVolDisplay,
  ...
  private final BinnedXY3dmodButton btnMakeSurfaceModel =
    new BinnedXY3dmodButton("Make Surface Model", this);
  private final CheckBox cbOneSurface = new CheckBox("Contours are all on one surface");
  private final LabeledTextField ltfWarpSpacingX =
    new LabeledTextField(FieldType.FLOATING_POINT, WARP_SPACING_X_LABEL + ": ");
  private final LabeledTextField ltfWarpSpacingY =
    new LabeledTextField(FieldType.FLOATING_POINT, " " + WARP_SPACING_Y_LABEL + ": ");
  ...
  private final RadioButton rbInputFileTrimVol =
    new RadioButton("Flatten the trimvol output", bgInputFile);
  private final RadioButton rbInputFileSqueezeVol =
    new RadioButton("Flatten the squeezevol output", bgInputFile);
  private final CheckBox cbInterpolationOrderLinear =
    new CheckBox("Linear interpolation");
  private final LabeledTextField ltfOutputSizeZ =
    new LabeledTextField(FieldType.INTEGER, OUTPUT_SIZE_Z_LABEL + ": ");
  private final Run3dmodButton btnImodFlatten =
    Run3dmodButton.get3dmodInstance("Open Flattened Tomogram", this);
  private final FileTextField ftfTemporaryDirectory =
    new FileTextField("Temporary directory:");
  ...
  private FlattenVolumePanel(final ApplicationManager manager, final AxisID axisID,
    ...
    btnFlatten =
      (Run3dmodButton) manager.getProcessResultDisplayFactory(axisID).getFlatten();
    btnFlattenWarp =
      (MultiLineButton) manager.getProcessResultDisplayFactory(axisID).getFlattenWarp();
  ...
  public void popUpContextMenu(MouseEvent mouseEvent) {
    ...
    ContextPopup contextPopup = new ContextPopup(pnlRoot.getContainer(), mouseEvent,
      "Flattening", ContextPopup.TOMO_GUIDE, manPagelabel, manPage, logFileLabel, logFile,
      applicationManager == null ? toolsManager : applicationManager, axisID);
  }
  ...
  private boolean validateFlattenWarp() {
    ...
    if (lambdaForSmoothing == null || lambdaForSmoothing.matches("\\s*")) {
      UIHarness.INSTANCE.openMessageDialog(manager,
        LAMBDA_FOR_SMOOTHING_LABEL + " is a required field.", "Entry Error", axisID);
      return false;
    }
    return true;
  }
  ...
  public boolean getParameters(final FlattenWarpParam param, final boolean doValidation) {
    try {
      String errorMessage =
        param.setLambdaForSmoothing(ltfLambdaForSmoothing.getText(doValidation));
      if (errorMessage != null) {
        UIHarness.INSTANCE.openMessageDialog(manager,
          "Error in " + LAMBDA_FOR_SMOOTHING_LABEL + ":  " + errorMessage, "Entry Error",
          axisID);
        return false;
      }
      ...
      if (errorMessage != null) {
        UIHarness.INSTANCE.openMessageDialog(manager,
          "Error in " + WARP_SPACING_X_LABEL + ":  " + errorMessage, "Entry Error",
          axisID);
        return false;
      }
      ...
      if (errorMessage != null) {
        UIHarness.INSTANCE.openMessageDialog(manager,
          "Error in " + WARP_SPACING_Y_LABEL + ":  " + errorMessage, "Entry Error",
          axisID);
        return false;
      }
      return true;
  ...
  public boolean getParameters(final WarpVolParam param, final boolean doValidation) {
    try {
      ...
      if (errorMessage != null) {
        UIHarness.INSTANCE.openMessageDialog(manager,
          "Error in " + OUTPUT_SIZE_Z_LABEL + ":  " + errorMessage, "Entry Error",
          axisID);
        return false;
      }
      ...
      if (panelId == PanelId.POST_FLATTEN_VOLUME) {
        ...
        param
          .setOutputFile(FileType.FLATTEN_TOOL_OUTPUT.getFileName(manager, AxisID.ONLY));
      }
  ...
  ImageFileType getInputFileType() {
    if (panelId == PanelId.POST_FLATTEN_VOLUME) {
      if (rbInputFileTrimVol.isSelected()) {
  ...
  private void inputFileAction() {
    ...
    chooser.setFileFilter(TomogramFileFilter.getInstance(manager));
  ...
  private void checkRotated(File file) {
    ...
    if (rotated == null) {
      UIHarness.INSTANCE.openMessageDialog(manager,
        "The MRC header of this file, " + file.getAbsolutePath() + ", is unreadable.",
        "Warning", axisID);
    }
    else if (!rotated.is()) {
      UIHarness.INSTANCE.openMessageDialog(manager,
  ...
  private void setToolTipText() {
    ...
    try {
      flattenWarpAutodoc =
        AutodocFactory.getInstance(manager, AutodocFactory.FLATTEN_WARP, axisID, false);
      warpVolAutodoc =
        AutodocFactory.getInstance(manager, AutodocFactory.WARP_VOL, axisID, false);
    }
    ...
    if (flattenWarpAutodoc != null) {
      ...
      cbOneSurface.setToolTipText(
        EtomoAutodoc.getTooltip(flattenWarpAutodoc, FlattenWarpParam.ONE_SURFACE_OPTION));
    ...
    if (warpVolAutodoc != null) {
      rbInputFileTrimVol.setToolTipText(
        EtomoAutodoc.getTooltip(warpVolAutodoc, WarpVolParam.INPUT_FILE_OPTION));
      rbInputFileSqueezeVol.setToolTipText(
        EtomoAutodoc.getTooltip(warpVolAutodoc, WarpVolParam.INPUT_FILE_OPTION));
      cbInterpolationOrderLinear.setToolTipText(
        EtomoAutodoc.getTooltip(warpVolAutodoc, WarpVolParam.INTERPOLATION_ORDER_OPTION));
      ltfOutputSizeZ.setToolTipText(
        EtomoAutodoc.getTooltip(warpVolAutodoc, WarpVolParam.OUTPUT_SIZE_X_Y_Z_OPTION));
      ftfTemporaryDirectory.setToolTipText(
        EtomoAutodoc.getTooltip(warpVolAutodoc, WarpVolParam.TEMPORARY_DIRECTORY_OPTION));
    }
    btnMakeSurfaceModel.setButtonToolTipText(
