...
public final class FrontPageDialog {
  ...
  private final MultiLineButton btnRecon =
    MultiLineButton.getDebugInstance(EtomoMenu.RECON_LABEL);
  ...
  private final MultiLineButton btnBatchRunTomo =
    new MultiLineButton(EtomoMenu.BATCH_RUN_TOMO_LABEL);
  ...
  private final MultiLineButton btnSerialSections =
    new MultiLineButton(EtomoMenu.SERIAL_SECTIONS_LABEL);
  private final MultiLineButton btnFlattenVolume =
    new MultiLineButton(EtomoMenu.FLATTEN_VOLUME_LABEL);
  private final MultiLineButton btnGpuTiltTest =
    new MultiLineButton(EtomoMenu.GPU_TILT_TEST_LABEL);
  ...
  public static FrontPageDialog getInstance(final BaseManager manager,
    final AxisID axisID) {
  ...
  private void createPanel() {
    ...
    JPanel pnlBatchRunTomo = new JPanel();
    // root panel
    ...
    pnlRoot.add(Box.createRigidArea(FixedDim.x0_y7));
    pnlRoot.add(pnlBatchRunTomo);
    ...
    // BatchRunTomo
    pnlBatchRunTomo.setLayout(new BoxLayout(pnlBatchRunTomo, BoxLayout.X_AXIS));
    pnlBatchRunTomo.add(Box.createHorizontalGlue());
    pnlBatchRunTomo.add(btnBatchRunTomo.getComponent());
    pnlBatchRunTomo.add(Box.createHorizontalGlue());
  }
