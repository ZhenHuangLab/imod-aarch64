 ...
 * <p>Description: </p>
 * 
 * <p>Copyright: Copyright 2009 - 2016 by the Regents of the University of Colorado</p>
 * 
...
abstract class NewstackOrBlendmont3dFindPanel implements Run3dmodButtonContainer {
  ...
  private final ActionListener actionListener =
    new NewstackOrBlendmont3dFindPanelActionListener(this);
  private final LabeledSpinner spinBinning = LabeledSpinner
    .getInstance(NewstackAndBlendmontParamPanel.BINNING_LABEL + ": ", 1, 1, 32, 1);
  private final Run3dmodButton btn3dmodFull =
    Run3dmodButton.get3dmodInstance("View Full Aligned Stack", this);
  ...
  void initialize() {
    ...
    if (!beadSize.isNull() && beadSize.isValid()) {
      int binning = Math.max((int) Math.round((beadSize.getDouble() / 5f)), 1);
      //Adjust the binning if the bead size is too small.
      if (binning > 1 && beadSize.getDouble() / binning < 4) {
        binning--;
      }
