...
import etomo.plugin.PluginFactory;
...
public final class TomogramGenerationExpert extends ReconUIExpert {
  ...
  private TomoGenMethodPlugin methodPlugin = null;

  public TomogramGenerationExpert(ApplicationManager manager, MainTomogramPanel mainPanel,
  ...
  public void openDialog() {
    ...
    if (methodPlugin == null) {
      if (EtomoDirector.INSTANCE.getArguments().isPlugin()) {
        methodPlugin = PluginFactory.loadDemoPlugin(manager.getMainPanel());
      }
      else {
        methodPlugin = PluginFactory.loadTomoGenMethodPlugin(manager.getMainPanel());
      }
      if (methodPlugin != null) {
        methodPlugin.init(manager, axisID, manager.getBaseMetaData().getAxisType(),
          dialogType);
        methodPlugin.initTomoGenMethod(manager, this);
      }
    }
    dialog = TomogramGenerationDialog.getInstance(manager, this, axisID, methodPlugin);
    ...
    if (!comScriptMgr.loadSirtsetup(axisID)) {
      MakecomfileParam makecomFileParam =
        new MakecomfileParam(manager, axisID, FileType.SIRTSETUP_COMSCRIPT);
      manager.makecomfile(axisID, makecomFileParam);
      comScriptMgr.loadSirtsetup(axisID);
    }
    sirtsetupParam = comScriptMgr.getSirtsetupParam(axisID);
    setParameters(sirtsetupParam);
    if (methodPlugin != null) {
      methodPlugin.setParameters();
    }
    ...
    // MultifiltSetup
    if (!comScriptMgr.loadMultifiltSetup(axisID, false)) {
      BaseProcessManager.touch(
        FileType.MULTIFILT_SETUP_COMSCRIPT.getFile(manager, axisID).getAbsolutePath(),
        manager);
      comScriptMgr.loadMultifiltSetup(axisID, true);
    }
    MultifiltSetupParam multifiltSetupParam = comScriptMgr.getMultifiltSetupParam(axisID);
    setParameters(multifiltSetupParam);
    //
  ...
  private void sirtCheckpoint() {
    if (dialog == null) {
      return;
    }
    comScriptMgr.loadTiltForSirt(axisID);
    dialog.sirtCheckpoint(comScriptMgr.getTiltParamFromTiltForSirt(axisID), state);
  }

  public void msgSirtSucceeded(final ProcessSeries processSeries) {
    if (dialog != null) {
  ...
  public boolean startNextProcess(ProcessSeries.Process process,
    ...
    if (process.equals(ProcessName.PROCESSCHUNKS.toString())) {
      ...
      else if (process.getSubprocessName() == ProcessName.TILT_MULTIFILT) {
        processchunks(manager, dialog, processResultDisplay, processSeries,
          ProcessName.TILT_MULTIFILT.toString() + axisID.getExtension(),
          process.getOutputImageFileKey(), process.getProcessingMethod(), false);
      }
      else {
        processchunks(manager, dialog, processResultDisplay, processSeries,
    ...
    if (process.equals(SIRT_DONE)) {
      msgSirtSucceeded(processSeries);
      return true;
    }
    return false;
  }
  ...
  void saveDialog() {
    ...
    manager.updateMultifiltSetupCom(dialog.getMultifiltSetupDisplay(), axisID, false);
    try {
  ...
  private void setParameters(final MultifiltSetupParam param) {
    if (dialog == null) {
      return;
    }
