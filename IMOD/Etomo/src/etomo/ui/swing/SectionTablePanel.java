 ...
 * <p> </p>
 */
final class SectionTablePanel extends HighlightableTable
  ...
  private static final String UNIQUE_KEY = "Section";
  ...
  private final MultiLineButton btnMoveSectionUp =
    new MultiLineButton(true, "Move Section Up");
  private final MultiLineButton btnMoveSectionDown =
    new MultiLineButton(true, "Move Section Down");
  private final MultiLineButton btnAddSection = new MultiLineButton(true, "Add Section");
  private final MultiLineButton btnDeleteSection =
    new MultiLineButton(true, "Delete Section");
  private final MultiLineButton btnGetAngles =
    new MultiLineButton(true, "Get Angles from Slicer");
  private final MultiLineButton btnInvertTable =
    new MultiLineButton(true, "Invert Table");
  // first header row
  private final HeaderCell header1ZOrder = new HeaderCell("Z Order");
  private final HeaderCell header1SetupSections =
    new HeaderCell(HEADER1_SECTIONS_LABEL, UIParameters.getInstance().getSectionsWidth());
  private ExpandButton button1ExpandSections = null;
  private final HeaderCell header1JoinSections =
    new HeaderCell(HEADER1_SECTIONS_LABEL, UIParameters.getInstance().getSectionsWidth());
  ...
  private final HeaderCell header3SampleBottomStart =
    new HeaderCell("Start", UIParameters.getInstance().getNumericWidth());
  private final HeaderCell header3SampleBottomEnd =
    new HeaderCell("End", UIParameters.getInstance().getNumericWidth());
  private final HeaderCell header3SampleTopStart =
    new HeaderCell("Start", UIParameters.getInstance().getNumericWidth());
  private final HeaderCell header3SampleTopEnd =
    new HeaderCell("End", UIParameters.getInstance().getNumericWidth());
  private final HeaderCell header3SetupFinalStart =
    new HeaderCell("Start", UIParameters.getInstance().getNumericWidth());
  private final HeaderCell header3SetupFinalEnd =
    new HeaderCell("End", UIParameters.getInstance().getNumericWidth());
  private final HeaderCell header3JoinFinalStart =
    new HeaderCell("Start", UIParameters.getInstance().getNumericWidth());
  private final HeaderCell header3JoinFinalEnd =
    new HeaderCell("End", UIParameters.getInstance().getNumericWidth());
  private final HeaderCell header3RotationX =
    new HeaderCell("X", UIParameters.getInstance().getNumericWidth());
  private final HeaderCell header3RotationY =
    new HeaderCell("Y", UIParameters.getInstance().getNumericWidth());
  private final HeaderCell header3RotationZ =
    new HeaderCell("Z", UIParameters.getInstance().getNumericWidth());
  ...
  private final Viewport viewport = new Viewport(this,
    EtomoDirector.INSTANCE.getUserConfiguration().getJoinTableSize().getInt(),
    UNIQUE_KEY);
  ...
  SectionTablePanel(final JoinDialog joinDialog, final JoinManager manager,
    ...
    focusableParents = new JComponent[] { joinDialog.getSetupTabJComponent(),
  ...
  public String getChunkSizes() {
    StringBuilder chunkSizes = new StringBuilder();
    for (int i = 0; i < size(); i++) {
      int currRowChunkSize = rowList.get(i).getData().getJoinFinalEnd().getInt()
        - rowList.get(i).getData().getJoinFinalStart().getInt() + 1;
      chunkSizes.append(String.valueOf(currRowChunkSize));
      if (i < (size() - 1)) {
        chunkSizes.append(",");
      }
    }
    return chunkSizes.toString();
  }
    ...
    if (invertedCount > rowList.size() / 2) {
      uiHarness.openMessageDialog(manager,
        "Most of the sections in this join will be inverted.  "
          + SectionTableRow.INVERTED_WARNING + "  If you don't want these inversions, "
  ...
  void setMode(final int mode) {
    ...
    enableRowButtons(rowList.getHighlightedIndex());
  ...
  private void enableRowButtons(final int highlightedRowIndex) {
    ...
    if (mode != JoinDialog.SAMPLE_PRODUCED_MODE) {
      ...
      btnMoveSectionDown
        .setEnabled(highlightedRowIndex > -1 && highlightedRowIndex < rowsSize - 1);
  ...
  public boolean equals(final ConstJoinMetaData metaData) {
  ...
  private void moveSectionDown() {
    ...
    if (index == rowList.size() - 1) {
      uiHarness.openMessageDialog(manager, "Can't move the row down.  Its at the bottom.",
        "Wrong Row", AxisID.ONLY);
      return;
    }
  ...
  private void addSection() {
    ...
    FileChooser chooser = new FileChooser(manager, lastLocation);
    chooser.setDialogTitle("Choose a section");
    TomogramFileFilter tomogramFilter =
      TomogramFileFilter.getAllImageFilenameStyleInstance(manager);
  ...
  private boolean readHeader(final MRCHeader header) {
    try {
      ...
      if (header.getNRows() == -1 || header.getNSections() == -1) {
        String msgIOException[] = { "The header command returned an error (IOException).",
  ...
  void addSection(final File tomogram) {
    ...
    if (!tomogram.isFile()) {
      uiHarness.openMessageDialog(manager, tomogram.getAbsolutePath() + " is not a file.",
        "File Error", AxisID.ONLY);
      return;
    }
  ...
  private void imodSection(final Run3dmodMenuOptions menuOptions) {
    int rowIndex = rowList.getHighlightedIndex();
    if (rowIndex == -1) {
      return;
    }
  ...
  private void setToolTipText() {
    ...
    header2SampleBottom.setToolTipText(
    ...
    header3SampleBottomStart.setToolTipText(
    ...
    header3SampleBottomEnd.setToolTipText(
    ...
    header3SampleTopEnd.setToolTipText(
  ...
  private static final class RowList {
    ...
    private void highlight(final int index) {
      ...
      ((SectionTableRow) list.get(index)).selectHighlightButton();
    }
    ...
    public boolean equals(final ArrayList array) {
