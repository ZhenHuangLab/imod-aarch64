 ...
 * <p>Description: </p>
 * 
 * <p>Copyright: Copyright 2010 - 2019 by the Regents of the University of Colorado</p>
 * 
...
final class FilterFullVolumePanel implements Run3dmodButtonContainer {
  ...
  private final Run3dmodButton btnRunFilterFullVolume =
    Run3dmodButton.getDeferred3dmodInstance(FILTER_FULL_VOLUME_LABEL, this);
  private final LabeledTextField ltfKValue =
    new LabeledTextField(FieldType.FLOATING_POINT, "K value: ");
  private final Spinner spIteration =
    Spinner.getLabeledInstance("Iterations: ", 10, 1, 200);
  private final Spinner spMemoryPerChunk =
    Spinner.getLabeledInstance(MEMORY_PER_CHUNK_LABEL + " (MB): ",
      MEMORY_PER_CHUNK_DEFAULT, ChunksetupParam.MEMORY_TO_VOXEL,
      30 * ChunksetupParam.MEMORY_TO_VOXEL, ChunksetupParam.MEMORY_TO_VOXEL);
  private final Run3dmodButton btnViewFilteredVolume =
    Run3dmodButton.get3dmodInstance("View Filtered Volume", this);
  private final MultiLineButton btnCleanup = new MultiLineButton(CLEANUP_LABEL);
  private final CheckBox cbOverlapTimesFour =
    new CheckBox("Overlap chunks by 4 times # of iterations");
  ...
  static FilterFullVolumePanel getInstance(final ParallelManager manager,
    final DialogType dialogType, final FilterFullVolumeParent parent) {
    FilterFullVolumePanel instance =
      new FilterFullVolumePanel(manager, dialogType, parent);
  ...
  boolean getParameters(final AnisotropicDiffusionParam param,
  ...
  private void setTooltips() {
    btnRunFilterFullVolume
      .setToolTipText("Run diffusion on the full volume in chunks, creates a "
        + Extension.NAD.getSuffix(manager.getBaseMetaData().getImageFilenameStyle())
        + "file."/*"was: filename.nad"*/);
