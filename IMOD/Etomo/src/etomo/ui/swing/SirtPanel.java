...
final class SirtPanel
  implements Run3dmodButtonContainer, SirtsetupDisplay, Expandable, DisplaySettings {
  private static final String RESUME_FROM_LAST_ITERATION_LABEL =
    "Resume from last iteration";

  private final JPanel pnlRoot = new JPanel();
  private final CheckBox cbSubarea = new CheckBox("Reconstruct subarea");
  private final LabeledTextField ltfYOffsetOfSubarea =
    LabeledTextField.getNumericInstance(" Offset in Y: ");
  private final LabeledTextField ltfSubareaSize =
    new LabeledTextField(FieldType.INTEGER_PAIR, "Size in X and Y" + ": ");
  private final LabeledTextField ltfLeaveIterations =
    new LabeledTextField(FieldType.INTEGER_LIST, "Iteration #'s to retain: ");
  private final CheckBox cbScaleToInteger =
    new CheckBox("Scale retained volumes to integers");
  private final ActionListener listener = new SirtActionListener(this);
  private final Run3dmodButton btn3dmodSirt =
    Run3dmodButton.get3dmodInstance("View Tomogram(s) In 3dmod", this);
  private final CheckBox cbCleanUpPastStart =
    new CheckBox("Delete existing reconstructions after starting point");
  private final LabeledTextField ltfFlatFilterFraction =
    new LabeledTextField(FieldType.FLOATING_POINT, "Flat filter fraction: ");
  ...
  private final RadioButton rbStartFromZero =
    new RadioButton("Start from beginning", bgStartingIteration);
  private final RadioButton rbResumeFromLastIteration =
    new RadioButton(RESUME_FROM_LAST_ITERATION_LABEL, bgStartingIteration);
  private final RadioButton rbResumeFromIteration =
    new RadioButton("Go back, resume from iteration:", bgStartingIteration);
  private final ComboBox cmbResumeFromIteration =
    ComboBox.getUnlabeledInstance(rbResumeFromIteration.getText());
  private CheckBox cbSkipVertSliceOutput =
    new CheckBox("Do not make vertical slice output files used for resuming");
  ...
  private SirtPanel(final ApplicationManager manager, final AxisID axisID,
    ...
    radiusAndSigmaPanel =
      RadialPanel.getInstance(manager, axisID, PanelId.SIRTSETUP, this, parent);
    ...
    sirtSetupParamsHeader = PanelHeader.getAdvancedBasicInstance("SIRT", this, dialogType,
  ...
  static SirtPanel getInstance(final ApplicationManager manager, final AxisID axisID,
    ...
    final TomogramGenerationDialog parent) {
    SirtPanel instance =
      new SirtPanel(manager, axisID, dialogType, globalAdvancedButton, parent);
  ...
  private void createPanel() {
    ...
    pnlSkipVertSliceOutput
      .setLayout(new BoxLayout(pnlSkipVertSliceOutput, BoxLayout.X_AXIS));
    ...
    pnlResumeFromLastIteration
      .setLayout(new BoxLayout(pnlResumeFromLastIteration, BoxLayout.X_AXIS));
    ...
    pnlResumeFromIteration
      .setLayout(new BoxLayout(pnlResumeFromIteration, BoxLayout.X_AXIS));
  ...
  @Deprecated // 8/6/18 Saving tilt.com no longer effects SIRT resume - Bug# 2098, comment
              // 10
  ...
  private void updateDisplay() {
    ...
    boolean enableResume =
      numFiles > 0 && !isDifferentFromCheckpoint() && !differentFromCheckpointFlag;
    ...
    radiusAndSigmaPanel.setEditable(!resume);
    cmbResumeFromIteration.setEnabled(
  ...
  void getParameters(final MetaData metaData) {
    ...
    radiusAndSigmaPanel.getParameters(metaData);
  }
  ...
  void setParameters(final ConstMetaData metaData) {
    ...
    radiusAndSigmaPanel.setParameters(metaData);
    loadResumeFrom();
  }
  ...
  public boolean getParameters(final SirtsetupParam param, final boolean doValidation) {
    try {
      try {
        if (doValidation && ltfLeaveIterations.isEmpty()) {
          UIHarness.INSTANCE.openMessageDialog(manager,
            ltfLeaveIterations.getLabel() + " is empty.", "Entry Error", axisID);
          Thread.dumpStack();
          return false;
        }
        ...
        if (cbSubarea.isSelected()) {
          if (ltfSubareaSize.isEmpty()) {
            UIHarness.INSTANCE.openMessageDialog(manager,
              ltfSubareaSize.getLabel() + " is empty.", "Entry Error", axisID);
            return false;
          }
        ...
        UIHarness.INSTANCE.openMessageDialog(manager,
          ltfSubareaSize.getLabel() + " is an invalid list of integers.", "Entry Error",
          axisID);
        return false;
      }
      ...
      if (rbStartFromZero.isSelected()) {
        ...
        param.setStartFromZero(false);
        param.setResumeFromIteration(
          (ConstEtomoNumber) cmbResumeFromIteration.getSelectedItem());
        resume = true;
      }
      else {
        UIHarness.INSTANCE.openMessageDialog(manager,
  ...
  private void loadResumeFrom() {
    ...
    if (subarea) {
      filter = SirtOutputFileFilter.getSubareaInstance(manager, imageFilenameStyle,
        axisID, false);
    }
    else {
      filter =
        SirtOutputFileFilter.getFullInstance(manager, imageFilenameStyle, axisID, false);
    }
    ...
    if (fileNameList != null && fileNameList.length > 0) {
      ...
      for (int i = 0; i < fileNumberList.length; i++) {
        ...
        Extension extension =
          Extension.getInstance(manager, fileName, imageFilenameStyle);
        if (extension != null) {
          ConstEtomoNumber fileNumber =
            extension.getFileNumber(fileName, imageFilenameStyle);
          if (fileNumber != null) {
      ...
      for (int i = fileNumberList.length - 1; i >= 0; i--) {
        ...
        if (i == fileNumberList.length - 1) {
          ...
          rbResumeFromLastIteration
            .setText(RESUME_FROM_LAST_ITERATION_LABEL + ": " + fileNumber);
        }
  ...
  private void openFilesInImod(final Run3dmodMenuOptions run3dmodMenuOptions) {
    ...
    SirtOutputFileFilter sirtOutputFileFilter = SirtOutputFileFilter.getInstance(manager,
      imageFilenameStyle, axisID, true, true, true);
  ...
  void useSirt() {
    ...
    if (fileList != null && fileList.length == 1) {
      if (UIHarness.INSTANCE.openYesNoDialog(manager,
        "Use " + fileList[0].getName() + " as the tomogram?", axisID)) {
  ...
  public boolean isAdvanced() {
    return sirtSetupParamsHeader.isAdvanced();
  }
  ...
  private void setToolTipText() {
    ...
    ltfYOffsetOfSubarea.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, SirtsetupParam.Y_OFFSET_OF_SUBAREA_KEY));
    ltfSubareaSize
      .setToolTipText(EtomoAutodoc.getTooltip(autodoc, SirtsetupParam.SUBAREA_SIZE_KEY));
    ltfLeaveIterations.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, SirtsetupParam.LEAVE_ITERATIONS_KEY));
    cbScaleToInteger.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, SirtsetupParam.SCALE_TO_INTEGER_KEY));
    ltfFlatFilterFraction.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, SirtsetupParam.FLAT_FILTER_FRACTION_KEY));
    btn3dmodSirt.setToolTipText(
      "Opens a file chooser for picking SIRT iteration files to open together in 3dmod");
    cbCleanUpPastStart.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, SirtsetupParam.CLEAN_UP_PAST_START_KEY));
    cbSkipVertSliceOutput.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, SirtsetupParam.SKIP_VERT_SLICE_OUTPUT_KEY));
    rbStartFromZero.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, SirtsetupParam.START_FROM_ZERO_KEY));
    rbResumeFromLastIteration
      .setToolTipText("Iterate from the last existing reconstruction.");
    String tooltip =
      EtomoAutodoc.getTooltip(autodoc, SirtsetupParam.RESUME_FROM_ITERATION_KEY);
    rbResumeFromIteration.setToolTipText(tooltip);
    cmbResumeFromIteration.setToolTipText(tooltip);
    btnSirt.setToolTipText(
      "Run sirtsetup, and then run the resulting .com files with processchunks.");
    btnUseSirt.setToolTipText(
