 ...
 * <p> </p>
 */
final class TiltxcorrPanel
  implements Expandable, TiltXcorrDisplay, Run3dmodButtonContainer, ContextMenu {
  ...
  private final CheckBox cbExcludeCentralPeak =
    new CheckBox("Exclude central peak due to fixed pattern noise");

  private final LabeledTextField ltfTestOutput =
    new LabeledTextField(FieldType.STRING, "Test output: ");
  private final LabeledTextField ltfFilterSigma1 =
    new LabeledTextField(FieldType.FLOATING_POINT, "Low frequency rolloff sigma: ");
  private final LabeledTextField ltfFilterRadius2 =
    new LabeledTextField(FieldType.FLOATING_POINT, "High frequency cutoff radius: ");
  private final LabeledTextField ltfFilterSigma2 =
    new LabeledTextField(FieldType.FLOATING_POINT, "High frequency rolloff sigma: ");
  private final LabeledTextField ltfTrim =
    new LabeledTextField(FieldType.INTEGER_PAIR, "Pixels to trim (x,y): ");
  private final LabeledTextField ltfXMin =
    new LabeledTextField(FieldType.INTEGER, "X axis min ");
  private final LabeledTextField ltfXMax =
    new LabeledTextField(FieldType.INTEGER, "Max ");
  private final LabeledTextField ltfYMin =
    new LabeledTextField(FieldType.INTEGER, "Y axis min ");
  private final LabeledTextField ltfYMax =
    new LabeledTextField(FieldType.INTEGER, "Max ");
  private final LabeledTextField ltfPadPercent =
    new LabeledTextField(FieldType.INTEGER_PAIR, "Pixels to pad (x,y): ");
  private final LabeledTextField ltfTaperPercent =
    new LabeledTextField(FieldType.INTEGER_PAIR, "Pixels to taper (x,y): ");
  ...
  private final LabeledTextField ltfViewRange =
    new LabeledTextField(FieldType.INTEGER_PAIR, "View range (start,end): ");
  private final LabeledTextField ltfAngleOffset =
    new LabeledTextField(FieldType.FLOATING_POINT, "Tilt angle offset: ");
  private final CrossCorrelationActionListener actionListener =
    new CrossCorrelationActionListener(this);
  private final LabeledTextField ltfSkipViews =
    new LabeledTextField(FieldType.INTEGER_LIST, "Views to skip: ");

  // Patch tracking
  private final LabeledTextField ltfSizeOfPatchesXandY =
    new LabeledTextField(FieldType.INTEGER_PAIR, "Size of patches (X,Y): ");
  private final ButtonGroup bgPatchLayout = new ButtonGroup();
  private final RadioTextField rtfOverlapOfPatchesXandY =
    RadioTextField.getInstance(FieldType.FLOATING_POINT_PAIR,
      "Fractional overlap of patches (X,Y): ", bgPatchLayout);
  private final RadioTextField rtfNumberOfPatchesXandY = RadioTextField
    .getInstance(FieldType.INTEGER_PAIR, "Number of patches (X,Y): ", bgPatchLayout);
  private final Spinner spIterateCorrelations =
    Spinner.getLabeledInstance("Iterations to increase subpixel accuracy: ",
  ...
  private final Run3dmodButton btn3dmodBoundaryModel =
    Run3dmodButton.get3dmodInstance("Create Boundary Model", this);
  private final ButtonGroup bgLengthOfPieces = new ButtonGroup();
  private final RadioButton rbLengthOfPiecesDefault =
    new RadioButton("Use default length", bgLengthOfPieces);
  private RadioTextField rtfLengthOfPieces =
    RadioTextField.getInstance(FieldType.INTEGER, "Use length", bgLengthOfPieces);

  private final AxisID axisID;
  private final DialogType dialogType;
  private final MultiLineButton btnTiltxcorr;
  private final MultiLineButton btnImodchopconts;
  private final Run3dmodButton btn3dmodPatchTracking =
    Run3dmodButton.get3dmodInstance("Open Tracked Patches", this);
  ...
  private void createPanel() {
    ...
    rbLengthOfPiecesDefault
      .setText("Use default length (" + ImodchopcontsParam.getLengthOfPiecesDefault(
    ...
    if (panelId == PanelId.CROSS_CORRELATION) {
      ...
      pnlIterateCorrelations
        .setLayout(new BoxLayout(pnlIterateCorrelations, BoxLayout.X_AXIS));
  ...
  public boolean getParameters(final ImodchopcontsParam param,
  ...
  public boolean getParameters(final TiltxcorrParam tiltXcorrParams,
    ...
    try {
      ...
      if (panelId == PanelId.CROSS_CORRELATION) {
        ...
        tiltXcorrParams.setInputFile(
          FileType.PREALIGNED_STACK.getFileName(applicationManager, axisID));
        tiltXcorrParams.setOutputFile(
          FileType.FIDUCIAL_PATCH_TRACKING_MODEL.getFileName(applicationManager, axisID));
      ...
      try {
        ...
        if (panelId == PanelId.CROSS_CORRELATION) {
          ...
          if (rtfOverlapOfPatchesXandY.isSelected()) {
            tiltXcorrParams
              .setOverlapOfPatchesXandY(rtfOverlapOfPatchesXandY.getText(doValidation));
          }
          else {
          ...
          if (rtfNumberOfPatchesXandY.isSelected()) {
            tiltXcorrParams
              .setNumberOfPatchesXandY(rtfNumberOfPatchesXandY.getText(doValidation));
          }
          else {
          ...
          tiltXcorrParams.setImagesAreBinned(UIExpertUtilities.INSTANCE
            .getStackBinning(applicationManager, axisID, FileType.PREALIGNED_STACK));
        }
        if (ctfMagChanges != null) {
  ...
  void updatePanel() {
    ...
    if (ctfMagChanges != null) {
      cbCumulativeCorrelation
        .setEnabled(!ctfMagChanges.isSelected() || !ctfMagChanges.isEnabled());
      ctfMagChanges.setEnabled(
        !cbCumulativeCorrelation.isSelected() || !cbCumulativeCorrelation.isEnabled());
    }
  ...
  public void action(final String actionCommand,
    ...
    if (runTiltxcorr && panelId == PanelId.CROSS_CORRELATION) {
      ...
      applicationManager.tiltxcorr(axisID, runTiltxcorr ? btnTiltxcorr : btnImodchopconts,
        deferred3dmodButton, run3dmodMenuOptions, null, dialogType, this, false,
        ProcessName.XCORR_PT/*was: FileType.PATCH_TRACKING_COMSCRIPT*/, runTiltxcorr,
  ...
  private static class CrossCorrelationActionListener implements ActionListener {
    ...
    CrossCorrelationActionListener(final TiltxcorrPanel adaptee) {
  ...
  private void setToolTipText() {
    ...
    ltfFilterSigma1
      .setToolTipText(EtomoAutodoc.getTooltip(autodoc, TiltxcorrParam.FILTER_SIGMA1_KEY));
    ...
    if (ctfMagChanges != null) {
      ctfMagChanges.setCheckBoxToolTipText(
        EtomoAutodoc.getTooltip(autodoc, TiltxcorrParam.SEARCH_MAG_CHANGES_KEY));
      ctfMagChanges.setCheckBoxToolTipText(
        EtomoAutodoc.getTooltip(autodoc, TiltxcorrParam.VIEWS_WITH_MAG_CHANGES_KEY));
    }
    ...
    cbCumulativeCorrelation
      .setToolTipText(EtomoAutodoc.getTooltip(autodoc, "CumulativeCorrelation"));
    cbAbsoluteCosineStretch
      .setToolTipText(EtomoAutodoc.getTooltip(autodoc, "AbsoluteCosineStretch"));
    ...
    ltfSkipViews
      .setToolTipText(EtomoAutodoc.getTooltip(autodoc, TiltxcorrParam.SKIP_VIEWS_KEY));
    cbExcludeCentralPeak
      .setToolTipText(EtomoAutodoc.getTooltip(autodoc, "ExcludeCentralPeak"));
    ...
    btn3dmodPatchTracking.setToolTipText(
      "Open the pre-aligned stack with the patch tracking " + "fiducial model.");
    ltfSizeOfPatchesXandY.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, TiltxcorrParam.SIZE_OF_PATCHES_X_AND_Y_KEY));
    rtfOverlapOfPatchesXandY.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, TiltxcorrParam.OVERLAP_OF_PATCHES_X_AND_Y_KEY));
    rtfNumberOfPatchesXandY.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, TiltxcorrParam.NUMBER_OF_PATCHES_X_AND_Y_KEY));
    spIterateCorrelations.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, TiltxcorrParam.ITERATE_CORRELATIONS_KEY));
    ltfShiftLimitsXandY.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, TiltxcorrParam.SHIFT_LIMITS_X_AND_Y_KEY));
    ...
    ctfLengthOfPiecesMinimumOverlap.setCheckBoxToolTipText(
      EtomoAutodoc.getTooltip(autodoc, ImodchopcontsParam.LENGTH_OF_PIECES_KEY));
    ctfLengthOfPiecesMinimumOverlap.setFieldToolTipText(
      EtomoAutodoc.getTooltip(autodoc, ImodchopcontsParam.MINIMUM_OVERLAP_KEY));
    rbLengthOfPiecesDefault.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, ImodchopcontsParam.LENGTH_OF_PIECES_KEY));
    rtfLengthOfPieces.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, ImodchopcontsParam.LENGTH_OF_PIECES_KEY));
    btnImodchopconts
