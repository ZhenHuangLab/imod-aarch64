 ...
 * </p>
 */
class FinalCombinePanel
  implements ContextMenu, FinalCombineFields, Run3dmodButtonContainer, Expandable {
  ...
  private LabeledTextField ltfXPatchSize =
    new LabeledTextField(FieldType.INTEGER, "X patch size :");
  private LabeledTextField ltfYPatchSize =
    new LabeledTextField(FieldType.INTEGER, "Z patch size :");
  private LabeledTextField ltfZPatchSize =
    new LabeledTextField(FieldType.INTEGER, "Y patch size :");
  private JPanel pnlPatchsizeButtons = new JPanel();
  private MultiLineButton btnPatchsizeIncrease = new MultiLineButton("Patch Size +20%");
  private MultiLineButton btnPatchsizeDecrease = new MultiLineButton("Patch Size -20%");

  private LabeledTextField ltfXNPatches =
    new LabeledTextField(FieldType.INTEGER, "Number of X patches :");
  private LabeledTextField ltfYNPatches =
    new LabeledTextField(FieldType.INTEGER, "Number of Z patches :");
  private LabeledTextField ltfZNPatches =
    new LabeledTextField(FieldType.INTEGER, "Number of Y patches :");
  private CheckBox cbKernelSigma = new CheckBox(KERNEL_SIGMA_LABEL);
  private TextField tfKernelSigma =
    new TextField(FieldType.FLOATING_POINT, KERNEL_SIGMA_LABEL, null);
  ...
  private Run3dmodButton btnPatchRegionModel =
    Run3dmodButton.get3dmodInstance("Create/Edit Patch Region Model", this);
  private LabeledTextField ltfWarpLimit =
    new LabeledTextField(FieldType.STRING, "Warping residual limits: ");
  private LabeledTextField ltfRefineLimit = new LabeledTextField(FieldType.FLOATING_POINT,
  ...
  private LabeledTextField ltfZUpperExclude =
    new LabeledTextField(FieldType.INTEGER, "Number of rows to exclude on top (in Y): ");
  ...
  private MultiLineButton btnMatchorwarpTrial =
    new MultiLineButton("Matchorwarp Trial Run");
  ...
  private MultiLineButton btnPatchVectorModel =
    new MultiLineButton("Examine Patch Vector Model");
  private MultiLineButton btnReplacePatchOut =
    new MultiLineButton("Replace Patch Vectors");
  private Run3dmodButton btnImodMatchedTo =
    Run3dmodButton.get3dmodInstance("Open Volume Being Matched To", this);
  private final Run3dmodButton btnImodCombined =
    Run3dmodButton.get3dmodInstance("Open Combined Volume", this);
  private CheckBox cbNoVolcombine = new CheckBox(NO_VOLCOMBINE_TITLE);
  private LabeledTextField ltfReductionFactor =
    new LabeledTextField(FieldType.FLOATING_POINT,
      "Reduction factor for matching amplitudes in combined FFT: ");
  private LabeledTextField ltfLowFromBothRadius =
    new LabeledTextField(FieldType.FLOATING_POINT,
  ...
  private final LabeledTextField ltfInitialShiftX =
    new LabeledTextField(FieldType.FLOATING_POINT, "Initial shift in X:");
  private final LabeledTextField ltfInitialShiftY =
    new LabeledTextField(FieldType.FLOATING_POINT, "Z:");
  private final LabeledTextField ltfInitialShiftZ =
    new LabeledTextField(FieldType.FLOATING_POINT, "Y:");
  private final SpacedPanel pnlInitialShiftXYZ = SpacedPanel.getInstance();
  private MultiLineButton btnPatchVectorCCCModel =
    new MultiLineButton("Open Vector Model with Correlations");
   ...
   * @param appMgr
   */
  FinalCombinePanel(TomogramCombinationDialog parent, ApplicationManager appMgr,
    ...
    volcombineHeader = PanelHeader.getAdvancedBasicInstance("Volcombine Parameters", this,
      dialogType, globalAdvancedButton);
  ...
  final void setParameters(ReconScreenState screenState) {
    ...
    btnPatchcorrRestart.setButtonState(
      screenState.getButtonState(btnPatchcorrRestart.getButtonStateKey()));
    btnMatchorwarpRestart.setButtonState(
      screenState.getButtonState(btnMatchorwarpRestart.getButtonStateKey()));
    btnVolcombineRestart.setButtonState(
      screenState.getButtonState(btnVolcombineRestart.getButtonStateKey()));
   ...
   */
  void setPatchcrawl3DParams(ConstPatchcrawl3DParam patchrawlParam) {
   ...
   */
  boolean getPatchcrawl3DParams(final Patchcrawl3DParam patchcrawl3DParam,
    ...
    try {
      ...
      patchcrawl3DParam
        .setXPatchSize(Integer.parseInt(ltfXPatchSize.getText(doValidation)));
      badParameter = ltfYPatchSize.getLabel();
      patchcrawl3DParam
        .setYPatchSize(Integer.parseInt(ltfYPatchSize.getText(doValidation)));
      badParameter = ltfZPatchSize.getLabel();
      patchcrawl3DParam
        .setZPatchSize(Integer.parseInt(ltfZPatchSize.getText(doValidation)));
   ...
   */
  void setMatchorwarpParams(ConstMatchorwarpParam matchorwarpParam) {
   ...
   * @throws NumberFormatException
   */
  boolean getMatchorwarpParams(MatchorwarpParam matchorwarpParam,
    final boolean doValidation) throws NumberFormatException {
    try {
      String badParameter = "";
    ...
    try {
      ...
      // pixels
        ...
        applicationManager.patchcorrCombine(btnPatchcorrRestart, null,
          ...
          tomogramCombinationDialog.getRunProcessingMethod(), isParallel(),
          !isRunVolcombine());
      }
        ...
        if (cbParallelProcess.isSelected()) {
          applicationManager.splitcombine(null, deferred3dmodButton, run3dmodMenuOptions,
            dialogType, tomogramCombinationDialog.getRunProcessingMethod(), isParallel(),
            !isRunVolcombine());
        }
        else {
  ...
  private void setToolTipText() {
    ...
    btnPatchVectorCCCModel.setToolTipText(
    ...
    btnReplacePatchOut.setToolTipText(
    ...
    if (adocCombineFft != null) {
      ltfReductionFactor
        .setToolTipText(EtomoAutodoc.getTooltip(adocCombineFft, "ReductionFraction"));
      ltfLowFromBothRadius
        .setToolTipText(EtomoAutodoc.getTooltip(adocCombineFft, "LowFromBothRadius"));
    }
