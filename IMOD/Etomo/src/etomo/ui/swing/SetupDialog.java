...
import etomo.storage.ExcludeViewsInfoFileNameFilter;
...
/**
 * <p>Description: Setup dialog for tomogram reconstruction</p>
 *
 ...
 * @version $Id$
 */
final class SetupDialog extends ProcessDialog
  implements ContextMenu, Run3dmodButtonContainer, Expandable, SetupReconInterface,
  FocusListener, ControlListener {
  ...
  private static final String REMOVE_EXCLUDE_VIEW_MSG =
    "Excluded views have been removed";
  ...
  private final ButtonControlTextEfield bctfRawImageStack =
    ButtonControlTextEfield.getLabeledFileInstance("Raw Image Stack: ");

  private final FileTextField ftfBackupDirectory =
    new FileTextField(BACKUP_DIRECTORY_LABEL);
  ...
  private final Run3dmodButton btnViewRawStackA =
    Run3dmodButton.get3dmodInstance(VIEW_RAW_STACK_LABEL, this);
  private Run3dmodButton btnViewRawStackB =
    Run3dmodButton.get3dmodInstance(VIEW_RAW_STACK_LABEL, this);
  ...
  private final LabeledTextField ltfPixelSize =
    new LabeledTextField(FieldType.FLOATING_POINT, "Pixel size (nm): ");
  private final LabeledTextField ltfFiducialDiameter =
    new LabeledTextField(FieldType.FLOATING_POINT, FIDUCIAL_DIAMETER_LABEL);
  private final LabeledTextField ltfImageRotation =
    new LabeledTextField(FieldType.FLOATING_POINT, "Image rotation (degrees): ");

  private final JPanel pnlDistortionInfo = new JPanel();
  private final FileTextField ftfDistortionFile =
    new FileTextField("Image distortion field file: ");
  private final LabeledTextField ltfBinning =
    new LabeledTextField(FieldType.FLOATING_POINT, "Binning: ");

  private final JPanel pnlMagGradientInfo = new JPanel();
  private final FileTextField ftfMagGradientFile =
    new FileTextField("Mag gradients correction: ");
  ...
  private final LabeledTextField ltfExcludeListA =
    new LabeledTextField(FieldType.INTEGER_LIST, "Exclude views: ");
  private final JPanel pnlAdjustedFocusA = new JPanel();
  private final CheckBox cbAdjustedFocusA =
    new CheckBox("Focus was adjusted between montage frames");

  private final BeveledBorder borderAxisInfoB = new BeveledBorder("Axis B: ");
  private final LabeledTextField ltfExcludeListB =
    new LabeledTextField(FieldType.INTEGER_LIST, "Exclude views: ");
  private final JPanel pnlAdjustedFocusB = new JPanel();
  private final CheckBox cbAdjustedFocusB =
    new CheckBox("Focus was adjusted between montage frames");
  private final CheckTextField ctfTwodir =
    CheckTextField.getInstance(FieldType.FLOATING_POINT, TWODIR_LABEL_1);
  private final CheckTextField ctfBtwodir =
    CheckTextField.getInstance(FieldType.FLOATING_POINT, TWODIR_LABEL_1);
  private final JLabel lTwodir = new JLabel(TWODIR_LABEL_2);
  private final JLabel lBtwodir = new JLabel(TWODIR_LABEL_2);
  private final CheckBox cbRemoveExcludedViews = new CheckBox("Remove excluded views");
  private final CheckBox cbDeleteOldFiles = new CheckBox("Delete original files");
  private final JLabel lRemoveExcludeViewsMsgA = new JLabel();
  private final JLabel lRemoveExcludeViewsMsgB = new JLabel();
  ...
  private final AxisProgressPanel progressPanel;
  private final TiltAnglePanel tiltAnglesA;
  private final TiltAnglePanel tiltAnglesB;

  private DirectiveFileCollection directiveFileCollection = null;
  private boolean excludeViewsSucceededA = false;
  private boolean excludeViewsSucceededB = false;

  // Construct the setup dialog
  private SetupDialog(final SetupDialogExpert expert, final ApplicationManager manager,
    final AxisID axisID, final DialogType dialogType, final boolean calibrationAvailable,
    final ValidationSet binningValidationSet, final AxisProgressPanel progressPanel) {
    super(manager, axisID, dialogType, false);
    tiltAnglesA = expert.getTiltAnglesPanelExpert(AxisID.FIRST).getPanel();
    tiltAnglesA.setParent(this);
    tiltAnglesB = expert.getTiltAnglesPanelExpert(AxisID.SECOND).getPanel();
    tiltAnglesB.setParent(this);
    this.expert = expert;
    this.calibrationAvailable = calibrationAvailable;
    ltfBinning.setValidationSet(binningValidationSet);
    listener = new SetupDialogActionListener(expert);
    templatePanel =
      TemplatePanel.getInstance(manager, axisID, listener, "Templates", null, false);
    // progressPanel = AxisProgressPanel.getInstance(axisID, manager);
    this.progressPanel = progressPanel;
    progressPanel.setVisible(false);
    rootPanel.setLayout(new BoxLayout(rootPanel, BoxLayout.Y_AXIS));
    bctfRawImageStack.setFileSelectionMode(JFileChooser.FILES_ONLY);
    bctfRawImageStack.setSelectFileDir(expert.getDatasetDir());
    // Defaults to absolute path
    bctfRawImageStack.setFileFilter(StackFileFilter.getInstance(manager, false));
    bctfRawImageStack.setLimitDisplayedFilePath(50);
    createDatasetPanel();
    createDataTypePanel();
  ...
  static SetupDialog getInstance(final SetupDialogExpert expert,
    ...
    boolean calibrationAvailable, final ValidationSet binningValidationSet,
    final AxisProgressPanel progressPanel) {
    SetupDialog instance = new SetupDialog(expert, manger, axisID, dialogType,
      calibrationAvailable, binningValidationSet, progressPanel);
  ...
  public void done() {
    String datasetDirString = null;
    File datasetDir = bctfRawImageStack.getFile();
    if (datasetDir != null) {
      datasetDirString = datasetDir.getParent();
    }
    boolean removeExcludedViews = isRemoveExcludedViews();
    excludeViewsSucceededA = false;
    excludeViewsSucceededB = false;

    applicationManager.doneSetupDialog(removeExcludedViews, removeExcludedViews,
      datasetDirString, rbDualAxis.isSelected(), progressPanel);
  }

  void msgSetupReconFailed() {
    if (excludeViewsSucceededA || excludeViewsSucceededB) {
      UIHarness.INSTANCE.openInfoMessageDialog(applicationManager, cbRemoveExcludedViews,
        "Excludeviews has run successfully.  Views have been removed.", "Views Excluded",
        null);
    }
    updateDisplay(false, true);
  }

  private boolean isRemoveExcludedViews() {
    return cbRemoveExcludedViews.isEnabled() && cbRemoveExcludedViews.isSelected();
  }
  ...
  public void msgExcludeViewsSucceeded(final AxisID axisID, final boolean processRunning,
    ...
    if (axisID == AxisID.SECOND) {
      excludeViewsSucceededB = true;
      ltfExcludeListB.setText("");
      lRemoveExcludeViewsMsgB.setText(REMOVE_EXCLUDE_VIEW_MSG);
      tiltAnglesB.msgExcludeViewsSucceeded();
      if (ltfExcludeListA.isEmpty()) {
        cbRemoveExcludedViews.setSelected(false);
      }
    }
    else {
      excludeViewsSucceededA = true;
      ltfExcludeListA.setText("");
      lRemoveExcludeViewsMsgA.setText(REMOVE_EXCLUDE_VIEW_MSG);
      tiltAnglesA.msgExcludeViewsSucceeded();
      if (!ltfExcludeListB.isEnabled() || ltfExcludeListB.isEmpty()) {
        cbRemoveExcludedViews.setSelected(false);
      }
    }
    updateDisplay(processRunning, processDone);
  }

  /**
   * Enable/disable fields.
   * @param processDone - set when a process from this dialog has completed.
   */
  void updateDisplay(final boolean processRunning, final boolean processDone) {
    cbDeleteOldFiles.setEnabled(cbRemoveExcludedViews.isSelected());
    ...
    if (processRunning) {
      ...
      enabled = progressPanel.isStopped();
    }
    btnExecute.setEnabled(enabled);
    btnCancel.setEnabled(enabled);
    tiltAnglesA.updateDisplay();
    tiltAnglesB.updateDisplay();
  }

  public String getRawImageStack() {
    return bctfRawImageStack.getText();
  }

  /**
   * Derive the dataset name from whatever is in ftfDataset (which make be a file name, a
   * file with path, or a dataset name).
   * @return
   */
  String getDatasetName() {
    String dataset = bctfRawImageStack.getText();
    if (dataset == null) {
      return null;
    }
    dataset = dataset.trim();
    if (dataset == null || dataset.matches("\\s*")) {
      return null;
    }
    ...
    if (index != -1 && index > 1) {
      datasetName = datasetName.substring(0, index);
    }
    int length = datasetName.length();
    // Check for and remove axis extension.
    int axisExtensionLen = AxisID.getExtensionLength();
    if (rbDualAxis.isSelected() && length > axisExtensionLen
      && (datasetName.endsWith(AxisID.FIRST.getExtension()))
      || datasetName.endsWith(AxisID.SECOND.getExtension())) {
      datasetName = datasetName.substring(0, length - axisExtensionLen);
    }
    return datasetName;
  }

  File getDirectory() {
    String rawImageStack = bctfRawImageStack.getText();
    if (rawImageStack == null || rawImageStack.length() == 0) {
      return null;
    }
    return new File(rawImageStack).getParentFile();
  }

  public boolean buttonExecuteAction() {
    if (bctfRawImageStack.getText().indexOf(File.separator) != -1) {
      if (!DatasetTool.validateDatasetName(applicationManager, null, AxisID.ONLY,
        bctfRawImageStack.getFile(), DataFileType.RECON, expert.getAxisType())) {
        return false;
      }
    }
    else {
      String datasetName = bctfRawImageStack.getText();
      if (!DatasetTool.validateDatasetName(applicationManager, null, AxisID.ONLY,
        new File(expert.getPropertyUserDir()), datasetName, DataFileType.RECON,
        expert.getAxisType(), !datasetName.endsWith(
          applicationManager.getBaseMetaData().getRawImageStackExtension().toString()))) {
        return false;
      }
    }
  ...
  public void action(final String actionCommand,
    ...
    if (rawImageStack == null || rawImageStack.isEmpty()) {
      UIHarness.INSTANCE.openMessageDialog(applicationManager,
        "Raw image stack has not been entered", "Raw Image Stack", AxisID.ONLY);
      return;
    }
    ...
    String fileExtension = Extension.EXTENSION_DIVIDER + extension.toString();
  ...
  /**
   * Listening only to the dataset focus.  Need to see if the dataset has changed.
   */
  public void focusGained(final FocusEvent event) {}

  /**
   * Listening only to the dataset focus.  Find out if the dataset has changed.
   */
  ...
  public void controlEvent() {
    ...
    File rawImageStack = bctfRawImageStack.getFile();
    if (rawImageStack == null) {
      lRemoveExcludeViewsMsgA.setText("");
      lRemoveExcludeViewsMsgB.setText("");
      return;
    }
    boolean dualAxis = rbDualAxis.isSelected();
    String datasetName = DatasetTool.getDatasetName(rawImageStack.getName(), dualAxis);
    AxisID axisID = AxisID.ONLY;
    if (dualAxis) {
      axisID = AxisID.FIRST;
    }
    ExcludeViewsInfoFileNameFilter fileNameFilter =
      new ExcludeViewsInfoFileNameFilter(datasetName);
    fileNameFilter.setAxisID(axisID);
    File dir = rawImageStack.getParentFile();
    String[] fileNameList = dir.list(fileNameFilter);
    ...
    if (dualAxis) {
      fileNameFilter.setAxisID(AxisID.SECOND);
      fileNameList = dir.list(fileNameFilter);
      if (fileNameList != null && fileNameList.length > 0) {
        // Excludeviews was run for this dataset name previously.
        lRemoveExcludeViewsMsgB.setText(REMOVE_EXCLUDE_VIEW_MSG);
      }
  ...
  boolean isRemoveExcludeViewsMsg(final AxisID axisID) {
    if (axisID == AxisID.SECOND) {
      return !lRemoveExcludeViewsMsgB.getText().equals("");
    }
    return !lRemoveExcludeViewsMsgA.getText().equals("");
  }
  ...
  void setParameters(final UserConfiguration userConfig) {
    ...
    cbRemoveExcludedViews.setSelected(userConfig.isRemoveExcludedViews());
    updateDisplay(false, false);
  }
  ...
  public boolean getParameters(final ExcludeViewsParam param, final AxisID axisID,
    ...
    File rawImageStack = bctfRawImageStack.getFile();
    if (rawImageStack != null) {
      boolean dual = rbDualAxis.isSelected();
      String rootName = DatasetTool.getDatasetName(rawImageStack.getName(), dual);
      if (rootName != null) {
        param.setStackName(DatasetTool.getStackFileName(rawImageStack.getParent(),
          rootName, axisID, dual));
      }
    }
    param.setMontagedImages(rbMontage.isSelected());
    param
      .setDeleteOldFiles(cbDeleteOldFiles.isEnabled() && cbDeleteOldFiles.isSelected());
    return true;
  }
  ...
  void updateTemplateValues() {
    ...
    boolean twodirASet = false;
    String twodirA = null;
    if (twodirASet = directiveFileCollection.isTwodir(AxisID.FIRST)) {
      ctfTwodir.setSelected(true);
      twodirA = directiveFileCollection.getTwodir(AxisID.FIRST, false);
      ctfTwodir.setText(twodirA);
    }
    else {
    ...
    if (directiveFileCollection.contains(DirectiveDef.ROTATION)) {
      ltfImageRotation
        .setText(directiveFileCollection.getImageRotation(AxisID.FIRST, false));
    }
    else {
    ...
    if (directiveFileCollection.contains(DirectiveDef.BINNING)) {
      ltfBinning.setText(directiveFileCollection.getBinning());
    }
    else {
      ltfBinning.resetToCheckpoint();
    }
    ...
    if (directiveFileCollection.contains(DirectiveDef.FOCUS, AxisID.FIRST)) {
      cbAdjustedFocusA
        .setSelected(directiveFileCollection.isAdjustedFocusSelected(AxisID.FIRST));
    }
    else {
    ...
    if (directiveFileCollection.contains(DirectiveDef.FOCUS, AxisID.SECOND)) {
      cbAdjustedFocusB
        .setSelected(directiveFileCollection.isAdjustedFocusSelected(AxisID.SECOND));
    }
    else {
      cbAdjustedFocusB.resetToCheckpoint();
    }
    if (directiveFileCollection.contains(DirectiveDef.REMOVE_EXCLUDED_VIEWS)) {
      cbRemoveExcludedViews.setSelected(
        directiveFileCollection.isValue(DirectiveDef.REMOVE_EXCLUDED_VIEWS, true));
    }
    else {
      cbRemoveExcludedViews.resetToCheckpoint();
    }
    if (directiveFileCollection.contains(DirectiveDef.DELETE_OLD_FILES)) {
      cbDeleteOldFiles.setSelected(
        directiveFileCollection.isValue(DirectiveDef.DELETE_OLD_FILES, true));
    }
    else {
      cbDeleteOldFiles.resetToCheckpoint();
    }
  ...
  void setRawImageStack(final String input) {
    bctfRawImageStack.setText(input);
  }
  ...
  void setBinningTooltip(final String tooltip) {
    ltfBinning.setToolTipText(tooltip);
  }
  ...
  AxisType getAxisType() {
    return rbDualAxis.isSelected() ? AxisType.DUAL_AXIS : AxisType.SINGLE_AXIS;
  }
  ...
  public void setBinning(final String input) {
    ltfBinning.setText(input);
  }

  public String getBinning(final boolean doValidation)
    throws FieldValidationFailedException {
    return ltfBinning.getText(doValidation);
  }
  ...
  /**
   * @deprecated 4/8/2019 replaced by getRawImageStack
   */
  ...
  String getViewsToSkip(final AxisID axisID, final boolean doValidation) {
    try {
      if (axisID == AxisID.SECOND) {
        return ltfExcludeListB.getText(doValidation);
      }
  ...
  void updateAdvanced(final boolean advanced) {
    ...
    ltfBinning.setVisible(advanced);
  ...
  File getFile(String dir, FileFilter fileFilter, int selectionMode) {
    JFileChooser chooser = new FileChooser(applicationManager, dir);
    if (fileFilter != null) {
  ...
  void setRawImageStackTooltip(final String fieldTooltip, final String buttonTooltip) {
    bctfRawImageStack.setTooltip(fieldTooltip, buttonTooltip);
  }
  ...
  private void addListeners() {
    ...
    cbRemoveExcludedViews.addActionListener(listener);
    bctfRawImageStack.addControlListener(this);
    bctfRawImageStack.addFocusListener(this);
  }
  ...
  private void createDatasetPanel() {
    ...
    pnlDataset.add(bctfRawImageStack.getComponent());
  ...
  private void createDataTypePanel() {
    ...
    JPanel pnlRemoveExcludedViews = new JPanel();
    // init
    ...
    pnlDistortionInfo.add(Box.createRigidArea(FixedDim.x10_y0));
    pnlDistortionInfo.add(ltfBinning.getContainer());
    ...
    pnlParallelProcess.add(Box.createRigidArea(FixedDim.x15_y0));
    pnlParallelProcess.add(cbGpuProcessing.getComponent());
    ...
    pnlImageRows.add(pnlRemoveExcludedViews);
    ...
    pnlDataParameters.add(progressPanel.getComponent());
    ...
    pnlRemoveExcludedViews
      .setLayout(new BoxLayout(pnlRemoveExcludedViews, BoxLayout.X_AXIS));
    pnlRemoveExcludedViews.add(cbRemoveExcludedViews.getComponent());
    pnlRemoveExcludedViews.add(Box.createRigidArea(FixedDim.x10_y0));
    pnlRemoveExcludedViews.add(cbDeleteOldFiles.getComponent());
  }
  ...
  private void createPerAxisInfoPanel() {
    ...
    lRemoveExcludeViewsMsgA.setForeground(ProcessControlPanel.colorComplete);
    lRemoveExcludeViewsMsgB.setForeground(ProcessControlPanel.colorComplete);
    ...
    lRemoveExcludeViewsMsgA.setAlignmentX(Component.CENTER_ALIGNMENT);

    pnlAxisInfoA.add(tiltAnglesA.getComponent());
    ...
    pnlAxisInfoA.add(lRemoveExcludeViewsMsgA);
    ...
    lRemoveExcludeViewsMsgB.setAlignmentX(Component.CENTER_ALIGNMENT);
    pnlAxisInfoB.add(tiltAnglesB.getComponent());
    ...
    pnlAxisInfoB.add(lRemoveExcludeViewsMsgB);
