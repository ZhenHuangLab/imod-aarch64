 ...
 * <p> </p>
 */
final class IterationTable extends HighlightableTable {
  ...
  private final HeaderCell header1DPhiDThetaDPsi =
    new HeaderCell(D_PHI_D_THETA_D_PSI_HEADER1);
  ...
  private final HeaderCell header3DPhiMax =
    new HeaderCell(MAX_HEADER3, UIParameters.getInstance().getNumericWidth());
  private final HeaderCell header3DPhiIncrement =
    new HeaderCell(INCR_HEADER3, UIParameters.getInstance().getNumericWidth());
  private final HeaderCell header3DThetaMax =
    new HeaderCell(MAX_HEADER3, UIParameters.getInstance().getNumericWidth());
  private final HeaderCell header3DThetaIncrement =
    new HeaderCell(INCR_HEADER3, UIParameters.getInstance().getNumericWidth());
  private final HeaderCell header3DPsiMax =
    new HeaderCell(MAX_HEADER3, UIParameters.getInstance().getNumericWidth());
  private final HeaderCell header3DPsiIncrement =
    new HeaderCell(INCR_HEADER3, UIParameters.getInstance().getNumericWidth());
  ...
  private final HeaderCell header3SearchRadius =
    new HeaderCell(UIParameters.getInstance().getIntegerTripletWidth());
  ...
  private final HeaderCell header3HiCutoff =
    new HeaderCell(HI_CUTOFF_HEADER3, UIParameters.getInstance().getWideNumericWidth());
  private final HeaderCell header3LowCutoff =
    new HeaderCell(LOW_CUTOFF_HEADER3, UIParameters.getInstance().getWideNumericWidth());
  ...
  private final HeaderCell header1DuplicateTolerance =
    new HeaderCell(DUPLICATE_TOLERANCE_HEADER1);
  private final HeaderCell header2DuplicateTolerance =
    new HeaderCell(DUPLICATE_TOLERANCE_HEADER2);
  private final HeaderCell header3DuplicateShiftTolerance =
    new HeaderCell(DUPLICATE_SHIFT_TOLERANCE_HEADER3);
  private final HeaderCell header3DuplicateAngularTolerance =
    new HeaderCell(DUPLICATE_ANGULAR_TOLERANCE_HEADER3);
  private final SingleLineButton btnMoveUp = SingleLineButton.getHtmlInstance("Up");
  private final SingleLineButton btnMoveDown = SingleLineButton.getHtmlInstance("Down");
  private final SingleLineButton btnAddRow = SingleLineButton.getHtmlInstance("Insert");
  private final SingleLineButton btnDeleteRow = SingleLineButton.getHtmlInstance("Delete");
  private final SingleLineButton btnCopyRow = SingleLineButton.getHtmlInstance("Dup");
  private final CheckBox cbFlgRemoveDuplicates =
    new CheckBox(SharedStrings.FLG_REMOVE_DUPLICATES_LABEL);
  ...
  static IterationTable getInstance(BaseManager manager, IterationParent parent) {
    ...
    instance.initHighlightHotkeys();
    return instance;
  }

  JComponent[] getFocusableParents() {
  ...
  private IterationRow addRow(final boolean init) {
    IterationRow row = rowList.add(this, pnlTable, layout, constraints);
    row.display();
    parent.updateDisplay(init);
    refreshVerticalPadding();
    return row;
  }
  ...
  private void setToolTipText() {
    ...
    header3DuplicateShiftTolerance.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, MatlabParam.DUPLICATE_SHIFT_TOLERANCE_KEY));
    header3DuplicateAngularTolerance.setToolTipText(
      EtomoAutodoc.getTooltip(autodoc, MatlabParam.DUPLICATE_ANGULAR_TOLERANCE_KEY));

    btnAddRow.setToolTipText("Add a new iteration row to the table.");
    btnCopyRow.setToolTipText(
    ...
    cbFlgRemoveDuplicates.setToolTipText(
      "Remove mulitple references to the same particle after" + "each iteration.");
  }
  ...
  private static final class RowList {
    ...
    private synchronized IterationRow add(final Highlightable parent, final JPanel panel,
      ...
      int index = list.size();
      IterationRow row =
        new IterationRow(index, parent, panel, layout, constraints, manager, table);
      row.setNames();
      list.add(row);
      return row;
    }
