 ...
 * <p>Description: </p>
 * 
 * <p>Copyright: Copyright 2010 - 2020 by the Regents of the University of Colorado</p>
 *
...
public final class ToolsManager extends BaseManager {
  ...
  public ToolsManager(final ToolType toolType) {
    ...
    this.metaData =
      new ToolsMetaData(this, DIALOG_TYPE, toolType, getLogProperties(), true);
    createState();
  ...
  public void openToolsDialog() {
    ...
    String actionMessage =
      Utilities.prepareDialogActionMessage(DialogType.TOOLS, AxisID.ONLY, null);
  ...
  public void imodMakeSurfaceModel(Run3dmodMenuOptions menuOptions, AxisID axisID,
    ...
    try {
      ...
      imodManager.open(key, file,
        FileType.FLATTEN_WARP_INPUT_MODEL.getFileName(this, AXIS_ID), true, menuOptions);
    }
    catch (SystemProcessException except) {
      except.printStackTrace();
      uiHarness.openMessageDialog(this,
  ...
  private boolean isFlipped(File mrcFile) {
    MRCHeader header =
      MRCHeader.getInstance(getPropertyUserDir(), mrcFile.getAbsolutePath(), AXIS_ID);
    try {
      if (!header.read(this)) {
        return false;
      }
