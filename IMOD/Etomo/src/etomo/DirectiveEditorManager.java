...
* <p>Description: </p>
* 
 * <p>Copyright: Copyright 2013 - 2020 by the Regents of the University of Colorado</p>
 * 
...
public final class DirectiveEditorManager extends BaseManager {
  ...
  public DirectiveEditorManager(final DirectiveFileType type,
    ...
    this.metaData = new DirectiveEditorMetaData(this, type, getLogProperties(), true);
    this.type = type;
  ...
  public void openDirectiveEditorDialog() {
    if (dialog == null) {
      ...
      dialogErrmsg =
        builder.build(sourceManager.getBaseMetaData().getAxisType(), dialogErrmsg);
      dialog = DirectiveEditorDialog.getInstance(this, type, builder,
        sourceManager.getBaseMetaData().getAxisType(), sourceManager.getStatus(),
        timestamp, dialogErrmsg);
    }
    mainPanel.showProcess(dialog.getContainer(), AXIS_ID);
    String actionMessage = Utilities
      .prepareDialogActionMessage(DialogType.DIRECTIVE_EDITOR, AxisID.ONLY, null);
  ...
  public boolean closeFrame() {
    if (dialog.isDifferentFromCheckpoint(true)) {
      EtomoBoolean2 response = uiHarness.openYesNoCancelDialog(this,
        metaData.getName() + " has been modified.  Do you want to save you changes?",
        AXIS_ID);
      if (response == null) {
        return false;
      }
  ...
  public boolean saveAsToFile() {
    ...
    if (saveFile.exists()) {
      if (!uiHarness.openYesNoDialog(this,
        "The file " + saveFile.getAbsolutePath() + " already exists.  Overwrite it?",
        AxisID.ONLY)) {
        return false;
      }
      if (!saveFile.isFile()) {
        uiHarness.openMessageDialog(this, "Cannot to write to "
          + saveFile.getAbsolutePath() + " because it is not a file.",
   ...
   *
   */
  private static final class ConflictFileFilter extends javax.swing.filechooser.FileFilter
