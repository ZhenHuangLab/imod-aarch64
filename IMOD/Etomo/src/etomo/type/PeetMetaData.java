...
public class PeetMetaData extends BaseMetaData implements ConstPeetMetaData {
  ...
  private final IntKeyList tiltRangeMultiAxesFile =
    IntKeyList.getStringInstance("TiltRangeMultiAxesFile");
  private final IntKeyList tiltRangeMin =
    IntKeyList.getStringInstance(TILT_RANGE_KEY + "." + START_KEY);
  private final IntKeyList tiltRangeMax =
    IntKeyList.getStringInstance(TILT_RANGE_KEY + "." + END_KEY);
  private final EtomoNumber referenceVolume =
    new EtomoNumber(REFERENCE_KEY + "." + VOLUME_KEY);
  private final EtomoNumber referenceParticle =
    new EtomoNumber(REFERENCE_KEY + "." + PARTICLE_KEY);
   ...
   * @deprecated
   */
  private final EtomoBoolean2 maskUseReferenceParticle =
    new EtomoBoolean2("Mask.UseReferenceParticle");
  /**
   * @deprecated replaced by maskModelPtsZRotation
   * Replacement fields is not equivalent - no backward compatibility.
   */
  private final EtomoNumber maskModelPtsModelNumber =
    new EtomoNumber(MASK_MODEL_PTS_KEY + "." + MODEL_NUMBER_KEY);
  private final EtomoNumber maskModelPtsZRotation =
    new EtomoNumber(MASK_MODEL_PTS_KEY + ".ZRotation");
  /**
   * @deprecated replaced by maskModelPtsYRotation
   * Replacement fields is not equivalent - no backward compatibility.
   */
  private final EtomoNumber maskModelPtsParticle =
    new EtomoNumber(MASK_MODEL_PTS_KEY + "." + PARTICLE_KEY);
  private final EtomoNumber maskModelPtsYRotation =
    new EtomoNumber(MASK_MODEL_PTS_KEY + ".YRotation");
  private final StringProperty maskTypeVolume =
    new StringProperty("MastType." + VOLUME_KEY);
  ...
  private final EtomoBoolean2 manualCylinderOrientation =
    new EtomoBoolean2("MaskType.ManualCylinderOrientation");
  private final EtomoNumber referenceMultiparticleLevel =
    new EtomoNumber(REFERENCE_KEY + ".Multiparticle.level");
  ...
  private final StringProperty browsingDirectory =
    new StringProperty("BrowsingDirectory", true);

  public PeetMetaData(final BaseManager manager, final LogProperties logProperties,
    final boolean newDataset) {
    super(manager, logProperties, true, newDataset, true);
  ...
  public void copy(PeetMetaData input) {
    ...
    browsingDirectory.set(input.browsingDirectory);
  }
  ...
  public void setReferenceVolume(final String referenceVolume) {
    this.referenceVolume.set(referenceVolume);
  }
