...
/**
 * <p>Description: Represents processes of various types.  Only one process with a given
 * name can exist - and case does not count.  Processes with different cases and the same
 * characters will collide.</p>
 * 
 * <p>This class can be inherited.  Use the getExtendedInstance functions to create more
 * ProcessName instances.  Extended instances will be returned via the getInstance
 * functions.</p>
 *
 ...
 * <p> $$ </p>
 */
public class ProcessName implements Status {
  // known process names
  ...
  // Key is lower case version of name.
  private static final Map<String, ProcessName> INSTANCE_MAP =
    new HashMap<String, ProcessName>();
  ...
  private ProcessName(final String name, final boolean resumable) {
    ...
    if (name != null && !INSTANCE_MAP.containsKey(name)) {
      INSTANCE_MAP.put(name.toLowerCase(), this);
    }
  }

  /**
   * If an instance with this name doesn't already exist, constructs a new process name
   * instance, otherwise returns the existing one.  The same name with a different case is
   * not allowed.
   * @param name
   * @return
   */
  protected static ProcessName constructInstance(final String name) {
    ProcessName processName = getInstance(name);
    if (processName != null) {
      // Cannot replace an existing process name instance.
      System.err.println("Warning: Failed attempt to replace a process name:" + name
        + ".  Names that only have a different case will collide.");
      return processName;
    }
    return new ProcessName(name, false);
  }
   ...
   * @return
   */
  private static ProcessName constructInstance(final String name,
    final boolean resumable) {
    ProcessName processName = getInstance(name);
    if (processName != null) {
      // Cannot replace an existing process name instance.
      System.err.println("Warning: Failed attempt to replace an etomo process name:"
        + name + ".  Names that only have a different case will collide.");
      return processName;
    }
    return new ProcessName(name, resumable);
  }
  ...
  public static final ProcessName ANISOTROPIC_DIFFUSION =
    new ProcessName(anisotropicDiffusion);
  ...
  public static final ProcessName TILT_3D_FIND_REPROJECT =
    new ProcessName(tilt_3dfind_reproject);
  ...
  public static final ProcessName CRYO_POSITION = new ProcessName("cryoposition");
  public static final ProcessName FIND_SECTION = new ProcessName("findsection");
  public static final ProcessName EXCLUDE_VIEWS = new ProcessName("excludeviews");
  public static final ProcessName FIND_SECTION_POS = new ProcessName("findsection_pos");
  public static final ProcessName FIND_SECTION_LIM = new ProcessName("findsection_lim");
  public static final ProcessName MULTIFILT_SETUP = new ProcessName("multifiltsetup");
  public static final ProcessName TILT_MULTIFILT = new ProcessName("tilt_mulfil");
  public static final ProcessName CCDERASER = new ProcessName("ccderaser");
  public static final ProcessName JOIN_WARP_2_MODEL = new ProcessName("joinwarp2model");
   ...
   * correct static object.  The string is case insensitive.  Null is returned if the
   * string is not one of the known process names.  All ProcessName instances, including
   * those created in child classes, can be returned.
   * @param name
   */
  public static ProcessName getInstance(String name) {
    ...
    return INSTANCE_MAP.get(name.toLowerCase());
  }
  ...
  public static ProcessName getInstance(final File file, final String excludeString) {
    ...
    if (processStringBuffer.toString().endsWith(AxisID.FIRST.getExtension())) {
      return ProcessName.getInstance(processStringBuffer
        .substring(processStringBuffer.lastIndexOf(AxisID.FIRST.getExtension())));
    }
    if (processStringBuffer.toString().endsWith(AxisID.SECOND.getExtension())) {
      return ProcessName.getInstance(processStringBuffer
        .substring(processStringBuffer.lastIndexOf(AxisID.SECOND.getExtension())));
    }
    return null;
  }
