...
public final class EtomoVersion implements Storable, ConstEtomoVersion {
  ...
  /**
   * Returns true if equal to the parameter.  Ignores extra string.
   * @param version
   * @return
   */
  ...
  /**
   * Returns true if less then the parameter.
   * @param version
   * @return
   */
  ...
  /**
   * Returns true if less then or equal to the parameter.
   * @param version
   * @return
   */
  ...
  /**
   * Returns true if greater then the parameter.
   * @param version
   * @return
   */
  ...
  /**
   * Returns true if greater or equal to the parameter.
   * @param version
   * @return
   */
  ...
  /**
   * Returns the parseable format: v.v...[ ...]
   */
  public String toString() {
    ...
    if (extra != null && !extra.equals("")) {
      return sectionList.toString() + " " + extra;
    }
  ...
  public void store(final Properties props, final String prepend) {
    ...
    if (debug) {
      System.err.println(
        "store:prepend=" + prepend + ",key=" + key + ",toString()=" + toString());
    }
    if (isNull()) {
      if (debug) {
  ...
  /**
   * Parses version, and saves any extra strings.
   * If null resets the value.
   * @param version
   */
  public void set(final String version) {
    ...
    String[] array = version.trim().split("\\s+");
    if (array == null || array.length == 0) {
      return;
    }
    // assume then that the version string comes first
    sectionList.parse(array[0]);
    if (sectionList.isNull()) {
      extra = version;
    }
    else if (array.length > 1 && array[0] != null) {
      extra = version.substring(array[0].length());
      if (extra != null) {
        extra = extra.trim();
      }
    }
  ...
  private static final class SectionList {
    ...
    /**
     * Parses a string into sections.
     * @param string
     * @return
     */
    private void parse(final String string) {
      if (string == null || string.indexOf(DIVIDER) == -1) {
        System.err.println("WARNING: Bad version: " + string);
        return;
      }
      reset();
      // sections are divided by ".".
      String[] array = string.split("\\" + DIVIDER);
      if (array == null || array.length == 0) {
        return;
      }
     ...
     * @return
     */
    public boolean equals(final SectionList sectionList, final int index) {
