...
public abstract class ConstEtomoNumber implements Storable {
  ...
  void setInvalidReason() {
    ...
    // If validValues is not set, validate against validFloor
      ...
      addInvalidReason(toString(currentValue) + " is not a valid value.");
      addInvalidReason(
        "Valid values are greater or equal to " + toString(validFloor) + ".");
    }
  }
  ...
  public static void remove(final String name, final Properties props) {
    props.remove(name);
  }
  ...
  /**
   * Must show the string version of getValue() - not a description!
   */
  ...
  public boolean gt(String value) {
    return gt(getValue(), newNumber(value, new StringBuffer()));
  }
  ...
  public static boolean isNumberNull(Number number) {
    ...
    throw new IllegalStateException(
      "Unknown type.  value.getClass()=" + number.getClass());
  }
  ...
  void internalTestDeepCopy(ConstEtomoNumber original) {
    ...
    // equality test
    if (type != original.type || !name.equals(original.name)
      ...
      || (validValues == null && original.validValues != null)
      || (validValues != null && original.validValues != null
        && !validValues.equals(original.validValues))
      || nullIsValid != original.nullIsValid
      || (invalidReason != null && original.invalidReason == null)
      || (invalidReason == null && original.invalidReason != null)
      || (invalidReason != null && original.invalidReason != null
        && !invalidReason.toString().equals(original.invalidReason.toString()))) {
  ...
  private Number doMath(final Operator operator, final float number1,
    final double number2) {
    if (operator == Operator.ADD) {
      return newNumber(number1 + number2);
    }
  ...
  private Number doMath(final Operator operator, final long number1,
    final double number2) {
    if (operator == Operator.ADD) {
      return newNumber(number1 + number2);
    }
  ...
  public static final class Type {
    ...
    public static Type getType(final Number number) {
      if (number == null) {
        return null;
      }
      if (number instanceof AtomicInteger || number instanceof BigInteger
        || number instanceof Byte || number instanceof Integer
        || number instanceof Short) {
        return Type.INTEGER;
      }
      if (number instanceof AtomicLong || number instanceof Long) {
        return Type.LONG;
      }
      return Type.DOUBLE;
    }
