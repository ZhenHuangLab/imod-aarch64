...
public final class ParsedList {
  ...
  public static final Character OPEN_SYMBOL = new Character('{');
  static final Character CLOSE_SYMBOL = new Character('}');
  public static final Character DIVIDER_SYMBOL = new Character(',');
  ...
  private Token parseElement(Token token, final PrimativeTokenizer tokenizer,
    ...
    if (type == ParsedElementType.STRING) {
      ...
      element =
        ParsedArray.getInstance(type, etomoNumberType, isDebug(), defaultValue, descr);
      token = element.parse(token, tokenizer, lineNum);
    }
    else {
      ...
      ParsedDescriptor descriptor =
        ParsedDescriptor.getInstance(type, etomoNumberType, isDebug(), defaultValue,
          descr);
