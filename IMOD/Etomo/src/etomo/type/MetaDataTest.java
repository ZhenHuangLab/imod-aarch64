...
public class MetaDataTest extends TestCase {
  ...
  private static final String validAFileName = new String(validDatasetName + "a.hdf");
  ...
  protected void setUp() throws Exception {
    ...
    // create test site
    testInst = new MetaData(manager, manager.getLogProperties(), true);
    ...
    assertTrue(
      validFileDir.isDirectory() && validFileDir.canRead() && validFileDir.canWrite());
    // create valid files
  ...
  final public void testIsDatasetNameValid() {
    ...
    String workingDir =
      EtomoDirector.INSTANCE.setCurrentPropertyUserDir(dummyDir.getAbsolutePath());
    testInst.setBackupDirectory(dummyDir2.getAbsolutePath());
    testInst.setDatasetName(validFileName);
    if (!windowsOs) {
      assertFalse(testInst.isDatasetNameValid());
    }
    // Test successes
    testInst.setDatasetName(validAFileName);
  ...
  File createUnreadableFile(File dir, String name) throws IOException {
  ...
  File createValidFile(File dir, String name) throws IOException {
    // create a valid file
    File file = new File(dir, name);
    file.delete();
    file.createNewFile();
