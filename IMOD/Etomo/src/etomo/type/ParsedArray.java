...
public final class ParsedArray extends ParsedElement {
  ...
  private boolean backwardCompatibleNullKey = false;
  ...
  static ParsedArray getInstance(ParsedElementType type, EtomoNumber.Type etomoNumberType,
  ...
  /**
   * A null key was used because the description was set instead of the key.  Create a
   * backwards compatible key set to "null".
   */
  public void setBackwardCompatibleNullKey() {
    backwardCompatibleNullKey = true;
  }
  ...
  public void parse(ReadOnlyAttribute attribute) {
    ...
    PrimativeTokenizer tokenizer =
      createTokenizer(attribute.getValue(), attribute.getLineNum());
    try {
  ...
  private ParsedArrayDescriptor getAddFirstArrayDescriptor(final String addIfNumber,
    ...
    if (descriptor == null) {
      ParsedNumber number =
        ParsedNumber.getInstance(type, etomoNumberType, debug, defaultValue, descr);
  ...
  void load(final Properties props, String prepend) {
    StringProperty property = new StringProperty(key);
    if (backwardCompatibleNullKey) {
      property.setBackwardCompatibleKey("null");
    }
   ...
   */
  public ParsedElementList
    getParsedNumberExpandedArray(ParsedElementList parsedNumberExpandedArray) {
    for (int i = 0; i < array.size(); i++) {
      ParsedElement element = array.get(i);
      if (element != null) {
        parsedNumberExpandedArray =
          element.getParsedNumberExpandedArray(parsedNumberExpandedArray);
      }
    }
    if (parsedNumberExpandedArray == null) {
      parsedNumberExpandedArray =
        new ParsedElementList(type, etomoNumberType, debug, defaultValue, descr);
    }
  ...
  private Token parseElement(Token token, PrimativeTokenizer tokenizer, final int index,
    ...
    if (debug) {
      System.out.println(
        "ParsedArray.parseElement:token=" + token + ",type=" + type + ",index:" + index);
    }
    ParsedElement element;
    // First assume that there might be an array descriptor.
    ParsedDescriptor descriptor =
      ParsedDescriptor.getInstance(type, etomoNumberType, debug, defaultValue, descr);
    if (descriptor != null) {
