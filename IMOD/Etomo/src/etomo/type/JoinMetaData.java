...
public final class JoinMetaData extends BaseMetaData implements ConstJoinMetaData {
   ...
   * @deprecated
   */
  private static final String rotationTranslationMagnificationString =
  ...
  private static final EtomoVersion latestRevisionNumber =
  ...
  private ScriptParameter densityRefSection =
    new ScriptParameter(EtomoNumber.Type.INTEGER, "DensityRefSection");
  /**
   * @deprecated
   */
  private ScriptParameter sigmaLowFrequency =
    new ScriptParameter(EtomoNumber.Type.DOUBLE, "SigmaLowFrequency");
  /**
   * @deprecated
   */
  private ScriptParameter cutoffHighFrequency =
    new ScriptParameter(EtomoNumber.Type.DOUBLE, "CutoffHighFrequency");
  /**
   * @deprecated
   */
  private ScriptParameter sigmaHighFrequency =
    new ScriptParameter(EtomoNumber.Type.DOUBLE, "SigmaHighFrequency");
  /**
   * @deprecated
   */
  ...
  private ScriptParameter alignmentRefSection =
    new ScriptParameter(EtomoNumber.Type.INTEGER, "AlignmentRefSection");
  private ScriptParameter sizeInX =
    new ScriptParameter(EtomoNumber.Type.INTEGER, "SizeInX");
  private ScriptParameter sizeInY =
    new ScriptParameter(EtomoNumber.Type.INTEGER, "SizeInY");
  private ScriptParameter shiftInX =
    new ScriptParameter(EtomoNumber.Type.INTEGER, "ShiftInX");
  private ScriptParameter shiftInY =
    new ScriptParameter(EtomoNumber.Type.INTEGER, "ShiftInY");
  // FinishJoin -local. Checkbox in Join tab.
  private final EtomoBoolean2 localFits = new EtomoBoolean2("LocalFits");
  private EtomoNumber useEveryNSlices =
    new EtomoNumber(EtomoNumber.Type.INTEGER, "UseEveryNSlices");
  private final ScriptParameter trialBinning =
    new ScriptParameter(EtomoNumber.Type.INTEGER, "TrialBinning");
  private final ScriptParameter rejoinTrialBinning =
    new ScriptParameter(EtomoNumber.Type.INTEGER, "RejoinTrialBinning");
  ...
  private final IntKeyList boundaryRowStartList =
    IntKeyList.getNumberInstance("BoundaryRow" + '.' + "StartList");
  private final IntKeyList boundaryRowEndList =
    IntKeyList.getNumberInstance("BoundaryRow" + '.' + "EndList");
  private EtomoNumber rejoinUseEveryNSlices =
    new EtomoNumber(EtomoNumber.Type.INTEGER, "RejoinUseEveryNSlices");
  ...
  public void load(Properties props, String prepend) {
    ...
    if (revisionNumber.le("1.1")) {
      ...
      if (revisionNumber.le("1.0")) {
        ...
        autoAlignmentMetaData.setAlignTransform(
          Transform.load(props, prepend, ALIGN_TRANFORM_KEY, Transform.DEFAULT));
      }
    }
    modelTransform =
      Transform.load(props, prepend, MODEL_TRANFORM_KEY, Transform.DEFAULT);
    ...
    useAlignmentRefSection =
      Boolean.valueOf(props.getProperty(group + useAlignmentRefSectionString, "false"))
        .booleanValue();
    ...
    int sectionTableRowsSize =
      Integer.parseInt(props.getProperty(group + sectionTableDataSizeString, "-1"));
    ...
    for (int i = 0; i < sectionTableRowsSize; i++) {
      ...
      if (rowIndex < 0) {
        UIHarness.INSTANCE.openMessageDialog(manager, "Invalid row index: " + rowIndex
          + ".  Corrupted: " + DataFileType.JOIN.extension + " file.", "Corrupted File",
          AxisID.ONLY);
      }
  ...
  private void loadVersion1_0(Properties props, String prepend) {
    String group = prepend + '.';
    if (Boolean
      .valueOf(props.getProperty(group + fullLinearTransformationString, "false"))
      .booleanValue()) {
      alignTransform = Transform.FULL_LINEAR_TRANSFORMATION;
    }
    else if (Boolean
      .valueOf(props.getProperty(group + rotationTranslationMagnificationString, "false"))
      .booleanValue()) {
      alignTransform = Transform.ROTATION_TRANSLATION_MAGNIFICATION;
    }
    else if (Boolean
      .valueOf(props.getProperty(group + rotationTranslationString, "false"))
      .booleanValue()) {
      alignTransform = Transform.FULL_LINEAR_TRANSFORMATION;
    }
    else {
