...
[[ifnot = build]]
	...
	assert.exists.image-file=%{dataset}%{axis}.ali
...
assert.exists.image-file=%{dataset}%{axis}.ali~
# run 3dmod
if.not.var.clear-view.bn.view-full-aligned-stack=
# check values
...
[[if = test-gui]]
	...
	assert.tf.expected-defocus=5.0
	mb.ctf-correction=A
...
[[if = test-gui]]
	...
	popup.correct-ctf-output-missing=OK
	# check values
...
assert.exists.image-file=%{dataset}%{axis}_ctfcorr.ali
[[if = test-gui]]
  #check unbinned pixel size
  tb.create=
  popup.entry-warning=OK
  sp.aligned-image-stack-binning=1
  bn.create-full-aligned-stack=
  if.not.var.montage.wait.process.creating-aligned-stack=done
  if.var.montage.wait.process.full-alignment=done
...
assert.exists.image-file= %{dataset}%{axis}_ctfcorr.ali~
# run 3dmod
if.not.var.clear-view.bn.view-ctf-correction=
#check use warning
tb.create.2=
popup.entry-warning=OK
tb.create.1=
...
bn.use-ctf-correction=
# check files
assert.not-exists.image-file=%{dataset}%{axis}_ctfcorr.ali
# check values
...
assert.exists.file=%{dataset}%{axis}_erase.fid~
# run 3dmod
if.not.var.clear-view.bn.view-transformed-model=
# Run Findbeads3d
rb.use-findbeads3d=
...
assert.exists.file=%{dataset}%{axis}_erase.fid~
# run 3dmod
if.not.var.clear-view.bn.view-2d-model-on-aligned-stack=
# check values
...
[[if = test-gui]]
	...
	assert.cb.parallel-processing=on
	assert.tf.additional-unbinned-diameters-to-add=5
...
assert.exists.image-file=%{dataset}%{axis}_3dfind.ali
assert.exists.image-file=%{dataset}%{axis}_3dfind.rec
assert.exists.file=tilt_3dfind_reproject%{axis}.com
...
assert.exists.file=%{dataset}%{axis}_3dfind.mod~
# run 3dmod
if.not.var.clear-view.bn.view-3d-model-on-tomogram=
# check values
...
[[if = test-gui]]
	# check uninitialized errors
	bn.use-erased-stack=
	popup.erase-beads-output-missing=OK
	# check values
...
# check files
assert.exists.image-file=%{dataset}%{axis}_erase.ali
...
assert.exists.image-file=%{dataset}%{axis}_erase.ali~
# run 3dmod
if.not.var.clear-view.bn.view-erased-stack=
#check use warning
tb.create.3=
popup.entry-warning=OK
tb.create.2=
...
# use file
bn.use-erased-stack=
assert.not-exists.image-file=%{dataset}%{axis}_erase.ali
# check values
...
[[if = test-gui]]
  ...
  mb.2d-filtering=B
	...
	popup.filtered-full-aligned-stack-missing=OK
	# Basic
	# check values
	mb.2d-filtering=B
	assert.tf.low-pass=0.35,0.05
	assert.cb.apply-dose-weighting=off
	assert.cb.no-dose-file=off
	assert.tf.no-dose-file=
	assert.rb.metadata-in-mdoc-file=on
	assert.rb.dose-for-each-image=off
	assert.rb.prior-dose-and-dose-of-image=off
	assert.rb.cumulative-dose-before-and-after-image=off
	# Put this back for Bug# 2243
	#assert.contains.tf.dose-information-file=%{dataset}%{axis}.st.mdoc
	assert.cb.microscope-voltage-is-200-kv=off
	assert.bn.filter=off
  assert.bn.use-filtered-stack=off
	# check enabled/disabled
	assert.enabled.tf.low-pass=
  assert.enabled.cb.apply-dose-weighting=
  assert.disabled.cb.no-dose-file=
  assert.disabled.tf.no-dose-file=
  assert.disabled.rb.metadata-in-mdoc-file=
  assert.disabled.rb.prior-dose-and-dose-of-image=
  assert.disabled.rb.cumulative-dose-before-and-after-image=
  assert.disabled.tf.dose-information-file=
  assert.disabled.bn.dose-information-file-select-file=
  assert.disabled.cb.microscope-voltage-is-200-kv=
	assert.enabled.bn.filter=
	assert.enabled.bn.view-filtered-stack=
	assert.enabled.bn.use-filtered-stack=
	# check behavior
	# cb.apply-dose-weighting
	cb.apply-dose-weighting=on
	assert.disabled.tf.low-pass=
	assert.enabled.cb.no-dose-file=
  assert.disabled.tf.no-dose-file=
  assert.enabled.rb.metadata-in-mdoc-file=
  assert.enabled.rb.prior-dose-and-dose-of-image=
  assert.enabled.rb.cumulative-dose-before-and-after-image=
  assert.enabled.cb.microscope-voltage-is-200-kv=
  # cb.no-dose-file
  cb.no-dose-file=on
  assert.enabled.tf.no-dose-file=
  assert.disabled.rb.metadata-in-mdoc-file=
  assert.disabled.rb.prior-dose-and-dose-of-image=
  assert.disabled.rb.cumulative-dose-before-and-after-image=
  cb.no-dose-file=off
  assert.disabled.tf.dose-information-file=
  assert.enabled.bn.dose-information-file-select-file=
  run.function.reset-2d-filter=  
	# Advanced
	# check values
	mb.2d-filtering=A
  assert.tf.mtf-file=
  ...
  assert.tf.optimal-dose-scaling=
  assert.tf.#-of-views-in-first-half-of-bidirectional-series=
	# check enabled/disabled
  assert.tf.optimal-dose-scaling=
  assert.enabled.tf.mtf-file=
  assert.enabled.tf.maximum-inverse=
  assert.enabled.tf.rolloff=
  assert.disabled.tf.optimal-dose-scaling=
  assert.disabled.tf.#-of-views-in-first-half-of-bidirectional-series=
  # check behavior
  # cb.apply-dose-weighting
  cb.apply-dose-weighting=on
  assert.disabled.tf.mtf-file=
  assert.disabled.tf.maximum-inverse=
  assert.disabled.tf.rolloff=
  assert.enabled.tf.optimal-dose-scaling=
  assert.enabled.tf.#-of-views-in-first-half-of-bidirectional-series=
...
assert.exists.image-file=%{dataset}%{axis}_filt.ali
# run again
bn.filter =
wait.process.running-mtf-filter=done
# check files
...
assert.exists.image-file=%{dataset}%{axis}_filt.ali~
# run 3dmod
if.not.var.clear-view.bn.view-filtered-stack=
#check use warning
tb.create.2=
popup.entry-warning=OK
tb.create.3=
...
bn.use-filtered-stack=
# check files
assert.not-exists.image-file=%{dataset}%{axis}_filt.ali


[function = reset-2d-filter]
cb.apply-dose-weighting=on
cb.no-dose-file=off
rb.metadata-in-mdoc-file=
cb.apply-dose-weighting=off
mb.2d-filtering=B
