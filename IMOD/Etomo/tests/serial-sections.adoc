...
[[if = montage]]
  copy.file = %{dataset}.st.mdoc|%{dataset}.rec.mdoc
[[]]
...
[[if = test-gui]]
  ...
  assert.cb.extract-montage-piece-list-from-mdoc-file=off
  assert.tf.image-distortion-field-file=
  assert.sp.binning=1
  #test enabled
  assert.disabled.tf.stack=
  assert.enabled.rb.single-frame=
  assert.enabled.rb.montage=
  assert.disabled.cb.extract-montage-piece-list-from-mdoc-file=
  ...
  bn.ok=
  popup.invalid-dataset-directory=OK
[[]]
bn.stack=
file-chooser.stack=%{dataset}.rec
[[if = test-gui]]
  #test wrong view type
  if.not.var.montage.rb.montage=
  bn.ok=
  popup.incorrect-frame-type=OK
  if.not.var.montage.rb.single-frame=
[[]]
#test montage-only behavior
[[if = test-gui]]
  if.not.var.montage.return=
  rb.montage=
  assert.enabled.cb.extract-montage-piece-list-from-mdoc-file=
  rb.single-frame=
[[]]
...
[[if = test-gui]]
  #test values
  assert.cb.use-existing-edge-displacement-file=off
  ...
  #test behavior
  cb.use-existing-edge-displacement-file=on
  assert.disabled.cb.treat-as-very-sloppy-montage=
  cb.use-existing-edge-displacement-file=off
[[]]
cb.treat-as-very-sloppy-montage=on
if.not.var.clear-view.bn.open-raw-stack=
[[if = test-gui]]
  #field validation
...
[[if = test-gui]]
  #test values
  assert.cb.use-existing-edge-displacement-file=on
  #test enabled
  assert.disabled.cb.treat-as-very-sloppy-montage=
  #test files
  assert.exists.image-file=%{dataset}_preblend.mrc
  ...
  assert.contains.file=preblend.com|$blendmont -StandardInput
  assert.contains.file=preblend.com|ImageInputFile*mediumscansb2.rec
  assert.contains.file=preblend.com|PieceListInput*mediumscansb2.pl
  assert.contains.file=preblend.com|ImageOutputFile*mediumscansb2_preblend.
  assert.contains.file=preblend.com|RootNameForEdges*mediumscansb2
  assert.contains.file=preblend.com|ReadInXcorrs*0
  assert.contains.file=preblend.com|OldEdgeFunctions*0
  assert.contains.file=preblend.com|AdjustOrigin
  assert.contains.file=preblend.com|VerySloppyMontage
  #test tab warning
  touch.file=%{dataset}.ecd
  tb.initial-blend.1=
  popup.dataset-consistency-warning=OK
  tb.initial-blend=
[[]]
#with "use-existing-edge-displacement-file" on it uses the existing .ecd file and does not #remake xef and yef
bn.make-blended-stack=
wait.process.initial-blend=done
[[if = test-gui]]
  #test tab warning
...
run.function.check-image-files=
bn.make-aligned-stack=
wait.process.%{descr}=done
[[if = test-gui]]
  assert.exists.file=%{dataset}.xg~
  assert.exists.image-file=%{dataset}_ali.mrc~
  assert.exists.file=%{cmd}.log~
[[]]
if.not.var.clear-view.bn.open-aligned-stack=

[function = check-image-files]
if.not.var.test-gui.return=
assert.exists.file=%{dataset}.xg
assert.exists.image-file=%{dataset}_ali.mrc
assert.exists.file=%{cmd}.log
#assert.same.file=%{cmd}.com
assert.contains.file=%{cmd}.com|%{comline}
if.not.var.montage.return=
assert.contains.file=%{cmd}.com|%{comline2}