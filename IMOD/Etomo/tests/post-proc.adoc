...
[[if = test-gui]]
	#check files
	assert.exists.image-file=%{dataset}.rec
	bn.trim-volume=
	wait.process.trimming-volume = done
	assert.exists.image-file=%{dataset}.rec~
	assert.exists.image-file=%{dataset}.rec
	#run 3dmod
	if.not.var.clear-view.bn.3dmod-trimmed-volume=
...
[[]]
#run 3dmod
if.not.var.clear-view.bn.make-surface-model=
[[if = test-values]]
...
[[if = test-values]]
	...
	bn.run-flattenwarp=
	popup.flattenwarp-terminated=OK
[[]]
run.function.copy=
...
assert.exists.image-file=%{dataset}_flat.rec
#check values
assert.bn.flatten=on
#run 3dmod
if.not.var.clear-view.bn.open-flattened-tomogram=
...
wait.process.running-flattenwarp=done
if.not.var.clear-view.bn.open-assessment-in-3dmod=
...
[[if = test-gui]]
	assert.exists.image-file=%{dataset}.sqz
	if.not.var.clear-view.bn.open-squeezed-volume-in-3dmod=
[[]]
