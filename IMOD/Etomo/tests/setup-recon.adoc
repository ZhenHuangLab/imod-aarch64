...
[[if = test-gui]]
	...
	assert.cb.remove-excluded-views=off
	assert.cb.delete-original-files=off
	#check enabled/disabled
	assert.disabled.tf.raw-image-stack=
	assert.enabled.bn.raw-image-stack-select-file
	...
	assert.enabled.cb.remove-excluded-views=
  assert.disabled.cb.delete-original-files=
	#check error
	bn.scan-header=
	popup.missing-raw-image-stack=OK
	bn.view-raw-image-stack.1=
	popup.raw-image-stack=OK
	run.function.axis-a=
	run.function.axis-b=
[[]]
#fill in data
[[if = single]]
  set.file.tf.raw-image-stack=%{dataset}.st
[[]]
[[if = dual]]
...
[[if = test-gui]]
  ...
  cb.remove-excluded-views=on
  assert.enabled.cb.delete-original-files=
  cb.remove-excluded-views=off
[[]]
...
[[if = montage]]
  ...
  bn.scan-header=
  popup.pixel-size-is-missing=OK
[[]]
[[if = montage]]
...
[[if = test-gui]]
	#run 3dmod
	if.not.var.clear-view.bn.view-raw-image-stack=
...
assert.tf.series-was-bidirectional-from=0.0
...
assert.tf.series-was-bidirectional-from.1=0.0
