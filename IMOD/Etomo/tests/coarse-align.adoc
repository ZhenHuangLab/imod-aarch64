...
[[if = single-frame-test-gui]]
	assert.exists.file=prenewst%{axis}.log
[[]]
#fidless
if.var.fidless.cb.coarse-alignment-only=
#ending gui test
[[if = test-gui]]
  if.not.var.clear-view.bn.view-aligned-stack-in-3dmod=
  run.function.test2=
  assert.contains.file=xcorr%{axis}.com|$tiltxcorr*-StandardInput
  assert.contains.file=xcorr%{axis}.com|InputFile*%{dataset}%{axis}.st
  assert.contains.file=xcorr%{axis}.com|OutputFile*%{dataset}%{axis}.prexf
  assert.contains.file=xcorr%{axis}.com|TiltFile*%{dataset}%{axis}.rawtlt
[[]]
bn.done=
...
assert.cb.coarse-alignment-only=off
...
# thread problem
#bn.generate-coarse-aligned-stack =
#popup.prenewst%{axis}com-terminated = OK
#wait.process = failed
mb.tiltxcorr=-
