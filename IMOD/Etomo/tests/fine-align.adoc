...
[[if = test-gui]]
  # run 3dmod
  if.not.var.clear-view.bn.view-3d-model=
  if.not.var.clear-view.bn.view-residual-vectors=
  if.not.var.clear-view.bn.view/edit-fiducial-model=
[[]]
run.function.copy=
...
if.var.patch-tracking.assert.cb.find-weights-for-contours,-not-points=on
if.not.var.patch-tracking.assert.cb.find-weights-for-contours,-not-points=off
...
assert.disabled.tf.fixed-beam-tilt=
rb.no-beam-tilt=
mb.single-variables=B


[function = restrict-alignment-variables-test-gui]
#values
#Bug# 2316 - this value has changed - no idea why
