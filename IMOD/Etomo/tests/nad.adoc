...
#Extract test volume.
bn.pick-a-volume=
file-chooser.open=%{dataset}1.rec
if.not.var.clear-view.bn.view-full-volume=
tf.x-min=286
tf.x-max=400
tf.y-min=274
tf.y-max=389
...
assert.exists.image-file=%{subdir}/test.input
if.not.var.clear-view.bn.view-test-volume=
[[if = test-gui]]
...
if.not.var.clear-view.bn.view-different-k-values-test-results=
#Find iteration.
tf.k-value=10
#Test bad iteration
tf.list-of-iterations=10 20 30 100
...
if.not.var.clear-view.bn.view-different-iteration-test-results=
run.function.filter-full-volume=
# Test input file names
[[if = test-gui]]
  # Input files are not changed by standardization.
  assert.exists.image-file=%{dataset}1.rec.nad
[[]]
...
if.not.var.clear-view.bn.view-filtered-volume=
